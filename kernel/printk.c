...
id|LOG_BUF_LEN
suffix:semicolon
multiline_comment|/* The logged_chars, log_start, and log_size are serialized&n;&t;&t;   by the console_lock (the console_lock can be acquired also&n;&t;&t;   from irqs by printk). */
id|spin_lock_irq
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* While writing data to the userspace buffer printk may&n;&t;&t;   trash our information but the _only_ thing we care is to&n;&t;&t;   have a coherent `j&squot; value. */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/* the increment done in printk may undo our&n;&t;&t;&t;   not atomic assigment if we do it without the&n;&t;&t;&t;   console lock held. */
id|spin_lock_irq
c_func
(paren
op_amp
