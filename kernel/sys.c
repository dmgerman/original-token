...
suffix:semicolon
multiline_comment|/*&n; * Reboot system call: for obvious reasons only root may call it,&n; * and even root needs to set up some magic numbers in the registers&n; * so that some mistake won&squot;t make this reboot the whole machine.&n; * You can also set the meaning of the ctrl-alt-del-key here.&n; *&n; * reboot doesn&squot;t sync: do that yourself before calling this.&n; *&n; */
...
(brace
multiline_comment|/* What if a process setreuid()&squot;s and this brings the&n;&t;&t; * new uid over his NPROC rlimit?  We can check this now&n;&t;&t; * cheaply with the new uid cache, so if it matters&n;&t;&t; * we should be checking for it.  -DaveM&n;&t;&t; */
id|charge_uid
c_func
(paren
id|current
...
(brace
multiline_comment|/* See comment above about NPROC rlimit issues... */
id|charge_uid
c_func
(paren
id|current
...
(brace
multiline_comment|/* See above commentary about NPROC rlimit issues here. */
id|charge_uid
c_func
(paren
id|current
...
(brace
multiline_comment|/* ?? use copy_to_user() */
r_if
c_cond
(paren
op_logical_neg
