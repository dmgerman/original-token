multiline_comment|/*&n; * bios32.c - BIOS32, PCI BIOS functions.&n; *&n; * Sponsored by &n; *&t;iX Multiuser Multitasking Magazine&n; *&t;Hannover, Germany&n; *&t;hm@ix.de&n; *&n; * Copyright 1993, 1994 Drew Eckhardt&n; *      Visionary Computing &n; *      (Unix and Linux consulting and custom programming)&n; *      Drew@Colorado.EDU&n; *      +1 (303) 786-7975&n; *&n; * For more information, please consult &n; * &n; * PCI BIOS Specification Revision&n; * PCI Local Bus Specification&n; * PCI System Design Guide&n; *&n; * PCI Special Interest Group&n; * M/S HF3-15A&n; * 5200 N.E. Elam Young Parkway&n; * Hillsboro, Oregon 97124-6497&n; * +1 (503) 696-2000 &n; * +1 (800) 433-5177&n; * &n; * Manuals are $25 each or $50 for all three, plus $7 shipping &n; * within the United States, $35 abroad.&n; *&n; *&n; * CHANGELOG : &n; * Jun 17, 1994 : Modified to accomodate the broken pre-PCI BIOS SPECIFICATION&n; *&t;Revision 2.0 present on &lt;thys@dennis.ee.up.ac.za&gt;&squot;s ASUS mainboard.&n; */
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/segment.h&gt;
macro_line|#include &lt;linux/bios32.h&gt;
macro_line|#include &lt;linux/pci.h&gt;
DECL|macro|PCIBIOS_PCI_FUNCTION_ID
mdefine_line|#define PCIBIOS_PCI_FUNCTION_ID &t;0xb1
DECL|macro|PCIBIOS_PCI_BIOS_PRESENT
mdefine_line|#define PCIBIOS_PCI_BIOS_PRESENT &t;0x01
DECL|macro|PCIBIOS_FIND_PCI_DEVICE
mdefine_line|#define PCIBIOS_FIND_PCI_DEVICE&t;&t;0x02
DECL|macro|PCIBIOS_FIND_PCI_CLASS_CODE
mdefine_line|#define PCIBIOS_FIND_PCI_CLASS_CODE&t;0x03
DECL|macro|PCIBIOS_GENERATE_SPECIAL_CYCLE
mdefine_line|#define PCIBIOS_GENERATE_SPECIAL_CYCLE&t;0x06
DECL|macro|PCIBIOS_READ_CONFIG_BYTE
mdefine_line|#define PCIBIOS_READ_CONFIG_BYTE&t;0x08&t;&t;
DECL|macro|PCIBIOS_READ_CONFIG_WORD
mdefine_line|#define PCIBIOS_READ_CONFIG_WORD&t;0x09
DECL|macro|PCIBIOS_READ_CONFIG_DWORD
mdefine_line|#define PCIBIOS_READ_CONFIG_DWORD&t;0x0a
DECL|macro|PCIBIOS_WRITE_CONFIG_BYTE
mdefine_line|#define PCIBIOS_WRITE_CONFIG_BYTE&t;0x0b
DECL|macro|PCIBIOS_WRITE_CONFIG_WORD
mdefine_line|#define PCIBIOS_WRITE_CONFIG_WORD&t;0x0c
DECL|macro|PCIBIOS_WRITE_CONFIG_DWORD
mdefine_line|#define PCIBIOS_WRITE_CONFIG_DWORD&t;0x0d
DECL|union|signature
r_union
id|signature
(brace
DECL|member|chars
r_char
id|chars
(braket
l_int|4
)braket
suffix:semicolon
DECL|member|scalar
r_int
r_int
id|scalar
suffix:semicolon
)brace
suffix:semicolon
multiline_comment|/*&n; * This is the standard structure used to identify the entry point &n; * to the BIOS32 Service Directory, as documented in &n; * &t;Standard BIOS 32-bit Service Directory Proposal&n; * &t;Revision 0.4 May 24, 1993&n; * &t;Phoenix Technologies Ltd.&n; *&t;Norwood, MA&n; * and the PCI BIOS specfication.&n; */
DECL|variable|bios32_signature
r_static
r_union
id|signature
id|bios32_signature
op_assign
(brace
l_string|&quot;_32_&quot;
comma
)brace
suffix:semicolon
DECL|union|bios32
r_union
id|bios32
(brace
r_struct
(brace
DECL|member|signature
r_union
id|signature
id|signature
suffix:semicolon
multiline_comment|/* _32_ */
DECL|member|entry
r_int
id|entry
suffix:semicolon
multiline_comment|/* 32 bit physical address */
DECL|member|revision
r_int
r_char
id|revision
suffix:semicolon
multiline_comment|/* Revision level, 0 */
DECL|member|length
r_int
r_char
id|length
suffix:semicolon
multiline_comment|/* Length in paragraphs should be 01 */
DECL|member|checksum
r_int
r_char
id|checksum
suffix:semicolon
multiline_comment|/* All bytes must add up to zero */
DECL|member|reserved
r_int
r_char
id|reserved
(braket
l_int|5
)braket
suffix:semicolon
multiline_comment|/* Must be zero */
DECL|member|fields
)brace
id|fields
suffix:semicolon
DECL|member|chars
r_char
id|chars
(braket
l_int|16
)braket
suffix:semicolon
)brace
suffix:semicolon
multiline_comment|/* &n; * Physical address of the service directory.  I don&squot;t know if we&squot;re&n; * allowed to have more than one of these or not, so just in case &n; * we&squot;ll make bios32_init() take a memory start parameter and store&n; * the array there.&n; */
DECL|variable|bios32_entry
r_static
r_int
id|bios32_entry
op_assign
l_int|0
suffix:semicolon
DECL|variable|bios32_indirect
r_static
r_int
r_char
id|bios32_indirect
(braket
l_int|6
)braket
suffix:semicolon
r_static
r_int
id|pcibios_init
(paren
r_int
comma
r_int
)paren
suffix:semicolon
DECL|function|bios32_init
r_int
id|bios32_init
(paren
r_int
id|memory_start
comma
r_int
id|memory_end
)paren
(brace
r_union
id|bios32
op_star
id|check
suffix:semicolon
r_int
r_char
id|sum
suffix:semicolon
r_int
id|i
comma
id|length
suffix:semicolon
multiline_comment|/*&n;     * Follow the standard procedure for locating the BIOS32 Serivce&n;     * directory by scanning the permissable address range from &n;     * 0xe0000 through 0xfffff for a valid BIOS32 structure.&n;     */
r_for
c_loop
(paren
id|check
op_assign
(paren
r_union
id|bios32
op_star
)paren
l_int|0xe0000
suffix:semicolon
id|check
op_le
(paren
r_union
id|bios32
op_star
)paren
l_int|0xffff0
suffix:semicolon
op_increment
id|check
)paren
(brace
r_if
c_cond
(paren
id|check-&gt;fields.signature.scalar
op_eq
id|bios32_signature.scalar
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
id|sum
op_assign
l_int|0
comma
id|length
op_assign
id|check-&gt;fields.length
op_star
l_int|16
suffix:semicolon
id|i
OL
id|check-&gt;fields.length
op_star
l_int|16
suffix:semicolon
op_increment
id|i
)paren
id|sum
op_add_assign
id|check-&gt;chars
(braket
id|i
)braket
suffix:semicolon
r_if
c_cond
(paren
id|sum
op_ne
l_int|0
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|check-&gt;fields.revision
op_ne
l_int|0
)paren
(brace
id|printk
(paren
l_string|&quot;bios32_init : unsupported revision %d at 0x%x, mail drew@colorado.edu&bslash;n&quot;
comma
id|check-&gt;fields.revision
comma
(paren
r_int
)paren
id|check
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
id|printk
(paren
l_string|&quot;bios32_init : BIOS32 Service Directory structure at 0x%x&bslash;n&quot;
comma
(paren
r_int
)paren
id|check
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bios32_entry
)paren
(brace
op_star
(paren
(paren
r_int
op_star
)paren
id|bios32_indirect
)paren
op_assign
id|bios32_entry
op_assign
id|check-&gt;fields.entry
suffix:semicolon
op_star
(paren
(paren
r_int
op_star
)paren
(paren
id|bios32_indirect
op_plus
l_int|4
)paren
)paren
op_assign
id|KERNEL_CS
suffix:semicolon
id|printk
(paren
l_string|&quot;bios32_init : BIOS32 Service Directory entry at 0x%x&bslash;n&quot;
comma
(paren
r_int
)paren
id|bios32_entry
)paren
suffix:semicolon
)brace
r_else
id|printk
(paren
l_string|&quot;bios32_init : multiple entries, mail drew@colorado.edu&bslash;n&quot;
)paren
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|bios32_entry
)paren
(brace
id|memory_start
op_assign
id|pcibios_init
(paren
id|memory_start
comma
id|memory_end
)paren
suffix:semicolon
)brace
r_return
id|memory_start
suffix:semicolon
)brace
multiline_comment|/* &n; * Returns the entry point for the given service, NULL on error &n; */
DECL|function|bios32_service
r_static
r_int
id|bios32_service
(paren
r_int
id|service
)paren
(brace
r_int
r_char
id|return_code
suffix:semicolon
multiline_comment|/* %al */
r_int
id|address
suffix:semicolon
multiline_comment|/* %ebx */
r_int
id|length
suffix:semicolon
multiline_comment|/* %ecx */
r_int
id|entry
suffix:semicolon
multiline_comment|/* %edx */
id|__asm__
(paren
"&quot;"
id|movl
"$"
l_int|0
comma
op_mod
op_mod
id|ebx
id|lcall
(paren
op_mod
op_mod
id|edi
)paren
"&quot;"
suffix:colon
l_string|&quot;=al&quot;
(paren
id|return_code
)paren
comma
l_string|&quot;=ebx&quot;
(paren
id|address
)paren
comma
l_string|&quot;=ecx&quot;
(paren
id|length
)paren
comma
l_string|&quot;=edx&quot;
(paren
id|entry
)paren
suffix:colon
l_string|&quot;eax&quot;
(paren
id|service
)paren
comma
l_string|&quot;D&quot;
(paren
id|bios32_indirect
)paren
suffix:colon
l_string|&quot;ebx&quot;
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|return_code
)paren
(brace
r_case
l_int|0
suffix:colon
r_return
id|address
op_plus
id|entry
suffix:semicolon
r_case
l_int|0x80
suffix:colon
multiline_comment|/* Not present */
id|printk
(paren
l_string|&quot;bios32_service(%ld) : not present&bslash;n&quot;
comma
id|service
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
r_default
suffix:colon
multiline_comment|/* Shouldn&squot;t happen */
id|printk
(paren
l_string|&quot;bios32_service(%ld) : returned 0x%x, mail drew@colorado.edu&bslash;n&quot;
comma
id|service
comma
(paren
r_int
)paren
id|return_code
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
)brace
DECL|variable|pci_signature
r_static
r_union
id|signature
id|pci_signature
op_assign
(brace
l_string|&quot;PCI &quot;
comma
)brace
suffix:semicolon
DECL|variable|pci_service
r_static
r_union
id|signature
id|pci_service
op_assign
(brace
l_string|&quot;$PCI&quot;
comma
)brace
suffix:semicolon
DECL|variable|pcibios_entry
r_static
r_int
id|pcibios_entry
op_assign
l_int|0
suffix:semicolon
DECL|variable|pci_indirect
r_static
r_int
r_char
id|pci_indirect
(braket
l_int|6
)braket
suffix:semicolon
r_void
id|NCR53c810_test
c_func
(paren
r_void
)paren
suffix:semicolon
DECL|function|pcibios_init
r_static
r_int
id|pcibios_init
(paren
r_int
id|memory_start
comma
r_int
id|memory_end
)paren
(brace
r_union
id|signature
id|signature
suffix:semicolon
r_int
r_char
id|present_status
suffix:semicolon
r_int
r_char
id|major_revision
suffix:semicolon
r_int
r_char
id|minor_revision
suffix:semicolon
r_int
id|pack
suffix:semicolon
r_if
c_cond
(paren
(paren
id|pcibios_entry
op_assign
id|bios32_service
(paren
id|pci_service.scalar
)paren
)paren
)paren
(brace
op_star
(paren
(paren
r_int
op_star
)paren
id|pci_indirect
)paren
op_assign
id|pcibios_entry
suffix:semicolon
op_star
(paren
(paren
r_int
op_star
)paren
(paren
id|pci_indirect
op_plus
l_int|4
)paren
)paren
op_assign
id|KERNEL_CS
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb101
comma
op_mod
op_mod
id|eax
id|lcall
(paren
op_mod
op_mod
id|edi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
id|shl
"$"
l_int|8
comma
op_mod
op_mod
id|eax
suffix:semicolon
id|movw
op_mod
op_mod
id|bx
comma
op_mod
op_mod
id|ax
suffix:semicolon
"&quot;"
suffix:colon
l_string|&quot;=edx&quot;
(paren
id|signature.scalar
)paren
comma
l_string|&quot;=eax&quot;
(paren
id|pack
)paren
suffix:colon
l_string|&quot;D&quot;
(paren
id|pci_indirect
)paren
suffix:colon
l_string|&quot;cx&quot;
)paren
suffix:semicolon
id|present_status
op_assign
(paren
id|pack
op_rshift
l_int|16
)paren
op_amp
l_int|0xff
suffix:semicolon
id|major_revision
op_assign
(paren
id|pack
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
id|minor_revision
op_assign
id|pack
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
id|present_status
op_logical_or
(paren
id|signature.scalar
op_ne
id|pci_signature.scalar
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;pcibios_init : %s : BIOS32 Service Directory says PCI BIOS is present,&bslash;n&quot;
l_string|&quot;    but PCI_BIOS_PRESENT subfunction fails with present status of 0x%x&bslash;n&quot;
l_string|&quot;    and signature of 0x%08lx (%c%c%c%c).  mail drew@Colorado.EDU&bslash;n&quot;
comma
(paren
id|signature.scalar
op_eq
id|pci_signature.scalar
)paren
ques
c_cond
l_string|&quot;WARNING&quot;
suffix:colon
l_string|&quot;ERROR&quot;
comma
(paren
r_int
)paren
id|present_status
comma
id|signature.scalar
comma
id|signature.chars
(braket
l_int|0
)braket
comma
id|signature.chars
(braket
l_int|1
)braket
comma
id|signature.chars
(braket
l_int|2
)braket
comma
id|signature.chars
(braket
l_int|3
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
id|signature.scalar
op_ne
id|pci_signature.scalar
)paren
id|pcibios_entry
op_assign
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|pcibios_entry
)paren
(brace
id|printk
(paren
l_string|&quot;pcibios_init : PCI BIOS revision %x.%02x entry at 0x%lx&bslash;n&quot;
comma
(paren
r_int
)paren
id|major_revision
comma
(paren
r_int
)paren
id|minor_revision
comma
id|pcibios_entry
)paren
suffix:semicolon
)brace
)brace
macro_line|#if 0
id|NCR53c810_test
c_func
(paren
)paren
suffix:semicolon
macro_line|#endif
r_return
id|memory_start
suffix:semicolon
)brace
DECL|function|pcibios_present
r_int
id|pcibios_present
(paren
r_void
)paren
(brace
r_return
id|pcibios_entry
ques
c_cond
l_int|1
suffix:colon
l_int|0
suffix:semicolon
)brace
DECL|function|pcibios_find_class_code
r_int
id|pcibios_find_class_code
(paren
r_int
r_int
id|class_code
comma
r_int
r_int
id|index
comma
r_int
r_char
op_star
id|bus
comma
r_int
r_char
op_star
id|device_fn
)paren
(brace
r_int
r_int
id|bx
suffix:semicolon
r_int
r_int
id|ret
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb103
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|edi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;cx&quot;
(paren
id|class_code
)paren
comma
l_string|&quot;S&quot;
(paren
(paren
r_int
)paren
id|index
)paren
comma
l_string|&quot;D&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
op_star
id|bus
op_assign
(paren
id|bx
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
op_star
id|device_fn
op_assign
id|bx
op_amp
l_int|0xff
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_find_device
r_int
id|pcibios_find_device
(paren
r_int
r_int
id|vendor
comma
r_int
r_int
id|device_id
comma
r_int
r_int
id|index
comma
r_int
r_char
op_star
id|bus
comma
r_int
r_char
op_star
id|device_fn
)paren
(brace
r_int
r_int
id|bx
suffix:semicolon
r_int
r_int
id|ret
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb102
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|edi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;cx&quot;
(paren
id|device_id
)paren
comma
l_string|&quot;dx&quot;
(paren
id|vendor
)paren
comma
l_string|&quot;S&quot;
(paren
(paren
r_int
)paren
id|index
)paren
comma
l_string|&quot;D&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
op_star
id|bus
op_assign
(paren
id|bx
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
op_star
id|device_fn
op_assign
id|bx
op_amp
l_int|0xff
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_read_config_byte
r_int
id|pcibios_read_config_byte
(paren
r_int
r_char
id|bus
comma
r_int
r_char
id|device_fn
comma
r_int
r_char
id|where
comma
r_int
r_char
op_star
id|value
)paren
(brace
r_int
r_int
id|ret
suffix:semicolon
r_int
r_int
id|bx
op_assign
(paren
id|bus
op_lshift
l_int|8
)paren
op_or
id|device_fn
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb108
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|esi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=cl&quot;
(paren
op_star
id|value
)paren
comma
l_string|&quot;=eax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|where
)paren
comma
l_string|&quot;S&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_read_config_word
r_int
id|pcibios_read_config_word
(paren
r_int
r_char
id|bus
comma
r_int
r_char
id|device_fn
comma
r_int
r_char
id|where
comma
r_int
r_int
op_star
id|value
)paren
(brace
r_int
r_int
id|ret
suffix:semicolon
r_int
r_int
id|bx
op_assign
(paren
id|bus
op_lshift
l_int|8
)paren
op_or
id|device_fn
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb109
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|esi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=cx&quot;
(paren
op_star
id|value
)paren
comma
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|where
)paren
comma
l_string|&quot;S&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_read_config_dword
r_int
id|pcibios_read_config_dword
(paren
r_int
r_char
id|bus
comma
r_int
r_char
id|device_fn
comma
r_int
r_char
id|where
comma
r_int
r_int
op_star
id|value
)paren
(brace
r_int
r_int
id|ret
suffix:semicolon
r_int
r_int
id|bx
op_assign
(paren
id|bus
op_lshift
l_int|8
)paren
op_or
id|device_fn
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb10a
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|esi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=ecx&quot;
(paren
op_star
id|value
)paren
comma
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|where
)paren
comma
l_string|&quot;S&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_write_config_byte
r_int
id|pcibios_write_config_byte
(paren
r_int
r_char
id|bus
comma
r_int
r_char
id|device_fn
comma
r_int
r_char
id|where
comma
r_int
r_char
id|value
)paren
(brace
r_int
r_int
id|ret
suffix:semicolon
r_int
r_int
id|bx
op_assign
(paren
id|bus
op_lshift
l_int|8
)paren
op_or
id|device_fn
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb108
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|esi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;cl&quot;
(paren
id|value
)paren
comma
l_string|&quot;bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|where
)paren
comma
l_string|&quot;S&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_write_config_word
r_int
id|pcibios_write_config_word
(paren
r_int
r_char
id|bus
comma
r_int
r_char
id|device_fn
comma
r_int
r_char
id|where
comma
r_int
r_int
id|value
)paren
(brace
r_int
r_int
id|ret
suffix:semicolon
r_int
r_int
id|bx
op_assign
(paren
id|bus
op_lshift
l_int|8
)paren
op_or
id|device_fn
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb109
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|esi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;cx&quot;
(paren
id|value
)paren
comma
l_string|&quot;bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|where
)paren
comma
l_string|&quot;S&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|pcibios_write_config_dword
r_int
id|pcibios_write_config_dword
(paren
r_int
r_char
id|bus
comma
r_int
r_char
id|device_fn
comma
r_int
r_char
id|where
comma
r_int
r_int
id|value
)paren
(brace
r_int
r_int
id|ret
suffix:semicolon
r_int
r_int
id|bx
op_assign
(paren
id|bus
op_lshift
l_int|8
)paren
op_or
id|device_fn
suffix:semicolon
id|__asm__
(paren
"&quot;"
id|movw
"$"
l_int|0xb10a
comma
op_mod
op_mod
id|ax
id|lcall
(paren
op_mod
op_mod
id|esi
)paren
id|jc
l_float|1f
op_xor
op_mod
op_mod
id|ah
comma
op_mod
op_mod
id|ah
l_int|1
suffix:colon
"&quot;"
suffix:colon
l_string|&quot;=ax&quot;
(paren
id|ret
)paren
suffix:colon
l_string|&quot;ecx&quot;
(paren
id|value
)paren
comma
l_string|&quot;bx&quot;
(paren
id|bx
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|where
)paren
comma
l_string|&quot;S&quot;
(paren
id|pci_indirect
)paren
)paren
suffix:semicolon
r_return
(paren
r_int
)paren
(paren
id|ret
op_amp
l_int|0xff00
)paren
op_rshift
l_int|8
suffix:semicolon
)brace
DECL|function|NCR53c810_test
r_void
id|NCR53c810_test
c_func
(paren
r_void
)paren
(brace
r_int
r_char
id|bus
comma
id|device_fn
suffix:semicolon
r_int
r_int
id|index
suffix:semicolon
r_int
id|ret
suffix:semicolon
r_int
r_char
id|row
comma
id|col
suffix:semicolon
r_int
r_int
id|val
suffix:semicolon
r_for
c_loop
(paren
id|index
op_assign
l_int|0
suffix:semicolon
id|index
OL
l_int|4
suffix:semicolon
op_increment
id|index
)paren
(brace
id|ret
op_assign
id|pcibios_find_device
(paren
(paren
r_int
r_int
)paren
id|PCI_VENDOR_ID_NCR
comma
(paren
r_int
r_int
)paren
id|PCI_DEVICE_ID_NCR_53C810
comma
id|index
comma
op_amp
id|bus
comma
op_amp
id|device_fn
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ret
)paren
r_break
suffix:semicolon
id|printk
(paren
l_string|&quot;ncr53c810 : at PCI bus %d, device %d, function %d.&quot;
comma
(paren
r_int
)paren
id|bus
comma
(paren
r_int
)paren
(paren
(paren
id|device_fn
op_amp
l_int|0xf8
)paren
op_rshift
l_int|3
)paren
comma
(paren
r_int
)paren
(paren
id|device_fn
op_amp
l_int|7
)paren
)paren
suffix:semicolon
r_for
c_loop
(paren
id|row
op_assign
l_int|0
suffix:semicolon
id|row
OL
l_int|0x3c
suffix:semicolon
id|row
op_add_assign
l_int|0x10
)paren
(brace
id|printk
(paren
l_string|&quot;&bslash;n    reg 0x%02x    &quot;
comma
id|row
)paren
suffix:semicolon
r_for
c_loop
(paren
id|col
op_assign
l_int|0
suffix:semicolon
id|col
OL
l_int|0x10
suffix:semicolon
id|col
op_add_assign
l_int|4
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|ret
op_assign
id|pcibios_read_config_dword
(paren
id|bus
comma
id|device_fn
comma
id|row
op_plus
id|col
comma
op_amp
id|val
)paren
)paren
)paren
id|printk
(paren
l_string|&quot;0x%08lx      &quot;
comma
id|val
)paren
suffix:semicolon
r_else
id|printk
(paren
l_string|&quot;error 0x%02x &quot;
comma
id|ret
)paren
suffix:semicolon
)brace
)brace
id|printk
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
)brace
)brace
DECL|function|pcibios_strerror
r_char
op_star
id|pcibios_strerror
(paren
r_int
id|error
)paren
(brace
r_char
id|buf
(braket
l_int|80
)braket
suffix:semicolon
r_switch
c_cond
(paren
id|error
)paren
(brace
r_case
id|PCIBIOS_SUCCESFUL
suffix:colon
r_return
l_string|&quot;SUCCESFUL&quot;
suffix:semicolon
r_case
id|PCIBIOS_FUNC_NOT_SUPPORTED
suffix:colon
r_return
l_string|&quot;FUNC_NOT_SUPPORTED&quot;
suffix:semicolon
r_case
id|PCIBIOS_BAD_VENDOR_ID
suffix:colon
r_return
l_string|&quot;BAD_VENDOR_ID&quot;
suffix:semicolon
r_case
id|PCIBIOS_DEVICE_NOT_FOUND
suffix:colon
r_return
l_string|&quot;DEVICE_NOT_FOUND&quot;
suffix:semicolon
r_case
id|PCIBIOS_BAD_REGISTER_NUMBER
suffix:colon
r_return
l_string|&quot;BAD_REGISTER_NUMBER&quot;
suffix:semicolon
r_default
suffix:colon
id|sprintf
(paren
id|buf
comma
l_string|&quot;UNKNOWN RETURN 0x%x&quot;
comma
id|error
)paren
suffix:semicolon
r_return
id|buf
suffix:semicolon
)brace
)brace
eof
