...
multiline_comment|/*&n; * This is how a process data structure is allocated. In most&n; * cases, the &quot;tsk&quot; pointers point to the same allocation unit&n; * substructures, but if the new process shares part (or all)&n; * of the sub-units with the parent process, the tsk pointers&n; * may point to the parent instead.&n; *&n; * Regardless, we always allocate the full allocation unit, as&n; * the normal fork() semantics require all of them and doing&n; * suballocations would be wasteful.&n; */
DECL|struct|allocation_struct
r_struct
id|allocation_struct
(brace
DECL|member|tsk
r_struct
id|task_struct
