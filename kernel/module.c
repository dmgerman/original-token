...
multiline_comment|/*&n; * Copy the kernel symbol table to user space.  If the argument is null,&n; * just return the size of the table.&n; *&n; * Note that the transient module symbols are copied _first_,&n; * in lifo order!!!&n; *&n; * The symbols to &quot;insmod&quot; are according to the &quot;old&quot; format: struct kernel_sym,&n; * which is actually quite handy for this purpose.&n; * Note that insmod inserts a struct symbol_table later on...&n; * (as that format is quite handy for the kernel...)&n; *&n; * For every module, the first (pseudo)symbol copied is the module name&n; * and the adress of the module struct.&n; * This lets &quot;insmod&quot; keep track of references, and build the array of&n; * struct module_refs in the symbol table.&n; * The format of the module name is &quot;#module&quot;, so that &quot;insmod&quot; can easily&n; * notice when a module name comes along. Also, this will make it possible&n; * to use old versions of &quot;insmod&quot;, albeit with reduced functionality...&n; * The &quot;kernel&quot; module has an empty name.&n; */
id|asmlinkage
r_int
