...
multiline_comment|/*&n; * Gets the address for a symbol in the given module.  If modname is&n; * NULL, it looks for the name in any registered symbol table.  If the&n; * modname is an empty string, it looks for the symbol in kernel exported&n; * symbol tables. Increase the usage count of the module in which the&n; * symbol was found - it&squot;s the only way we can guarantee that it&squot;s still&n; * there by the time our caller actually uses it.&n; */
r_int
r_int
...
multiline_comment|/* Decrease the use count of the module containing a symbol with the &n; * address passed.&n; */
