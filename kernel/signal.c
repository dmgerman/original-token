...
id|out_nolock
suffix:semicolon
multiline_comment|/* If t-&gt;sig is gone, we must be trying to kill the task.  So&n;&t;   pretend that it doesn&squot;t exist anymore.  */
id|ret
op_assign
op_minus
id|ESRCH
...
multiline_comment|/* Don&squot;t bother with ignored sigs (SIGCHLD is special) */
op_logical_and
(paren
(paren
id|ka-&gt;sa.sa_handler
...
multiline_comment|/* Some signals are ignored by default.. (but SIGCONT&n;&t;&t;   already did its deed) */
op_logical_or
(paren
id|ka-&gt;sa.sa_handler
op_eq
