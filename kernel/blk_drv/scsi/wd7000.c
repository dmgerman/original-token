multiline_comment|/* $Id: $&n; *  linux/kernel/wd7000.c&n; *&n; *  Copyright (C) 1992  Thomas Wuensche&n; *&t;closely related to the aha1542 driver from Tommy Thorn&n; *&t;( as close as different hardware allows on a lowlevel-driver :-) )&n; *&n; *  Revised (and renamed) by John Boyd &lt;boyd@cis.ohio-state.edu&gt; to&n; *  accomodate Eric Youngdale&squot;s modifications to scsi.c.  Nov 1992.&n; */
...
macro_line|#endif
multiline_comment|/*&n;   Driver data structures:&n;   - mb and scbs are required for interfacing with the host adapter.&n;     An SCB has extra fields not visible to the adapter; mb&squot;s&n;     _cannot_ do this, since the adapter assumes they are contiguous in&n;     memory, 4 bytes each, with ICMBs following OGMBs, and uses this fact&n;     to access them.&n;   - An icb is for host-only (non-SCSI) commands.  ICBs are 16 bytes each;&n;     the additional bytes are used only by the driver.&n;   - For now, a pool of SCBs are kept in global storage by this driver,&n;     and are allocated and freed as needed.&n;&n;  For two reasons, I decided not to use a single SCB per OGMB:&n;   - the 7000-FASST2 marks OGMBs empty as soon as it has _started_ a command,&n;     not when it has finished.  Since the SCB must be around for completion,&n;     problems arise when SCBs correspond to OGMBs, which may be reallocated&n;     earlier (or delayed unnecessarily until a command completes).&n;   - Scatter/gather can be implemented by this driver using an SCB per&n;     scatter/gather buffer, but this is difficult when SCBs and Scsi_Cmnds&n;     are matched 1-1 via this correspondence.&n;&n;  Instead, mailboxes are used as transient data structures, simply for&n;  carrying SCB addresses to/from the 7000-FASST2.  SCBs are allocated as&n;  &quot;host_scribble&quot; in an Scsi_Cmnd, and are maintained relative to that&n;  Scsi_Cmnd.&n;&n;  Note also since SCBs are not &quot;permanently&quot; associated with mailboxes,&n;  there is no need to keep a global list of Scsi_Cmnd pointers indexed&n;  by OGMB.   Again, SCBs reference their Scsi_Cmnds directly, so mailbox&n;  indices need not be involved.&n;*/
r_static
r_struct
(brace
DECL|member|ogmb
...
c_func
(paren
l_string|&quot; ICMB %d posted for SCB/ICB %06x, status %02x&quot;
comma
id|icmb
comma
id|scb
...
(brace
multiline_comment|/* set up linked SCBs to do scatter/gather */
macro_line|#ifdef 0
r_struct
id|scatterlist
op_star
...
id|scbn
suffix:semicolon
multiline_comment|/*&n;&t;    Allocate the scbs first, and set the next pointers, since we&n;&t;    need these later.&n;&t;    Save the list via host_scribble.&n;&t;*/
id|scbn
op_assign
l_int|NULL
suffix:semicolon
...
id|scb
suffix:semicolon
multiline_comment|/*&n;&t;    Note that the following is set up do to scatter/gather via&n;&t;    linked commands, with an interrupt only after _all_ are&n;&t;    finished.  This is mostly to simplify error handling, which&n;&t;    hasn&squot;t been added to the interrupt handler for the other case&n;&t;    (interrupt per SCB).   However, this should work with intr/SCB&n;&t;    by setting phase = use_sg and setting the flag bit in each&n;&t;    of the CDBs.&n;&t;*/
id|SCpnt-&gt;SCp.phase
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* set this to the # of interrupts expected */
r_for
c_loop
(paren
id|i
...
suffix:semicolon
multiline_comment|/*&n;&t;      Here, set CDB fields for block address &amp; block count.&n;&t;      I don&squot;t know block size; that&squot;s why this whole thing is&n;&t;      commented out.  Would be nice if scatterlist had both&n;&t;      byte and block counts per element (block address could be&n;&t;      computed).&n;&t;      Also need to know for sure if any SCSI commands other than&n;&t;      READ/WRITE can use scatter/gather - I would hope not.&n;&t;    */
id|any2scsi
c_func
(paren
id|scb-&gt;dataptr
...
(brace
multiline_comment|/* if this isn&squot;t the last */
id|any2scsi
c_func
(paren
id|scb-&gt;linkptr
...
l_int|0x01
suffix:semicolon
multiline_comment|/* set link bit */
)brace
id|scb-&gt;direc
op_assign
l_int|0x40
...
c_func
(paren
l_string|&quot;wd7000_queuecommand: scatter/gather not implemented.&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
)brace
r_else
(brace
multiline_comment|/* just one command - use scb[0] */
id|SCpnt-&gt;SCp.phase
op_assign
l_int|1
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* now format the above, and append to info */
id|wd7000_append_info
c_func
(paren
id|info
