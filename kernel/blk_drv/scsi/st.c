multiline_comment|/*&n;  SCSI Tape Driver for Linux&n;&n;  Version 0.02 for Linux 0.98.4 and Eric Youngdale&squot;s new scsi driver&n;&n;  History:&n;  Rewritten from Dwayne Forsyth&squot;s SCSI tape driver by Kai Makisara.&n;&n;  Features:&n;  - support for different block sizes and internal buffering&n;  - *nix-style ioctl with codes from mtio.h from the QIC-02 driver by&n;    Hennus Bergman (command MTSETBLK added)&n;  - character device&n;  - rewind and non-rewind devices&n;  - capability to handle several tape drives simultaneously&n;  - one buffer if one drive, two buffers if more than one drive (limits the&n;    number of simultaneously open drives to two)&n;  - write behind&n;&n;  Devices:&n;  Autorewind devices have minor numbers equal to the tape numbers (0 &gt; ).&n;  Nonrewind device has the minor number equal to tape number + 128.&n;&n;  Problems:&n;  The end of media detection may not work correctly because of the buffering.&n;  If you want to do multiple tape backups relying on end of tape detection,&n;  you should disable write behind and in addition to that check that the&n;  tapes are readable.&n;&n;  Kai Makisara, Nov 9, 1992  email makisara@vtinsx.ins.vtt.fi or&n;                                    Kai.Makisara@vtt.fi&n;  Last changes Nov 29, 1992.&n;*/
...
mdefine_line|#define ST_TIMEOUT 1000
...
(brace
multiline_comment|/* What should actually be done here ? */
id|printk
c_func
(paren
...
c_func
(paren
l_string|&quot;st%d: Final transfer %d bytes (length %d).&bslash;n&quot;
comma
id|dev
comma
id|transfer
...
c_func
(paren
l_string|&quot;st%d: Error on last write:&bslash;n&quot;
comma
id|dev
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;st%d: No write allowed after read.&bslash;n&quot;
comma
id|dev
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;st%d: No read allowed after write.&bslash;n&quot;
comma
id|dev
)paren
suffix:semicolon
