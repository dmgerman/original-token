macro_line|#include &lt;linux/config.h&gt;
macro_line|#ifdef CONFIG_SCSI 
macro_line|#include &lt;errno.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;asm/segment.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &quot;scsi.h&quot;
macro_line|#include &quot;hosts.h&quot;
macro_line|#include &quot;scsi_ioctl.h&quot;
DECL|macro|MAX_RETRIES
mdefine_line|#define MAX_RETRIES 5&t;
DECL|macro|MAX_TIMEOUT
mdefine_line|#define MAX_TIMEOUT 200
DECL|macro|MAX_BUF
mdefine_line|#define MAX_BUF 1024  &t;
DECL|macro|max
mdefine_line|#define max(a,b) (((a) &gt; (b)) ? (a) : (b))
multiline_comment|/*&n; * If we are told to probe a host, we will return 0 if  the host is not&n; * present, 1 if the host is present, and will return an identifying&n; * string at *arg, if arg is non null, filling to the length stored at&n; * (int *) arg&n; */
DECL|function|ioctl_probe
r_static
r_int
id|ioctl_probe
c_func
(paren
r_int
id|dev
comma
r_void
op_star
id|buffer
)paren
(brace
r_int
id|temp
suffix:semicolon
r_int
id|len
suffix:semicolon
r_if
c_cond
(paren
(paren
id|temp
op_assign
id|scsi_hosts
(braket
id|dev
)braket
dot
id|present
)paren
op_logical_and
id|buffer
)paren
(brace
id|len
op_assign
id|get_fs_long
(paren
(paren
r_int
op_star
)paren
id|buffer
)paren
suffix:semicolon
id|memcpy_tofs
(paren
id|buffer
comma
id|scsi_hosts
(braket
id|dev
)braket
dot
id|info
c_func
(paren
)paren
comma
id|len
)paren
suffix:semicolon
)brace
r_return
id|temp
suffix:semicolon
)brace
multiline_comment|/*&n; * &n; * The SCSI_IOCTL_SEND_COMMAND ioctl sends a command out to the SCSI host.&n; * The MAX_TIMEOUT and MAX_RETRIES  variables are used.  &n; * &n; * dev is the SCSI device number, *(int *) arg the length of the input&n; * data, *((int *)arg + 1) the output buffer.&n; * &n; * *(char *) ((int *) arg)[1] the actual command.   &n; * &n; * Note that no more than MAX_BUF bytes will be transfered.  Since&n; * SCSI block device size is 512 bytes, I figured 1K was good.&n; * &n; * This size * does  * include  the initial lengths that were passed.&n; * &n; * The SCSI command is read from  the memory location immediately after the&n; * length words, and the out data after the command.  The SCSI routines know the&n; * command size based on the length byte.  &n; * &n; * The area is then filled in from the byte at offset 0. &n; */
DECL|variable|the_result
r_static
r_int
id|the_result
(braket
id|MAX_SCSI_HOSTS
)braket
suffix:semicolon
DECL|function|scsi_ioctl_done
r_static
r_void
id|scsi_ioctl_done
(paren
r_int
id|host
comma
r_int
id|result
)paren
(brace
id|the_result
(braket
id|host
)braket
op_assign
id|result
suffix:semicolon
)brace
DECL|function|ioctl_command
r_static
r_int
id|ioctl_command
c_func
(paren
id|Scsi_Device
op_star
id|dev
comma
r_void
op_star
id|buffer
)paren
(brace
r_char
id|buf
(braket
id|MAX_BUF
)braket
suffix:semicolon
r_char
id|cmd
(braket
l_int|10
)braket
suffix:semicolon
r_char
op_star
id|cmd_in
suffix:semicolon
r_int
r_char
id|opcode
suffix:semicolon
r_int
id|inlen
comma
id|outlen
comma
id|cmdlen
comma
id|temp
comma
id|host
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|buffer
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|inlen
op_assign
id|get_fs_long
c_func
(paren
(paren
r_int
op_star
)paren
id|buffer
)paren
suffix:semicolon
id|outlen
op_assign
id|get_fs_long
c_func
(paren
(paren
(paren
r_int
op_star
)paren
id|buffer
)paren
op_plus
l_int|1
)paren
suffix:semicolon
id|cmd_in
op_assign
(paren
r_char
op_star
)paren
(paren
(paren
(paren
r_int
op_star
)paren
id|buffer
)paren
op_plus
l_int|2
)paren
suffix:semicolon
id|opcode
op_assign
id|get_fs_byte
c_func
(paren
id|cmd_in
)paren
suffix:semicolon
id|memcpy_fromfs
(paren
(paren
r_void
op_star
)paren
id|cmd
comma
id|cmd_in
comma
id|cmdlen
op_assign
id|COMMAND_SIZE
(paren
id|opcode
)paren
)paren
suffix:semicolon
id|memcpy_fromfs
(paren
(paren
r_void
op_star
)paren
id|buf
comma
(paren
r_void
op_star
)paren
(paren
id|cmd_in
op_plus
id|cmdlen
)paren
comma
id|inlen
)paren
suffix:semicolon
id|host
op_assign
id|dev-&gt;host_no
suffix:semicolon
macro_line|#ifndef DEBUG_NO_CMD
r_do
(brace
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|the_result
(braket
id|host
)braket
)paren
(brace
id|sti
c_func
(paren
)paren
suffix:semicolon
r_while
c_loop
(paren
id|the_result
(braket
id|host
)braket
)paren
(brace
multiline_comment|/* nothing */
suffix:semicolon
)brace
)brace
r_else
(brace
id|the_result
(braket
id|host
)braket
op_assign
op_minus
l_int|1
suffix:semicolon
id|sti
c_func
(paren
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
)brace
r_while
c_loop
(paren
l_int|1
)paren
suffix:semicolon
id|scsi_do_cmd
c_func
(paren
id|host
comma
id|dev-&gt;id
comma
id|cmd
comma
id|buf
comma
(paren
(paren
id|outlen
OG
id|MAX_BUF
)paren
ques
c_cond
id|MAX_BUF
suffix:colon
id|outlen
)paren
comma
id|scsi_ioctl_done
comma
id|MAX_TIMEOUT
comma
id|buf
comma
id|MAX_RETRIES
)paren
suffix:semicolon
r_while
c_loop
(paren
id|the_result
(braket
id|host
)braket
op_eq
op_minus
l_int|1
)paren
multiline_comment|/* nothing */
suffix:semicolon
id|temp
op_assign
id|the_result
(braket
id|host
)braket
suffix:semicolon
id|the_result
(braket
id|host
)braket
op_assign
l_int|0
suffix:semicolon
id|memcpy_tofs
(paren
id|buffer
comma
id|buf
comma
(paren
id|outlen
OG
id|MAX_BUF
)paren
ques
c_cond
id|MAX_BUF
suffix:colon
id|outlen
)paren
suffix:semicolon
r_return
id|temp
suffix:semicolon
macro_line|#else
(brace
r_int
id|i
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;scsi_ioctl : device %d.  command = &quot;
comma
id|dev
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|10
suffix:semicolon
op_increment
id|i
)paren
id|printk
c_func
(paren
l_string|&quot;%02x &quot;
comma
id|cmd
(braket
id|i
)braket
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;r&bslash;nbuffer =&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|20
suffix:semicolon
op_increment
id|i
)paren
id|printk
c_func
(paren
l_string|&quot;%02x &quot;
comma
id|buf
(braket
id|i
)braket
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;r&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
macro_line|#endif
)brace
multiline_comment|/*&n;&t;the scsi_ioctl() function differs from most ioctls in that it does&n;&t;not take a major/minor number as the dev filed.  Rather, it takes&n;&t;an index in scsi_devices[] &n;*/
DECL|function|scsi_ioctl
r_int
id|scsi_ioctl
(paren
r_int
id|dev
comma
r_int
id|cmd
comma
r_void
op_star
id|arg
)paren
(brace
r_if
c_cond
(paren
(paren
id|cmd
op_ne
l_int|0
op_logical_and
id|dev
OG
id|NR_SCSI_DEVICES
)paren
op_logical_or
id|dev
OL
l_int|0
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
r_if
c_cond
(paren
(paren
id|cmd
op_eq
l_int|0
op_logical_and
id|dev
OG
id|MAX_SCSI_HOSTS
)paren
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
r_switch
c_cond
(paren
id|cmd
)paren
(brace
r_case
id|SCSI_IOCTL_PROBE_HOST
suffix:colon
r_return
id|ioctl_probe
c_func
(paren
id|dev
comma
id|arg
)paren
suffix:semicolon
r_case
id|SCSI_IOCTL_SEND_COMMAND
suffix:colon
r_return
id|ioctl_command
c_func
(paren
(paren
id|Scsi_Device
op_star
)paren
id|dev
comma
id|arg
)paren
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
)brace
macro_line|#endif
eof
