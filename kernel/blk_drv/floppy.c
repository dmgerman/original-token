...
multiline_comment|/*&n; * Note that MAX_ERRORS=X doesn&squot;t imply that we retry every bad read&n; * max X times - some types of errors increase the errorcount by 2,&n; * so we might actually retry only X/2 times before giving up.&n; */
