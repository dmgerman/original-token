...
multiline_comment|/*&n; * Note that MAX_ERRORS=10 doesn&squot;t imply that we retry every bad read&n; * max 10 times - some types of errors increase the errorcount by 2,&n; * so we might actually retry only 6-7 times before giving up.&n; */
DECL|macro|MAX_ERRORS
mdefine_line|#define MAX_ERRORS 10
...
multiline_comment|/*&n; * This struct defines the different floppy types. Unlike minix&n; * linux doesn&squot;t have a &quot;search for right type&quot;-type, as the code&n; * for that is convoluted and weird.&n; *&n; * The &squot;stretch&squot; tells if the tracks need to be boubled for some&n; * types (ie 360kB diskette in 1.2MB drive etc). Others should&n; * be self-explanatory.&n; */
...
l_int|0
suffix:semicolon
multiline_comment|/*&n; * floppy-change is never called from an interrupt, so we can relax a bit&n; * here.&n; */
...
multiline_comment|/* are we on the right track? */
r_if
c_cond
(paren
id|ST1
...
multiline_comment|/* yes - set up DMA and read/write command */
id|setup_DMA
c_func
(paren
)paren
