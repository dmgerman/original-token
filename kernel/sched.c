...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * bingo, we couldnt get a better&n;&t;&t;&t;&t; * CPU, activate it.&n;&t;&t;&t;&t; */
r_goto
id|send
suffix:semicolon
multiline_comment|/* this one helps GCC ... */
)brace
)brace
)brace
r_while
...
suffix:semicolon
multiline_comment|/*&n;&t; * found any idle CPU?&n;&t; */
r_if
c_cond
(paren
id|target_tsk
...
multiline_comment|/*&n; * If there is a dependency between p1 and p2,&n; * don&squot;t be too eager to go into the slow schedule.&n; * In particular, if p1 and p2 both want the kernel&n; * lock, there is no point in trying to make them&n; * extremely parallel..&n; *&n; * (No lock - lock_depth &lt; 0)&n; */
DECL|macro|related
mdefine_line|#define related(p1,p2) ((p1)-&gt;lock_depth &gt;= 0 &amp;&amp; (p2)-&gt;lock_depth &gt;= 0)
...
multiline_comment|/*&n; * This one aligns per-CPU data on cacheline boundaries.&n; */
r_static
r_union
(brace
...
id|sched_data
suffix:semicolon
multiline_comment|/*&n;&t; * We might have switched CPUs:&n;&t; */
id|sched_data
op_assign
op_amp
id|aligned_data
...
id|schedule_data
suffix:semicolon
multiline_comment|/*&n;&t; * Subtle. In the rare event that we got a wakeup to &squot;prev&squot; just&n;&t; * during the reschedule (this is possible, the scheduler is pretty&n;&t; * parallel), we should do another reschedule in the next task&squot;s&n;&t; * context. schedule() will do the right thing next time around.&n;&t; * this is equivalent to &squot;delaying&squot; the wakeup until the reschedule&n;&t; * has finished.&n;&t; */
r_if
c_cond
(paren
id|sched_data-&gt;prev-&gt;state
op_ne
id|sched_data-&gt;prevstate
)paren
id|current-&gt;need_resched
op_assign
l_int|1
...
id|t
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Simple, exponentially fading average calculation:&n;&t;&t; */
id|prev-&gt;avg_slice
op_assign
id|this_slice
op_plus
...
mdefine_line|#define&t;SLEEP_ON_VAR&t;&t;&t;&t;&bslash;&n;&t;unsigned long flags;&t;&t;&t;&bslash;&n;&t;struct wait_queue wait;
DECL|macro|SLEEP_ON_HEAD
mdefine_line|#define&t;SLEEP_ON_HEAD&t;&t;&t;&t;&t;&bslash;&n;&t;wait.task = current;&t;&t;&t;&t;&bslash;&n;&t;write_lock_irqsave(&amp;waitqueue_lock, flags);&t;&bslash;&n;&t;__add_wait_queue(p, &amp;wait);&t;&t;&t;&bslash;&n;&t;write_unlock(&amp;waitqueue_lock);
DECL|macro|SLEEP_ON_TAIL
mdefine_line|#define&t;SLEEP_ON_TAIL&t;&t;&t;&t;&t;&t;&bslash;&n;&t;write_lock_irq(&amp;waitqueue_lock);&t;&t;&t;&bslash;&n;&t;__remove_wait_queue(p, &amp;wait);&t;&t;&t;&t;&bslash;&n;&t;write_unlock_irqrestore(&amp;waitqueue_lock, flags);
