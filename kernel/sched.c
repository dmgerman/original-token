...
suffix:semicolon
multiline_comment|/*&n; * Wake up a process. Put it on the run-queue if it&squot;s not&n; * already there.  The &quot;current&quot; process is always on the&n; * run-queue, and as such you&squot;re allowed to do the simpler&n; * &quot;current-&gt;state = TASK_RUNNING&quot; to mark yourself runnable&n; * without the overhead of this.&n; *&n; * (actually, the run-queue isn&squot;t implemented yet, so this&n; * function is mostly a dummy one)&n; */
...
suffix:semicolon
multiline_comment|/* already on run-queue? */
r_if
c_cond
(paren
id|oldstate
