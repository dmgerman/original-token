...
multiline_comment|/*&n; * Try-to-share-buffers tries to minimize memory use by trying to keep&n; * both code pages and the buffer area in the same page. This is done by&n; * trying to load them into memory the way we want them.&n; *&n; * This doesn&squot;t guarantee that the memory is shared, but should under most&n; * circumstances work very well indeed (ie &gt;90% sharing of code pages on&n; * demand-loadable executables).&n; */
