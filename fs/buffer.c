...
id|interval
suffix:semicolon
multiline_comment|/* Interval (seconds) between spontaneous&n;&t;&t;&t;&t;  bdflush runs */
...
suffix:semicolon
multiline_comment|/* try to allocate something until we get it or we&squot;re asking&n;           for something that is really too small */
r_do
(brace
id|nr_hash
op_assign
...
id|bh-&gt;b_next_free
suffix:semicolon
multiline_comment|/* Dirty/locked buffer on clean list?  Refile it */
r_if
c_cond
(paren
id|buffer_locked
...
(brace
multiline_comment|/* We may have stalled while waiting for&n;&t;&t;&t;   I/O to complete. */
r_if
c_cond
(paren
id|bh-&gt;b_list
...
r_continue
suffix:semicolon
multiline_comment|/* Should we write back buffers that are&n;&t;&t;&t;   shared or not??  Currently dirty buffers&n;&t;&t;&t;   are not shared, so it does not matter */
id|next-&gt;b_count
op_increment
suffix:semicolon
id|bh-&gt;b_count
...
multiline_comment|/* If we didn&squot;t write anything, but there are still&n;&t; * dirty buffers, then make the next write to a&n;&t; * loop device to be a blocking write.&n;&t; * This lets us block--which we _must_ do! */
r_if
c_cond
(paren
id|ndirty
...
multiline_comment|/* Func 1 used to call sync_old_buffers; a user space&n;&t;&t;   daemon would call it periodically.  This is no&n;&t;&t;   longer necessary.  Returning -EPERM here makes the&n;&t;&t;   daemon silently exit.  */
r_goto
id|out
suffix:semicolon
...
multiline_comment|/* This is the actual bdflush daemon itself. It used to be started&n; * from the syscall above, but now we launch it ourselves internally&n; * with kernel_thread(...)  directly after the first thread in&n; * init/main.c.  Every so often, or when woken up by another task that&n; * needs memory, we call sync_old_buffers to partially clear the dirty list.&n; */
...
l_int|0
suffix:semicolon
multiline_comment|/* inhibit ptrace() */
id|strcpy
c_func
(paren
id|tsk-&gt;comm
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Also try to flush inodes and supers, since&n;&t;&t;&t; * otherwise there would be no way of ensuring&n;&t;&t;&t; * that these quantities ever get written&n;&t;&t;&t; * back.  Ideally, we would have a timestamp&n;&t;&t;&t; * on the inodes and superblocks so that we&n;&t;&t;&t; * could write back only the old ones.&n;&t;&t;&t; */
id|sync_supers
c_func
(paren
l_int|0
...
multiline_comment|/* Keep flushing till there aren&squot;t very many dirty buffers */
r_do
(brace
id|sync_old_buffers
c_func
