...
multiline_comment|/*&n; * Try to free up some pages by shrinking the buffer-cache&n; *&n; * Priority tells the routine how hard to try to shrink the&n; * buffers: 0 means &quot;don&squot;t bother too much&quot;, while a value&n; * of 3 means &quot;we&squot;d better get some free pages now&quot;.&n; */
...
multiline_comment|/*&n; * This initializes the low 1M that isn&squot;t used by the kernel to buffer&n; * cache. It should really be used for paging memory, but it takes a lot&n; * of special-casing, which I don&squot;t want to do.&n; *&n; * The biggest problem with this approach is that all low-mem buffers&n; * have a fixed size of 1024 chars: not good if/when the other sizes&n; * are implemented.&n; */
...
c_func
(paren
l_string|&quot;unable to get a single buffer-head&quot;
)paren
suffix:semicolon
id|free_list-&gt;b_prev_free
op_assign
