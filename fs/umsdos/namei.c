...
id|out_remove
suffix:semicolon
multiline_comment|/* keep the short name anonymous ... */
r_if
c_cond
(paren
id|dentry
...
(paren
id|KERN_ERR
l_string|&quot;umsdos_rename_f: old entry %s/%s not in EMD, ret=%d&bslash;n&quot;
comma
...
c_func
(paren
l_string|&quot;umsdos_rename_f: %s/%s old sticky bit, fsuid=%d, uid=%d, dir=%d&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_info.entry.name
...
(brace
multiline_comment|/* Are we allowed to replace it? */
r_if
c_cond
(paren
id|is_sticky
...
suffix:semicolon
multiline_comment|/* create the new entry ... */
id|umsdos_ren_init
(paren
op_amp
id|new_info
...
(paren
id|KERN_WARNING
l_string|&quot;umsdos_rename_f: newentry %s/%s failed, ret=%d&bslash;n&quot;
comma
id|new_dentry-&gt;d_parent-&gt;d_name.name
comma
id|new_info.entry.name
...
c_func
(paren
l_string|&quot;umsdos_rename_f: moving hardlink %s/%s, rehash=%d&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_info.entry.name
...
multiline_comment|/* Do a real lookup to get the old short name dentry */
id|old
op_assign
id|umsdos_lookup_dentry
c_func
...
(paren
id|KERN_WARNING
l_string|&quot;umsdos_rename_f: lookup old dentry %s/%s, ret=%d&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_info.fake.fname
...
id|out_dput
suffix:semicolon
multiline_comment|/*&n;&t;&t; * A cross-directory move with different short and long&n;&t;&t; * names has nasty complications: msdos-fs will need to&n;&t;&t; * change inodes, so we must check whether the original&n;&t;&t; * dentry is busy, and if the rename succeeds the short&n;&t;&t; * dentry will come back with a different inode.&n;&t;&t; *&n;&t;&t; * To handle this, we drop the dentry and free the inode,&n;&t;&t; * and then pick up the new inode after the rename.&n;&t;&t; */
r_if
c_cond
(paren
id|old_dir
...
c_func
(paren
l_string|&quot;umsdos_rename_f: old dentry %s/%s busy, d_count=%d&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_dentry-&gt;d_name.name
...
c_func
(paren
l_string|&quot;umsdos_rename_f: cross-dir move, %s/%s dropped&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_dentry-&gt;d_name.name
...
(paren
id|KERN_WARNING
l_string|&quot;umsdos_rename_f: lookup new dentry %s/%s, ret=%d&bslash;n&quot;
comma
id|new_dentry-&gt;d_parent-&gt;d_name.name
comma
id|new_info.fake.fname
...
c_func
(paren
l_string|&quot;umsdos_rename_f: new %s/%s, inode %p!=%p??&bslash;n&quot;
comma
r_new
op_member_access_from_pointer
...
c_func
(paren
l_string|&quot;umsdos_rename_f: msdos_rename %s/%s(%ld) to %s/%s(%ld)&bslash;n&quot;
comma
id|old-&gt;d_parent-&gt;d_name.name
comma
id|old-&gt;d_name.name
comma
id|old-&gt;d_inode-&gt;i_ino
comma
r_new
op_member_access_from_pointer
id|d_parent-&gt;d_name.name
...
(paren
(paren
l_string|&quot;umsdos_rename_f: rename failed, ret=%d, removing %s/%s&bslash;n&quot;
comma
id|ret
comma
id|new_dentry-&gt;d_parent-&gt;d_name.name
...
multiline_comment|/*&n;&t; * Check whether to update the dcache ... if both &n;&t; * old and new dentries match, it&squot;s already correct.&n;&t; * If the targets were aliases, the old short-name&n;&t; * dentry has the original target name.&n; &t; */
r_if
c_cond
(paren
id|old_dentry
...
c_func
(paren
l_string|&quot;umsdos_rename_f: %s/%s gets new ino=%ld&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_dentry-&gt;d_name.name
...
id|move_it
suffix:colon
multiline_comment|/* this will rehash the dentry ... */
id|d_move
c_func
(paren
id|old_dentry
...
multiline_comment|/* Check whether the old inode changed ... */
r_if
c_cond
(paren
id|old_dentry-&gt;d_inode
...
suffix:semicolon
multiline_comment|/* Check again for a busy dentry */
id|ret
op_assign
op_minus
id|EBUSY
...
(paren
(paren
l_string|&quot;UMSDOS_unlink: hard link %s/%s, fake=%s, rehash=%d&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
...
c_func
(paren
l_string|&quot;umsdos_rename: enter, %s/%s(%ld) to %s/%s(%ld)&bslash;n&quot;
comma
id|old_dentry-&gt;d_parent-&gt;d_name.name
comma
id|old_dentry-&gt;d_name.name
...
id|out
suffix:semicolon
multiline_comment|/*&n;&t; * If the target already exists, delete it first.&n;&t; */
r_if
c_cond
(paren
id|new_dentry-&gt;d_inode
...
multiline_comment|/*&n;&t; * If we didn&squot;t get a negative dentry, make a copy and hash it.&n;&t; */
id|new_target
op_assign
id|new_dentry
suffix:semicolon
...
c_func
(paren
l_string|&quot;umsdos_rename: %s/%s not negative, hash=%d&bslash;n&quot;
comma
id|new_dentry-&gt;d_parent-&gt;d_name.name
comma
id|new_dentry-&gt;d_name.name
