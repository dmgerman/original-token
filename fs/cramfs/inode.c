...
multiline_comment|/*&n; * BLKS_PER_BUF_SHIFT must be at least 1 to allow for &quot;compressed&quot;&n; * data that takes up more space than the original.  1 is guaranteed&n; * to suffice, though.  Larger values provide more read-ahead and&n; * proportionally less wastage at the end of the buffer.&n; */
DECL|macro|BLKS_PER_BUF_SHIFT
mdefine_line|#define BLKS_PER_BUF_SHIFT (2)
DECL|macro|BLKS_PER_BUF
mdefine_line|#define BLKS_PER_BUF (1 &lt;&lt; BLKS_PER_BUF_SHIFT)
...
(paren
id|KERN_ERR
l_string|&quot;cramfs (device %s): requested chunk (%u:+%u) bigger than buffer&bslash;n&quot;
comma
id|bdevname
c_func
(paren
...
suffix:semicolon
multiline_comment|/* TODO: return EIO to process or kill the current process&n;           instead of resuming. */
op_star
(paren
(paren
r_int
...
l_int|0
suffix:semicolon
multiline_comment|/* XXX: doesn&squot;t work on all archs */
r_goto
id|resume
suffix:semicolon
)brace
...
id|tmp
suffix:semicolon
multiline_comment|/* Unsupported fields set to -1 as per man page. */
id|memset
c_func
(paren
op_amp
