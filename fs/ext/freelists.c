...
multiline_comment|/*&n;&n;   The free blocks are managed by a linked list. The super block contains the&n;   number of the first free block. This block contains 254 numbers of other&n;   free blocks and the number of the next block in the list.&n;&n;   When an ext fs is mounted, the number of the first free block is stored&n;   in s-&gt;u.ext_sb.s_zmap[0] and the block header is stored in s-&gt;u.ext_sb.s_zmap[1]. u.ext_sb.s_zmap[2]&n;   contains the count of free blocks.&n;&n;   Currently, it is a hack to allow this kind of management with the super_block&n;   structure.&n;   Perhaps, in the future, we may have to change the super_block structure to&n;   include dedicated fields.&n;&n;   The free inodes are also managed by a linked list in a similar way. The&n;   super block contains the number of the first free inode. This inode contains&n;   14 numbers of other free inodes and the number of the next inode in the list.&n;   &n;   The number of the first free inode is stored in s-&gt;u.ext_sb.s_imap[0] and the header&n;   of the block containing the inode is stored in s-&gt;u.ext_sb.s_imap[1]. u.ext_sb.s_imap[2] contains&n;   the count of free inodes.&n;&n;*/
...
id|printk
(paren
l_string|&quot;u.ext_sb.s_imap[0] = %d, ino = %d&bslash;n&quot;
comma
(paren
r_int
)paren
