multiline_comment|/*&n; *  linux/fs/ext/inode.c&n; *&n; *  Copyright (C) 1992  Remy Card (card@masi.ibp.fr)&n; *&n; *  from&n; *&n; *  linux/fs/minix/inode.c&n; *&n; *  Copyright (C) 1991, 1992  Linus Torvalds&n; */
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/ext_fs.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/segment.h&gt;
r_int
id|sync_dev
c_func
(paren
r_int
id|dev
)paren
suffix:semicolon
DECL|function|ext_put_inode
r_void
id|ext_put_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|inode-&gt;i_size
op_assign
l_int|0
suffix:semicolon
id|ext_truncate
c_func
(paren
id|inode
)paren
suffix:semicolon
id|ext_free_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
DECL|function|ext_put_super
r_void
id|ext_put_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
macro_line|#ifdef EXTFS_BITMAP
r_int
id|i
suffix:semicolon
macro_line|#endif
id|lock_super
c_func
(paren
id|sb
)paren
suffix:semicolon
id|sb-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
macro_line|#ifdef EXTFS_BITMAP
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|EXT_I_MAP_SLOTS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|brelse
c_func
(paren
id|sb-&gt;u.ext_sb.s_imap
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|EXT_Z_MAP_SLOTS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|brelse
c_func
(paren
id|sb-&gt;u.ext_sb.s_zmap
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
macro_line|#endif
macro_line|#ifdef EXTFS_FREELIST
r_if
c_cond
(paren
id|sb-&gt;u.ext_sb.s_imap
(braket
l_int|1
)braket
)paren
id|brelse
(paren
id|sb-&gt;u.ext_sb.s_imap
(braket
l_int|1
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sb-&gt;u.ext_sb.s_zmap
(braket
l_int|1
)braket
)paren
id|brelse
(paren
id|sb-&gt;u.ext_sb.s_zmap
(braket
l_int|1
)braket
)paren
suffix:semicolon
macro_line|#endif
id|free_super
c_func
(paren
id|sb
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
DECL|variable|ext_sops
r_static
r_struct
id|super_operations
id|ext_sops
op_assign
(brace
id|ext_read_inode
comma
id|ext_write_inode
comma
id|ext_put_inode
comma
id|ext_put_super
comma
id|ext_write_super
comma
id|ext_statfs
)brace
suffix:semicolon
DECL|function|ext_read_super
r_struct
id|super_block
op_star
id|ext_read_super
c_func
(paren
r_struct
id|super_block
op_star
id|s
comma
r_void
op_star
id|data
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext_super_block
op_star
id|es
suffix:semicolon
r_int
id|dev
op_assign
id|s-&gt;s_dev
comma
id|block
suffix:semicolon
macro_line|#ifdef EXTFS_BITMAP
r_int
id|i
suffix:semicolon
macro_line|#endif
id|lock_super
c_func
(paren
id|s
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|dev
comma
l_int|1
comma
id|BLOCK_SIZE
)paren
)paren
)paren
(brace
id|s-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|free_super
c_func
(paren
id|s
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;bread failed&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
multiline_comment|/*&t;*((struct ext_super_block *) s) =&n;&t;&t;*((struct ext_super_block *) bh-&gt;b_data); */
id|es
op_assign
(paren
r_struct
id|ext_super_block
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
id|s-&gt;s_blocksize
op_assign
l_int|1024
suffix:semicolon
id|s-&gt;u.ext_sb.s_ninodes
op_assign
id|es-&gt;s_ninodes
suffix:semicolon
id|s-&gt;u.ext_sb.s_nzones
op_assign
id|es-&gt;s_nzones
suffix:semicolon
macro_line|#ifdef EXTFS_BITMAP
id|s-&gt;u.ext_sb.s_imap_blocks
op_assign
id|es-&gt;s_imap_blocks
suffix:semicolon
id|s-&gt;u.ext_sb.s_zmap_blocks
op_assign
id|es-&gt;s_zmap_blocks
suffix:semicolon
macro_line|#endif
id|s-&gt;u.ext_sb.s_firstdatazone
op_assign
id|es-&gt;s_firstdatazone
suffix:semicolon
id|s-&gt;u.ext_sb.s_log_zone_size
op_assign
id|es-&gt;s_log_zone_size
suffix:semicolon
id|s-&gt;u.ext_sb.s_max_size
op_assign
id|es-&gt;s_max_size
suffix:semicolon
id|s-&gt;s_magic
op_assign
id|es-&gt;s_magic
suffix:semicolon
macro_line|#ifdef EXTFS_FREELIST
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|0
)braket
op_assign
(paren
r_struct
id|buffer_head
op_star
)paren
id|es-&gt;s_firstfreeblock
suffix:semicolon
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|2
)braket
op_assign
(paren
r_struct
id|buffer_head
op_star
)paren
id|es-&gt;s_freeblockscount
suffix:semicolon
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|0
)braket
op_assign
(paren
r_struct
id|buffer_head
op_star
)paren
id|es-&gt;s_firstfreeinode
suffix:semicolon
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|2
)braket
op_assign
(paren
r_struct
id|buffer_head
op_star
)paren
id|es-&gt;s_freeinodescount
suffix:semicolon
macro_line|#endif
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|s-&gt;s_magic
op_ne
id|EXT_SUPER_MAGIC
)paren
(brace
id|s-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|free_super
c_func
(paren
id|s
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;magic match failed&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
macro_line|#ifdef EXTFS_BITMAP
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|EXT_I_MAP_SLOTS
suffix:semicolon
id|i
op_increment
)paren
id|s-&gt;u.ext_sb.s_imap
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|EXT_Z_MAP_SLOTS
suffix:semicolon
id|i
op_increment
)paren
id|s-&gt;u.ext_sb.s_zmap
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
id|block
op_assign
l_int|2
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|s-&gt;u.ext_sb.s_imap_blocks
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|s-&gt;u.ext_sb.s_imap
(braket
id|i
)braket
op_assign
id|bread
c_func
(paren
id|dev
comma
id|block
comma
id|BLOCK_SIZE
)paren
)paren
id|block
op_increment
suffix:semicolon
r_else
r_break
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|s-&gt;u.ext_sb.s_zmap_blocks
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|s-&gt;u.ext_sb.s_zmap
(braket
id|i
)braket
op_assign
id|bread
c_func
(paren
id|dev
comma
id|block
comma
id|BLOCK_SIZE
)paren
)paren
id|block
op_increment
suffix:semicolon
r_else
r_break
suffix:semicolon
r_if
c_cond
(paren
id|block
op_ne
l_int|2
op_plus
id|s-&gt;u.ext_sb.s_imap_blocks
op_plus
id|s-&gt;u.ext_sb.s_zmap_blocks
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|EXT_I_MAP_SLOTS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|brelse
c_func
(paren
id|s-&gt;u.ext_sb.s_imap
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|EXT_Z_MAP_SLOTS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|brelse
c_func
(paren
id|s-&gt;u.ext_sb.s_zmap
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
id|s-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|free_super
c_func
(paren
id|s
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;block failed&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|0
)braket
op_member_access_from_pointer
id|b_data
(braket
l_int|0
)braket
op_or_assign
l_int|1
suffix:semicolon
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|0
)braket
op_member_access_from_pointer
id|b_data
(braket
l_int|0
)braket
op_or_assign
l_int|1
suffix:semicolon
macro_line|#endif
macro_line|#ifdef EXTFS_FREELIST
r_if
c_cond
(paren
op_logical_neg
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|0
)braket
)paren
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|1
)braket
op_assign
l_int|NULL
suffix:semicolon
r_else
r_if
c_cond
(paren
op_logical_neg
(paren
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|1
)braket
op_assign
id|bread
c_func
(paren
id|dev
comma
(paren
r_int
r_int
)paren
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|0
)braket
comma
id|BLOCK_SIZE
)paren
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext_read_super: unable to read first free block&bslash;n&quot;
)paren
suffix:semicolon
id|s-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|free_super
c_func
(paren
id|s
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|0
)braket
)paren
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|1
)braket
op_assign
l_int|NULL
suffix:semicolon
r_else
(brace
id|block
op_assign
l_int|2
op_plus
(paren
(paren
(paren
r_int
r_int
)paren
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|0
)braket
)paren
op_minus
l_int|1
)paren
op_div
id|EXT_INODES_PER_BLOCK
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|s-&gt;u.ext_sb.s_imap
(braket
l_int|1
)braket
op_assign
id|bread
c_func
(paren
id|dev
comma
id|block
comma
id|BLOCK_SIZE
)paren
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext_read_super: unable to read first free inode block&bslash;n&quot;
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|s-&gt;u.ext_sb.s_zmap
(braket
l_int|1
)braket
)paren
suffix:semicolon
id|s-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|free_super
(paren
id|s
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
)brace
macro_line|#endif
id|free_super
c_func
(paren
id|s
)paren
suffix:semicolon
multiline_comment|/* set up enough so that it can read an inode */
id|s-&gt;s_dev
op_assign
id|dev
suffix:semicolon
id|s-&gt;s_op
op_assign
op_amp
id|ext_sops
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|s-&gt;s_mounted
op_assign
id|iget
c_func
(paren
id|dev
comma
id|EXT_ROOT_INO
)paren
)paren
)paren
(brace
id|s-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;get root inode failed&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
r_return
id|s
suffix:semicolon
)brace
DECL|function|ext_write_super
r_void
id|ext_write_super
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
macro_line|#ifdef EXTFS_FREELIST
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext_super_block
op_star
id|es
suffix:semicolon
macro_line|#ifdef EXTFS_DEBUG
id|printk
(paren
l_string|&quot;ext_write_super called&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|sb-&gt;s_dev
comma
l_int|1
comma
id|BLOCK_SIZE
)paren
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext_write_super: bread failed&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|es
op_assign
(paren
r_struct
id|ext_super_block
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
id|es-&gt;s_firstfreeblock
op_assign
(paren
r_int
r_int
)paren
id|sb-&gt;u.ext_sb.s_zmap
(braket
l_int|0
)braket
suffix:semicolon
id|es-&gt;s_freeblockscount
op_assign
(paren
r_int
r_int
)paren
id|sb-&gt;u.ext_sb.s_zmap
(braket
l_int|2
)braket
suffix:semicolon
id|es-&gt;s_firstfreeinode
op_assign
(paren
r_int
r_int
)paren
id|sb-&gt;u.ext_sb.s_imap
(braket
l_int|0
)braket
suffix:semicolon
id|es-&gt;s_freeinodescount
op_assign
(paren
r_int
r_int
)paren
id|sb-&gt;u.ext_sb.s_imap
(braket
l_int|2
)braket
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|sb-&gt;s_dirt
op_assign
l_int|0
suffix:semicolon
macro_line|#endif
)brace
DECL|function|ext_statfs
r_void
id|ext_statfs
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|statfs
op_star
id|buf
)paren
(brace
r_int
id|tmp
suffix:semicolon
id|put_fs_long
c_func
(paren
id|EXT_SUPER_MAGIC
comma
op_amp
id|buf-&gt;f_type
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
l_int|1024
comma
op_amp
id|buf-&gt;f_bsize
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
id|sb-&gt;u.ext_sb.s_nzones
op_lshift
id|sb-&gt;u.ext_sb.s_log_zone_size
comma
op_amp
id|buf-&gt;f_blocks
)paren
suffix:semicolon
id|tmp
op_assign
id|ext_count_free_blocks
c_func
(paren
id|sb
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
id|tmp
comma
op_amp
id|buf-&gt;f_bfree
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
id|tmp
comma
op_amp
id|buf-&gt;f_bavail
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
id|sb-&gt;u.ext_sb.s_ninodes
comma
op_amp
id|buf-&gt;f_files
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
id|ext_count_free_inodes
c_func
(paren
id|sb
)paren
comma
op_amp
id|buf-&gt;f_ffree
)paren
suffix:semicolon
multiline_comment|/* Don&squot;t know what value to put in buf-&gt;f_fsid */
)brace
DECL|function|_ext_bmap
r_static
r_int
id|_ext_bmap
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_int
id|block
comma
r_int
id|create
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_int
id|i
suffix:semicolon
r_if
c_cond
(paren
id|block
OL
l_int|0
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;_ext_bmap: block&lt;0&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|block
op_ge
l_int|9
op_plus
l_int|256
op_plus
l_int|256
op_star
l_int|256
op_plus
l_int|256
op_star
l_int|256
op_star
l_int|256
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;_ext_bmap: block&gt;big&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|block
OL
l_int|9
)paren
(brace
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|inode-&gt;i_data
(braket
id|block
)braket
)paren
r_if
c_cond
(paren
id|inode-&gt;i_data
(braket
id|block
)braket
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
id|inode-&gt;i_ctime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
)brace
r_return
id|inode-&gt;i_data
(braket
id|block
)braket
suffix:semicolon
)brace
id|block
op_sub_assign
l_int|9
suffix:semicolon
r_if
c_cond
(paren
id|block
OL
l_int|256
)paren
(brace
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|inode-&gt;i_data
(braket
l_int|9
)braket
)paren
r_if
c_cond
(paren
id|inode-&gt;i_data
(braket
l_int|9
)braket
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_ctime
op_assign
id|CURRENT_TIME
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|inode-&gt;i_data
(braket
l_int|9
)braket
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|inode-&gt;i_data
(braket
l_int|9
)braket
comma
id|BLOCK_SIZE
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
r_int
op_star
)paren
(paren
id|bh-&gt;b_data
)paren
)paren
(braket
id|block
)braket
suffix:semicolon
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|i
)paren
r_if
c_cond
(paren
id|i
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
(paren
(paren
r_int
r_int
op_star
)paren
(paren
id|bh-&gt;b_data
)paren
)paren
(braket
id|block
)braket
op_assign
id|i
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_return
id|i
suffix:semicolon
)brace
id|block
op_sub_assign
l_int|256
suffix:semicolon
r_if
c_cond
(paren
id|block
OL
l_int|256
op_star
l_int|256
)paren
(brace
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|inode-&gt;i_data
(braket
l_int|10
)braket
)paren
r_if
c_cond
(paren
id|inode-&gt;i_data
(braket
l_int|10
)braket
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_ctime
op_assign
id|CURRENT_TIME
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|inode-&gt;i_data
(braket
l_int|10
)braket
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|inode-&gt;i_data
(braket
l_int|10
)braket
comma
id|BLOCK_SIZE
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
id|block
op_rshift
l_int|8
)braket
suffix:semicolon
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|i
)paren
r_if
c_cond
(paren
id|i
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
(paren
(paren
r_int
r_int
op_star
)paren
(paren
id|bh-&gt;b_data
)paren
)paren
(braket
id|block
op_rshift
l_int|8
)braket
op_assign
id|i
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|i
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|i
comma
id|BLOCK_SIZE
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
id|block
op_amp
l_int|255
)braket
suffix:semicolon
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|i
)paren
r_if
c_cond
(paren
id|i
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
(paren
(paren
r_int
r_int
op_star
)paren
(paren
id|bh-&gt;b_data
)paren
)paren
(braket
id|block
op_amp
l_int|255
)braket
op_assign
id|i
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_return
id|i
suffix:semicolon
)brace
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|inode-&gt;i_data
(braket
l_int|11
)braket
)paren
r_if
c_cond
(paren
id|inode-&gt;i_data
(braket
l_int|11
)braket
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_ctime
op_assign
id|CURRENT_TIME
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|inode-&gt;i_data
(braket
l_int|11
)braket
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|inode-&gt;i_data
(braket
l_int|11
)braket
comma
id|BLOCK_SIZE
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
id|block
op_rshift
l_int|16
)braket
suffix:semicolon
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|i
)paren
r_if
c_cond
(paren
id|i
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
id|block
op_rshift
l_int|16
)braket
op_assign
id|i
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|i
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|i
comma
id|BLOCK_SIZE
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
(paren
id|block
op_rshift
l_int|8
)paren
op_amp
l_int|255
)braket
suffix:semicolon
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|i
)paren
r_if
c_cond
(paren
id|i
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
(paren
id|block
op_rshift
l_int|8
)paren
op_amp
l_int|255
)braket
op_assign
id|i
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|i
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|i
comma
id|BLOCK_SIZE
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
id|block
op_amp
l_int|255
)braket
suffix:semicolon
r_if
c_cond
(paren
id|create
op_logical_and
op_logical_neg
id|i
)paren
r_if
c_cond
(paren
id|i
op_assign
id|ext_new_block
c_func
(paren
id|inode-&gt;i_dev
)paren
)paren
(brace
(paren
(paren
r_int
r_int
op_star
)paren
id|bh-&gt;b_data
)paren
(braket
id|block
op_amp
l_int|255
)braket
op_assign
id|i
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
id|i
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;ext_bmap: triple indirection not yet implemented&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ext_bmap
r_int
id|ext_bmap
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_int
id|block
)paren
(brace
r_return
id|_ext_bmap
c_func
(paren
id|inode
comma
id|block
comma
l_int|0
)paren
suffix:semicolon
)brace
DECL|function|ext_create_block
r_int
id|ext_create_block
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_int
id|block
)paren
(brace
r_return
id|_ext_bmap
c_func
(paren
id|inode
comma
id|block
comma
l_int|1
)paren
suffix:semicolon
)brace
DECL|function|ext_read_inode
r_void
id|ext_read_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext_inode
op_star
id|raw_inode
suffix:semicolon
r_int
id|block
suffix:semicolon
macro_line|#ifdef EXTFS_BITMAP
id|block
op_assign
l_int|2
op_plus
id|inode-&gt;i_sb-&gt;u.ext_sb.s_imap_blocks
op_plus
id|inode-&gt;i_sb-&gt;u.ext_sb.s_zmap_blocks
op_plus
(paren
id|inode-&gt;i_ino
op_minus
l_int|1
)paren
op_div
id|EXT_INODES_PER_BLOCK
suffix:semicolon
macro_line|#endif
macro_line|#ifdef EXTFS_FREELIST
id|block
op_assign
l_int|2
op_plus
(paren
id|inode-&gt;i_ino
op_minus
l_int|1
)paren
op_div
id|EXT_INODES_PER_BLOCK
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|block
comma
id|BLOCK_SIZE
)paren
)paren
)paren
id|panic
c_func
(paren
l_string|&quot;unable to read i-node block&quot;
)paren
suffix:semicolon
id|raw_inode
op_assign
(paren
(paren
r_struct
id|ext_inode
op_star
)paren
id|bh-&gt;b_data
)paren
op_plus
(paren
id|inode-&gt;i_ino
op_minus
l_int|1
)paren
op_mod
id|EXT_INODES_PER_BLOCK
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|raw_inode-&gt;i_mode
suffix:semicolon
id|inode-&gt;i_uid
op_assign
id|raw_inode-&gt;i_uid
suffix:semicolon
id|inode-&gt;i_gid
op_assign
id|raw_inode-&gt;i_gid
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
id|raw_inode-&gt;i_nlinks
suffix:semicolon
id|inode-&gt;i_size
op_assign
id|raw_inode-&gt;i_size
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|inode-&gt;i_atime
op_assign
id|inode-&gt;i_ctime
op_assign
id|raw_inode-&gt;i_time
suffix:semicolon
r_if
c_cond
(paren
id|S_ISCHR
c_func
(paren
id|inode-&gt;i_mode
)paren
op_logical_or
id|S_ISBLK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_rdev
op_assign
id|raw_inode-&gt;i_zone
(braket
l_int|0
)braket
suffix:semicolon
r_else
r_for
c_loop
(paren
id|block
op_assign
l_int|0
suffix:semicolon
id|block
OL
l_int|12
suffix:semicolon
id|block
op_increment
)paren
id|inode-&gt;i_data
(braket
id|block
)braket
op_assign
id|raw_inode-&gt;i_zone
(braket
id|block
)braket
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
id|inode-&gt;i_op
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|S_ISREG
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext_file_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext_dir_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISLNK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext_symlink_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISCHR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext_chrdev_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext_blkdev_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISFIFO
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
id|inode-&gt;i_op
op_assign
op_amp
id|ext_fifo_inode_operations
suffix:semicolon
id|inode-&gt;i_size
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_pipe
op_assign
l_int|1
suffix:semicolon
id|PIPE_HEAD
c_func
(paren
op_star
id|inode
)paren
op_assign
id|PIPE_TAIL
c_func
(paren
op_star
id|inode
)paren
op_assign
l_int|0
suffix:semicolon
id|PIPE_READERS
c_func
(paren
op_star
id|inode
)paren
op_assign
id|PIPE_WRITERS
c_func
(paren
op_star
id|inode
)paren
op_assign
l_int|0
suffix:semicolon
)brace
)brace
DECL|function|ext_write_inode
r_void
id|ext_write_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext_inode
op_star
id|raw_inode
suffix:semicolon
r_int
id|block
suffix:semicolon
macro_line|#ifdef EXTFS_BITMAP
id|block
op_assign
l_int|2
op_plus
id|inode-&gt;i_sb-&gt;u.ext_sb.s_imap_blocks
op_plus
id|inode-&gt;i_sb-&gt;u.ext_sb.s_zmap_blocks
op_plus
(paren
id|inode-&gt;i_ino
op_minus
l_int|1
)paren
op_div
id|EXT_INODES_PER_BLOCK
suffix:semicolon
macro_line|#endif
macro_line|#ifdef EXTFS_FREELIST
id|block
op_assign
l_int|2
op_plus
(paren
id|inode-&gt;i_ino
op_minus
l_int|1
)paren
op_div
id|EXT_INODES_PER_BLOCK
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|block
comma
id|BLOCK_SIZE
)paren
)paren
)paren
id|panic
c_func
(paren
l_string|&quot;unable to read i-node block&quot;
)paren
suffix:semicolon
id|raw_inode
op_assign
(paren
(paren
r_struct
id|ext_inode
op_star
)paren
id|bh-&gt;b_data
)paren
op_plus
(paren
id|inode-&gt;i_ino
op_minus
l_int|1
)paren
op_mod
id|EXT_INODES_PER_BLOCK
suffix:semicolon
id|raw_inode-&gt;i_mode
op_assign
id|inode-&gt;i_mode
suffix:semicolon
id|raw_inode-&gt;i_uid
op_assign
id|inode-&gt;i_uid
suffix:semicolon
id|raw_inode-&gt;i_gid
op_assign
id|inode-&gt;i_gid
suffix:semicolon
id|raw_inode-&gt;i_nlinks
op_assign
id|inode-&gt;i_nlink
suffix:semicolon
id|raw_inode-&gt;i_size
op_assign
id|inode-&gt;i_size
suffix:semicolon
id|raw_inode-&gt;i_time
op_assign
id|inode-&gt;i_mtime
suffix:semicolon
r_if
c_cond
(paren
id|S_ISCHR
c_func
(paren
id|inode-&gt;i_mode
)paren
op_logical_or
id|S_ISBLK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|raw_inode-&gt;i_zone
(braket
l_int|0
)braket
op_assign
id|inode-&gt;i_rdev
suffix:semicolon
r_else
r_for
c_loop
(paren
id|block
op_assign
l_int|0
suffix:semicolon
id|block
OL
l_int|12
suffix:semicolon
id|block
op_increment
)paren
id|raw_inode-&gt;i_zone
(braket
id|block
)braket
op_assign
id|inode-&gt;i_data
(braket
id|block
)braket
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|0
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
)brace
eof
