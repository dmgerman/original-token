multiline_comment|/*&n; * fs/dcache.c&n; *&n; * Complete reimplementation&n; * (C) 1997 Thomas Schoebel-Theuer&n; */
multiline_comment|/*&n; * Notes on the allocation strategy:&n; *&n; * The dcache is a master of the icache - whenever a dcache entry&n; * exists, the inode will always exist. &quot;iput()&quot; is done either when&n; * the dcache entry is deleted or garbage collected.&n; */
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/fs.h&gt;
macro_line|#include &lt;linux/malloc.h&gt;
multiline_comment|/*&n; * This is the single most critical data structure when it comes&n; * to the dcache: the hashtable for lookups. Somebody should try&n; * to make this good - I&squot;ve just made it work.&n; *&n; * This hash-function tries to avoid losing too many bits of hash&n; * information, yet avoid using a prime hash-size or similar.&n; */
DECL|macro|D_HASHBITS
mdefine_line|#define D_HASHBITS     10
DECL|macro|D_HASHSIZE
mdefine_line|#define D_HASHSIZE     (1UL &lt;&lt; D_HASHBITS)
DECL|macro|D_HASHMASK
mdefine_line|#define D_HASHMASK     (D_HASHSIZE-1)
DECL|variable|dentry_hashtable
r_struct
id|list_head
id|dentry_hashtable
(braket
id|D_HASHSIZE
)braket
suffix:semicolon
DECL|function|d_free
r_void
id|d_free
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
r_if
c_cond
(paren
id|dentry
)paren
(brace
id|kfree
c_func
(paren
id|dentry-&gt;d_name.name
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dentry
)paren
suffix:semicolon
)brace
)brace
DECL|function|dput
r_void
id|dput
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
id|repeat
suffix:colon
r_if
c_cond
(paren
id|dentry
)paren
(brace
id|dentry-&gt;d_count
op_decrement
suffix:semicolon
r_if
c_cond
(paren
id|dentry-&gt;d_count
OL
l_int|0
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;dentry-&gt;count = %d for %s&bslash;n&quot;
comma
id|dentry-&gt;d_count
comma
id|dentry-&gt;d_name.name
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/*&n;&t;&t; * This is broken right now: we should really put&n;&t;&t; * the dentry on a free list to be reclaimed later&n;&t;&t; * when we think we should throw it away.&n;&t;&t; *&n;&t;&t; * Instead we free it completely if the inode count&n;&t;&t; * indicates that we&squot;re the only ones holding onto&n;&t;&t; * the inode - if not we just fall back on the old&n;&t;&t; * (broken) behaviour of not reclaiming it at all.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|dentry-&gt;d_count
op_logical_and
(paren
op_logical_neg
id|dentry-&gt;d_inode
op_logical_or
id|atomic_read
c_func
(paren
op_amp
id|dentry-&gt;d_inode-&gt;i_count
)paren
op_eq
l_int|1
)paren
)paren
(brace
r_struct
id|dentry
op_star
id|parent
op_assign
id|dentry-&gt;d_parent
suffix:semicolon
r_if
c_cond
(paren
id|parent
op_ne
id|dentry
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_if
c_cond
(paren
id|inode
)paren
(brace
id|list_del
c_func
(paren
op_amp
id|dentry-&gt;d_list
)paren
suffix:semicolon
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
id|dentry-&gt;d_inode
op_assign
l_int|NULL
suffix:semicolon
)brace
id|list_del
c_func
(paren
op_amp
id|dentry-&gt;d_hash
)paren
suffix:semicolon
id|d_free
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|dentry
op_assign
id|parent
suffix:semicolon
r_goto
id|repeat
suffix:semicolon
)brace
)brace
)brace
)brace
DECL|macro|NAME_ALLOC_LEN
mdefine_line|#define NAME_ALLOC_LEN(len)&t;((len+16) &amp; ~15)
DECL|function|d_alloc
r_struct
id|dentry
op_star
id|d_alloc
c_func
(paren
r_struct
id|dentry
op_star
id|parent
comma
r_const
r_struct
id|qstr
op_star
id|name
)paren
(brace
r_char
op_star
id|str
suffix:semicolon
r_struct
id|dentry
op_star
id|res
suffix:semicolon
id|res
op_assign
id|kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|dentry
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|res
)paren
r_return
l_int|NULL
suffix:semicolon
id|str
op_assign
id|kmalloc
c_func
(paren
id|NAME_ALLOC_LEN
c_func
(paren
id|name-&gt;len
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|str
)paren
(brace
id|kfree
c_func
(paren
id|res
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
id|memcpy
c_func
(paren
id|str
comma
id|name-&gt;name
comma
id|name-&gt;len
)paren
suffix:semicolon
id|str
(braket
id|name-&gt;len
)braket
op_assign
l_int|0
suffix:semicolon
id|memset
c_func
(paren
id|res
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|dentry
)paren
)paren
suffix:semicolon
id|res-&gt;d_parent
op_assign
id|parent
suffix:semicolon
id|res-&gt;d_mounts
op_assign
id|res
suffix:semicolon
id|res-&gt;d_covers
op_assign
id|res
suffix:semicolon
id|res-&gt;d_flags
op_assign
l_int|0
suffix:semicolon
id|res-&gt;d_name.name
op_assign
id|str
suffix:semicolon
id|res-&gt;d_name.len
op_assign
id|name-&gt;len
suffix:semicolon
id|res-&gt;d_name.hash
op_assign
id|name-&gt;hash
suffix:semicolon
r_return
id|res
suffix:semicolon
)brace
multiline_comment|/*&n; * Fill in inode information in the entry.&n; *&n; * This turns negative dentries into productive full members&n; * of society.&n; *&n; * NOTE! This assumes that the inode count has been incremented&n; * (or otherwise set) by the caller to indicate that it is now&n; * in use by the dcache..&n; */
DECL|function|d_instantiate
r_void
id|d_instantiate
c_func
(paren
r_struct
id|dentry
op_star
id|entry
comma
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_if
c_cond
(paren
id|inode
)paren
id|list_add
c_func
(paren
op_amp
id|entry-&gt;d_list
comma
op_amp
id|inode-&gt;i_dentry
)paren
suffix:semicolon
id|entry-&gt;d_inode
op_assign
id|inode
suffix:semicolon
)brace
DECL|function|d_alloc_root
r_struct
id|dentry
op_star
id|d_alloc_root
c_func
(paren
r_struct
id|inode
op_star
id|root_inode
comma
r_struct
id|dentry
op_star
id|old_root
)paren
(brace
r_struct
id|dentry
op_star
id|res
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|root_inode
)paren
(brace
id|res
op_assign
id|d_alloc
c_func
(paren
l_int|NULL
comma
op_amp
(paren
r_const
r_struct
id|qstr
)paren
(brace
l_string|&quot;/&quot;
comma
l_int|1
comma
l_int|0
)brace
)paren
suffix:semicolon
id|res-&gt;d_parent
op_assign
id|res
suffix:semicolon
id|d_instantiate
c_func
(paren
id|res
comma
id|root_inode
)paren
suffix:semicolon
)brace
r_return
id|res
suffix:semicolon
)brace
DECL|function|d_hash
r_static
r_inline
r_struct
id|list_head
op_star
id|d_hash
c_func
(paren
r_struct
id|dentry
op_star
id|parent
comma
r_int
r_int
id|hash
)paren
(brace
id|hash
op_add_assign
(paren
r_int
r_int
)paren
id|parent
suffix:semicolon
id|hash
op_assign
id|hash
op_xor
(paren
id|hash
op_rshift
id|D_HASHBITS
)paren
op_xor
(paren
id|hash
op_rshift
id|D_HASHBITS
op_star
l_int|2
)paren
suffix:semicolon
r_return
id|dentry_hashtable
op_plus
(paren
id|hash
op_amp
id|D_HASHMASK
)paren
suffix:semicolon
)brace
DECL|function|__dlookup
r_static
r_inline
r_struct
id|dentry
op_star
id|__dlookup
c_func
(paren
r_struct
id|list_head
op_star
id|head
comma
r_struct
id|dentry
op_star
id|parent
comma
r_struct
id|qstr
op_star
id|name
)paren
(brace
r_struct
id|list_head
op_star
id|tmp
op_assign
id|head-&gt;next
suffix:semicolon
r_int
id|len
op_assign
id|name-&gt;len
suffix:semicolon
r_int
id|hash
op_assign
id|name-&gt;hash
suffix:semicolon
r_const
r_int
r_char
op_star
id|str
op_assign
id|name-&gt;name
suffix:semicolon
r_while
c_loop
(paren
id|tmp
op_ne
id|head
)paren
(brace
r_struct
id|dentry
op_star
id|dentry
op_assign
id|list_entry
c_func
(paren
id|tmp
comma
r_struct
id|dentry
comma
id|d_hash
)paren
suffix:semicolon
id|tmp
op_assign
id|tmp-&gt;next
suffix:semicolon
r_if
c_cond
(paren
id|dentry-&gt;d_name.hash
op_ne
id|hash
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|dentry-&gt;d_name.len
op_ne
id|len
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|dentry-&gt;d_parent
op_ne
id|parent
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|memcmp
c_func
(paren
id|dentry-&gt;d_name.name
comma
id|str
comma
id|len
)paren
)paren
r_continue
suffix:semicolon
r_return
id|dentry
suffix:semicolon
)brace
r_return
l_int|NULL
suffix:semicolon
)brace
DECL|function|d_lookup
r_struct
id|dentry
op_star
id|d_lookup
c_func
(paren
r_struct
id|dentry
op_star
id|dir
comma
r_struct
id|qstr
op_star
id|name
)paren
(brace
r_return
id|__dlookup
c_func
(paren
id|d_hash
c_func
(paren
id|dir
comma
id|name-&gt;hash
)paren
comma
id|dir
comma
id|name
)paren
suffix:semicolon
)brace
DECL|function|d_insert_to_parent
r_static
r_inline
r_void
id|d_insert_to_parent
c_func
(paren
r_struct
id|dentry
op_star
id|entry
comma
r_struct
id|dentry
op_star
id|parent
)paren
(brace
id|list_add
c_func
(paren
op_amp
id|entry-&gt;d_hash
comma
id|d_hash
c_func
(paren
id|dget
c_func
(paren
id|parent
)paren
comma
id|entry-&gt;d_name.hash
)paren
)paren
suffix:semicolon
)brace
DECL|function|d_remove_from_parent
r_static
r_inline
r_void
id|d_remove_from_parent
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|dentry
op_star
id|parent
)paren
(brace
id|list_del
c_func
(paren
op_amp
id|dentry-&gt;d_hash
)paren
suffix:semicolon
id|dput
c_func
(paren
id|parent
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Remove the inode from the dentry.. This removes&n; * it from the parent hashes but otherwise leaves it&n; * around - it may be a &quot;zombie&quot;, part of a path&n; * that is still in use...&n; *&n; * &quot;The Night of the Living Dead IV - the Dentry&quot;&n; */
DECL|function|d_delete
r_void
id|d_delete
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
id|list_del
c_func
(paren
op_amp
id|dentry-&gt;d_hash
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Make the hash lists point to itself.. When we&n;&t; * later dput this, we want the list_del() there&n;&t; * to not do anything strange..&n;&t; */
id|INIT_LIST_HEAD
c_func
(paren
op_amp
id|dentry-&gt;d_hash
)paren
suffix:semicolon
)brace
DECL|function|d_add
r_void
id|d_add
c_func
(paren
r_struct
id|dentry
op_star
id|entry
comma
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|d_insert_to_parent
c_func
(paren
id|entry
comma
id|entry-&gt;d_parent
)paren
suffix:semicolon
id|d_instantiate
c_func
(paren
id|entry
comma
id|inode
)paren
suffix:semicolon
)brace
DECL|function|alloc_new_name
r_static
r_inline
r_void
id|alloc_new_name
c_func
(paren
r_struct
id|dentry
op_star
id|entry
comma
r_struct
id|qstr
op_star
id|newname
)paren
(brace
r_int
id|len
op_assign
id|newname-&gt;len
suffix:semicolon
r_int
id|hash
op_assign
id|newname-&gt;hash
suffix:semicolon
r_char
op_star
id|name
op_assign
(paren
r_char
op_star
)paren
id|entry-&gt;d_name.name
suffix:semicolon
r_if
c_cond
(paren
id|NAME_ALLOC_LEN
c_func
(paren
id|len
)paren
op_ne
id|NAME_ALLOC_LEN
c_func
(paren
id|entry-&gt;d_name.len
)paren
)paren
(brace
id|name
op_assign
id|kmalloc
c_func
(paren
id|NAME_ALLOC_LEN
c_func
(paren
id|len
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|name
)paren
id|printk
c_func
(paren
l_string|&quot;out of memory for dcache&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|entry-&gt;d_name.name
)paren
suffix:semicolon
id|entry-&gt;d_name.name
op_assign
id|name
suffix:semicolon
)brace
id|memcpy
c_func
(paren
id|name
comma
id|newname-&gt;name
comma
id|len
)paren
suffix:semicolon
id|name
(braket
id|len
)braket
op_assign
l_int|0
suffix:semicolon
id|entry-&gt;d_name.len
op_assign
id|len
suffix:semicolon
id|entry-&gt;d_name.hash
op_assign
id|hash
suffix:semicolon
)brace
DECL|function|d_move
r_void
id|d_move
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|dentry
op_star
id|newdir
comma
r_struct
id|qstr
op_star
id|newname
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|dentry
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dentry-&gt;d_inode
)paren
id|printk
c_func
(paren
l_string|&quot;VFS: moving negative dcache entry&bslash;n&quot;
)paren
suffix:semicolon
id|d_remove_from_parent
c_func
(paren
id|dentry
comma
id|dentry-&gt;d_parent
)paren
suffix:semicolon
id|alloc_new_name
c_func
(paren
id|dentry
comma
id|newname
)paren
suffix:semicolon
id|dentry-&gt;d_parent
op_assign
id|newdir
suffix:semicolon
id|d_insert_to_parent
c_func
(paren
id|dentry
comma
id|newdir
)paren
suffix:semicolon
)brace
DECL|function|d_path
r_int
id|d_path
c_func
(paren
r_struct
id|dentry
op_star
id|entry
comma
r_struct
id|dentry
op_star
id|chroot
comma
r_char
op_star
id|buf
)paren
(brace
r_if
c_cond
(paren
id|IS_ROOT
c_func
(paren
id|entry
)paren
op_logical_or
(paren
id|chroot
op_logical_and
id|entry
op_eq
id|chroot
)paren
)paren
(brace
op_star
id|buf
op_assign
l_char|&squot;/&squot;
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
r_else
(brace
r_int
id|len
op_assign
id|d_path
c_func
(paren
id|entry-&gt;d_parent
comma
id|chroot
comma
id|buf
)paren
suffix:semicolon
id|buf
op_add_assign
id|len
suffix:semicolon
r_if
c_cond
(paren
id|len
OG
l_int|1
)paren
(brace
op_star
id|buf
op_increment
op_assign
l_char|&squot;/&squot;
suffix:semicolon
id|len
op_increment
suffix:semicolon
)brace
id|memcpy
c_func
(paren
id|buf
comma
id|entry-&gt;d_name.name
comma
id|entry-&gt;d_name.len
)paren
suffix:semicolon
r_return
id|len
op_plus
id|entry-&gt;d_name.len
suffix:semicolon
)brace
)brace
DECL|function|dcache_init
r_void
id|dcache_init
c_func
(paren
r_void
)paren
(brace
r_int
id|i
suffix:semicolon
r_struct
id|list_head
op_star
id|d
op_assign
id|dentry_hashtable
suffix:semicolon
id|i
op_assign
id|D_HASHSIZE
suffix:semicolon
r_do
(brace
id|INIT_LIST_HEAD
c_func
(paren
id|d
)paren
suffix:semicolon
id|d
op_increment
suffix:semicolon
id|i
op_decrement
suffix:semicolon
)brace
r_while
c_loop
(paren
id|i
)paren
suffix:semicolon
)brace
eof
