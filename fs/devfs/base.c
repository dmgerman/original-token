...
mdefine_line|#define DEVFS_VERSION            &quot;0.92 (20000203)&quot;
macro_line|#ifndef DEVFS_NAME
DECL|macro|DEVFS_NAME
macro_line|#  define DEVFS_NAME &quot;devfs&quot;
macro_line|#endif
multiline_comment|/*  Compatibility for 2.2.x kernel series  */
macro_line|#if (LINUX_VERSION_CODE &lt; KERNEL_VERSION(2,3,1))
DECL|macro|init_waitqueue_head
macro_line|#  define init_waitqueue_head(p)  init_waitqueue(p)
DECL|macro|DECLARE_WAITQUEUE
macro_line|#  define DECLARE_WAITQUEUE(wait, p) struct wait_queue wait = {p, NULL}
...
suffix:semicolon
macro_line|#endif
macro_line|#if (LINUX_VERSION_CODE &lt; KERNEL_VERSION(2,3,6))
DECL|macro|D_ALLOC_ROOT
macro_line|#  define D_ALLOC_ROOT(inode) d_alloc_root (inode, NULL)
macro_line|#else
DECL|macro|D_ALLOC_ROOT
macro_line|#  define D_ALLOC_ROOT(inode) d_alloc_root (inode)
macro_line|#endif
macro_line|#if (LINUX_VERSION_CODE &lt; KERNEL_VERSION(2,3,13))
DECL|macro|SETUP_STATIC
macro_line|#  define SETUP_STATIC
DECL|macro|__setup
macro_line|#  define __setup(a,b)
macro_line|#else
DECL|macro|SETUP_STATIC
macro_line|#  define SETUP_STATIC static
macro_line|#endif
...
id|prev
suffix:semicolon
multiline_comment|/*  This pair are used to associate a list of */
DECL|member|next
r_struct
id|devfs_inode
op_star
id|next
suffix:semicolon
multiline_comment|/*  inodes (one per FS) for a devfs entry     */
DECL|member|dentry
r_struct
id|dentry
op_star
...
DECL|function|get_devfs_inode_from_vfs_inode
r_static
r_struct
id|devfs_inode
op_star
id|get_devfs_inode_from_vfs_inode
(paren
r_struct
id|inode
op_star
...
multiline_comment|/*  If entry already exists free any dentries associated with it  */
r_if
c_cond
(paren
id|de-&gt;registered
...
c_cond
(paren
id|di-&gt;dentry-&gt;d_inode
op_eq
l_int|NULL
)paren
r_continue
suffix:semicolon
id|di-&gt;dentry-&gt;d_inode-&gt;i_size
op_assign
id|size
suffix:semicolon
)brace
...
id|printk
(paren
l_string|&quot;%s: update_devfs_inode_from_entry(): NULL entry&bslash;n&quot;
comma
id|DEVFS_NAME
)paren
suffix:semicolon
...
id|di-&gt;uid
op_assign
id|di-&gt;de-&gt;u.fcb.default_uid
suffix:semicolon
id|di-&gt;gid
op_assign
id|di-&gt;de-&gt;u.fcb.default_gid
suffix:semicolon
)brace
)brace
...
multiline_comment|/*  [SUMMARY] Create a devfs inode entry.&n;    &lt;entry&gt; The devfs entry to associate the new inode with.&n;    &lt;fs_info&gt; The FS info.&n;    [RETURNS] A pointer to the devfs inode on success, else NULL.&n;*/
(brace
r_struct
id|devfs_inode
op_star
...
id|printk
(paren
l_string|&quot;%s: create_devfs_inode(): grew inode table to: %u entries&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|fs_info-&gt;table_size
...
id|printk
(paren
l_string|&quot;%s: create_devfs_inode(): new di(%u): %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|di-&gt;ino
...
(brace
multiline_comment|/*  Unhook this inode from the devfs tree  */
id|di
op_assign
id|fs_info-&gt;table
(braket
...
id|printk
(paren
l_string|&quot;%s: read_inode(%d): VFS inode: %p  NO devfs_inode&bslash;n&quot;
comma
id|DEVFS_NAME
comma
(paren
...
id|printk
(paren
l_string|&quot;%s: read_inode(%d): VFS inode: %p  devfs_inode: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
(paren
...
id|MKDEV
(paren
id|di-&gt;de-&gt;u.fcb.u.device.major
comma
id|di-&gt;de-&gt;u.fcb.u.device.minor
)paren
suffix:semicolon
r_else
r_if
...
id|printk
(paren
l_string|&quot;%s: write_inode(%d): VFS inode: %p  devfs_inode: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
(paren
...
id|printk
(paren
l_string|&quot;%s: put_super(): devfs ptr: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|fs_info
...
id|printk
(paren
l_string|&quot;%s: get_vfs_inode(%u): old di-&gt;dentry: %p &bslash;&quot;%s&bslash;&quot;  new dentry: %p &bslash;&quot;%s&bslash;&quot;&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|di-&gt;ino
...
multiline_comment|/*  Must search for an inode for this FS  */
r_for
c_loop
(paren
id|di
...
r_continue
suffix:semicolon
multiline_comment|/*  Have to create the inode right now  */
id|di
op_assign
id|create_devfs_inode
(paren
...
l_int|0
suffix:semicolon
multiline_comment|/*  No fancy operations  */
...
id|printk
(paren
l_string|&quot;%s: d_iput(): dentry: %p inode: %p di: %p  di-&gt;dentry: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|dentry
...
id|printk
(paren
l_string|&quot;%s: d_delete(): dentry: %p  inode: %p  devfs_inode: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|dentry
...
(brace
multiline_comment|/*  Search for an inode for this FS  */
r_for
c_loop
(paren
id|di
...
id|printk
(paren
l_string|&quot;%s: d_revalidate(): new VFS inode(%u): %p  devfs_inode: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|di-&gt;ino
...
(brace
multiline_comment|/*  Make the dentry negative so a subsequent operation can deal&n;&t;&t;with it (for the benefit of mknod()). Leaving the dentry&n;&t;&t;unhashed will cause &lt;lock_parent&gt; to fail which in turns causes&n;&t;&t;&lt;do_mknod&gt; to fail  */
id|d_add
(paren
id|dentry
comma
...
id|printk
(paren
l_string|&quot;%s: lookup(): new VFS inode(%u): %p  devfs_inode: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|di-&gt;ino
...
id|printk
(paren
l_string|&quot;%s: readlink(): dentry: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|dentry
...
id|printk
(paren
l_string|&quot;%s: follow_link(): dentry: %p&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|dentry
...
id|printk
(paren
l_string|&quot;%s: read_super: root inode number is: %d!&bslash;n&quot;
comma
id|DEVFS_NAME
comma
id|di-&gt;ino
...
suffix:semicolon
macro_line|#if defined(CONFIG_DEVFS_DEBUG) &amp;&amp; !defined(MODULE)
id|devfs_debug
op_assign
id|devfs_debug_init
suffix:semicolon
...
id|printk
(paren
l_string|&quot;%s: free_entry(): unfreed inodes!&bslash;n&quot;
comma
id|DEVFS_NAME
)paren
suffix:semicolon
...
(brace
multiline_comment|/*  Recursively free the subdirectories: this is a stack chomper */
id|free_entry
(paren
id|de
)paren
...
multiline_comment|/*  End Function free_entry  */
...
id|printk
(paren
l_string|&quot;%s: cleanup_module(): still mounted mounted filesystems!&bslash;n&quot;
comma
id|DEVFS_NAME
)paren
suffix:semicolon
