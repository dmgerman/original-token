...
multiline_comment|/*&n; *&t;find_entry()&n; *&n; * finds and entry in the specified directory with the wanted name. It&n; * returns the cache buffer in which the entry was found, and the entry&n; * itself (as a parameter - res_dir). It does NOT read the inode of the&n; * entry - you&squot;ll have to do that yourself if you want to.&n; *&n; * This also takes care of the few special cases due to &squot;..&squot;-traversal&n; * over a pseudo-root and a mount point.&n; */
...
l_int|NULL
suffix:semicolon
multiline_comment|/* check for &squot;..&squot;, as we might have to do some &quot;magic&quot; for it */
r_if
c_cond
(paren
id|namelen
...
(brace
multiline_comment|/* &squot;..&squot; in a pseudo-root results in a faked &squot;.&squot; (just change namelen) */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* &squot;..&squot; over a mount-point results in &squot;dir&squot; being exchanged for the mounted&n;   directory-inode. NOTE! We set mounted, so that we can iput the new dir */
id|sb
op_assign
id|get_super
c_func
...
multiline_comment|/*&n;&t; * If the directory has the sticky bit, the user must either&n;&t; * own the file or own the directory or be the superuser to &n;&t; * delete a file in that directory.  This is typically used &n;&t; * for /tmp and /usr/tmp.&n;&t; */
r_if
c_cond
(paren
(paren
