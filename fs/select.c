...
multiline_comment|/*&n; * Due to kernel stack usage, we use a _limited_ fd_set type here, and once&n; * we really start supporting &gt;256 file descriptors we&squot;ll probably have to&n; * allocate the kernel fd_set copies dynamically.. (The kernel select routines&n; * are careful to touch only the defined low bits of any fd_set pointer, this&n; * is important for performance too).&n; *&n; * Note a few subtleties: we use &quot;long&quot; for the dummy, not int, and we do a&n; * subtract by 1 on the nr of file descriptors. The former is better for&n; * machines with long &gt; int, and the latter allows us to test the bit count&n; * against &quot;zero or positive&quot;, which can mostly be just a sign bit test..&n; */
r_typedef
r_struct
(brace
DECL|member|dummy
