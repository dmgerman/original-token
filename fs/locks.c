...
mdefine_line|#define OFFSET_MAX&t;(~(loff_t)0ULL &gt;&gt; 1)&t;/* FIXME: move elsewhere? */
r_static
r_int
id|flock_make_lock
c_func
...
l_int|NULL
suffix:semicolon
multiline_comment|/* Allocate a new lock, and initialize its fields from fl.&n; * The lock is not inserted into any lists until locks_insert_lock() or &n; * locks_insert_block() are called.&n; */
...
id|out
suffix:semicolon
multiline_comment|/* Convert to 64-bit offsets for internal use */
id|fl64.l_type
op_assign
id|flock.l_type
suffix:semicolon
...
id|out
suffix:semicolon
multiline_comment|/* and back again... */
id|flock.l_type
op_assign
id|fl64.l_type
suffix:semicolon
...
macro_line|#if BITS_PER_LONG == 32&t;/* LFS versions for 32 bit platforms */
...
id|repeat
suffix:colon
multiline_comment|/* Check signals each time we start */
id|error
op_assign
op_minus
id|ERESTARTSYS
...
(brace
multiline_comment|/* The new lock breaks the old one in two pieces,&n;&t;&t;&t; * so we have to use the second new lock (in this&n;&t;&t;&t; * case, even F_UNLCK may fail!).&n;&t;&t;&t; */
id|left
op_assign
id|locks_init_lock
c_func
...
id|out
suffix:colon
multiline_comment|/*&n;&t; * Free any unused locks.  (They haven&squot;t&n;&t; * ever been used, so we use kfree().)&n;&t; */
r_if
c_cond
(paren
id|new_fl
...
multiline_comment|/*&n; * Allocate an empty lock structure. We can use GFP_KERNEL now that&n; * all allocations are done in advance.&n; */
DECL|function|locks_empty_lock
r_static
r_struct
id|file_lock
op_star
