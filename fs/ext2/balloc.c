multiline_comment|/*&n; *  linux/fs/ext2/balloc.c&n; *&n; *  Copyright (C) 1992, 1993  Remy Card (card@masi.ibp.fr)&n; *&n; */
multiline_comment|/* balloc.c contains the blocks allocation and deallocation routines */
multiline_comment|/*&n;&n;   The free blocks are managed by bitmaps.  A file system contains several&n;   blocks groups.  Each group contains 1 bitmap block for blocks, 1 bitmap&n;   block for inodes, N blocks for the inode table and data blocks.&n;&n;   The file system contains group descriptors which are located after the&n;   super block.  Each descriptor contains the number of the bitmap block and&n;   the free blocks count in the block.  The descriptors are loaded in memory&n;   when a file system is mounted (see ext2_read_super).&n;&n;*/
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/ext2_fs.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/locks.h&gt;
DECL|macro|clear_block
mdefine_line|#define clear_block(addr,size) &bslash;&n;&t;__asm__(&quot;cld&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;rep&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;stosl&quot; &bslash;&n;&t;&t;: &bslash;&n;&t;&t;:&quot;a&quot; (0), &quot;c&quot; (size/4), &quot;D&quot; ((long) (addr)) &bslash;&n;&t;&t;:&quot;cx&quot;, &quot;di&quot;)
DECL|macro|set_bit
mdefine_line|#define set_bit(nr,addr) ( &bslash;&n;{ &bslash;&n;&t;char res; &bslash;&n;&t;__asm__ __volatile__(&quot;btsl %1,%2&bslash;n&bslash;tsetb %0&quot; &bslash;&n;&t;&t;&t;     :&quot;=q&quot; (res) &bslash;&n;&t;&t;&t;     :&quot;r&quot; (nr),&quot;m&quot; (*(addr))); &bslash;&n;&t;res; &bslash;&n;} &bslash;&n;)
DECL|macro|clear_bit
mdefine_line|#define clear_bit(nr,addr) ( &bslash;&n;{ &bslash;&n;&t;char res; &bslash;&n;&t;__asm__ __volatile__(&quot;btrl %1,%2&bslash;n&bslash;tsetnb %0&quot; &bslash;&n;&t;&t;&t;     :&quot;=q&quot; (res) &bslash;&n;&t;&t;&t;     :&quot;r&quot; (nr),&quot;m&quot; (*(addr))); &bslash;&n;&t;res; &bslash;&n;} &bslash;&n;)
DECL|macro|find_first_zero
mdefine_line|#define find_first_zero(addr,size) ( &bslash;&n;{ &bslash;&n;&t;int __res; &bslash;&n;&t;__asm__(&quot;cld&bslash;n&quot; &bslash;&n;&t;&t;&quot;1:&bslash;tlodsl&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;notl %%eax&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;bsfl %%eax,%%edx&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;jne 2f&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;addl $32,%%ecx&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;cmpl %%ebx,%%ecx&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;jl 1b&bslash;n&bslash;t&quot; &bslash;&n;&t;&t;&quot;xorl %%edx,%%edx&bslash;n&quot; &bslash;&n;&t;&t;&quot;2:&bslash;taddl %%edx,%%ecx&quot; &bslash;&n;&t;&t;:&quot;=c&quot; (__res):&quot;0&quot; (0), &quot;S&quot; (addr), &quot;b&quot; (size) &bslash;&n;&t;&t;:&quot;ax&quot;, &quot;bx&quot;, &quot;dx&quot;, &quot;si&quot;); &bslash;&n;&t;__res; &bslash;&n;} &bslash;&n;)
DECL|function|read_block_bitmap
r_static
r_void
id|read_block_bitmap
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
r_int
id|block_group
comma
r_int
r_int
id|bitmap_nr
)paren
(brace
r_int
r_int
id|group_desc
suffix:semicolon
r_int
r_int
id|desc
suffix:semicolon
r_struct
id|ext2_group_desc
op_star
id|gdp
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
id|group_desc
op_assign
id|block_group
op_div
id|EXT2_DESC_PER_BLOCK
c_func
(paren
id|sb
)paren
suffix:semicolon
id|desc
op_assign
id|block_group
op_mod
id|EXT2_DESC_PER_BLOCK
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
)paren
(brace
id|printk
(paren
l_string|&quot;block_group = %d,group_desc = %d,desc = %d&bslash;n&quot;
comma
id|block_group
comma
id|group_desc
comma
id|desc
)paren
suffix:semicolon
id|panic
(paren
l_string|&quot;read_block_bitmap: Group descriptor not loaded&quot;
)paren
suffix:semicolon
)brace
id|gdp
op_assign
(paren
r_struct
id|ext2_group_desc
op_star
)paren
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
op_member_access_from_pointer
id|b_data
suffix:semicolon
id|bh
op_assign
id|bread
(paren
id|sb-&gt;s_dev
comma
id|gdp
(braket
id|desc
)braket
dot
id|bg_block_bitmap
comma
id|sb-&gt;s_blocksize
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|printk
(paren
l_string|&quot;block_group = %d,group_desc = %d,desc = %d,block_bitmap = %d&bslash;n&quot;
comma
id|block_group
comma
id|group_desc
comma
id|desc
comma
id|gdp
(braket
id|desc
)braket
dot
id|bg_block_bitmap
)paren
suffix:semicolon
id|panic
(paren
l_string|&quot;read_block_bitmap: Cannot read block bitmap&quot;
)paren
suffix:semicolon
)brace
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|bitmap_nr
)braket
op_assign
id|block_group
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|bitmap_nr
)braket
op_assign
id|bh
suffix:semicolon
)brace
multiline_comment|/*&n; * load_block_bitmap loads the block bitmap for a blocks group&n; *&n; * It maintains a cache for the last bitmaps loaded.  This cache is managed&n; * with a LRU algorithm.&n; *&n; * Notes:&n; * 1/ There is one cache per mounted file system.&n; * 2/ If the file system contains less than EXT2_MAX_GROUP_LOADED groups,&n; *    this function reads the bitmap without maintaining a LRU cache.&n; */
DECL|function|load_block_bitmap
r_static
r_int
id|load_block_bitmap
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
r_int
id|block_group
)paren
(brace
r_int
id|i
comma
id|j
suffix:semicolon
r_int
r_int
id|block_bitmap_number
suffix:semicolon
r_struct
id|buffer_head
op_star
id|block_bitmap
suffix:semicolon
r_if
c_cond
(paren
id|block_group
op_ge
id|sb-&gt;u.ext2_sb.s_groups_count
)paren
(brace
id|printk
(paren
l_string|&quot;block_group = %d, groups_count = %d&bslash;n&quot;
comma
id|block_group
comma
id|sb-&gt;u.ext2_sb.s_groups_count
)paren
suffix:semicolon
id|panic
(paren
l_string|&quot;load_block_bitmap: block_group &gt;= groups_count&quot;
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|sb-&gt;u.ext2_sb.s_loaded_block_bitmaps
OG
l_int|0
op_logical_and
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
l_int|0
)braket
op_eq
id|block_group
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|sb-&gt;u.ext2_sb.s_groups_count
op_le
id|EXT2_MAX_GROUP_LOADED
)paren
(brace
r_if
c_cond
(paren
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|block_group
)braket
)paren
(brace
r_if
c_cond
(paren
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|block_group
)braket
op_ne
id|block_group
)paren
id|panic
(paren
l_string|&quot;load_block_bitmap: block_group != block_bitmap_number&quot;
)paren
suffix:semicolon
r_else
r_return
id|block_group
suffix:semicolon
)brace
r_else
(brace
id|read_block_bitmap
(paren
id|sb
comma
id|block_group
comma
id|block_group
)paren
suffix:semicolon
r_return
id|block_group
suffix:semicolon
)brace
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|sb-&gt;u.ext2_sb.s_loaded_block_bitmaps
op_logical_and
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|i
)braket
op_ne
id|block_group
suffix:semicolon
id|i
op_increment
)paren
suffix:semicolon
r_if
c_cond
(paren
id|i
OL
id|sb-&gt;u.ext2_sb.s_loaded_block_bitmaps
op_logical_and
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|i
)braket
op_eq
id|block_group
)paren
(brace
id|block_bitmap_number
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|i
)braket
suffix:semicolon
id|block_bitmap
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|i
)braket
suffix:semicolon
r_for
c_loop
(paren
id|j
op_assign
id|i
suffix:semicolon
id|j
OG
l_int|0
suffix:semicolon
id|j
op_decrement
)paren
(brace
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|j
)braket
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|j
op_minus
l_int|1
)braket
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|j
)braket
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|j
op_minus
l_int|1
)braket
suffix:semicolon
)brace
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
l_int|0
)braket
op_assign
id|block_bitmap_number
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
l_int|0
)braket
op_assign
id|block_bitmap
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
id|sb-&gt;u.ext2_sb.s_loaded_block_bitmaps
OL
id|EXT2_MAX_GROUP_LOADED
)paren
id|sb-&gt;u.ext2_sb.s_loaded_block_bitmaps
op_increment
suffix:semicolon
r_else
id|brelse
(paren
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|EXT2_MAX_GROUP_LOADED
op_minus
l_int|1
)braket
)paren
suffix:semicolon
r_for
c_loop
(paren
id|j
op_assign
id|sb-&gt;u.ext2_sb.s_loaded_block_bitmaps
op_minus
l_int|1
suffix:semicolon
id|j
OG
l_int|0
suffix:semicolon
id|j
op_decrement
)paren
(brace
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|j
)braket
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap_number
(braket
id|j
op_minus
l_int|1
)braket
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|j
)braket
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|j
op_minus
l_int|1
)braket
suffix:semicolon
)brace
id|read_block_bitmap
(paren
id|sb
comma
id|block_group
comma
l_int|0
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ext2_free_block
r_void
id|ext2_free_block
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
r_int
id|block
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh2
suffix:semicolon
r_int
r_int
id|block_group
suffix:semicolon
r_int
r_int
id|bit
suffix:semicolon
r_int
r_int
id|group_desc
suffix:semicolon
r_int
r_int
id|desc
suffix:semicolon
r_int
id|bitmap_nr
suffix:semicolon
r_struct
id|ext2_group_desc
op_star
id|gdp
suffix:semicolon
r_struct
id|ext2_super_block
op_star
id|es
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_free_block: nonexistant device&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|lock_super
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
id|block
OL
id|sb-&gt;u.ext2_sb.s_first_data_block
op_logical_or
id|block
op_ge
id|sb-&gt;u.ext2_sb.s_blocks_count
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_free_block: block not in datazone&bslash;n&quot;
)paren
suffix:semicolon
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|es
op_assign
(paren
r_struct
id|ext2_super_block
op_star
)paren
id|sb-&gt;u.ext2_sb.s_sbh-&gt;b_data
suffix:semicolon
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_free_block: freeing block %d&bslash;n&quot;
comma
id|block
)paren
suffix:semicolon
macro_line|#endif
id|bh
op_assign
id|get_hash_table
(paren
id|sb-&gt;s_dev
comma
id|block
comma
id|sb-&gt;s_blocksize
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
)paren
id|bh-&gt;b_dirt
op_assign
l_int|0
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|block_group
op_assign
(paren
id|block
op_minus
id|sb-&gt;u.ext2_sb.s_first_data_block
)paren
op_div
id|EXT2_BLOCKS_PER_GROUP
c_func
(paren
id|sb
)paren
suffix:semicolon
id|bit
op_assign
(paren
id|block
op_minus
id|sb-&gt;u.ext2_sb.s_first_data_block
)paren
op_mod
id|EXT2_BLOCKS_PER_GROUP
c_func
(paren
id|sb
)paren
suffix:semicolon
id|bitmap_nr
op_assign
id|load_block_bitmap
(paren
id|sb
comma
id|block_group
)paren
suffix:semicolon
id|bh
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|bitmap_nr
)braket
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|printk
(paren
l_string|&quot;block_group = %d&bslash;n&quot;
comma
id|block_group
)paren
suffix:semicolon
id|panic
(paren
l_string|&quot;ext2_free_block: Unable to load group bitmap&quot;
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|clear_bit
(paren
id|bit
comma
id|bh-&gt;b_data
)paren
)paren
id|printk
(paren
l_string|&quot;ext2_free_block (%04x:%d): bit already cleared&bslash;n&quot;
comma
id|sb-&gt;s_dev
comma
id|block
)paren
suffix:semicolon
r_else
(brace
id|group_desc
op_assign
id|block_group
op_div
id|EXT2_DESC_PER_BLOCK
c_func
(paren
id|sb
)paren
suffix:semicolon
id|desc
op_assign
id|block_group
op_mod
id|EXT2_DESC_PER_BLOCK
c_func
(paren
id|sb
)paren
suffix:semicolon
id|bh2
op_assign
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh2
)paren
(brace
id|printk
(paren
l_string|&quot;group_desc = %d&bslash;n&quot;
comma
id|group_desc
)paren
suffix:semicolon
id|panic
(paren
l_string|&quot;ext2_free_block: Group descriptor not loaded&quot;
)paren
suffix:semicolon
)brace
id|gdp
op_assign
(paren
r_struct
id|ext2_group_desc
op_star
)paren
id|bh2-&gt;b_data
suffix:semicolon
id|gdp
(braket
id|desc
)braket
dot
id|bg_free_blocks_count
op_increment
suffix:semicolon
id|bh2-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|es-&gt;s_free_blocks_count
op_increment
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_sbh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|sb-&gt;s_dirt
op_assign
l_int|1
suffix:semicolon
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/*&n; * ext2_new_block does not use a very clever allocation algorithm yet&n; *&n; * Currently, the group descriptors are scanned until a free block is found&n; */
DECL|function|ext2_new_block
r_int
id|ext2_new_block
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
r_int
id|block_group
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_int
id|i
comma
id|j
suffix:semicolon
r_int
r_int
id|group_desc
suffix:semicolon
r_int
r_int
id|desc
suffix:semicolon
r_int
id|bitmap_nr
suffix:semicolon
r_struct
id|ext2_group_desc
op_star
id|gdp
suffix:semicolon
r_struct
id|ext2_super_block
op_star
id|es
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_new_block: nonexistant device&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|lock_super
(paren
id|sb
)paren
suffix:semicolon
id|es
op_assign
(paren
r_struct
id|ext2_super_block
op_star
)paren
id|sb-&gt;u.ext2_sb.s_sbh-&gt;b_data
suffix:semicolon
r_if
c_cond
(paren
id|es-&gt;s_free_blocks_count
op_le
id|es-&gt;s_r_blocks_count
op_logical_and
op_logical_neg
id|suser
c_func
(paren
)paren
)paren
(brace
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|repeat
suffix:colon
id|group_desc
op_assign
l_int|0
suffix:semicolon
id|desc
op_assign
l_int|0
suffix:semicolon
id|gdp
op_assign
l_int|NULL
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|sb-&gt;u.ext2_sb.s_groups_count
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|gdp
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
)paren
id|panic
(paren
l_string|&quot;ext2_new_block: Descriptor not loaded&quot;
)paren
suffix:semicolon
id|gdp
op_assign
(paren
r_struct
id|ext2_group_desc
op_star
)paren
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
op_member_access_from_pointer
id|b_data
suffix:semicolon
)brace
r_if
c_cond
(paren
id|gdp
(braket
id|desc
)braket
dot
id|bg_free_blocks_count
OG
l_int|0
)paren
r_break
suffix:semicolon
id|desc
op_increment
suffix:semicolon
r_if
c_cond
(paren
id|desc
op_eq
id|EXT2_DESC_PER_BLOCK
c_func
(paren
id|sb
)paren
)paren
(brace
id|group_desc
op_increment
suffix:semicolon
id|desc
op_assign
l_int|0
suffix:semicolon
id|gdp
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|i
op_ge
id|sb-&gt;u.ext2_sb.s_groups_count
)paren
(brace
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_new_block: using block group %d(%d,%d,%d)&bslash;n&quot;
comma
id|i
comma
id|group_desc
comma
id|desc
comma
id|gdp
(braket
id|desc
)braket
dot
id|bg_free_blocks_count
)paren
suffix:semicolon
macro_line|#endif
id|bitmap_nr
op_assign
id|load_block_bitmap
(paren
id|sb
comma
id|i
)paren
suffix:semicolon
id|bh
op_assign
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|bitmap_nr
)braket
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|printk
(paren
l_string|&quot;block_group = %d&bslash;n&quot;
comma
id|i
)paren
suffix:semicolon
id|panic
(paren
l_string|&quot;ext2_new_block: Unable to load group bitmap&quot;
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|j
op_assign
id|find_first_zero
(paren
id|bh-&gt;b_data
comma
id|EXT2_BLOCKS_PER_GROUP
c_func
(paren
id|sb
)paren
)paren
)paren
OL
id|EXT2_BLOCKS_PER_GROUP
c_func
(paren
id|sb
)paren
)paren
(brace
r_if
c_cond
(paren
id|set_bit
(paren
id|j
comma
id|bh-&gt;b_data
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_new_block: bit already set&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|repeat
suffix:semicolon
)brace
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
)brace
r_else
r_goto
id|repeat
suffix:semicolon
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_new_block: found bit %d&bslash;n&quot;
comma
id|j
)paren
suffix:semicolon
macro_line|#endif
id|j
op_add_assign
id|i
op_star
id|EXT2_BLOCKS_PER_GROUP
c_func
(paren
id|sb
)paren
op_plus
id|sb-&gt;u.ext2_sb.s_first_data_block
suffix:semicolon
r_if
c_cond
(paren
id|j
op_ge
id|sb-&gt;u.ext2_sb.s_blocks_count
)paren
(brace
id|printk
(paren
l_string|&quot;block_group = %d,block=%d&bslash;n&quot;
comma
id|i
comma
id|j
)paren
suffix:semicolon
id|printk
(paren
l_string|&quot;ext2_new_block: block &gt;= blocks count&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|getblk
(paren
id|sb-&gt;s_dev
comma
id|j
comma
id|sb-&gt;s_blocksize
)paren
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_new_block: cannot get block&quot;
)paren
suffix:semicolon
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|clear_block
(paren
id|bh-&gt;b_data
comma
id|sb-&gt;s_blocksize
)paren
suffix:semicolon
id|bh-&gt;b_uptodate
op_assign
l_int|1
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
macro_line|#ifdef EXT2FS_DEBUG
id|printk
c_func
(paren
l_string|&quot;ext2_new_block: allocating block %d&bslash;n&quot;
comma
id|j
)paren
suffix:semicolon
macro_line|#endif
id|gdp
(braket
id|desc
)braket
dot
id|bg_free_blocks_count
op_decrement
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
op_member_access_from_pointer
id|b_dirt
op_assign
l_int|1
suffix:semicolon
id|es-&gt;s_free_blocks_count
op_decrement
suffix:semicolon
id|sb-&gt;u.ext2_sb.s_sbh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|sb-&gt;s_dirt
op_assign
l_int|1
suffix:semicolon
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
id|j
suffix:semicolon
)brace
DECL|function|ext2_count_free_blocks
r_int
r_int
id|ext2_count_free_blocks
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|ext2_super_block
op_star
id|es
suffix:semicolon
macro_line|#ifdef EXT2FS_DEBUG
r_int
r_int
id|desc_count
comma
id|bitmap_count
comma
id|x
suffix:semicolon
r_int
r_int
id|group_desc
suffix:semicolon
r_int
r_int
id|desc
suffix:semicolon
r_int
id|bitmap_nr
suffix:semicolon
r_struct
id|ext2_group_desc
op_star
id|gdp
suffix:semicolon
r_int
id|i
suffix:semicolon
id|lock_super
(paren
id|sb
)paren
suffix:semicolon
id|es
op_assign
(paren
r_struct
id|ext2_super_block
op_star
)paren
id|sb-&gt;u.ext2_sb.s_sbh-&gt;b_data
suffix:semicolon
id|desc_count
op_assign
l_int|0
suffix:semicolon
id|bitmap_count
op_assign
l_int|0
suffix:semicolon
id|group_desc
op_assign
l_int|0
suffix:semicolon
id|desc
op_assign
l_int|0
suffix:semicolon
id|gdp
op_assign
l_int|NULL
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|sb-&gt;u.ext2_sb.s_groups_count
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|gdp
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_count_free_block: Descriptor not loaded&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
id|gdp
op_assign
(paren
r_struct
id|ext2_group_desc
op_star
)paren
id|sb-&gt;u.ext2_sb.s_group_desc
(braket
id|group_desc
)braket
op_member_access_from_pointer
id|b_data
suffix:semicolon
)brace
id|desc_count
op_add_assign
id|gdp
(braket
id|desc
)braket
dot
id|bg_free_blocks_count
suffix:semicolon
id|bitmap_nr
op_assign
id|load_block_bitmap
(paren
id|sb
comma
id|i
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|bitmap_nr
)braket
)paren
id|x
op_assign
id|ext2_count_free
(paren
id|sb-&gt;u.ext2_sb.s_block_bitmap
(braket
id|bitmap_nr
)braket
comma
id|sb-&gt;s_blocksize
)paren
suffix:semicolon
r_else
(brace
id|x
op_assign
l_int|0
suffix:semicolon
id|printk
(paren
l_string|&quot;Cannot load bitmap for group %d&bslash;n&quot;
comma
id|i
)paren
suffix:semicolon
)brace
id|printk
(paren
l_string|&quot;group %d: stored = %d, counted = %d&bslash;n&quot;
comma
id|i
comma
id|gdp
(braket
id|desc
)braket
dot
id|bg_free_blocks_count
comma
id|x
)paren
suffix:semicolon
id|bitmap_count
op_add_assign
id|x
suffix:semicolon
id|desc
op_increment
suffix:semicolon
r_if
c_cond
(paren
id|desc
op_eq
id|EXT2_DESC_PER_BLOCK
c_func
(paren
id|sb
)paren
)paren
(brace
id|group_desc
op_increment
suffix:semicolon
id|desc
op_assign
l_int|0
suffix:semicolon
id|gdp
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
id|printk
c_func
(paren
l_string|&quot;ext2_count_free_blocks: stored = %d, computed = %d, %d&bslash;n&quot;
comma
id|es-&gt;s_free_blocks_count
comma
id|desc_count
comma
id|bitmap_count
)paren
suffix:semicolon
id|unlock_super
(paren
id|sb
)paren
suffix:semicolon
r_return
id|bitmap_count
suffix:semicolon
macro_line|#else
id|es
op_assign
(paren
r_struct
id|ext2_super_block
op_star
)paren
id|sb-&gt;u.ext2_sb.s_sbh-&gt;b_data
suffix:semicolon
r_return
id|es-&gt;s_free_blocks_count
suffix:semicolon
macro_line|#endif
)brace
eof
