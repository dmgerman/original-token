...
multiline_comment|/*&n; *&t;ext2_add_entry()&n; *&n; * adds a file entry to the specified directory, using the same&n; * semantics as ext2_find_entry(). It returns NULL if it failed.&n; *&n; * NOTE!! The inode part of &squot;de&squot; is left at 0 - which means you&n; * may not sleep between calling this and putting something into&n; * the entry, as someone else might have used it while you slept.&n; */
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * Is this a busy deleted directory?  Can&squot;t create new files if so&n;&t; */
r_if
c_cond
(paren
id|dir-&gt;i_size
...
multiline_comment|/*&n; * Ok, rename also locks out other renames, as they can change the parent of&n; * a directory, and we don&squot;t want any races. Other races are checked for by&n; * &quot;do_rename()&quot;, which restarts if there are inconsistencies.&n; *&n; * Note that there is no race between different filesystems: it&squot;s only within&n; * the same device that races occur: many renames can happen at once, as long&n; * as they are on different partitions.&n; *&n; * In the second extended file system, we use a lock flag stored in the memory&n; * super-block.  This way, we really lock other renames only if they occur&n; * on the same file system&n; */
...
(paren
op_amp
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_wait
)paren
suffix:semicolon
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_lock
op_assign
