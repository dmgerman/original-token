multiline_comment|/*&n; *  linux/fs/ext2/namei.c&n; *&n; *  Copyright (C) 1992, 1993  Remy Card (card@masi.ibp.fr)&n; *&n; *  from&n; *&n; *  linux/fs/minix/namei.c&n; *&n; *  Copyright (C) 1991, 1992  Linus Torvalds&n; */
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/ext2_fs.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/fcntl.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;asm/segment.h&gt;
multiline_comment|/*&n; * comment out this line if you want names &gt; EXT2_NAME_LEN chars to be&n; * truncated. Else they will be disallowed.&n; */
multiline_comment|/* #define NO_TRUNCATE */
multiline_comment|/*&n; * ok, we cannot use strncmp, as the name is not in our data space.&n; * Thus we&squot;ll have to use ext2_match. No big problem. ext2_match also makes&n; * some sanity tests.&n; *&n; * NOTE! unlike strncmp, ext2_match returns 1 for success, 0 for failure.&n; */
DECL|function|ext2_match
r_static
r_int
id|ext2_match
(paren
r_int
id|len
comma
r_const
r_char
op_star
id|name
comma
r_struct
id|ext2_dir_entry
op_star
id|de
)paren
(brace
r_register
r_int
id|same
id|__asm__
c_func
(paren
l_string|&quot;ax&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|de
op_logical_or
op_logical_neg
id|de-&gt;inode
op_logical_or
id|len
OG
id|EXT2_NAME_LEN
)paren
r_return
l_int|0
suffix:semicolon
multiline_comment|/* &quot;&quot; means &quot;.&quot; ---&gt; so paths like &quot;/usr/lib//libc.a&quot; work */
r_if
c_cond
(paren
op_logical_neg
id|len
op_logical_and
(paren
id|de-&gt;name
(braket
l_int|0
)braket
op_eq
l_char|&squot;.&squot;
)paren
op_logical_and
(paren
id|de-&gt;name
(braket
l_int|1
)braket
op_eq
l_char|&squot;&bslash;0&squot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|len
OL
id|EXT2_NAME_LEN
op_logical_and
id|len
op_ne
id|de-&gt;name_len
)paren
r_return
l_int|0
suffix:semicolon
id|__asm__
c_func
(paren
l_string|&quot;cld&bslash;n&bslash;t&quot;
l_string|&quot;repe ; cmpsb&bslash;n&bslash;t&quot;
l_string|&quot;setz %%al&quot;
suffix:colon
l_string|&quot;=a&quot;
(paren
id|same
)paren
suffix:colon
l_string|&quot;0&quot;
(paren
l_int|0
)paren
comma
l_string|&quot;S&quot;
(paren
(paren
r_int
)paren
id|name
)paren
comma
l_string|&quot;D&quot;
(paren
(paren
r_int
)paren
id|de-&gt;name
)paren
comma
l_string|&quot;c&quot;
(paren
id|len
)paren
suffix:colon
l_string|&quot;cx&quot;
comma
l_string|&quot;di&quot;
comma
l_string|&quot;si&quot;
)paren
suffix:semicolon
r_return
id|same
suffix:semicolon
)brace
multiline_comment|/*&n; *&t;ext2_find_entry()&n; *&n; * finds an entry in the specified directory with the wanted name. It&n; * returns the cache buffer in which the entry was found, and the entry&n; * itself (as a parameter - res_dir). It does NOT read the inode of the&n; * entry - you&squot;ll have to do that yourself if you want to.&n; *&n; * In the ext2 file system, this function also returns a pointer on the&n; * previous directory entry because functions which remove directory&n; * entries need it&n; */
DECL|function|ext2_find_entry
r_static
r_struct
id|buffer_head
op_star
id|ext2_find_entry
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|namelen
comma
r_struct
id|ext2_dir_entry
op_star
op_star
id|res_dir
comma
r_struct
id|ext2_dir_entry
op_star
op_star
id|prev_dir
)paren
(brace
r_int
id|offset
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
r_struct
id|super_block
op_star
id|sb
suffix:semicolon
op_star
id|res_dir
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir
)paren
r_return
l_int|NULL
suffix:semicolon
id|sb
op_assign
id|dir-&gt;i_sb
suffix:semicolon
macro_line|#ifdef NO_TRUNCATE
r_if
c_cond
(paren
id|namelen
OG
id|EXT2_NAME_LEN
)paren
r_return
l_int|NULL
suffix:semicolon
macro_line|#else
r_if
c_cond
(paren
id|namelen
OG
id|EXT2_NAME_LEN
)paren
id|namelen
op_assign
id|EXT2_NAME_LEN
suffix:semicolon
macro_line|#endif
id|bh
op_assign
id|ext2_bread
(paren
id|dir
comma
l_int|0
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_return
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|prev_dir
)paren
op_star
id|prev_dir
op_assign
l_int|NULL
suffix:semicolon
id|offset
op_assign
l_int|0
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
r_while
c_loop
(paren
id|offset
OL
id|dir-&gt;i_size
)paren
(brace
r_if
c_cond
(paren
(paren
r_char
op_star
)paren
id|de
op_ge
id|sb-&gt;s_blocksize
op_plus
id|bh-&gt;b_data
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|bh
op_assign
l_int|NULL
suffix:semicolon
id|bh
op_assign
id|ext2_bread
(paren
id|dir
comma
id|offset
op_rshift
id|EXT2_BLOCK_SIZE_BITS
c_func
(paren
id|sb
)paren
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_continue
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
r_if
c_cond
(paren
id|prev_dir
)paren
op_star
id|prev_dir
op_assign
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|de-&gt;rec_len
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
l_int|1
)paren
op_logical_or
id|de-&gt;rec_len
op_mod
l_int|4
op_ne
l_int|0
op_logical_or
id|de-&gt;rec_len
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_find_entry: bad directory entry (dev %04x, dir %d)&bslash;n&quot;
comma
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
)paren
suffix:semicolon
id|printk
(paren
l_string|&quot;offset=%d, inode=%d, rec_len=%d, name_len=%d&bslash;n&quot;
comma
id|offset
comma
id|de-&gt;inode
comma
id|de-&gt;rec_len
comma
id|de-&gt;name_len
)paren
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|ext2_match
(paren
id|namelen
comma
id|name
comma
id|de
)paren
)paren
(brace
op_star
id|res_dir
op_assign
id|de
suffix:semicolon
r_return
id|bh
suffix:semicolon
)brace
id|offset
op_add_assign
id|de-&gt;rec_len
suffix:semicolon
r_if
c_cond
(paren
id|prev_dir
)paren
op_star
id|prev_dir
op_assign
id|de
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de
op_plus
id|de-&gt;rec_len
)paren
suffix:semicolon
)brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
DECL|function|ext2_lookup
r_int
id|ext2_lookup
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
comma
r_struct
id|inode
op_star
op_star
id|result
)paren
(brace
r_int
id|ino
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
op_star
id|result
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir
)paren
r_return
op_minus
id|ENOENT
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|S_ISDIR
c_func
(paren
id|dir-&gt;i_mode
)paren
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOENT
suffix:semicolon
)brace
macro_line|#ifndef DONT_USE_DCACHE
r_if
c_cond
(paren
op_logical_neg
(paren
id|ino
op_assign
id|ext2_dcache_lookup
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|name
comma
id|len
)paren
)paren
)paren
(brace
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
l_int|NULL
)paren
)paren
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOENT
suffix:semicolon
)brace
id|ino
op_assign
id|de-&gt;inode
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_add
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
comma
id|ino
)paren
suffix:semicolon
macro_line|#endif
id|brelse
(paren
id|bh
)paren
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
)brace
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
(paren
op_star
id|result
op_assign
id|iget
(paren
id|dir-&gt;i_sb
comma
id|ino
)paren
)paren
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|EACCES
suffix:semicolon
)brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *&t;ext2_add_entry()&n; *&n; * adds a file entry to the specified directory, using the same&n; * semantics as ext2_find_entry(). It returns NULL if it failed.&n; *&n; * NOTE!! The inode part of &squot;de&squot; is left at 0 - which means you&n; * may not sleep between calling this and putting something into&n; * the entry, as someone else might have used it while you slept.&n; */
DECL|function|ext2_add_entry
r_static
r_struct
id|buffer_head
op_star
id|ext2_add_entry
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|namelen
comma
r_struct
id|ext2_dir_entry
op_star
op_star
id|res_dir
)paren
(brace
r_int
id|i
suffix:semicolon
r_int
id|offset
suffix:semicolon
r_int
r_int
id|rec_len
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
comma
op_star
id|de1
suffix:semicolon
r_struct
id|super_block
op_star
id|sb
suffix:semicolon
op_star
id|res_dir
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir
)paren
r_return
l_int|NULL
suffix:semicolon
id|sb
op_assign
id|dir-&gt;i_sb
suffix:semicolon
macro_line|#ifdef NO_TRUNCATE
r_if
c_cond
(paren
id|namelen
OG
id|EXT2_NAME_LEN
)paren
r_return
l_int|NULL
suffix:semicolon
macro_line|#else
r_if
c_cond
(paren
id|namelen
OG
id|EXT2_NAME_LEN
)paren
id|namelen
op_assign
id|EXT2_NAME_LEN
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
id|namelen
)paren
r_return
l_int|NULL
suffix:semicolon
id|bh
op_assign
id|ext2_bread
(paren
id|dir
comma
l_int|0
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_return
l_int|NULL
suffix:semicolon
id|rec_len
op_assign
id|EXT2_DIR_REC_LEN
c_func
(paren
id|namelen
)paren
suffix:semicolon
id|offset
op_assign
l_int|0
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
r_while
c_loop
(paren
l_int|1
)paren
(brace
r_if
c_cond
(paren
(paren
r_char
op_star
)paren
id|de
op_ge
id|sb-&gt;s_blocksize
op_plus
id|bh-&gt;b_data
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|bh
op_assign
l_int|NULL
suffix:semicolon
id|bh
op_assign
id|ext2_bread
(paren
id|dir
comma
id|offset
op_rshift
id|EXT2_BLOCK_SIZE_BITS
c_func
(paren
id|sb
)paren
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_add_entry: creating next block&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|bh
op_assign
id|ext2_bread
(paren
id|dir
comma
id|offset
op_rshift
id|EXT2_BLOCK_SIZE_BITS
c_func
(paren
id|sb
)paren
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_return
l_int|NULL
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
id|de-&gt;inode
op_assign
l_int|0
suffix:semicolon
id|de-&gt;rec_len
op_assign
id|sb-&gt;s_blocksize
suffix:semicolon
id|dir-&gt;i_size
op_add_assign
id|sb-&gt;s_blocksize
suffix:semicolon
id|dir-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
)brace
r_else
(brace
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_add_entry: skipping to next block&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|de-&gt;rec_len
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
l_int|1
)paren
op_logical_or
id|de-&gt;rec_len
op_mod
l_int|4
op_ne
l_int|0
op_logical_or
id|de-&gt;rec_len
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;ext2_add_entry: bad directory entry (dev %04x, dir %d)&bslash;n&quot;
comma
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
)paren
suffix:semicolon
id|printk
(paren
l_string|&quot;offset=%d, inode=%d, rec_len=%d, name_len=%d&bslash;n&quot;
comma
id|offset
comma
id|de-&gt;inode
comma
id|de-&gt;rec_len
comma
id|de-&gt;name_len
)paren
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
op_logical_neg
id|de-&gt;inode
op_logical_and
id|de-&gt;rec_len
op_ge
id|rec_len
)paren
op_logical_or
(paren
id|de-&gt;rec_len
op_ge
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
op_plus
id|rec_len
)paren
)paren
(brace
id|offset
op_add_assign
id|de-&gt;rec_len
suffix:semicolon
r_if
c_cond
(paren
id|de-&gt;inode
)paren
(brace
id|de1
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de
op_plus
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
)paren
suffix:semicolon
id|de1-&gt;rec_len
op_assign
id|de-&gt;rec_len
op_minus
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
suffix:semicolon
id|de-&gt;rec_len
op_assign
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
suffix:semicolon
id|de
op_assign
id|de1
suffix:semicolon
)brace
id|de-&gt;inode
op_assign
l_int|0
suffix:semicolon
id|de-&gt;name_len
op_assign
id|namelen
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|namelen
suffix:semicolon
id|i
op_increment
)paren
id|de-&gt;name
(braket
id|i
)braket
op_assign
id|name
(braket
id|i
)braket
suffix:semicolon
id|dir-&gt;i_mtime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
op_star
id|res_dir
op_assign
id|de
suffix:semicolon
r_return
id|bh
suffix:semicolon
)brace
id|offset
op_add_assign
id|de-&gt;rec_len
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de
op_plus
id|de-&gt;rec_len
)paren
suffix:semicolon
)brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
DECL|function|ext2_create
r_int
id|ext2_create
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
comma
r_int
id|mode
comma
r_struct
id|inode
op_star
op_star
id|result
)paren
(brace
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
op_star
id|result
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir
)paren
r_return
op_minus
id|ENOENT
suffix:semicolon
id|inode
op_assign
id|ext2_new_inode
(paren
id|dir
comma
id|mode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|inode-&gt;i_op
op_assign
op_amp
id|ext2_file_inode_operations
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|mode
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|bh
op_assign
id|ext2_add_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|de-&gt;inode
op_assign
id|inode-&gt;i_ino
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_add
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
comma
id|de-&gt;inode
)paren
suffix:semicolon
macro_line|#endif
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
op_star
id|result
op_assign
id|inode
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ext2_mknod
r_int
id|ext2_mknod
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
comma
r_int
id|mode
comma
r_int
id|rdev
)paren
(brace
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir
)paren
r_return
op_minus
id|ENOENT
suffix:semicolon
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|EEXIST
suffix:semicolon
)brace
id|inode
op_assign
id|ext2_new_inode
(paren
id|dir
comma
id|mode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|inode-&gt;i_uid
op_assign
id|current-&gt;euid
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|mode
suffix:semicolon
id|inode-&gt;i_op
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|S_ISREG
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext2_file_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
id|inode-&gt;i_op
op_assign
op_amp
id|ext2_dir_inode_operations
suffix:semicolon
r_if
c_cond
(paren
id|dir-&gt;i_mode
op_amp
id|S_ISGID
)paren
id|inode-&gt;i_mode
op_or_assign
id|S_ISGID
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|S_ISLNK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|ext2_symlink_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISCHR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|chrdev_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|blkdev_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISFIFO
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
id|inode-&gt;i_op
op_assign
op_amp
id|fifo_inode_operations
suffix:semicolon
id|inode-&gt;i_pipe
op_assign
l_int|1
suffix:semicolon
id|PIPE_BASE
c_func
(paren
op_star
id|inode
)paren
op_assign
l_int|NULL
suffix:semicolon
id|PIPE_HEAD
c_func
(paren
op_star
id|inode
)paren
op_assign
id|PIPE_TAIL
c_func
(paren
op_star
id|inode
)paren
op_assign
l_int|0
suffix:semicolon
id|PIPE_READ_WAIT
c_func
(paren
op_star
id|inode
)paren
op_assign
id|PIPE_WRITE_WAIT
c_func
(paren
op_star
id|inode
)paren
op_assign
l_int|NULL
suffix:semicolon
id|PIPE_READERS
c_func
(paren
op_star
id|inode
)paren
op_assign
id|PIPE_WRITERS
c_func
(paren
op_star
id|inode
)paren
op_assign
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|mode
)paren
op_logical_or
id|S_ISCHR
c_func
(paren
id|mode
)paren
)paren
id|inode-&gt;i_rdev
op_assign
id|rdev
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|inode-&gt;i_atime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|bh
op_assign
id|ext2_add_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|de-&gt;inode
op_assign
id|inode-&gt;i_ino
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_add
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
comma
id|de-&gt;inode
)paren
suffix:semicolon
macro_line|#endif
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ext2_mkdir
r_int
id|ext2_mkdir
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
comma
r_int
id|mode
)paren
(brace
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
comma
op_star
id|dir_block
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|EEXIST
suffix:semicolon
)brace
id|inode
op_assign
id|ext2_new_inode
(paren
id|dir
comma
id|S_IFDIR
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|inode-&gt;i_op
op_assign
op_amp
id|ext2_dir_inode_operations
suffix:semicolon
id|inode-&gt;i_size
op_assign
id|inode-&gt;i_sb-&gt;s_blocksize
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|inode-&gt;i_atime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|dir_block
op_assign
id|ext2_bread
(paren
id|inode
comma
l_int|0
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir_block
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|inode-&gt;i_blocks
op_assign
id|inode-&gt;i_sb-&gt;s_blocksize
op_div
l_int|512
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|dir_block-&gt;b_data
suffix:semicolon
id|de-&gt;inode
op_assign
id|inode-&gt;i_ino
suffix:semicolon
id|de-&gt;name_len
op_assign
l_int|1
suffix:semicolon
id|de-&gt;rec_len
op_assign
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
suffix:semicolon
id|strcpy
(paren
id|de-&gt;name
comma
l_string|&quot;.&quot;
)paren
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de
op_plus
id|de-&gt;rec_len
)paren
suffix:semicolon
id|de-&gt;inode
op_assign
id|dir-&gt;i_ino
suffix:semicolon
id|de-&gt;rec_len
op_assign
id|inode-&gt;i_sb-&gt;s_blocksize
op_minus
id|EXT2_DIR_REC_LEN
c_func
(paren
l_int|1
)paren
suffix:semicolon
id|de-&gt;name_len
op_assign
l_int|2
suffix:semicolon
id|strcpy
(paren
id|de-&gt;name
comma
l_string|&quot;..&quot;
)paren
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
l_int|2
suffix:semicolon
id|dir_block-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|dir_block
)paren
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|S_IFDIR
op_or
(paren
id|mode
op_amp
l_int|0777
op_amp
op_complement
id|current-&gt;umask
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dir-&gt;i_mode
op_amp
id|S_ISGID
)paren
id|inode-&gt;i_mode
op_or_assign
id|S_ISGID
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|bh
op_assign
id|ext2_add_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
l_int|0
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|de-&gt;inode
op_assign
id|inode-&gt;i_ino
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_add
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
comma
id|de-&gt;inode
)paren
suffix:semicolon
macro_line|#endif
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|dir-&gt;i_nlink
op_increment
suffix:semicolon
id|dir-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * routine to check that the specified directory is empty (for rmdir)&n; */
DECL|function|empty_dir
r_static
r_int
id|empty_dir
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_int
r_int
id|offset
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
comma
op_star
id|de1
suffix:semicolon
r_struct
id|super_block
op_star
id|sb
suffix:semicolon
id|sb
op_assign
id|inode-&gt;i_sb
suffix:semicolon
r_if
c_cond
(paren
id|inode-&gt;i_size
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
l_int|1
)paren
op_plus
id|EXT2_DIR_REC_LEN
c_func
(paren
l_int|2
)paren
op_logical_or
op_logical_neg
(paren
id|bh
op_assign
id|ext2_bread
(paren
id|inode
comma
l_int|0
comma
l_int|0
)paren
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;warning - bad directory (dev %04x, dir %d)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
id|de1
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de
op_plus
id|de-&gt;rec_len
)paren
suffix:semicolon
r_if
c_cond
(paren
id|de-&gt;inode
op_ne
id|inode-&gt;i_ino
op_logical_or
op_logical_neg
id|de1-&gt;inode
op_logical_or
id|strcmp
(paren
l_string|&quot;.&quot;
comma
id|de-&gt;name
)paren
op_logical_or
id|strcmp
(paren
l_string|&quot;..&quot;
comma
id|de1-&gt;name
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;warning - bad directory (dev %04x, dir %d)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
id|offset
op_assign
id|de-&gt;rec_len
op_plus
id|de1-&gt;rec_len
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de1
op_plus
id|de1-&gt;rec_len
)paren
suffix:semicolon
r_while
c_loop
(paren
id|offset
OL
id|inode-&gt;i_size
)paren
(brace
r_if
c_cond
(paren
(paren
r_void
op_star
)paren
id|de
op_ge
(paren
r_void
op_star
)paren
(paren
id|bh-&gt;b_data
op_plus
id|sb-&gt;s_blocksize
)paren
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|bh
op_assign
id|ext2_bread
(paren
id|inode
comma
id|offset
op_rshift
id|EXT2_BLOCK_SIZE_BITS
c_func
(paren
id|sb
)paren
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|offset
op_add_assign
id|sb-&gt;s_blocksize
suffix:semicolon
r_continue
suffix:semicolon
)brace
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
)brace
r_if
c_cond
(paren
id|de-&gt;rec_len
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
l_int|1
)paren
op_logical_or
id|de-&gt;rec_len
op_mod
l_int|4
op_ne
l_int|0
op_logical_or
id|de-&gt;rec_len
OL
id|EXT2_DIR_REC_LEN
c_func
(paren
id|de-&gt;name_len
)paren
)paren
(brace
id|printk
(paren
l_string|&quot;empty_dir: bad directory entry (dev %04x, dir %d)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
id|printk
(paren
l_string|&quot;offset=%d, inode=%d, rec_len=%d, name_len=%d&bslash;n&quot;
comma
id|offset
comma
id|de-&gt;inode
comma
id|de-&gt;rec_len
comma
id|de-&gt;name_len
)paren
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|de-&gt;inode
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|offset
op_add_assign
id|de-&gt;rec_len
suffix:semicolon
id|de
op_assign
(paren
r_struct
id|ext2_dir_entry
op_star
)paren
(paren
(paren
r_char
op_star
)paren
id|de
op_plus
id|de-&gt;rec_len
)paren
suffix:semicolon
)brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
DECL|function|ext2_rmdir
r_int
id|ext2_rmdir
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
)paren
(brace
r_int
id|retval
suffix:semicolon
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
comma
op_star
id|pde
suffix:semicolon
id|inode
op_assign
l_int|NULL
suffix:semicolon
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
op_amp
id|pde
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|ENOENT
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_goto
id|end_rmdir
suffix:semicolon
id|retval
op_assign
op_minus
id|EPERM
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|inode
op_assign
id|iget
(paren
id|dir-&gt;i_sb
comma
id|de-&gt;inode
)paren
)paren
)paren
r_goto
id|end_rmdir
suffix:semicolon
r_if
c_cond
(paren
(paren
id|dir-&gt;i_mode
op_amp
id|S_ISVTX
)paren
op_logical_and
id|current-&gt;euid
op_logical_and
id|inode-&gt;i_uid
op_ne
id|current-&gt;euid
)paren
r_goto
id|end_rmdir
suffix:semicolon
r_if
c_cond
(paren
id|inode-&gt;i_dev
op_ne
id|dir-&gt;i_dev
)paren
r_goto
id|end_rmdir
suffix:semicolon
r_if
c_cond
(paren
id|inode
op_eq
id|dir
)paren
multiline_comment|/* we may not delete &quot;.&quot;, but &quot;../dir&quot; is ok */
r_goto
id|end_rmdir
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
id|retval
op_assign
op_minus
id|ENOTDIR
suffix:semicolon
r_goto
id|end_rmdir
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|empty_dir
(paren
id|inode
)paren
)paren
(brace
id|retval
op_assign
op_minus
id|ENOTEMPTY
suffix:semicolon
r_goto
id|end_rmdir
suffix:semicolon
)brace
r_if
c_cond
(paren
id|inode-&gt;i_count
OG
l_int|1
)paren
(brace
id|retval
op_assign
op_minus
id|EBUSY
suffix:semicolon
r_goto
id|end_rmdir
suffix:semicolon
)brace
r_if
c_cond
(paren
id|inode-&gt;i_nlink
op_ne
l_int|2
)paren
id|printk
(paren
l_string|&quot;empty  directory has nlink!=2 (%d)&bslash;n&quot;
comma
id|inode-&gt;i_nlink
)paren
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_remove
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|pde
)paren
id|pde-&gt;rec_len
op_add_assign
id|de-&gt;rec_len
suffix:semicolon
r_else
id|de-&gt;inode
op_assign
l_int|0
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|dir-&gt;i_nlink
op_decrement
suffix:semicolon
id|dir-&gt;i_ctime
op_assign
id|dir-&gt;i_mtime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|dir-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|retval
op_assign
l_int|0
suffix:semicolon
id|end_rmdir
suffix:colon
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
r_return
id|retval
suffix:semicolon
)brace
DECL|function|ext2_unlink
r_int
id|ext2_unlink
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
)paren
(brace
r_int
id|retval
suffix:semicolon
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|de
comma
op_star
id|pde
suffix:semicolon
id|retval
op_assign
op_minus
id|ENOENT
suffix:semicolon
id|inode
op_assign
l_int|NULL
suffix:semicolon
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
op_amp
id|pde
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_goto
id|end_unlink
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|inode
op_assign
id|iget
(paren
id|dir-&gt;i_sb
comma
id|de-&gt;inode
)paren
)paren
)paren
r_goto
id|end_unlink
suffix:semicolon
id|retval
op_assign
op_minus
id|EPERM
suffix:semicolon
r_if
c_cond
(paren
(paren
id|dir-&gt;i_mode
op_amp
id|S_ISVTX
)paren
op_logical_and
op_logical_neg
id|suser
c_func
(paren
)paren
op_logical_and
id|current-&gt;euid
op_ne
id|inode-&gt;i_uid
op_logical_and
id|current-&gt;euid
op_ne
id|dir-&gt;i_uid
)paren
r_goto
id|end_unlink
suffix:semicolon
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
r_goto
id|end_unlink
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode-&gt;i_nlink
)paren
(brace
id|printk
(paren
l_string|&quot;Deleting nonexistent file (%04x:%d), %d&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
comma
id|inode-&gt;i_nlink
)paren
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
l_int|1
suffix:semicolon
)brace
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_remove
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|pde
)paren
id|pde-&gt;rec_len
op_add_assign
id|de-&gt;rec_len
suffix:semicolon
r_else
id|de-&gt;inode
op_assign
l_int|0
suffix:semicolon
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|dir-&gt;i_ctime
op_assign
id|dir-&gt;i_mtime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|dir-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_ctime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|retval
op_assign
l_int|0
suffix:semicolon
id|end_unlink
suffix:colon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
id|retval
suffix:semicolon
)brace
DECL|function|ext2_symlink
r_int
id|ext2_symlink
(paren
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
comma
r_const
r_char
op_star
id|symname
)paren
(brace
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
r_struct
id|inode
op_star
id|inode
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
op_assign
l_int|NULL
comma
op_star
id|name_block
op_assign
l_int|NULL
suffix:semicolon
r_char
op_star
id|link
suffix:semicolon
r_int
id|i
suffix:semicolon
r_int
id|l
suffix:semicolon
r_char
id|c
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|inode
op_assign
id|ext2_new_inode
(paren
id|dir
comma
id|S_IFLNK
)paren
)paren
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|inode-&gt;i_mode
op_assign
id|S_IFLNK
op_or
l_int|0777
suffix:semicolon
id|inode-&gt;i_op
op_assign
op_amp
id|ext2_symlink_inode_operations
suffix:semicolon
r_for
c_loop
(paren
id|l
op_assign
l_int|0
suffix:semicolon
id|l
OL
id|inode-&gt;i_sb-&gt;s_blocksize
op_minus
l_int|1
op_logical_and
id|symname
(braket
id|l
)braket
suffix:semicolon
id|l
op_increment
)paren
suffix:semicolon
r_if
c_cond
(paren
id|l
op_ge
id|EXT2_N_BLOCKS
op_star
r_sizeof
(paren
r_int
r_int
)paren
)paren
(brace
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_symlink: l=%d, normal symlink&bslash;n&quot;
comma
id|l
)paren
suffix:semicolon
macro_line|#endif
id|name_block
op_assign
id|ext2_bread
(paren
id|inode
comma
l_int|0
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|name_block
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|link
op_assign
id|name_block-&gt;b_data
suffix:semicolon
)brace
r_else
(brace
id|link
op_assign
(paren
r_char
op_star
)paren
id|inode-&gt;u.ext2_i.i_data
suffix:semicolon
macro_line|#ifdef EXT2FS_DEBUG
id|printk
(paren
l_string|&quot;ext2_symlink: l=%d, fast symlink&bslash;n&quot;
comma
id|l
)paren
suffix:semicolon
macro_line|#endif
)brace
id|i
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
id|i
OL
id|inode-&gt;i_sb-&gt;s_blocksize
op_minus
l_int|1
op_logical_and
(paren
id|c
op_assign
op_star
(paren
id|symname
op_increment
)paren
)paren
)paren
id|link
(braket
id|i
op_increment
)braket
op_assign
id|c
suffix:semicolon
id|link
(braket
id|i
)braket
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|name_block
)paren
(brace
id|name_block-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|name_block
)paren
suffix:semicolon
)brace
id|inode-&gt;i_size
op_assign
id|i
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
)paren
(brace
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|EEXIST
suffix:semicolon
)brace
id|bh
op_assign
id|ext2_add_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|de-&gt;inode
op_assign
id|inode-&gt;i_ino
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_add
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
comma
id|de-&gt;inode
)paren
suffix:semicolon
macro_line|#endif
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|iput
(paren
id|inode
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ext2_link
r_int
id|ext2_link
(paren
r_struct
id|inode
op_star
id|oldinode
comma
r_struct
id|inode
op_star
id|dir
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
)paren
(brace
r_struct
id|ext2_dir_entry
op_star
id|de
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|oldinode-&gt;i_mode
)paren
)paren
(brace
id|iput
(paren
id|oldinode
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|EPERM
suffix:semicolon
)brace
r_if
c_cond
(paren
id|oldinode-&gt;i_nlink
OG
l_int|32000
)paren
(brace
id|iput
(paren
id|oldinode
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
r_return
op_minus
id|EMLINK
suffix:semicolon
)brace
id|bh
op_assign
id|ext2_find_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
)paren
(brace
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|iput
(paren
id|oldinode
)paren
suffix:semicolon
r_return
op_minus
id|EEXIST
suffix:semicolon
)brace
id|bh
op_assign
id|ext2_add_entry
(paren
id|dir
comma
id|name
comma
id|len
comma
op_amp
id|de
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
(brace
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|iput
(paren
id|oldinode
)paren
suffix:semicolon
r_return
op_minus
id|ENOSPC
suffix:semicolon
)brace
id|de-&gt;inode
op_assign
id|oldinode-&gt;i_ino
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_add
(paren
id|dir-&gt;i_dev
comma
id|dir-&gt;i_ino
comma
id|de-&gt;name
comma
id|de-&gt;name_len
comma
id|de-&gt;inode
)paren
suffix:semicolon
macro_line|#endif
id|bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|brelse
(paren
id|bh
)paren
suffix:semicolon
id|iput
(paren
id|dir
)paren
suffix:semicolon
id|oldinode-&gt;i_nlink
op_increment
suffix:semicolon
id|oldinode-&gt;i_ctime
op_assign
id|CURRENT_TIME
suffix:semicolon
id|oldinode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|iput
(paren
id|oldinode
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|subdir
r_static
r_int
id|subdir
(paren
r_struct
id|inode
op_star
r_new
comma
r_struct
id|inode
op_star
id|old
)paren
(brace
r_int
id|ino
suffix:semicolon
r_int
id|result
suffix:semicolon
r_new
op_member_access_from_pointer
id|i_count
op_increment
suffix:semicolon
id|result
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
suffix:semicolon
)paren
(brace
r_if
c_cond
(paren
r_new
op_eq
id|old
)paren
(brace
id|result
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
r_new
op_member_access_from_pointer
id|i_dev
op_ne
id|old-&gt;i_dev
)paren
r_break
suffix:semicolon
id|ino
op_assign
r_new
op_member_access_from_pointer
id|i_ino
suffix:semicolon
r_if
c_cond
(paren
id|ext2_lookup
(paren
r_new
comma
l_string|&quot;..&quot;
comma
l_int|2
comma
op_amp
r_new
)paren
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
r_new
op_member_access_from_pointer
id|i_ino
op_eq
id|ino
)paren
r_break
suffix:semicolon
)brace
id|iput
(paren
r_new
)paren
suffix:semicolon
r_return
id|result
suffix:semicolon
)brace
DECL|macro|PARENT_INO
mdefine_line|#define PARENT_INO(buffer) &bslash;&n;&t;((struct ext2_dir_entry *) ((char *) buffer + &bslash;&n;&t;((struct ext2_dir_entry *) buffer)-&gt;rec_len))-&gt;inode
DECL|macro|PARENT_NAME
mdefine_line|#define PARENT_NAME(buffer) &bslash;&n;&t;((struct ext2_dir_entry *) ((char *) buffer + &bslash;&n;&t;((struct ext2_dir_entry *) buffer)-&gt;rec_len))-&gt;name
multiline_comment|/*&n; * rename uses retrying to avoid race-conditions: at least they should be&n; * minimal.&n; * it tries to allocate all the blocks, then sanity-checks, and if the sanity-&n; * checks fail, it tries to restart itself again. Very practical - no changes&n; * are done until we know everything works ok.. and then all the changes can be&n; * done in one fell swoop when we have claimed all the buffers needed.&n; *&n; * Anybody can rename anything with this: the permission checks are left to the&n; * higher-level routines.&n; */
DECL|function|do_ext2_rename
r_static
r_int
id|do_ext2_rename
(paren
r_struct
id|inode
op_star
id|old_dir
comma
r_const
r_char
op_star
id|old_name
comma
r_int
id|old_len
comma
r_struct
id|inode
op_star
id|new_dir
comma
r_const
r_char
op_star
id|new_name
comma
r_int
id|new_len
)paren
(brace
r_struct
id|inode
op_star
id|old_inode
comma
op_star
id|new_inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|old_bh
comma
op_star
id|new_bh
comma
op_star
id|dir_bh
suffix:semicolon
r_struct
id|ext2_dir_entry
op_star
id|old_de
comma
op_star
id|new_de
comma
op_star
id|pde
suffix:semicolon
r_int
id|retval
suffix:semicolon
r_goto
id|start_up
suffix:semicolon
id|try_again
suffix:colon
id|brelse
(paren
id|old_bh
)paren
suffix:semicolon
id|brelse
(paren
id|new_bh
)paren
suffix:semicolon
id|brelse
(paren
id|dir_bh
)paren
suffix:semicolon
id|iput
(paren
id|old_inode
)paren
suffix:semicolon
id|iput
(paren
id|new_inode
)paren
suffix:semicolon
id|current-&gt;counter
op_assign
l_int|0
suffix:semicolon
id|schedule
(paren
)paren
suffix:semicolon
id|start_up
suffix:colon
id|old_inode
op_assign
id|new_inode
op_assign
l_int|NULL
suffix:semicolon
id|old_bh
op_assign
id|new_bh
op_assign
id|dir_bh
op_assign
l_int|NULL
suffix:semicolon
id|old_bh
op_assign
id|ext2_find_entry
(paren
id|old_dir
comma
id|old_name
comma
id|old_len
comma
op_amp
id|old_de
comma
op_amp
id|pde
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|ENOENT
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|old_bh
)paren
r_goto
id|end_rename
suffix:semicolon
id|old_inode
op_assign
id|iget
(paren
id|old_dir-&gt;i_sb
comma
id|old_de-&gt;inode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|old_inode
)paren
r_goto
id|end_rename
suffix:semicolon
r_if
c_cond
(paren
(paren
id|old_dir-&gt;i_mode
op_amp
id|S_ISVTX
)paren
op_logical_and
id|current-&gt;euid
op_ne
id|old_inode-&gt;i_uid
op_logical_and
id|current-&gt;euid
op_ne
id|old_dir-&gt;i_uid
op_logical_and
op_logical_neg
id|suser
c_func
(paren
)paren
)paren
r_goto
id|end_rename
suffix:semicolon
id|new_bh
op_assign
id|ext2_find_entry
(paren
id|new_dir
comma
id|new_name
comma
id|new_len
comma
op_amp
id|new_de
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|new_bh
)paren
(brace
id|new_inode
op_assign
id|iget
(paren
id|new_dir-&gt;i_sb
comma
id|new_de-&gt;inode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|new_inode
)paren
(brace
id|brelse
(paren
id|new_bh
)paren
suffix:semicolon
id|new_bh
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|new_inode
op_eq
id|old_inode
)paren
(brace
id|retval
op_assign
l_int|0
suffix:semicolon
r_goto
id|end_rename
suffix:semicolon
)brace
r_if
c_cond
(paren
id|new_inode
op_logical_and
id|S_ISDIR
c_func
(paren
id|new_inode-&gt;i_mode
)paren
)paren
(brace
id|retval
op_assign
op_minus
id|EEXIST
suffix:semicolon
r_goto
id|end_rename
suffix:semicolon
)brace
id|retval
op_assign
op_minus
id|EPERM
suffix:semicolon
r_if
c_cond
(paren
id|new_inode
op_logical_and
(paren
id|new_dir-&gt;i_mode
op_amp
id|S_ISVTX
)paren
op_logical_and
id|current-&gt;euid
op_ne
id|new_inode-&gt;i_uid
op_logical_and
id|current-&gt;euid
op_ne
id|new_dir-&gt;i_uid
op_logical_and
op_logical_neg
id|suser
c_func
(paren
)paren
)paren
r_goto
id|end_rename
suffix:semicolon
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|old_inode-&gt;i_mode
)paren
)paren
(brace
id|retval
op_assign
op_minus
id|EEXIST
suffix:semicolon
r_if
c_cond
(paren
id|new_bh
)paren
r_goto
id|end_rename
suffix:semicolon
id|retval
op_assign
op_minus
id|EACCES
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|permission
(paren
id|old_inode
comma
id|MAY_WRITE
)paren
)paren
r_goto
id|end_rename
suffix:semicolon
id|retval
op_assign
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
id|subdir
(paren
id|new_dir
comma
id|old_inode
)paren
)paren
r_goto
id|end_rename
suffix:semicolon
id|retval
op_assign
op_minus
id|EIO
suffix:semicolon
id|dir_bh
op_assign
id|ext2_bread
(paren
id|old_inode
comma
l_int|0
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dir_bh
)paren
r_goto
id|end_rename
suffix:semicolon
r_if
c_cond
(paren
id|PARENT_INO
c_func
(paren
id|dir_bh-&gt;b_data
)paren
op_ne
id|old_dir-&gt;i_ino
)paren
r_goto
id|end_rename
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|new_bh
)paren
id|new_bh
op_assign
id|ext2_add_entry
(paren
id|new_dir
comma
id|new_name
comma
id|new_len
comma
op_amp
id|new_de
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|ENOSPC
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|new_bh
)paren
r_goto
id|end_rename
suffix:semicolon
multiline_comment|/* sanity checking before doing the rename - avoid races */
r_if
c_cond
(paren
id|new_inode
op_logical_and
(paren
id|new_de-&gt;inode
op_ne
id|new_inode-&gt;i_ino
)paren
)paren
r_goto
id|try_again
suffix:semicolon
r_if
c_cond
(paren
id|new_de-&gt;inode
op_logical_and
op_logical_neg
id|new_inode
)paren
r_goto
id|try_again
suffix:semicolon
r_if
c_cond
(paren
id|old_de-&gt;inode
op_ne
id|old_inode-&gt;i_ino
)paren
r_goto
id|try_again
suffix:semicolon
multiline_comment|/* ok, that&squot;s it */
id|new_de-&gt;inode
op_assign
id|old_inode-&gt;i_ino
suffix:semicolon
macro_line|#ifndef DONT_USE_DCACHE
id|ext2_dcache_remove
(paren
id|old_dir-&gt;i_dev
comma
id|old_dir-&gt;i_ino
comma
id|old_de-&gt;name
comma
id|old_de-&gt;name_len
)paren
suffix:semicolon
id|ext2_dcache_add
(paren
id|new_dir-&gt;i_dev
comma
id|new_dir-&gt;i_ino
comma
id|new_de-&gt;name
comma
id|new_de-&gt;name_len
comma
id|new_de-&gt;inode
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
(paren
(paren
r_char
op_star
)paren
id|new_de
)paren
op_plus
id|new_de-&gt;rec_len
op_eq
(paren
r_char
op_star
)paren
id|old_de
)paren
id|new_de-&gt;rec_len
op_add_assign
id|old_de-&gt;rec_len
suffix:semicolon
r_else
r_if
c_cond
(paren
id|pde
)paren
id|pde-&gt;rec_len
op_add_assign
id|old_de-&gt;rec_len
suffix:semicolon
r_else
id|old_de-&gt;inode
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|new_inode
)paren
(brace
id|new_inode-&gt;i_nlink
op_decrement
suffix:semicolon
id|new_inode-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|old_bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|new_bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|dir_bh
)paren
(brace
id|PARENT_INO
c_func
(paren
id|dir_bh-&gt;b_data
)paren
op_assign
id|new_dir-&gt;i_ino
suffix:semicolon
id|dir_bh-&gt;b_dirt
op_assign
l_int|1
suffix:semicolon
id|old_dir-&gt;i_nlink
op_decrement
suffix:semicolon
id|new_dir-&gt;i_nlink
op_increment
suffix:semicolon
id|old_dir-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
id|new_dir-&gt;i_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|retval
op_assign
l_int|0
suffix:semicolon
id|end_rename
suffix:colon
id|brelse
(paren
id|dir_bh
)paren
suffix:semicolon
id|brelse
(paren
id|old_bh
)paren
suffix:semicolon
id|brelse
(paren
id|new_bh
)paren
suffix:semicolon
id|iput
(paren
id|old_inode
)paren
suffix:semicolon
id|iput
(paren
id|new_inode
)paren
suffix:semicolon
id|iput
(paren
id|old_dir
)paren
suffix:semicolon
id|iput
(paren
id|new_dir
)paren
suffix:semicolon
r_return
id|retval
suffix:semicolon
)brace
multiline_comment|/*&n; * Ok, rename also locks out other renames, as they can change the parent of&n; * a directory, and we don&squot;t want any races. Other races are checked for by&n; * &quot;do_rename()&quot;, which restarts if there are inconsistencies.&n; *&n; * Note that there is no race between different filesystems: it&squot;s only within&n; * the same device that races occur: many renames can happen at once, as long&n; * as they are on different partitions.&n; *&n; * In the second extended file system, we use a lock flag stored in the memory&n; * super-block.  This way, we really lock other renames only if they occur&n; * on the same file system&n; */
DECL|function|ext2_rename
r_int
id|ext2_rename
(paren
r_struct
id|inode
op_star
id|old_dir
comma
r_const
r_char
op_star
id|old_name
comma
r_int
id|old_len
comma
r_struct
id|inode
op_star
id|new_dir
comma
r_const
r_char
op_star
id|new_name
comma
r_int
id|new_len
)paren
(brace
r_int
id|result
suffix:semicolon
r_while
c_loop
(paren
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_lock
)paren
id|sleep_on
(paren
op_amp
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_wait
)paren
suffix:semicolon
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_lock
op_assign
l_int|1
suffix:semicolon
id|result
op_assign
id|do_ext2_rename
(paren
id|old_dir
comma
id|old_name
comma
id|old_len
comma
id|new_dir
comma
id|new_name
comma
id|new_len
)paren
suffix:semicolon
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_lock
op_assign
l_int|0
suffix:semicolon
id|wake_up
(paren
op_amp
id|old_dir-&gt;i_sb-&gt;u.ext2_sb.s_rename_wait
)paren
suffix:semicolon
r_return
id|result
suffix:semicolon
)brace
eof
