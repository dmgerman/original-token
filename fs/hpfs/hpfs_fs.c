...
multiline_comment|/*&n; * readdir.  Return exactly 1 dirent.  (I tried and tried, but currently&n; * the interface with libc just does not permit more than 1.  If it gets&n; * fixed, throw this out and just walk the tree and write records into&n; * the user buffer.)&n; *&n; * We keep track of our position in the dnode tree with a sort of&n; * dewey-decimal record of subtree locations.  Like so:&n; *&n; *   (1 (1.1 1.2 1.3) 2 3 (3.1 (3.1.1 3.1.2) 3.2 3.3 (3.3.1)) 4)&n; *&n; * Subtrees appear after their file, out of lexical order,&n; * which would be before their file.  It&squot;s easier.&n; *&n; * A directory can&squot;t hold more than 56 files, so 6 bits are used for&n; * position numbers.  If the tree is so deep that the position encoding&n; * doesn&squot;t fit, I&squot;m sure something absolutely fascinating happens.&n; *&n; * The actual sequence of f_pos values is&n; *     0 =&gt; .   -1 =&gt; ..   1 1.1 ... 8.9 9 =&gt; files  -2 =&gt; eof&n; *&n; * The directory inode caches one position-to-dnode correspondence so&n; * we won&squot;t have to repeatedly scan the top levels of the tree. &n; */
...
multiline_comment|/*&n; * Send the given name and ino off to the user dirent struct at *dirent.&n; * Blam it to lowercase if the mount option said to.&n; *&n; * Note that Linux d_reclen is the length of the file name, and has nothing&n; * to do with the length of the dirent record.&n; */
DECL|function|write_one_dirent
r_static
r_void
id|write_one_dirent
c_func
