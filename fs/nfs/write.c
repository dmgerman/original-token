...
l_int|0
suffix:semicolon
multiline_comment|/*&n; * Local structures&n; *&n; * Valid flags for a dirty buffer&n; */
DECL|macro|PG_BUSY
mdefine_line|#define PG_BUSY&t;&t;&t;0x0001
multiline_comment|/*&n; * This is the struct where the WRITE/COMMIT arguments go.&n; */
...
id|wb_page
suffix:semicolon
multiline_comment|/* page to write out */
DECL|member|wb_wait
...
id|wb_timeout
suffix:semicolon
multiline_comment|/* when to write/commit */
DECL|member|wb_offset
r_int
r_int
id|wb_offset
comma
multiline_comment|/* Offset of write */
...
multiline_comment|/*&n; * Write a page to the server. This was supposed to be used for&n; * NFS swapping only.&n; * FIXME: Using this for mmap is pointless, breaks asynchronous&n; *        writebacks, and is extremely slow.&n; */
r_int
...
id|inode
op_assign
id|req-&gt;wb_file-&gt;f_dentry-&gt;d_inode
suffix:semicolon
id|list_del
c_func
(paren
...
(paren
id|KERN_ERR
l_string|&quot;NFS: Unhashed request attempted added to a list!&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
...
multiline_comment|/*&n; * Create a write request.&n; * Page must be locked by the caller. This makes sure we never create&n; * two different requests for the same page, and avoids possible deadlock&n; * when we reach the hard limit on the number of dirty pages.&n; */
r_static
r_struct
id|nfs_page
op_star
...
r_do
(brace
multiline_comment|/* If we&squot;re over the soft limit, flush out old requests */
r_if
c_cond
(paren
id|nfs_nr_requests
...
suffix:semicolon
multiline_comment|/* If we&squot;re still over the soft limit, wake up some requests */
r_if
c_cond
(paren
id|nfs_nr_requests
...
multiline_comment|/* If we haven&squot;t reached the hard limit yet,&n;&t;&t; * try to allocate the request struct */
r_if
c_cond
(paren
id|nfs_nr_requests
...
r_static
r_int
DECL|function|coalesce_requests
id|coalesce_requests
c_func
(paren
r_struct
id|list_head
...
suffix:semicolon
multiline_comment|/* Create the request. It&squot;s safe to sleep in this call because&n;&t;&t; * we only get here if the page is locked.&n;&t;&t; */
r_new
op_assign
id|nfs_create_request
c_func
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t; * If we&squot;re running out of requests, flush out everything&n;&t; * in order to reduce memory useage...&n;&t; */
r_if
c_cond
(paren
id|nfs_nr_requests
...
c_func
(paren
l_string|&quot;NFS: %4d initiated write call (req %s/%s count %d nriov %d)&bslash;n&quot;
comma
id|task-&gt;tk_pid
comma
id|dentry-&gt;d_parent-&gt;d_name.name
...
(brace
multiline_comment|/* We tried a write call, but the server did not&n;&t;&t; * commit data to stable storage even though we&n;&t;&t; * requested it.&n;&t;&t; */
r_static
r_int
r_int
id|complain
...
c_func
(paren
l_string|&quot;NFS: write (%s/%s %d@%Ld)&quot;
comma
id|req-&gt;wb_file-&gt;f_dentry-&gt;d_parent-&gt;d_name.name
comma
id|req-&gt;wb_file-&gt;f_dentry-&gt;d_name.name
comma
id|req-&gt;wb_bytes
comma
id|page_offset
