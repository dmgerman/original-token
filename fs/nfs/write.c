...
id|ENOBUFS
suffix:semicolon
multiline_comment|/*&n;&t; * Ok, there&squot;s another user of this page with the new request..&n;&t; * Increment the usage count, and schedule the request (the&n;&t; * end of the request will drop the usage count..)&n;&t; */
id|atomic_inc
c_func
(paren
op_amp
...
multiline_comment|/*&n; * Try to flush any dirty pages, returning a success marker..&n; *&n; * Unlike &quot;nfs_flush_dirty_pages()&quot; this does not invalidate&n; * the writes if it is interrupted. The caller will instead&n; * look at the error code and gracefully fail to do what it&n; * wanted to do. &n; */
r_int
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Iterate over all outstanding write requests,&n;&t;&t; * looking for any that are ours..&n;&t;&t; */
r_for
c_loop
(paren
suffix:semicolon
