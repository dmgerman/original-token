multiline_comment|/*&n; *  linux/fs/nfs/inode.c&n; *&n; *  Copyright (C) 1992  Rick Sladkey&n; *&n; *  nfs inode and superblock handling functions&n; *&n; *  Modularised by Alan Cox &lt;Alan.Cox@linux.org&gt;, while hacking some&n; *  experimental NFS changes. Modularisation taken straight from SYS5 fs.&n; *&n; *  Change to nfs_read_super() to permit NFS mounts to multi-homed hosts.&n; *  J.S.Peatfield@damtp.cam.ac.uk&n; *&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/locks.h&gt;
macro_line|#include &lt;linux/unistd.h&gt;
macro_line|#include &lt;linux/sunrpc/clnt.h&gt;
macro_line|#include &lt;linux/sunrpc/stats.h&gt;
macro_line|#include &lt;linux/nfs_fs.h&gt;
macro_line|#include &lt;linux/lockd/bind.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
DECL|macro|CONFIG_NFS_SNAPSHOT
mdefine_line|#define CONFIG_NFS_SNAPSHOT 1
DECL|macro|NFSDBG_FACILITY
mdefine_line|#define NFSDBG_FACILITY&t;&t;NFSDBG_VFS
DECL|macro|NFS_PARANOIA
mdefine_line|#define NFS_PARANOIA 1
r_static
r_struct
id|inode
op_star
id|__nfs_fhget
c_func
(paren
r_struct
id|super_block
op_star
comma
r_struct
id|nfs_fattr
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_zap_caches
c_func
(paren
r_struct
id|inode
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_invalidate_inode
c_func
(paren
r_struct
id|inode
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_read_inode
c_func
(paren
r_struct
id|inode
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_put_inode
c_func
(paren
r_struct
id|inode
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_delete_inode
c_func
(paren
r_struct
id|inode
op_star
)paren
suffix:semicolon
r_static
r_int
id|nfs_notify_change
c_func
(paren
r_struct
id|dentry
op_star
comma
r_struct
id|iattr
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_put_super
c_func
(paren
r_struct
id|super_block
op_star
)paren
suffix:semicolon
r_static
r_void
id|nfs_umount_begin
c_func
(paren
r_struct
id|super_block
op_star
)paren
suffix:semicolon
r_static
r_int
id|nfs_statfs
c_func
(paren
r_struct
id|super_block
op_star
comma
r_struct
id|statfs
op_star
comma
r_int
)paren
suffix:semicolon
DECL|variable|nfs_sops
r_static
r_struct
id|super_operations
id|nfs_sops
op_assign
(brace
id|nfs_read_inode
comma
multiline_comment|/* read inode */
l_int|NULL
comma
multiline_comment|/* write inode */
id|nfs_put_inode
comma
multiline_comment|/* put inode */
id|nfs_delete_inode
comma
multiline_comment|/* delete inode */
id|nfs_notify_change
comma
multiline_comment|/* notify change */
id|nfs_put_super
comma
multiline_comment|/* put superblock */
l_int|NULL
comma
multiline_comment|/* write superblock */
id|nfs_statfs
comma
multiline_comment|/* stat filesystem */
l_int|NULL
comma
multiline_comment|/* no remount */
l_int|NULL
comma
multiline_comment|/* no clear inode */
id|nfs_umount_begin
multiline_comment|/* umount attempt begin */
)brace
suffix:semicolon
DECL|variable|nfs_rpcstat
r_struct
id|rpc_stat
id|nfs_rpcstat
op_assign
(brace
op_amp
id|nfs_program
)brace
suffix:semicolon
multiline_comment|/*&n; * The &quot;read_inode&quot; function doesn&squot;t actually do anything:&n; * the real data is filled in later in nfs_fhget. Here we&n; * just mark the cache times invalid, and zero out i_mode&n; * (the latter makes &quot;nfs_refresh_inode&quot; do the right thing&n; * wrt pipe inodes)&n; */
r_static
r_void
DECL|function|nfs_read_inode
id|nfs_read_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|inode-&gt;i_blksize
op_assign
id|inode-&gt;i_sb-&gt;s_blocksize
suffix:semicolon
id|inode-&gt;i_mode
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_rdev
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_op
op_assign
l_int|NULL
suffix:semicolon
id|NFS_CACHEINV
c_func
(paren
id|inode
)paren
suffix:semicolon
id|NFS_ATTRTIMEO
c_func
(paren
id|inode
)paren
op_assign
id|NFS_MINATTRTIMEO
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|nfs_put_inode
id|nfs_put_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|dprintk
c_func
(paren
l_string|&quot;NFS: put_inode(%x/%ld)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * We want to get rid of unused inodes ...&n;&t; */
r_if
c_cond
(paren
id|inode-&gt;i_count
op_eq
l_int|1
)paren
id|inode-&gt;i_nlink
op_assign
l_int|0
suffix:semicolon
)brace
r_static
r_void
DECL|function|nfs_delete_inode
id|nfs_delete_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_int
id|failed
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;NFS: delete_inode(%x/%ld)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Flush out any pending write requests ...&n;&t; */
r_if
c_cond
(paren
id|NFS_WRITEBACK
c_func
(paren
id|inode
)paren
op_ne
l_int|NULL
)paren
(brace
r_int
r_int
id|timeout
op_assign
id|jiffies
op_plus
l_int|5
op_star
id|HZ
suffix:semicolon
macro_line|#ifdef NFS_DEBUG_VERBOSE
id|printk
c_func
(paren
l_string|&quot;nfs_delete_inode: inode %ld has pending RPC requests&bslash;n&quot;
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
macro_line|#endif
id|nfs_inval
c_func
(paren
id|inode
)paren
suffix:semicolon
r_while
c_loop
(paren
id|NFS_WRITEBACK
c_func
(paren
id|inode
)paren
op_ne
l_int|NULL
op_logical_and
id|time_before
c_func
(paren
id|jiffies
comma
id|timeout
)paren
)paren
(brace
id|current-&gt;state
op_assign
id|TASK_INTERRUPTIBLE
suffix:semicolon
id|schedule_timeout
c_func
(paren
id|HZ
op_div
l_int|10
)paren
suffix:semicolon
)brace
id|current-&gt;state
op_assign
id|TASK_RUNNING
suffix:semicolon
r_if
c_cond
(paren
id|NFS_WRITEBACK
c_func
(paren
id|inode
)paren
op_ne
l_int|NULL
)paren
id|printk
c_func
(paren
l_string|&quot;NFS: Arghhh, stuck RPC requests!&bslash;n&quot;
)paren
suffix:semicolon
)brace
id|failed
op_assign
id|nfs_check_failed_request
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|failed
)paren
id|printk
c_func
(paren
l_string|&quot;NFS: inode %ld had %d failed requests&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
id|failed
)paren
suffix:semicolon
id|clear_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
r_void
DECL|function|nfs_put_super
id|nfs_put_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|nfs_server
op_star
id|server
op_assign
op_amp
id|sb-&gt;u.nfs_sb.s_server
suffix:semicolon
r_struct
id|rpc_clnt
op_star
id|rpc
suffix:semicolon
r_if
c_cond
(paren
(paren
id|rpc
op_assign
id|server-&gt;client
)paren
op_ne
l_int|NULL
)paren
id|rpc_shutdown_client
c_func
(paren
id|rpc
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|server-&gt;flags
op_amp
id|NFS_MOUNT_NONLM
)paren
)paren
id|lockd_down
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/* release rpc.lockd */
id|rpciod_down
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/* release rpciod */
multiline_comment|/*&n;&t; * Invalidate the dircache for this superblock.&n;&t; */
id|nfs_invalidate_dircache_sb
c_func
(paren
id|sb
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|server-&gt;hostname
)paren
suffix:semicolon
id|MOD_DEC_USE_COUNT
suffix:semicolon
)brace
r_void
DECL|function|nfs_umount_begin
id|nfs_umount_begin
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|nfs_server
op_star
id|server
op_assign
op_amp
id|sb-&gt;u.nfs_sb.s_server
suffix:semicolon
r_struct
id|rpc_clnt
op_star
id|rpc
suffix:semicolon
multiline_comment|/* -EIO all pending I/O */
r_if
c_cond
(paren
(paren
id|rpc
op_assign
id|server-&gt;client
)paren
op_ne
l_int|NULL
)paren
id|rpc_killall_tasks
c_func
(paren
id|rpc
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Compute and set NFS server blocksize&n; */
r_static
r_int
r_int
DECL|function|nfs_block_size
id|nfs_block_size
c_func
(paren
r_int
r_int
id|bsize
comma
r_int
r_char
op_star
id|nrbitsp
)paren
(brace
r_if
c_cond
(paren
id|bsize
OL
l_int|1024
)paren
id|bsize
op_assign
id|NFS_DEF_FILE_IO_BUFFER_SIZE
suffix:semicolon
r_else
r_if
c_cond
(paren
id|bsize
op_ge
id|NFS_MAX_FILE_IO_BUFFER_SIZE
)paren
id|bsize
op_assign
id|NFS_MAX_FILE_IO_BUFFER_SIZE
suffix:semicolon
multiline_comment|/* make sure blocksize is a power of two */
r_if
c_cond
(paren
(paren
id|bsize
op_amp
(paren
id|bsize
op_minus
l_int|1
)paren
)paren
op_logical_or
id|nrbitsp
)paren
(brace
r_int
r_int
id|nrbits
suffix:semicolon
r_for
c_loop
(paren
id|nrbits
op_assign
l_int|31
suffix:semicolon
id|nrbits
op_logical_and
op_logical_neg
(paren
id|bsize
op_amp
(paren
l_int|1
op_lshift
id|nrbits
)paren
)paren
suffix:semicolon
id|nrbits
op_decrement
)paren
suffix:semicolon
id|bsize
op_assign
l_int|1
op_lshift
id|nrbits
suffix:semicolon
r_if
c_cond
(paren
id|nrbitsp
)paren
op_star
id|nrbitsp
op_assign
id|nrbits
suffix:semicolon
r_if
c_cond
(paren
id|bsize
OL
id|NFS_DEF_FILE_IO_BUFFER_SIZE
)paren
id|bsize
op_assign
id|NFS_DEF_FILE_IO_BUFFER_SIZE
suffix:semicolon
)brace
r_return
id|bsize
suffix:semicolon
)brace
multiline_comment|/*&n; * The way this works is that the mount process passes a structure&n; * in the data argument which contains the server&squot;s IP address&n; * and the root file handle obtained from the server&squot;s mount&n; * daemon. We stash these away in the private superblock fields.&n; */
r_struct
id|super_block
op_star
DECL|function|nfs_read_super
id|nfs_read_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_void
op_star
id|raw_data
comma
r_int
id|silent
)paren
(brace
r_struct
id|nfs_mount_data
op_star
id|data
op_assign
(paren
r_struct
id|nfs_mount_data
op_star
)paren
id|raw_data
suffix:semicolon
r_struct
id|nfs_server
op_star
id|server
suffix:semicolon
r_struct
id|rpc_xprt
op_star
id|xprt
suffix:semicolon
r_struct
id|rpc_clnt
op_star
id|clnt
suffix:semicolon
r_struct
id|nfs_fh
op_star
id|root_fh
suffix:semicolon
r_struct
id|inode
op_star
id|root_inode
suffix:semicolon
r_int
r_int
id|authflavor
suffix:semicolon
r_int
id|tcp
suffix:semicolon
r_struct
id|sockaddr_in
id|srvaddr
suffix:semicolon
r_struct
id|rpc_timeout
id|timeparms
suffix:semicolon
r_struct
id|nfs_fattr
id|fattr
suffix:semicolon
id|MOD_INC_USE_COUNT
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|data
)paren
r_goto
id|out_miss_args
suffix:semicolon
r_if
c_cond
(paren
id|data-&gt;version
op_ne
id|NFS_MOUNT_VERSION
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;nfs warning: mount version %s than kernel&bslash;n&quot;
comma
id|data-&gt;version
OL
id|NFS_MOUNT_VERSION
ques
c_cond
l_string|&quot;older&quot;
suffix:colon
l_string|&quot;newer&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|data-&gt;version
OL
l_int|2
)paren
id|data-&gt;namlen
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|data-&gt;version
OL
l_int|3
)paren
id|data-&gt;bsize
op_assign
l_int|0
suffix:semicolon
)brace
multiline_comment|/* We now require that the mount process passes the remote address */
id|memcpy
c_func
(paren
op_amp
id|srvaddr
comma
op_amp
id|data-&gt;addr
comma
r_sizeof
(paren
id|srvaddr
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|srvaddr.sin_addr.s_addr
op_eq
id|INADDR_ANY
)paren
r_goto
id|out_no_remote
suffix:semicolon
id|lock_super
c_func
(paren
id|sb
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_ODD_RENAME
suffix:semicolon
multiline_comment|/* This should go away */
id|sb-&gt;s_magic
op_assign
id|NFS_SUPER_MAGIC
suffix:semicolon
id|sb-&gt;s_op
op_assign
op_amp
id|nfs_sops
suffix:semicolon
id|sb-&gt;s_blocksize
op_assign
id|nfs_block_size
c_func
(paren
id|data-&gt;bsize
comma
op_amp
id|sb-&gt;s_blocksize_bits
)paren
suffix:semicolon
id|sb-&gt;u.nfs_sb.s_root
op_assign
id|data-&gt;root
suffix:semicolon
id|server
op_assign
op_amp
id|sb-&gt;u.nfs_sb.s_server
suffix:semicolon
id|server-&gt;rsize
op_assign
id|nfs_block_size
c_func
(paren
id|data-&gt;rsize
comma
l_int|NULL
)paren
suffix:semicolon
id|server-&gt;wsize
op_assign
id|nfs_block_size
c_func
(paren
id|data-&gt;wsize
comma
l_int|NULL
)paren
suffix:semicolon
id|server-&gt;flags
op_assign
id|data-&gt;flags
suffix:semicolon
r_if
c_cond
(paren
id|data-&gt;flags
op_amp
id|NFS_MOUNT_NOAC
)paren
(brace
id|data-&gt;acregmin
op_assign
id|data-&gt;acregmax
op_assign
l_int|0
suffix:semicolon
id|data-&gt;acdirmin
op_assign
id|data-&gt;acdirmax
op_assign
l_int|0
suffix:semicolon
)brace
id|server-&gt;acregmin
op_assign
id|data-&gt;acregmin
op_star
id|HZ
suffix:semicolon
id|server-&gt;acregmax
op_assign
id|data-&gt;acregmax
op_star
id|HZ
suffix:semicolon
id|server-&gt;acdirmin
op_assign
id|data-&gt;acdirmin
op_star
id|HZ
suffix:semicolon
id|server-&gt;acdirmax
op_assign
id|data-&gt;acdirmax
op_star
id|HZ
suffix:semicolon
id|server-&gt;hostname
op_assign
id|kmalloc
c_func
(paren
id|strlen
c_func
(paren
id|data-&gt;hostname
)paren
op_plus
l_int|1
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|server-&gt;hostname
)paren
r_goto
id|out_unlock
suffix:semicolon
id|strcpy
c_func
(paren
id|server-&gt;hostname
comma
id|data-&gt;hostname
)paren
suffix:semicolon
multiline_comment|/* Which protocol do we use? */
id|tcp
op_assign
(paren
id|data-&gt;flags
op_amp
id|NFS_MOUNT_TCP
)paren
suffix:semicolon
multiline_comment|/* Initialize timeout values */
id|timeparms.to_initval
op_assign
id|data-&gt;timeo
op_star
id|HZ
op_div
l_int|10
suffix:semicolon
id|timeparms.to_retries
op_assign
id|data-&gt;retrans
suffix:semicolon
id|timeparms.to_maxval
op_assign
id|tcp
ques
c_cond
id|RPC_MAX_TCP_TIMEOUT
suffix:colon
id|RPC_MAX_UDP_TIMEOUT
suffix:semicolon
id|timeparms.to_exponential
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* Now create transport and client */
id|xprt
op_assign
id|xprt_create_proto
c_func
(paren
id|tcp
ques
c_cond
id|IPPROTO_TCP
suffix:colon
id|IPPROTO_UDP
comma
op_amp
id|srvaddr
comma
op_amp
id|timeparms
)paren
suffix:semicolon
r_if
c_cond
(paren
id|xprt
op_eq
l_int|NULL
)paren
r_goto
id|out_no_xprt
suffix:semicolon
multiline_comment|/* Choose authentication flavor */
id|authflavor
op_assign
id|RPC_AUTH_UNIX
suffix:semicolon
r_if
c_cond
(paren
id|data-&gt;flags
op_amp
id|NFS_MOUNT_SECURE
)paren
id|authflavor
op_assign
id|RPC_AUTH_DES
suffix:semicolon
r_else
r_if
c_cond
(paren
id|data-&gt;flags
op_amp
id|NFS_MOUNT_KERBEROS
)paren
id|authflavor
op_assign
id|RPC_AUTH_KRB
suffix:semicolon
id|clnt
op_assign
id|rpc_create_client
c_func
(paren
id|xprt
comma
id|server-&gt;hostname
comma
op_amp
id|nfs_program
comma
id|NFS_VERSION
comma
id|authflavor
)paren
suffix:semicolon
r_if
c_cond
(paren
id|clnt
op_eq
l_int|NULL
)paren
r_goto
id|out_no_client
suffix:semicolon
id|clnt-&gt;cl_intr
op_assign
(paren
id|data-&gt;flags
op_amp
id|NFS_MOUNT_INTR
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
suffix:semicolon
id|clnt-&gt;cl_softrtry
op_assign
(paren
id|data-&gt;flags
op_amp
id|NFS_MOUNT_SOFT
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
suffix:semicolon
id|clnt-&gt;cl_chatty
op_assign
l_int|1
suffix:semicolon
id|server-&gt;client
op_assign
id|clnt
suffix:semicolon
multiline_comment|/* Fire up rpciod if not yet running */
r_if
c_cond
(paren
id|rpciod_up
c_func
(paren
)paren
op_ne
l_int|0
)paren
r_goto
id|out_no_iod
suffix:semicolon
multiline_comment|/*&n;&t; * Keep the super block locked while we try to get &n;&t; * the root fh attributes.&n;&t; */
id|root_fh
op_assign
id|kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|nfs_fh
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|root_fh
)paren
r_goto
id|out_no_fh
suffix:semicolon
op_star
id|root_fh
op_assign
id|data-&gt;root
suffix:semicolon
r_if
c_cond
(paren
id|nfs_proc_getattr
c_func
(paren
id|server
comma
id|root_fh
comma
op_amp
id|fattr
)paren
op_ne
l_int|0
)paren
r_goto
id|out_no_fattr
suffix:semicolon
id|root_inode
op_assign
id|__nfs_fhget
c_func
(paren
id|sb
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|root_inode
)paren
r_goto
id|out_no_root
suffix:semicolon
id|sb-&gt;s_root
op_assign
id|d_alloc_root
c_func
(paren
id|root_inode
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;s_root
)paren
r_goto
id|out_no_root
suffix:semicolon
id|sb-&gt;s_root-&gt;d_op
op_assign
op_amp
id|nfs_dentry_operations
suffix:semicolon
id|sb-&gt;s_root-&gt;d_fsdata
op_assign
id|root_fh
suffix:semicolon
multiline_comment|/* We&squot;re airborne */
id|unlock_super
c_func
(paren
id|sb
)paren
suffix:semicolon
multiline_comment|/* Check whether to start the lockd process */
r_if
c_cond
(paren
op_logical_neg
(paren
id|server-&gt;flags
op_amp
id|NFS_MOUNT_NONLM
)paren
)paren
id|lockd_up
c_func
(paren
)paren
suffix:semicolon
r_return
id|sb
suffix:semicolon
multiline_comment|/* Yargs. It didn&squot;t work out. */
id|out_no_root
suffix:colon
id|printk
c_func
(paren
l_string|&quot;nfs_read_super: get root inode failed&bslash;n&quot;
)paren
suffix:semicolon
id|iput
c_func
(paren
id|root_inode
)paren
suffix:semicolon
r_goto
id|out_free_fh
suffix:semicolon
id|out_no_fattr
suffix:colon
id|printk
c_func
(paren
l_string|&quot;nfs_read_super: get root fattr failed&bslash;n&quot;
)paren
suffix:semicolon
id|out_free_fh
suffix:colon
id|kfree
c_func
(paren
id|root_fh
)paren
suffix:semicolon
id|out_no_fh
suffix:colon
id|rpciod_down
c_func
(paren
)paren
suffix:semicolon
r_goto
id|out_shutdown
suffix:semicolon
id|out_no_iod
suffix:colon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;NFS: couldn&squot;t start rpciod!&bslash;n&quot;
)paren
suffix:semicolon
id|out_shutdown
suffix:colon
id|rpc_shutdown_client
c_func
(paren
id|server-&gt;client
)paren
suffix:semicolon
r_goto
id|out_free_host
suffix:semicolon
id|out_no_client
suffix:colon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;NFS: cannot create RPC client.&bslash;n&quot;
)paren
suffix:semicolon
id|xprt_destroy
c_func
(paren
id|xprt
)paren
suffix:semicolon
r_goto
id|out_free_host
suffix:semicolon
id|out_no_xprt
suffix:colon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;NFS: cannot create RPC transport.&bslash;n&quot;
)paren
suffix:semicolon
id|out_free_host
suffix:colon
id|kfree
c_func
(paren
id|server-&gt;hostname
)paren
suffix:semicolon
id|out_unlock
suffix:colon
id|unlock_super
c_func
(paren
id|sb
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
id|out_no_remote
suffix:colon
id|printk
c_func
(paren
l_string|&quot;NFS: mount program didn&squot;t pass remote address!&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
id|out_miss_args
suffix:colon
id|printk
c_func
(paren
l_string|&quot;nfs_read_super: missing data argument&bslash;n&quot;
)paren
suffix:semicolon
id|out_fail
suffix:colon
id|sb-&gt;s_dev
op_assign
l_int|0
suffix:semicolon
id|MOD_DEC_USE_COUNT
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
r_static
r_int
DECL|function|nfs_statfs
id|nfs_statfs
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|statfs
op_star
id|buf
comma
r_int
id|bufsiz
)paren
(brace
r_int
id|error
suffix:semicolon
r_struct
id|nfs_fsinfo
id|res
suffix:semicolon
r_struct
id|statfs
id|tmp
suffix:semicolon
id|error
op_assign
id|nfs_proc_statfs
c_func
(paren
op_amp
id|sb-&gt;u.nfs_sb.s_server
comma
op_amp
id|sb-&gt;u.nfs_sb.s_root
comma
op_amp
id|res
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;nfs_statfs: statfs error = %d&bslash;n&quot;
comma
op_minus
id|error
)paren
suffix:semicolon
id|res.bsize
op_assign
id|res.blocks
op_assign
id|res.bfree
op_assign
id|res.bavail
op_assign
l_int|0
suffix:semicolon
)brace
id|tmp.f_type
op_assign
id|NFS_SUPER_MAGIC
suffix:semicolon
id|tmp.f_bsize
op_assign
id|res.bsize
suffix:semicolon
id|tmp.f_blocks
op_assign
id|res.blocks
suffix:semicolon
id|tmp.f_bfree
op_assign
id|res.bfree
suffix:semicolon
id|tmp.f_bavail
op_assign
id|res.bavail
suffix:semicolon
id|tmp.f_files
op_assign
l_int|0
suffix:semicolon
id|tmp.f_ffree
op_assign
l_int|0
suffix:semicolon
id|tmp.f_namelen
op_assign
id|NAME_MAX
suffix:semicolon
r_return
id|copy_to_user
c_func
(paren
id|buf
comma
op_amp
id|tmp
comma
id|bufsiz
)paren
ques
c_cond
op_minus
id|EFAULT
suffix:colon
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Free all unused dentries in an inode&squot;s alias list.&n; *&n; * Subtle note: we have to be very careful not to cause&n; * any IO operations with the stale dentries, as this&n; * could cause file corruption. But since the dentry&n; * count is 0 and all pending IO for a dentry has been&n; * flushed when the count went to 0, we&squot;re safe here.&n; * Also returns the number of unhashed dentries&n; */
r_static
r_int
DECL|function|nfs_free_dentries
id|nfs_free_dentries
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|list_head
op_star
id|tmp
comma
op_star
id|head
op_assign
op_amp
id|inode-&gt;i_dentry
suffix:semicolon
r_int
id|unhashed
suffix:semicolon
id|restart
suffix:colon
id|tmp
op_assign
id|head
suffix:semicolon
id|unhashed
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
(paren
id|tmp
op_assign
id|tmp-&gt;next
)paren
op_ne
id|head
)paren
(brace
r_struct
id|dentry
op_star
id|dentry
op_assign
id|list_entry
c_func
(paren
id|tmp
comma
r_struct
id|dentry
comma
id|d_alias
)paren
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;nfs_free_dentries: found %s/%s, d_count=%d, hashed=%d&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
comma
id|dentry-&gt;d_count
comma
op_logical_neg
id|list_empty
c_func
(paren
op_amp
id|dentry-&gt;d_hash
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dentry-&gt;d_count
)paren
(brace
id|dget
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|d_drop
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|dput
c_func
(paren
id|dentry
)paren
suffix:semicolon
r_goto
id|restart
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|list_empty
c_func
(paren
op_amp
id|dentry-&gt;d_hash
)paren
)paren
id|unhashed
op_increment
suffix:semicolon
)brace
r_return
id|unhashed
suffix:semicolon
)brace
multiline_comment|/*&n; * Invalidate the local caches&n; */
r_static
r_void
DECL|function|nfs_zap_caches
id|nfs_zap_caches
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|NFS_ATTRTIMEO
c_func
(paren
id|inode
)paren
op_assign
id|NFS_MINATTRTIMEO
c_func
(paren
id|inode
)paren
suffix:semicolon
id|NFS_CACHEINV
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|nfs_invalidate_dircache
c_func
(paren
id|inode
)paren
suffix:semicolon
r_else
id|invalidate_inode_pages
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Invalidate, but do not unhash, the inode&n; */
r_static
r_void
DECL|function|nfs_invalidate_inode
id|nfs_invalidate_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|umode_t
id|save_mode
op_assign
id|inode-&gt;i_mode
suffix:semicolon
id|make_bad_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|save_mode
suffix:semicolon
id|nfs_inval
c_func
(paren
id|inode
)paren
suffix:semicolon
id|nfs_zap_caches
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Fill in inode information from the fattr.&n; */
r_static
r_void
DECL|function|nfs_fill_inode
id|nfs_fill_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|nfs_fattr
op_star
id|fattr
)paren
(brace
multiline_comment|/*&n;&t; * Check whether the mode has been set, as we only want to&n;&t; * do this once. (We don&squot;t allow inodes to change types.)&n;&t; */
r_if
c_cond
(paren
id|inode-&gt;i_mode
op_eq
l_int|0
)paren
(brace
id|inode-&gt;i_mode
op_assign
id|fattr-&gt;mode
suffix:semicolon
r_if
c_cond
(paren
id|S_ISREG
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|nfs_file_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|nfs_dir_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISLNK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|inode-&gt;i_op
op_assign
op_amp
id|nfs_symlink_inode_operations
suffix:semicolon
r_else
r_if
c_cond
(paren
id|S_ISCHR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
id|inode-&gt;i_op
op_assign
op_amp
id|chrdev_inode_operations
suffix:semicolon
id|inode-&gt;i_rdev
op_assign
id|to_kdev_t
c_func
(paren
id|fattr-&gt;rdev
)paren
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
id|inode-&gt;i_op
op_assign
op_amp
id|blkdev_inode_operations
suffix:semicolon
id|inode-&gt;i_rdev
op_assign
id|to_kdev_t
c_func
(paren
id|fattr-&gt;rdev
)paren
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|S_ISFIFO
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|init_fifo
c_func
(paren
id|inode
)paren
suffix:semicolon
r_else
id|inode-&gt;i_op
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Preset the size and mtime, as there&squot;s no need&n;&t;&t; * to invalidate the caches.&n;&t;&t; */
id|inode-&gt;i_size
op_assign
id|fattr-&gt;size
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|fattr-&gt;mtime.seconds
suffix:semicolon
id|NFS_OLDMTIME
c_func
(paren
id|inode
)paren
op_assign
id|fattr-&gt;mtime.seconds
suffix:semicolon
)brace
id|nfs_refresh_inode
c_func
(paren
id|inode
comma
id|fattr
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * This is our own version of iget that looks up inodes by file handle&n; * instead of inode number.  We use this technique instead of using&n; * the vfs read_inode function because there is no way to pass the&n; * file handle or current attributes into the read_inode function.&n; *&n; * We provide a special check for NetApp .snapshot directories to avoid&n; * inode aliasing problems. All snapshot inodes are anonymous (unhashed).&n; */
r_struct
id|inode
op_star
DECL|function|nfs_fhget
id|nfs_fhget
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|nfs_fh
op_star
id|fhandle
comma
r_struct
id|nfs_fattr
op_star
id|fattr
)paren
(brace
r_struct
id|super_block
op_star
id|sb
op_assign
id|dentry-&gt;d_sb
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;NFS: nfs_fhget(%s/%s fileid=%d)&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
comma
id|fattr-&gt;fileid
)paren
suffix:semicolon
multiline_comment|/* Install the file handle in the dentry */
op_star
(paren
(paren
r_struct
id|nfs_fh
op_star
)paren
id|dentry-&gt;d_fsdata
)paren
op_assign
op_star
id|fhandle
suffix:semicolon
macro_line|#ifdef CONFIG_NFS_SNAPSHOT
multiline_comment|/*&n;&t; * Check for NetApp snapshot dentries, and get an &n;&t; * unhashed inode to avoid aliasing problems.&n;&t; */
r_if
c_cond
(paren
(paren
id|dentry-&gt;d_parent-&gt;d_inode-&gt;u.nfs_i.flags
op_amp
id|NFS_IS_SNAPSHOT
)paren
op_logical_or
(paren
id|dentry-&gt;d_name.len
op_eq
l_int|9
op_logical_and
id|memcmp
c_func
(paren
id|dentry-&gt;d_name.name
comma
l_string|&quot;.snapshot&quot;
comma
l_int|9
)paren
op_eq
l_int|0
)paren
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|get_empty_inode
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
r_goto
id|out
suffix:semicolon
id|inode-&gt;i_sb
op_assign
id|sb
suffix:semicolon
id|inode-&gt;i_dev
op_assign
id|sb-&gt;s_dev
suffix:semicolon
id|inode-&gt;i_flags
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_ino
op_assign
id|fattr-&gt;fileid
suffix:semicolon
id|nfs_read_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
id|nfs_fill_inode
c_func
(paren
id|inode
comma
id|fattr
)paren
suffix:semicolon
id|inode-&gt;u.nfs_i.flags
op_or_assign
id|NFS_IS_SNAPSHOT
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;NFS: nfs_fhget(snapshot ino=%ld)&bslash;n&quot;
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
id|out
suffix:colon
r_return
id|inode
suffix:semicolon
)brace
macro_line|#endif
r_return
id|__nfs_fhget
c_func
(paren
id|sb
comma
id|fattr
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Look up the inode by super block and fattr-&gt;fileid.&n; *&n; * Note carefully the special handling of busy inodes (i_count &gt; 1).&n; * With the kernel 2.1.xx dcache all inodes except hard links must&n; * have i_count == 1 after iget(). Otherwise, it indicates that the&n; * server has reused a fileid (i_ino) and we have a stale inode.&n; */
r_static
r_struct
id|inode
op_star
DECL|function|__nfs_fhget
id|__nfs_fhget
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|nfs_fattr
op_star
id|fattr
)paren
(brace
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_int
id|max_count
comma
id|stale_inode
comma
id|unhashed
op_assign
l_int|0
suffix:semicolon
id|retry
suffix:colon
id|inode
op_assign
id|iget
c_func
(paren
id|sb
comma
id|fattr-&gt;fileid
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
r_goto
id|out_no_inode
suffix:semicolon
multiline_comment|/* N.B. This should be impossible ... */
r_if
c_cond
(paren
id|inode-&gt;i_ino
op_ne
id|fattr-&gt;fileid
)paren
r_goto
id|out_bad_id
suffix:semicolon
multiline_comment|/*&n;&t; * Check for busy inodes, and attempt to get rid of any&n;&t; * unused local references. If successful, we release the&n;&t; * inode and try again.&n;&t; *&n;&t; * Note that the busy test uses the values in the fattr,&n;&t; * as the inode may have become a different object.&n;&t; * (We can probably handle modes changes here, too.)&n;&t; */
id|stale_inode
op_assign
id|inode-&gt;i_mode
op_logical_and
(paren
(paren
id|fattr-&gt;mode
op_xor
id|inode-&gt;i_mode
)paren
op_amp
id|S_IFMT
)paren
suffix:semicolon
id|stale_inode
op_or_assign
id|inode-&gt;i_count
op_logical_and
id|inode-&gt;i_count
op_eq
id|unhashed
suffix:semicolon
id|max_count
op_assign
id|S_ISDIR
c_func
(paren
id|fattr-&gt;mode
)paren
ques
c_cond
l_int|1
suffix:colon
id|fattr-&gt;nlink
suffix:semicolon
r_if
c_cond
(paren
id|stale_inode
op_logical_or
id|inode-&gt;i_count
OG
id|max_count
op_plus
id|unhashed
)paren
(brace
id|dprintk
c_func
(paren
l_string|&quot;__nfs_fhget: inode %ld busy, i_count=%d, i_nlink=%d&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
id|inode-&gt;i_count
comma
id|inode-&gt;i_nlink
)paren
suffix:semicolon
id|unhashed
op_assign
id|nfs_free_dentries
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|stale_inode
op_logical_or
id|inode-&gt;i_count
OG
id|max_count
op_plus
id|unhashed
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;__nfs_fhget: inode %ld still busy, i_count=%d&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
id|inode-&gt;i_count
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|list_empty
c_func
(paren
op_amp
id|inode-&gt;i_dentry
)paren
)paren
(brace
r_struct
id|dentry
op_star
id|dentry
suffix:semicolon
id|dentry
op_assign
id|list_entry
c_func
(paren
id|inode-&gt;i_dentry.next
comma
r_struct
id|dentry
comma
id|d_alias
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;__nfs_fhget: killing %s/%s filehandle&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
)paren
suffix:semicolon
id|memset
c_func
(paren
id|dentry-&gt;d_fsdata
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|nfs_fh
)paren
)paren
suffix:semicolon
)brace
id|remove_inode_hash
c_func
(paren
id|inode
)paren
suffix:semicolon
id|nfs_invalidate_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
id|unhashed
op_assign
l_int|0
suffix:semicolon
)brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_goto
id|retry
suffix:semicolon
)brace
id|nfs_fill_inode
c_func
(paren
id|inode
comma
id|fattr
)paren
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;NFS: __nfs_fhget(%x/%ld ct=%d)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
comma
id|inode-&gt;i_count
)paren
suffix:semicolon
id|out
suffix:colon
r_return
id|inode
suffix:semicolon
id|out_no_inode
suffix:colon
id|printk
c_func
(paren
l_string|&quot;__nfs_fhget: iget failed&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
id|out_bad_id
suffix:colon
id|printk
c_func
(paren
l_string|&quot;__nfs_fhget: unexpected inode from iget&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
r_int
DECL|function|nfs_notify_change
id|nfs_notify_change
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|iattr
op_star
id|attr
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_int
id|error
suffix:semicolon
r_struct
id|nfs_sattr
id|sattr
suffix:semicolon
r_struct
id|nfs_fattr
id|fattr
suffix:semicolon
multiline_comment|/*&n;&t; * Make sure the inode is up-to-date.&n;&t; */
id|error
op_assign
id|nfs_revalidate
c_func
(paren
id|dentry
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
(brace
macro_line|#ifdef NFS_PARANOIA
id|printk
c_func
(paren
l_string|&quot;nfs_notify_change: revalidate failed, error=%d&bslash;n&quot;
comma
id|error
)paren
suffix:semicolon
macro_line|#endif
r_goto
id|out
suffix:semicolon
)brace
id|sattr.mode
op_assign
(paren
id|u32
)paren
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_MODE
)paren
id|sattr.mode
op_assign
id|attr-&gt;ia_mode
suffix:semicolon
id|sattr.uid
op_assign
(paren
id|u32
)paren
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_UID
)paren
id|sattr.uid
op_assign
id|attr-&gt;ia_uid
suffix:semicolon
id|sattr.gid
op_assign
(paren
id|u32
)paren
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_GID
)paren
id|sattr.gid
op_assign
id|attr-&gt;ia_gid
suffix:semicolon
id|sattr.size
op_assign
(paren
id|u32
)paren
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_SIZE
)paren
op_logical_and
id|S_ISREG
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|sattr.size
op_assign
id|attr-&gt;ia_size
suffix:semicolon
id|sattr.mtime.seconds
op_assign
id|sattr.mtime.useconds
op_assign
(paren
id|u32
)paren
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_MTIME
)paren
(brace
id|sattr.mtime.seconds
op_assign
id|attr-&gt;ia_mtime
suffix:semicolon
id|sattr.mtime.useconds
op_assign
l_int|0
suffix:semicolon
)brace
id|sattr.atime.seconds
op_assign
id|sattr.atime.useconds
op_assign
(paren
id|u32
)paren
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_ATIME
)paren
(brace
id|sattr.atime.seconds
op_assign
id|attr-&gt;ia_atime
suffix:semicolon
id|sattr.atime.useconds
op_assign
l_int|0
suffix:semicolon
)brace
id|error
op_assign
id|nfs_wb_all
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
id|error
op_assign
id|nfs_proc_setattr
c_func
(paren
id|NFS_DSERVER
c_func
(paren
id|dentry
)paren
comma
id|NFS_FH
c_func
(paren
id|dentry
)paren
comma
op_amp
id|sattr
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
multiline_comment|/*&n;&t; * If we changed the size or mtime, update the inode&n;&t; * now to avoid invalidating the page cache.&n;&t; */
r_if
c_cond
(paren
id|sattr.size
op_ne
(paren
id|u32
)paren
op_minus
l_int|1
)paren
(brace
r_if
c_cond
(paren
id|sattr.size
op_ne
id|fattr.size
)paren
id|printk
c_func
(paren
l_string|&quot;nfs_notify_change: sattr=%d, fattr=%d??&bslash;n&quot;
comma
id|sattr.size
comma
id|fattr.size
)paren
suffix:semicolon
id|inode-&gt;i_size
op_assign
id|sattr.size
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|fattr.mtime.seconds
suffix:semicolon
)brace
r_if
c_cond
(paren
id|sattr.mtime.seconds
op_ne
(paren
id|u32
)paren
op_minus
l_int|1
)paren
id|inode-&gt;i_mtime
op_assign
id|fattr.mtime.seconds
suffix:semicolon
id|error
op_assign
id|nfs_refresh_inode
c_func
(paren
id|inode
comma
op_amp
id|fattr
)paren
suffix:semicolon
id|out
suffix:colon
r_return
id|error
suffix:semicolon
)brace
multiline_comment|/*&n; * Externally visible revalidation function&n; */
r_int
DECL|function|nfs_revalidate
id|nfs_revalidate
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
r_return
id|nfs_revalidate_inode
c_func
(paren
id|NFS_DSERVER
c_func
(paren
id|dentry
)paren
comma
id|dentry
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Revalidate the file on open (this&n; * is separate from the path-revalidation&n; * that we do on any lookup).&n; *&n; * When we actually open a file, we want&n; * fairly strict consistency: make sure that&n; * we&squot;ve updated the attributes within the&n; * last second or so..&n; */
DECL|function|nfs_open
r_int
id|nfs_open
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|filp
)paren
(brace
r_int
id|retval
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|time_after
c_func
(paren
id|jiffies
comma
id|NFS_READTIME
c_func
(paren
id|inode
)paren
op_plus
id|HZ
op_div
l_int|2
)paren
)paren
(brace
r_struct
id|dentry
op_star
id|dentry
op_assign
id|filp-&gt;f_dentry
suffix:semicolon
r_struct
id|nfs_server
op_star
id|server
op_assign
id|NFS_DSERVER
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|retval
op_assign
id|_nfs_revalidate_inode
c_func
(paren
id|server
comma
id|dentry
)paren
suffix:semicolon
)brace
r_return
id|retval
suffix:semicolon
)brace
multiline_comment|/*&n; * This function is called whenever some part of NFS notices that&n; * the cached attributes have to be refreshed.&n; */
r_int
DECL|function|_nfs_revalidate_inode
id|_nfs_revalidate_inode
c_func
(paren
r_struct
id|nfs_server
op_star
id|server
comma
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_int
id|status
op_assign
l_int|0
suffix:semicolon
r_struct
id|nfs_fattr
id|fattr
suffix:semicolon
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;NFS: revalidating %s/%s, ino=%ld&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
id|status
op_assign
id|nfs_proc_getattr
c_func
(paren
id|server
comma
id|NFS_FH
c_func
(paren
id|dentry
)paren
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|status
)paren
(brace
r_int
id|error
suffix:semicolon
id|u32
op_star
id|fh
suffix:semicolon
r_struct
id|nfs_fh
id|fhandle
suffix:semicolon
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;nfs_revalidate_inode: %s/%s getattr failed, ino=%ld, error=%d&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
comma
id|inode-&gt;i_ino
comma
id|status
)paren
suffix:semicolon
r_if
c_cond
(paren
id|status
op_ne
op_minus
id|ESTALE
)paren
r_goto
id|out
suffix:semicolon
multiline_comment|/*&n;&t;&t; * A &quot;stale filehandle&quot; error ... show the current fh&n;&t;&t; * and find out what the filehandle should be.&n;&t;&t; */
id|fh
op_assign
(paren
id|u32
op_star
)paren
id|NFS_FH
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;NFS: bad fh %08x%08x%08x%08x%08x%08x%08x%08x&bslash;n&quot;
comma
id|fh
(braket
l_int|0
)braket
comma
id|fh
(braket
l_int|1
)braket
comma
id|fh
(braket
l_int|2
)braket
comma
id|fh
(braket
l_int|3
)braket
comma
id|fh
(braket
l_int|4
)braket
comma
id|fh
(braket
l_int|5
)braket
comma
id|fh
(braket
l_int|6
)braket
comma
id|fh
(braket
l_int|7
)braket
)paren
suffix:semicolon
id|error
op_assign
id|nfs_proc_lookup
c_func
(paren
id|server
comma
id|NFS_FH
c_func
(paren
id|dentry-&gt;d_parent
)paren
comma
id|dentry-&gt;d_name.name
comma
op_amp
id|fhandle
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
(brace
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;NFS: lookup failed, error=%d&bslash;n&quot;
comma
id|error
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|fh
op_assign
(paren
id|u32
op_star
)paren
op_amp
id|fhandle
suffix:semicolon
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;            %08x%08x%08x%08x%08x%08x%08x%08x&bslash;n&quot;
comma
id|fh
(braket
l_int|0
)braket
comma
id|fh
(braket
l_int|1
)braket
comma
id|fh
(braket
l_int|2
)braket
comma
id|fh
(braket
l_int|3
)braket
comma
id|fh
(braket
l_int|4
)braket
comma
id|fh
(braket
l_int|5
)braket
comma
id|fh
(braket
l_int|6
)braket
comma
id|fh
(braket
l_int|7
)braket
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|status
op_assign
id|nfs_refresh_inode
c_func
(paren
id|inode
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|status
)paren
(brace
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;nfs_revalidate_inode: %s/%s refresh failed, ino=%ld, error=%d&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
comma
id|inode-&gt;i_ino
comma
id|status
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|dfprintk
c_func
(paren
id|PAGECACHE
comma
l_string|&quot;NFS: %s/%s revalidation complete&bslash;n&quot;
comma
id|dentry-&gt;d_parent-&gt;d_name.name
comma
id|dentry-&gt;d_name.name
)paren
suffix:semicolon
id|out
suffix:colon
r_return
id|status
suffix:semicolon
)brace
multiline_comment|/*&n; * Many nfs protocol calls return the new file attributes after&n; * an operation.  Here we update the inode to reflect the state&n; * of the server&squot;s inode.&n; *&n; * This is a bit tricky because we have to make sure all dirty pages&n; * have been sent off to the server before calling invalidate_inode_pages.&n; * To make sure no other process adds more write requests while we try&n; * our best to flush them, we make them sleep during the attribute refresh.&n; *&n; * A very similar scenario holds for the dir cache.&n; */
r_int
DECL|function|nfs_refresh_inode
id|nfs_refresh_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|nfs_fattr
op_star
id|fattr
)paren
(brace
r_int
id|invalid
op_assign
l_int|0
suffix:semicolon
r_int
id|error
op_assign
op_minus
id|EIO
suffix:semicolon
id|dfprintk
c_func
(paren
id|VFS
comma
l_string|&quot;NFS: refresh_inode(%x/%ld ct=%d)&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
comma
id|inode-&gt;i_count
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
op_logical_or
op_logical_neg
id|fattr
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;nfs_refresh_inode: inode or fattr is NULL&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
r_if
c_cond
(paren
id|inode-&gt;i_ino
op_ne
id|fattr-&gt;fileid
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;nfs_refresh_inode: mismatch, ino=%ld, fattr=%d&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
id|fattr-&gt;fileid
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Make sure the inode&squot;s type hasn&squot;t changed.&n;&t; */
r_if
c_cond
(paren
(paren
id|inode-&gt;i_mode
op_amp
id|S_IFMT
)paren
op_ne
(paren
id|fattr-&gt;mode
op_amp
id|S_IFMT
)paren
)paren
r_goto
id|out_changed
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|fattr-&gt;mode
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
id|fattr-&gt;nlink
suffix:semicolon
id|inode-&gt;i_uid
op_assign
id|fattr-&gt;uid
suffix:semicolon
id|inode-&gt;i_gid
op_assign
id|fattr-&gt;gid
suffix:semicolon
id|inode-&gt;i_blocks
op_assign
id|fattr-&gt;blocks
suffix:semicolon
id|inode-&gt;i_atime
op_assign
id|fattr-&gt;atime.seconds
suffix:semicolon
id|inode-&gt;i_ctime
op_assign
id|fattr-&gt;ctime.seconds
suffix:semicolon
multiline_comment|/*&n;&t; * Update the read time so we don&squot;t revalidate too often.&n;&t; */
id|NFS_READTIME
c_func
(paren
id|inode
)paren
op_assign
id|jiffies
suffix:semicolon
id|error
op_assign
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * If we have pending write-back entries, we don&squot;t want&n;&t; * to look at the size or the mtime the server sends us&n;&t; * too closely, as we&squot;re in the middle of modifying them.&n;&t; */
r_if
c_cond
(paren
id|NFS_WRITEBACK
c_func
(paren
id|inode
)paren
)paren
r_goto
id|out
suffix:semicolon
r_if
c_cond
(paren
id|inode-&gt;i_size
op_ne
id|fattr-&gt;size
)paren
(brace
macro_line|#ifdef NFS_DEBUG_VERBOSE
id|printk
c_func
(paren
l_string|&quot;NFS: size change on %x/%ld&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
macro_line|#endif
id|inode-&gt;i_size
op_assign
id|fattr-&gt;size
suffix:semicolon
id|invalid
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|inode-&gt;i_mtime
op_ne
id|fattr-&gt;mtime.seconds
)paren
(brace
macro_line|#ifdef NFS_DEBUG_VERBOSE
id|printk
c_func
(paren
l_string|&quot;NFS: mtime change on %x/%ld&bslash;n&quot;
comma
id|inode-&gt;i_dev
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
macro_line|#endif
id|inode-&gt;i_mtime
op_assign
id|fattr-&gt;mtime.seconds
suffix:semicolon
id|invalid
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|invalid
)paren
r_goto
id|out_invalid
suffix:semicolon
multiline_comment|/* Update attrtimeo value */
r_if
c_cond
(paren
id|fattr-&gt;mtime.seconds
op_eq
id|NFS_OLDMTIME
c_func
(paren
id|inode
)paren
)paren
(brace
r_if
c_cond
(paren
(paren
id|NFS_ATTRTIMEO
c_func
(paren
id|inode
)paren
op_lshift_assign
l_int|1
)paren
OG
id|NFS_MAXATTRTIMEO
c_func
(paren
id|inode
)paren
)paren
id|NFS_ATTRTIMEO
c_func
(paren
id|inode
)paren
op_assign
id|NFS_MAXATTRTIMEO
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
id|NFS_OLDMTIME
c_func
(paren
id|inode
)paren
op_assign
id|fattr-&gt;mtime.seconds
suffix:semicolon
id|out
suffix:colon
r_return
id|error
suffix:semicolon
id|out_changed
suffix:colon
multiline_comment|/*&n;&t; * Big trouble! The inode has become a different object.&n;&t; */
macro_line|#ifdef NFS_PARANOIA
id|printk
c_func
(paren
l_string|&quot;nfs_refresh_inode: inode %ld mode changed, %07o to %07o&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
id|inode-&gt;i_mode
comma
id|fattr-&gt;mode
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t; * No need to worry about unhashing the dentry, as the&n;&t; * lookup validation will know that the inode is bad.&n;&t; */
id|nfs_invalidate_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
id|out_invalid
suffix:colon
macro_line|#ifdef NFS_DEBUG_VERBOSE
id|printk
c_func
(paren
l_string|&quot;nfs_refresh_inode: invalidating %ld pages&bslash;n&quot;
comma
id|inode-&gt;i_nrpages
)paren
suffix:semicolon
macro_line|#endif
id|nfs_zap_caches
c_func
(paren
id|inode
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
multiline_comment|/*&n; * File system information&n; */
DECL|variable|nfs_fs_type
r_static
r_struct
id|file_system_type
id|nfs_fs_type
op_assign
(brace
l_string|&quot;nfs&quot;
comma
l_int|0
multiline_comment|/* FS_NO_DCACHE - this doesn&squot;t work right now*/
comma
id|nfs_read_super
comma
l_int|NULL
)brace
suffix:semicolon
multiline_comment|/*&n; * Initialize NFS&n; */
r_int
DECL|function|init_nfs_fs
id|init_nfs_fs
c_func
(paren
r_void
)paren
(brace
macro_line|#ifdef CONFIG_PROC_FS
id|rpc_register_sysctl
c_func
(paren
)paren
suffix:semicolon
id|rpc_proc_init
c_func
(paren
)paren
suffix:semicolon
id|rpc_proc_register
c_func
(paren
op_amp
id|nfs_rpcstat
)paren
suffix:semicolon
macro_line|#endif
r_return
id|register_filesystem
c_func
(paren
op_amp
id|nfs_fs_type
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Every kernel module contains stuff like this.&n; */
macro_line|#ifdef MODULE
id|EXPORT_NO_SYMBOLS
suffix:semicolon
multiline_comment|/* Not quite true; I just maintain it */
id|MODULE_AUTHOR
c_func
(paren
l_string|&quot;Olaf Kirch &lt;okir@monad.swb.de&gt;&quot;
)paren
suffix:semicolon
r_int
DECL|function|init_module
id|init_module
c_func
(paren
r_void
)paren
(brace
r_return
id|init_nfs_fs
c_func
(paren
)paren
suffix:semicolon
)brace
r_void
DECL|function|cleanup_module
id|cleanup_module
c_func
(paren
r_void
)paren
(brace
macro_line|#ifdef CONFIG_PROC_FS
id|rpc_proc_unregister
c_func
(paren
l_string|&quot;nfs&quot;
)paren
suffix:semicolon
macro_line|#endif
id|unregister_filesystem
c_func
(paren
op_amp
id|nfs_fs_type
)paren
suffix:semicolon
id|nfs_free_dircache
c_func
(paren
)paren
suffix:semicolon
)brace
macro_line|#endif
eof
