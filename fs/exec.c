macro_line|#include &lt;errno.h&gt;
macro_line|#include &lt;sys/stat.h&gt;
macro_line|#include &lt;a.out.h&gt;
macro_line|#include &lt;linux/fs.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;asm/segment.h&gt;
r_extern
r_int
id|sys_exit
c_func
(paren
r_int
id|exit_code
)paren
suffix:semicolon
r_extern
r_int
id|sys_close
c_func
(paren
r_int
id|fd
)paren
suffix:semicolon
multiline_comment|/*&n; * MAX_ARG_PAGES defines the number of pages allocated for arguments&n; * and envelope for the new program. 32 should suffice, this gives&n; * a maximum env+arg of 128kB !&n; */
DECL|macro|MAX_ARG_PAGES
mdefine_line|#define MAX_ARG_PAGES 32
DECL|macro|cp_block
mdefine_line|#define cp_block(from,to) &bslash;&n;__asm__(&quot;pushl $0x10&bslash;n&bslash;t&quot; &bslash;&n;&t;&quot;pushl $0x17&bslash;n&bslash;t&quot; &bslash;&n;&t;&quot;pop %%es&bslash;n&bslash;t&quot; &bslash;&n;&t;&quot;cld&bslash;n&bslash;t&quot; &bslash;&n;&t;&quot;rep&bslash;n&bslash;t&quot; &bslash;&n;&t;&quot;movsl&bslash;n&bslash;t&quot; &bslash;&n;&t;&quot;pop %%es&quot; &bslash;&n;&t;::&quot;c&quot; (BLOCK_SIZE/4),&quot;S&quot; (from),&quot;D&quot; (to) &bslash;&n;&t;:&quot;cx&quot;,&quot;di&quot;,&quot;si&quot;)
multiline_comment|/*&n; * read_head() reads blocks 1-6 (not 0). Block 0 has already been&n; * read for header information.&n; */
DECL|function|read_head
r_int
id|read_head
c_func
(paren
r_struct
id|m_inode
op_star
id|inode
comma
r_int
id|blocks
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_int
id|count
suffix:semicolon
r_if
c_cond
(paren
id|blocks
OG
l_int|6
)paren
id|blocks
op_assign
l_int|6
suffix:semicolon
r_for
c_loop
(paren
id|count
op_assign
l_int|0
suffix:semicolon
id|count
OL
id|blocks
suffix:semicolon
id|count
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|inode-&gt;i_zone
(braket
id|count
op_plus
l_int|1
)braket
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|inode-&gt;i_zone
(braket
id|count
op_plus
l_int|1
)braket
)paren
)paren
)paren
r_return
op_minus
l_int|1
suffix:semicolon
id|cp_block
c_func
(paren
id|bh-&gt;b_data
comma
id|count
op_star
id|BLOCK_SIZE
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|read_ind
r_int
id|read_ind
c_func
(paren
r_int
id|dev
comma
r_int
id|ind
comma
r_int
id|size
comma
r_int
r_int
id|offset
)paren
(brace
r_struct
id|buffer_head
op_star
id|ih
comma
op_star
id|bh
suffix:semicolon
r_int
r_int
op_star
id|table
comma
id|block
suffix:semicolon
r_if
c_cond
(paren
id|size
op_le
l_int|0
)paren
id|panic
c_func
(paren
l_string|&quot;size&lt;=0 in read_ind&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|size
OG
l_int|512
op_star
id|BLOCK_SIZE
)paren
id|size
op_assign
l_int|512
op_star
id|BLOCK_SIZE
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ind
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|ih
op_assign
id|bread
c_func
(paren
id|dev
comma
id|ind
)paren
)paren
)paren
r_return
op_minus
l_int|1
suffix:semicolon
id|table
op_assign
(paren
r_int
r_int
op_star
)paren
id|ih-&gt;b_data
suffix:semicolon
r_while
c_loop
(paren
id|size
OG
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|block
op_assign
op_star
(paren
id|table
op_increment
)paren
)paren
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|dev
comma
id|block
)paren
)paren
)paren
(brace
id|brelse
c_func
(paren
id|ih
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_else
(brace
id|cp_block
c_func
(paren
id|bh-&gt;b_data
comma
id|offset
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
)brace
id|size
op_sub_assign
id|BLOCK_SIZE
suffix:semicolon
id|offset
op_add_assign
id|BLOCK_SIZE
suffix:semicolon
)brace
id|brelse
c_func
(paren
id|ih
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * read_area() reads an area into %fs:mem.&n; */
DECL|function|read_area
r_int
id|read_area
c_func
(paren
r_struct
id|m_inode
op_star
id|inode
comma
r_int
id|size
)paren
(brace
r_struct
id|buffer_head
op_star
id|dind
suffix:semicolon
r_int
r_int
op_star
id|table
suffix:semicolon
r_int
id|i
comma
id|count
suffix:semicolon
r_if
c_cond
(paren
(paren
id|i
op_assign
id|read_head
c_func
(paren
id|inode
comma
(paren
id|size
op_plus
id|BLOCK_SIZE
op_minus
l_int|1
)paren
op_div
id|BLOCK_SIZE
)paren
)paren
op_logical_or
(paren
id|size
op_sub_assign
id|BLOCK_SIZE
op_star
l_int|6
)paren
op_le
l_int|0
)paren
r_return
id|i
suffix:semicolon
r_if
c_cond
(paren
(paren
id|i
op_assign
id|read_ind
c_func
(paren
id|inode-&gt;i_dev
comma
id|inode-&gt;i_zone
(braket
l_int|7
)braket
comma
id|size
comma
id|BLOCK_SIZE
op_star
l_int|6
)paren
)paren
op_logical_or
(paren
id|size
op_sub_assign
id|BLOCK_SIZE
op_star
l_int|512
)paren
op_le
l_int|0
)paren
r_return
id|i
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|i
op_assign
id|inode-&gt;i_zone
(braket
l_int|8
)braket
)paren
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|dind
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|i
)paren
)paren
)paren
r_return
op_minus
l_int|1
suffix:semicolon
id|table
op_assign
(paren
r_int
r_int
op_star
)paren
id|dind-&gt;b_data
suffix:semicolon
r_for
c_loop
(paren
id|count
op_assign
l_int|0
suffix:semicolon
id|count
OL
l_int|512
suffix:semicolon
id|count
op_increment
)paren
r_if
c_cond
(paren
(paren
id|i
op_assign
id|read_ind
c_func
(paren
id|inode-&gt;i_dev
comma
op_star
(paren
id|table
op_increment
)paren
comma
id|size
comma
id|BLOCK_SIZE
op_star
(paren
l_int|518
op_plus
id|count
)paren
)paren
)paren
op_logical_or
(paren
id|size
op_sub_assign
id|BLOCK_SIZE
op_star
l_int|512
)paren
op_le
l_int|0
)paren
r_return
id|i
suffix:semicolon
id|panic
c_func
(paren
l_string|&quot;Impossibly long executable&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * create_tables() parses the env- and arg-strings in new user&n; * memory and creates the pointer tables from them, and puts their&n; * addresses on the &quot;stack&quot;, returning the new stack pointer value.&n; */
DECL|function|create_tables
r_static
r_int
r_int
op_star
id|create_tables
c_func
(paren
r_char
op_star
id|p
comma
r_int
id|argc
comma
r_int
id|envc
)paren
(brace
r_int
r_int
op_star
id|argv
comma
op_star
id|envp
suffix:semicolon
r_int
r_int
op_star
id|sp
suffix:semicolon
id|sp
op_assign
(paren
r_int
r_int
op_star
)paren
(paren
l_int|0xfffffffc
op_amp
(paren
r_int
r_int
)paren
id|p
)paren
suffix:semicolon
id|sp
op_sub_assign
id|envc
op_plus
l_int|1
suffix:semicolon
id|envp
op_assign
id|sp
suffix:semicolon
id|sp
op_sub_assign
id|argc
op_plus
l_int|1
suffix:semicolon
id|argv
op_assign
id|sp
suffix:semicolon
id|put_fs_long
c_func
(paren
(paren
r_int
r_int
)paren
id|envp
comma
op_decrement
id|sp
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
(paren
r_int
r_int
)paren
id|argv
comma
op_decrement
id|sp
)paren
suffix:semicolon
id|put_fs_long
c_func
(paren
(paren
r_int
r_int
)paren
id|argc
comma
op_decrement
id|sp
)paren
suffix:semicolon
r_while
c_loop
(paren
id|argc
op_decrement
OG
l_int|0
)paren
(brace
id|put_fs_long
c_func
(paren
(paren
r_int
r_int
)paren
id|p
comma
id|argv
op_increment
)paren
suffix:semicolon
r_while
c_loop
(paren
id|get_fs_byte
c_func
(paren
id|p
op_increment
)paren
)paren
multiline_comment|/* nothing */
suffix:semicolon
)brace
id|put_fs_long
c_func
(paren
l_int|0
comma
id|argv
)paren
suffix:semicolon
r_while
c_loop
(paren
id|envc
op_decrement
OG
l_int|0
)paren
(brace
id|put_fs_long
c_func
(paren
(paren
r_int
r_int
)paren
id|p
comma
id|envp
op_increment
)paren
suffix:semicolon
r_while
c_loop
(paren
id|get_fs_byte
c_func
(paren
id|p
op_increment
)paren
)paren
multiline_comment|/* nothing */
suffix:semicolon
)brace
id|put_fs_long
c_func
(paren
l_int|0
comma
id|envp
)paren
suffix:semicolon
r_return
id|sp
suffix:semicolon
)brace
multiline_comment|/*&n; * count() counts the number of arguments/envelopes&n; */
DECL|function|count
r_static
r_int
id|count
c_func
(paren
r_char
op_star
op_star
id|argv
)paren
(brace
r_int
id|i
op_assign
l_int|0
suffix:semicolon
r_char
op_star
op_star
id|tmp
suffix:semicolon
r_if
c_cond
(paren
id|tmp
op_assign
id|argv
)paren
r_while
c_loop
(paren
id|get_fs_long
c_func
(paren
(paren
r_int
r_int
op_star
)paren
(paren
id|tmp
op_increment
)paren
)paren
)paren
id|i
op_increment
suffix:semicolon
r_return
id|i
suffix:semicolon
)brace
multiline_comment|/*&n; * &squot;copy_string()&squot; copies argument/envelope strings from user&n; * memory to free pages in kernel mem. These are in a format ready&n; * to be put directly into the top of new user memory.&n; */
DECL|function|copy_strings
r_static
r_int
r_int
id|copy_strings
c_func
(paren
r_int
id|argc
comma
r_char
op_star
op_star
id|argv
comma
r_int
r_int
op_star
id|page
comma
r_int
r_int
id|p
)paren
(brace
r_int
id|len
comma
id|i
suffix:semicolon
r_char
op_star
id|tmp
suffix:semicolon
r_while
c_loop
(paren
id|argc
op_decrement
OG
l_int|0
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|tmp
op_assign
(paren
r_char
op_star
)paren
id|get_fs_long
c_func
(paren
(paren
(paren
r_int
r_int
op_star
)paren
id|argv
)paren
op_plus
id|argc
)paren
)paren
)paren
id|panic
c_func
(paren
l_string|&quot;argc is wrong&quot;
)paren
suffix:semicolon
id|len
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* remember zero-padding */
r_do
(brace
id|len
op_increment
suffix:semicolon
)brace
r_while
c_loop
(paren
id|get_fs_byte
c_func
(paren
id|tmp
op_increment
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|p
op_minus
id|len
OL
l_int|0
)paren
multiline_comment|/* this shouldn&squot;t happen - 128kB */
r_return
l_int|0
suffix:semicolon
id|i
op_assign
(paren
(paren
r_int
)paren
(paren
id|p
op_minus
id|len
)paren
)paren
op_rshift
l_int|12
suffix:semicolon
r_while
c_loop
(paren
id|i
OL
id|MAX_ARG_PAGES
op_logical_and
op_logical_neg
id|page
(braket
id|i
)braket
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|page
(braket
id|i
)braket
op_assign
id|get_free_page
c_func
(paren
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|i
op_increment
suffix:semicolon
)brace
r_do
(brace
op_decrement
id|p
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|page
(braket
id|p
op_div
id|PAGE_SIZE
)braket
)paren
id|panic
c_func
(paren
l_string|&quot;nonexistent page in exec.c&quot;
)paren
suffix:semicolon
(paren
(paren
r_char
op_star
)paren
id|page
(braket
id|p
op_div
id|PAGE_SIZE
)braket
)paren
(braket
id|p
op_mod
id|PAGE_SIZE
)braket
op_assign
id|get_fs_byte
c_func
(paren
op_decrement
id|tmp
)paren
suffix:semicolon
)brace
r_while
c_loop
(paren
op_decrement
id|len
)paren
suffix:semicolon
)brace
r_return
id|p
suffix:semicolon
)brace
DECL|function|change_ldt
r_static
r_int
r_int
id|change_ldt
c_func
(paren
r_int
r_int
id|text_size
comma
r_int
r_int
op_star
id|page
)paren
(brace
r_int
r_int
id|code_limit
comma
id|data_limit
comma
id|code_base
comma
id|data_base
suffix:semicolon
r_int
id|i
suffix:semicolon
id|code_limit
op_assign
id|text_size
op_plus
id|PAGE_SIZE
op_minus
l_int|1
suffix:semicolon
id|code_limit
op_and_assign
l_int|0xFFFFF000
suffix:semicolon
id|data_limit
op_assign
l_int|0x4000000
suffix:semicolon
id|code_base
op_assign
id|get_base
c_func
(paren
id|current-&gt;ldt
(braket
l_int|1
)braket
)paren
suffix:semicolon
id|data_base
op_assign
id|code_base
suffix:semicolon
id|set_base
c_func
(paren
id|current-&gt;ldt
(braket
l_int|1
)braket
comma
id|code_base
)paren
suffix:semicolon
id|set_limit
c_func
(paren
id|current-&gt;ldt
(braket
l_int|1
)braket
comma
id|code_limit
)paren
suffix:semicolon
id|set_base
c_func
(paren
id|current-&gt;ldt
(braket
l_int|2
)braket
comma
id|data_base
)paren
suffix:semicolon
id|set_limit
c_func
(paren
id|current-&gt;ldt
(braket
l_int|2
)braket
comma
id|data_limit
)paren
suffix:semicolon
multiline_comment|/* make sure fs points to the NEW data segment */
id|__asm__
c_func
(paren
l_string|&quot;pushl $0x17&bslash;n&bslash;tpop %%fs&quot;
op_scope_resolution
)paren
suffix:semicolon
id|data_base
op_add_assign
id|data_limit
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
id|MAX_ARG_PAGES
op_minus
l_int|1
suffix:semicolon
id|i
op_ge
l_int|0
suffix:semicolon
id|i
op_decrement
)paren
(brace
id|data_base
op_sub_assign
id|PAGE_SIZE
suffix:semicolon
r_if
c_cond
(paren
id|page
(braket
id|i
)braket
)paren
id|put_page
c_func
(paren
id|page
(braket
id|i
)braket
comma
id|data_base
)paren
suffix:semicolon
)brace
r_return
id|data_limit
suffix:semicolon
)brace
multiline_comment|/*&n; * &squot;do_execve()&squot; executes a new program.&n; */
DECL|function|do_execve
r_int
id|do_execve
c_func
(paren
r_int
r_int
op_star
id|eip
comma
r_int
id|tmp
comma
r_char
op_star
id|filename
comma
r_char
op_star
op_star
id|argv
comma
r_char
op_star
op_star
id|envp
)paren
(brace
r_struct
id|m_inode
op_star
id|inode
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|exec
id|ex
suffix:semicolon
r_int
r_int
id|page
(braket
id|MAX_ARG_PAGES
)braket
suffix:semicolon
r_int
id|i
comma
id|argc
comma
id|envc
suffix:semicolon
r_int
r_int
id|p
suffix:semicolon
r_if
c_cond
(paren
(paren
l_int|0xffff
op_amp
id|eip
(braket
l_int|1
)braket
)paren
op_ne
l_int|0x000f
)paren
id|panic
c_func
(paren
l_string|&quot;execve called from supervisor mode&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_ARG_PAGES
suffix:semicolon
id|i
op_increment
)paren
multiline_comment|/* clear page-table */
id|page
(braket
id|i
)braket
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|inode
op_assign
id|namei
c_func
(paren
id|filename
)paren
)paren
)paren
multiline_comment|/* get executables inode */
r_return
op_minus
id|ENOENT
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|S_ISREG
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
(brace
multiline_comment|/* must be regular file */
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|EACCES
suffix:semicolon
)brace
id|i
op_assign
id|inode-&gt;i_mode
suffix:semicolon
r_if
c_cond
(paren
id|current-&gt;uid
op_logical_and
id|current-&gt;euid
)paren
(brace
r_if
c_cond
(paren
id|current-&gt;euid
op_eq
id|inode-&gt;i_uid
)paren
id|i
op_rshift_assign
l_int|6
suffix:semicolon
r_else
r_if
c_cond
(paren
id|current-&gt;egid
op_eq
id|inode-&gt;i_gid
)paren
id|i
op_rshift_assign
l_int|3
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|0111
)paren
id|i
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|i
op_amp
l_int|1
)paren
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|ENOEXEC
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|bread
c_func
(paren
id|inode-&gt;i_dev
comma
id|inode-&gt;i_zone
(braket
l_int|0
)braket
)paren
)paren
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|EACCES
suffix:semicolon
)brace
id|ex
op_assign
op_star
(paren
(paren
r_struct
id|exec
op_star
)paren
id|bh-&gt;b_data
)paren
suffix:semicolon
multiline_comment|/* read exec-header */
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|N_MAGIC
c_func
(paren
id|ex
)paren
op_ne
id|ZMAGIC
op_logical_or
id|ex.a_trsize
op_logical_or
id|ex.a_drsize
op_logical_or
id|ex.a_text
op_plus
id|ex.a_data
op_plus
id|ex.a_bss
OG
l_int|0x3000000
op_logical_or
id|inode-&gt;i_size
OL
id|ex.a_text
op_plus
id|ex.a_data
op_plus
id|ex.a_syms
op_plus
id|N_TXTOFF
c_func
(paren
id|ex
)paren
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
id|ENOEXEC
suffix:semicolon
)brace
r_if
c_cond
(paren
id|N_TXTOFF
c_func
(paren
id|ex
)paren
op_ne
id|BLOCK_SIZE
)paren
id|panic
c_func
(paren
l_string|&quot;N_TXTOFF != BLOCK_SIZE. See a.out.h.&quot;
)paren
suffix:semicolon
id|argc
op_assign
id|count
c_func
(paren
id|argv
)paren
suffix:semicolon
id|envc
op_assign
id|count
c_func
(paren
id|envp
)paren
suffix:semicolon
id|p
op_assign
id|copy_strings
c_func
(paren
id|envc
comma
id|envp
comma
id|page
comma
id|PAGE_SIZE
op_star
id|MAX_ARG_PAGES
op_minus
l_int|4
)paren
suffix:semicolon
id|p
op_assign
id|copy_strings
c_func
(paren
id|argc
comma
id|argv
comma
id|page
comma
id|p
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|p
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_ARG_PAGES
suffix:semicolon
id|i
op_increment
)paren
id|free_page
c_func
(paren
id|page
(braket
id|i
)braket
)paren
suffix:semicolon
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
multiline_comment|/* OK, This is the point of no return */
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|32
suffix:semicolon
id|i
op_increment
)paren
id|current-&gt;sig_fn
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|NR_OPEN
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
(paren
id|current-&gt;close_on_exec
op_rshift
id|i
)paren
op_amp
l_int|1
)paren
id|sys_close
c_func
(paren
id|i
)paren
suffix:semicolon
id|current-&gt;close_on_exec
op_assign
l_int|0
suffix:semicolon
id|free_page_tables
c_func
(paren
id|get_base
c_func
(paren
id|current-&gt;ldt
(braket
l_int|1
)braket
)paren
comma
id|get_limit
c_func
(paren
l_int|0x0f
)paren
)paren
suffix:semicolon
id|free_page_tables
c_func
(paren
id|get_base
c_func
(paren
id|current-&gt;ldt
(braket
l_int|2
)braket
)paren
comma
id|get_limit
c_func
(paren
l_int|0x17
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|last_task_used_math
op_eq
id|current
)paren
id|last_task_used_math
op_assign
l_int|NULL
suffix:semicolon
id|current-&gt;used_math
op_assign
l_int|0
suffix:semicolon
id|p
op_add_assign
id|change_ldt
c_func
(paren
id|ex.a_text
comma
id|page
)paren
op_minus
id|MAX_ARG_PAGES
op_star
id|PAGE_SIZE
suffix:semicolon
id|p
op_assign
(paren
r_int
r_int
)paren
id|create_tables
c_func
(paren
(paren
r_char
op_star
)paren
id|p
comma
id|argc
comma
id|envc
)paren
suffix:semicolon
id|current-&gt;brk
op_assign
id|ex.a_bss
op_plus
(paren
id|current-&gt;end_data
op_assign
id|ex.a_data
op_plus
(paren
id|current-&gt;end_code
op_assign
id|ex.a_text
)paren
)paren
suffix:semicolon
id|current-&gt;start_stack
op_assign
id|p
op_amp
l_int|0xfffff000
suffix:semicolon
id|i
op_assign
id|read_area
c_func
(paren
id|inode
comma
id|ex.a_text
op_plus
id|ex.a_data
)paren
suffix:semicolon
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|i
OL
l_int|0
)paren
id|sys_exit
c_func
(paren
op_minus
l_int|1
)paren
suffix:semicolon
id|i
op_assign
id|ex.a_text
op_plus
id|ex.a_data
suffix:semicolon
r_while
c_loop
(paren
id|i
op_amp
l_int|0xfff
)paren
id|put_fs_byte
c_func
(paren
l_int|0
comma
(paren
r_char
op_star
)paren
(paren
id|i
op_increment
)paren
)paren
suffix:semicolon
id|eip
(braket
l_int|0
)braket
op_assign
id|ex.a_entry
suffix:semicolon
multiline_comment|/* eip, magic happens :-) */
id|eip
(braket
l_int|3
)braket
op_assign
id|p
suffix:semicolon
multiline_comment|/* stack pointer */
r_return
l_int|0
suffix:semicolon
)brace
eof
