...
mdefine_line|#define CCACHE_INC ((PAGE_SIZE - sizeof(void *))/sizeof(struct hfs_cat_entry))
...
suffix:semicolon
multiline_comment|/* all the dirty entries */
r_static
id|LIST_HEAD
c_func
(paren
...
multiline_comment|/* put entry on mdb dirty list. this only does it if it&squot;s on the hash&n; * list. we also add it to the global dirty list as well. */
...
id|HFS_DIRTY
suffix:semicolon
multiline_comment|/* Only add valid (ie hashed) entries to the&n;&t;&t; * dirty list */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* prune all entries */
...
multiline_comment|/*&n; * try_to_free_entries works by getting the underlying &n; * cache system to release entries. it gets called with the entry lock&n; * held. &n; *&n; * count can be up to 2 due to both a resource and data fork being&n; * listed. we can unuse dirty entries as well.&n; */
DECL|macro|CAN_UNUSE
mdefine_line|#define CAN_UNUSE(tmp) (((tmp)-&gt;count &lt; 3) &amp;&amp; ((tmp)-&gt;state &lt;= HFS_DIRTY))
...
l_int|1
suffix:semicolon
multiline_comment|/* try freeing from entry_in_use */
r_while
c_loop
(paren
(paren
...
(brace
multiline_comment|/* try freeing from global dirty list */
id|head
op_assign
op_amp
id|entry_dirty
...
multiline_comment|/*&n; * grow_entries()&n; *&n; * Try to allocate more entries, adding them to the free list. this returns&n; * with the spinlock held if successful &n; */
...
multiline_comment|/* allocation failed. do some pruning and try again */
id|spin_lock
c_func
(paren
op_amp
...
(brace
multiline_comment|/* uh oh. we failed to read the record */
id|entry-&gt;state
op_or_assign
id|HFS_DELETED
suffix:semicolon
...
multiline_comment|/*&n;&t; * Uhhuh.. We need to expand. Note that &quot;grow_entries()&quot; will&n;&t; * release the spinlock, but will return with the lock held&n;&t; * again if the allocation succeeded.&n;&t; */
id|entry
op_assign
id|grow_entries
c_func
...
c_func
(paren
l_string|&quot;hfs_fs: entry %p(%u:%lu) busy on removed device %s.&bslash;n&quot;
comma
id|entry
comma
id|entry-&gt;count
...
suffix:semicolon
multiline_comment|/* remove from global dirty list */
id|list_del
c_func
(paren
op_amp
