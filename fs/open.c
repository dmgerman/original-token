...
multiline_comment|/* This routine looks through all the process&squot;s and closes any&n;   references to the current processes tty.  To avoid problems with&n;   process sleeping on an inode which has already been iput, anyprocess&n;   which is sleeping on the tty is sent a sigkill (It&squot;s probably a rogue&n;   process.)  Also no process should ever have /dev/console as it&squot;s&n;   controlling tty, or have it open for reading.  So we don&squot;t have to&n;   worry about messing with all the daemons abilities to write messages&n;   to the console.  (Besides they should be using syslog.) */
r_int
...
r_continue
suffix:semicolon
multiline_comment|/* now we need to check to see if this file points to the&n;&t;&t;device we are trying to close. */
r_if
c_cond
(paren
op_logical_neg
id|S_ISCHR
(paren
id|filep-&gt;f_inode-&gt;i_mode
)paren
)paren
r_continue
suffix:semicolon
multiline_comment|/* This will catch both /dev/tty and the explicit terminal&n;&t;&t;device.  However, we must make sure that f_rdev is&n;&t;&t;defined and correct. */
r_if
c_cond
(paren
(paren
