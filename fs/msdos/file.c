...
suffix:semicolon
multiline_comment|/* Buffers not already read */
r_int
id|nbreq
suffix:semicolon
multiline_comment|/* Number of buffers to read */
r_struct
id|buffer_head
op_star
id|bhlist
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t;Tell the buffer cache which block we expect to read in advance&n;&t;&t;Since we are limited with the stack, we preread only 64&n;&t;&t;because we have to keep the result into the local&n;&t;&t;arrays pre.bhlist and pre.bhreq.&n;&t;*/
(brace
r_int
id|file_sector
op_assign
...
(paren
(paren
l_string|&quot;file_read pos %d&bslash;n&quot;
comma
id|filp-&gt;f_pos
)paren
)paren
...
(brace
multiline_comment|/* This code is executed when more than 64 sectors */
multiline_comment|/* are request at once */
r_int
id|sector
suffix:semicolon
r_if
