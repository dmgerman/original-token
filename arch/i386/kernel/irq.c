...
macro_line|#ifdef __SMP__
multiline_comment|/*&n; * In the SMP+IOAPIC case it might happen that there are an unspecified&n; * number of pending IRQ events unhandled. These cases are very rare,&n; * so we &squot;resend&squot; these IRQs via IPIs, to the same CPU. It&squot;s much&n; * better to do it this way as thus we dont have to be aware of&n; * &squot;pending&squot; interrupts in the IRQ path, except at this point.&n; */
...
multiline_comment|/*&n; * Has to be called with the irq controller locked, and has to exit&n; * with with the lock held as well.&n; */
DECL|function|handle_ioapic_event
r_static
r_void
id|handle_ioapic_event
(paren
r_int
r_int
id|irq
...
id|handler
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If there is no IRQ handler, exit early, leaving the irq&n;&t;&t; * &quot;in progress&quot;&n;&t;&t; */
id|handler
op_assign
id|handle_IRQ_event
c_func
...
id|irq
suffix:semicolon
multiline_comment|/*&n;&t; * Edge triggered IRQs can be acked immediately&n;&t; */
id|ack_APIC_irq
c_func
(paren
)paren
...
DECL|function|do_level_ioapic_IRQ
r_static
r_void
id|do_level_ioapic_IRQ
(paren
...
suffix:semicolon
multiline_comment|/*&n;&t; * In the level triggered case we first disable the IRQ&n;&t; * in the IO-APIC, then we &squot;early ACK&squot; the IRQ, then we&n;&t; * handle it and enable the IRQ when finished.&n;&t; *&n;&t; * disable has to happen before the ACK, to avoid IRQ storms.&n;&t; * So this all has to be within the spinlock.&n;&t; */
id|disable_IO_APIC_irq
c_func
(paren
id|irq
...
suffix:semicolon
multiline_comment|/* we still have the spinlock held here */
id|enable_IO_APIC_irq
c_func
(paren
id|irq
...
suffix:semicolon
multiline_comment|/*&n;&t; * In contrast to the above, we should _not_ have any concurrent&n;&t; * interrupt activity here, so we just clear both disabled bits.&n;&t; *&n;&t; * This allows us to have IRQ_INPROGRESS set until we actually&n;&t; * install a handler for this interrupt (make irq autodetection&n;&t; * work by just looking at the status field for the irq)&n;&t; */
id|irq_desc
(braket
id|irq
)braket
...
multiline_comment|/*&n;&t;&t;&t;&t; * break visibly for now, FIXME&n;&t;&t;&t;&t; */
id|panic
c_func
(paren
l_string|&quot;ayiee, tell mingo&quot;
)paren
suffix:semicolon
