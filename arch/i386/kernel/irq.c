...
(brace
multiline_comment|/*&n;&t; * The IO-APIC part will be moved to assembly, nested&n;&t; * interrupts will be ~5 instructions from entry to iret ...&n;&t; */
r_int
...
suffix:semicolon
multiline_comment|/*&n;&t; * We always call this with local irqs disabled&n;&t; */
id|spin_lock
c_func
(paren
op_amp
...
r_else
(brace
multiline_comment|/*&n;&t; * 8259-triggered INTA-cycle interrupt&n;&t; */
r_if
c_cond
(paren
id|should_handle_irq
...
multiline_comment|/*&n; * [FIXME: anyone with 2 separate PCI buses and 2 IO-APICs,&n; *         please speak up and request experimental patches.&n; *         --mingo ]&n; */
...
multiline_comment|/*&n; * This is called when we want to synchronize with&n; * interrupts. We may for example tell a device to&n; * stop sending interrupts: but to make sure there&n; * are no interrupts that are executing on another&n; * CPU we need to call this function.&n; *&n; * We have to give pending interrupts a chance to&n; * arrive (ie. let them get until hard_irq_enter()),&n; * even if they are arriving to another CPU.&n; *&n; * On UP this is a no-op.&n; *&n; * UPDATE: this method is not quite safe, as it wont&n; * catch irq handlers polling for the irq lock bit&n; * in __global_cli():get_interrupt_lock():wait_on_irq().&n; * drivers should rather use disable_irq()/enable_irq()&n; * and/or synchronize_one_irq()&n; */
...
id|i
suffix:semicolon
multiline_comment|/* The very stupid way to do this */
r_for
c_loop
(paren
id|i
...
DECL|function|synchronize_one_irq
r_void
...
c_func
(paren
l_string|&quot;&bslash;n...WHAT??.#1...&bslash;n&quot;
)paren
suffix:semicolon
id|atomic_add
c_func
...
macro_line|#ifdef __SMP__
multiline_comment|/* FIXME: move this into hardirq.h */
id|irq_owner
(braket
id|irq
)braket
...
suffix:semicolon
macro_line|#ifdef __SMP__
multiline_comment|/*&n;&t; * We have to wait for all irq handlers belonging to this IRQ&n;&t; * vector to finish executing.&n;&t; */
r_if
c_cond
(paren
(paren
...
c_func
(paren
l_string|&quot;disable_irq(%d), infinit recursion!&bslash;n&quot;
comma
id|irq
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;more enable_irq(%d)&squot;s than disable_irq(%d)&squot;s!!&quot;
comma
id|irq
comma
id|irq
...
macro_line|#endif
multiline_comment|/*&n;&t; * Nobody is executing this irq handler currently, lets check&n;&t; * wether we have outstanding events to be handled.&n;&t; */
r_if
c_cond
(paren
id|irq_events
...
l_int|0xff
suffix:semicolon
multiline_comment|/*&n;&t;printk(&quot;&lt;%d&gt;&quot;,irq);&n; */
r_if
c_cond
(paren
op_logical_neg
