...
multiline_comment|/* It isnt really a PCI quirk directly, but the cure is the&n;&t;&t;   same. The MediaGX has deep magic SMM stuff that handles the&n;&t;&t;   SB emulation. It thows away the fifo on disable_dma() which&n;                   is wrong and ruins the audio. &n;                   &n;                   Bug2: VSA1 has a wrap bug so that using maximum sized DMA &n;                   causes bad things. According to NatSemi VSA2 has another&n;                   bug to do with &squot;hlt&squot;. I&squot;ve not seen any boards using VSA2&n;                   and X doesn&squot;t seem to support it either so who cares 8).&n;                   VSA1 we work around however.&n;                   &n;&t;&t;*/
id|printk
c_func
(paren
id|KERN_INFO
