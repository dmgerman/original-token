...
DECL|variable|smp_apic_timer_ticks
r_volatile
r_int
r_int
...
c_func
(paren
l_string|&quot;Unknown SMP message %d&bslash;n&quot;
comma
id|msg
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;stuck on target_map IPI wait&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
...
(brace
multiline_comment|/* Take care of &quot;crossing&quot; invalidates */
r_if
c_cond
(paren
id|test_bit
...
c_func
(paren
l_string|&quot;stuck on smp_invalidate_needed IPI wait&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
...
suffix:semicolon
DECL|variable|default_do_profile
r_static
r_void
(paren
op_star
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n; *&t;APIC timer interrupt&n; */
...
id|flags
suffix:semicolon
multiline_comment|/*&n;&t;&t; * local timer interrupt is not NMI yet, so&n;&t;&t; * it&squot;s simple, we just aquire the global cli&n;&t;&t; * lock to mess around with profiling info.&n;&t;&t; *&n;&t;&t; * later, in the NMI version, we have to build&n;&t;&t; * our own &squot;current&squot; pointer (as we could have&n;&t;&t; * interrupted code that just changes &quot;current&quot;)&n;&t;&t; * and have to lock profiling info between NMI&n;&t;&t; * interrupts properly.&n;&t;&t; */
id|save_flags
c_func
(paren
id|flags
...
multiline_comment|/*&n;&t; * this is safe outside the lock.&n;&t; */
id|smp_apic_timer_ticks
(braket
id|cpu
)braket
...
multiline_comment|/*&n; * This part sets up the APIC 32 bit clock in LVTT1, with HZ interrupts&n; * per second. We assume that the caller has already set up the local&n; * APIC at apic_addr.&n; *&n; * Later we might want to split HZ into two parts: introduce&n; * PROFILING_HZ and scheduling HZ. The APIC timer is not exactly&n; * sync with the external timer chip, it closely follows bus clocks.&n; */
DECL|macro|RTDSC
...
multiline_comment|/*&n; * This function sets up the local APIC timer, with a timeout of&n; * &squot;clocks&squot; APIC bus clock. During calibration we actually call&n; * this function twice, once with a bogus timeout value, second&n; * time for real. The other (noncalibrating) CPUs call this&n; * function only once, with the real value.&n; *&n; * We are strictly in irqs off mode here, as we do not want to&n; * get an APIC interrupt go off accidentally.&n; */
DECL|macro|APIC_DIVISOR
mdefine_line|#define APIC_DIVISOR 16
multiline_comment|/*&n; * We do reads before writes even if unnecessary, to get around the&n; * APIC double write &squot;bug&squot;.&n; */
...
multiline_comment|/*&n; * In this function we calibrate APIC bus clocks to the external&n; * timer here. Unfortunately we cannot use jiffies and&n; * the timer irq to calibrate, since some later bootup&n; * code depends on getting the first irq? Ugh.&n; *&n; * We want to do the calibration only once since we&n; * want to have local timer irqs syncron. CPUs connected&n; * by the same APIC bus have the very same bus frequency.&n; * And we want to have irqs off anyways, no accidental&n; * APIC irq that way.&n; */
...
suffix:semicolon
multiline_comment|/*&n;&t; * lets wait until we get to the next wrapround:&n;&t; */
id|wait_8254_wraparound
(paren
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t; * The APIC bus clock counter is 32 bits only, it&n;&t; * might have overflown:&n;&t; */
r_if
c_cond
(paren
id|tt2
...
c_func
(paren
l_string|&quot;&bslash;n..... %ld CPU clocks in 1 timer chip tick.&bslash;n&quot;
comma
(paren
r_int
r_int
...
c_func
(paren
l_string|&quot;..... %ld APIC bus clocks in 1 timer chip tick.&bslash;n&quot;
comma
id|calibration_result
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;..... CPU clock speed is %ld.%ld MHz.&bslash;n&quot;
comma
(paren
(paren
r_int
...
suffix:semicolon
multiline_comment|/*&n;&t; * We use a private profiling function. (This is preparation&n;&t; * for NMI local timer interrupts.)&n;&t; *&n;&t; * [ setup_APIC_clock() is called from all CPUs, but we want&n;&t; *   to do this part of the setup only once ... and it fits&n;&t; *   here best ]&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;waiting for other CPU calibrating APIC timer ... &quot;
)paren
suffix:semicolon
r_while
c_loop
