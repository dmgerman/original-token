...
mdefine_line|#define SWAP_ADDR(nr)&t;&t;((void *)(swap_start + ((nr)&lt;&lt;PAGE_SHIFT)))
...
multiline_comment|/* Start and end of the (pre-mem_init) reserved ST-RAM region */
...
DECL|macro|BLOCK_STATIC
mdefine_line|#define BLOCK_STATIC&t;0x04&t;/* pre-mem_init() allocated block */
...
multiline_comment|/*&n; * This function is called from mem_init() to reserve the pages needed for&n; * ST-RAM management.&n; */
DECL|function|__initfunc
id|__initfunc
c_func
(paren
...
c_func
(paren
l_string|&quot;atari_stram_reserve_pages: swapping disabled&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
(brace
multiline_comment|/* Reserve all pages that have been marked by pre-mem_init&n;&t;&t;&t; * stram_alloc() (e.g. for the screen memory). */
id|reserve_region
c_func
(paren
id|rsvd_stram_beg
...
c_func
(paren
l_string|&quot;atari_stram_reserve_pages: reseverved %08lx-%08lx&bslash;n&quot;
comma
id|rsvd_stram_beg
comma
id|rsvd_stram_end
...
multiline_comment|/* else (kernel in ST-RAM): nothing to do, ST-RAM buffers are&n;&t;&t; * kernel data */
macro_line|#ifdef CONFIG_STRAM_SWAP
)brace
r_else
(brace
...
id|p
suffix:semicolon
multiline_comment|/* determine first page to use as swap:&n;&t;&t; * if the kernel is in TT-RAM, this is the first page of (usable)&n;&t;&t; * ST-RAM; else if there were already some allocations (probable...),&n;&t;&t; * use the lowest address of these (the list is sorted by address!);&n;&t;&t; * otherwise just use the end of kernel data (= start_mem) */
id|swap_start
op_assign
op_logical_neg
id|kernel_in_stram
...
l_string|&quot;swap=%08lx-%08lx&bslash;n&quot;
comma
id|swap_start
comma
id|swap_end
...
suffix:semicolon
multiline_comment|/* Formerly static areas have been included in the swap area. */
r_for
c_loop
(paren
id|p
...
l_int|0xffffffff
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Ok, num_physpages needs not be really exact, but it&squot;s better to&n;&t;&t; * subtract the pages set aside for swapping.&n;&t;&t; */
id|num_physpages
op_sub_assign
id|SWAP_NR
c_func
...
c_func
(paren
l_string|&quot;atari_stram_alloc(size=%08lx,*start_mem=%08lx,owner=%s)&bslash;n&quot;
comma
id|size
comma
id|start_mem
...
(paren
id|KERN_ERR
l_string|&quot;atari_stram_alloc called with start_mem!=NULL &quot;
l_string|&quot;after mem_init() from %p&bslash;n&quot;
comma
...
id|KERN_ERR
l_string|&quot;atari_stram_alloc called with start_mem==NULL &quot;
l_string|&quot;before mem_init() from %p&bslash;n&quot;
comma
id|__builtin_return_address
c_func
(paren
...
(brace
multiline_comment|/* before mem_init(): allocate &quot;statically&quot;, i.e. either in the kernel&n;&t;&t; * data space (current end in *start_mem), or at the end of currently&n;&t;&t; * reserved ST-RAM. */
r_if
c_cond
(paren
id|kernel_in_stram
...
c_func
(paren
l_string|&quot;atari_stram_alloc: pre-mem_init and k/ST: &quot;
l_string|&quot;shifted start_mem to %08lx, addr=%p&bslash;n&quot;
comma
op_star
id|start_mem
comma
...
c_func
(paren
l_string|&quot;atari_stram_alloc: pre-mem_init and k/TT: &quot;
l_string|&quot;shifted rsvd_stram_end to %08lx, addr=%p&bslash;n&quot;
comma
id|rsvd_stram_end
comma
id|addr
...
(brace
multiline_comment|/* If swapping is active (can only be the case after mem_init()!):&n;&t;&t; * make some free space in the swap &quot;device&quot;. */
id|DPRINTK
c_func
(paren
...
c_func
(paren
l_string|&quot;swap_init(start_mem=%08lx, swap_data=%08lx)&bslash;n&quot;
comma
id|start_mem
comma
id|swap_data
...
c_func
(paren
l_string|&quot;unswap_pte: page %08lx = entry %08lx was in swap cache; &quot;
l_string|&quot;exchanging to %08lx&bslash;n&quot;
comma
id|page_address
c_func
(paren
...
(paren
l_string|&quot;unswap_pte: page %08lx = entry %08lx was in swap cache; &quot;
l_string|&quot;deleted there&bslash;n&quot;
comma
id|page_address
c_func
(paren
...
c_func
(paren
l_string|&quot;unswap_pte: replacing entry %08lx by %08lx&quot;
comma
id|entry
comma
id|page
...
c_func
(paren
l_string|&quot;unswap_pte: replacing entry %08lx by new page %08lx&quot;
comma
id|entry
comma
id|page
...
c_func
(paren
l_string|&quot;unswap_pmd: bad pmd (%08lx)&bslash;n&quot;
comma
id|pmd_val
c_func
(paren
...
c_func
(paren
l_string|&quot;unswap_pgd: bad pgd (%08lx)&bslash;n&quot;
comma
id|pgd_val
c_func
(paren
...
c_func
(paren
l_string|&quot;unswapping %lu..%lu by moving in swap&bslash;n&quot;
comma
id|start
comma
id|start
...
suffix:semicolon
multiline_comment|/* can free the allocated pages by moving them to other swap pages */
r_for
c_loop
(paren
id|i
...
c_func
(paren
l_string|&quot;unswap: page %lu was free&bslash;n&quot;
comma
id|i
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;unswap: page %lu is alloced, count=%u&bslash;n&quot;
comma
id|i
comma
id|map
...
suffix:semicolon
multiline_comment|/* find a free page not in our region */
r_for
c_loop
(paren
id|j
...
(paren
id|KERN_ERR
l_string|&quot;get_stram_region: not enough free swap &quot;
l_string|&quot;pages now??&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
c_func
(paren
l_string|&quot;unswap: map[i=%lu]=%u map[j=%lu]=%u nr_swap=%u&bslash;n&quot;
comma
id|i
comma
id|map
...
(paren
id|KERN_ERR
l_string|&quot;get_stram_region: ST-RAM swap page %lu &quot;
l_string|&quot;not used by any process&bslash;n&quot;
comma
id|i
)paren
suffix:semicolon
multiline_comment|/* quit while loop and overwrite bad map entry */
r_break
suffix:semicolon
)brace
r_else
...
(brace
multiline_comment|/* somebody else must have swapped in that page, so free the&n;&t;&t;&t;&t; * new one (we&squot;re moving to) */
id|DPRINTK
c_func
(paren
l_string|&quot;unswap: map[i] became 0, also clearing map[j]&bslash;n&quot;
)paren
suffix:semicolon
id|map
(braket
...
(paren
id|KERN_NOTICE
l_string|&quot;get_stram_region: out of memory&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
c_func
(paren
l_string|&quot;unswap: reading swap page %lu to %08lx&bslash;n&quot;
comma
id|i
comma
id|page
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * If we couldn&squot;t find an entry, there are several&n;&t;&t;&t;&t; * possible reasons: someone else freed it first,&n;&t;&t;&t;&t; * we freed the last reference to an overflowed entry,&n;&t;&t;&t;&t; * or the system has lost track of the use counts.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|map
...
(paren
id|KERN_ERR
l_string|&quot;get_stram_region: swap entry %08lx &quot;
l_string|&quot;not used by any process&bslash;n&quot;
comma
id|entry
)paren
...
c_func
(paren
l_string|&quot;unswap: map[i] became 0&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_break
...
c_func
(paren
l_string|&quot;stram: request list destroyed&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;stram: block not locked&quot;
)paren
suffix:semicolon
)brace
id|start
...
suffix:semicolon
multiline_comment|/* to avoid warning */
r_return
l_int|0
suffix:semicolon
)brace
...
multiline_comment|/* ------------------------------------------------------------------------ */
multiline_comment|/* return log2 of #pages for size */
DECL|function|get_gfp_order
r_static
r_int
id|get_gfp_order
c_func
...
multiline_comment|/* reserve a range of pages in mem_map[] */
...
c_func
(paren
l_string|&quot;Total ST-RAM:      %8lu kB&bslash;n&quot;
...
l_string|&quot;Swap Moves:        %8u&bslash;n&quot;
...
(paren
l_string|&quot;Total ST-RAM:      %8lu kB&bslash;n&quot;
l_string|&quot;Reserved ST-RAM:   %8lu kB&bslash;n&quot;
comma
(paren
id|stram_end
op_minus
