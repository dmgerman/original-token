...
suffix:semicolon
multiline_comment|/*&n; * Array with isr&squot;s and their parameter data. This array is accessed from low&n; * level assembler code, so an element size of 8 allows usage of index scaling&n; * addressing mode.&n; */
...
suffix:semicolon
multiline_comment|/*&n; * Counter for next free interrupt vector number&n; * (new vectors starting from 0x70 can be allocated by&n; * atari_register_vme_int())&n; */
DECL|variable|next_free_vme_vec
r_static
r_int
id|next_free_vme_vec
op_assign
id|VME_SOURCE_BASE
suffix:semicolon
...
"&quot;"
id|addql
macro_line|#1,&quot;SYMBOL_NAME_STR(intr_count)&quot;
id|movew
op_mod
op_mod
id|sp
...
op_mod
id|d0
multiline_comment|/* get vector number from stack frame */
id|andil
macro_line|#0xfff,%%d0&t;&t;/* mask off format nibble */
id|lsrl
macro_line|#2,%%d0&t;&t;&t;/* convert vector to source */
id|subl
...
id|d0
id|andw
macro_line|#0xf8ff,%d0
id|orw
macro_line|#0x0200,%d0&t;/* set saved ipl to 2 */
id|movew
op_mod
id|d0
comma
...
suffix:semicolon
multiline_comment|/* Defined in entry.S; only increments &squot;num_suprious&squot; */
id|asmlinkage
r_void
id|bad_interrupt
c_func
...
suffix:semicolon
multiline_comment|/*&n; * void atari_init_INTS (void)&n; *&n; * Parameters:&t;None&n; *&n; * Returns:&t;Nothing&n; *&n; * This function should be called during kernel startup to initialize&n; * the atari IRQ handling routines.&n; */
DECL|function|atari_init_INTS
r_void
id|atari_init_INTS
c_func
(paren
...
r_else
(brace
multiline_comment|/* If no SCU, the HSYNC interrupt needs to be disabled this&n;&t;&t; * way. (Else _inthandler in kernel/sys_call.S gets overruns)&n;&t;&t; */
id|vectors
(braket
id|VEC_INT2
)braket
...
DECL|function|atari_call_isr_list
r_static
r_void
id|atari_call_isr_list
c_func
(paren
r_int
id|irq
...
multiline_comment|/*&n; * atari_add_isr : add an interrupt service routine for a particular&n; *&t;&t;   machine specific interrupt source.&n; *&t;&t;   If the addition was successful, it returns 1, otherwise&n; *&t;&t;   it returns 0.  It will fail if the interrupt is already&n; *                 occupied of another handler with different type&n; */
DECL|function|atari_add_isr
r_int
id|atari_add_isr
c_func
(paren
...
id|printk
(paren
l_string|&quot;atari_add_isr: Bad irq type %d requested from %s&bslash;n&quot;
comma
id|type
comma
id|name
...
id|printk
(paren
l_string|&quot;atari_add_isr: Unknown irq %ld requested from %s&bslash;n&quot;
comma
id|source
comma
id|name
...
id|printk
(paren
l_string|&quot;atari_add_isr: Slow irq requested for non-MFP source %ld from %s&bslash;n&quot;
comma
id|source
comma
id|name
...
id|printk
(paren
l_string|&quot;atari_add_isr: Irq %ld allocated by other type int (call from %s)&bslash;n&quot;
comma
id|source
comma
id|name
...
DECL|function|atari_remove_isr
r_int
id|atari_remove_isr
c_func
(paren
...
c_func
(paren
l_string|&quot;atari_remove_isr: Unknown irq %ld&bslash;n&quot;
comma
id|source
)paren
suffix:semicolon
...
multiline_comment|/* The interrupt is chained, find the isr on the list */
r_for
c_loop
(paren
id|p
...
id|next
suffix:semicolon
multiline_comment|/* If there&squot;s now only one handler, unchain the interrupt, i.e. plug in&n;&t; * the handler directly again and omit atari_call_isr_list */
id|q
op_assign
(paren
id|isr_node_t
...
c_func
(paren
l_string|&quot;atari_remove_isr: isr %p not found on list!&bslash;n&quot;
comma
id|isr
)paren
suffix:semicolon
