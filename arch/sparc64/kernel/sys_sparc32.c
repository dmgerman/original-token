...
multiline_comment|/* This is a single page for faster getname.&n; *   If the page is available when entering getname, use it.&n; *   If the page is not available, call __get_free_page instead.&n; * This works even though do_getname can block (think about it).&n; * -- Michael Chastain, based on idea of Linus Torvalds, 1 Dec 1996.&n; * We don&squot;t use the common getname/putname from namei.c, so that&n; * this still works well, as every routine which calls getname32&n; * will then call getname, then putname and then putname32.&n; */
DECL|variable|name_page_cache32
r_static
r_int
r_int
id|name_page_cache32
op_assign
l_int|0
suffix:semicolon
DECL|function|putname32
r_void
id|putname32
c_func
(paren
...
multiline_comment|/* Modules will be supported with 64bit modutils only */
DECL|function|sys32_no_modules
id|asmlinkage
r_int
id|sys32_no_modules
c_func
