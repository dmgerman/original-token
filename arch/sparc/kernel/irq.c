multiline_comment|/*  arch/sparc/kernel/irq.c:  Interrupt request handling routines. On the&n; *                            Sparc the IRQ&squot;s are basically &squot;cast in stone&squot;&n; *                            and you are supposed to probe the prom&squot;s device&n; *                            node trees to find out who&squot;s got which IRQ.&n; *&n; *  Copyright (C) 1995 David S. Miller (davem@caip.rutgers.edu)&n; *  Copyright (C) 1995 Miguel de Icaza (miguel@nuclecu.unam.mx)&n; *  Copyright (C) 1995 Pete A. Zaitcev (zaitcev@jamica.lab.ipmce.su)&n; */
multiline_comment|/*&n; * IRQ&squot;s are in fact implemented a bit like signal handlers for the kernel.&n; * The same sigaction struct is used, and with similar semantics (ie there&n; * is a SA_INTERRUPT flag etc). Naturally it&squot;s not a 1:1 relation, but there&n; * are similarities.&n; *&n; * sa_handler(int irq_NR) is the default function called (0 if no).&n; * sa_mask is horribly ugly (I won&squot;t even mention it)&n; * sa_flags contains various info: SA_INTERRUPT etc&n; * sa_restorer is the unused&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/ptrace.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/linkage.h&gt;
macro_line|#include &lt;linux/kernel_stat.h&gt;
macro_line|#include &lt;linux/signal.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/interrupt.h&gt;
macro_line|#include &lt;asm/ptrace.h&gt;
macro_line|#include &lt;asm/processor.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/psr.h&gt;
macro_line|#include &lt;asm/vaddrs.h&gt;
macro_line|#include &lt;asm/timer.h&gt;
macro_line|#include &lt;asm/openprom.h&gt;
macro_line|#include &lt;asm/oplib.h&gt;
macro_line|#include &lt;asm/irq.h&gt;
macro_line|#include &lt;asm/io.h&gt;
multiline_comment|/* Pointer to the interrupt enable byte */
multiline_comment|/* XXX Ugh, this is so sun4c specific it&squot;s driving me nuts. XXX */
DECL|variable|interrupt_enable
r_int
r_char
op_star
id|interrupt_enable
op_assign
l_int|0
suffix:semicolon
DECL|variable|sun4m_interrupts
r_struct
id|sun4m_intregs
op_star
id|sun4m_interrupts
suffix:semicolon
multiline_comment|/* XXX Needs to handle Sun4m semantics XXX */
r_void
DECL|function|disable_irq
id|disable_irq
c_func
(paren
r_int
r_int
id|irq_nr
)paren
(brace
r_int
r_int
id|flags
suffix:semicolon
r_int
r_char
id|current_mask
comma
id|new_mask
suffix:semicolon
r_if
c_cond
(paren
id|sparc_cpu_model
op_ne
id|sun4c
)paren
(brace
r_return
suffix:semicolon
)brace
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|current_mask
op_assign
op_star
id|interrupt_enable
suffix:semicolon
r_switch
c_cond
(paren
id|irq_nr
)paren
(brace
r_case
l_int|1
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_amp
(paren
op_complement
(paren
id|SUN4C_INT_E1
)paren
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|4
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_amp
(paren
op_complement
(paren
id|SUN4C_INT_E4
)paren
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|6
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_amp
(paren
op_complement
(paren
id|SUN4C_INT_E6
)paren
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|8
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_amp
(paren
op_complement
(paren
id|SUN4C_INT_E8
)paren
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|10
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_amp
(paren
op_complement
(paren
id|SUN4C_INT_E10
)paren
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|14
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_amp
(paren
op_complement
(paren
id|SUN4C_INT_E14
)paren
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
macro_line|#if 0 /* Actually this is safe, as the floppy driver needs this */
id|printk
c_func
(paren
l_string|&quot;AIEEE, Illegal interrupt disable requested irq=%d&bslash;n&quot;
comma
(paren
r_int
)paren
id|irq_nr
)paren
suffix:semicolon
id|prom_halt
c_func
(paren
)paren
suffix:semicolon
macro_line|#endif
r_break
suffix:semicolon
)brace
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/* XXX Needs to handle sun4m semantics XXX */
r_void
DECL|function|enable_irq
id|enable_irq
c_func
(paren
r_int
r_int
id|irq_nr
)paren
(brace
r_int
r_int
id|flags
suffix:semicolon
r_int
r_char
id|current_mask
comma
id|new_mask
suffix:semicolon
r_if
c_cond
(paren
id|sparc_cpu_model
op_ne
id|sun4c
)paren
(brace
r_return
suffix:semicolon
)brace
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|current_mask
op_assign
op_star
id|interrupt_enable
suffix:semicolon
r_switch
c_cond
(paren
id|irq_nr
)paren
(brace
r_case
l_int|1
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_or
id|SUN4C_INT_E1
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|4
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_or
id|SUN4C_INT_E4
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|6
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_or
id|SUN4C_INT_E6
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|8
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_or
id|SUN4C_INT_E8
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|10
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_or
id|SUN4C_INT_E10
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|14
suffix:colon
id|new_mask
op_assign
(paren
(paren
id|current_mask
)paren
op_or
id|SUN4C_INT_E14
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
macro_line|#if 0  /* Floppy driver does this on sun4c&squot;s anyhow */
id|printk
(paren
l_string|&quot;Interrupt does not need to enable IE&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
macro_line|#endif
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
suffix:semicolon
op_star
id|interrupt_enable
op_assign
id|new_mask
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/*&n; * Initial irq handlers.&n; */
DECL|struct|irqaction
r_struct
id|irqaction
(brace
DECL|member|handler
r_void
(paren
op_star
id|handler
)paren
(paren
r_int
comma
r_struct
id|pt_regs
op_star
)paren
suffix:semicolon
DECL|member|flags
r_int
r_int
id|flags
suffix:semicolon
DECL|member|mask
r_int
r_int
id|mask
suffix:semicolon
DECL|member|name
r_const
r_char
op_star
id|name
suffix:semicolon
)brace
suffix:semicolon
DECL|variable|irq_action
r_static
r_struct
id|irqaction
id|irq_action
(braket
l_int|16
)braket
op_assign
(brace
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
comma
l_int|NULL
)brace
)brace
suffix:semicolon
r_int
DECL|function|get_irq_list
id|get_irq_list
c_func
(paren
r_char
op_star
id|buf
)paren
(brace
r_int
id|i
comma
id|len
op_assign
l_int|0
suffix:semicolon
r_struct
id|irqaction
op_star
id|action
op_assign
id|irq_action
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
comma
id|action
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|action-&gt;handler
)paren
r_continue
suffix:semicolon
id|len
op_add_assign
id|sprintf
c_func
(paren
id|buf
op_plus
id|len
comma
l_string|&quot;%2d: %8d %c %s&bslash;n&quot;
comma
id|i
comma
id|kstat.interrupts
(braket
id|i
)braket
comma
(paren
id|action-&gt;flags
op_amp
id|SA_INTERRUPT
)paren
ques
c_cond
l_char|&squot;+&squot;
suffix:colon
l_char|&squot; &squot;
comma
id|action-&gt;name
)paren
suffix:semicolon
)brace
r_return
id|len
suffix:semicolon
)brace
r_void
DECL|function|free_irq
id|free_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
r_struct
id|irqaction
op_star
id|action
op_assign
id|irq
op_plus
id|irq_action
suffix:semicolon
r_int
r_int
id|flags
suffix:semicolon
r_if
c_cond
(paren
id|irq
OG
l_int|14
)paren
(brace
multiline_comment|/* 14 irq levels on the sparc */
id|printk
c_func
(paren
l_string|&quot;Trying to free bogus IRQ %d&bslash;n&quot;
comma
id|irq
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|action-&gt;handler
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;Trying to free free IRQ%d&bslash;n&quot;
comma
id|irq
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|disable_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
id|action-&gt;handler
op_assign
l_int|NULL
suffix:semicolon
id|action-&gt;flags
op_assign
l_int|0
suffix:semicolon
id|action-&gt;mask
op_assign
l_int|0
suffix:semicolon
id|action-&gt;name
op_assign
l_int|NULL
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
r_void
DECL|function|unexpected_irq
id|unexpected_irq
c_func
(paren
r_int
id|irq
comma
r_struct
id|pt_regs
op_star
id|regs
)paren
(brace
r_int
id|i
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;IO device interrupt, irq = %d&bslash;n&quot;
comma
id|irq
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;PC = %08lx NPC = %08lx FP=%08lx&bslash;n&quot;
comma
id|regs-&gt;pc
comma
id|regs-&gt;npc
comma
id|regs-&gt;u_regs
(braket
l_int|14
)braket
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Expecting: &quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|irq_action
(braket
id|i
)braket
dot
id|handler
)paren
id|printk
c_func
(paren
l_string|&quot;[%s:%d:0x%x] &quot;
comma
id|irq_action
(braket
id|i
)braket
dot
id|name
comma
(paren
r_int
)paren
id|i
comma
(paren
r_int
r_int
)paren
id|irq_action
(braket
id|i
)braket
dot
id|handler
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;AIEEE&bslash;n&quot;
)paren
suffix:semicolon
id|prom_halt
c_func
(paren
)paren
suffix:semicolon
)brace
r_void
DECL|function|handler_irq
id|handler_irq
c_func
(paren
r_int
id|irq
comma
r_struct
id|pt_regs
op_star
id|regs
)paren
(brace
r_struct
id|irqaction
op_star
id|action
op_assign
id|irq_action
op_plus
id|irq
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|action-&gt;handler
)paren
(brace
id|unexpected_irq
c_func
(paren
id|irq
comma
id|regs
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|action
op_member_access_from_pointer
id|handler
c_func
(paren
id|irq
comma
id|regs
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * do_IRQ handles IRQ&squot;s that have been installed without the&n; * SA_INTERRUPT flag: it uses the full signal-handling return&n; * and runs with other interrupts enabled. All relatively slow&n; * IRQ&squot;s should use this format: notably the keyboard/timer&n; * routines.&n; */
id|asmlinkage
r_void
DECL|function|do_IRQ
id|do_IRQ
c_func
(paren
r_int
id|irq
comma
r_struct
id|pt_regs
op_star
id|regs
)paren
(brace
r_struct
id|irqaction
op_star
id|action
op_assign
id|irq
op_plus
id|irq_action
suffix:semicolon
id|kstat.interrupts
(braket
id|irq
)braket
op_increment
suffix:semicolon
id|action
op_member_access_from_pointer
id|handler
c_func
(paren
id|irq
comma
id|regs
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/*&n; * do_fast_IRQ handles IRQ&squot;s that don&squot;t need the fancy interrupt return&n; * stuff - the handler is also running with interrupts disabled unless&n; * it explicitly enables them later.&n; */
id|asmlinkage
r_void
DECL|function|do_fast_IRQ
id|do_fast_IRQ
c_func
(paren
r_int
id|irq
)paren
(brace
id|kstat.interrupts
(braket
id|irq
)braket
op_increment
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Got FAST_IRQ number %04lx&bslash;n&quot;
comma
(paren
r_int
r_int
r_int
)paren
id|irq
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_extern
r_void
id|probe_clock
c_func
(paren
r_void
)paren
suffix:semicolon
r_int
DECL|function|request_irq
id|request_irq
c_func
(paren
r_int
r_int
id|irq
comma
r_void
(paren
op_star
id|handler
)paren
(paren
r_int
comma
r_struct
id|pt_regs
op_star
)paren
comma
r_int
r_int
id|irqflags
comma
r_const
r_char
op_star
id|devname
)paren
(brace
r_struct
id|irqaction
op_star
id|action
suffix:semicolon
r_int
r_int
id|flags
suffix:semicolon
r_if
c_cond
(paren
id|irq
OG
l_int|14
)paren
(brace
multiline_comment|/* Only levels 1-14 are valid on the Sparc. */
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
multiline_comment|/* i386 keyboard interrupt request, just return */
r_if
c_cond
(paren
id|irq
op_eq
l_int|1
)paren
(brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* sched_init() requesting the timer IRQ */
r_if
c_cond
(paren
id|irq
op_eq
l_int|0
)paren
(brace
id|irq
op_assign
l_int|10
suffix:semicolon
)brace
id|action
op_assign
id|irq
op_plus
id|irq_action
suffix:semicolon
r_if
c_cond
(paren
id|action-&gt;handler
)paren
(brace
r_return
op_minus
id|EBUSY
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|handler
)paren
(brace
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|action-&gt;handler
op_assign
id|handler
suffix:semicolon
id|action-&gt;flags
op_assign
id|irqflags
suffix:semicolon
id|action-&gt;mask
op_assign
l_int|0
suffix:semicolon
id|action-&gt;name
op_assign
id|devname
suffix:semicolon
id|enable_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
multiline_comment|/* Init the timer/clocks if necessary. */
r_if
c_cond
(paren
id|irq
op_eq
l_int|10
)paren
(brace
id|probe_clock
c_func
(paren
)paren
suffix:semicolon
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_void
DECL|function|sun4c_init_IRQ
id|sun4c_init_IRQ
c_func
(paren
r_void
)paren
(brace
r_struct
id|linux_prom_registers
id|int_regs
(braket
l_int|2
)braket
suffix:semicolon
r_int
id|ie_node
suffix:semicolon
id|ie_node
op_assign
id|prom_searchsiblings
(paren
id|prom_getchild
c_func
(paren
id|prom_root_node
)paren
comma
l_string|&quot;interrupt-enable&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ie_node
op_eq
l_int|0
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;Cannot find /interrupt-enable node&bslash;n&quot;
)paren
suffix:semicolon
id|prom_halt
c_func
(paren
)paren
suffix:semicolon
)brace
multiline_comment|/* Depending on the &quot;address&quot; property is bad news... */
id|prom_getproperty
c_func
(paren
id|ie_node
comma
l_string|&quot;reg&quot;
comma
(paren
r_char
op_star
)paren
id|int_regs
comma
r_sizeof
(paren
id|int_regs
)paren
)paren
suffix:semicolon
id|sparc_alloc_io
c_func
(paren
id|int_regs
(braket
l_int|0
)braket
dot
id|phys_addr
comma
(paren
r_void
op_star
)paren
id|INTREG_VADDR
comma
id|int_regs
(braket
l_int|0
)braket
dot
id|reg_size
comma
l_string|&quot;sun4c_interrupts&quot;
comma
id|int_regs
(braket
l_int|0
)braket
dot
id|which_io
comma
l_int|0x0
)paren
suffix:semicolon
id|interrupt_enable
op_assign
(paren
r_char
op_star
)paren
id|INTREG_VADDR
suffix:semicolon
multiline_comment|/* Default value, accept interrupts, but no one is actually active */
multiline_comment|/* We also turn on level14 interrupts so PROM can run the console. */
op_star
id|interrupt_enable
op_assign
(paren
id|SUN4C_INT_ENABLE
op_or
id|SUN4C_INT_E14
)paren
suffix:semicolon
id|sti
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/* As of NOW, L1-A works.  Turn irq&squot;s on full-blast. */
r_return
suffix:semicolon
)brace
r_void
DECL|function|sun4m_init_IRQ
id|sun4m_init_IRQ
c_func
(paren
r_void
)paren
(brace
r_int
id|ie_node
comma
id|i
suffix:semicolon
r_struct
id|linux_prom_registers
id|int_regs
(braket
id|PROMREG_MAX
)braket
suffix:semicolon
r_int
id|num_regs
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|ie_node
op_assign
id|prom_searchsiblings
c_func
(paren
id|prom_getchild
c_func
(paren
id|prom_root_node
)paren
comma
l_string|&quot;obio&quot;
)paren
)paren
op_eq
l_int|0
op_logical_or
(paren
id|ie_node
op_assign
id|prom_getchild
(paren
id|ie_node
)paren
)paren
op_eq
l_int|0
op_logical_or
(paren
id|ie_node
op_assign
id|prom_searchsiblings
(paren
id|ie_node
comma
l_string|&quot;interrupt&quot;
)paren
)paren
op_eq
l_int|0
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;Cannot find /obio/interrupt node&bslash;n&quot;
)paren
suffix:semicolon
id|prom_halt
c_func
(paren
)paren
suffix:semicolon
)brace
id|num_regs
op_assign
id|prom_getproperty
c_func
(paren
id|ie_node
comma
l_string|&quot;reg&quot;
comma
(paren
r_char
op_star
)paren
id|int_regs
comma
r_sizeof
(paren
id|int_regs
)paren
)paren
suffix:semicolon
id|num_regs
op_assign
(paren
id|num_regs
op_div
r_sizeof
(paren
r_struct
id|linux_prom_registers
)paren
)paren
suffix:semicolon
multiline_comment|/* Apply the obio ranges to these registers. */
id|prom_apply_obio_ranges
c_func
(paren
id|int_regs
comma
id|num_regs
)paren
suffix:semicolon
multiline_comment|/* Map the interrupt registers for all possible cpus. */
id|sparc_alloc_io
c_func
(paren
id|int_regs
(braket
l_int|0
)braket
dot
id|phys_addr
comma
(paren
r_void
op_star
)paren
id|INTREG_VADDR
comma
id|PAGE_SIZE
op_star
id|NCPUS
comma
l_string|&quot;interrupts_percpu&quot;
comma
id|int_regs
(braket
l_int|0
)braket
dot
id|which_io
comma
l_int|0x0
)paren
suffix:semicolon
multiline_comment|/* Map the system interrupt control registers. */
id|sparc_alloc_io
c_func
(paren
id|int_regs
(braket
id|num_regs
op_minus
l_int|1
)braket
dot
id|phys_addr
comma
(paren
r_void
op_star
)paren
id|INTREG_VADDR
op_plus
(paren
id|NCPUS
op_star
id|PAGE_SIZE
)paren
comma
id|int_regs
(braket
id|num_regs
op_minus
l_int|1
)braket
dot
id|reg_size
comma
l_string|&quot;interrupts_system&quot;
comma
id|int_regs
(braket
id|num_regs
op_minus
l_int|1
)braket
dot
id|which_io
comma
l_int|0x0
)paren
suffix:semicolon
id|sun4m_interrupts
op_assign
(paren
r_struct
id|sun4m_intregs
op_star
)paren
id|INTREG_VADDR
suffix:semicolon
macro_line|#if 0
id|printk
c_func
(paren
l_string|&quot;Interrupt register dump...&bslash;n&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|NCPUS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;cpu%d: tbt %08x&bslash;n&quot;
comma
id|i
comma
id|sun4m_interrupts-&gt;cpu_intregs
(braket
id|i
)braket
dot
id|tbt
)paren
suffix:semicolon
)brace
id|printk
c_func
(paren
l_string|&quot;Master tbt %08x&bslash;n&quot;
comma
id|sun4m_interrupts-&gt;tbt
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Master irqs %08x&bslash;n&quot;
comma
id|sun4m_interrupts-&gt;irqs
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Master set %08x&bslash;n&quot;
comma
id|sun4m_interrupts-&gt;set
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Master clear %08x&bslash;n&quot;
comma
id|sun4m_interrupts-&gt;clear
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Undirected ints taken by: %08x&bslash;n&quot;
comma
id|sun4m_interrupts-&gt;undirected_target
)paren
suffix:semicolon
id|prom_halt
c_func
(paren
)paren
suffix:semicolon
macro_line|#endif
id|sti
c_func
(paren
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_void
DECL|function|init_IRQ
id|init_IRQ
c_func
(paren
r_void
)paren
(brace
r_switch
c_cond
(paren
id|sparc_cpu_model
)paren
(brace
r_case
id|sun4c
suffix:colon
id|sun4c_init_IRQ
c_func
(paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|sun4m
suffix:colon
id|sun4m_init_IRQ
c_func
(paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|printk
c_func
(paren
l_string|&quot;Cannot initialize IRQ&squot;s on this Sun machine...&bslash;n&quot;
)paren
suffix:semicolon
id|halt
c_func
(paren
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
suffix:semicolon
r_return
suffix:semicolon
)brace
eof
