multiline_comment|/* $Id: sun4c.c,v 1.121 1996/11/01 20:36:27 ecd Exp $&n; * sun4c.c: Doing in software what should be done in hardware.&n; *&n; * Copyright (C) 1996 David S. Miller (davem@caip.rutgers.edu)&n; * Copyright (C) 1996 Eddie C. Dost (ecd@skynet.be)&n; * Copyright (C) 1996 Andrew Tridgell (Andrew.Tridgell@anu.edu.au)&n; */
...
id|num_contexts
suffix:semicolon
multiline_comment|/* Small structure for ease of handling in the low level kernel fault&n; * handler. This holds all information necessary, like the sun4c_ufree_ring&n; * for user segments.&n; */
DECL|struct|sun4c_segment_info
r_struct
id|sun4c_segment_info
(brace
DECL|member|vaddr
r_int
r_int
...
suffix:semicolon
DECL|variable|sun4c_kernel_next
r_struct
id|sun4c_segment_info
op_star
id|sun4c_kernel_next
suffix:semicolon
DECL|macro|SUN4C_KERNEL_BUCKETS
mdefine_line|#define SUN4C_KERNEL_BUCKETS 32
DECL|macro|SUN4C_KERNEL_BSIZE
mdefine_line|#define SUN4C_KERNEL_BSIZE (sizeof(struct sun4c_segment_info) &bslash;&n;&t;&t;&t;    * SUN4C_KERNEL_BUCKETS)
...
r_else
(brace
multiline_comment|/* AJT: possibly read the tags and avoid flushing the ones that &n;&t;&t;   are above 0xf0000000 so the kernel isn&squot;t flushed all the time */
id|__asm__
id|__volatile__
c_func
(paren
l_string|&quot;add&t;%1, %1, %%g1&bslash;n&bslash;t&quot;
l_string|&quot;add&t;%1, %%g1, %%g2&bslash;n&bslash;t&quot;
l_string|&quot;add&t;%1, %%g2, %%g3&bslash;n&bslash;t&quot;
l_string|&quot;add&t;%1, %%g3, %%g4&bslash;n&bslash;t&quot;
l_string|&quot;add&t;%1, %%g4, %%g5&bslash;n&bslash;t&quot;
l_string|&quot;add&t;%1, %%g5, %%o4&bslash;n&bslash;t&quot;
l_string|&quot;add&t;%1, %%o4, %%o5&bslash;n&quot;
l_string|&quot;1:&bslash;n&bslash;t&quot;
l_string|&quot;subcc&t;%0, %%o5, %0&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0] %2&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %1] %2&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g1] %2&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g2] %2&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g3] %2&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g4] %2&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g5] %2&bslash;n&bslash;t&quot;
l_string|&quot;bg&t;1b&bslash;n&bslash;t&quot;
l_string|&quot; sta&t;%%g0, [%0 + %%o4] %2&bslash;n&bslash;t&quot;
suffix:colon
suffix:colon
l_string|&quot;r&quot;
(paren
...
l_string|&quot;sta&t;%%g0, [%0] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %2] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g1] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g2] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g3] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g4] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%g5] %3&bslash;n&bslash;t&quot;
l_string|&quot;sta&t;%%g0, [%0 + %%o4] %3&bslash;n&bslash;t&quot;
l_string|&quot;bg&t;1b&bslash;n&bslash;t&quot;
...
suffix:semicolon
multiline_comment|/* Only vac-hwflush (with a dash) is reliable, weitek&n;&t; * power-up processor claims vac_hwflush (underscore)&n;&t; * yet crashes if you try to use hardware based flushes.&n;&t; */
id|sun4c_vacinfo.do_hwflushes
op_assign
id|prom_getintdefault
c_func
...
r_int
r_int
id|sun4c_kernel_buckets_patch
suffix:semicolon
r_extern
r_int
r_int
id|sun4c_kernel_buckets_patch_32
suffix:semicolon
...
c_func
(paren
l_string|&quot;Unhandled number of kernel buckets: %d&bslash;n&quot;
comma
id|SUN4C_KERNEL_BUCKETS
)paren
suffix:semicolon
...
DECL|function|sun4c_next_kernel_bucket
r_static
r_inline
r_void
id|sun4c_next_kernel_bucket
c_func
(paren
r_struct
id|sun4c_segment_info
...
l_int|0
suffix:semicolon
multiline_comment|/* This needs to be aligned to twice it&squot;s size for speed. */
id|sun4c_kernel_next
op_assign
id|sparc_init_alloc
c_func
...
suffix:semicolon
id|sun4c_kernel_next-&gt;vaddr
op_assign
l_int|0
suffix:semicolon
id|sun4c_kernel_next-&gt;pseg
op_assign
id|mmu_entry_pool
(braket
id|i
...
(brace
multiline_comment|/* PM: need flush_user_windows() ?? */
id|sun4c_put_segmap
c_func
(paren
id|uentry-&gt;vaddr
...
c_func
(paren
l_string|&quot;sun4c fault handler bolixed...&quot;
)paren
suffix:semicolon
)brace
...
multiline_comment|/* sun4c_init_lock_area(sun4c_iobuffer_start, sun4c_iobuffer_end); */
...
id|entry2
suffix:semicolon
multiline_comment|/* don&squot;t flush kernel memory as its always valid in&n;&t;   all contexts */
r_if
c_cond
(paren
id|start
...
suffix:semicolon
multiline_comment|/* we are betting that the entry will not be &n;&t;&t;&t;&t;   needed for a while */
id|sun4c_user_unmap
c_func
(paren
id|entry
...
suffix:semicolon
multiline_comment|/* XXX P3: Is it necessary for I/O page? */
id|sun4c_put_pte
c_func
(paren
id|virt_addr
...
suffix:semicolon
multiline_comment|/* start_mem = sun4c_init_alloc_dvma_pages(start_mem); */
id|start_mem
op_assign
id|sparc_context_init
c_func
