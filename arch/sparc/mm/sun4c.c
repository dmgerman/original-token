multiline_comment|/* $Id: sun4c.c,v 1.133 1996/12/18 06:43:28 tridge Exp $&n; * sun4c.c: Doing in software what should be done in hardware.&n; *&n; * Copyright (C) 1996 David S. Miller (davem@caip.rutgers.edu)&n; * Copyright (C) 1996 Eddie C. Dost (ecd@skynet.be)&n; * Copyright (C) 1996 Andrew Tridgell (Andrew.Tridgell@anu.edu.au)&n; */
...
multiline_comment|/* Please take special note on the foo_kernel() routines below, our&n; * fast in window fault handler wants to get at the pte&squot;s for vmalloc&n; * area with traps off, therefore they _MUST_ be locked down to prevent&n; * a watchdog from happening.  The lowest VMALLOC&squot;d thing is the buffer&n; * cache hash table, it is never free&squot;d ever, so the pg0 we use for the&n; * lowest most page of pte&squot;s will never be freed and is guarenteed to&n; * be locked down.  Be careful with this code thanks.&n; */
...
id|PAGE_MASK
suffix:semicolon
multiline_comment|/* See above comment. */
r_if
c_cond
(paren
id|pg0_addr
