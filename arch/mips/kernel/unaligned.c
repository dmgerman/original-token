multiline_comment|/*&n; * Handle unaligned accesses by emulation.&n; *&n; * This file is subject to the terms and conditions of the GNU General Public&n; * License.  See the file &quot;COPYING&quot; in the main directory of this archive&n; * for more details.&n; *&n; * Copyright (C) 1996 by Ralf Baechle&n; *&n; * This file contains exception handler for address error exception with the&n; * special capability to execute faulting instructions in software.  The&n; * handler does not try to handle the case when the program counter points&n; * to an address not aligned to a word boundary.&n; *&n; * Putting data to unaligned addresses is a bad practice even on Intel where&n; * only the performance is affected.  Much worse is that such code is non-&n; * portable.  Due to several programs that die on MIPS due to alignment&n; * problems I decieded to implement this handler anyway though I originally&n; * didn&squot;t intend to do this at all for user code.&n; *&n; * For now I enable fixing of address errors by default to make life easier.&n; * I however intend to disable this somewhen in the future when the alignment&n; * problems with user programs have been fixed.  For programmers this is the&n; * right way to go.&n; *&n; * Fixing address errors is a per process option.  The option is inherited&n; * across fork(2) and execve(2) calls.  If you really want to use the&n; * option in your user programs - I discourage the use of the software&n; * emulation strongly - use the following code in your userland stuff:&n; *&n; * #include &lt;sys/sysmips.h&gt;&n; *&n; * ...&n; * sysmips(MIPS_FIXADE, x);&n; * ...&n; *&n; * The parameter x is 0 for disabeling software emulation.  Set bit 0 for&n; * enabeling software emulation and bit 1 for enabeling printing debug&n; * messages into syslog to aid finding address errors in programs.&n; *&n; * The logging feature is an addition over RISC/os and IRIX where only the&n; * values 0 and 1 are acceptable values for x.  I&squot;ll probably remove this&n; * hack later on.&n; *&n; * Below a little program to play around with this feature.&n; *&n; * #include &lt;stdio.h&gt;&n; * #include &lt;asm/sysmips.h&gt;&n; * &n; * struct foo {&n; *         unsigned char bar[8];&n; * };&n; *&n; * main(int argc, char *argv[])&n; * {&n; *         struct foo x = {0, 1, 2, 3, 4, 5, 6, 7};&n; *         unsigned int *p = (unsigned int *) (x.bar + 3);&n; *         int i;&n; *&n; *         if (argc &gt; 1)&n; *                 sysmips(MIPS_FIXADE, atoi(argv[1]));&n; *&n; *         printf(&quot;*p = %08lx&bslash;n&quot;, *p);&n; *&n; *         *p = 0xdeadface;&n; *&n; *         for(i = 0; i &lt;= 7; i++)&n; *         printf(&quot;%02x &quot;, x.bar[i]);&n; *         printf(&quot;&bslash;n&quot;);&n; * }&n; *&n; * Until I&squot;ve written the code to handle branch delay slots it may happen&n; * that the kernel receives an ades/adel instruction from an insn in a&n; * branch delay slot but is unable to handle this case.  The kernel knows&n; * this fact and therefore will kill the process.  For most code you can&n; * fix this temporarily by compiling with flags -fno-delayed-branch -Wa,-O0.&n; *&n; * Coprozessor loads are not supported; I think this case is unimportant&n; * in the practice.&n; *&n; * TODO: Handle ndc (attempted store to doubleword in uncached memory)&n; *       exception for the R6000.&n; *       A store crossing a page boundary might be executed only partially.&n; *       Undo the partial store in this case.&n; */
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/signal.h&gt;
macro_line|#include &lt;linux/smp.h&gt;
macro_line|#include &lt;linux/smp_lock.h&gt;
macro_line|#include &lt;asm/branch.h&gt;
macro_line|#include &lt;asm/byteorder.h&gt;
macro_line|#include &lt;asm/inst.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
DECL|macro|CONF_NO_UNALIGNED_KERNEL_ACCESS
macro_line|#undef CONF_NO_UNALIGNED_KERNEL_ACCESS
DECL|macro|CONF_LOG_UNALIGNED_ACCESSES
macro_line|#undef CONF_LOG_UNALIGNED_ACCESSES
DECL|macro|STR
mdefine_line|#define STR(x)  __STR(x)
DECL|macro|__STR
mdefine_line|#define __STR(x)  #x
DECL|typedef|register_t
r_typedef
r_int
r_int
id|register_t
suffix:semicolon
multiline_comment|/*&n; * User code may only access USEG; kernel code may access the&n; * entire address space.&n; */
DECL|macro|check_axs
mdefine_line|#define check_axs(p,a,s)                                &bslash;&n;&t;if ((long)(~(pc) &amp; ((a) | ((a)+(s)))) &lt; 0)      &bslash;&n;&t;&t;goto sigbus;
r_static
r_inline
r_void
DECL|function|emulate_load_store_insn
id|emulate_load_store_insn
c_func
(paren
r_struct
id|pt_regs
op_star
id|regs
comma
r_int
r_int
id|addr
comma
r_int
r_int
id|pc
)paren
(brace
r_union
id|mips_instruction
id|insn
suffix:semicolon
id|register_t
id|value
suffix:semicolon
id|regs-&gt;regs
(braket
l_int|0
)braket
op_assign
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * This load never faults.&n;&t; */
id|__get_user
c_func
(paren
id|insn.word
comma
(paren
r_int
r_int
op_star
)paren
id|pc
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|insn.i_format.opcode
)paren
(brace
multiline_comment|/*&n;&t; * These are instructions that a compiler doesn&squot;t generate.  We&n;&t; * can assume therefore that the code is MIPS-aware and&n;&t; * really buggy.  Emulating these instructions would break the&n;&t; * semantics anyway.&n;&t; */
r_case
id|ll_op
suffix:colon
r_case
id|lld_op
suffix:colon
r_case
id|sc_op
suffix:colon
r_case
id|scd_op
suffix:colon
multiline_comment|/*&n;&t; * For these instructions the only way to create an address&n;&t; * error is an attempted access to kernel/supervisor address&n;&t; * space.&n;&t; */
r_case
id|ldl_op
suffix:colon
r_case
id|ldr_op
suffix:colon
r_case
id|lwl_op
suffix:colon
r_case
id|lwr_op
suffix:colon
r_case
id|sdl_op
suffix:colon
r_case
id|sdr_op
suffix:colon
r_case
id|swl_op
suffix:colon
r_case
id|swr_op
suffix:colon
r_case
id|lb_op
suffix:colon
r_case
id|lbu_op
suffix:colon
r_case
id|sb_op
suffix:colon
r_goto
id|sigbus
suffix:semicolon
multiline_comment|/*&n;&t; * The remaining opcodes are the ones that are really of interrest.&n;&t; */
r_case
id|lh_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|2
)paren
suffix:semicolon
id|__asm__
c_func
(paren
l_string|&quot;.set&bslash;tnoat&bslash;n&quot;
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tlb&bslash;t%0,0(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlbu&bslash;t$1,1(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tlb&bslash;t%0,1(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlbu&bslash;t$1,0(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;sll&bslash;t%0,0x8&bslash;n&bslash;t&quot;
l_string|&quot;or&bslash;t%0,$1&bslash;n&bslash;t&quot;
l_string|&quot;.set&bslash;tat&bslash;n&bslash;t&quot;
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
l_string|&quot;=&amp;r&quot;
(paren
id|value
)paren
suffix:colon
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
suffix:colon
l_string|&quot;$1&quot;
)paren
suffix:semicolon
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
op_assign
id|value
suffix:semicolon
r_return
suffix:semicolon
r_case
id|lw_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|4
)paren
suffix:semicolon
id|__asm__
c_func
(paren
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tlwl&bslash;t%0,(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlwr&bslash;t%0,3(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tlwl&bslash;t%0,3(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlwr&bslash;t%0,(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
l_string|&quot;=&amp;r&quot;
(paren
id|value
)paren
suffix:colon
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
)paren
suffix:semicolon
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
op_assign
id|value
suffix:semicolon
r_return
suffix:semicolon
r_case
id|lhu_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|2
)paren
suffix:semicolon
id|__asm__
c_func
(paren
l_string|&quot;.set&bslash;tnoat&bslash;n&quot;
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tlbu&bslash;t%0,0(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlbu&bslash;t$1,1(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tlbu&bslash;t%0,1(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlbu&bslash;t$1,0(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;sll&bslash;t%0,0x8&bslash;n&bslash;t&quot;
l_string|&quot;or&bslash;t%0,$1&bslash;n&bslash;t&quot;
l_string|&quot;.set&bslash;tat&bslash;n&bslash;t&quot;
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
l_string|&quot;=&amp;r&quot;
(paren
id|value
)paren
suffix:colon
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
suffix:colon
l_string|&quot;$1&quot;
)paren
suffix:semicolon
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
op_assign
id|value
suffix:semicolon
r_return
suffix:semicolon
r_case
id|lwu_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|4
)paren
suffix:semicolon
id|__asm__
c_func
(paren
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tlwl&bslash;t%0,(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlwr&bslash;t%0,3(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tlwl&bslash;t%0,3(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tlwr&bslash;t%0,(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
l_string|&quot;=&amp;r&quot;
(paren
id|value
)paren
suffix:colon
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
)paren
suffix:semicolon
id|value
op_and_assign
l_int|0xffffffff
suffix:semicolon
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
op_assign
id|value
suffix:semicolon
r_return
suffix:semicolon
r_case
id|ld_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|8
)paren
suffix:semicolon
id|__asm__
c_func
(paren
l_string|&quot;.set&bslash;tmips3&bslash;n&quot;
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tldl&bslash;t%0,(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tldr&bslash;t%0,7(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tldl&bslash;t%0,7(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tldr&bslash;t%0,(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;.set&bslash;tmips0&bslash;n&bslash;t&quot;
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
l_string|&quot;=&amp;r&quot;
(paren
id|value
)paren
suffix:colon
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
)paren
suffix:semicolon
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
op_assign
id|value
suffix:semicolon
r_return
suffix:semicolon
r_case
id|sh_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|2
)paren
suffix:semicolon
id|value
op_assign
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
suffix:semicolon
id|__asm__
c_func
(paren
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;.set&bslash;tnoat&bslash;n&quot;
l_string|&quot;1:&bslash;tsb&bslash;t%0,1(%1)&bslash;n&bslash;t&quot;
l_string|&quot;srl&bslash;t$1,%0,0x8&bslash;n&quot;
l_string|&quot;2:&bslash;tsb&bslash;t$1,0(%1)&bslash;n&bslash;t&quot;
l_string|&quot;.set&bslash;tat&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;.set&bslash;tnoat&bslash;n&quot;
l_string|&quot;1:&bslash;tsb&bslash;t%0,0(%1)&bslash;n&bslash;t&quot;
l_string|&quot;srl&bslash;t$1,%0,0x8&bslash;n&quot;
l_string|&quot;2:&bslash;tsb&bslash;t$1,1(%1)&bslash;n&bslash;t&quot;
l_string|&quot;.set&bslash;tat&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
multiline_comment|/* no outputs */
suffix:colon
l_string|&quot;r&quot;
(paren
id|value
)paren
comma
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
suffix:colon
l_string|&quot;$1&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
r_case
id|sw_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|4
)paren
suffix:semicolon
id|value
op_assign
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
suffix:semicolon
id|__asm__
c_func
(paren
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tswl&bslash;t%0,(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tswr&bslash;t%0,3(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tswl&bslash;t%0,3(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tswr&bslash;t%0,(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
multiline_comment|/* no outputs */
suffix:colon
l_string|&quot;r&quot;
(paren
id|value
)paren
comma
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
)paren
suffix:semicolon
r_return
suffix:semicolon
r_case
id|sd_op
suffix:colon
id|check_axs
c_func
(paren
id|pc
comma
id|addr
comma
l_int|8
)paren
suffix:semicolon
id|value
op_assign
id|regs-&gt;regs
(braket
id|insn.i_format.rt
)braket
suffix:semicolon
id|__asm__
c_func
(paren
l_string|&quot;.set&bslash;tmips3&bslash;n&quot;
macro_line|#ifdef __BIG_ENDIAN
l_string|&quot;1:&bslash;tsdl&bslash;t%0,(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tsdr&bslash;t%0,7(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
macro_line|#ifdef __LITTLE_ENDIAN
l_string|&quot;1:&bslash;tsdl&bslash;t%0,7(%1)&bslash;n&quot;
l_string|&quot;2:&bslash;tsdr&bslash;t%0,(%1)&bslash;n&bslash;t&quot;
macro_line|#endif
l_string|&quot;.set&bslash;tmips0&bslash;n&bslash;t&quot;
l_string|&quot;.section&bslash;t__ex_table,&bslash;&quot;a&bslash;&quot;&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t1b,%2&bslash;n&bslash;t&quot;
id|STR
c_func
(paren
id|PTR
)paren
l_string|&quot;&bslash;t2b,%2&bslash;n&bslash;t&quot;
l_string|&quot;.previous&quot;
suffix:colon
multiline_comment|/* no outputs */
suffix:colon
l_string|&quot;r&quot;
(paren
id|value
)paren
comma
l_string|&quot;r&quot;
(paren
id|addr
)paren
comma
l_string|&quot;i&quot;
(paren
op_logical_and
id|fault
)paren
)paren
suffix:semicolon
r_return
suffix:semicolon
r_case
id|lwc1_op
suffix:colon
r_case
id|ldc1_op
suffix:colon
r_case
id|swc1_op
suffix:colon
r_case
id|sdc1_op
suffix:colon
multiline_comment|/*&n;&t;&t; * I herewith declare: this does not happen.  So send SIGBUS.&n;&t;&t; */
r_goto
id|sigbus
suffix:semicolon
r_case
id|lwc2_op
suffix:colon
r_case
id|ldc2_op
suffix:colon
r_case
id|swc2_op
suffix:colon
r_case
id|sdc2_op
suffix:colon
multiline_comment|/*&n;&t;&t; * These are the coprozessor 2 load/stores.  The current&n;&t;&t; * implementations don&squot;t use cp2 and cp2 should always be&n;&t;&t; * disabled in c0_status.  So send SIGILL.&n;                 * (No longer true: The Sony Praystation uses cp2 for&n;                 * 3D matrix operations.  Dunno if that thingy has a MMU ...)&n;&t;&t; */
r_default
suffix:colon
multiline_comment|/*&n;&t;&t; * Pheeee...  We encountered an yet unknown instruction ...&n;&t;&t; */
id|force_sig
c_func
(paren
id|SIGILL
comma
id|current
)paren
suffix:semicolon
)brace
r_return
suffix:semicolon
id|fault
suffix:colon
id|send_sig
c_func
(paren
id|SIGSEGV
comma
id|current
comma
l_int|1
)paren
suffix:semicolon
r_return
suffix:semicolon
id|sigbus
suffix:colon
id|send_sig
c_func
(paren
id|SIGBUS
comma
id|current
comma
l_int|1
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
DECL|variable|unaligned_instructions
r_int
r_int
id|unaligned_instructions
suffix:semicolon
r_static
r_inline
r_void
DECL|function|fix_ade
id|fix_ade
c_func
(paren
r_struct
id|pt_regs
op_star
id|regs
comma
r_int
r_int
id|pc
)paren
(brace
multiline_comment|/*&n;&t; * Did we catch a fault trying to load an instruction?&n;&t; */
r_if
c_cond
(paren
id|regs-&gt;cp0_badvaddr
op_eq
id|pc
)paren
(brace
multiline_comment|/*&n;&t;&t; * Phee...  Either the code is severly messed up or the&n;&t;&t; * process tried to activate some MIPS16 code.&n;&t;&t; */
id|force_sig
c_func
(paren
id|SIGBUS
comma
id|current
)paren
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Ok, this wasn&squot;t a failed instruction load.  The CPU was capable of&n;&t; * reading the instruction and faulted after this.  So we don&squot;t need&n;&t; * to verify_area the address of the instrucion.  We still don&squot;t&n;&t; * know whether the address used was legal and therefore need to do&n;&t; * verify_area().  The CPU already did the checking for legal&n;&t; * instructions for us, so we don&squot;t need to do this.&n;&t; */
id|emulate_load_store_insn
c_func
(paren
id|regs
comma
id|regs-&gt;cp0_badvaddr
comma
id|pc
)paren
suffix:semicolon
id|unaligned_instructions
op_increment
suffix:semicolon
)brace
DECL|macro|kernel_address
mdefine_line|#define kernel_address(x) ((long)(x) &lt; 0)
id|asmlinkage
r_void
DECL|function|do_ade
id|do_ade
c_func
(paren
r_struct
id|pt_regs
op_star
id|regs
)paren
(brace
id|register_t
id|pc
op_assign
id|regs-&gt;cp0_epc
suffix:semicolon
id|register_t
id|badvaddr
id|__attribute__
(paren
(paren
id|unused
)paren
)paren
op_assign
id|regs-&gt;cp0_badvaddr
suffix:semicolon
r_char
id|adels
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|adels
op_assign
(paren
(paren
(paren
id|regs-&gt;cp0_cause
op_amp
id|CAUSEF_EXCCODE
)paren
op_rshift
id|CAUSEB_EXCCODE
)paren
op_eq
l_int|4
)paren
ques
c_cond
l_char|&squot;l&squot;
suffix:colon
l_char|&squot;s&squot;
suffix:semicolon
macro_line|#ifdef CONF_NO_UNALIGNED_KERNEL_ACCESS
multiline_comment|/*&n;&t; * In an ideal world there are no unaligned accesses by the kernel.&n;&t; * So be a bit noisy ...&n;&t; */
r_if
c_cond
(paren
id|kernel_address
c_func
(paren
id|badvaddr
)paren
op_logical_and
op_logical_neg
id|user_mode
c_func
(paren
id|regs
)paren
)paren
(brace
id|show_regs
c_func
(paren
id|regs
)paren
suffix:semicolon
id|panic
c_func
(paren
l_string|&quot;Caught adel%c exception in kernel mode accessing %08lx.&quot;
comma
id|adels
comma
id|badvaddr
)paren
suffix:semicolon
)brace
macro_line|#endif /* CONF_NO_UNALIGNED_KERNEL_ACCESS */
macro_line|#ifdef CONF_LOG_UNALIGNED_ACCESSES
r_if
c_cond
(paren
id|current-&gt;tss.mflags
op_amp
id|MF_LOGADE
)paren
(brace
id|register_t
id|logpc
op_assign
id|pc
suffix:semicolon
r_if
c_cond
(paren
id|regs-&gt;cp0_cause
op_amp
id|CAUSEF_BD
)paren
id|logpc
op_add_assign
l_int|4
suffix:semicolon
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Caught adel%c in &squot;%s&squot; at 0x%08lx accessing 0x%08lx.&bslash;n&quot;
comma
id|adels
comma
id|current-&gt;comm
comma
id|logpc
comma
id|regs-&gt;cp0_badvaddr
)paren
suffix:semicolon
)brace
macro_line|#endif /* CONF_LOG_UNALIGNED_ACCESSES */
r_if
c_cond
(paren
id|compute_return_epc
c_func
(paren
id|regs
)paren
)paren
r_goto
id|out
suffix:semicolon
r_if
c_cond
(paren
id|current-&gt;tss.mflags
op_amp
id|MF_FIXADE
)paren
(brace
id|pc
op_add_assign
(paren
(paren
id|regs-&gt;cp0_cause
op_amp
id|CAUSEF_BD
)paren
ques
c_cond
l_int|4
suffix:colon
l_int|0
)paren
suffix:semicolon
id|fix_ade
c_func
(paren
id|regs
comma
id|pc
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
macro_line|#ifdef CONF_DEBUG_EXCEPTIONS
id|show_regs
c_func
(paren
id|regs
)paren
suffix:semicolon
macro_line|#endif
id|force_sig
c_func
(paren
id|SIGBUS
comma
id|current
)paren
suffix:semicolon
id|out
suffix:colon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
eof
