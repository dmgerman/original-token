multiline_comment|/*&n; * Dump R3000 TLB for debugging purposes.&n; *&n; * Copyright (C) 1994, 1995 by Waldorf Electronics, written by Ralf Baechle.&n; * Copyright (C) 1999 by Silicon Graphics, Inc.&n; * Copyright (C) 1999 by Harald Koerfgen&n; */
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;asm/bootinfo.h&gt;
macro_line|#include &lt;asm/cachectl.h&gt;
macro_line|#include &lt;asm/mipsregs.h&gt;
macro_line|#include &lt;asm/page.h&gt;
macro_line|#include &lt;asm/pgtable.h&gt;
DECL|macro|mips_tlb_entries
mdefine_line|#define mips_tlb_entries 64
r_void
DECL|function|dump_tlb
id|dump_tlb
c_func
(paren
r_int
id|first
comma
r_int
id|last
)paren
(brace
r_int
id|i
suffix:semicolon
r_int
r_int
id|asid
suffix:semicolon
r_int
r_int
id|entryhi
comma
id|entrylo0
suffix:semicolon
id|asid
op_assign
id|get_entryhi
c_func
(paren
)paren
op_amp
l_int|0xfc0
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
id|first
suffix:semicolon
id|i
op_le
id|last
suffix:semicolon
id|i
op_increment
)paren
(brace
id|write_32bit_cp0_register
c_func
(paren
id|CP0_INDEX
comma
id|i
op_lshift
l_int|8
)paren
suffix:semicolon
id|__asm__
id|__volatile__
c_func
(paren
l_string|&quot;.set&bslash;tnoreorder&bslash;n&bslash;t&quot;
l_string|&quot;tlbr&bslash;n&bslash;t&quot;
l_string|&quot;nop&bslash;n&bslash;t&quot;
l_string|&quot;.set&bslash;treorder&quot;
)paren
suffix:semicolon
id|entryhi
op_assign
id|read_32bit_cp0_register
c_func
(paren
id|CP0_ENTRYHI
)paren
suffix:semicolon
id|entrylo0
op_assign
id|read_32bit_cp0_register
c_func
(paren
id|CP0_ENTRYLO0
)paren
suffix:semicolon
multiline_comment|/* Unused entries have a virtual address of KSEG0.  */
r_if
c_cond
(paren
(paren
id|entryhi
op_amp
l_int|0xffffe000
)paren
op_ne
l_int|0x80000000
op_logical_and
(paren
id|entryhi
op_amp
l_int|0xfc0
)paren
op_eq
id|asid
)paren
(brace
multiline_comment|/*&n;&t;&t;&t; * Only print entries in use&n;&t;&t;&t; */
id|printk
c_func
(paren
l_string|&quot;Index: %2d &quot;
comma
id|i
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;va=%08lx asid=%08lx&quot;
l_string|&quot;  [pa=%06lx n=%d d=%d v=%d g=%d]&quot;
comma
(paren
id|entryhi
op_amp
l_int|0xffffe000
)paren
comma
id|entryhi
op_amp
l_int|0xfc0
comma
id|entrylo0
op_amp
id|PAGE_MASK
comma
(paren
id|entrylo0
op_amp
(paren
l_int|1
op_lshift
l_int|11
)paren
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
comma
(paren
id|entrylo0
op_amp
(paren
l_int|1
op_lshift
l_int|10
)paren
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
comma
(paren
id|entrylo0
op_amp
(paren
l_int|1
op_lshift
l_int|9
)paren
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
comma
(paren
id|entrylo0
op_amp
(paren
l_int|1
op_lshift
l_int|8
)paren
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
)paren
suffix:semicolon
)brace
)brace
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
id|set_entryhi
c_func
(paren
id|asid
)paren
suffix:semicolon
)brace
r_void
DECL|function|dump_tlb_all
id|dump_tlb_all
c_func
(paren
r_void
)paren
(brace
id|dump_tlb
c_func
(paren
l_int|0
comma
id|mips_tlb_entries
op_minus
l_int|1
)paren
suffix:semicolon
)brace
r_void
DECL|function|dump_tlb_wired
id|dump_tlb_wired
c_func
(paren
r_void
)paren
(brace
r_int
id|wired
op_assign
l_int|7
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Wired: %d&quot;
comma
id|wired
)paren
suffix:semicolon
id|dump_tlb
c_func
(paren
l_int|0
comma
id|read_32bit_cp0_register
c_func
(paren
id|CP0_WIRED
)paren
)paren
suffix:semicolon
)brace
r_void
DECL|function|dump_tlb_addr
id|dump_tlb_addr
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_int
r_int
id|flags
comma
id|oldpid
suffix:semicolon
r_int
id|index
suffix:semicolon
id|__save_and_cli
c_func
(paren
id|flags
)paren
suffix:semicolon
id|oldpid
op_assign
id|get_entryhi
c_func
(paren
)paren
op_amp
l_int|0xff
suffix:semicolon
id|set_entryhi
c_func
(paren
(paren
id|addr
op_amp
id|PAGE_MASK
)paren
op_or
id|oldpid
)paren
suffix:semicolon
id|tlb_probe
c_func
(paren
)paren
suffix:semicolon
id|index
op_assign
id|get_index
c_func
(paren
)paren
suffix:semicolon
id|set_entryhi
c_func
(paren
id|oldpid
)paren
suffix:semicolon
id|__restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_if
c_cond
(paren
id|index
OL
l_int|0
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;No entry for address 0x%08lx in TLB&bslash;n&quot;
comma
id|addr
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|printk
c_func
(paren
l_string|&quot;Entry %d maps address 0x%08lx&bslash;n&quot;
comma
id|index
comma
id|addr
)paren
suffix:semicolon
id|dump_tlb
c_func
(paren
id|index
comma
id|index
)paren
suffix:semicolon
)brace
r_void
DECL|function|dump_tlb_nonwired
id|dump_tlb_nonwired
c_func
(paren
r_void
)paren
(brace
id|dump_tlb
c_func
(paren
l_int|8
comma
id|mips_tlb_entries
op_minus
l_int|1
)paren
suffix:semicolon
)brace
r_void
DECL|function|dump_list_process
id|dump_list_process
c_func
(paren
r_struct
id|task_struct
op_star
id|t
comma
r_void
op_star
id|address
)paren
(brace
id|pgd_t
op_star
id|page_dir
comma
op_star
id|pgd
suffix:semicolon
id|pmd_t
op_star
id|pmd
suffix:semicolon
id|pte_t
op_star
id|pte
comma
id|page
suffix:semicolon
r_int
r_int
id|addr
suffix:semicolon
r_int
r_int
id|val
suffix:semicolon
id|addr
op_assign
(paren
r_int
r_int
)paren
id|address
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Addr                 == %08x&bslash;n&quot;
comma
id|addr
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;tasks-&gt;mm.pgd        == %08x&bslash;n&quot;
comma
(paren
r_int
r_int
)paren
id|t-&gt;mm-&gt;pgd
)paren
suffix:semicolon
id|page_dir
op_assign
id|pgd_offset
c_func
(paren
id|t-&gt;mm
comma
l_int|0
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;page_dir == %08x&bslash;n&quot;
comma
(paren
r_int
r_int
)paren
id|page_dir
)paren
suffix:semicolon
id|pgd
op_assign
id|pgd_offset
c_func
(paren
id|t-&gt;mm
comma
id|addr
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;pgd == %08x, &quot;
comma
(paren
r_int
r_int
)paren
id|pgd
)paren
suffix:semicolon
id|pmd
op_assign
id|pmd_offset
c_func
(paren
id|pgd
comma
id|addr
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;pmd == %08x, &quot;
comma
(paren
r_int
r_int
)paren
id|pmd
)paren
suffix:semicolon
id|pte
op_assign
id|pte_offset
c_func
(paren
id|pmd
comma
id|addr
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;pte == %08x, &quot;
comma
(paren
r_int
r_int
)paren
id|pte
)paren
suffix:semicolon
id|page
op_assign
op_star
id|pte
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;page == %08x&bslash;n&quot;
comma
(paren
r_int
r_int
)paren
id|pte_val
c_func
(paren
id|page
)paren
)paren
suffix:semicolon
id|val
op_assign
id|pte_val
c_func
(paren
id|page
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_PRESENT
)paren
id|printk
c_func
(paren
l_string|&quot;present &quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_READ
)paren
id|printk
c_func
(paren
l_string|&quot;read &quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_WRITE
)paren
id|printk
c_func
(paren
l_string|&quot;write &quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_ACCESSED
)paren
id|printk
c_func
(paren
l_string|&quot;accessed &quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_MODIFIED
)paren
id|printk
c_func
(paren
l_string|&quot;modified &quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_GLOBAL
)paren
id|printk
c_func
(paren
l_string|&quot;global &quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|val
op_amp
id|_PAGE_VALID
)paren
id|printk
c_func
(paren
l_string|&quot;valid &quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_void
DECL|function|dump_list_current
id|dump_list_current
c_func
(paren
r_void
op_star
id|address
)paren
(brace
id|dump_list_process
c_func
(paren
id|current
comma
id|address
)paren
suffix:semicolon
)brace
r_int
r_int
DECL|function|vtop
id|vtop
c_func
(paren
r_void
op_star
id|address
)paren
(brace
id|pgd_t
op_star
id|pgd
suffix:semicolon
id|pmd_t
op_star
id|pmd
suffix:semicolon
id|pte_t
op_star
id|pte
suffix:semicolon
r_int
r_int
id|addr
comma
id|paddr
suffix:semicolon
id|addr
op_assign
(paren
r_int
r_int
)paren
id|address
suffix:semicolon
id|pgd
op_assign
id|pgd_offset
c_func
(paren
id|current-&gt;mm
comma
id|addr
)paren
suffix:semicolon
id|pmd
op_assign
id|pmd_offset
c_func
(paren
id|pgd
comma
id|addr
)paren
suffix:semicolon
id|pte
op_assign
id|pte_offset
c_func
(paren
id|pmd
comma
id|addr
)paren
suffix:semicolon
id|paddr
op_assign
(paren
id|KSEG1
op_or
(paren
r_int
r_int
)paren
id|pte_val
c_func
(paren
op_star
id|pte
)paren
)paren
op_amp
id|PAGE_MASK
suffix:semicolon
id|paddr
op_or_assign
(paren
id|addr
op_amp
op_complement
id|PAGE_MASK
)paren
suffix:semicolon
r_return
id|paddr
suffix:semicolon
)brace
r_void
DECL|function|dump16
id|dump16
c_func
(paren
r_int
r_int
op_star
id|p
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|8
suffix:semicolon
id|i
op_increment
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;*%08lx == %08lx, &quot;
comma
(paren
r_int
r_int
)paren
id|p
comma
(paren
r_int
r_int
)paren
op_star
id|p
op_increment
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;*%08lx == %08lx&bslash;n&quot;
comma
(paren
r_int
r_int
)paren
id|p
comma
(paren
r_int
r_int
)paren
op_star
id|p
op_increment
)paren
suffix:semicolon
)brace
)brace
eof
