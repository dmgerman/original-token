multiline_comment|/*&n; * r4xx0.c: R4000 processor variant specific MMU/Cache routines.&n; *&n; * Copyright (C) 1996 David S. Miller (dm@engr.sgi.com)&n; *&n; * $Id: r4xx0.c,v 1.13 1998/05/21 07:34:35 davem Exp $&n; *&n; * To do:&n; *&n; *  - this code is a overbloated pig&n; *  - many of the bug workarounds are not efficient at all, but at&n; *    least they are functional ...&n; */
...
multiline_comment|/*&n; * Zero an entire page.  Basically a simple unrolled loop should do the&n; * job but we want more performance by saving memory bus bandwidth.  We&n; * have five flavours of the routine available for:&n; *&n; * - 16byte cachelines and no second level cache&n; * - 32byte cachelines second level cache&n; * - a version which handles the buggy R4600 v1.x&n; * - a version which handles the buggy R4600 v2.0&n; * - Finally a last version without fancy cache games for the SC and MC&n; *   versions of R4000 and R4400.  Cache instructions are quite expensive&n; *   and I guess using them for both the primary and the second level cache&n; *   wouldn&squot;t be worth the effort.&n; *   This needs to be verified by benchmarking.&n; */
