multiline_comment|/*&n; * file.c: ARCS firmware interface to files.&n; *&n; * Copyright (C) 1996 David S. Miller (dm@engr.sgi.com)&n; *&n; * $Id: file.c,v 1.2 1999/10/09 00:00:57 ralf Exp $&n; */
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;asm/sgialib.h&gt;
DECL|function|prom_getvdirent
r_int
id|__init
id|prom_getvdirent
c_func
(paren
r_int
r_int
id|fd
comma
r_struct
id|linux_vdirent
op_star
id|ent
comma
r_int
r_int
id|num
comma
r_int
r_int
op_star
id|cnt
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|get_vdirent
c_func
(paren
id|fd
comma
id|ent
comma
id|num
comma
id|cnt
)paren
suffix:semicolon
)brace
DECL|function|prom_open
r_int
id|__init
id|prom_open
c_func
(paren
r_char
op_star
id|name
comma
r_enum
id|linux_omode
id|md
comma
r_int
r_int
op_star
id|fd
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|open
c_func
(paren
id|name
comma
id|md
comma
id|fd
)paren
suffix:semicolon
)brace
DECL|function|prom_close
r_int
id|__init
id|prom_close
c_func
(paren
r_int
r_int
id|fd
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|close
c_func
(paren
id|fd
)paren
suffix:semicolon
)brace
DECL|function|prom_read
r_int
id|__init
id|prom_read
c_func
(paren
r_int
r_int
id|fd
comma
r_void
op_star
id|buf
comma
r_int
r_int
id|num
comma
r_int
r_int
op_star
id|cnt
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|read
c_func
(paren
id|fd
comma
id|buf
comma
id|num
comma
id|cnt
)paren
suffix:semicolon
)brace
DECL|function|prom_getrstatus
r_int
id|__init
id|prom_getrstatus
c_func
(paren
r_int
r_int
id|fd
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|get_rstatus
c_func
(paren
id|fd
)paren
suffix:semicolon
)brace
DECL|function|prom_write
r_int
id|__init
id|prom_write
c_func
(paren
r_int
r_int
id|fd
comma
r_void
op_star
id|buf
comma
r_int
r_int
id|num
comma
r_int
r_int
op_star
id|cnt
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|write
c_func
(paren
id|fd
comma
id|buf
comma
id|num
comma
id|cnt
)paren
suffix:semicolon
)brace
DECL|function|prom_seek
r_int
id|__init
id|prom_seek
c_func
(paren
r_int
r_int
id|fd
comma
r_struct
id|linux_bigint
op_star
id|off
comma
r_enum
id|linux_seekmode
id|sm
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|seek
c_func
(paren
id|fd
comma
id|off
comma
id|sm
)paren
suffix:semicolon
)brace
DECL|function|prom_mount
r_int
id|__init
id|prom_mount
c_func
(paren
r_char
op_star
id|name
comma
r_enum
id|linux_mountops
id|op
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|mount
c_func
(paren
id|name
comma
id|op
)paren
suffix:semicolon
)brace
DECL|function|prom_getfinfo
r_int
id|__init
id|prom_getfinfo
c_func
(paren
r_int
r_int
id|fd
comma
r_struct
id|linux_finfo
op_star
id|buf
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|get_finfo
c_func
(paren
id|fd
comma
id|buf
)paren
suffix:semicolon
)brace
DECL|function|prom_setfinfo
r_int
id|__init
id|prom_setfinfo
c_func
(paren
r_int
r_int
id|fd
comma
r_int
r_int
id|flags
comma
r_int
r_int
id|msk
)paren
(brace
r_return
id|romvec
op_member_access_from_pointer
id|set_finfo
c_func
(paren
id|fd
comma
id|flags
comma
id|msk
)paren
suffix:semicolon
)brace
eof
