...
id|temp
suffix:semicolon
multiline_comment|/* Set up error reporting. Make sure CPU_PE is OFF in the mask.  */
id|temp
op_assign
op_star
(paren
...
l_int|4
suffix:semicolon
multiline_comment|/* Enable master/target abort.  */
id|temp
op_assign
op_star
(paren
...
l_int|0x180
suffix:semicolon
multiline_comment|/* Clear the PYXIS_CFG register, which gets used  for PCI Config&n;&t;   Space accesses.  That is the way we want to use it, and we do&n;&t;   not want to depend on what ARC or SRM might have left behind.  */
op_star
(paren
id|vuip
)paren
...
l_int|0
suffix:semicolon
multiline_comment|/* Finally, check that the PYXIS_CTRL1 has IOA_BEN set for&n;&t;   enabling byte/word PCI bus space(s) access.  */
id|temp
op_assign
op_star
(paren
...
macro_line|#if 1
multiline_comment|/* ??? There&squot;s some bit of syncronization wrt writing new tlb&n;&t;   entries that&squot;s missing.  Sometimes it works, sometimes invalid&n;&t;   tlb machine checks, sometimes hard lockup.  And this just within&n;&t;   the boot sequence.&n;&n;&t;   I&squot;ve tried extra memory barriers, extra alignment, pyxis&n;&t;   register reads, tlb flushes, and loopback tlb accesses.&n;&n;&t;   I guess the pyxis revision in the sx164 is just too buggy...  */
id|hose-&gt;sg_isa
op_assign
id|hose-&gt;sg_pci
op_assign
