...
multiline_comment|/*&n; * entUna has a different register layout to be reasonably simple. It&n; * needs access to all the integer registers (the kernel doesn&squot;t use&n; * fp-regs), and it needs to have them in order for simpler access.&n; *&n; * Due to the non-standard register layout (and because we don&squot;t want&n; * to handle floating-point regs), we disallow user-mode unaligned&n; * accesses (we&squot;d need to do &quot;verify_area()&quot; checking, as well as&n; * do a full &quot;ret_from_sys_call&quot; return).&n; *&n; * Oh, btw, we don&squot;t handle the &quot;gp&quot; register correctly, but if we fault&n; * on a gp-register unaligned load/store, something is _very_ wrong&n; * in the kernel anyway..&n; */
DECL|struct|allregs
r_struct
id|allregs
(brace
