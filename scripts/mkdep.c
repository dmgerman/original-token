...
DECL|macro|first_byte
mdefine_line|#define first_byte(x) current = (unsigned char) x; x &gt;&gt;= 8;
macro_line|#else
DECL|macro|first_byte
mdefine_line|#define first_byte(x) current = x &gt;&gt; 8*(sizeof(unsigned long)-1); x &lt;&lt;= 8;
macro_line|#endif
DECL|macro|GETNEXT
mdefine_line|#define GETNEXT { &bslash;&n;if (!__buf) { &bslash;&n;&t;__buf = *(unsigned long *) next; &bslash;&n;&t;if (!__buf) &bslash;&n;&t;&t;break; &bslash;&n;} first_byte(__buf); next++; }
DECL|macro|CASE
