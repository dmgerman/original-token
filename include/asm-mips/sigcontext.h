macro_line|#ifndef __ASM_MIPS_SIGCONTEXT_H
DECL|macro|__ASM_MIPS_SIGCONTEXT_H
mdefine_line|#define __ASM_MIPS_SIGCONTEXT_H
multiline_comment|/*&n; * This struct isn&squot;t in the ABI, so we continue to use the old&n; * pre 1.3 definition.  Needs to be changed for 64 bit kernels,&n; * but it&squot;s 4am ...&n; */
DECL|struct|sigcontext_struct
r_struct
id|sigcontext_struct
(brace
DECL|member|sc_at
DECL|member|sc_v0
DECL|member|sc_v1
DECL|member|sc_a0
DECL|member|sc_a1
DECL|member|sc_a2
DECL|member|sc_a3
r_int
r_int
id|sc_at
comma
id|sc_v0
comma
id|sc_v1
comma
id|sc_a0
comma
id|sc_a1
comma
id|sc_a2
comma
id|sc_a3
suffix:semicolon
DECL|member|sc_t0
DECL|member|sc_t1
DECL|member|sc_t2
DECL|member|sc_t3
DECL|member|sc_t4
DECL|member|sc_t5
DECL|member|sc_t6
DECL|member|sc_t7
r_int
r_int
id|sc_t0
comma
id|sc_t1
comma
id|sc_t2
comma
id|sc_t3
comma
id|sc_t4
comma
id|sc_t5
comma
id|sc_t6
comma
id|sc_t7
suffix:semicolon
DECL|member|sc_s0
DECL|member|sc_s1
DECL|member|sc_s2
DECL|member|sc_s3
DECL|member|sc_s4
DECL|member|sc_s5
DECL|member|sc_s6
DECL|member|sc_s7
r_int
r_int
id|sc_s0
comma
id|sc_s1
comma
id|sc_s2
comma
id|sc_s3
comma
id|sc_s4
comma
id|sc_s5
comma
id|sc_s6
comma
id|sc_s7
suffix:semicolon
DECL|member|sc_t8
DECL|member|sc_t9
DECL|member|sc_gp
DECL|member|sc_sp
DECL|member|sc_fp
DECL|member|sc_ra
r_int
r_int
id|sc_t8
comma
id|sc_t9
comma
id|sc_gp
comma
id|sc_sp
comma
id|sc_fp
comma
id|sc_ra
suffix:semicolon
DECL|member|sc_epc
r_int
r_int
id|sc_epc
suffix:semicolon
DECL|member|sc_cause
r_int
r_int
id|sc_cause
suffix:semicolon
DECL|member|sc_oldmask
r_int
r_int
id|sc_oldmask
suffix:semicolon
)brace
suffix:semicolon
macro_line|#endif /* __ASM_MIPS_SIGCONTEXT_H */
eof
