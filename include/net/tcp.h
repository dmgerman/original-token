...
multiline_comment|/* &n; * How much of the receive buffer do we advertize &n; * (the rest is reserved for headers and driver packet overhead)&n; * Use a power of 2.&n; */
DECL|macro|TCP_WINDOW_ADVERTISE_DIVISOR
mdefine_line|#define TCP_WINDOW_ADVERTISE_DIVISOR 2
...
mdefine_line|#define TCP_TW_RECYCLE_TICK (7+2-TCP_TW_RECYCLE_SLOTS_LOG)
macro_line|#elif HZ == 1024
DECL|macro|TCP_TW_RECYCLE_TICK
mdefine_line|#define TCP_TW_RECYCLE_TICK (10+2-TCP_TW_RECYCLE_SLOTS_LOG)
macro_line|#else
macro_line|#error HZ != 100 &amp;&amp; HZ != 1024.
macro_line|#endif
...
multiline_comment|/* Recalculate snd_ssthresh, we want to set it to:&n; *&n; * &t;one half the current congestion window, but no&n; *&t;less than two segments&n; *&n; * We must take into account the current send window&n; * as well, however we keep track of that using different&n; * units so a conversion is necessary.  -DaveM&n; *&n; * RED-PEN.&n; *  RFC 2581: &quot;an easy mistake to make is to simply use cwnd,&n; *             rather than FlightSize&quot;&n; * I see no references to FlightSize here. snd_wnd is not FlightSize,&n; * it is also apriory characteristics.&n; *&n; *   FlightSize = min((snd_nxt-snd_una)/mss, packets_out) ?&n; */
...
DECL|function|__tcp_enter_cong_avoid
r_extern
id|__inline__
r_void
...
DECL|function|tcp_enter_cong_avoid
r_extern
id|__inline__
r_void
...
multiline_comment|/* Increase initial CWND conservatively, i.e. only if estimated&n;   RTT is low enough. It is not quite correct, we should use&n;   POWER i.e. RTT*BANDWIDTH, but we still cannot estimate this.&n;&n;   Numbers are taken from RFC1414.&n; */
...
(brace
multiline_comment|/*&n;&t; * Reset CWND after idle period longer RTO to &quot;restart window&quot;.&n;&t; * It is &quot;side&quot; effect of the function, which is _not_ good&n;&t; * from viewpoint of clarity. But we have to make it before&n;&t; * checking congestion window below. Alternative is to prepend&n;&t; * all the calls with this test.&n;&t; */
r_if
c_cond
(paren
id|tp-&gt;packets_out
...
op_div
id|TCP_WINDOW_ADVERTISE_DIVISOR
suffix:semicolon
)brace
...
multiline_comment|/* This function does not return reliable answer. Use it only as advice.&n; */
...
macro_line|#if 0 /* TCP_DEBUG */
mdefine_line|#define TCP_CHECK_TIMER(sk) &bslash;&n;do { &t;struct tcp_opt *__tp = &amp;sk-&gt;tp_pinfo.af_tcp; &bslash;&n;&t;if (sk-&gt;state != TCP_CLOSE) { &bslash;&n;&t;&t;if (__tp-&gt;packets_out) { &bslash;&n;&t;&t;&t;if (!tcp_timer_is_set(sk, TCP_TIME_RETRANS) &amp;&amp; !timer_is_running(&amp;__tp-&gt;retransmit_timer) &amp;&amp; net_ratelimit()) &bslash;&n;&t;&t;&t;&t;printk(KERN_DEBUG &quot;sk=%p RETRANS&quot; __FUNCTION__ &quot;(%d) %d&bslash;n&quot;, sk, __LINE__, sk-&gt;state); &bslash;&n;&t;&t;} else if (__tp-&gt;send_head) { &bslash;&n;&t;&t;&t;if (!tcp_timer_is_set(sk, TCP_TIME_PROBE0) &amp;&amp; !timer_is_running(&amp;__tp-&gt;probe_timer) &amp;&amp; net_ratelimit()) &bslash;&n;&t;&t;&t;&t;printk(KERN_DEBUG &quot;sk=%p PROBE0&quot; __FUNCTION__ &quot;(%d) %d&bslash;n&quot;, sk, __LINE__, sk-&gt;state); &bslash;&n;&t;&t;} &bslash;&n;&t;        if (__tp-&gt;ack.pending) { &bslash;&n;&t;&t;&t;if (!tcp_timer_is_set(sk, TCP_TIME_DACK) &amp;&amp; !timer_is_running(&amp;__tp-&gt;delack_timer) &amp;&amp; net_ratelimit()) &bslash;&n;&t;&t;&t;&t;printk(KERN_DEBUG &quot;sk=%p DACK&quot; __FUNCTION__ &quot;(%d) %d&bslash;n&quot;, sk, __LINE__, sk-&gt;state); &bslash;&n;&t;&t;} &bslash;&n;                if (__tp-&gt;packets_out &gt; skb_queue_len(&amp;sk-&gt;write_queue) || &bslash;&n;&t;&t;    (__tp-&gt;send_head &amp;&amp; skb_queue_len(&amp;sk-&gt;write_queue) == 0)) { &bslash;&n;&t;&t;&t; printk(KERN_DEBUG &quot;sk=%p QUEUE&quot; __FUNCTION__ &quot;(%d) %d %d %d %p&bslash;n&quot;, sk, __LINE__, sk-&gt;state, __tp-&gt;packets_out, skb_queue_len(&amp;sk-&gt;write_queue), __tp-&gt;send_head); &bslash;&n;&t;&t;} &bslash;&n;&t;} } while (0)
macro_line|#else
