...
multiline_comment|/* Recalculate snd_ssthresh, we want to set it to:&n; *&n; * &t;one half the current congestion window, but no&n; *&t;less than two segments&n; *&n; * We must take into account the current send window&n; * as well, however we keep track of that using different&n; * units so a conversion is necessary.  -DaveM&n; */
