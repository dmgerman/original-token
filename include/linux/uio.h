...
multiline_comment|/* A word of warning: Our uio structure will clash with the C library one (which is now obsolete). Remove the C&n;   library one from sys/uio.h */
DECL|struct|iovec
r_struct
id|iovec
(brace
...
mdefine_line|#define MAX_IOVEC&t;8&t;/* Maximum iovec&squot;s in one operation */
macro_line|#endif
eof
