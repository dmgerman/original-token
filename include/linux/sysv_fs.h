...
multiline_comment|/* The number of indirect pointers per block is&n;   sb-&gt;sv_ind_per_block = block_size / sizeof(unsigned long) */
...
multiline_comment|/* Because the block size may be smaller than 1024 (which is the unit used by&n;   the disk drivers and the buffer code), many functions must return a pointer&n;   to the buffer data additionally to the buffer head pointer.&n;*/
macro_line|#if 0
r_struct
id|bh_data
(brace
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* sysv_bread(sb,dev,block,...) would be equivalent to&n;   bread(dev,block,BLOCK_SIZE)&n;   if the block size were always 1024, which is the only one bread() supports.&n;*/
r_static
r_inline
r_struct
id|buffer_head
op_star
DECL|function|sysv_bread
id|sysv_bread
(paren
r_struct
id|super_block
...
multiline_comment|/* locks - protect against simultaneous write and truncate */
r_extern
r_void
...
suffix:semicolon
DECL|function|coh_wait_on_inode
r_extern
r_inline
r_void
id|coh_wait_on_inode
(paren
r_struct
id|inode
op_star
...
DECL|function|coh_unlock_inode
r_extern
r_inline
r_void
id|coh_unlock_inode
