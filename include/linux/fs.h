...
multiline_comment|/* public flags for i_status */
DECL|macro|ST_MODIFIED
mdefine_line|#define ST_MODIFIED 1024
...
multiline_comment|/*&n; * Public flags for namei()&n; */
DECL|macro|NAM_PLAIN
mdefine_line|#define NAM_PLAIN&t;&t;0 /* Retrieve last component of pathname as is. */
DECL|macro|NAM_FOLLOW_LINK
mdefine_line|#define NAM_FOLLOW_LINK&t;&t;2 /* If last component of path is a symlink, follow it */
DECL|macro|NAM_FOLLOW_TRAILSLASH
mdefine_line|#define NAM_FOLLOW_TRAILSLASH&t;4 /* Follow last symlink only if trailed by slash. */
...
suffix:semicolon
DECL|member|i_lru_next
r_struct
id|inode
op_star
id|i_lru_next
suffix:semicolon
DECL|member|i_lru_prev
r_struct
id|inode
op_star
id|i_lru_prev
suffix:semicolon
DECL|member|i_basket_next
r_struct
id|inode
op_star
id|i_basket_next
suffix:semicolon
DECL|member|i_basket_prev
r_struct
id|inode
op_star
id|i_basket_prev
...
id|i_dentry
suffix:semicolon
DECL|member|i_ddir_count
r_int
id|i_ddir_count
suffix:semicolon
DECL|member|i_dent_count
r_int
id|i_dent_count
suffix:semicolon
...
multiline_comment|/* Not to be used by ordinary vfs users */
r_extern
r_void
id|_get_inode
c_func
...
suffix:semicolon
multiline_comment|/* This must not be called if the inode is not in use (i.e. given&n; * back with iput(). The atomic inc assumes that the inode is&n; * already in use, and just has to be incremented higher. &n; * Please do not directly manipulate i_count any more.&n; * Use iget, iinc and iput.&n; * You may test i_count for zero if you are aware that it&n; * might change under you.&n; */
DECL|function|iinc
r_extern
r_inline
r_void
...
multiline_comment|/* The same, but the inode may not be in use. This must be called&n; * with vfslock() held, and be asure that the inode argument is&n; * valid (i.e. not out of cache). So the vfs_lock() must span the&n; * retrieval method of the inode.&n; */
DECL|function|iinc_zero
r_extern
r_inline
r_void
id|iinc_zero
...
(paren
op_amp
id|inode-&gt;u.pipe_i.wait
)paren
suffix:semicolon
)brace
multiline_comment|/* It does not matter if somebody re-increments it in between,&n;&t;&t; * only the _last_ user needs to call _iput().&n;&t;&t; */
r_if
c_cond
(paren
