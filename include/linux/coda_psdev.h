macro_line|#ifndef __CODA_PSDEV_H
DECL|macro|__CODA_PSDEV_H
mdefine_line|#define __CODA_PSDEV_H
DECL|macro|CODA_PSDEV_MAJOR
mdefine_line|#define CODA_PSDEV_MAJOR 67
DECL|macro|MAX_CODADEVS
mdefine_line|#define MAX_CODADEVS  5&t;   /* how many do we allow */
r_extern
r_struct
id|vcomm
id|psdev_vcomm
(braket
)braket
suffix:semicolon
DECL|macro|CODA_SUPER_MAGIC
mdefine_line|#define CODA_SUPER_MAGIC&t;0x73757245
DECL|struct|coda_sb_info
r_struct
id|coda_sb_info
(brace
DECL|member|sbi_psdev
r_struct
id|inode
op_star
id|sbi_psdev
suffix:semicolon
multiline_comment|/* /dev/cfs? Venus/kernel device */
DECL|member|sbi_ctlcp
r_struct
id|inode
op_star
id|sbi_ctlcp
suffix:semicolon
multiline_comment|/* control magic file */
DECL|member|sbi_refct
r_int
id|sbi_refct
suffix:semicolon
DECL|member|sbi_vcomm
r_struct
id|vcomm
op_star
id|sbi_vcomm
suffix:semicolon
DECL|member|sbi_root
r_struct
id|inode
op_star
id|sbi_root
suffix:semicolon
)brace
suffix:semicolon
DECL|function|coda_sbp
r_static
r_inline
r_struct
id|coda_sb_info
op_star
id|coda_sbp
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_return
(paren
(paren
r_struct
id|coda_sb_info
op_star
)paren
(paren
(paren
id|sb
)paren
op_member_access_from_pointer
id|u.generic_sbp
)paren
)paren
suffix:semicolon
)brace
r_extern
r_void
id|coda_psdev_detach
c_func
(paren
r_int
id|unit
)paren
suffix:semicolon
r_extern
r_int
id|init_coda_psdev
c_func
(paren
r_void
)paren
suffix:semicolon
multiline_comment|/* to aid procedures make upcalls. They must have a &n;   declaration at the top containing:&n;       struct inputArgs *inp;&n;&t;struct outputArgs *outp;&n;&t;int error=0;&n;&t;int size;&n;*/
DECL|macro|UPARG
mdefine_line|#define UPARG(bsize, op)&bslash;&n;do {&bslash;&n;  &t;CODA_ALLOC(inp, struct inputArgs *, (bsize));&bslash;&n;&t;outp = (struct outputArgs *) (inp);&bslash;&n;&t;INIT_IN(inp, (op))&bslash;&n;&t;coda_load_creds(&amp;(inp-&gt;cred));&bslash;&n;        size = (bsize);&bslash;&n;} while (0)
multiline_comment|/* upcalls */
r_int
id|venus_rootfid
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
id|ViceFid
op_star
id|fidp
)paren
suffix:semicolon
r_int
id|venus_getattr
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_struct
id|coda_vattr
op_star
id|attr
)paren
suffix:semicolon
r_int
id|venus_setattr
c_func
(paren
r_struct
id|super_block
op_star
comma
r_struct
id|ViceFid
op_star
comma
r_struct
id|coda_vattr
op_star
)paren
suffix:semicolon
r_int
id|venus_lookup
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|length
comma
r_int
op_star
id|type
comma
r_struct
id|ViceFid
op_star
id|resfid
)paren
suffix:semicolon
r_int
id|venus_release
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_int
id|flags
)paren
suffix:semicolon
r_int
id|venus_open
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_int
id|flags
comma
id|ino_t
op_star
id|ino
comma
id|dev_t
op_star
id|dev
)paren
suffix:semicolon
r_int
id|venus_mkdir
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|dirfid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|length
comma
r_struct
id|ViceFid
op_star
id|newfid
comma
r_struct
id|coda_vattr
op_star
id|attrs
)paren
suffix:semicolon
r_int
id|venus_create
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|dirfid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|length
comma
r_int
id|excl
comma
r_int
id|mode
comma
r_struct
id|ViceFid
op_star
id|newfid
comma
r_struct
id|coda_vattr
op_star
id|attrs
)paren
suffix:semicolon
r_int
id|venus_rmdir
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|dirfid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|length
)paren
suffix:semicolon
r_int
id|venus_remove
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|dirfid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|length
)paren
suffix:semicolon
r_int
id|venus_readlink
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_char
op_star
id|buffer
comma
r_int
op_star
id|length
)paren
suffix:semicolon
r_int
id|venus_rename
c_func
(paren
r_struct
id|super_block
op_star
comma
r_struct
id|ViceFid
op_star
id|new_fid
comma
r_struct
id|ViceFid
op_star
id|old_fid
comma
r_int
id|old_length
comma
r_int
id|new_length
comma
r_const
r_char
op_star
id|old_name
comma
r_const
r_char
op_star
id|new_name
)paren
suffix:semicolon
r_int
id|venus_link
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_struct
id|ViceFid
op_star
id|dirfid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
)paren
suffix:semicolon
r_int
id|venus_symlink
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_const
r_char
op_star
id|name
comma
r_int
id|len
comma
r_const
r_char
op_star
id|symname
comma
r_int
id|symlen
)paren
suffix:semicolon
r_int
id|venus_access
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_int
id|mask
)paren
suffix:semicolon
r_int
id|venus_pioctl
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ViceFid
op_star
id|fid
comma
r_int
r_int
id|cmd
comma
r_struct
id|PioctlData
op_star
id|data
)paren
suffix:semicolon
r_int
id|coda_downcall
c_func
(paren
r_int
id|opcode
comma
r_struct
id|outputArgs
op_star
id|out
)paren
suffix:semicolon
r_int
id|coda_upcall
c_func
(paren
r_struct
id|coda_sb_info
op_star
id|mntinfo
comma
r_int
id|inSize
comma
r_int
op_star
id|outSize
comma
r_struct
id|inputArgs
op_star
id|buffer
)paren
suffix:semicolon
multiline_comment|/* messages between coda filesystem in kernel and Venus */
DECL|struct|vmsg
r_struct
id|vmsg
(brace
DECL|member|vm_chain
r_struct
id|queue
id|vm_chain
suffix:semicolon
DECL|member|vm_data
id|caddr_t
id|vm_data
suffix:semicolon
DECL|member|vm_flags
id|u_short
id|vm_flags
suffix:semicolon
DECL|member|vm_inSize
id|u_short
id|vm_inSize
suffix:semicolon
multiline_comment|/* Size is at most 5000 bytes */
DECL|member|vm_outSize
id|u_short
id|vm_outSize
suffix:semicolon
DECL|member|vm_opcode
id|u_short
id|vm_opcode
suffix:semicolon
multiline_comment|/* copied from data to save lookup */
DECL|member|vm_unique
r_int
id|vm_unique
suffix:semicolon
DECL|member|vm_sleep
r_struct
id|wait_queue
op_star
id|vm_sleep
suffix:semicolon
multiline_comment|/* process&squot; wait queue */
)brace
suffix:semicolon
multiline_comment|/* communication pending/processing queues queues */
DECL|struct|vcomm
r_struct
id|vcomm
(brace
DECL|member|vc_seq
id|u_long
id|vc_seq
suffix:semicolon
DECL|member|vc_waitq
r_struct
id|wait_queue
op_star
id|vc_waitq
suffix:semicolon
multiline_comment|/* Venus wait queue */
DECL|member|vc_pending
r_struct
id|queue
id|vc_pending
suffix:semicolon
DECL|member|vc_processing
r_struct
id|queue
id|vc_processing
suffix:semicolon
)brace
suffix:semicolon
DECL|function|vcomm_open
r_static
r_inline
r_int
id|vcomm_open
c_func
(paren
r_struct
id|vcomm
op_star
id|vcp
)paren
(brace
r_return
(paren
(paren
id|vcp
)paren
op_member_access_from_pointer
id|vc_pending.forw
op_ne
l_int|NULL
)paren
suffix:semicolon
)brace
DECL|function|mark_vcomm_closed
r_static
r_inline
r_void
id|mark_vcomm_closed
c_func
(paren
r_struct
id|vcomm
op_star
id|vcp
)paren
(brace
(paren
id|vcp
)paren
op_member_access_from_pointer
id|vc_pending.forw
op_assign
l_int|NULL
suffix:semicolon
)brace
multiline_comment|/*&n; * Statistics&n; */
DECL|struct|coda_upcallstats
r_struct
id|coda_upcallstats
(brace
DECL|member|ncalls
r_int
id|ncalls
suffix:semicolon
multiline_comment|/* client requests */
DECL|member|nbadcalls
r_int
id|nbadcalls
suffix:semicolon
multiline_comment|/* upcall failures */
DECL|member|reqs
r_int
id|reqs
(braket
id|CFS_NCALLS
)braket
suffix:semicolon
multiline_comment|/* count of each request */
)brace
suffix:semicolon
r_extern
r_struct
id|coda_upcallstats
id|coda_callstats
suffix:semicolon
DECL|function|clstats
r_static
r_inline
r_void
id|clstats
c_func
(paren
r_int
id|opcode
)paren
(brace
id|coda_callstats.ncalls
op_increment
suffix:semicolon
r_if
c_cond
(paren
(paren
l_int|0
op_le
id|opcode
)paren
op_logical_and
(paren
id|opcode
op_le
id|CFS_NCALLS
)paren
)paren
id|coda_callstats.reqs
(braket
id|opcode
)braket
op_increment
suffix:semicolon
r_else
id|printk
c_func
(paren
l_string|&quot;clstats called with bad opcode %d&bslash;n&quot;
comma
id|opcode
)paren
suffix:semicolon
)brace
DECL|function|badclstats
r_static
r_inline
r_void
id|badclstats
c_func
(paren
r_void
)paren
(brace
id|coda_callstats.nbadcalls
op_increment
suffix:semicolon
)brace
macro_line|#endif
eof
