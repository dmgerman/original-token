...
multiline_comment|/*&n; * We cannot use kmalloc for shm_alloc since this restricts the&n; * maximum size of the segments.&n; *&n; * We also cannot use vmalloc, since this uses too much of the vmalloc&n; * space and we run out of this on highend machines.&n; *&n; * So we have to use this complicated indirect scheme to alloc the shm&n; * page tables.&n; *&n; */
macro_line|#ifdef PTE_INIT
DECL|function|init_ptes
r_static
r_inline
r_void
id|init_ptes
...
macro_line|#endif
DECL|macro|PTES_PER_PAGE
mdefine_line|#define PTES_PER_PAGE (PAGE_SIZE/sizeof(pte_t))
DECL|macro|SHM_ENTRY
mdefine_line|#define SHM_ENTRY(shp, index) (shp)-&gt;shm_dir[(index)/PTES_PER_PAGE][(index)%PTES_PER_PAGE]
...
id|size_out
suffix:semicolon
multiline_comment|/* Now we set them to the real values */
id|old_dir
op_assign
id|shp-&gt;shm_dir
suffix:semicolon
...
multiline_comment|/* FIXME: maybe we need lock_kernel() here */
...
l_int|1
suffix:semicolon
multiline_comment|/* correct the /dev/zero hack */
id|shm_info.shm_rss
op_assign
id|shm_rss
suffix:semicolon
...
multiline_comment|/*&n; * Enter the shm page into the SHM data structures.&n; *&n; * This turns the physical page into a swap cache entry.&n; */
...
suffix:semicolon
multiline_comment|/* Add it to the shm data structures */
id|swap_duplicate
c_func
(paren
id|entry
...
suffix:semicolon
multiline_comment|/* swap-cache and SHM_ENTRY */
id|shp
op_assign
id|shm_lock
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t; * We had one extra page count for the SHM_ENTRY.&n;&t; * We just overwrote it, so we should free that&n;&t; * count too (the VM layer will do an additional&n;&t; * free that free&squot;s the page table count that&n;&t; * it got rid of itself).&n;&t; */
id|page_cache_free
c_func
(paren
id|page
...
id|repeat
suffix:colon
multiline_comment|/* Do we already have the page in memory? */
id|pte
op_assign
id|SHM_ENTRY
c_func
...
(brace
multiline_comment|/* Yes - just increment the page count */
id|page
op_assign
id|pte_page
c_func
...
multiline_comment|/* No, but maybe we ahve a page cache entry for it? */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Look it up or read it in.. */
id|page
op_assign
...
id|idx
suffix:semicolon
multiline_comment|/* Did somebody else allocate it while we slept? */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* one for the page table, once more for SHM_ENTRY */
r_return
id|page
suffix:semicolon
id|oom
...
op_assign
id|shm_fs_type.kern_mnt-&gt;mnt_sb-&gt;s_dev
suffix:semicolon
