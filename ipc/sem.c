multiline_comment|/*&n; * linux/ipc/sem.c&n; * Copyright (C) 1992 Krishna Balasubramanian&n; * Copyright (C) 1995 Eric Schenk, Bruno Haible&n; *&n; * IMPLEMENTATION NOTES ON CODE REWRITE (Eric Schenk, January 1995):&n; * This code underwent a massive rewrite in order to solve some problems&n; * with the original code. In particular the original code failed to&n; * wake up processes that were waiting for semval to go to 0 if the&n; * value went to 0 and was then incremented rapidly enough. In solving&n; * this problem I have also modified the implementation so that it&n; * processes pending operations in a FIFO manner, thus give a guarentee&n; * that processes waiting for a lock on the semaphore won&squot;t starve&n; * unless another locking process fails to unlock.&n; * In addition the following two changes in behavior have been introduced:&n; * - The original implementation of semop returned the value&n; *   last semaphore element examined on success. This does not&n; *   match the manual page specifications, and effectively&n; *   allows the user to read the semaphore even if they do not&n; *   have read permissions. The implementation now returns 0&n; *   on success as stated in the manual page.&n; * - There is some confusion over whether the set of undo adjustments&n; *   to be peformed at exit should be done in an atomic manner.&n; *   That is, if we are attempting to decrement the semval should we queue&n; *   up and wait until we can do so legally?&n; *   The original implementation attempted to do this.&n; *   The current implementation does not do so. This is because I don&squot;t&n; *   think it is the right thing (TM) to do, and because I couldn&squot;t&n; *   see a clean way to get the old behavior with the new design.&n; *   The POSIX standard and SVID should be consulted to determine&n; *   what behavior is mandated.&n; */
...
DECL|function|insert_into_queue
r_static
r_inline
r_void
id|insert_into_queue
(paren
r_struct
id|semid_ds
op_star
...
multiline_comment|/* Determine whether a sequence of semaphore operations would succeed&n; * all at once. Return 0 if yes, 1 if need to sleep, else return error code.&n; */
DECL|function|try_semop
r_static
r_int
id|try_semop
(paren
...
multiline_comment|/* Actually perform a sequence of semaphore operations. Atomically. */
multiline_comment|/* This assumes that try_semop() already returned 0. */
DECL|function|do_semop
r_static
r_int
id|do_semop
(paren
...
c_func
(paren
l_string|&quot;do_semop: race&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
...
id|CURRENT_TIME
suffix:semicolon
multiline_comment|/* Previous implementation returned the last semaphore&squot;s semval.&n;&t; * This is wrong because we may not have checked read permission,&n;&t; * only write permission.&n;&t; */
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Go through the pending queue for the indicated semaphore&n; * looking for tasks that can be completed. Keep cycling through&n; * the queue until a pass is made in which no process is woken up.&n; */
...
r_continue
suffix:semicolon
multiline_comment|/* Perform the operations the sleeper was waiting for */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* the operations go through immediately */
id|error
op_assign
id|do_semop
c_func
...
r_else
(brace
multiline_comment|/* We need to sleep on this operation, so we put the current&n;&t;&t; * task into the pending queue and go to sleep.&n;&t;&t; */
r_struct
id|sem_queue
id|queue
suffix:semicolon
...
l_int|NULL
suffix:semicolon
multiline_comment|/* When we wake up, either the operation is finished,&n;&t;&t; * or some kind of error happened.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* operation is finished, update_queue() removed us */
r_return
id|queue.status
suffix:semicolon
)brace
