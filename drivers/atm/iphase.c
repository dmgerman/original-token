...
multiline_comment|/* write all 1&squot;s into the base address register.  &n;&t;   read the register whic returns us 0&squot;s in the don&squot;t care bits.  &n;&t;   size is calculated as ~(don&squot;t cre bits) + 1 */
r_if
c_cond
(paren
id|pci_write_config_dword
...
multiline_comment|/* strip flags (last 4 bits )  ---&gt; mask with 0xfffffff0 */
id|real_base
op_and_assign
id|MEM_VALID
suffix:semicolon
multiline_comment|/* enabling the responses in memory space */
id|command
op_or_assign
(paren
...
(paren
id|DEV_LABEL
l_string|&quot;(itf %d): can&squot;t enable memory (0x%x)&bslash;n&quot;
comma
id|dev-&gt;number
comma
id|error
