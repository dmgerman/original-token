...
multiline_comment|/* The device is based on the FTDI FT8U100AX chip, DB25 on one side, USB on the other */
multiline_comment|/* Thanx to FTDI for so kindly providing details of the protocol required */
multiline_comment|/* http://www.ftdi.co.uk  */
multiline_comment|/* The implementation of the device I have is called a USC-1000 */
multiline_comment|/*  which is available from http://www.dse.co.nz - cat no XH4214 */
multiline_comment|/* It looks similar to this: http://www.dansdata.com/usbser.htm but I can&squot;t be sure */
multiline_comment|/* There are other USC-1000s which don&squot;t look like my device though so beware */
multiline_comment|/* Definitions for the FTDI USB Single Port Serial Converter */
multiline_comment|/* known as FTDI_SIO (Serial Input/Output application of the chipset) */
DECL|macro|FTDI_VID
...
multiline_comment|/* The device this is tested with one has one port */
DECL|macro|PIT_SIOB
...
multiline_comment|/* &n;   BmRequestType:  0100 0000B&n;   bRequest:       FTDI_SIO_RESET&n;   wValue:         Control Value &n;                         0 = Reset SIO&n;                         1 = Purge RX buffer&n;                         2 = Purge TX buffer&n;   wIndex:         Port&n;   wLength:        0&n;   Data:           None&n;&n;   */
...
multiline_comment|/*    BmRequestType:  0100 0000B */
multiline_comment|/*    bRequest:       FTDI_SIO_SET_DATA */
multiline_comment|/*    wValue:         Data characteristics (see below) */
multiline_comment|/*    wIndex:         Port */
multiline_comment|/*    wLength:        0 */
multiline_comment|/*    Data:           None */
multiline_comment|/* &n;  Data characteristics&n;&n;B0..7   Number of data bits&n;B8..10  Parity&n;           0 = None&n;           1 = Odd&n;           2 = Even&n;           3 = Mark&n;           4 = Space&n; B11..13 Stop Bits&n;           0 = 1&n;           1 = 1.5&n;           2 = 2&n; B14..15 Reserved&n;*/
...
multiline_comment|/* &n;  BmRequestType:   0100 0000B&n;  bRequest:        FTDI_SIO_MODEM_CTRL&n;  wValue:          ControlValue (see below)&n;  wIndex:          Port&n;  wLength:         0&n;  Data:            None&n;&n;  NOTE: If the device is in RTS/CTS flow control, the RTS set by this&n;        command will be IGNORED without an error being returned&n;*/
...
multiline_comment|/* ControlValue&n;   B0    DTR state&n;           0 = reset&n;           1 = set&n;   B1    RTS state&n;           0 = reset&n;           1 = set&n;   B2..7 Reserved&n;   B8    DTR state enable&n;           0 = ignore&n;           1 = use DTR state&n;   B9    RTS state enable&n;           0 = ignore&n;           1 = use RTS state&n;   B10..15 Reserved&n;*/
...
mdefine_line|#define FTDI_SIO_RTS_CTS_HS 0x1
DECL|macro|FTDI_SIO_DTR_DSR_HS
mdefine_line|#define FTDI_SIO_DTR_DSR_HS 0x2
DECL|macro|FTDI_SIO_XON_XOFF_HS
mdefine_line|#define FTDI_SIO_XON_XOFF_HS 0x4
multiline_comment|/* &n;   BmRequestType:  0100 0000b&n;   bRequest:       FTDI_SIO_SET_FLOW_CTRL&n;   wValue:         Xoff/Xon&n;   wIndex:         Protocol/Port - hIndex is protocl / lIndex is port&n;   wLength:        0 &n;   Data:           None&n;&n;hIndex - protocol has&n; B0 Output handshaking using RTS/CTS&n;      0 = disabled&n;      1 = enabled&n; B1 Output handshaking using DTR/DSR&n;      0 = disabled&n;      1 = enabled&n; B2 Xon/Xoff handshaking&n;      0 = disabled&n;      1 = enabled&n;&n;A value of zero in the hIndex field selects no handshaking&n;&n;If Xon/Xoff handshaking is specified, the hValue field contains the Xoff character &n;and the lValue field contains the Xon character.&n;&n;*/
multiline_comment|/* FTDI_SIO_SET_EVENT_CHAR */
multiline_comment|/* Set the special event character for the specified communications port */
multiline_comment|/* If the device sees this character it will immediately return the */
multiline_comment|/* data read so far - rather than wait 40ms or until 62 bytes is read */
