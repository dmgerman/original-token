...
macro_line|#if&t;defined(CONFIG_KMOD) &amp;&amp; defined(CONFIG_HOTPLUG)
...
multiline_comment|/* waitpid() call glue uses this */
DECL|variable|errno
r_static
r_int
id|errno
...
multiline_comment|/*&n; * This entrypoint gets called for each new device.&n; *&n; * We now walk the list of registered USB drivers,&n; * looking for one that will accept this interface.&n; *&n; * The probe return value is changed to be a private pointer.  This way&n; * the drivers don&squot;t have to dig around in our structures to set the&n; * private pointer if they only need one interface. &n; *&n; * Returns: 0 if a driver accepted the interface, -1 otherwise&n; */
...
macro_line|#if&t;defined(CONFIG_KMOD) &amp;&amp; defined(CONFIG_HOTPLUG)
multiline_comment|/*&n; * USB hotplugging invokes what /proc/sys/kernel/hotplug says&n; * (normally /sbin/hotplug) when USB devices get added or removed.&n; */
...
id|err
(paren
l_string|&quot;failed fork of %s, errno = %d&quot;
comma
id|argv
(braket
l_int|0
...
multiline_comment|/* set signal mask? */
id|fs
op_assign
id|get_fs
(paren
...
suffix:semicolon
multiline_comment|/* retval is in kernel space. */
id|pid2
op_assign
id|waitpid
(paren
...
suffix:semicolon
multiline_comment|/* &quot;errno&quot; gets assigned */
id|set_fs
(paren
id|fs
)paren
...
id|err
(paren
l_string|&quot;waitpid(%d) failed, returned %d&bslash;n&quot;
comma
id|pid
comma
id|pid2
...
id|temp
suffix:semicolon
multiline_comment|/* digits are 0-9 then &quot;:;&lt;=&gt;?&quot; for devices using&n;&t; * non-bcd (non-standard!) values here ... */
multiline_comment|/* No leading (or later, trailing) zeroes since scripts do&n;&t; * literal matches, and that&squot;s how they&squot;re doing them. */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* If this is available, userspace programs can directly read&n;&t; * all the device descriptors we don&squot;t tell them about.  Or&n;&t; * even act as usermode drivers.&n;&t; *&n;&t; * XXX how little intelligence can we hardwire?&n;&t; * (a) mount point: /devfs, /dev, /proc/bus/usb etc.&n;&t; * (b) naming convention: bus1/device3, 001/003 etc.&n;&t; */
id|envp
(braket
id|i
op_increment
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* per-device configuration hacks are often necessary */
id|envp
(braket
id|i
op_increment
...
l_int|1
suffix:semicolon
multiline_comment|/* otherwise, use a simple (so far) generic driver binding model */
id|envp
(braket
id|i
op_increment
...
id|act_altsetting
suffix:semicolon
multiline_comment|/* simple/common case: one config, one interface, one driver&n;&t;&t; * unsimple cases:  everything else&n;&t;&t; */
id|scratch
op_add_assign
id|sprintf
(paren
...
r_else
(brace
multiline_comment|/* simple/common case: generic device, handled generically */
id|scratch
op_add_assign
id|sprintf
(paren
...
macro_line|#endif&t;/* KMOD &amp;&amp; HOTPLUG */
