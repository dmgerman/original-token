...
multiline_comment|/*-------------------------------------------------------------------*&n; * completion handler for compatibility wrappers (async bulk)        *&n; *-------------------------------------------------------------------*/
DECL|function|usb_api_async_completion
r_static
r_void
id|usb_api_async_completion
c_func
(paren
id|urb_t
op_star
...
singleline_comment|// compatibility wrapper. Remove urb only if it is called before the
singleline_comment|// transaction&squot;s completion interrupt. If called from within the
singleline_comment|// completion handler (urb-&gt;completed==1), it does nothing, since the
singleline_comment|// qh is already removed 
...
id|EUSERS
suffix:semicolon
singleline_comment|// no bandwidth left
r_if
c_cond
(paren
op_logical_neg
...
id|ENOMEM
suffix:semicolon
multiline_comment|/*&n;&t; * is this two step process necessary? can&squot;t we just&n;&t; * ask for a maximum length string and then take the length&n;&t; * that was returned?&n;&t; */
id|err
op_assign
id|usb_get_string
c_func
...
multiline_comment|/* get langid for strings */
id|buf
op_assign
id|kmalloc
c_func
...
c_func
(paren
l_string|&quot;string descriptpr 0 too short&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_else
