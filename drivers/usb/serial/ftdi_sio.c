...
suffix:semicolon
multiline_comment|/* Was seeing a race here, got a read callback, then write callback before&n;&t;&t;   hitting interuuptible_sleep_on  - so wrapping in add_wait_queue stuff */
id|add_wait_queue
c_func
(paren
op_amp
...
multiline_comment|/* ftdi_sio_set_termios */
multiline_comment|/*&n; * set_termios - Taken from tty_ioctl.c with some mods &n; *&n; *   Used to correctly handle various options (ie TERMIOS_FLUSH etc)&n; *&n; */
...
multiline_comment|/* &n; * set_termios - assumes old_termios is in kernel space not user space &n; *               sets current to port-&gt;tty-&gt;termios, might check diffs against old_termios one day&n; */
...
id|retval
suffix:semicolon
macro_line|#ifdef  NOT_IMPLEMENTED_YET
id|retval
op_assign
id|tty_check_change
c_func
...
multiline_comment|/* remember port-&gt;tty-&gt;termios has the new settings */
id|ftdi_sio_set_termios
c_func
(paren
id|port
...
r_break
suffix:semicolon
multiline_comment|/* Code for the following is based on code from tty_ioctl.c */
r_case
id|TCGETS
suffix:colon
r_if
...
suffix:semicolon
macro_line|#ifdef NOT_COMPLETED_YET_NEED_TO_CONVERT_TERMIO_TO_TERMIOS
multiline_comment|/* the &squot;A&squot; flavours work on termio not termios */
r_case
id|TCGETA
suffix:colon
r_if
