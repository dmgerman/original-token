...
id|PIPE_CONTROL
suffix:colon
multiline_comment|/* short packet error?, unlink URB, call complete */
id|urb_rm_priv
(paren
id|purb
)paren
...
id|USB_ST_URB_PENDING
suffix:semicolon
singleline_comment|// queue_urb(s, &amp;purb-&gt;urb_list);
r_return
l_int|0
suffix:semicolon
)brace
...
multiline_comment|/*-------------------------------------------------------------------------*/
multiline_comment|/* there are some pending requests to remove &n; * - some of the eds (if ed-&gt;state &amp; ED_DEL (set by sohci_free_dev)&n; * - some URBs/TDs if purb_priv-&gt;state == URB_DEL */
...
id|td-&gt;next_dl_td
suffix:semicolon
multiline_comment|/* td may be freed by urb_rm_priv */
r_if
c_cond
(paren
(paren
...
multiline_comment|/*-------------------------------------------------------------------------*/
DECL|macro|OK
mdefine_line|#define OK(x) &t;&t;&t;&t;len = (x); req_reply = 0; break
