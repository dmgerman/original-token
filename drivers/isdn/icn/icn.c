multiline_comment|/* $Id icn.c,v 1.15 1996/01/10 20:57:39 fritz Exp fritz $&n; *&n; * ISDN low-level module for the ICN active ISDN-Card.&n; *&n; * Copyright 1994,95,96 by Fritz Elfert (fritz@wuemaus.franken.de)&n; *&n; * This program is free software; you can redistribute it and/or modify&n; * it under the terms of the GNU General Public License as published by&n; * the Free Software Foundation; either version 2, or (at your option)&n; * any later version.&n; *&n; * This program is distributed in the hope that it will be useful,&n; * but WITHOUT ANY WARRANTY; without even the implied warranty of&n; * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the&n; * GNU General Public License for more details.&n; *&n; * You should have received a copy of the GNU General Public License&n; * along with this program; if not, write to the Free Software&n; * Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA. &n; *&n; * $Log: icn.c,v $&n; * Revision 1.17  1996/02/11 02:39:04  fritz&n; * Increased Buffer for status-messages.&n; * Removed conditionals for HDLC-firmware.&n; *&n; * Revision 1.16  1996/01/22 05:01:55  fritz&n; * Revert to GPL.&n; *&n; * Revision 1.15  1996/01/10 20:57:39  fritz&n; * Bugfix: Loading firmware twice caused the device stop working.&n; *&n; * Revision 1.14  1995/12/18  18:23:37  fritz&n; * Support for ICN-2B Cards.&n; * Change for supporting user-settable service-octet.&n; *&n; * Revision 1.13  1995/10/29  21:41:07  fritz&n; * Added support for DriverId&squot;s, added Jan&squot;s patches for Kernel versions.&n; *&n; * Revision 1.12  1995/04/29  13:07:35  fritz&n; * Added support for new Euro-ISDN-firmware&n; *&n; * Revision 1.11  1995/04/23  13:40:45  fritz&n; * Added support for SPV&squot;s.&n; * Changed Dial-Command to support MSN&squot;s on DSS1-Lines.&n; *&n; * Revision 1.10  1995/03/25  23:23:24  fritz&n; * Changed configurable Ports, to allow settings for DIP-Switch Cardversions.&n; *&n; * Revision 1.9  1995/03/25  23:17:30  fritz&n; * Fixed race-condition in pollbchan_send&n; *&n; * Revision 1.8  1995/03/15  12:49:44  fritz&n; * Added support for SPV&squot;s&n; * Split pollbchan_work ifor calling send-routine directly&n; *&n; * Revision 1.7  1995/02/20  03:48:03  fritz&n; * Added support of new request_region-function.&n; * Minor bugfixes.&n; *&n; * Revision 1.6  1995/01/31  15:48:45  fritz&n; * Added Cause-Messages to be signaled to upper layers.&n; * Added Revision-Info on load.&n; *&n; * Revision 1.5  1995/01/29  23:34:59  fritz&n; * Added stopdriver() and appropriate calls.&n; * Changed printk-statements to support loglevels.&n; *&n; * Revision 1.4  1995/01/09  07:40:46  fritz&n; * Added GPL-Notice&n; *&n; * Revision 1.3  1995/01/04  05:15:18  fritz&n; * Added undocumented &quot;bootload-finished&quot;-command in download-code&n; * to satisfy some brain-damaged icn card-versions.&n; *&n; * Revision 1.2  1995/01/02  02:14:45  fritz&n; * Misc Bugfixes&n; *&n; * Revision 1.1  1994/12/14  17:56:06  fritz&n; * Initial revision&n; *&n; */
macro_line|#include &quot;icn.h&quot;
multiline_comment|/*&n; * Verbose bootcode- and protocol-downloading.&n; */
DECL|macro|BOOT_DEBUG
macro_line|#undef BOOT_DEBUG
multiline_comment|/*&n; * Verbose Shmem-Mapping.&n; */
DECL|macro|MAP_DEBUG
macro_line|#undef MAP_DEBUG
multiline_comment|/* If defined, no bootcode- and protocol-downloading is supported and&n; * you must use an external loader&n; */
DECL|macro|LOADEXTERN
macro_line|#undef LOADEXTERN
r_static
r_char
DECL|variable|revision
op_star
id|revision
op_assign
l_string|&quot;$Revision: 1.17 $&quot;
suffix:semicolon
r_static
r_void
id|icn_pollcard
c_func
(paren
r_int
r_int
id|dummy
)paren
suffix:semicolon
multiline_comment|/* Try to allocate a new buffer, link it into queue. */
r_static
id|u_char
op_star
DECL|function|icn_new_buf
id|icn_new_buf
c_func
(paren
id|pqueue
op_star
op_star
id|queue
comma
r_int
id|length
)paren
(brace
id|pqueue
op_star
id|p
suffix:semicolon
id|pqueue
op_star
id|q
suffix:semicolon
r_if
c_cond
(paren
(paren
id|p
op_assign
op_star
id|queue
)paren
)paren
(brace
r_while
c_loop
(paren
id|p
)paren
(brace
id|q
op_assign
id|p
suffix:semicolon
id|p
op_assign
(paren
id|pqueue
op_star
)paren
id|p-&gt;next
suffix:semicolon
)brace
id|p
op_assign
(paren
id|pqueue
op_star
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
id|pqueue
)paren
op_plus
id|length
comma
id|GFP_ATOMIC
)paren
suffix:semicolon
id|q-&gt;next
op_assign
(paren
id|u_char
op_star
)paren
id|p
suffix:semicolon
)brace
r_else
id|p
op_assign
op_star
id|queue
op_assign
(paren
id|pqueue
op_star
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
id|pqueue
)paren
op_plus
id|length
comma
id|GFP_ATOMIC
)paren
suffix:semicolon
r_if
c_cond
(paren
id|p
)paren
(brace
id|p-&gt;size
op_assign
r_sizeof
(paren
id|pqueue
)paren
op_plus
id|length
suffix:semicolon
id|p-&gt;length
op_assign
id|length
suffix:semicolon
id|p-&gt;next
op_assign
l_int|NULL
suffix:semicolon
id|p-&gt;rptr
op_assign
id|p-&gt;buffer
suffix:semicolon
r_return
id|p-&gt;buffer
suffix:semicolon
)brace
r_else
(brace
r_return
(paren
id|u_char
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
)brace
macro_line|#ifdef MODULE
DECL|function|icn_free_queue
r_static
r_void
id|icn_free_queue
c_func
(paren
id|pqueue
op_star
op_star
id|queue
)paren
(brace
id|pqueue
op_star
id|p
suffix:semicolon
id|pqueue
op_star
id|q
suffix:semicolon
id|p
op_assign
op_star
id|queue
suffix:semicolon
r_while
c_loop
(paren
id|p
)paren
(brace
id|q
op_assign
id|p
suffix:semicolon
id|p
op_assign
(paren
id|pqueue
op_star
)paren
id|p-&gt;next
suffix:semicolon
id|kfree_s
c_func
(paren
id|q
comma
id|q-&gt;size
)paren
suffix:semicolon
)brace
op_star
id|queue
op_assign
(paren
id|pqueue
op_star
)paren
l_int|0
suffix:semicolon
)brace
macro_line|#endif
multiline_comment|/* Put a value into a shift-register, highest bit first.&n; * Parameters:&n; *            port     = port for output (bit 0 is significant)&n; *            val      = value to be output&n; *            firstbit = Bit-Number of highest bit&n; *            bitcount = Number of bits to output&n; */
DECL|function|icn_shiftout
r_static
r_inline
r_void
id|icn_shiftout
c_func
(paren
r_int
r_int
id|port
comma
r_int
r_int
id|val
comma
r_int
id|firstbit
comma
r_int
id|bitcount
)paren
(brace
r_register
id|u_char
id|s
suffix:semicolon
r_register
id|u_char
id|c
suffix:semicolon
r_for
c_loop
(paren
id|s
op_assign
id|firstbit
comma
id|c
op_assign
id|bitcount
suffix:semicolon
id|c
OG
l_int|0
suffix:semicolon
id|s
op_decrement
comma
id|c
op_decrement
)paren
id|OUTB_P
c_func
(paren
(paren
id|u_char
)paren
(paren
(paren
id|val
op_rshift
id|s
)paren
op_amp
l_int|1
)paren
ques
c_cond
l_int|0xff
suffix:colon
l_int|0
comma
id|port
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Map Cannel0 (Bank0/Bank8) or Channel1 (Bank4/Bank12)&n; */
DECL|function|icn_map_channel
r_static
r_inline
r_void
id|icn_map_channel
c_func
(paren
r_int
id|channel
)paren
(brace
r_static
id|u_char
id|chan2bank
(braket
)braket
op_assign
(brace
l_int|0
comma
l_int|4
comma
l_int|8
comma
l_int|12
)brace
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_map_channel %d %d&bslash;n&quot;
comma
id|dev-&gt;channel
comma
id|channel
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|channel
op_eq
id|dev-&gt;channel
)paren
r_return
suffix:semicolon
id|OUTB_P
c_func
(paren
l_int|0
comma
id|ICN_MAPRAM
)paren
suffix:semicolon
multiline_comment|/* Disable RAM          */
id|icn_shiftout
c_func
(paren
id|ICN_BANK
comma
id|chan2bank
(braket
id|channel
)braket
comma
l_int|3
comma
l_int|4
)paren
suffix:semicolon
multiline_comment|/* Select Bank          */
id|OUTB_P
c_func
(paren
l_int|0xff
comma
id|ICN_MAPRAM
)paren
suffix:semicolon
multiline_comment|/* Enable RAM           */
id|dev-&gt;channel
op_assign
id|channel
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_map_channel done&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
)brace
DECL|function|icn_lock_channel
r_static
r_inline
r_int
id|icn_lock_channel
c_func
(paren
r_int
id|channel
)paren
(brace
r_register
r_int
id|retval
suffix:semicolon
id|ulong
id|flags
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_lock_channel %d&bslash;n&quot;
comma
id|channel
)paren
suffix:semicolon
macro_line|#endif
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;channel
op_eq
id|channel
)paren
(brace
id|dev-&gt;chanlock
op_increment
suffix:semicolon
id|retval
op_assign
l_int|1
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_lock_channel %d OK&bslash;n&quot;
comma
id|channel
)paren
suffix:semicolon
macro_line|#endif
)brace
r_else
(brace
id|retval
op_assign
l_int|0
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_lock_channel %d FAILED, dc=%d&bslash;n&quot;
comma
id|channel
comma
id|dev-&gt;channel
)paren
suffix:semicolon
macro_line|#endif
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
id|retval
suffix:semicolon
)brace
DECL|function|icn_release_channel
r_static
r_inline
r_void
id|icn_release_channel
c_func
(paren
r_void
)paren
(brace
id|ulong
id|flags
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_release_channel l=%d&bslash;n&quot;
comma
id|dev-&gt;chanlock
)paren
suffix:semicolon
macro_line|#endif
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;chanlock
)paren
id|dev-&gt;chanlock
op_decrement
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
DECL|function|icn_trymaplock_channel
r_static
r_inline
r_int
id|icn_trymaplock_channel
c_func
(paren
r_int
id|channel
)paren
(brace
id|ulong
id|flags
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;trymaplock c=%d dc=%d l=%d&bslash;n&quot;
comma
id|channel
comma
id|dev-&gt;channel
comma
id|dev-&gt;chanlock
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
(paren
op_logical_neg
id|dev-&gt;chanlock
)paren
op_logical_or
(paren
id|dev-&gt;channel
op_eq
id|channel
)paren
)paren
(brace
id|dev-&gt;chanlock
op_increment
suffix:semicolon
id|icn_map_channel
c_func
(paren
id|channel
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;trymaplock %d OK&bslash;n&quot;
comma
id|channel
)paren
suffix:semicolon
macro_line|#endif
r_return
l_int|1
suffix:semicolon
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;trymaplock %d FAILED&bslash;n&quot;
comma
id|channel
)paren
suffix:semicolon
macro_line|#endif
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|icn_maprelease_channel
r_static
r_inline
r_void
id|icn_maprelease_channel
c_func
(paren
r_int
id|channel
)paren
(brace
id|ulong
id|flags
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef MAP_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;map_release c=%d l=%d&bslash;n&quot;
comma
id|channel
comma
id|dev-&gt;chanlock
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|dev-&gt;chanlock
)paren
id|dev-&gt;chanlock
op_decrement
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev-&gt;chanlock
)paren
id|icn_map_channel
c_func
(paren
id|channel
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
multiline_comment|/* Get Data from the B-Channel, assemble fragmented packets and put them&n; * into receive-queue. Wake up any B-Channel-reading processes.&n; * This routine is called via timer-callback from pollbchan().&n; * It schedules itself while any B-Channel is open.&n; */
macro_line|#ifdef DEBUG_RCVCALLBACK
DECL|variable|max_pending
r_static
r_int
id|max_pending
(braket
l_int|2
)braket
op_assign
(brace
l_int|0
comma
l_int|0
)brace
suffix:semicolon
macro_line|#endif
DECL|function|icn_pollbchan_receive
r_static
r_void
id|icn_pollbchan_receive
c_func
(paren
r_int
id|channel
comma
id|icn_dev
op_star
id|dev
)paren
(brace
r_int
id|mch
op_assign
id|channel
op_plus
(paren
(paren
id|dev-&gt;secondhalf
)paren
ques
c_cond
l_int|2
suffix:colon
l_int|0
)paren
suffix:semicolon
r_int
id|eflag
suffix:semicolon
r_int
id|cnt
suffix:semicolon
r_int
id|flags
suffix:semicolon
macro_line|#ifdef DEBUG_RCVCALLBACK
r_int
id|rcv_pending1
suffix:semicolon
r_int
id|rcv_pending2
suffix:semicolon
r_int
id|akt_pending
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|icn_trymaplock_channel
c_func
(paren
id|mch
)paren
)paren
(brace
r_while
c_loop
(paren
id|rbavl
)paren
(brace
id|cnt
op_assign
id|rbuf_l
suffix:semicolon
r_if
c_cond
(paren
(paren
id|dev-&gt;rcvidx
(braket
id|channel
)braket
op_plus
id|cnt
)paren
OG
l_int|4000
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: bogus packet on ch%d, dropping.&bslash;n&quot;
comma
id|channel
op_plus
l_int|1
)paren
suffix:semicolon
id|dev-&gt;rcvidx
(braket
id|channel
)braket
op_assign
l_int|0
suffix:semicolon
id|eflag
op_assign
l_int|0
suffix:semicolon
)brace
r_else
(brace
id|memcpy
c_func
(paren
op_amp
id|dev-&gt;rcvbuf
(braket
id|channel
)braket
(braket
id|dev-&gt;rcvidx
(braket
id|channel
)braket
)braket
comma
id|rbuf_d
comma
id|cnt
)paren
suffix:semicolon
id|dev-&gt;rcvidx
(braket
id|channel
)braket
op_add_assign
id|cnt
suffix:semicolon
id|eflag
op_assign
id|rbuf_f
suffix:semicolon
)brace
id|rbnext
suffix:semicolon
id|icn_maprelease_channel
c_func
(paren
id|mch
op_amp
l_int|2
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|eflag
)paren
(brace
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef DEBUG_RCVCALLBACK
id|rcv_pending1
op_assign
(paren
id|dev-&gt;shmem-&gt;data_control.ecnr
OG
id|dev-&gt;shmem-&gt;data_control.ecns
)paren
ques
c_cond
l_int|0xf
op_minus
id|dev-&gt;shmem-&gt;data_control.ecnr
op_plus
id|dev-&gt;shmem-&gt;data_control.ecns
suffix:colon
id|dev-&gt;shmem-&gt;data_control.ecns
op_minus
id|dev-&gt;shmem-&gt;data_control.ecnr
suffix:semicolon
macro_line|#endif
id|dev-&gt;interface
dot
id|rcvcallb
c_func
(paren
id|dev-&gt;myid
comma
id|channel
comma
id|dev-&gt;rcvbuf
(braket
id|channel
)braket
comma
id|dev-&gt;rcvidx
(braket
id|channel
)braket
)paren
suffix:semicolon
id|dev-&gt;rcvidx
(braket
id|channel
)braket
op_assign
l_int|0
suffix:semicolon
macro_line|#ifdef DEBUG_RCVCALLBACK
id|rcv_pending2
op_assign
(paren
id|dev-&gt;shmem-&gt;data_control.ecnr
OG
id|dev-&gt;shmem-&gt;data_control.ecns
)paren
ques
c_cond
l_int|0xf
op_minus
id|dev-&gt;shmem-&gt;data_control.ecnr
op_plus
id|dev-&gt;shmem-&gt;data_control.ecns
suffix:colon
id|dev-&gt;shmem-&gt;data_control.ecns
op_minus
id|dev-&gt;shmem-&gt;data_control.ecnr
suffix:semicolon
id|akt_pending
op_assign
id|rcv_pending2
op_minus
id|rcv_pending1
suffix:semicolon
r_if
c_cond
(paren
id|akt_pending
OG
id|max_pending
(braket
id|channel
)braket
)paren
(brace
id|max_pending
(braket
id|channel
)braket
op_assign
id|akt_pending
suffix:semicolon
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;ICN_DEBUG: pend: %d %d&bslash;n&quot;
comma
id|max_pending
(braket
l_int|0
)braket
comma
id|max_pending
(braket
l_int|1
)braket
)paren
suffix:semicolon
)brace
macro_line|#endif
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|icn_trymaplock_channel
c_func
(paren
id|mch
)paren
)paren
r_break
suffix:semicolon
)brace
id|icn_maprelease_channel
c_func
(paren
id|mch
op_amp
l_int|2
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* Send data-packet to B-Channel, split it up into fragments of&n; * ICN_FRAGSIZE length. If last fragment is sent out, signal&n; * success to upper layers via statcallb with ISDN_STAT_BSENT argument.&n; * This routine is called via timer-callback from pollbchan() or&n; * directly from sendbuf().&n; */
DECL|function|icn_pollbchan_send
r_static
r_void
id|icn_pollbchan_send
c_func
(paren
r_int
id|channel
comma
id|icn_dev
op_star
id|dev
)paren
(brace
r_int
id|mch
op_assign
id|channel
op_plus
(paren
(paren
id|dev-&gt;secondhalf
)paren
ques
c_cond
l_int|2
suffix:colon
l_int|0
)paren
suffix:semicolon
r_int
id|eflag
op_assign
l_int|0
suffix:semicolon
r_int
id|cnt
suffix:semicolon
r_int
id|left
suffix:semicolon
r_int
id|flags
suffix:semicolon
id|pqueue
op_star
id|p
suffix:semicolon
id|isdn_ctrl
id|cmd
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev-&gt;sndcount
(braket
id|channel
)braket
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|icn_trymaplock_channel
c_func
(paren
id|mch
)paren
)paren
(brace
r_while
c_loop
(paren
id|sbfree
op_logical_and
id|dev-&gt;sndcount
(braket
id|channel
)braket
)paren
(brace
id|left
op_assign
id|dev-&gt;spqueue
(braket
id|channel
)braket
op_member_access_from_pointer
id|length
suffix:semicolon
id|cnt
op_assign
(paren
id|sbuf_l
op_assign
(paren
id|left
OG
id|ICN_FRAGSIZE
)paren
ques
c_cond
(paren
(paren
id|sbuf_f
op_assign
l_int|0xff
)paren
comma
id|ICN_FRAGSIZE
)paren
suffix:colon
(paren
(paren
id|sbuf_f
op_assign
l_int|0
)paren
comma
id|left
)paren
)paren
suffix:semicolon
id|memcpy
c_func
(paren
id|sbuf_d
comma
id|dev-&gt;spqueue
(braket
id|channel
)braket
op_member_access_from_pointer
id|rptr
comma
id|cnt
)paren
suffix:semicolon
id|sbnext
suffix:semicolon
multiline_comment|/* switch to next buffer        */
id|icn_maprelease_channel
c_func
(paren
id|mch
op_amp
l_int|2
)paren
suffix:semicolon
id|dev-&gt;spqueue
(braket
id|channel
)braket
op_member_access_from_pointer
id|rptr
op_add_assign
id|cnt
suffix:semicolon
id|eflag
op_assign
(paren
(paren
id|dev-&gt;spqueue
(braket
id|channel
)braket
op_member_access_from_pointer
id|length
op_sub_assign
id|cnt
)paren
op_eq
l_int|0
)paren
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|p
op_assign
id|dev-&gt;spqueue
(braket
id|channel
)braket
suffix:semicolon
id|dev-&gt;sndcount
(braket
id|channel
)braket
op_sub_assign
id|cnt
suffix:semicolon
r_if
c_cond
(paren
id|eflag
)paren
id|dev-&gt;spqueue
(braket
id|channel
)braket
op_assign
(paren
id|pqueue
op_star
)paren
id|dev-&gt;spqueue
(braket
id|channel
)braket
op_member_access_from_pointer
id|next
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_if
c_cond
(paren
id|eflag
)paren
(brace
id|kfree_s
c_func
(paren
id|p
comma
id|p-&gt;size
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_BSENT
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|channel
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|icn_trymaplock_channel
c_func
(paren
id|mch
)paren
)paren
r_break
suffix:semicolon
)brace
id|icn_maprelease_channel
c_func
(paren
id|mch
op_amp
l_int|2
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* Send/Receive Data to/from the B-Channel.&n; * This routine is called via timer-callback.&n; * It schedules itself while any B-Channel is open.&n; */
DECL|function|icn_pollbchan
r_static
r_void
id|icn_pollbchan
c_func
(paren
r_int
r_int
id|dummy
)paren
(brace
r_int
r_int
id|flags
suffix:semicolon
id|dev-&gt;flags
op_or_assign
id|ICN_FLAGS_RBTIMER
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;flags
op_amp
id|ICN_FLAGS_B1ACTIVE
)paren
(brace
id|icn_pollbchan_receive
c_func
(paren
l_int|0
comma
id|dev
)paren
suffix:semicolon
id|icn_pollbchan_send
c_func
(paren
l_int|0
comma
id|dev
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev-&gt;flags
op_amp
id|ICN_FLAGS_B2ACTIVE
)paren
(brace
id|icn_pollbchan_receive
c_func
(paren
l_int|1
comma
id|dev
)paren
suffix:semicolon
id|icn_pollbchan_send
c_func
(paren
l_int|1
comma
id|dev
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
r_if
c_cond
(paren
id|dev2-&gt;flags
op_amp
id|ICN_FLAGS_B1ACTIVE
)paren
(brace
id|icn_pollbchan_receive
c_func
(paren
l_int|0
comma
id|dev2
)paren
suffix:semicolon
id|icn_pollbchan_send
c_func
(paren
l_int|0
comma
id|dev2
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev2-&gt;flags
op_amp
id|ICN_FLAGS_B2ACTIVE
)paren
(brace
id|icn_pollbchan_receive
c_func
(paren
l_int|1
comma
id|dev2
)paren
suffix:semicolon
id|icn_pollbchan_send
c_func
(paren
l_int|1
comma
id|dev2
)paren
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|dev-&gt;flags
op_amp
(paren
id|ICN_FLAGS_B1ACTIVE
op_or
id|ICN_FLAGS_B2ACTIVE
)paren
)paren
(brace
multiline_comment|/* schedule b-channel polling again */
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|del_timer
c_func
(paren
op_amp
id|dev-&gt;rb_timer
)paren
suffix:semicolon
id|dev-&gt;rb_timer.function
op_assign
id|icn_pollbchan
suffix:semicolon
id|dev-&gt;rb_timer.expires
op_assign
id|jiffies
op_plus
id|ICN_TIMER_BCREAD
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|dev-&gt;rb_timer
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
r_else
id|dev-&gt;flags
op_and_assign
op_complement
id|ICN_FLAGS_RBTIMER
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
r_if
c_cond
(paren
id|dev2-&gt;flags
op_amp
(paren
id|ICN_FLAGS_B1ACTIVE
op_or
id|ICN_FLAGS_B2ACTIVE
)paren
)paren
(brace
multiline_comment|/* schedule b-channel polling again */
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|del_timer
c_func
(paren
op_amp
id|dev2-&gt;rb_timer
)paren
suffix:semicolon
id|dev2-&gt;rb_timer.function
op_assign
id|icn_pollbchan
suffix:semicolon
id|dev2-&gt;rb_timer.expires
op_assign
id|jiffies
op_plus
id|ICN_TIMER_BCREAD
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|dev2-&gt;rb_timer
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
r_else
id|dev2-&gt;flags
op_and_assign
op_complement
id|ICN_FLAGS_RBTIMER
suffix:semicolon
)brace
)brace
DECL|function|icn_pollit
r_static
r_void
id|icn_pollit
c_func
(paren
id|icn_dev
op_star
id|dev
)paren
(brace
r_int
id|mch
op_assign
id|dev-&gt;secondhalf
ques
c_cond
l_int|2
suffix:colon
l_int|0
suffix:semicolon
r_int
id|avail
op_assign
l_int|0
suffix:semicolon
r_int
id|dflag
op_assign
l_int|0
suffix:semicolon
r_int
id|left
suffix:semicolon
id|u_char
id|c
suffix:semicolon
r_int
id|ch
suffix:semicolon
r_int
id|flags
suffix:semicolon
r_int
id|i
suffix:semicolon
id|u_char
op_star
id|p
suffix:semicolon
id|isdn_ctrl
id|cmd
suffix:semicolon
r_if
c_cond
(paren
id|icn_trymaplock_channel
c_func
(paren
id|mch
)paren
)paren
(brace
id|avail
op_assign
id|msg_avail
suffix:semicolon
r_for
c_loop
(paren
id|left
op_assign
id|avail
comma
id|i
op_assign
id|msg_o
suffix:semicolon
id|left
OG
l_int|0
suffix:semicolon
id|i
op_increment
comma
id|left
op_decrement
)paren
(brace
id|c
op_assign
id|dev-&gt;shmem-&gt;comm_buffers.iopc_buf
(braket
id|i
op_amp
l_int|0xff
)braket
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
op_star
id|dev-&gt;msg_buf_write
op_increment
op_assign
(paren
id|c
op_eq
l_int|0xff
)paren
ques
c_cond
l_char|&squot;&bslash;n&squot;
suffix:colon
id|c
suffix:semicolon
multiline_comment|/* No checks for buffer overflow for raw-status-device */
r_if
c_cond
(paren
id|dev-&gt;msg_buf_write
OG
id|dev-&gt;msg_buf_end
)paren
id|dev-&gt;msg_buf_write
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_if
c_cond
(paren
id|c
op_eq
l_int|0xff
)paren
(brace
id|dev-&gt;imsg
(braket
id|dev-&gt;iptr
)braket
op_assign
l_int|0
suffix:semicolon
id|dev-&gt;iptr
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;imsg
(braket
l_int|0
)braket
op_eq
l_char|&squot;0&squot;
op_logical_and
id|dev-&gt;imsg
(braket
l_int|1
)braket
op_ge
l_char|&squot;0&squot;
op_logical_and
id|dev-&gt;imsg
(braket
l_int|1
)braket
op_le
l_char|&squot;2&squot;
op_logical_and
id|dev-&gt;imsg
(braket
l_int|2
)braket
op_eq
l_char|&squot;;&squot;
)paren
(brace
id|ch
op_assign
id|dev-&gt;imsg
(braket
l_int|1
)braket
op_minus
l_char|&squot;0&squot;
suffix:semicolon
id|p
op_assign
op_amp
id|dev-&gt;imsg
(braket
l_int|3
)braket
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;BCON_&quot;
comma
l_int|5
)paren
)paren
(brace
r_switch
c_cond
(paren
id|ch
)paren
(brace
r_case
l_int|1
suffix:colon
id|dev-&gt;flags
op_or_assign
id|ICN_FLAGS_B1ACTIVE
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|dev-&gt;flags
op_or_assign
id|ICN_FLAGS_B2ACTIVE
suffix:semicolon
r_break
suffix:semicolon
)brace
id|cmd.command
op_assign
id|ISDN_STAT_BCONN
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;TEI OK&quot;
comma
l_int|6
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_RUN
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;BDIS_&quot;
comma
l_int|5
)paren
)paren
(brace
r_switch
c_cond
(paren
id|ch
)paren
(brace
r_case
l_int|1
suffix:colon
id|dev-&gt;flags
op_and_assign
op_complement
id|ICN_FLAGS_B1ACTIVE
suffix:semicolon
id|dflag
op_or_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|dev-&gt;flags
op_and_assign
op_complement
id|ICN_FLAGS_B2ACTIVE
suffix:semicolon
id|dflag
op_or_assign
l_int|2
suffix:semicolon
r_break
suffix:semicolon
)brace
id|cmd.command
op_assign
id|ISDN_STAT_BHUP
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;DCON_&quot;
comma
l_int|5
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_DCONN
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;DDIS_&quot;
comma
l_int|5
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_DHUP
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;E_L1: ACT FAIL&quot;
comma
l_int|14
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_BHUP
suffix:semicolon
id|cmd.arg
op_assign
l_int|0
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_DHUP
suffix:semicolon
id|cmd.arg
op_assign
l_int|0
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;CIF&quot;
comma
l_int|3
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_CINF
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|strncpy
c_func
(paren
id|cmd.num
comma
id|p
op_plus
l_int|3
comma
r_sizeof
(paren
id|cmd.num
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;CAU&quot;
comma
l_int|3
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_CAUSE
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|strncpy
c_func
(paren
id|cmd.num
comma
id|p
op_plus
l_int|3
comma
r_sizeof
(paren
id|cmd.num
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;DCAL_I&quot;
comma
l_int|6
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_ICALL
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|strncpy
c_func
(paren
id|cmd.num
comma
id|p
op_plus
l_int|6
comma
r_sizeof
(paren
id|cmd.num
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;FCALL&quot;
comma
l_int|5
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_ICALL
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|strcpy
c_func
(paren
id|cmd.num
comma
l_string|&quot;LEASED,07,00,1&quot;
)paren
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;DSCA_I&quot;
comma
l_int|6
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_ICALL
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|strncpy
c_func
(paren
id|cmd.num
comma
id|p
op_plus
l_int|6
comma
r_sizeof
(paren
id|cmd.num
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;NO D-CHAN&quot;
comma
l_int|9
)paren
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_NODCH
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ch
op_minus
l_int|1
suffix:semicolon
id|strncpy
c_func
(paren
id|cmd.num
comma
id|p
op_plus
l_int|6
comma
r_sizeof
(paren
id|cmd.num
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
)brace
r_else
(brace
id|p
op_assign
id|dev-&gt;imsg
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
comma
l_string|&quot;DRV1.&quot;
comma
l_int|5
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: %s&bslash;n&quot;
comma
id|p
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
op_plus
l_int|7
comma
l_string|&quot;TC&quot;
comma
l_int|2
)paren
)paren
(brace
id|dev-&gt;ptype
op_assign
id|ISDN_PTYPE_1TR6
suffix:semicolon
id|dev-&gt;interface.features
op_or_assign
id|ISDN_FEATURE_P_1TR6
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: 1TR6-Protocol loaded and running&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|p
op_plus
l_int|7
comma
l_string|&quot;EC&quot;
comma
l_int|2
)paren
)paren
(brace
id|dev-&gt;ptype
op_assign
id|ISDN_PTYPE_EURO
suffix:semicolon
id|dev-&gt;interface.features
op_or_assign
id|ISDN_FEATURE_P_EURO
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: Euro-Protocol loaded and running&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_continue
suffix:semicolon
)brace
)brace
)brace
r_else
(brace
id|dev-&gt;imsg
(braket
id|dev-&gt;iptr
)braket
op_assign
id|c
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;iptr
OL
l_int|59
)paren
id|dev-&gt;iptr
op_increment
suffix:semicolon
)brace
)brace
id|msg_o
op_assign
(paren
id|msg_o
op_plus
id|avail
)paren
op_amp
l_int|0xff
suffix:semicolon
id|icn_release_channel
c_func
(paren
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|avail
)paren
(brace
id|cmd.command
op_assign
id|ISDN_STAT_STAVAIL
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|avail
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dflag
op_amp
l_int|1
)paren
id|dev-&gt;interface
dot
id|rcvcallb
c_func
(paren
id|dev-&gt;myid
comma
l_int|0
comma
id|dev-&gt;rcvbuf
(braket
l_int|0
)braket
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dflag
op_amp
l_int|2
)paren
id|dev-&gt;interface
dot
id|rcvcallb
c_func
(paren
id|dev-&gt;myid
comma
l_int|1
comma
id|dev-&gt;rcvbuf
(braket
l_int|1
)braket
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;flags
op_amp
(paren
id|ICN_FLAGS_B1ACTIVE
op_or
id|ICN_FLAGS_B2ACTIVE
)paren
)paren
r_if
c_cond
(paren
op_logical_neg
(paren
id|dev-&gt;flags
op_amp
id|ICN_FLAGS_RBTIMER
)paren
)paren
(brace
multiline_comment|/* schedule b-channel polling */
id|dev-&gt;flags
op_or_assign
id|ICN_FLAGS_RBTIMER
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|del_timer
c_func
(paren
op_amp
id|dev-&gt;rb_timer
)paren
suffix:semicolon
id|dev-&gt;rb_timer.function
op_assign
id|icn_pollbchan
suffix:semicolon
id|dev-&gt;rb_timer.expires
op_assign
id|jiffies
op_plus
id|ICN_TIMER_BCREAD
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|dev-&gt;rb_timer
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * Check Statusqueue-Pointer from isdn-card.&n; * If there are new status-replies from the interface, check&n; * them against B-Channel-connects/disconnects and set flags accordingly.&n; * Wake-Up any processes, who are reading the status-device.&n; * If there are B-Channels open, initiate a timer-callback to&n; * icn_pollbchan().&n; * This routine is called periodically via timer.&n; */
DECL|function|icn_pollcard
r_static
r_void
id|icn_pollcard
c_func
(paren
r_int
r_int
id|dummy
)paren
(brace
id|ulong
id|flags
suffix:semicolon
id|icn_pollit
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|icn_pollit
c_func
(paren
id|dev2
)paren
suffix:semicolon
multiline_comment|/* schedule again */
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|del_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|dev-&gt;st_timer.function
op_assign
id|icn_pollcard
suffix:semicolon
id|dev-&gt;st_timer.expires
op_assign
id|jiffies
op_plus
id|ICN_TIMER_DCREAD
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
multiline_comment|/* Send a packet to the transmit-buffers, handle fragmentation if necessary.&n; * Parameters:&n; *            channel = Number of B-channel&n; *            buffer  = pointer to packet&n; *            len     = size of packet (max 4000)&n; *            dev     = pointer to device-struct&n; *            user    = 1 = call from userproc, 0 = call from kernel&n; * Return:&n; *        Number of bytes transferred, -E??? on error&n; */
DECL|function|icn_sendbuf
r_static
r_int
id|icn_sendbuf
c_func
(paren
r_int
id|channel
comma
r_const
id|u_char
op_star
id|buffer
comma
r_int
id|len
comma
r_int
id|user
comma
id|icn_dev
op_star
id|dev
)paren
(brace
r_register
id|u_char
op_star
id|p
suffix:semicolon
r_int
id|flags
suffix:semicolon
r_if
c_cond
(paren
id|len
OG
l_int|4000
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
id|len
)paren
(brace
r_if
c_cond
(paren
id|dev-&gt;sndcount
(braket
id|channel
)braket
OG
id|ICN_MAX_SQUEUE
)paren
r_return
l_int|0
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|p
op_assign
id|icn_new_buf
c_func
(paren
op_amp
id|dev-&gt;spqueue
(braket
id|channel
)braket
comma
id|len
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|p
)paren
(brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
id|user
)paren
(brace
id|memcpy_fromfs
c_func
(paren
id|p
comma
id|buffer
comma
id|len
)paren
suffix:semicolon
)brace
r_else
(brace
id|memcpy
c_func
(paren
id|p
comma
id|buffer
comma
id|len
)paren
suffix:semicolon
)brace
id|dev-&gt;sndcount
(braket
id|channel
)braket
op_add_assign
id|len
suffix:semicolon
id|icn_pollbchan_send
c_func
(paren
id|channel
comma
id|dev
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
r_return
id|len
suffix:semicolon
)brace
macro_line|#ifndef LOADEXTERN
DECL|function|icn_check_loader
r_static
r_int
id|icn_check_loader
c_func
(paren
r_int
id|cardnumber
)paren
(brace
r_int
id|timer
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
l_int|1
)paren
(brace
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Loader %d ?&bslash;n&quot;
comma
id|cardnumber
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|dev-&gt;shmem-&gt;data_control.scns
op_logical_or
id|dev-&gt;shmem-&gt;data_control.scnr
)paren
(brace
r_if
c_cond
(paren
id|timer
op_increment
OG
l_int|5
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: Boot-Loader %d timed out.&bslash;n&quot;
comma
id|cardnumber
)paren
suffix:semicolon
id|icn_release_channel
c_func
(paren
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Loader %d TO?&bslash;n&quot;
comma
id|cardnumber
)paren
suffix:semicolon
macro_line|#endif
id|current-&gt;state
op_assign
id|TASK_INTERRUPTIBLE
suffix:semicolon
id|current-&gt;timeout
op_assign
id|jiffies
op_plus
id|ICN_BOOT_TIMEOUT1
suffix:semicolon
id|schedule
c_func
(paren
)paren
suffix:semicolon
)brace
r_else
(brace
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Loader %d OK&bslash;n&quot;
comma
id|cardnumber
)paren
suffix:semicolon
macro_line|#endif
id|icn_release_channel
c_func
(paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
)brace
)brace
multiline_comment|/* Load the boot-code into the interface-card&squot;s memory and start it.&n; * Always called from user-process.&n; * &n; * Parameters:&n; *            buffer = pointer to packet&n; * Return:&n; *        0 if successfully loaded&n; */
macro_line|#ifdef BOOT_DEBUG
DECL|macro|SLEEP
mdefine_line|#define SLEEP(sec) { &bslash;&n;int slsec = sec; &bslash;&n;  printk(KERN_DEBUG &quot;SLEEP(%d)&bslash;n&quot;,slsec); &bslash;&n;  while (slsec) { &bslash;&n;    current-&gt;state = TASK_INTERRUPTIBLE; &bslash;&n;    current-&gt;timeout = jiffies + HZ; &bslash;&n;    schedule(); &bslash;&n;    slsec--; &bslash;&n;  } &bslash;&n;}
macro_line|#else
DECL|macro|SLEEP
mdefine_line|#define SLEEP(sec)
macro_line|#endif
DECL|function|icn_loadboot
r_static
r_int
id|icn_loadboot
c_func
(paren
id|u_char
op_star
id|buffer
comma
id|icn_dev
op_star
id|dev
)paren
(brace
r_int
id|ret
suffix:semicolon
id|ulong
id|flags
suffix:semicolon
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_loadboot called, buffaddr=%08lx&bslash;n&quot;
comma
(paren
id|ulong
)paren
id|buffer
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
(paren
id|ret
op_assign
id|verify_area
c_func
(paren
id|VERIFY_READ
comma
(paren
r_void
op_star
)paren
id|buffer
comma
id|ICN_CODE_STAGE1
)paren
)paren
)paren
r_return
id|ret
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev-&gt;rvalid
)paren
(brace
r_if
c_cond
(paren
id|check_region
c_func
(paren
id|dev-&gt;port
comma
id|ICN_PORTLEN
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: ports 0x%03x-0x%03x in use.&bslash;n&quot;
comma
id|dev-&gt;port
comma
id|dev-&gt;port
op_plus
id|ICN_PORTLEN
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
op_minus
id|EBUSY
suffix:semicolon
)brace
id|request_region
c_func
(paren
id|dev-&gt;port
comma
id|ICN_PORTLEN
comma
id|regname
)paren
suffix:semicolon
id|dev-&gt;rvalid
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|dev-&gt;mvalid
)paren
(brace
r_if
c_cond
(paren
id|check_shmem
c_func
(paren
(paren
id|ulong
)paren
id|dev-&gt;shmem
comma
l_int|0x4000
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: memory at 0x%08lx in use.&bslash;n&quot;
comma
(paren
id|ulong
)paren
id|dev-&gt;shmem
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_return
op_minus
id|EBUSY
suffix:semicolon
)brace
id|request_shmem
c_func
(paren
(paren
id|ulong
)paren
id|dev-&gt;shmem
comma
l_int|0x4000
comma
id|regname
)paren
suffix:semicolon
id|dev-&gt;mvalid
op_assign
l_int|1
suffix:semicolon
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|OUTB_P
c_func
(paren
l_int|0
comma
id|ICN_RUN
)paren
suffix:semicolon
multiline_comment|/* Reset Controller */
id|OUTB_P
c_func
(paren
l_int|0
comma
id|ICN_MAPRAM
)paren
suffix:semicolon
multiline_comment|/* Disable RAM     */
id|icn_shiftout
c_func
(paren
id|ICN_CFG
comma
l_int|0x0f
comma
l_int|3
comma
l_int|4
)paren
suffix:semicolon
multiline_comment|/* Windowsize= 16k */
id|icn_shiftout
c_func
(paren
id|ICN_CFG
comma
(paren
r_int
r_int
)paren
id|dev-&gt;shmem
comma
l_int|23
comma
l_int|10
)paren
suffix:semicolon
multiline_comment|/* Set RAM-Addr.   */
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;shmem=%08lx&bslash;n&quot;
comma
(paren
id|ulong
)paren
id|dev-&gt;shmem
)paren
suffix:semicolon
macro_line|#endif
id|SLEEP
c_func
(paren
l_int|1
)paren
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|dev-&gt;channel
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* Force Mapping   */
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Map Bank 0&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|icn_map_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
multiline_comment|/* Select Bank 0   */
id|icn_lock_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
multiline_comment|/* Lock Bank 0     */
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|SLEEP
c_func
(paren
l_int|1
)paren
suffix:semicolon
id|memcpy_fromfs
c_func
(paren
id|dev-&gt;shmem
comma
id|buffer
comma
id|ICN_CODE_STAGE1
)paren
suffix:semicolon
multiline_comment|/* Copy code       */
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Bootloader transfered&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
id|SLEEP
c_func
(paren
l_int|1
)paren
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|icn_release_channel
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Map Bank 8&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|icn_map_channel
c_func
(paren
l_int|2
)paren
suffix:semicolon
multiline_comment|/* Select Bank 8   */
id|icn_lock_channel
c_func
(paren
l_int|2
)paren
suffix:semicolon
multiline_comment|/* Lock Bank 8     */
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|SLEEP
c_func
(paren
l_int|1
)paren
suffix:semicolon
id|memcpy_fromfs
c_func
(paren
id|dev-&gt;shmem
comma
id|buffer
comma
id|ICN_CODE_STAGE1
)paren
suffix:semicolon
multiline_comment|/* Copy code       */
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Bootloader transfered&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
)brace
id|SLEEP
c_func
(paren
l_int|1
)paren
suffix:semicolon
id|OUTB_P
c_func
(paren
l_int|0xff
comma
id|ICN_RUN
)paren
suffix:semicolon
multiline_comment|/* Start Boot-Code */
r_if
c_cond
(paren
(paren
id|ret
op_assign
id|icn_check_loader
c_func
(paren
id|dev-&gt;doubleS0
ques
c_cond
l_int|2
suffix:colon
l_int|1
)paren
)paren
)paren
r_return
id|ret
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev-&gt;doubleS0
)paren
r_return
l_int|0
suffix:semicolon
multiline_comment|/* reached only, if we have a Double-S0-Card */
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Map Bank 0&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|icn_map_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
multiline_comment|/* Select Bank 0   */
id|icn_lock_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
multiline_comment|/* Lock Bank 0     */
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|SLEEP
c_func
(paren
l_int|1
)paren
suffix:semicolon
r_return
(paren
id|icn_check_loader
c_func
(paren
l_int|1
)paren
)paren
suffix:semicolon
)brace
DECL|function|icn_loadproto
r_static
r_int
id|icn_loadproto
c_func
(paren
id|u_char
op_star
id|buffer
comma
id|icn_dev
op_star
id|ldev
)paren
(brace
r_register
id|u_char
op_star
id|p
op_assign
id|buffer
suffix:semicolon
id|uint
id|left
op_assign
id|ICN_CODE_STAGE2
suffix:semicolon
id|uint
id|cnt
suffix:semicolon
r_int
id|timer
suffix:semicolon
r_int
id|ret
suffix:semicolon
r_int
r_int
id|flags
suffix:semicolon
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;icn_loadproto called&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
(paren
id|ret
op_assign
id|verify_area
c_func
(paren
id|VERIFY_READ
comma
(paren
r_void
op_star
)paren
id|buffer
comma
id|ICN_CODE_STAGE2
)paren
)paren
)paren
r_return
id|ret
suffix:semicolon
id|timer
op_assign
l_int|0
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ldev-&gt;secondhalf
)paren
(brace
id|icn_map_channel
c_func
(paren
l_int|2
)paren
suffix:semicolon
id|icn_lock_channel
c_func
(paren
l_int|2
)paren
suffix:semicolon
)brace
r_else
(brace
id|icn_map_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
id|icn_lock_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
r_while
c_loop
(paren
id|left
)paren
(brace
r_if
c_cond
(paren
id|sbfree
)paren
(brace
multiline_comment|/* If there is a free buffer...  */
id|cnt
op_assign
id|MIN
c_func
(paren
l_int|256
comma
id|left
)paren
suffix:semicolon
id|memcpy_fromfs
c_func
(paren
op_amp
id|sbuf_l
comma
id|p
comma
id|cnt
)paren
suffix:semicolon
multiline_comment|/* copy data                     */
id|sbnext
suffix:semicolon
multiline_comment|/* switch to next buffer         */
id|p
op_add_assign
id|cnt
suffix:semicolon
id|left
op_sub_assign
id|cnt
suffix:semicolon
id|timer
op_assign
l_int|0
suffix:semicolon
)brace
r_else
(brace
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;boot 2 !sbfree&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|timer
op_increment
OG
l_int|5
)paren
(brace
id|icn_maprelease_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
id|current-&gt;state
op_assign
id|TASK_INTERRUPTIBLE
suffix:semicolon
id|current-&gt;timeout
op_assign
id|jiffies
op_plus
l_int|10
suffix:semicolon
id|schedule
c_func
(paren
)paren
suffix:semicolon
)brace
)brace
id|sbuf_n
op_assign
l_int|0x20
suffix:semicolon
id|timer
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
l_int|1
)paren
(brace
r_if
c_cond
(paren
id|cmd_o
op_logical_or
id|cmd_i
)paren
(brace
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Proto?&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|timer
op_increment
OG
l_int|5
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: Protocol timed out.&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Proto TO!&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|icn_maprelease_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Proto TO?&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|current-&gt;state
op_assign
id|TASK_INTERRUPTIBLE
suffix:semicolon
id|current-&gt;timeout
op_assign
id|jiffies
op_plus
id|ICN_BOOT_TIMEOUT1
suffix:semicolon
id|schedule
c_func
(paren
)paren
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
(paren
id|ldev-&gt;secondhalf
)paren
op_logical_or
(paren
op_logical_neg
id|dev-&gt;doubleS0
)paren
)paren
(brace
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef BOOT_DEBUG
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;Proto loaded, install poll-timer %d&bslash;n&quot;
comma
id|ldev-&gt;secondhalf
)paren
suffix:semicolon
macro_line|#endif
id|init_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|dev-&gt;st_timer.expires
op_assign
id|jiffies
op_plus
id|ICN_TIMER_DCREAD
suffix:semicolon
id|dev-&gt;st_timer.function
op_assign
id|icn_pollcard
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
id|icn_maprelease_channel
c_func
(paren
l_int|0
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
)brace
)brace
macro_line|#endif&t;&t;&t;&t;/* !LOADEXTERN */
multiline_comment|/* Read the Status-replies from the Interface */
DECL|function|icn_readstatus
r_static
r_int
id|icn_readstatus
c_func
(paren
id|u_char
op_star
id|buf
comma
r_int
id|len
comma
r_int
id|user
comma
id|icn_dev
op_star
id|dev
)paren
(brace
r_int
id|count
suffix:semicolon
id|u_char
op_star
id|p
suffix:semicolon
r_for
c_loop
(paren
id|p
op_assign
id|buf
comma
id|count
op_assign
l_int|0
suffix:semicolon
id|count
OL
id|len
suffix:semicolon
id|p
op_increment
comma
id|count
op_increment
)paren
(brace
r_if
c_cond
(paren
id|user
)paren
id|put_fs_byte
c_func
(paren
op_star
id|dev-&gt;msg_buf_read
op_increment
comma
id|p
)paren
suffix:semicolon
r_else
op_star
id|p
op_assign
op_star
id|dev-&gt;msg_buf_read
op_increment
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;msg_buf_read
OG
id|dev-&gt;msg_buf_end
)paren
id|dev-&gt;msg_buf_read
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
)brace
r_return
id|count
suffix:semicolon
)brace
multiline_comment|/* Put command-strings into the command-queue of the Interface */
DECL|function|icn_writecmd
r_static
r_int
id|icn_writecmd
c_func
(paren
r_const
id|u_char
op_star
id|buf
comma
r_int
id|len
comma
r_int
id|user
comma
id|icn_dev
op_star
id|dev
comma
r_int
id|waitflg
)paren
(brace
r_int
id|mch
op_assign
id|dev-&gt;secondhalf
ques
c_cond
l_int|2
suffix:colon
l_int|0
suffix:semicolon
r_int
id|avail
suffix:semicolon
r_int
id|pp
suffix:semicolon
r_int
id|i
suffix:semicolon
r_int
id|count
suffix:semicolon
r_int
id|ocount
suffix:semicolon
r_int
r_int
id|flags
suffix:semicolon
id|u_char
op_star
id|p
suffix:semicolon
id|isdn_ctrl
id|cmd
suffix:semicolon
id|u_char
id|msg
(braket
l_int|0x100
)braket
suffix:semicolon
r_while
c_loop
(paren
l_int|1
)paren
(brace
r_if
c_cond
(paren
id|icn_trymaplock_channel
c_func
(paren
id|mch
)paren
)paren
(brace
id|avail
op_assign
id|cmd_free
suffix:semicolon
id|count
op_assign
id|MIN
c_func
(paren
id|avail
comma
id|len
)paren
suffix:semicolon
r_if
c_cond
(paren
id|user
)paren
id|memcpy_fromfs
c_func
(paren
id|msg
comma
id|buf
comma
id|count
)paren
suffix:semicolon
r_else
id|memcpy
c_func
(paren
id|msg
comma
id|buf
comma
id|count
)paren
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|ocount
op_assign
l_int|1
suffix:semicolon
op_star
id|dev-&gt;msg_buf_write
op_increment
op_assign
l_char|&squot;&gt;&squot;
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;msg_buf_write
OG
id|dev-&gt;msg_buf_end
)paren
id|dev-&gt;msg_buf_write
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
r_for
c_loop
(paren
id|p
op_assign
id|msg
comma
id|pp
op_assign
id|cmd_i
comma
id|i
op_assign
id|count
suffix:semicolon
id|i
OG
l_int|0
suffix:semicolon
id|i
op_decrement
comma
id|p
op_increment
comma
id|pp
op_increment
)paren
(brace
id|dev-&gt;shmem-&gt;comm_buffers.pcio_buf
(braket
id|pp
op_amp
l_int|0xff
)braket
op_assign
(paren
op_star
id|p
op_eq
l_char|&squot;&bslash;n&squot;
)paren
ques
c_cond
l_int|0xff
suffix:colon
op_star
id|p
suffix:semicolon
op_star
id|dev-&gt;msg_buf_write
op_increment
op_assign
op_star
id|p
suffix:semicolon
r_if
c_cond
(paren
(paren
op_star
id|p
op_eq
l_char|&squot;&bslash;n&squot;
)paren
op_logical_and
(paren
id|i
OG
l_int|1
)paren
)paren
(brace
op_star
id|dev-&gt;msg_buf_write
op_increment
op_assign
l_char|&squot;&gt;&squot;
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;msg_buf_write
OG
id|dev-&gt;msg_buf_end
)paren
id|dev-&gt;msg_buf_write
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
id|ocount
op_increment
suffix:semicolon
)brace
multiline_comment|/* No checks for buffer overflow of raw-status-device */
r_if
c_cond
(paren
id|dev-&gt;msg_buf_write
OG
id|dev-&gt;msg_buf_end
)paren
id|dev-&gt;msg_buf_write
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
id|ocount
op_increment
suffix:semicolon
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_STAVAIL
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
id|ocount
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
id|cmd_i
op_assign
(paren
id|cmd_i
op_plus
id|count
)paren
op_amp
l_int|0xff
suffix:semicolon
id|icn_release_channel
c_func
(paren
)paren
suffix:semicolon
id|waitflg
op_assign
l_int|0
suffix:semicolon
)brace
r_else
id|count
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|waitflg
)paren
r_break
suffix:semicolon
id|current-&gt;timeout
op_assign
id|jiffies
op_plus
l_int|10
suffix:semicolon
id|schedule
c_func
(paren
)paren
suffix:semicolon
)brace
r_return
id|count
suffix:semicolon
)brace
DECL|function|icn_stopdriver
r_static
r_void
id|icn_stopdriver
c_func
(paren
id|icn_dev
op_star
id|ldev
)paren
(brace
r_int
r_int
id|flags
suffix:semicolon
id|isdn_ctrl
id|cmd
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|del_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|del_timer
c_func
(paren
op_amp
id|ldev-&gt;rb_timer
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_STOP
suffix:semicolon
id|cmd.driver
op_assign
id|ldev-&gt;myid
suffix:semicolon
id|ldev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
DECL|function|my_atoi
r_static
r_int
id|my_atoi
c_func
(paren
r_char
op_star
id|s
)paren
(brace
r_int
id|i
comma
id|n
suffix:semicolon
id|n
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|s
)paren
r_return
op_minus
l_int|1
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
op_star
id|s
op_ge
l_char|&squot;0&squot;
op_logical_and
op_star
id|s
op_le
l_char|&squot;9&squot;
suffix:semicolon
id|i
op_increment
comma
id|s
op_increment
)paren
id|n
op_assign
l_int|10
op_star
id|n
op_plus
(paren
op_star
id|s
op_minus
l_char|&squot;0&squot;
)paren
suffix:semicolon
r_return
id|n
suffix:semicolon
)brace
DECL|function|icn_command
r_static
r_int
id|icn_command
c_func
(paren
id|isdn_ctrl
op_star
id|c
comma
id|icn_dev
op_star
id|ldev
)paren
(brace
id|ulong
id|a
suffix:semicolon
id|ulong
id|flags
suffix:semicolon
r_int
id|i
suffix:semicolon
r_char
id|cbuf
(braket
l_int|60
)braket
suffix:semicolon
id|isdn_ctrl
id|cmd
suffix:semicolon
r_switch
c_cond
(paren
id|c-&gt;command
)paren
(brace
r_case
id|ISDN_CMD_IOCTL
suffix:colon
id|memcpy
c_func
(paren
op_amp
id|a
comma
id|c-&gt;num
comma
r_sizeof
(paren
id|ulong
)paren
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|c-&gt;arg
)paren
(brace
r_case
id|ICN_IOCTL_SETMMIO
suffix:colon
r_if
c_cond
(paren
(paren
r_int
r_int
)paren
id|dev-&gt;shmem
op_ne
(paren
id|a
op_amp
l_int|0x0ffc000
)paren
)paren
(brace
r_if
c_cond
(paren
id|check_shmem
c_func
(paren
(paren
id|ulong
)paren
(paren
id|a
op_amp
l_int|0x0ffc000
)paren
comma
l_int|0x4000
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: memory at 0x%08lx in use.&bslash;n&quot;
comma
(paren
id|ulong
)paren
(paren
id|a
op_amp
l_int|0x0ffc000
)paren
)paren
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
id|icn_stopdriver
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|icn_stopdriver
c_func
(paren
id|dev2
)paren
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;mvalid
)paren
id|release_shmem
c_func
(paren
(paren
id|ulong
)paren
id|dev-&gt;shmem
comma
l_int|0x4000
)paren
suffix:semicolon
id|dev-&gt;mvalid
op_assign
l_int|0
suffix:semicolon
id|dev-&gt;shmem
op_assign
(paren
id|icn_shmem
op_star
)paren
(paren
id|a
op_amp
l_int|0x0ffc000
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|dev2-&gt;shmem
op_assign
(paren
id|icn_shmem
op_star
)paren
(paren
id|a
op_amp
l_int|0x0ffc000
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: mmio set to 0x%08lx&bslash;n&quot;
comma
(paren
r_int
r_int
)paren
id|dev-&gt;shmem
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ICN_IOCTL_GETMMIO
suffix:colon
r_return
(paren
r_int
)paren
id|dev-&gt;shmem
suffix:semicolon
r_case
id|ICN_IOCTL_SETPORT
suffix:colon
r_if
c_cond
(paren
id|a
op_eq
l_int|0x300
op_logical_or
id|a
op_eq
l_int|0x310
op_logical_or
id|a
op_eq
l_int|0x320
op_logical_or
id|a
op_eq
l_int|0x330
op_logical_or
id|a
op_eq
l_int|0x340
op_logical_or
id|a
op_eq
l_int|0x350
op_logical_or
id|a
op_eq
l_int|0x360
op_logical_or
id|a
op_eq
l_int|0x308
op_logical_or
id|a
op_eq
l_int|0x318
op_logical_or
id|a
op_eq
l_int|0x328
op_logical_or
id|a
op_eq
l_int|0x338
op_logical_or
id|a
op_eq
l_int|0x348
op_logical_or
id|a
op_eq
l_int|0x358
op_logical_or
id|a
op_eq
l_int|0x368
)paren
(brace
r_if
c_cond
(paren
id|dev-&gt;port
op_ne
(paren
r_int
r_int
)paren
id|a
)paren
(brace
r_if
c_cond
(paren
id|check_region
c_func
(paren
(paren
r_int
r_int
)paren
id|a
comma
id|ICN_PORTLEN
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: ports 0x%03x-0x%03x in use.&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
comma
(paren
r_int
)paren
id|a
op_plus
id|ICN_PORTLEN
)paren
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
id|icn_stopdriver
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|icn_stopdriver
c_func
(paren
id|dev2
)paren
suffix:semicolon
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;rvalid
)paren
id|release_region
c_func
(paren
id|dev-&gt;port
comma
id|ICN_PORTLEN
)paren
suffix:semicolon
id|dev-&gt;port
op_assign
(paren
r_int
r_int
)paren
id|a
suffix:semicolon
id|dev-&gt;rvalid
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
id|dev2-&gt;port
op_assign
(paren
r_int
r_int
)paren
id|a
suffix:semicolon
id|dev2-&gt;rvalid
op_assign
l_int|0
suffix:semicolon
)brace
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: port set to 0x%03x&bslash;n&quot;
comma
id|dev-&gt;port
)paren
suffix:semicolon
)brace
)brace
r_else
r_return
op_minus
id|EINVAL
suffix:semicolon
r_break
suffix:semicolon
r_case
id|ICN_IOCTL_GETPORT
suffix:colon
r_return
(paren
r_int
)paren
id|dev-&gt;port
suffix:semicolon
r_case
id|ICN_IOCTL_GETDOUBLE
suffix:colon
r_return
(paren
r_int
)paren
id|dev-&gt;doubleS0
suffix:semicolon
r_case
id|ICN_IOCTL_DEBUGVAR
suffix:colon
r_return
(paren
id|ulong
)paren
id|ldev
suffix:semicolon
macro_line|#ifndef LOADEXTERN
r_case
id|ICN_IOCTL_LOADBOOT
suffix:colon
id|icn_stopdriver
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|icn_stopdriver
c_func
(paren
id|dev2
)paren
suffix:semicolon
r_return
(paren
id|icn_loadboot
c_func
(paren
(paren
id|u_char
op_star
)paren
id|a
comma
id|dev
)paren
)paren
suffix:semicolon
r_case
id|ICN_IOCTL_LOADPROTO
suffix:colon
id|icn_stopdriver
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|icn_stopdriver
c_func
(paren
id|dev2
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|i
op_assign
(paren
id|icn_loadproto
c_func
(paren
(paren
id|u_char
op_star
)paren
id|a
comma
id|dev
)paren
)paren
)paren
)paren
r_return
id|i
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|i
op_assign
id|icn_loadproto
c_func
(paren
(paren
id|u_char
op_star
)paren
(paren
id|a
op_plus
id|ICN_CODE_STAGE2
)paren
comma
id|dev2
)paren
suffix:semicolon
r_return
id|i
suffix:semicolon
macro_line|#endif
r_case
id|ICN_IOCTL_LEASEDCFG
suffix:colon
r_if
c_cond
(paren
id|a
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|ldev-&gt;leased
)paren
(brace
id|ldev-&gt;leased
op_assign
l_int|1
suffix:semicolon
r_while
c_loop
(paren
id|ldev-&gt;ptype
op_eq
id|ISDN_PTYPE_UNKNOWN
)paren
(brace
id|current-&gt;timeout
op_assign
id|jiffies
op_plus
id|ICN_BOOT_TIMEOUT1
suffix:semicolon
id|schedule
c_func
(paren
)paren
suffix:semicolon
)brace
id|current-&gt;timeout
op_assign
id|jiffies
op_plus
id|ICN_BOOT_TIMEOUT1
suffix:semicolon
id|schedule
c_func
(paren
)paren
suffix:semicolon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;00;FV2ON&bslash;n01;EAZ1&bslash;n&quot;
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: Leased-line mode enabled&bslash;n&quot;
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_RUN
suffix:semicolon
id|cmd.driver
op_assign
id|ldev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
l_int|0
suffix:semicolon
id|ldev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
)brace
)brace
r_else
(brace
r_if
c_cond
(paren
id|ldev-&gt;leased
)paren
(brace
id|ldev-&gt;leased
op_assign
l_int|0
suffix:semicolon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;00;FV2OFF&bslash;n&quot;
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;icn: Leased-line mode disabled&bslash;n&quot;
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_RUN
suffix:semicolon
id|cmd.driver
op_assign
id|ldev-&gt;myid
suffix:semicolon
id|cmd.arg
op_assign
l_int|0
suffix:semicolon
id|ldev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
)brace
)brace
r_return
l_int|0
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_DIAL
suffix:colon
r_if
c_cond
(paren
id|ldev-&gt;leased
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
(paren
id|c-&gt;arg
op_amp
l_int|255
)paren
OL
id|ICN_BCH
)paren
(brace
r_char
op_star
id|p
suffix:semicolon
r_char
op_star
id|p2
suffix:semicolon
r_char
id|dial
(braket
l_int|50
)braket
suffix:semicolon
r_char
id|sis
(braket
l_int|50
)braket
suffix:semicolon
r_char
id|dcode
(braket
l_int|4
)braket
suffix:semicolon
r_int
id|si1
comma
id|si2
suffix:semicolon
id|a
op_assign
id|c-&gt;arg
suffix:semicolon
id|strcpy
c_func
(paren
id|sis
comma
id|c-&gt;num
)paren
suffix:semicolon
id|p
op_assign
id|strrchr
c_func
(paren
id|sis
comma
l_char|&squot;,&squot;
)paren
suffix:semicolon
op_star
id|p
op_increment
op_assign
l_char|&squot;&bslash;0&squot;
suffix:semicolon
id|si2
op_assign
id|my_atoi
c_func
(paren
id|p
)paren
suffix:semicolon
id|p
op_assign
id|strrchr
c_func
(paren
id|sis
comma
l_char|&squot;,&squot;
)paren
op_plus
l_int|1
suffix:semicolon
id|si1
op_assign
id|my_atoi
c_func
(paren
id|p
)paren
suffix:semicolon
id|p
op_assign
id|c-&gt;num
suffix:semicolon
r_if
c_cond
(paren
op_star
id|p
op_eq
l_char|&squot;s&squot;
op_logical_or
op_star
id|p
op_eq
l_char|&squot;S&squot;
)paren
(brace
multiline_comment|/* Dial for SPV */
id|p
op_increment
suffix:semicolon
id|strcpy
c_func
(paren
id|dcode
comma
l_string|&quot;SCA&quot;
)paren
suffix:semicolon
)brace
r_else
multiline_comment|/* Normal Dial */
id|strcpy
c_func
(paren
id|dcode
comma
l_string|&quot;CAL&quot;
)paren
suffix:semicolon
id|strcpy
c_func
(paren
id|dial
comma
id|p
)paren
suffix:semicolon
id|p
op_assign
id|strchr
c_func
(paren
id|dial
comma
l_char|&squot;,&squot;
)paren
suffix:semicolon
op_star
id|p
op_increment
op_assign
l_char|&squot;&bslash;0&squot;
suffix:semicolon
id|p2
op_assign
id|strchr
c_func
(paren
id|p
comma
l_char|&squot;,&squot;
)paren
suffix:semicolon
op_star
id|p2
op_assign
l_char|&squot;&bslash;0&squot;
suffix:semicolon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;D%s_R%s,%02d,%02d,%s&bslash;n&quot;
comma
(paren
r_int
)paren
(paren
id|a
op_plus
l_int|1
)paren
comma
id|dcode
comma
id|dial
comma
id|si1
comma
id|si2
comma
id|p
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_ACCEPTD
suffix:colon
r_if
c_cond
(paren
id|c-&gt;arg
OL
id|ICN_BCH
)paren
(brace
id|a
op_assign
id|c-&gt;arg
op_plus
l_int|1
suffix:semicolon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;DCON_R&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_ACCEPTB
suffix:colon
r_if
c_cond
(paren
id|c-&gt;arg
OL
id|ICN_BCH
)paren
(brace
id|a
op_assign
id|c-&gt;arg
op_plus
l_int|1
suffix:semicolon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;BCON_R&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_HANGUP
suffix:colon
r_if
c_cond
(paren
id|c-&gt;arg
OL
id|ICN_BCH
)paren
(brace
id|a
op_assign
id|c-&gt;arg
op_plus
l_int|1
suffix:semicolon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;BDIS_R&bslash;n%02d;DDIS_R&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
comma
(paren
r_int
)paren
id|a
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_SETEAZ
suffix:colon
r_if
c_cond
(paren
id|ldev-&gt;leased
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|c-&gt;arg
OL
id|ICN_BCH
)paren
(brace
id|a
op_assign
id|c-&gt;arg
op_plus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|ldev-&gt;ptype
op_eq
id|ISDN_PTYPE_EURO
)paren
(brace
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;MS%s%s&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
comma
id|c-&gt;num
(braket
l_int|0
)braket
ques
c_cond
l_string|&quot;N&quot;
suffix:colon
l_string|&quot;ALL&quot;
comma
id|c-&gt;num
)paren
suffix:semicolon
)brace
r_else
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;EAZ%s&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
comma
id|c-&gt;num
(braket
l_int|0
)braket
ques
c_cond
id|c-&gt;num
suffix:colon
l_string|&quot;0123456789&quot;
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_CLREAZ
suffix:colon
r_if
c_cond
(paren
id|ldev-&gt;leased
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|c-&gt;arg
OL
id|ICN_BCH
)paren
(brace
id|a
op_assign
id|c-&gt;arg
op_plus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|ldev-&gt;ptype
op_eq
id|ISDN_PTYPE_EURO
)paren
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;MSNC&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
)paren
suffix:semicolon
r_else
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;EAZC&bslash;n&quot;
comma
(paren
r_int
)paren
id|a
)paren
suffix:semicolon
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_SETL2
suffix:colon
r_if
c_cond
(paren
(paren
id|c-&gt;arg
op_amp
l_int|255
)paren
OL
id|ICN_BCH
)paren
(brace
id|a
op_assign
id|c-&gt;arg
suffix:semicolon
r_switch
c_cond
(paren
id|a
op_rshift
l_int|8
)paren
(brace
r_case
id|ISDN_PROTO_L2_X75I
suffix:colon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;BX75&bslash;n&quot;
comma
(paren
r_int
)paren
(paren
id|a
op_amp
l_int|255
)paren
op_plus
l_int|1
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|ISDN_PROTO_L2_HDLC
suffix:colon
id|sprintf
c_func
(paren
id|cbuf
comma
l_string|&quot;%02d;BTRA&bslash;n&quot;
comma
(paren
r_int
)paren
(paren
id|a
op_amp
l_int|255
)paren
op_plus
l_int|1
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
id|i
op_assign
id|icn_writecmd
c_func
(paren
id|cbuf
comma
id|strlen
c_func
(paren
id|cbuf
)paren
comma
l_int|0
comma
id|ldev
comma
l_int|1
)paren
suffix:semicolon
id|ldev-&gt;l2_proto
(braket
id|a
op_amp
l_int|255
)braket
op_assign
(paren
id|a
op_rshift
l_int|8
)paren
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|ISDN_CMD_GETL2
suffix:colon
r_if
c_cond
(paren
(paren
id|c-&gt;arg
op_amp
l_int|255
)paren
OL
id|ICN_BCH
)paren
r_return
id|ldev-&gt;l2_proto
(braket
id|c-&gt;arg
op_amp
l_int|255
)braket
suffix:semicolon
r_else
r_return
op_minus
id|ENODEV
suffix:semicolon
r_case
id|ISDN_CMD_SETL3
suffix:colon
r_return
l_int|0
suffix:semicolon
r_case
id|ISDN_CMD_GETL3
suffix:colon
r_if
c_cond
(paren
(paren
id|c-&gt;arg
op_amp
l_int|255
)paren
OL
id|ICN_BCH
)paren
r_return
id|ISDN_PROTO_L3_TRANS
suffix:semicolon
r_else
r_return
op_minus
id|ENODEV
suffix:semicolon
r_case
id|ISDN_CMD_GETEAZ
suffix:colon
r_break
suffix:semicolon
r_case
id|ISDN_CMD_SETSIL
suffix:colon
r_break
suffix:semicolon
r_case
id|ISDN_CMD_GETSIL
suffix:colon
r_break
suffix:semicolon
r_case
id|ISDN_CMD_LOCK
suffix:colon
id|MOD_INC_USE_COUNT
suffix:semicolon
r_break
suffix:semicolon
r_case
id|ISDN_CMD_UNLOCK
suffix:colon
id|MOD_DEC_USE_COUNT
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * For second half of doubleS0-Card add channel-offset.&n; */
DECL|function|if_command1
r_static
r_int
id|if_command1
c_func
(paren
id|isdn_ctrl
op_star
id|c
)paren
(brace
r_return
(paren
id|icn_command
c_func
(paren
id|c
comma
id|dev
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_command2
r_static
r_int
id|if_command2
c_func
(paren
id|isdn_ctrl
op_star
id|c
)paren
(brace
r_return
(paren
id|icn_command
c_func
(paren
id|c
comma
id|dev2
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_writecmd1
r_static
r_int
id|if_writecmd1
c_func
(paren
r_const
id|u_char
op_star
id|buf
comma
r_int
id|len
comma
r_int
id|user
)paren
(brace
r_return
(paren
id|icn_writecmd
c_func
(paren
id|buf
comma
id|len
comma
id|user
comma
id|dev
comma
l_int|0
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_writecmd2
r_static
r_int
id|if_writecmd2
c_func
(paren
r_const
id|u_char
op_star
id|buf
comma
r_int
id|len
comma
r_int
id|user
)paren
(brace
r_return
(paren
id|icn_writecmd
c_func
(paren
id|buf
comma
id|len
comma
id|user
comma
id|dev2
comma
l_int|0
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_readstatus1
r_static
r_int
id|if_readstatus1
c_func
(paren
id|u_char
op_star
id|buf
comma
r_int
id|len
comma
r_int
id|user
)paren
(brace
r_return
(paren
id|icn_readstatus
c_func
(paren
id|buf
comma
id|len
comma
id|user
comma
id|dev
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_readstatus2
r_static
r_int
id|if_readstatus2
c_func
(paren
id|u_char
op_star
id|buf
comma
r_int
id|len
comma
r_int
id|user
)paren
(brace
r_return
(paren
id|icn_readstatus
c_func
(paren
id|buf
comma
id|len
comma
id|user
comma
id|dev2
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_sendbuf1
r_static
r_int
id|if_sendbuf1
c_func
(paren
r_int
id|id
comma
r_int
id|channel
comma
r_const
id|u_char
op_star
id|buffer
comma
r_int
id|len
comma
r_int
id|user
)paren
(brace
r_return
(paren
id|icn_sendbuf
c_func
(paren
id|channel
comma
id|buffer
comma
id|len
comma
id|user
comma
id|dev
)paren
)paren
suffix:semicolon
)brace
DECL|function|if_sendbuf2
r_static
r_int
id|if_sendbuf2
c_func
(paren
r_int
id|id
comma
r_int
id|channel
comma
r_const
id|u_char
op_star
id|buffer
comma
r_int
id|len
comma
r_int
id|user
)paren
(brace
r_return
(paren
id|icn_sendbuf
c_func
(paren
id|channel
comma
id|buffer
comma
id|len
comma
id|user
comma
id|dev2
)paren
)paren
suffix:semicolon
)brace
macro_line|#ifdef MODULE
DECL|macro|icn_init
mdefine_line|#define icn_init init_module
macro_line|#else
DECL|function|icn_setup
r_void
id|icn_setup
c_func
(paren
r_char
op_star
id|str
comma
r_int
op_star
id|ints
)paren
(brace
r_char
op_star
id|p
suffix:semicolon
r_static
r_char
id|sid
(braket
l_int|20
)braket
suffix:semicolon
r_static
r_char
id|sid2
(braket
l_int|20
)braket
suffix:semicolon
r_if
c_cond
(paren
id|ints
(braket
l_int|0
)braket
)paren
id|portbase
op_assign
id|ints
(braket
l_int|1
)braket
suffix:semicolon
r_if
c_cond
(paren
id|ints
(braket
l_int|0
)braket
OG
l_int|1
)paren
id|membase
op_assign
id|ints
(braket
l_int|2
)braket
suffix:semicolon
r_if
c_cond
(paren
id|strlen
c_func
(paren
id|str
)paren
)paren
(brace
id|strcpy
c_func
(paren
id|sid
comma
id|str
)paren
suffix:semicolon
id|icn_id
op_assign
id|sid
suffix:semicolon
r_if
c_cond
(paren
(paren
id|p
op_assign
id|strchr
c_func
(paren
id|sid
comma
l_char|&squot;,&squot;
)paren
)paren
)paren
(brace
op_star
id|p
op_increment
op_assign
l_int|0
suffix:semicolon
id|strcpy
c_func
(paren
id|sid2
comma
id|p
)paren
suffix:semicolon
id|icn_id2
op_assign
id|sid2
suffix:semicolon
)brace
)brace
)brace
macro_line|#endif
DECL|function|icn_init
r_int
id|icn_init
c_func
(paren
r_void
)paren
(brace
macro_line|#ifdef LOADEXTERN
r_int
r_int
id|flags
suffix:semicolon
macro_line|#endif
r_char
op_star
id|p
suffix:semicolon
id|isdn_ctrl
id|cmd
suffix:semicolon
r_char
id|rev
(braket
l_int|10
)braket
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|dev
op_assign
(paren
id|icn_devptr
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
id|icn_dev
)paren
comma
id|GFP_KERNEL
)paren
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: Could not allocate device-struct.&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
id|memset
c_func
(paren
(paren
r_char
op_star
)paren
id|dev
comma
l_int|0
comma
r_sizeof
(paren
id|icn_dev
)paren
)paren
suffix:semicolon
id|dev-&gt;port
op_assign
id|portbase
suffix:semicolon
id|dev-&gt;shmem
op_assign
(paren
id|icn_shmem
op_star
)paren
(paren
id|membase
op_amp
l_int|0x0ffc000
)paren
suffix:semicolon
r_if
c_cond
(paren
id|strlen
c_func
(paren
id|icn_id2
)paren
)paren
id|dev-&gt;doubleS0
op_assign
l_int|1
suffix:semicolon
id|dev-&gt;interface.channels
op_assign
id|ICN_BCH
suffix:semicolon
id|dev-&gt;interface.maxbufsize
op_assign
l_int|4000
suffix:semicolon
id|dev-&gt;interface.command
op_assign
id|if_command1
suffix:semicolon
id|dev-&gt;interface.writebuf
op_assign
id|if_sendbuf1
suffix:semicolon
id|dev-&gt;interface.writecmd
op_assign
id|if_writecmd1
suffix:semicolon
id|dev-&gt;interface.readstat
op_assign
id|if_readstatus1
suffix:semicolon
id|dev-&gt;interface.features
op_assign
id|ISDN_FEATURE_L2_X75I
op_or
id|ISDN_FEATURE_L2_HDLC
op_or
id|ISDN_FEATURE_L3_TRANS
op_or
id|ISDN_FEATURE_P_UNKNOWN
suffix:semicolon
id|dev-&gt;ptype
op_assign
id|ISDN_PTYPE_UNKNOWN
suffix:semicolon
id|strncpy
c_func
(paren
id|dev-&gt;interface.id
comma
id|icn_id
comma
r_sizeof
(paren
id|dev-&gt;interface.id
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|dev-&gt;msg_buf_write
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
id|dev-&gt;msg_buf_read
op_assign
id|dev-&gt;msg_buf
suffix:semicolon
id|dev-&gt;msg_buf_end
op_assign
op_amp
id|dev-&gt;msg_buf
(braket
r_sizeof
(paren
id|dev-&gt;msg_buf
)paren
op_minus
l_int|1
)braket
suffix:semicolon
id|memset
c_func
(paren
(paren
r_char
op_star
)paren
id|dev-&gt;l2_proto
comma
id|ISDN_PROTO_L2_X75I
comma
r_sizeof
(paren
id|dev-&gt;l2_proto
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|strlen
c_func
(paren
id|icn_id2
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|dev2
op_assign
(paren
id|icn_devptr
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
id|icn_dev
)paren
comma
id|GFP_KERNEL
)paren
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: Could not allocate device-struct.&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev2
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
id|memcpy
c_func
(paren
(paren
r_char
op_star
)paren
id|dev2
comma
(paren
r_char
op_star
)paren
id|dev
comma
r_sizeof
(paren
id|icn_dev
)paren
)paren
suffix:semicolon
id|dev2-&gt;interface.command
op_assign
id|if_command2
suffix:semicolon
id|dev2-&gt;interface.writebuf
op_assign
id|if_sendbuf2
suffix:semicolon
id|dev2-&gt;interface.writecmd
op_assign
id|if_writecmd2
suffix:semicolon
id|dev2-&gt;interface.readstat
op_assign
id|if_readstatus2
suffix:semicolon
id|strncpy
c_func
(paren
id|dev2-&gt;interface.id
comma
id|icn_id2
comma
r_sizeof
(paren
id|dev-&gt;interface.id
)paren
op_minus
l_int|1
)paren
suffix:semicolon
id|dev2-&gt;msg_buf_write
op_assign
id|dev2-&gt;msg_buf
suffix:semicolon
id|dev2-&gt;msg_buf_read
op_assign
id|dev2-&gt;msg_buf
suffix:semicolon
id|dev2-&gt;msg_buf_end
op_assign
op_amp
id|dev2-&gt;msg_buf
(braket
r_sizeof
(paren
id|dev2-&gt;msg_buf
)paren
op_minus
l_int|1
)braket
suffix:semicolon
id|dev2-&gt;secondhalf
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|register_isdn
c_func
(paren
op_amp
id|dev-&gt;interface
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: Unable to register&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
id|kfree
c_func
(paren
id|dev2
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
id|dev-&gt;myid
op_assign
id|dev-&gt;interface.channels
suffix:semicolon
id|sprintf
c_func
(paren
id|regname
comma
l_string|&quot;icn-isdn (%s)&quot;
comma
id|dev-&gt;interface.id
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|register_isdn
c_func
(paren
op_amp
id|dev2-&gt;interface
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;icn: Unable to register&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev2
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
id|icn_stopdriver
c_func
(paren
id|dev
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_UNLOAD
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev
)paren
suffix:semicolon
)brace
r_return
op_minus
id|EIO
suffix:semicolon
)brace
id|dev2-&gt;myid
op_assign
id|dev2-&gt;interface.channels
suffix:semicolon
)brace
multiline_comment|/* No symbols to export, hide all symbols */
id|register_symtab
c_func
(paren
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|p
op_assign
id|strchr
c_func
(paren
id|revision
comma
l_char|&squot;:&squot;
)paren
)paren
)paren
(brace
id|strcpy
c_func
(paren
id|rev
comma
id|p
op_plus
l_int|1
)paren
suffix:semicolon
id|p
op_assign
id|strchr
c_func
(paren
id|rev
comma
l_char|&squot;$&squot;
)paren
suffix:semicolon
op_star
id|p
op_assign
l_int|0
suffix:semicolon
)brace
r_else
id|strcpy
c_func
(paren
id|rev
comma
l_string|&quot; ??? &quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;ICN-ISDN-driver Rev%sport=0x%03x mmio=0x%08x id=&squot;%s&squot;&bslash;n&quot;
comma
id|rev
comma
id|dev-&gt;port
comma
(paren
id|uint
)paren
id|dev-&gt;shmem
comma
id|dev-&gt;interface.id
)paren
suffix:semicolon
macro_line|#ifdef LOADEXTERN
id|save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|cli
c_func
(paren
)paren
suffix:semicolon
id|init_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|dev-&gt;st_timer.expires
op_assign
id|jiffies
op_plus
id|ICN_TIMER_DCREAD
suffix:semicolon
id|dev-&gt;st_timer.function
op_assign
id|icn_pollcard
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|dev-&gt;st_timer
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
macro_line|#endif
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#ifdef MODULE
DECL|function|cleanup_module
r_void
id|cleanup_module
c_func
(paren
r_void
)paren
(brace
id|isdn_ctrl
id|cmd
suffix:semicolon
r_int
id|i
suffix:semicolon
id|icn_stopdriver
c_func
(paren
id|dev
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_UNLOAD
suffix:semicolon
id|cmd.driver
op_assign
id|dev-&gt;myid
suffix:semicolon
id|dev-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
id|icn_stopdriver
c_func
(paren
id|dev2
)paren
suffix:semicolon
id|cmd.command
op_assign
id|ISDN_STAT_UNLOAD
suffix:semicolon
id|cmd.driver
op_assign
id|dev2-&gt;myid
suffix:semicolon
id|dev2-&gt;interface
dot
id|statcallb
c_func
(paren
op_amp
id|cmd
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev-&gt;rvalid
)paren
(brace
id|OUTB_P
c_func
(paren
l_int|0
comma
id|ICN_RUN
)paren
suffix:semicolon
multiline_comment|/* Reset Controller     */
id|OUTB_P
c_func
(paren
l_int|0
comma
id|ICN_MAPRAM
)paren
suffix:semicolon
multiline_comment|/* Disable RAM          */
id|release_region
c_func
(paren
id|dev-&gt;port
comma
id|ICN_PORTLEN
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev-&gt;mvalid
)paren
id|release_shmem
c_func
(paren
(paren
id|ulong
)paren
id|dev-&gt;shmem
comma
l_int|0x4000
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;doubleS0
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|ICN_BCH
suffix:semicolon
id|i
op_increment
)paren
id|icn_free_queue
c_func
(paren
op_amp
id|dev2-&gt;spqueue
(braket
l_int|1
)braket
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev2
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|ICN_BCH
suffix:semicolon
id|i
op_increment
)paren
id|icn_free_queue
c_func
(paren
op_amp
id|dev-&gt;spqueue
(braket
l_int|1
)braket
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|dev
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;ICN-ISDN-driver unloaded&bslash;n&quot;
)paren
suffix:semicolon
)brace
macro_line|#endif
eof
