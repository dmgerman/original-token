multiline_comment|/*&n; * sound/wavefront.c&n; *&n; * A low level driver for Turtle Beach WaveFront Series&n; *    (Maui, Tropez, Tropez Plus, and perhaps the Monterey &amp; Rio)&n; *&n; * This driver supports the onboard wavetable synthesizer (an ICS2115),&n; * including patch, sample and program loading and unloading, conversion&n; * of GUS patches during loading, and full user-level access to all&n; * WaveFront commands. It tries to provide semi-intelligent patch and&n; * sample management as well.&n; *&n; * It also provides support for the ICS emulation of an MPU-401.  Full&n; * support for the ICS emulation&squot;s &quot;virtual MIDI mode&quot; is provided in&n; * wf_midi.c.&n; *&n; * Support is also provided for the Tropez Plus&squot; onboard FX processor,&n; * a Yamaha YSS225. Currently, code exists to configure the YSS225,&n; * and there is an interface allowing tweaking of any of its memory&n; * addresses. However, I have been unable to decipher the logical&n; * positioning of the configuration info for various effects, so for&n; * now, you just get the YSS225 in the same state as Turtle Beach&squot;s&n; * &quot;SETUPSND.EXE&quot; utility leaves it.&n; *&n; * The boards&squot; CODEC (a Crystal CS4232) is supported by cs4232.[co],&n; * This chip also controls the configuration of the card: the wavefront&n; * synth is logical unit 4.&n; *&n; * NOTE: this driver has been written to support multiple WaveFront&n; * cards, but without using PnP to configure the CS4232, all of them&n; * would end up with the same configuration. Further, the current&n; * module loading interface doesn&squot;t permit this, since it only allows&n; * once instance of a module (which happens to be equivalent to a&n; * single hardware configuration) to be installed at one time.In&n; * addition, there is a hard limit on the available DMA channels that&n; * also makes installing more than 1 card limited to purely MIDI/synth&n; * activities on the second card. Still, the coding style gets rid of&n; * virtually all globals, which I believe is a better way to code&n; * device drivers (or anything else, for that matter).&n; *&n; **********************************************************************&n; *&n; * Copyright (C) by Paul Barton-Davis 1998&n; *&n; * Some portions of this file are taken from work that is&n; * copyright (C) by Hannu Savolainen 1993-1996&n; *&n; * Although the relevant code here is all new, the handling of&n; * sample/alias/multi- samples is entirely based on a driver by Matt&n; * Martin and Rutger Nijlunsing which demonstrated how to get things&n; * to most aspects of this to work correctly. The GUS patch loading&n; * code has been almost unaltered by me, except to fit formatting and&n; * function names in the rest of the file. Many thanks to them.&n; *&n; * Appreciation and thanks to Hannu Savolainen for his early work on the Maui&n; * driver, and answering a few questions while this one was developed.&n; *&n; * Absolutely NO thanks to Turtle Beach/Voyetra and Yamaha for their&n; * complete lack of help in developing this driver, and in particular&n; * for their utter silence in response to questions about undocumented&n; * aspects of configuring a WaveFront soundcard, particularly the&n; * effects processor.&n; *&n; * This program is distributed under the GNU GENERAL PUBLIC LICENSE (GPL)&n; * Version 2 (June 1991). See the &quot;COPYING&quot; file distributed with this software&n; * for more info.  */
...
mdefine_line|#define COPY_FROM_USER(a,b,c) copy_from_user ((a),(b),(c))
DECL|macro|COPY_TO_USER
mdefine_line|#define COPY_TO_USER(a,b,c)   copy_to_user ((a),(b),(c))
...
mdefine_line|#define STAT_RINTR_ENABLED&t;0x10
...
mdefine_line|#define STAT_WINTR_ENABLED&t;0x01
...
DECL|variable|wf_debug_default
r_int
id|wf_debug_default
op_assign
l_int|0
...
suffix:semicolon
multiline_comment|/* where to find a processed&n;&t;&t;&t;&t;&t;&t;version of the WaveFront OS&n;&t;&t;&t;&t;&t;     */
multiline_comment|/* These three don&squot;t need to be messed with unless you&squot;re trying to&n;   tweak the driver for optimal I/O performance. Read wavefront_wait()&n;   and wavefront_sleep() to see what they do. You may need or want to&n;   tweak them for CPU&squot;s different than the 486/66Mhz that I run on.&n;*/
DECL|variable|wf_short_wait_count
r_int
id|wf_short_wait_count
op_assign
l_int|5000
suffix:semicolon
multiline_comment|/* loops, CPU dependent */
DECL|variable|wf_sleep_interval
r_int
id|wf_sleep_interval
op_assign
l_int|50
suffix:semicolon
multiline_comment|/* HZ/wf_sleep_interval seconds per sleep */
DECL|variable|wf_sleep_tries
r_int
id|wf_sleep_tries
op_assign
l_int|100
suffix:semicolon
multiline_comment|/*2sec*/
...
id|installed
suffix:semicolon
multiline_comment|/* well, is it ? note: doesn&squot;t mean &quot;working&quot; */
DECL|member|devno
r_int
id|devno
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;WaveFront: no hardware associated with device %d.&bslash;n&quot;
comma
id|dev
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* This one is truly weird.  What kind of weirdo decided that in&n;&t;   a system dominated by 16- and 32-bit integers, they would use&n;&t;   a 12-bit transfer size ?&n;&t;*/
id|shptr
op_assign
id|munge_int32
(paren
...
(paren
id|KERN_ERR
l_string|&quot;WaveFront: synth_control with unknown &quot;
l_string|&quot;device number %d&bslash;n&quot;
comma
id|dev
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;WaveFront: close() called on non-existent dev %d&quot;
comma
id|dev
)paren
suffix:semicolon
...
(brace
multiline_comment|/*midi_synth_controller(hw-&gt;mididev,channel,7,volume);*/
id|midi_synth_aftertouch
c_func
(paren
id|hw-&gt;mididev
...
(paren
id|KERN_WARNING
l_string|&quot;WaveFront: bogus interrupt %d recv&squot;d&bslash;n&quot;
comma
id|irq
)paren
suffix:semicolon
...
(paren
id|KERN_ALERT
l_string|&quot;WaveFront: interrupt from unknown hw (irq=%d).&bslash;n&quot;
comma
id|irq
)paren
suffix:semicolon
...
multiline_comment|/* STATUS REGISTER &n;&n;0 Host Rx Interrupt Enable (1=Enabled)&n;1 Host Rx Register Full (1=Full)&n;2 Host Rx Interrupt Pending (1=Interrupt)&n;3 Unused&n;4 Host Tx Interrupt (1=Enabled)&n;5 Host Tx Register empty (1=Empty)&n;6 Host Tx Interrupt Pending (1=Interrupt)&n;7 Unused&n;&n;*/
multiline_comment|/* CONTROL REGISTER&n;0 Host Rx Interrupt Enable (1=Enabled)      0x1&n;1 Unused                                    0x2&n;2 Unused                                    0x4&n;3 Unused                                    0x8&n;4 Host Tx Interrupt Enable                 0x10&n;5 Mute (0=Mute; 1=Play)                    0x20&n;6 Master Interrupt Enable (1=Enabled)      0x40&n;7 Master Reset (0=Reset; 1=Run)            0x80&n;*/
r_int
...
(paren
id|KERN_WARNING
l_string|&quot;WaveFront: no device slots available (max = %d).&bslash;n&quot;
comma
id|WAVEFRONT_MAX_DEVICES
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;WaveFront: not raw, but no hardware version!&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
...
suffix:semicolon
multiline_comment|/* take us out of reset, unmute, master + TX + RX interrupts on */
id|outb
(paren
l_int|0x80
op_or
...
suffix:semicolon
multiline_comment|/* Data port is now the data port, not the h/w initialization port &n;     &n;&t;   The boot ROM will check the OSRAM, and will then&n;&t;   wait for the either the &quot;download OS&quot; or&n;&t;   &quot;report h/w version&quot; commands.&n;&n;&t;   Any other command will supposedly be ignored.&n;&t;*/
r_if
c_cond
(paren
op_logical_neg
...
l_int|1
suffix:semicolon
multiline_comment|/* WaveFront SDK says:&n;     &n;&t;   &quot;When the Master Reset is set to zero (0), the audio board is held&n;&t;   in reset, which is the power-up condition. Setting Master Reset to one&n;&t;   (1) allows the on-board processor to run. It takes approximately two&n;&t;   to four seconds, depending on the memory configuration, for the&n;&t;   on-board processor to complete it&squot;s initialization routine before it&n;&t;   will respond to commands after a reset.&quot;&n;&n;&t;   Actually, it seems that most of the time, even with 8MB of RAM,&n;&t;   its actually ready immediately.&n;&t;*/
r_if
c_cond
(paren
op_logical_neg
...
(paren
id|KERN_WARNING
l_string|&quot;WaveFront: OS not ready after &quot;
l_string|&quot;memory check.&bslash;n&quot;
)paren
suffix:semicolon
id|free_irq
(paren
...
multiline_comment|/* get H/W version, and check we get an interrupt */
id|outb
(paren
id|WFC_HARDWARE_VERSION
comma
...
suffix:semicolon
multiline_comment|/* We don&squot;t need the IRQ anymore for the WaveFront code,&n;&t;   and to allow an MPU-401 driver to attach to it later, lets&n;&t;   give it back ....&n;&n;&t;   DO NOT alter irq2hw[], since we&squot;ll still use this to lookup&n;&t;   the config struct from an address_info struct.&n;&t;*/
id|free_irq
(paren
id|hw_config-&gt;irq
comma
...
(paren
id|KERN_WARNING
l_string|&quot;WaveFront: interrupt not received after &quot;
l_string|&quot;h/w version cmd.&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
...
(paren
id|KERN_WARNING
l_string|&quot;WaveFront @ 0x%x not ready, ignoring&bslash;n&quot;
comma
id|hw-&gt;base
)paren
suffix:semicolon
...
(paren
id|KERN_WARNING
l_string|&quot;WaveFront @ 0x%x not responding correctly, ignoring&bslash;n&quot;
comma
id|hw-&gt;base
)paren
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* XXX what state does the board need to be in before&n;&t;   I can download the firmware ? I think any state&n;&t;   after it acknowledges a hardware version command&n;&t;   post-booting.&n;&t;*/
r_if
c_cond
(paren
id|hw-&gt;israw
...
multiline_comment|/* enter normal operation:&n;&t;&t;   bit 7: (on) reset  0x80&n;&t;&t;   bit 6: interrupts enabled 0x40&n;&t;&t;   bit 5: (on) mute (i.e. in play mode) 0x20&n;&t;&t;   bits 4-0: zero&n;&t;   &n;&t;&t;   note: tx and rx interrupts turned off&n;&t;&t;*/
id|outb
(paren
l_int|0x80
op_or
...
suffix:semicolon
multiline_comment|/* Set up MPU-401 emulation mode.  For some reason, this always&n;&t;&t;   fails the first time, and generates no ACK, so we&squot;ll&n;&t;&t;   treat it as a special case by sleeping for a while, and then&n;&t;&t;   trying again.&n;&t;&t;*/
id|wavefront_write
(paren
id|hw
comma
...
(paren
id|KERN_ERR
l_string|&quot;WaveFront: set MPU emulation mode &quot;
...
(paren
id|KERN_ERR
l_string|&quot;WaveFront: cannot attach unknown irq 0x%x.&bslash;n&quot;
comma
id|hw_config-&gt;irq
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;WaveFront: unloading unrecognized device!&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
...
macro_line|#endif CONFIG_SOUND_WAVEFRONT 
eof
