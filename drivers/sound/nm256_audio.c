...
l_int|0
suffix:semicolon
multiline_comment|/* The size of the playback reserve. */
...
multiline_comment|/* Ditto, but find the card struct corresponding to the mixer device DEV &n;   instead. */
r_static
r_struct
id|nm256_info
op_star
...
suffix:semicolon
multiline_comment|/* Set the card samplerate, word size and stereo mode to correspond to&n;   the settings in the CARD struct for the specified device in DEV.&n;   We keep two separate sets of information, one for each device; the&n;   hardware is not actually configured until a read or write is&n;   attempted. */
r_int
...
multiline_comment|/* Request one chunk of AMT bytes from the recording device.  When the&n;   operation is complete, the data will be copied into BUFFER and the&n;   function DMAbuf_inputintr will be invoked. */
r_static
r_void
...
(brace
multiline_comment|/* Of course this won&squot;t actually work right, because the&n;&t;   caller is going to assume we will give what we got asked&n;&t;   for. */
id|printk
(paren
id|KERN_ERR
...
multiline_comment|/* If we&squot;re not currently recording, set up the start and end registers&n;       for the recording engine. */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Ring buffers, man.  That&squot;s where the hip-hop, wild-n-wooly action&squot;s at.&n;   1972?&n;&n;   Write AMT bytes of BUFFER to the playback ring buffer, and start the&n;   playback engine running.  It will only accept up to 1/2 of the total&n;   size of the ring buffer.  */
r_static
r_void
...
suffix:semicolon
multiline_comment|/* Setup the start-n-stop-n-limit registers, and start that engine&n;       goin&squot;. &n;&n;       Normally we just let it wrap around to avoid the click-click&n;       action scene. */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* The PBUFFER_END register in this case points to one &quot;word&quot;&n;&t;   before the end of the buffer. */
r_int
id|w
op_assign
(paren
...
id|card-&gt;requested_amt
suffix:semicolon
multiline_comment|/* Get a new block to write.  This will eventually invoke&n;&t;   nm256_write_block ().  */
...
multiline_comment|/* Read the last-recorded block from the ring buffer, copy it into the&n;   saved buffer pointer, and invoke DMAuf_inputintr() with the recording&n;   device. */
r_static
r_void
...
multiline_comment|/* This test is probably redundant; we shouldn&squot;t be here unless&n;       it&squot;s true.  */
r_if
c_cond
(paren
id|card-&gt;recording
...
multiline_comment|/* Initialize the hardware and various other card data we&squot;ll need&n;   later. */
r_static
r_void
...
suffix:semicolon
multiline_comment|/* Disable recording. */
id|nm256_writePort8
(paren
id|card
comma
...
multiline_comment|/* Handle a potential interrupt for the device referred to by DEV_ID. */
r_static
r_void
...
multiline_comment|/* Handle a potential interrupt for the device referred to by DEV_ID.&n;   This handler is for the 256ZX.  */
r_static
r_void
...
multiline_comment|/* Request our interrupt. */
r_static
r_int
...
multiline_comment|/* Release our interrupt. */
r_static
r_int
...
multiline_comment|/* Reset the card.  &squot;Tis magic!  */
id|nm256_writePort8
(paren
id|card
comma
...
multiline_comment|/* We don&squot;t do anything special here.  */
r_static
r_int
...
suffix:semicolon
multiline_comment|/* I &quot;love&quot; C sometimes.  Got braces?  */
...
multiline_comment|/* See if the signature left by the NM256 BIOS is intact; if so, we use&n;   the associated address as the end of our buffer. */
r_static
r_void
...
multiline_comment|/* Install a driver for the soundcard referenced by PCIDEV. */
r_static
r_int
...
id|rev
suffix:semicolon
multiline_comment|/* The NM256 has two memory ports.  The first port is nothing&n;       more than a chunk of video RAM, which is used as the I/O ring&n;       buffer.  The second port has the actual juicy stuff (like the&n;       mixer and the playback engine control registers). */
id|card-&gt;ports
(braket
l_int|1
)braket
...
(paren
id|KERN_ERR
l_string|&quot;NM256: Unable to remap port 2&bslash;n&quot;
)paren
suffix:semicolon
id|kfree_s
(paren
...
(paren
id|KERN_ERR
l_string|&quot;NM256: Unable to remap port 1&bslash;n&quot;
)paren
suffix:semicolon
id|release_region
(paren
...
id|EIO
suffix:semicolon
multiline_comment|/* If it&squot;s open for both read and write, and the card&squot;s currently&n;       being read or written to, then do the opposite of what has&n;       already been done.  Otherwise, don&squot;t specify any mode until the&n;       user actually tries to do I/O. */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* Given the dev DEV and an associated physical buffer PHYSBUF, return&n;   a pointer to the actual buffer in kernel space. */
r_static
r_char
op_star
...
(paren
id|KERN_INFO
l_string|&quot;NeoMagic 256AV/256ZX audio driver, version 1.0&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
