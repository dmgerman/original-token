...
mdefine_line|#define TRIDENT_DMA_MASK&t;&t;0x3fffffff /* DMA buffer mask for pci_alloc_consist */
DECL|macro|NR_HW_CH
...
(brace
multiline_comment|/* FIXME: channel attributes are configured by ioctls, but it is not&n;&t;&t;   implemented so just set to ZERO for the moment */
id|channel-&gt;attribute
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/* FIXME: channel attributes are configured by ioctls, but it is not&n;&t;&t;   implemented so just set to 0x8a80 for the moment, record from PCM L/R&n;&t;&t;   input and mono = (left + right + 1)/2*/
id|channel-&gt;attribute
op_assign
l_int|0x8A80
suffix:semicolon
...
id|TRIDENT_FMT_16BIT
suffix:semicolon
id|state-&gt;dmabuf.ossfragshift
op_assign
l_int|0
suffix:semicolon
id|state-&gt;dmabuf.ossmaxfrags
op_assign
l_int|0
suffix:semicolon
id|state-&gt;dmabuf.subdivision
op_assign
l_int|0
suffix:semicolon
id|trident_set_dac_rate
...
(brace
multiline_comment|/* FIXME: Trident 4d can only record in singed 16-bits stereo, 48kHz sample,&n;&t;&t;   to be dealed with in trident_set_adc_rate() ?? */
id|state-&gt;dmabuf.fmt
op_and_assign
op_complement
id|TRIDENT_FMT_MASK
