...
id|cdo-&gt;capability
suffix:semicolon
multiline_comment|/* hack, gcc complains OK */
r_if
c_cond
(paren
id|major
...
multiline_comment|/* We need our own cdrom error types! This is a temporary solution. */
DECL|macro|ENOMEDIUM
mdefine_line|#define ENOMEDIUM EAGAIN&t;&t;&t;&t;/* no medium in removable device */
multiline_comment|/* We use the open-option O_NONBLOCK as in indicator of that the&n; * purpose of opening is only for subsequent ioctl() calls; no device&n; * integrity checks are performed.&n; *&n; * We hope that all cd-player programs will adopt this convention. It&n; * is in their own advantage: device conotrol becomes a lot easier&n; * this way.&n; */
r_int
id|open_for_data
c_func
(paren
...
multiline_comment|/* We want to make media_changed accessible to the user through an&n; * ioctl. The main problem now is that we must double-buffer the&n; * low-level implementation, to assure that the VFS and the user both&n; * see a medium change once.&n; *&n; * For now, i&squot;ve implemented only 16 minor devs (half a long), i have to&n; * think of a better solution... $Queue$ is either 0 or 1. Queue 0 is&n; * in the lower 16 bits, queue 1 in the higher 16 bits.&n; */
...
DECL|macro|current
macro_line|#undef current&t;&t;&t;&t;&t;&t;&t;/* set in sched.h */
...
multiline_comment|/* All checking and format change makes this code really hard to read!&n; * So let&squot;s make some check and memory move macros.  These macros are&n; * a little inenficient when used both in the same piece of code, as&n; * verify_area is used twice, but who cares, as ioctl() calls&n; * shouldn&squot;t be in inner loops.&n; */
DECL|macro|GETARG
...
(paren
id|KERN_INFO
l_string|&quot;Module cdrom removed&bslash;n&quot;
)paren
suffix:semicolon
)brace
macro_line|#endif
multiline_comment|/*&n; * Local variables:&n; * comment-column: 40&n; * compile-command: &quot;gcc -DMODULE -D__KERNEL__ -I/usr/src/linux-obj/include -Wall -Wstrict-prototypes -O2 -m486 -c cdrom.c -o cdrom.o&quot;&n; * End:&n; */
eof
