...
multiline_comment|/* Wait for OP_OK = drive answers with AFL_OP_OK after recieving a command*/
DECL|macro|OP_OK
...
multiline_comment|/* Wait for PA_OK = drive answers with AFL_PA_OK after recieving parameters*/
DECL|macro|PA_OK
...
multiline_comment|/* used in azt_poll to poll the status, expects another programm to issue a &n; * ACMD_GET_STATUS directly before &n; */
...
multiline_comment|/*              if (aztAudioStatus != CDROM_AUDIO_PLAY) return -EINVAL; &n;*/
r_if
c_cond
(paren
id|aztGetQChannelInfo
