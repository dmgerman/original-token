...
multiline_comment|/*&n; * Return the mode name for a drive transfer mode value:&n; */
DECL|function|strmode
r_static
r_const
r_char
op_star
...
suffix:semicolon
multiline_comment|/*&n;&t; * If this is a DMA mode setting, then turn off all DMA bits.&n;&t; * We will set one of them back on afterwards, if all goes well.&n;&t; *&n;&t; * Not sure why this is needed (it looks very silly),&n;&t; * but other IDE chipset drivers also do this fiddling.  ???? -ml&n; &t; */
r_switch
c_cond
(paren
id|mode
...
multiline_comment|/*&n;&t; * Select the drive, and issue the SETFEATURES command&n;&t; */
id|disable_irq
c_func
(paren
id|hwif-&gt;irq
...
multiline_comment|/*&n;&t; * Allow status to settle, then read it again.&n;&t; * A few rare drives vastly violate the 400ns spec here,&n;&t; * so we&squot;ll wait up to 10usec for a &quot;good&quot; status&n;&t; * rather than expensively fail things immediately.&n;&t; */
r_for
c_loop
(paren
id|i
...
suffix:semicolon
multiline_comment|/*&n;&t; * Turn dma bit on if all is okay&n;&t; */
r_if
c_cond
(paren
id|error
