...
multiline_comment|/*&n;&t; * This has become somewhat more complicated with the addition of&n;&t; * the page cache.  The problem is that in some cases the furnished&n;&t; * buffer is &quot;real&quot;, i.e., part of the existing ramdisk, while in&n;&t; * others it is &quot;unreal&quot;, e.g., part of a page.  In the first case&n;&t; * not much needs to be done, while in the second, some kind of&n;&t; * transfer is needed.&n; &t; *&n;&t; * The two cases are distinguished here by checking whether the&n;&t; * real buffer is already in the buffer cache, and whether it is&n;&t; * the same as the one supplied.&n;&t; *&n;&t; * There are three cases with read/write to consider:&n;&t; *&n;&t; * 1. Supplied buffer matched one in the buffer cache:&n;&t; *    Read - Clear the buffer, as it wasn&squot;t already valid.&n;&t; *    Write - Mark the buffer as &quot;Protected&quot;.&n;&t; *&n;&t; * 2. Supplied buffer mismatched one in the buffer cache:&n;&t; *    Read - Copy the data from the buffer cache entry.&n;&t; *    Write - Copy the data to the buffer cache entry.&n;&t; *&n;&t; * 3  No buffer cache entry existed:&n;&t; *    Read - Clear the supplied buffer, but do not create a real&n;&t; *    one.&n;&t; *    Write - Create a real buffer, copy the data to it, and mark&n;&t; *    it as &quot;Protected&quot;.&n;&t; *&n;&t; * NOTE: There seems to be some schizophrenia here - the logic&n;&t; * using &quot;len&quot; seems to assume arbitrary request lengths, while&n;&t; * the &quot;protect&quot; logic assumes a single buffer cache entry.&n;&t; * This seems to be left over from the ancient contiguous ramdisk&n;&t; * logic.&n;&t; */
id|sbh
op_assign
id|CURRENT-&gt;bh
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* No buffer, what to do here? */
)brace
)brace
r_if
c_cond
