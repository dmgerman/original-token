...
multiline_comment|/*&n; * do_hwgroup_request() invokes ide_do_request() after first masking&n; * all possible interrupts for the current hwgroup.  This prevents race&n; * conditions in the event that an unexpected interrupt occurs while&n; * we are in the driver.&n; *&n; * Note that the io-request lock will guarantee that the driver never gets&n; * re-entered even on another interrupt level, so we no longer need to&n; * mask the irq&squot;s.&n; */
...
c_func
(paren
l_string|&quot;%s%s: unexpected interrupt, status=0x%02x&bslash;n&quot;
comma
id|hwif-&gt;name
comma
(paren
...
c_func
(paren
l_string|&quot;ide_intr: got IRQ from wrong device: email mlord@pobox.com!!&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
