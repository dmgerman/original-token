...
suffix:semicolon
multiline_comment|/* hwif info */
DECL|variable|irq_to_hwgroup
r_static
id|ide_hwgroup_t
op_star
id|irq_to_hwgroup
...
l_int|0
suffix:semicolon
multiline_comment|/* &quot;serialize&quot; option */
DECL|variable|disallow_unmask
r_static
r_int
id|disallow_unmask
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* for buggy hardware */
...
DECL|function|ide_set_recovery_timer
r_void
...
macro_line|#if SUPPORT_HT6560B
multiline_comment|/*&n; * This routine handles interface switching for the peculiar hardware design&n; * on the F.G.I./Holtek HT-6560B VLB IDE interface.&n; * The HT-6560B can only enable one IDE port at a time, and requires a&n; * silly sequence (below) whenever we switch between primary and secondary.&n; *&n; * Apparently, systems with multiple CMD640 chips may need something similar..&n; *&n; * This algorithm courtesy of malafoss@snakemail.hut.fi&n; */
DECL|function|ide_hwif_select
r_void
id|ide_hwif_select
(paren
id|ide_hwif_t
...
macro_line|#endif /* SUPPORT_HT6560B */
...
c_func
(paren
l_string|&quot;%s: ide_set_handler: old handler not null&bslash;n&quot;
comma
id|drive-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/*&n;&t; * The partition check in genhd.c needs this string to identify&n;&t; * our minor devices by name for display purposes.&n;&t; * Note that doing this will prevent us from working correctly&n;&t; * if ever called a second time for this major (never happens).&n;&t; */
id|gd-&gt;real_devices
op_assign
id|hwif-&gt;drives
(braket
...
suffix:semicolon
multiline_comment|/*&n; * reset_ihandler() is a dummy interrupt handler which we install during&n; * an ide interface reset operation.  This prevents other devices in the&n; * same hwgroup from being serviced while we play around with shared resources.&n; * If it ever gets invoked, we call unexpected_intr(), which treats the event&n; * the same as a timer_expiry().&n; */
DECL|function|reset_ihandler
r_static
r_void
id|reset_ihandler
(paren
...
multiline_comment|/*&n; * start_reset_timer() sets up a timer event for 50ms in the future,&n; * to poll for completion of an ide reset operation (no interrupt to help us).&n; */
DECL|function|start_reset_timer
r_static
r_void
...
id|WAIT_WORSTCASE
suffix:semicolon
multiline_comment|/* max waiting time */
id|hwgroup-&gt;handler
op_assign
op_amp
id|reset_ihandler
suffix:semicolon
multiline_comment|/* dummy irq handler */
id|hwgroup-&gt;timer.expires
op_assign
id|jiffies
op_plus
...
id|OK_TO_RESET_CONTROLLER
suffix:semicolon
multiline_comment|/* 1 = we are waiting, 0 = done */
)brace
...
multiline_comment|/*&n; * write_intr() is the handler for disk multwrite interrupts&n; */
...
c_func
(paren
l_string|&quot;%s: ide_do_request: current=0x%08lx&bslash;n&quot;
comma
id|hwif-&gt;name
comma
(paren
...
c_func
(paren
l_string|&quot;%s: [EZD] accessing sector 1 instead of sector 0&bslash;n&quot;
comma
id|drive-&gt;name
)paren
suffix:semicolon
...
(brace
multiline_comment|/* ide reset in progress? */
r_if
c_cond
(paren
op_logical_neg
...
r_else
(brace
multiline_comment|/* drive not responding */
id|hwgroup-&gt;handler
op_assign
l_int|NULL
suffix:semicolon
...
c_func
(paren
l_string|&quot;ide: probable bad entry for /dev/hd%c%d&bslash;n&quot;
comma
l_char|&squot;a&squot;
op_plus
id|unit
...
c_func
(paren
l_string|&quot;%s: VLB_SYNC not supported by this kernel&bslash;n&quot;
comma
id|drive-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/* check for removeable disks (eg. SYQUEST), ignore &squot;WD&squot; drives */
r_if
c_cond
(paren
id|id-&gt;config
...
(brace
multiline_comment|/* removeable disk ? */
r_if
c_cond
(paren
id|id-&gt;model
...
suffix:semicolon
r_else
multiline_comment|/* Mmmm.. multiple IRQs */
id|printk
c_func
(paren
...
DECL|function|probe_for_drives
r_static
r_void
id|probe_for_drives
(paren
...
suffix:semicolon
macro_line|#if (MAX_DRIVES &gt; 2)
id|printk
c_func
(paren
l_string|&quot;%s: probing for first 2 of %d possible drives&bslash;n&quot;
comma
id|hwif-&gt;name
comma
id|MAX_DRIVES
...
multiline_comment|/*&n;&t;&t; * Second drive should only exist if first drive was found,&n;&t;&t; * but a lot of cdrom drives seem to be configured as slave-only&n;&t;&t; */
r_for
c_loop
(paren
id|unit
...
(brace
multiline_comment|/* note the hardcoded &squot;2&squot; */
id|ide_drive_t
op_star
id|drive
op_assign
...
macro_line|#endif /* SET_DTC2278_MODE4 */
...
suffix:semicolon
macro_line|#if SET_DTC2278_MODE4
multiline_comment|/*&n;&t; * This enables PIO mode4 (3?) on the first interface&n;&t; */
id|sub22
c_func
(paren
l_int|1
...
macro_line|#ifdef SUPPORT_QD6580
multiline_comment|/*&n; * QDI QD6580 EIDE controller fast support by Colten Edwards.&n; * no net access but I can be reached at pje120@cs.usask.ca&n; *&n; * I suppose that a IOCTL could be used for this and other&n; * cards like it to modify the speed using hdparm.  Someday..&n; */
...
id|flags
suffix:semicolon
multiline_comment|/* looks like   0x4f is fast&n;&t; *              0x3f is medium&n;&t; *              0x2f is slower&n;&t; *              0x1f is slower yet&n;&t; *              ports are 0xb0 0xb2 and 0xb3&n;&t; */
id|save_flags
c_func
(paren
id|flags
...
suffix:semicolon
multiline_comment|/* select &quot;fast&quot; 0x4f */
id|inb
c_func
(paren
l_int|0x3f6
...
multiline_comment|/*&n; * ide_setup() gets called VERY EARLY during initialization,&n; * to handle kernel &quot;command line&quot; strings beginning with &quot;hdx=&quot;&n; * or &quot;ide&quot;.  Here is the complete set currently supported:&n; *&n; * &quot;hdx=&quot;  is recognized for all &quot;x&quot; from &quot;a&quot; to &quot;h&quot;, such as &quot;hdc&quot;.&n; * &quot;idex=&quot; is recognized for all &quot;x&quot; from &quot;0&quot; to &quot;3&quot;, such as &quot;ide1&quot;.&n; *&n; * &quot;hdx=noprobe&quot;&t;: drive may be present, but do not probe for it&n; * &quot;hdx=nowerr&quot;&t;&t;: ignore the WRERR_STAT bit on this drive&n; * &quot;hdx=cdrom&quot;&t;&t;: drive is present, and is a cdrom drive&n; * &quot;hdx=cyl,head,sect&quot;&t;: disk drive is present, with specified geometry&n; *&n; * &quot;idex=noprobe&quot;&t;: do not attempt to access/use this interface&n; * &quot;idex=base&quot;&t;&t;: probe for an interface at the addr specified,&n; *&t;&t;&t;&t;where &quot;base&quot; is usually 0x1f0 or 0x170&n; *&t;&t;&t;&t;and &quot;ctl&quot; is assumed to be &quot;base&quot;+0x206&n; * &quot;idex=base,ctl&quot;&t;: specify both base and ctl&n; * &quot;idex=base,ctl,irq&quot;&t;: specify base, ctl, and irq number&n; *&n; * The following two are valid ONLY on ide0 or ide1,&n; * and the defaults for the base,ctl ports must not be altered.&n; *&n; * &quot;idex=serialize&quot;&t;: do not overlap operations on ide0 and ide1.&n; * &quot;idex=dtc2278&quot;&t;: enables use of DTC2278 secondary i/f&n; * &quot;idex=ht6560b&quot;&t;: enables use of HT6560B secondary i/f&n; * &quot;idex=cmd640_vlb&quot;&t;: required for VLB cards with the CMD640 chip&n; *&t;&t;&t;  (not for PCI -- automatically detected)&n; *&n; * This option is valid ONLY on ide0, and the defaults for the base,ctl ports&n; * must not be altered.&n; *&n; * &quot;ide0=qd6580&quot;&t;: select &quot;fast&quot; interface speed on a qd6580 interface&n; */
...
c_func
(paren
l_string|&quot; -- USE &bslash;&quot;ide%c=serialize&bslash;&quot; INSTEAD&quot;
comma
l_char|&squot;0&squot;
op_plus
id|hw
...
id|bad_hwif
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * Using 0x1c and 0x1d apparently selects a&n;&t;&t;&t;&t; * faster interface speed than 0x3c and 0x3d.&n;&t;&t;&t;&t; *&n;&t;&t;&t;&t; * Need to add an ioctl to select between them.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|check_region
...
c_func
(paren
l_string|&quot; -- HT6560 PORT 0x3e6 ALREADY IN USE&quot;
)paren
suffix:semicolon
r_goto
id|done
...
multiline_comment|/*&n; * This routine is called from the partition-table code in genhd.c&n; * to &quot;convert&quot; a drive to a logical geometry with fewer than 1024 cyls&n; * It mimics the method used by Ontrack Disk Manager.&n; */
...
multiline_comment|/*&n; * This routine sets up the irq for an ide interface, and creates a new&n; * hwgroup for the irq/hwif if none was previously assigned.&n; *&n; * The SA_INTERRUPT in sa_flags means ide_intr() is always entered with&n; * interrupts completely disabled.  This can be bad for interrupt latency,&n; * but anything else has led to problems on some machines.  We re-enable&n; * interrupts as much as we can safely do in most places.&n; */
...
suffix:semicolon
multiline_comment|/*&n;&t; * Grab the irq if we don&squot;t already have it from a previous hwif&n;&t; */
r_if
c_cond
(paren
id|hwgroup
...
id|hwgroup
suffix:semicolon
multiline_comment|/*&n;&t; * If this is the first interface in a group,&n;&t; * then we need to create the hwgroup structure&n;&t; */
r_if
c_cond
(paren
id|hwgroup
...
macro_line|#if SUPPORT_RZ1000 || SUPPORT_CMD640
DECL|function|buggy_interface_fallback
r_void
...
macro_line|#endif /* SUPPORT_RZ1000 || SUPPORT_CMD640 */
macro_line|#if SUPPORT_RZ1000
...
suffix:semicolon
multiline_comment|/*&n;&t; * First, we determine what hardware is present&n;&t; */
macro_line|#ifdef CONFIG_PCI
...
multiline_comment|/*&n;&t; * Now we try to set up irqs and major devices for what was found&n;&t; */
r_for
c_loop
(paren
id|h
