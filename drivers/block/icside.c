...
multiline_comment|/*&n; * SG-DMA support.&n; *&n; * Similar to the BM-DMA, but we use the RiscPCs IOMD&n; * DMA controllers.  There is only one DMA controller&n; * per card, which means that only one drive can be&n; * accessed at one time.  NOTE! We do not inforce that&n; * here, but we rely on the main IDE driver spotting&n; * that both interfaces use the same IRQ, which should&n; * guarantee this.&n; *&n; * We are limited by the drives IOR/IOW pulse time.&n; * The closest that we can get to the requirements is&n; * a type C cycle for both mode 1 and mode 2.  However,&n; * this does give a burst of 8MB/s.&n; *&n; * This has been tested with a couple of Conner&n; * Peripherals 1080MB CFS1081A drives, one on each&n; * interface, which deliver about 2MB/s each.  I&n; * believe that this is limited by the lack of&n; * on-board drive cache.&n; */
DECL|macro|TABLE_SIZE
mdefine_line|#define TABLE_SIZE 2048
r_static
r_int
...
r_static
r_int
DECL|function|icside_config_drive
id|icside_config_drive
c_func
(paren
id|ide_drive_t
op_star
...
suffix:semicolon
multiline_comment|/* Enable DMA on any drive that has&n;&t;&t; * UltraDMA (mode 0/1/2) enabled&n;&t;&t; */
r_if
c_cond
(paren
id|id-&gt;field_valid
...
l_int|2
suffix:semicolon
multiline_comment|/* Enable DMA on any drive that has mode1&n;&t;&t; * or mode2 multiword DMA enabled&n;&t;&t; */
r_if
c_cond
(paren
id|id-&gt;field_valid
...
r_int
r_int
DECL|function|icside_alloc_dmatable
id|icside_alloc_dmatable
c_func
(paren
r_void
)paren
...
(brace
macro_line|#if PAGE_SIZE == TABLE_SIZE * 2
id|dmatable
op_assign
id|__get_free_pages
c_func
