...
DECL|macro|MAX_LINEAR_SECTORS
mdefine_line|#define MAX_LINEAR_SECTORS 128
DECL|macro|MAX
...
id|ptr
suffix:semicolon
multiline_comment|/*&n;&t; * now we are rather fault tolerant than nice, but&n;&t; * there are a couple of places in the RAID code where we&n;&t; * simply can not afford to fail an allocation because&n;&t; * there is no failure return path (eg. make_request())&n;&t; */
r_while
c_loop
(paren
op_logical_neg
...
id|printk
(paren
l_string|&quot;raid1: out of memory, retrying...&bslash;n&quot;
)paren
suffix:semicolon
id|memset
c_func
...
id|page
suffix:semicolon
multiline_comment|/*&n;&t; * now we are rather fault tolerant than nice, but&n;&t; * there are a couple of places in the RAID code where we&n;&t; * simply can not afford to fail an allocation because&n;&t; * there is no failure return path (eg. make_request())&n;&t; * FIXME: be nicer here.&n;&t; */
r_while
c_loop
(paren
op_logical_neg
...
id|printk
(paren
l_string|&quot;raid1: GFP out of memory, retrying...&bslash;n&quot;
)paren
suffix:semicolon
id|schedule_timeout
c_func
...
(brace
singleline_comment|// FIXME: make us a regular bcache member
id|kfree
c_func
(paren
id|bh
...
id|WRITERAW
suffix:semicolon
multiline_comment|/*&n;&t;&t; * we first clean the bh, then we start the IO, then&n;&t;&t; * when the IO has finished, we end_io the bh and&n;&t;&t; * mark it uptodate. This way we do not miss the&n;&t;&t; * case when the bh got dirty again during the IO.&n;&t;&t; *&n;&t;&t; * We do an important optimization here - if the&n;&t;&t; * buffer was not dirty and we are during resync or&n;&t;&t; * reconstruction, then we can skip writing it back&n;&t;&t; * to the master disk! (we still have to write it&n;&t;&t; * back to the other disks, because we are not sync&n;&t;&t; * yet.)&n;&t;&t; */
r_if
c_cond
(paren
...
id|sectors
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Do not switch disks if full resync is in progress ...&n;&t;&t; */
r_if
c_cond
(paren
id|switch_disks
...
id|mirror-&gt;next
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Do not switch to write-only disks ...&n;&t;&t;&t; * reconstruction is in progress&n;&t;&t;&t; */
r_while
c_loop
(paren
id|conf-&gt;mirrors
...
multiline_comment|/*&n;&t; * Only if everything went Ok.&n;&t; */
id|conf-&gt;resync_mirrors
op_assign
l_int|0
suffix:semicolon
...
id|conf-&gt;window
suffix:semicolon
singleline_comment|// Note: falling of the end is not a problem
id|conf-&gt;phase
op_assign
id|conf-&gt;phase
op_xor
...
id|j
suffix:semicolon
multiline_comment|/*&n;&t; * initialize the &squot;working disks&squot; list.&n;&t; */
r_for
c_loop
(paren
id|i
