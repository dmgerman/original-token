...
multiline_comment|/*&n; * The pool is stirred with a primitive polynomial of degree 128&n; * over GF(2), namely x^128 + x^119 + x^72 + x^64 + x^14 + x^8 + 1.&n; * For a pool of size 64, try x^64+x^62+x^38+x^10+x^6+x+1.&n; */
...
DECL|macro|TAP1
mdefine_line|#define TAP1    119     /* The polynomial taps */
DECL|macro|TAP2
mdefine_line|#define TAP2    72
DECL|macro|TAP3
mdefine_line|#define TAP3    64
DECL|macro|TAP4
mdefine_line|#define TAP4    14
DECL|macro|TAP5
mdefine_line|#define TAP5    8
...
r_return
suffix:semicolon
multiline_comment|/*&n;&t; * If kamlloc returns null, we just won&squot;t use that entropy&n;&t; * source.&n;&t; */
id|irq_timer_state
(braket
id|irq
)braket
...
multiline_comment|/*&n; * This function adds a byte into the entropy &quot;pool&quot;.  It does not&n; * update the entropy estimate.  The caller must do this if appropriate.&n; *&n; * The pool is stirred with a primitive polynomial of degree 128&n; * over GF(2), namely x^128 + x^119 + x^72 + x^64 + x^14 + x^8 + 1.&n; * For a pool of size 64, try x^64+x^62+x^38+x^10+x^6+x+1.&n; * &n; * We rotate the input word by a changing number of bits, to help&n; * assure that all bits in the entropy get toggled.  Otherwise, if we&n; * consistently feed the entropy pool small numbers (like jiffies and&n; * scancodes, for example), the upper bits of the entropy pool don&squot;t&n; * get affected. --- TYT, 10/11/95&n; */
