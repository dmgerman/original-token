...
DECL|macro|BH_TYPE_RECEIVE_DATA
mdefine_line|#define BH_TYPE_RECEIVE_DATA&t;1
DECL|macro|BH_TYPE_RECEIVE_STATUS
mdefine_line|#define BH_TYPE_RECEIVE_STATUS&t;2
DECL|macro|BH_TYPE_RECEIVE_DMA
mdefine_line|#define BH_TYPE_RECEIVE_DMA&t;3
DECL|macro|BH_TYPE_TRANSMIT_DATA
mdefine_line|#define BH_TYPE_TRANSMIT_DATA&t;4
DECL|macro|BH_TYPE_TRANSMIT_STATUS
mdefine_line|#define BH_TYPE_TRANSMIT_STATUS&t;5
DECL|macro|BH_TYPE_STATUS
mdefine_line|#define BH_TYPE_STATUS&t;&t;6
DECL|struct|_BH_EVENT
r_typedef
r_struct
id|_BH_EVENT
(brace
...
id|type
suffix:semicolon
multiline_comment|/* Set by interrupt routines to reqst */
DECL|member|status
id|u16
id|status
suffix:semicolon
...
id|link
suffix:semicolon
DECL|typedef|BH_EVENT
DECL|typedef|BH_QUEUE
)brace
id|BH_EVENT
comma
op_star
id|BH_QUEUE
suffix:semicolon
multiline_comment|/* Queue of BH actions to be done.  */
DECL|macro|MAX_BH_QUEUE_ENTRIES
mdefine_line|#define MAX_BH_QUEUE_ENTRIES 200
...
DECL|member|bh_queue
id|BH_EVENT
id|bh_queue
(braket
id|MAX_BH_QUEUE_ENTRIES
)braket
suffix:semicolon
multiline_comment|/* Pointer to alloc&squot;ed block */
DECL|member|bh_queue_head
id|BH_QUEUE
id|bh_queue_head
suffix:semicolon
multiline_comment|/* Queue of BH actions */
DECL|member|bh_queue_tail
id|BH_QUEUE
id|bh_queue_tail
suffix:semicolon
multiline_comment|/* Tail of above for perf. */
DECL|member|free_bh_queue_head
id|BH_QUEUE
id|free_bh_queue_head
suffix:semicolon
multiline_comment|/* Queue of Free BH */
DECL|member|free_bh_queue_tail
id|BH_QUEUE
id|free_bh_queue_tail
suffix:semicolon
multiline_comment|/* Tail of above for perf. */
DECL|member|bh_action
id|BH_QUEUE
id|bh_action
suffix:semicolon
multiline_comment|/* Action for BH */
...
multiline_comment|/* mgsl_format_bh_queue()&n; * &n; * &t;Initialize the bottom half processing queue&n; * &n; * Arguments:&t;&t;info&t;pointer to device instance data&n; * Return Value:&t;None&n; */
DECL|function|mgsl_format_bh_queue
r_void
id|mgsl_format_bh_queue
c_func
(paren
...
id|i
suffix:semicolon
multiline_comment|/* go through sequentially tacking the little bits together */
r_for
c_loop
(paren
id|i
...
c_cond
(paren
id|info-&gt;free_bh_queue_tail
op_eq
l_int|NULL
)paren
id|info-&gt;free_bh_queue_head
op_assign
id|bh_queue
suffix:semicolon
r_else
id|info-&gt;free_bh_queue_tail-&gt;link
op_assign
id|bh_queue
suffix:semicolon
id|info-&gt;free_bh_queue_tail
...
multiline_comment|/* As a safety measure, mark the end of the chain with a NULL */
id|info-&gt;free_bh_queue_tail-&gt;link
op_assign
l_int|NULL
suffix:semicolon
)brace
multiline_comment|/* end of mgsl_format_bh_queue() */
multiline_comment|/* mgsl_bh_queue_put()&n; * &n; * &t;Add a BH event to the BH queue&n; * &n; * Arguments:&t;&t;info&t;&t;pointer to device instance data&n; * &t;&t;&t;type&t;BH event type&n; * &t;&t;&t;status&t;&t;BH event status&n; * &n; * Return Value:&t;None&n; */
DECL|function|mgsl_bh_queue_put
r_void
id|mgsl_bh_queue_put
c_func
(paren
...
(brace
multiline_comment|/* remove free element from head of free list */
id|info-&gt;free_bh_queue_head
op_assign
id|event-&gt;link
suffix:semicolon
...
l_int|NULL
suffix:semicolon
multiline_comment|/* file out new BH event */
id|event-&gt;type
op_assign
id|type
suffix:semicolon
...
id|status
suffix:semicolon
multiline_comment|/* add element to tail of pending list */
r_if
c_cond
(paren
id|info-&gt;bh_queue_head
op_ne
l_int|NULL
)paren
(brace
multiline_comment|/* BH queue is not empty, add current element to tail */
id|info-&gt;bh_queue_tail-&gt;link
op_assign
id|event
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* the BH queue is empty so this element becomes the head of queue */
id|info-&gt;bh_queue_head
op_assign
id|event
suffix:semicolon
)brace
multiline_comment|/* the new element becomes tail of queue */
id|info-&gt;bh_queue_tail
op_assign
id|event
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* No more free BH action elements in queue. */
multiline_comment|/* This happens when too many interrupts are occuring */
multiline_comment|/* for the mgsl_bh_handler to process so set a flag. */
...
multiline_comment|/* end of mgsl_bh_queue_put() */
multiline_comment|/* mgsl_bh_queue_get()&n; * &n; *&t;Free the current work item (if any) and get the&n; * &t;next work item from the head of the pending work item queue.&n; *&n; * Effects:&n; * &n; * &t;If a BH action element is available on the BH action queue&n; * &t;then the head of the queue is removed and bh_action&n; * &t;is set to point to the removed element.&n; * &n; * Arguments:&t;&t;info&t;pointer to device instance data&n; * Return Value:&t;1 if BH action removed from queue&n; */
DECL|function|mgsl_bh_queue_get
r_int
id|mgsl_bh_queue_get
c_func
(paren
...
(brace
multiline_comment|/* free the current work item */
r_if
c_cond
(paren
id|info-&gt;free_bh_queue_head
...
(brace
multiline_comment|/* free queue is not empty, add current element to tail */
id|info-&gt;free_bh_queue_tail-&gt;link
op_assign
id|info-&gt;bh_action
suffix:semicolon
...
r_else
(brace
multiline_comment|/* free queue is empty so this element becomes the head of queue */
id|info-&gt;free_bh_queue_head
op_assign
id|info-&gt;bh_action
suffix:semicolon
)brace
multiline_comment|/* add element to tail of free queue */
id|info-&gt;free_bh_queue_tail
op_assign
id|info-&gt;bh_action
suffix:semicolon
...
multiline_comment|/* attempt to remove element from head of queue */
id|info-&gt;bh_action
op_assign
id|info-&gt;bh_queue_head
suffix:semicolon
...
(brace
multiline_comment|/* BH queue is not empty, remove element from queue head */
id|info-&gt;bh_queue_head
op_assign
id|info-&gt;bh_action-&gt;link
suffix:semicolon
id|spin_unlock_irqrestore
c_func
(paren
...
multiline_comment|/* end of mgsl_bh_queue_get() */
multiline_comment|/* mgsl_bh_handler()&n; * &n; * &t;Perform bottom half processing of work items queued by ISR.&n; *&n; * Arguments:&t;&t;Context&t;&t;pointer to device instance data&n; * Return Value:&t;None&n; */
...
l_int|1
suffix:semicolon
multiline_comment|/* Attempt to clear out the BH queue */
r_while
c_loop
(paren
id|mgsl_bh_queue_get
...
id|info
comma
id|info-&gt;bh_action-&gt;status
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|BH_TYPE_TRANSMIT_STATUS
suffix:colon
r_case
id|BH_TYPE_TRANSMIT_DATA
suffix:colon
id|mgsl_bh_transmit_data
c_func
(paren
...
c_func
(paren
l_string|&quot;ISR overflow detected.&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_if
...
multiline_comment|/* end of mgsl_bh_handler() */
multiline_comment|/* mgsl_bh_receive_dma()&n; * &n; * &t;Perform bottom half processing for a receive DMA interrupt&n; * &t;This occurs in HDLC mode after a DMA buffer has terminated&n; * &t;or the DMA buffers have been exhausted.&n; * &n; * Arguments:&n; * &n; * &t;info&t;&t;pointer to device instance data&n; * &t;status&t;&t;status word&n; * &n; * Return Value:&t;None&n; */
DECL|function|mgsl_bh_receive_dma
r_void
id|mgsl_bh_receive_dma
c_func
(paren
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_bh_receive_dma(%s)&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
multiline_comment|/* end of mgsl_bh_receive_dma() */
multiline_comment|/* mgsl_bh_transmit_data()&n; * &n; * &t;Process a transmit data interrupt event&n; * &t;This occurs in asynchronous communications mode.&n; * &n; * Arguments:&t;&t;info&t;pointer to device instance data&n; * Return Value:&t;None&n; */
DECL|function|mgsl_bh_transmit_data
r_void
id|mgsl_bh_transmit_data
c_func
(paren
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_bh_transmit_data() entry on %s&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
suffix:semicolon
multiline_comment|/* wakeup any waiting write requests */
r_if
c_cond
(paren
id|tty
...
multiline_comment|/* End Of mgsl_bh_transmit_data() */
multiline_comment|/* mgsl_bh_status_handler()&n; * &n; * &t;Peform bottom half processing for a status interrupt&n; * &n; * &t;This event is generated when a I/O pin (serial signal)&n; * &t;has a transition. If there is a pending WaitEvent call&n; * &t;and the status transition is identified in the EventMast&n; * &t;of the pending call then complete the pending call.&n; * &n; * Arguments:&n; * &n; * &t;info&t;&t;pointer to device instance data&n; * &t;status&t;&t;status word&n; * &n; * Return Value:&t;None&n; */
DECL|function|mgsl_bh_status_handler
r_void
id|mgsl_bh_status_handler
c_func
(paren
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_bh_status_handler() entry on %s&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
multiline_comment|/* End Of mgsl_bh_status_handler() */
...
multiline_comment|/* end of mgsl_isr_receive_data() */
...
suffix:semicolon
multiline_comment|/* Post a receive event for BH processing. */
id|mgsl_bh_queue_put
c_func
(paren
id|info
...
suffix:semicolon
multiline_comment|/* Interrupt overflow. Reset adapter and exit. */
singleline_comment|//&t;&t;&t;&t;UscReset(info);
...
multiline_comment|/* mgsl_change_params()&n; *&n; *&t;Reconfigure adapter based on new parameters&n; *&n; * Arguments:&t;&t;info&t;pointer to device instance data&n; * Return Value:&t;None&n; */
...
multiline_comment|/* reprogram the hardware */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* enable modem signal IRQs and read initial signal states */
...
multiline_comment|/* mgsl_wait_event() &t;wait for specified event to occur&n; * &t;&n; * Arguments:&t; &t;info&t;pointer to device instance data&n; * &t;&t;&t;mask&t;bitmask of events to wait for&n; * Return Value:&t;bit mask of triggering event, otherwise error code&n; */
...
id|ret
comma
l_string|&quot;txactive=%d bh_req=%d bh_run=%d bh_q=%p&bslash;n&quot;
comma
id|info-&gt;tx_active
comma
id|info-&gt;bh_requested
...
id|ret
comma
l_string|&quot;irq_spinlock=%08X&bslash;n&quot;
comma
id|info-&gt;irq_spinlock.lock
)paren
suffix:semicolon
macro_line|#endif
r_return
...
(brace
multiline_comment|/* claim 16C32 I/O base address */
r_if
c_cond
(paren
id|check_region
...
l_int|1
suffix:semicolon
multiline_comment|/* claim interrupt level */
r_if
c_cond
(paren
id|request_irq
...
(brace
multiline_comment|/* claim shared memory range */
id|info-&gt;memory_base
op_assign
id|ioremap
c_func
...
multiline_comment|/* test the shared memory range */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* claim LCR memory range */
id|info-&gt;lcr_base
op_assign
id|ioremap
c_func
...
multiline_comment|/* mgsl_release_resources()&n; * &n; * &t;Release all resources used by a device&n; * &t;&n; * Arguments:&t;&t;info&t;pointer to device instance data&n; * Return Value:&t;None&n; */
...
multiline_comment|/* mgsl_enumerate_devices()&n; * &n; * &t;Enumerate SyncLink serial devices based on user specified&n; *&t;options for ISA adapters and autodetected PCI adapters.&n; * &t;&n; * Arguments:&t;&t;None&n; * Return Value:&t;0 if success, otherwise error code&n; */
DECL|function|mgsl_enumerate_devices
r_int
id|mgsl_enumerate_devices
c_func
(paren
...
multiline_comment|/* Auto detect PCI adapters */
r_if
c_cond
(paren
id|pcibios_present
...
c_func
(paren
id|MICROGATE_VENDOR_ID
comma
id|SYNCLINK_DEVICE_ID
comma
id|i
comma
op_amp
...
c_func
(paren
l_string|&quot;%s(%d):Shared mem addr not set.&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
c_func
(paren
l_string|&quot;%s(%d):LCR mem addr not set.&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
c_func
(paren
l_string|&quot;%s(%d):USC I/O addr not set.&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
suffix:semicolon
multiline_comment|/* Because veremap only works on page boundaries we must map&n;&t;&t;&t;&t; * a larger area than is actually implemented for the LCR&n;&t;&t;&t;&t; * memory range. We map a full page starting at the page boundary.&n;&t;&t;&t;&t; */
id|info-&gt;phys_lcr_base
op_assign
id|lcr_mem_base
op_amp
...
macro_line|#endif
multiline_comment|/*&n;&t; * Allocate memory to hold the following tty/termios arrays&n;&t; * with an element for each enumerated device.&n;&t; */
id|serial_table
op_assign
(paren
r_struct
...
c_func
(paren
l_string|&quot;%s(%d):Can&squot;t allocate tty/termios arrays.&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
multiline_comment|/* end of mgsl_enumerate_devices() */
...
suffix:semicolon
multiline_comment|/* determine how many SyncLink devices are installed */
id|mgsl_enumerate_devices
c_func
(paren
)paren
...
multiline_comment|/* end of mgsl_init() */
...
multiline_comment|/* end of cleanup_module() */
...
multiline_comment|/* 60ns each for a maximum latency of 240ns on the&t;&t; */
multiline_comment|/* local bus.&t;&t;&t;&t;&t;&t;&t;&t;&t;&t;&t;&t;&t;&t; */
multiline_comment|/*******************************************************/
