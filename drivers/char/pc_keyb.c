...
multiline_comment|/*&n; * Wait for keyboard controller input buffer is empty.&n; *&n; * Don&squot;t use &squot;jiffies&squot; so that we don&squot;t depend on&n; * interrupts..&n; *&n; * Quote from PS/2 System Reference Manual:&n; *&n; * &quot;Address hex 0060 and address hex 0064 should be written only when&n; * the input-buffer-full bit and output-buffer-full bit in the&n; * Controller Status register are set 0.&quot;&n; */
...
multiline_comment|/* #define disable_keyboard()&t;do { send_cmd(KBD_CCMD_KBD_DISABLE); kb_wait(); } while (0) */
DECL|macro|disable_keyboard
mdefine_line|#define disable_keyboard()&t;send_cmd(KBD_CCMD_KBD_DISABLE)
DECL|macro|enable_keyboard
mdefine_line|#define enable_keyboard()&t;send_cmd(KBD_CCMD_KBD_ENABLE)
macro_line|#else
DECL|macro|disable_keyboard
