...
multiline_comment|/*&n; * This function is called when a user space process attempts to open the&n; * device. If the driver is compiled into the kernel it does nothing but&n; * succeed, but if it is compiled in as a module it also increments the&n; * module usage count to prevent the module from being removed whilst a&n; * process has the device open.&n; */
DECL|function|button_open
r_static
r_int
id|button_open
(paren
...
multiline_comment|/*&n; * This function is called when a user space process attempts to close the&n; * device. If the driver is compiled into the kernel it does nothing at all,&n; * but if it is compiled in as a module it also decrements the module usage&n; * count so that it will be possible to unload the module again once all the&n; * user processes have closed the device.&n; */
DECL|function|button_release
r_static
r_int
id|button_release
(paren
