...
multiline_comment|/*&n; *&t;console.c&n; *&n; * This module exports the console io functions:&n; *&n; *     &squot;int vc_allocate(unsigned int console)&squot;&n; *     &squot;int vc_cons_allocated(unsigned int console)&squot;&n; *     &squot;int vc_resize(unsigned long lines, unsigned long cols)&squot;&n; *     &squot;int vc_resize_con(unsigned long lines, unsigned long cols,&n; *&t;&t;&t;  unsigned int currcons)&squot;&n; *     &squot;void vc_disallocate(unsigned int currcons)&squot;&n; *&n; *     &squot;unsigned long con_init(unsigned long)&squot;&n; *     &squot;int con_open(struct tty_struct *tty, struct file * filp)&squot;&n; *     &squot;void con_write(struct tty_struct * tty)&squot;&n; *     &squot;void vt_console_print(struct console *co, const char * b,&n; *                            unsigned count)&squot;&n; *     &squot;void update_screen(int new_console)&squot;&n; *&n; *     &squot;void do_blank_screen(int)&squot;&n; *     &squot;void do_unblank_screen(void)&squot;&n; *     &squot;void poke_blanked_console(void)&squot;&n; *&n; *     &squot;unsigned short *screen_pos(int currcons, int w_offset, int viewed)&squot;&n; *     &squot;void complement_pos(int currcons, int offset)&squot;&n; *     &squot;void invert_screen(int currcons, int offset, int count, int shift)&squot;&n; *&n; *     &squot;void scrollback(int lines)&squot;&n; *     &squot;void scrollfront(int lines)&squot;&n; *&n; *     &squot;void mouse_report(struct tty_struct * tty, int butt, int mrx, int mry)&squot;&n; *     &squot;int mouse_reporting(void)&squot;&n; *&n; * Hopefully this will be a rather complete VT102 implementation.&n; *&n; * Beeping thanks to John T Kohl.&n; *&n; * Virtual Consoles, Screen Blanking, Screen Dumping, Color, Graphics&n; *   Chars, and VT100 enhancements by Peter MacDonald.&n; *&n; * Copy and paste function by Andrew Haylett,&n; *   some enhancements by Alessandro Rubini.&n; *&n; * Code to check for different video-cards mostly by Galen Hunt,&n; * &lt;g-hunt@ee.utah.edu&gt;&n; *&n; * Rudimentary ISO 10646/Unicode/UTF-8 character set support by&n; * Markus Kuhn, &lt;mskuhn@immd4.informatik.uni-erlangen.de&gt;.&n; *&n; * Dynamic allocation of consoles, aeb@cwi.nl, May 1994&n; * Resizing of consoles, aeb, 940926&n; *&n; * Code for xterm like mouse click reporting by Peter Orbaek 20-Jul-94&n; * &lt;poe@daimi.aau.dk&gt;&n; *&n; * User-defined bell sound, new setterm control sequences and printk&n; * redirection by Martin Mares &lt;mj@k332.feld.cvut.cz&gt; 19-Nov-95&n; *&n; * APM screenblank bug fixed Takashi Manabe &lt;manabe@roy.dsl.tutics.tut.jp&gt;&n; *&n; * Merge with the abstract console driver by Geert Uytterhoeven&n; * &lt;Geert.Uytterhoeven@cs.kuleuven.ac.be&gt;, Jan 1997.&n; *&n; *   Original m68k console driver modifications by&n; *&n; *     - Arno Griffioen &lt;arno@usn.nl&gt;&n; *     - David Carter &lt;carter@cs.bris.ac.uk&gt;&n; * &n; *   Note that the abstract console driver allows all consoles to be of&n; *   potentially different sizes, so the following variables depend on the&n; *   current console (currcons):&n; *&n; *     - video_num_columns&n; *     - video_num_lines&n; *     - video_size_row&n; *     - video_screen_size&n; *     - can_do_color&n; *&n; *   The abstract console driver provides a generic interface for a text&n; *   console. It supports VGA text mode, frame buffer based graphical consoles&n; *   and special graphics processors that are only accessible through some&n; *   registers (e.g. a TMS340x0 GSP).&n; *&n; *   The interface to the hardware is specified using a special structure&n; *   (struct consw) which contains function pointers to the following&n; *   operations:&n; *&n; *     unsigned long con_startup(unsigned long kmem_start,&n; *&t;&t;&t;&t; const char **display_desc)&n; *     void con_init(struct vc_data *conp)&n; *     int con_deinit(struct vc_data *conp)&n; *     int con_clear(struct vc_data *conp, int sy, int sx, int height,&n; *&t;&t;     int width)&n; *     int con_putc(struct vc_data *conp, int c, int y, int x)&n; *     int con_putcs(struct vc_data *conp, const char *s, int count, int y,&n; *&t;&t;     int x)&n; *     int con_cursor(struct vc_data *conp, int mode)&n; *     int con_scroll(struct vc_data *conp, int t, int b, int dir, int count)&n; *     int con_bmove(struct vc_data *conp, int sy, int sx, int dy, int dx,&n; *&t;&t;     int height, int width)&n; *     int con_switch(struct vc_data *conp)&n; *     int con_blank(int blank)&n; *     int con_get_font(struct vc_data *conp, int *w, int *h, char *data)&n; *     int con_set_font(struct vc_data *conp, int w, int h, char *data)&n; *     int con_set_palette(struct vc_data *conp, unsigned char *table)&n; *     int con_scrolldelta(struct vc_data *conp, int lines)&n; *&n; * Support for changeable cursor shape&n; * by Pavel Machek &lt;pavel@atrey.karlin.mff.cuni.cz&gt;, August 1997&n; *&n; * Ported to i386 and con_scrolldelta fixed&n; * by Emmanuel Marty &lt;core@ggi-project.org&gt;, April 1998&n; */
...
macro_line|#ifndef CONFIG_ABSCON_COMPAT
...
id|fg_console
suffix:semicolon
multiline_comment|/* ++Geert: Always assume that the number of lines did change? */
r_return
id|video_num_lines
suffix:semicolon
)brace
multiline_comment|/* dummy functions */
...
multiline_comment|/*&n; * ++Geert: Change # of rows and columns for one specific console.&n; * Of course it&squot;s not messy to have all consoles of potentially different&n; * sizes, except on PCish hardware :-)&n; */
...
id|osr
suffix:semicolon
multiline_comment|/* ++Geert: TODO: Because the attributes have different meanings&n;&t;       for monochrome and color, they should really be converted if&n;&t;       can_do_color changes... */
id|update_attr
c_func
(paren
id|currcons
...
DECL|function|visual_putc_attr
r_static
r_inline
r_void
id|visual_putc_attr
...
DECL|function|visual_putc_attr_next
r_static
r_inline
r_void
id|visual_putc_attr_next
c_func
(paren
r_int
id|currcons
...
l_char|&squot;E&squot;
suffix:semicolon
multiline_comment|/* Arno:&n;&t;&t;&t;&t;&t;* Doesn&squot;t work, because csi_J(c,2)&n;&t;&t;&t;&t;&t;* calls con_clear and doesn&squot;t print&n;&t;&t;&t;&t;&t;* the erase char..&n;&t;&t;&t;&t;&t;*/
id|csi_J
c_func
(paren
id|currcons
