...
multiline_comment|/*&n; * Routine to reset the visible &quot;screen&quot; to the top of video memory.&n; * This is necessary when exiting from the kernel back to a console&n; * which expects only the top of video memory to be used for the visible&n; * screen (with scrolling down by moving the memory contents).&n; * The normal action of the LINUX console is to scroll using all of the&n; * video memory and diddling the hardware top-of-video register as needed.&n; */
r_void
DECL|function|scrreset
id|scrreset
c_func
(paren
r_void
