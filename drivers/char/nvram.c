...
macro_line|#endif
multiline_comment|/* Note that *all* calls to CMOS_READ and CMOS_WRITE must be done with&n; * interrupts disabled. Due to the index-port/data-port design of the RTC, we&n; * don&squot;t want two different things trying to get to it at once. (e.g. the&n; * periodic 11 min sync from time.c vs. this driver.)&n; */
