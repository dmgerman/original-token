...
DECL|variable|rng_open_wait
r_static
id|wait_queue_head_t
id|rng_open_wait
suffix:semicolon
multiline_comment|/* Wait queue for serializing open/release */
DECL|variable|rng_open_mode
r_static
r_int
id|rng_open_mode
suffix:semicolon
multiline_comment|/* Open mode (we only allow reads) */
...
multiline_comment|/*&n; * rng_enable - enable or disable the RNG and internal timer&n; */
...
id|read_loop
suffix:colon
multiline_comment|/* using the fact that read() can return &gt;0 but&n;&t; * less than the requested amount, we simply&n;&t; * read up to PAGE_SIZE or buffer size, whichever&n;&t; * is smaller, and return that data.&n;&t; */
r_if
c_cond
(paren
(paren
