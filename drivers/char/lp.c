...
multiline_comment|/* If you want to see if you can get lp_poll working, define this. */
DECL|macro|SUPPORT_POLL
macro_line|#undef SUPPORT_POLL
...
(brace
macro_line|#if !defined(CONFIG_PARPORT_1284) || !defined (SUPPORT_POLL)
r_return
suffix:semicolon
macro_line|#else
r_struct
...
DECL|function|lp_parport_claim
r_static
r_void
id|lp_parport_claim
(paren
...
multiline_comment|/* We must have the port since we got an interrupt. */
id|lp_check_data
(paren
id|lp_dev
)paren
...
r_return
suffix:semicolon
multiline_comment|/* Grab the port if it can help (i.e. reverse mode is possible). */
r_if
c_cond
(paren
op_logical_neg
...
r_do
(brace
multiline_comment|/* Wait until lp_read has finished. */
r_if
c_cond
(paren
...
id|POLLWRNORM
suffix:semicolon
multiline_comment|/* always writable */
id|poll_wait
(paren
id|filp
comma
