...
l_int|NULL
suffix:semicolon
DECL|variable|stli_tmpwritesem
r_static
r_struct
id|semaphore
id|stli_tmpwritesem
...
suffix:semicolon
multiline_comment|/*&n; *&t;Memory allocation vars. These keep track of what memory allocation&n; *&t;we can currently use. They help deal with memory in a consistent&n; *&t;way, whether during init or run-time.&n; */
DECL|variable|stli_meminited
r_static
r_int
id|stli_meminited
op_assign
...
macro_line|#if STLI_HIMEMORY
DECL|macro|ONB_HIMEMENAB
mdefine_line|#define&t;ONB_HIMEMENAB&t;0x02
macro_line|#else
DECL|macro|ONB_HIMEMENAB
mdefine_line|#define&t;ONB_HIMEMENAB&t;0
macro_line|#endif
...
multiline_comment|/*&n; *&t;Define the maximal baud rate, and he default baud base for ports.&n; */
DECL|macro|STL_MAXBAUD
mdefine_line|#define&t;STL_MAXBAUD&t;230400
DECL|macro|STL_BAUDBASE
mdefine_line|#define&t;STL_BAUDBASE&t;115200
DECL|macro|STL_CLOSEDELAY
mdefine_line|#define&t;STL_CLOSEDELAY&t;50
multiline_comment|/*****************************************************************************/
...
DECL|macro|MIN
mdefine_line|#define&t;MIN(a,b)&t;&t;(((a) &lt;= (b)) ? (a) : (b))
multiline_comment|/*****************************************************************************/
...
l_int|0
suffix:semicolon
multiline_comment|/*&n; *&t;This is hack to allow for the kernel changes made to add_timer&n; *&t;in the newer 1.3.X kernels (changed around 1.3.1X).&n; */
macro_line|#ifdef LINUX_1_2_X_COMPAT
DECL|macro|STLI_TIMEOUT
mdefine_line|#define&t;STLI_TIMEOUT&t;0
macro_line|#else
DECL|macro|STLI_TIMEOUT
...
multiline_comment|/*****************************************************************************/
macro_line|#ifdef MODULE
multiline_comment|/*&n; *&t;Include kernel version number for modules.&n; */
...
c_func
(paren
l_string|&quot;STALLION: failed to un-register tty driver, errno=%d,%d&bslash;n&quot;
comma
op_minus
id|i
comma
...
c_func
(paren
l_string|&quot;STALLION: failed to un-register serial memory device, errno=%d&bslash;n&quot;
comma
op_minus
id|i
)paren
...
macro_line|#endif
multiline_comment|/*****************************************************************************/
multiline_comment|/*&n; *&t;Local memory allocation routines. These are used so we can deal with&n; *&t;memory allocation at init time and during run-time in a consistent&n; *&t;way. Everbody just calls the stli_memalloc routine to allocate&n; *&t;memory and it will do the right thing. There is no common memory&n; *&t;deallocation code - since this is only done is special cases, all of&n; *&t;which are tightly controlled.&n; */
macro_line|#ifndef MODULE
DECL|function|stli_meminit
r_static
r_void
id|stli_meminit
c_func
...
c_func
(paren
l_string|&quot;stli_cmdwait(brdp=%x,portp=%x,cmd=%x,arg=%x,size=%d,copyback=%d)&bslash;n&quot;
comma
(paren
r_int
)paren
...
c_func
(paren
l_string|&quot;stli_sendcmd(brdp=%x,portp=%x,cmd=%x,arg=%x,size=%d,copyback=%d)&bslash;n&quot;
comma
(paren
r_int
)paren
...
id|brdp-&gt;nrdevs
suffix:semicolon
multiline_comment|/*&n; *&t;&t;Check if slave wants any service. Basically we try to do as&n; *&t;&t;little work as possible here. There are 2 levels of service&n; *&t;&t;bits. So if there is nothing to do we bail early. We check&n; *&t;&t;8 service bits at a time in the inner loop, so we can bypass&n; *&t;&t;the lot if none of them want service.&n; */
r_if
c_cond
(paren
id|hostreq
...
multiline_comment|/*&n; *&t;&t;Check if any of the out-standing host commands have completed.&n; *&t;&t;It is a bit unfortunate that we need to check stuff that we&n; *&t;&t;initiated!  This ain&squot;t pretty, but it needs to be fast.&n; */
r_if
c_cond
(paren
id|slavereq
...
c_func
(paren
l_string|&quot;STALLION: shared memory pointer=%x out of range at line=%d(%d), brd=%d&bslash;n&quot;
comma
(paren
r_int
)paren
...
multiline_comment|/*****************************************************************************/
macro_line|#if STLI_HIMEMORY
DECL|macro|PAGE_IOMEM
mdefine_line|#define&t;PAGE_IOMEM&t;__pgprot(_PAGE_PRESENT | _PAGE_RW | _PAGE_DIRTY | _PAGE_ACCESSED | _PAGE_PCD)
multiline_comment|/*&n; *&t;To support shared memory addresses outside of the lower 1 Mb region&n; *&t;we will need to pull some tricks with memory management to map the&n; *&t;higher range into kernel virtual address space... Radical stuff...&n; */
DECL|function|stli_mapbrdmem
r_static
r_void
op_star
id|stli_mapbrdmem
...
c_func
(paren
l_string|&quot;stli_mapbrdmem(physaddr=%x,size=%x)&bslash;n&quot;
comma
(paren
r_int
)paren
...
c_func
(paren
l_string|&quot;STALLION: failed to allocate virtual address space, size=%x&bslash;n&quot;
comma
id|size
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;stli_initecp(brdp=%x,confp=%x)&bslash;n&quot;
comma
(paren
r_int
)paren
...
macro_line|#endif
multiline_comment|/*&n; *&t;Now that all specific code is set up, enable the shared memory and&n; *&t;look for the a signature area that will tell us exactly what board&n; *&t;this is, and what is connected to it.&n; */
id|EBRDENABLE
c_func
(paren
id|brdp
...
c_func
(paren
l_string|&quot;%s(%d): sig-&gt; magic=%x romver=%x panel=%x,%x,%x,%x,%x,%x,%x,%x&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
c_func
(paren
l_string|&quot;stli_initonb(brdp=%x,confp=%x)&bslash;n&quot;
comma
(paren
r_int
)paren
...
c_func
(paren
l_string|&quot;%s(%d): CDK version %d.%d.%d --&gt; nrdevs=%d memp=%x hostp=%x slavep=%x&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
c_func
(paren
l_string|&quot;STALLION: slave failed to allocate memory for all devices, devices=%d&bslash;n&quot;
comma
id|nrdevs
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;stli_brdinit()&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_if
...
c_func
(paren
l_string|&quot;STALLION: failed to allocate board structures&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
c_func
(paren
l_string|&quot;STALLION: unit=%d is unknown board type=%d&bslash;n&quot;
comma
id|i
comma
id|confp-&gt;brdtype
...
c_func
(paren
l_string|&quot;STALLION: %s board not found, unit=%d io=%x mem=%x&bslash;n&quot;
comma
id|stli_brdnames
(braket
id|brdp-&gt;brdtype
...
c_func
(paren
l_string|&quot;STALLION: %s found, unit=%d io=%x mem=%x nrpanels=%d nrports=%d&bslash;n&quot;
comma
id|stli_brdnames
(braket
id|brdp-&gt;brdtype
