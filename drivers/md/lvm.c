...
id|lvm_version
op_assign
l_string|&quot;LVM version 0.8final  by Heinz Mauelshagen  (15/02/2000)&bslash;n&quot;
suffix:semicolon
...
op_assign
l_string|&quot;version 0.8final  (15/02/2000)&quot;
suffix:semicolon
DECL|macro|MAJOR_NR
...
mdefine_line|#define&t;LVM_CORRECT_READ_AHEAD(a)&t;&t;&t;&t;&bslash;&n;&t;(((a) &lt; LVM_MIN_READ_AHEAD || (a) &gt; LVM_MAX_READ_AHEAD)&t;&bslash;&n;&t; ? LVM_MAX_READ_AHEAD : (a))
...
suffix:semicolon
DECL|variable|lvm_proc_get_info_ptr
r_static
r_int
(paren
op_star
id|lvm_proc_get_info_ptr
)paren
(paren
r_char
op_star
...
id|LV_REDUCE
suffix:colon
multiline_comment|/* create, remove, extend or reduce a logical volume */
r_if
c_cond
(paren
id|vg_ptr
...
macro_line|#if defined CONFIG_LVM_PROC_FS &amp;&amp; defined CONFIG_PROC_FS
multiline_comment|/*&n; * Support function /proc-Filesystem&n; */
DECL|macro|LVM_PROC_BUF
mdefine_line|#define  LVM_PROC_BUF   ( i == 0 ? dummy_buf : &amp;buf[sz])
DECL|function|lvm_proc_get_info
r_static
r_int
id|lvm_proc_get_info
c_func
...
(paren
id|KERN_DEBUG
l_string|&quot;%s - lvm_proc_get_info CALLED  pos: %lu  count: %d  whence: %d&bslash;n&quot;
comma
id|lvm_name
comma
id|pos
...
multiline_comment|/* lvm_proc_get_info() */
...
multiline_comment|/*&n;   if ( lv-&gt;lv_access &amp; LV_SNAPSHOT)&n;   printk ( &quot;%s -- %02d:%02d  block: %lu  rw: %d&bslash;n&quot;, lvm_name, MAJOR ( bh-&gt;b_dev), MINOR ( bh-&gt;b_dev), bh-&gt;b_blocknr, rw);&n; */
multiline_comment|/* take care of snapshot chunk writes before&n;&t;   check for writable logical volume */
r_if
c_cond
(paren
(paren
...
id|printk
(paren
l_string|&quot;%s -- doing snapshot write for %02d:%02d[%02d:%02d]  b_blocknr: %lu  b_rsector: %lu&bslash;n&quot;
comma
id|lvm_name
comma
id|MAJOR
...
(paren
id|KERN_ALERT
l_string|&quot;%s - lvm_map *rsector: %lu or size: %lu wrong for&quot;
l_string|&quot; minor: %2d&bslash;n&quot;
comma
id|lvm_name
comma
id|rsector_tmp
...
multiline_comment|/*&n; * plug device function is a noop because plugging has to happen&n; * in the queue of the physical blockdevice to allow the&n; * elevator to do a better job.&n; */
DECL|function|lvm_plug_device_noop
r_static
r_void
id|lvm_plug_device_noop
c_func
...
multiline_comment|/* We don&squot;t need the PE list&n;&t;&t;&t;   in kernel space as with LVs pe_t list (see below) */
id|pv_ptr-&gt;pe
op_assign
l_int|NULL
suffix:semicolon
...
(brace
multiline_comment|/* get pointer to original logical volume */
id|lv_ptr
op_assign
id|vg_ptr-&gt;lv
(braket
...
suffix:semicolon
multiline_comment|/* set necessary fields of original logical volume */
id|lv_ptr-&gt;lv_access
op_or_assign
id|LV_SNAPSHOT_ORG
suffix:semicolon
...
l_int|NULL
suffix:semicolon
multiline_comment|/* find last snapshot logical volume in the chain */
r_while
c_loop
(paren
id|lv_ptr-&gt;lv_snapshot_next
...
id|lv_ptr-&gt;lv_snapshot_next
suffix:semicolon
multiline_comment|/* set back pointer to this last one in our new logical volume */
id|vg_ptr-&gt;lv
(braket
id|l
)braket
...
id|lv_ptr
suffix:semicolon
multiline_comment|/* last logical volume now points to our new snapshot volume */
id|lv_ptr-&gt;lv_snapshot_next
op_assign
id|vg_ptr-&gt;lv
(braket
...
suffix:semicolon
multiline_comment|/* now point to the new one */
id|lv_ptr
op_assign
id|lv_ptr-&gt;lv_snapshot_next
suffix:semicolon
multiline_comment|/* set necessary fields of new snapshot logical volume */
id|lv_ptr-&gt;lv_snapshot_next
op_assign
l_int|NULL
suffix:semicolon
...
(paren
id|KERN_CRIT
l_string|&quot;%s -- VG_EXTEND: kmalloc error PV at line %d&bslash;n&quot;
comma
id|lvm_name
comma
id|__LINE__
...
multiline_comment|/* in case of lv_remove(), lv_create() pair; for eg. lvrename does this */
id|l_new
op_assign
op_minus
l_int|1
...
id|lv_ptr
suffix:semicolon
multiline_comment|/* our new one now back points to the previous last in the chain */
id|lv_ptr
op_assign
id|vg_ptr-&gt;lv
(braket
