...
mdefine_line|#define CONFIG_SERIAL_SHARE_IRQ&t;&t;/* Must be enabled for MrCoffee. */
DECL|macro|CONFIG_SERIAL_DETECT_IRQ
macro_line|#undef CONFIG_SERIAL_DETECT_IRQ&t;&t;/* code is removed from su.c */
...
DECL|macro|IRQ_4M
mdefine_line|#define IRQ_4M(n)&t;(n)
DECL|macro|IRQ_T
mdefine_line|#define IRQ_T(info) ((info-&gt;flags &amp; ASYNC_SHARE_IRQ) ? SA_SHIRQ : SA_INTERRUPT)
macro_line|#else
multiline_comment|/* 0x20 is sun4m thing, Dave Redman heritage. See arch/sparc/kernel/irq.c. */
DECL|macro|IRQ_4M
mdefine_line|#define IRQ_4M(n)&t;((n)|0x20)
multiline_comment|/* Interrupts must be shared on MrCoffee. */
DECL|macro|IRQ_T
mdefine_line|#define IRQ_T(info)&t;SA_SHIRQ
macro_line|#endif
...
macro_line|#if 0 /* P3: Needed if we to support /sbin/setserial. */
...
suffix:semicolon
macro_line|#if 0 /* P3: became unused after surgery. */
...
mdefine_line|#define serial_in(info, off)&t;su_inb(info, off)
DECL|macro|serial_inp
mdefine_line|#define serial_inp(info, off)&t;su_inb(info, off)
...
op_increment
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Now check to see if character should be&n;&t;&t;&t; * ignored, and mask off conditions which&n;&t;&t;&t; * should be ignored.&n; */
r_if
c_cond
(paren
op_star
...
multiline_comment|/*&n; * This is the serial driver&squot;s generic interrupt routine&n;&t; */
...
DECL|function|su_interrupt_single
r_static
r_void
id|su_interrupt_single
c_func
(paren
r_int
id|irq
...
c_func
(paren
l_string|&quot;su_interrupt_single(%d) int=%x ...&quot;
comma
id|irq
comma
id|serial_inp
...
c_func
(paren
l_string|&quot;su_single loop break.&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_break
...
c_func
(paren
l_string|&quot;rs port monitor (single) irq %s: 0x%x, 0x%x&bslash;n&quot;
comma
id|__irq_itoa
c_func
(paren
...
DECL|function|su_interrupt_multi
r_static
r_void
id|su_interrupt_multi
c_func
(paren
r_int
id|irq
...
(brace
multiline_comment|/* XXX P3: I would put a printk here but it may flood. */
r_return
suffix:semicolon
)brace
r_do
...
multiline_comment|/* Trigger warnings is fall through by a chance. */
)brace
...
multiline_comment|/*&n; * This routine is called whenever a serial port is opened.  It&n; * enables interrupts for a serial port, linking in its async structure into&n; * the IRQ chain.   It also performs the serial-specific&n; * initialization for the tty structure.&n;&t; */
...
multiline_comment|/*&n; * ---------------------------------------------------------------------&n; * su_init() and friends&n; *&n; * su_init() is called at boot-time to initialize the serial driver.&n; * ---------------------------------------------------------------------&n; */
...
multiline_comment|/*&n;&t; * There is no intr property on MrCoffee, so hardwire it. Krups?&n;&t; */
id|info-&gt;irq
op_assign
l_int|13
suffix:semicolon
...
macro_line|#if 0 /* P3 You will never beleive but SuperIO fails this test in MrCoffee. */
id|scratch
op_assign
id|serial_in
c_func
...
macro_line|#if 0&t;/* Must be shared with keyboard on MrCoffee. */
macro_line|#ifdef CONFIG_SERIAL_CONSOLE
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We want startup here because we want mouse and keyboard&n;&t;&t; * working without opening. On SPARC console will work&n;&t;&t; * without startup.&n;&t;&t; */
r_if
c_cond
(paren
id|info-&gt;kbd_node
...
id|len
suffix:semicolon
multiline_comment|/*&n;&t; * Find su on MrCoffee. We return OK code if find any.&n;&t; * Then su_init finds every one and initializes them.&n;&t; * We do this early because MrCoffee got no aliases.&n;&t; */
id|node
op_assign
id|prom_getchild
c_func
...
suffix:semicolon
macro_line|#ifdef CONFIG_SERIAL_CONSOLE
multiline_comment|/*&n;&t;&t;&t;&t; * Console must be initiated after the generic initialization.&n;&t;&t;&t;&t; * sunserial_setinitfunc inverts order, so call this before next one.&n;&t;&t;&t;&t; */
...
suffix:semicolon
multiline_comment|/* rs_init? su_probe? XXX */
)brace
r_void
id|cleanup_module
c_func
