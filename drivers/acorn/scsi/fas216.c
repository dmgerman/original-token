...
mdefine_line|#define SCSI2_WIDE
...
(brace
singleline_comment|//&t;    &t;while (residual &gt; 0) {
singleline_comment|//&t;&t;&t;if ((inb(REG_CFIS(info)) &amp; CFIS_CF) &lt; 8) {
id|outb
c_func
(paren
op_star
...
singleline_comment|//&t;&t;&t;if (inb(REG_STAT(info)) &amp; STAT_INT)
singleline_comment|//&t;&t;&t;&t;break;
singleline_comment|//&t;&t;}
singleline_comment|//&t;&t;correction = inb(REG_CFIS(info)) &amp; CFIS_CF;
)brace
r_else
(brace
singleline_comment|//&t;    &t;while (residual &gt; 0) {
singleline_comment|//&t;&t;&t;if ((inb(REG_CFIS(info)) &amp; CFIS_CF) != 0) {
op_star
id|ptr
op_increment
op_assign
...
id|info
comma
l_string|&quot;fas216_send_messageout&quot;
)paren
suffix:semicolon
id|outb
c_func
...
multiline_comment|/* This table describes the legal SCSI state transitions,&n;&t;&t; * as described by the SCSI II spec.&n;&t;&t; */
r_switch
c_cond
(paren
id|STATE
...
r_return
suffix:semicolon
multiline_comment|/* Error recovery rules.&n;&t;&t; *   These either attempt to abort or retry the operation.&n;&t;&t; * TODO: we need more of these&n;&t;&t; */
r_case
id|STATE
c_func
(paren
...
multiline_comment|/* error - we&squot;ve sent out all the command bytes&n;&t;&t;&t; * we have.&n;&t;&t;&t; * NOTE: we need SAVE DATA POINTERS/RESTORE DATA POINTERS&n;&t;&t;&t; * to include the command bytes sent for this to work&n;&t;&t;&t; * correctly.&n;&t;&t;&t; */
id|printk
c_func
(paren
id|KERN_ERR
...
multiline_comment|/* If we get another message out phase, this&n;&t;&t;&t; * usually means some parity error occurred.&n;&t;&t;&t; * Resend complete set of messages.  If we have&n;&t;&t;&t; * more than 1 byte to send, we need to assert&n;&t;&t;&t; * ATN again.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|msgqueue_msglength
...
l_int|0
suffix:semicolon
multiline_comment|/* add command into execute queue and let it complete under&n;&t; * whatever scheme we&squot;re using.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/*&n;&t;&t; * If we don&squot;t have an IRQ, then we must&n;&t;&t; * poll the card for it&squot;s interrupt, and&n;&t;&t; * use that to call this driver&squot;s interrupt&n;&t;&t; * routine.  That way, we keep the command&n;&t;&t; * progressing.&n;&t;&t; */
r_if
c_cond
(paren
id|info-&gt;scsi.irq
...
suffix:semicolon
multiline_comment|/*&n; * Prototype: enum res_abort fas216_do_abort(FAS216_Info *info, Scsi_Cmnd *SCpnt)&n; * Purpose  : abort a command on this host&n; * Params   : SCpnt - command to abort&n; * Returns  : abort status&n; */
r_static
r_enum
id|res_abort
...
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If the command has connected and done nothing futher,&n;&t;&t; * simply force a disconnect.  We also need to clear the&n;&t;&t; * busylun bit.&n;&t;&t; */
r_case
id|PHASE_SELECTION
suffix:colon
singleline_comment|//&t;&t;&t;info-&gt;SCpnt = NULL;
singleline_comment|//&t;&t;&t;res = res_success_clear;
