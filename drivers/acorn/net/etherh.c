multiline_comment|/*&n; * linux/drivers/net/etherh.c&n; *&n; * NS8390 ANT etherh specific driver&n; *  For Acorn machines&n; *&n; * By Russell King.&n; *&n; * Changelog:&n; *  08-Dec-1996&t;RMK&t;1.00&t;Created&n; *&t;&t;RMK&t;1.03&t;Added support for EtherLan500 cards&n; *  23-Nov-1997&t;RMK&t;1.04&t;Added media autodetection&n; *&n; * Insmod Module Parameters&n; * ------------------------&n; *   io=&lt;io_base&gt;&n; *   irq=&lt;irqno&gt;&n; *   xcvr=&lt;0|1&gt; 0 = 10bT, 1=10b2 (Lan600/600A only)&n; */
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/fcntl.h&gt;
macro_line|#include &lt;linux/interrupt.h&gt;
macro_line|#include &lt;linux/ptrace.h&gt;
macro_line|#include &lt;linux/ioport.h&gt;
macro_line|#include &lt;linux/in.h&gt;
macro_line|#include &lt;linux/malloc.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/netdevice.h&gt;
macro_line|#include &lt;linux/etherdevice.h&gt;
macro_line|#include &lt;linux/skbuff.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/bitops.h&gt;
macro_line|#include &lt;asm/ecard.h&gt;
macro_line|#include &lt;asm/delay.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;asm/irq.h&gt;
macro_line|#include &quot;8390.h&quot;
DECL|macro|NET_DEBUG
mdefine_line|#define NET_DEBUG  0
DECL|macro|DEBUG_INIT
mdefine_line|#define DEBUG_INIT 2
DECL|variable|net_debug
r_static
r_int
r_int
id|net_debug
op_assign
id|NET_DEBUG
suffix:semicolon
DECL|variable|etherh_cids
r_static
r_const
id|card_ids
id|etherh_cids
(braket
)braket
op_assign
(brace
(brace
id|MANU_I3
comma
id|PROD_I3_ETHERLAN500
)brace
comma
(brace
id|MANU_I3
comma
id|PROD_I3_ETHERLAN600
)brace
comma
(brace
id|MANU_I3
comma
id|PROD_I3_ETHERLAN600A
)brace
comma
(brace
l_int|0xffff
comma
l_int|0xffff
)brace
)brace
suffix:semicolon
DECL|variable|version
r_static
r_char
op_star
id|version
op_assign
l_string|&quot;etherh [500/600/600A] ethernet driver (c) 1997 R.M.King v1.04&bslash;n&quot;
suffix:semicolon
DECL|macro|ETHERH500_DATAPORT
mdefine_line|#define ETHERH500_DATAPORT&t;0x200&t;/* MEMC */
DECL|macro|ETHERH500_NS8390
mdefine_line|#define ETHERH500_NS8390&t;0x000&t;/* MEMC */
DECL|macro|ETHERH500_CTRLPORT
mdefine_line|#define ETHERH500_CTRLPORT&t;0x200&t;/* IOC  */
DECL|macro|ETHERH600_DATAPORT
mdefine_line|#define ETHERH600_DATAPORT&t;16&t;/* MEMC */
DECL|macro|ETHERH600_NS8390
mdefine_line|#define ETHERH600_NS8390&t;0x200&t;/* MEMC */
DECL|macro|ETHERH600_CTRLPORT
mdefine_line|#define ETHERH600_CTRLPORT&t;0x080&t;/* MEMC */
DECL|macro|ETHERH_CP_IE
mdefine_line|#define ETHERH_CP_IE&t;&t;1
DECL|macro|ETHERH_CP_IF
mdefine_line|#define ETHERH_CP_IF&t;&t;2
DECL|macro|ETHERH_TX_START_PAGE
mdefine_line|#define ETHERH_TX_START_PAGE&t;1
DECL|macro|ETHERH_STOP_PAGE
mdefine_line|#define ETHERH_STOP_PAGE&t;0x7f
multiline_comment|/* --------------------------------------------------------------------------- */
multiline_comment|/*&n; * Read the ethernet address string from the on board rom.&n; * This is an ascii string...&n; */
r_static
r_int
DECL|function|etherh_addr
id|etherh_addr
(paren
r_char
op_star
id|addr
comma
r_struct
id|expansion_card
op_star
id|ec
)paren
(brace
r_struct
id|in_chunk_dir
id|cd
suffix:semicolon
r_char
op_star
id|s
suffix:semicolon
r_if
c_cond
(paren
id|ecard_readchunk
c_func
(paren
op_amp
id|cd
comma
id|ec
comma
l_int|0xf5
comma
l_int|0
)paren
op_logical_and
(paren
id|s
op_assign
id|strchr
c_func
(paren
id|cd.d.string
comma
l_char|&squot;(&squot;
)paren
)paren
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|6
suffix:semicolon
id|i
op_increment
)paren
(brace
id|addr
(braket
id|i
)braket
op_assign
id|simple_strtoul
c_func
(paren
id|s
op_plus
l_int|1
comma
op_amp
id|s
comma
l_int|0x10
)paren
suffix:semicolon
r_if
c_cond
(paren
op_star
id|s
op_ne
(paren
id|i
op_eq
l_int|5
ques
c_cond
l_char|&squot;)&squot;
suffix:colon
l_char|&squot;:&squot;
)paren
)paren
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|i
op_eq
l_int|6
)paren
r_return
l_int|0
suffix:semicolon
)brace
r_return
id|ENODEV
suffix:semicolon
)brace
multiline_comment|/*&n; * Reset the 8390 (hard reset)&n; */
r_static
r_void
DECL|function|etherh_reset
id|etherh_reset
(paren
r_struct
id|device
op_star
id|dev
)paren
(brace
r_int
r_int
id|addr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
r_int
id|crb
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;mem_end
op_eq
id|PROD_I3_ETHERLAN600
op_logical_or
id|dev-&gt;mem_end
op_eq
id|PROD_I3_ETHERLAN600A
)paren
(brace
id|crb
op_assign
id|inb
(paren
id|addr
op_plus
id|EN0_RCNTHI
)paren
op_amp
l_int|0xf8
suffix:semicolon
id|outb
(paren
id|ei_status.interface_num
ques
c_cond
id|crb
op_or
l_int|1
suffix:colon
id|crb
comma
id|addr
op_plus
id|EN0_RCNTHI
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * Write a block of data out to the 8390&n; */
r_static
r_void
DECL|function|etherh_block_output
id|etherh_block_output
(paren
r_struct
id|device
op_star
id|dev
comma
r_int
id|count
comma
r_const
r_int
r_char
op_star
id|buf
comma
r_int
id|start_page
)paren
(brace
r_int
r_int
id|addr
comma
id|dma_addr
suffix:semicolon
r_int
r_int
id|dma_start
suffix:semicolon
r_if
c_cond
(paren
id|ei_status.dmaing
)paren
(brace
id|printk
(paren
l_string|&quot;%s: DMAing conflict in etherh_block_input: &quot;
l_string|&quot; DMAstat %d irqlock %d intr %d&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|ei_status.dmaing
comma
id|ei_status.irqlock
comma
id|dev-&gt;interrupt
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|ei_status.dmaing
op_or_assign
l_int|1
suffix:semicolon
id|addr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
id|dma_addr
op_assign
id|dev-&gt;mem_start
suffix:semicolon
id|count
op_assign
(paren
id|count
op_plus
l_int|1
)paren
op_amp
op_complement
l_int|1
suffix:semicolon
id|outb
(paren
id|E8390_NODMA
op_or
id|E8390_PAGE0
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
id|outb
(paren
l_int|0x42
comma
id|addr
op_plus
id|EN0_RCNTLO
)paren
suffix:semicolon
id|outb
(paren
l_int|0x00
comma
id|addr
op_plus
id|EN0_RCNTHI
)paren
suffix:semicolon
id|outb
(paren
l_int|0x42
comma
id|addr
op_plus
id|EN0_RSARLO
)paren
suffix:semicolon
id|outb
(paren
l_int|0x00
comma
id|addr
op_plus
id|EN0_RSARHI
)paren
suffix:semicolon
id|outb
(paren
id|E8390_RREAD
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
id|udelay
(paren
l_int|1
)paren
suffix:semicolon
id|outb
(paren
id|ENISR_RDC
comma
id|addr
op_plus
id|EN0_ISR
)paren
suffix:semicolon
id|outb
(paren
id|count
comma
id|addr
op_plus
id|EN0_RCNTLO
)paren
suffix:semicolon
id|outb
(paren
id|count
op_rshift
l_int|8
comma
id|addr
op_plus
id|EN0_RCNTHI
)paren
suffix:semicolon
id|outb
(paren
l_int|0
comma
id|addr
op_plus
id|EN0_RSARLO
)paren
suffix:semicolon
id|outb
(paren
id|start_page
comma
id|addr
op_plus
id|EN0_RSARHI
)paren
suffix:semicolon
id|outb
(paren
id|E8390_RWRITE
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ei_status.word16
)paren
id|outsw
(paren
id|dma_addr
comma
id|buf
comma
id|count
op_rshift
l_int|1
)paren
suffix:semicolon
macro_line|#ifdef BIT8
r_else
id|outsb
(paren
id|dma_addr
comma
id|buf
comma
id|count
)paren
suffix:semicolon
macro_line|#endif
id|dma_start
op_assign
id|jiffies
suffix:semicolon
r_while
c_loop
(paren
(paren
id|inb
(paren
id|addr
op_plus
id|EN0_ISR
)paren
op_amp
id|ENISR_RDC
)paren
op_eq
l_int|0
)paren
r_if
c_cond
(paren
id|jiffies
op_minus
id|dma_start
OG
l_int|2
op_star
id|HZ
op_div
l_int|100
)paren
(brace
multiline_comment|/* 20ms */
id|printk
(paren
l_string|&quot;%s: timeout waiting for TX RDC&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
id|etherh_reset
(paren
id|dev
)paren
suffix:semicolon
id|NS8390_init
(paren
id|dev
comma
l_int|1
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
id|outb
(paren
id|ENISR_RDC
comma
id|addr
op_plus
id|EN0_ISR
)paren
suffix:semicolon
id|ei_status.dmaing
op_and_assign
op_complement
l_int|1
suffix:semicolon
)brace
multiline_comment|/*&n; * Read a block of data from the 8390&n; */
r_static
r_void
DECL|function|etherh_block_input
id|etherh_block_input
(paren
r_struct
id|device
op_star
id|dev
comma
r_int
id|count
comma
r_struct
id|sk_buff
op_star
id|skb
comma
r_int
id|ring_offset
)paren
(brace
r_int
r_int
id|addr
comma
id|dma_addr
suffix:semicolon
r_int
r_char
op_star
id|buf
suffix:semicolon
r_if
c_cond
(paren
id|ei_status.dmaing
)paren
(brace
id|printk
(paren
l_string|&quot;%s: DMAing conflict in etherh_block_input: &quot;
l_string|&quot; DMAstat %d irqlock %d intr %d&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|ei_status.dmaing
comma
id|ei_status.irqlock
comma
id|dev-&gt;interrupt
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|ei_status.dmaing
op_or_assign
l_int|1
suffix:semicolon
id|addr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
id|dma_addr
op_assign
id|dev-&gt;mem_start
suffix:semicolon
id|buf
op_assign
id|skb-&gt;data
suffix:semicolon
id|outb
(paren
id|E8390_NODMA
op_or
id|E8390_PAGE0
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
id|outb
(paren
id|count
comma
id|addr
op_plus
id|EN0_RCNTLO
)paren
suffix:semicolon
id|outb
(paren
id|count
op_rshift
l_int|8
comma
id|addr
op_plus
id|EN0_RCNTHI
)paren
suffix:semicolon
id|outb
(paren
id|ring_offset
comma
id|addr
op_plus
id|EN0_RSARLO
)paren
suffix:semicolon
id|outb
(paren
id|ring_offset
op_rshift
l_int|8
comma
id|addr
op_plus
id|EN0_RSARHI
)paren
suffix:semicolon
id|outb
(paren
id|E8390_RREAD
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ei_status.word16
)paren
(brace
id|insw
(paren
id|dma_addr
comma
id|buf
comma
id|count
op_rshift
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|count
op_amp
l_int|1
)paren
id|buf
(braket
id|count
op_minus
l_int|1
)braket
op_assign
id|inb
(paren
id|dma_addr
)paren
suffix:semicolon
)brace
macro_line|#ifdef BIT8
r_else
id|insb
(paren
id|dma_addr
comma
id|buf
comma
id|count
)paren
suffix:semicolon
macro_line|#endif
id|outb
(paren
id|ENISR_RDC
comma
id|addr
op_plus
id|EN0_ISR
)paren
suffix:semicolon
id|ei_status.dmaing
op_and_assign
op_complement
l_int|1
suffix:semicolon
)brace
multiline_comment|/*&n; * Read a header from the 8390&n; */
r_static
r_void
DECL|function|etherh_get_header
id|etherh_get_header
(paren
r_struct
id|device
op_star
id|dev
comma
r_struct
id|e8390_pkt_hdr
op_star
id|hdr
comma
r_int
id|ring_page
)paren
(brace
r_int
r_int
id|addr
comma
id|dma_addr
suffix:semicolon
r_if
c_cond
(paren
id|ei_status.dmaing
)paren
(brace
id|printk
(paren
l_string|&quot;%s: DMAing conflict in etherh_get_header: &quot;
l_string|&quot; DMAstat %d irqlock %d intr %d&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|ei_status.dmaing
comma
id|ei_status.irqlock
comma
id|dev-&gt;interrupt
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|ei_status.dmaing
op_or_assign
l_int|1
suffix:semicolon
id|addr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
id|dma_addr
op_assign
id|dev-&gt;mem_start
suffix:semicolon
id|outb
(paren
id|E8390_NODMA
op_or
id|E8390_PAGE0
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
id|outb
(paren
r_sizeof
(paren
op_star
id|hdr
)paren
comma
id|addr
op_plus
id|EN0_RCNTLO
)paren
suffix:semicolon
id|outb
(paren
l_int|0
comma
id|addr
op_plus
id|EN0_RCNTHI
)paren
suffix:semicolon
id|outb
(paren
l_int|0
comma
id|addr
op_plus
id|EN0_RSARLO
)paren
suffix:semicolon
id|outb
(paren
id|ring_page
comma
id|addr
op_plus
id|EN0_RSARHI
)paren
suffix:semicolon
id|outb
(paren
id|E8390_RREAD
op_or
id|E8390_START
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ei_status.word16
)paren
id|insw
(paren
id|dma_addr
comma
id|hdr
comma
r_sizeof
(paren
op_star
id|hdr
)paren
op_rshift
l_int|1
)paren
suffix:semicolon
macro_line|#ifdef BIT8
r_else
id|insb
(paren
id|dma_addr
comma
id|hdr
comma
r_sizeof
(paren
op_star
id|hdr
)paren
)paren
suffix:semicolon
macro_line|#endif
id|outb
(paren
id|ENISR_RDC
comma
id|addr
op_plus
id|EN0_ISR
)paren
suffix:semicolon
id|ei_status.dmaing
op_and_assign
op_complement
l_int|1
suffix:semicolon
)brace
multiline_comment|/*&n; * Open/initialize the board.  This is called (in the current kernel)&n; * sometime after booting when the &squot;ifconfig&squot; program is run.&n; *&n; * This routine should set everything up anew at each open, even&n; * registers that &quot;should&quot; only need to be set once at boot, so that&n; * there is non-reboot way to recover if something goes wrong.&n; */
r_static
r_int
DECL|function|etherh_open
id|etherh_open
c_func
(paren
r_struct
id|device
op_star
id|dev
)paren
(brace
r_int
r_int
id|addr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
r_int
id|crb
suffix:semicolon
id|MOD_INC_USE_COUNT
suffix:semicolon
r_if
c_cond
(paren
id|request_irq
c_func
(paren
id|dev-&gt;irq
comma
id|ei_interrupt
comma
l_int|0
comma
l_string|&quot;etherh&quot;
comma
id|dev
)paren
)paren
(brace
id|MOD_DEC_USE_COUNT
suffix:semicolon
r_return
op_minus
id|EAGAIN
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev-&gt;mem_end
op_eq
id|PROD_I3_ETHERLAN600
op_logical_or
id|dev-&gt;mem_end
op_eq
id|PROD_I3_ETHERLAN600A
)paren
(brace
id|crb
op_assign
id|inb
(paren
id|addr
op_plus
id|EN0_RCNTHI
)paren
op_amp
l_int|0xf8
suffix:semicolon
id|outb
(paren
id|ei_status.interface_num
ques
c_cond
id|crb
op_or
l_int|1
suffix:colon
id|crb
comma
id|addr
op_plus
id|EN0_RCNTHI
)paren
suffix:semicolon
)brace
id|ei_open
(paren
id|dev
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * The inverse routine to etherh_open().&n; */
r_static
r_int
DECL|function|etherh_close
id|etherh_close
c_func
(paren
r_struct
id|device
op_star
id|dev
)paren
(brace
id|ei_close
(paren
id|dev
)paren
suffix:semicolon
id|free_irq
(paren
id|dev-&gt;irq
comma
id|dev
)paren
suffix:semicolon
id|MOD_DEC_USE_COUNT
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * This is the real probe routine.&n; */
r_static
r_int
DECL|function|etherh_probe1
id|etherh_probe1
c_func
(paren
r_struct
id|device
op_star
id|dev
)paren
(brace
r_static
r_int
id|version_printed
suffix:semicolon
r_int
r_int
id|addr
comma
id|i
comma
id|reg0
comma
id|tmp
suffix:semicolon
r_char
op_star
id|dev_type
suffix:semicolon
id|addr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
r_if
c_cond
(paren
id|net_debug
op_logical_and
id|version_printed
op_increment
op_eq
l_int|0
)paren
id|printk
c_func
(paren
id|version
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|dev-&gt;mem_end
)paren
(brace
r_case
id|PROD_I3_ETHERLAN500
suffix:colon
id|dev_type
op_assign
l_string|&quot;500 &quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
id|PROD_I3_ETHERLAN600
suffix:colon
id|dev_type
op_assign
l_string|&quot;600 &quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
id|PROD_I3_ETHERLAN600A
suffix:colon
id|dev_type
op_assign
l_string|&quot;600A &quot;
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|dev_type
op_assign
l_string|&quot;&quot;
suffix:semicolon
)brace
id|reg0
op_assign
id|inb
(paren
id|addr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|reg0
op_eq
l_int|0xff
)paren
(brace
r_if
c_cond
(paren
id|net_debug
op_amp
id|DEBUG_INIT
)paren
id|printk
(paren
l_string|&quot;%s: etherh error: NS8390 command register wrong&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
id|outb
(paren
id|E8390_NODMA
op_or
id|E8390_PAGE1
op_or
id|E8390_STOP
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
id|tmp
op_assign
id|inb
(paren
id|addr
op_plus
l_int|13
)paren
suffix:semicolon
id|outb
(paren
l_int|0xff
comma
id|addr
op_plus
l_int|13
)paren
suffix:semicolon
id|outb
(paren
id|E8390_NODMA
op_or
id|E8390_PAGE0
comma
id|addr
op_plus
id|E8390_CMD
)paren
suffix:semicolon
id|inb
(paren
id|addr
op_plus
id|EN0_COUNTER0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|inb
(paren
id|addr
op_plus
id|EN0_COUNTER0
)paren
op_ne
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|net_debug
op_amp
id|DEBUG_INIT
)paren
id|printk
(paren
l_string|&quot;%s: etherh error: NS8390 not found&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
id|outb
(paren
id|reg0
comma
id|addr
)paren
suffix:semicolon
id|outb
(paren
id|tmp
comma
id|addr
op_plus
l_int|13
)paren
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
r_if
c_cond
(paren
id|ethdev_init
(paren
id|dev
)paren
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|request_region
(paren
id|addr
comma
l_int|16
comma
l_string|&quot;etherh&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;%s: etherh %sfound at %lx, IRQ%d, ether address &quot;
comma
id|dev-&gt;name
comma
id|dev_type
comma
id|dev-&gt;base_addr
comma
id|dev-&gt;irq
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|6
suffix:semicolon
id|i
op_increment
)paren
id|printk
(paren
id|i
op_eq
l_int|5
ques
c_cond
l_string|&quot;%2.2x&bslash;n&quot;
suffix:colon
l_string|&quot;%2.2x:&quot;
comma
id|dev-&gt;dev_addr
(braket
id|i
)braket
)paren
suffix:semicolon
id|ei_status.name
op_assign
l_string|&quot;etherh&quot;
suffix:semicolon
id|ei_status.word16
op_assign
l_int|1
suffix:semicolon
id|ei_status.interface_num
op_assign
id|dev-&gt;if_port
op_amp
l_int|1
suffix:semicolon
id|ei_status.tx_start_page
op_assign
id|ETHERH_TX_START_PAGE
suffix:semicolon
id|ei_status.rx_start_page
op_assign
id|ei_status.tx_start_page
op_plus
id|TX_PAGES
suffix:semicolon
id|ei_status.stop_page
op_assign
id|ETHERH_STOP_PAGE
suffix:semicolon
id|ei_status.reset_8390
op_assign
id|etherh_reset
suffix:semicolon
id|ei_status.block_input
op_assign
id|etherh_block_input
suffix:semicolon
id|ei_status.block_output
op_assign
id|etherh_block_output
suffix:semicolon
id|ei_status.get_8390_hdr
op_assign
id|etherh_get_header
suffix:semicolon
id|dev-&gt;open
op_assign
id|etherh_open
suffix:semicolon
id|dev-&gt;stop
op_assign
id|etherh_close
suffix:semicolon
id|NS8390_init
(paren
id|dev
comma
l_int|0
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|etherh_irq_enable
r_static
r_void
id|etherh_irq_enable
(paren
id|ecard_t
op_star
id|ec
comma
r_int
id|irqnr
)paren
(brace
r_int
r_int
id|ctrl_addr
op_assign
(paren
r_int
r_int
)paren
id|ec-&gt;irq_data
suffix:semicolon
id|outb
(paren
id|inb
(paren
id|ctrl_addr
)paren
op_or
id|ETHERH_CP_IE
comma
id|ctrl_addr
)paren
suffix:semicolon
)brace
DECL|function|etherh_irq_disable
r_static
r_void
id|etherh_irq_disable
(paren
id|ecard_t
op_star
id|ec
comma
r_int
id|irqnr
)paren
(brace
r_int
r_int
id|ctrl_addr
op_assign
(paren
r_int
r_int
)paren
id|ec-&gt;irq_data
suffix:semicolon
id|outb
(paren
id|inb
(paren
id|ctrl_addr
)paren
op_amp
op_complement
id|ETHERH_CP_IE
comma
id|ctrl_addr
)paren
suffix:semicolon
)brace
DECL|variable|etherh_ops
r_static
id|expansioncard_ops_t
id|etherh_ops
op_assign
(brace
id|etherh_irq_enable
comma
id|etherh_irq_disable
comma
l_int|NULL
comma
l_int|NULL
)brace
suffix:semicolon
DECL|function|etherh_initdev
r_static
r_void
id|etherh_initdev
(paren
id|ecard_t
op_star
id|ec
comma
r_struct
id|device
op_star
id|dev
)paren
(brace
id|ecard_claim
(paren
id|ec
)paren
suffix:semicolon
id|dev-&gt;irq
op_assign
id|ec-&gt;irq
suffix:semicolon
id|dev-&gt;mem_end
op_assign
id|ec-&gt;cld.product
suffix:semicolon
r_switch
c_cond
(paren
id|ec-&gt;cld.product
)paren
(brace
r_case
id|PROD_I3_ETHERLAN500
suffix:colon
id|dev-&gt;base_addr
op_assign
id|ecard_address
(paren
id|ec
comma
id|ECARD_MEMC
comma
l_int|0
)paren
op_plus
id|ETHERH500_NS8390
suffix:semicolon
id|dev-&gt;mem_start
op_assign
id|dev-&gt;base_addr
op_plus
id|ETHERH500_DATAPORT
suffix:semicolon
id|ec-&gt;irq_data
op_assign
(paren
r_void
op_star
)paren
id|ecard_address
(paren
id|ec
comma
id|ECARD_IOC
comma
id|ECARD_FAST
)paren
op_plus
id|ETHERH500_CTRLPORT
suffix:semicolon
r_break
suffix:semicolon
r_case
id|PROD_I3_ETHERLAN600
suffix:colon
r_case
id|PROD_I3_ETHERLAN600A
suffix:colon
id|dev-&gt;base_addr
op_assign
id|ecard_address
(paren
id|ec
comma
id|ECARD_MEMC
comma
l_int|0
)paren
op_plus
id|ETHERH600_NS8390
suffix:semicolon
id|dev-&gt;mem_start
op_assign
id|dev-&gt;base_addr
op_plus
id|ETHERH600_DATAPORT
suffix:semicolon
id|ec-&gt;irq_data
op_assign
(paren
r_void
op_star
)paren
(paren
id|dev-&gt;base_addr
op_plus
id|ETHERH600_CTRLPORT
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|printk
(paren
l_string|&quot;%s: etherh error: unknown card type&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
)brace
id|ec-&gt;ops
op_assign
op_amp
id|etherh_ops
suffix:semicolon
id|etherh_addr
(paren
id|dev-&gt;dev_addr
comma
id|ec
)paren
suffix:semicolon
)brace
macro_line|#ifndef MODULE
r_int
DECL|function|etherh_probe
id|etherh_probe
c_func
(paren
r_struct
id|device
op_star
id|dev
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|dev
)paren
r_return
id|ENODEV
suffix:semicolon
id|ecard_startfind
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev-&gt;base_addr
)paren
(brace
r_struct
id|expansion_card
op_star
id|ec
suffix:semicolon
r_if
c_cond
(paren
(paren
id|ec
op_assign
id|ecard_find
(paren
l_int|0
comma
id|etherh_cids
)paren
)paren
op_eq
l_int|NULL
)paren
r_return
id|ENODEV
suffix:semicolon
id|etherh_initdev
(paren
id|ec
comma
id|dev
)paren
suffix:semicolon
)brace
r_return
id|etherh_probe1
(paren
id|dev
)paren
suffix:semicolon
)brace
macro_line|#endif
macro_line|#ifdef MODULE
DECL|macro|MAX_ETHERH_CARDS
mdefine_line|#define MAX_ETHERH_CARDS 2
DECL|variable|io
r_static
r_int
id|io
(braket
id|MAX_ETHERH_CARDS
)braket
suffix:semicolon
DECL|variable|irq
r_static
r_int
id|irq
(braket
id|MAX_ETHERH_CARDS
)braket
suffix:semicolon
DECL|variable|xcvr
r_static
r_int
id|xcvr
(braket
id|MAX_ETHERH_CARDS
)braket
op_assign
(brace
l_int|1
comma
l_int|1
)brace
suffix:semicolon
DECL|variable|ethernames
r_static
r_char
id|ethernames
(braket
id|MAX_ETHERH_CARDS
)braket
(braket
l_int|9
)braket
suffix:semicolon
DECL|variable|my_ethers
r_static
r_struct
id|device
op_star
id|my_ethers
(braket
id|MAX_ETHERH_CARDS
)braket
suffix:semicolon
DECL|variable|ec
r_static
r_struct
id|expansion_card
op_star
id|ec
(braket
id|MAX_ETHERH_CARDS
)braket
suffix:semicolon
r_int
DECL|function|init_module
id|init_module
c_func
(paren
r_void
)paren
(brace
r_struct
id|device
op_star
id|dev
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|expansion_card
op_star
id|boguscards
(braket
id|MAX_ETHERH_CARDS
)braket
suffix:semicolon
r_int
id|i
comma
id|found
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_ETHERH_CARDS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|my_ethers
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
id|boguscards
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
id|ec
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
id|strcpy
(paren
id|ethernames
(braket
id|i
)braket
comma
l_string|&quot;        &quot;
)paren
suffix:semicolon
)brace
id|ecard_startfind
c_func
(paren
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_ETHERH_CARDS
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|dev
)paren
id|dev
op_assign
(paren
r_struct
id|device
op_star
)paren
id|kmalloc
(paren
r_sizeof
(paren
r_struct
id|device
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev
)paren
id|memset
(paren
id|dev
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|device
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|io
(braket
id|i
)braket
)paren
(brace
r_if
c_cond
(paren
(paren
id|ec
(braket
id|i
)braket
op_assign
id|ecard_find
(paren
l_int|0
comma
id|etherh_cids
)paren
)paren
op_eq
l_int|NULL
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|etherh_initdev
(paren
id|ec
(braket
id|i
)braket
comma
id|dev
)paren
suffix:semicolon
)brace
r_else
(brace
id|ec
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|dev-&gt;base_addr
op_assign
id|io
(braket
id|i
)braket
suffix:semicolon
id|dev-&gt;irq
op_assign
id|irq
(braket
id|i
)braket
suffix:semicolon
)brace
id|dev-&gt;init
op_assign
id|etherh_probe1
suffix:semicolon
id|dev-&gt;name
op_assign
id|ethernames
(braket
id|i
)braket
suffix:semicolon
id|dev-&gt;if_port
op_assign
id|xcvr
(braket
id|i
)braket
suffix:semicolon
id|my_ethers
(braket
id|i
)braket
op_assign
id|dev
suffix:semicolon
r_if
c_cond
(paren
id|register_netdev
(paren
id|my_ethers
(braket
id|i
)braket
)paren
op_ne
l_int|0
)paren
(brace
id|printk
(paren
id|KERN_WARNING
l_string|&quot;No etherh card found at %08lX&bslash;n&quot;
comma
id|dev-&gt;base_addr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ec
(braket
id|i
)braket
)paren
(brace
id|boguscards
(braket
id|i
)braket
op_assign
id|ec
(braket
id|i
)braket
suffix:semicolon
id|ec
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
)brace
r_continue
suffix:semicolon
)brace
id|found
op_increment
suffix:semicolon
id|dev
op_assign
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|dev
)paren
id|kfree
(paren
id|dev
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_ETHERH_CARDS
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|boguscards
(braket
id|i
)braket
)paren
(brace
id|boguscards
(braket
id|i
)braket
op_member_access_from_pointer
id|ops
op_assign
l_int|NULL
suffix:semicolon
id|ecard_release
(paren
id|boguscards
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|found
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_void
DECL|function|cleanup_module
id|cleanup_module
c_func
(paren
r_void
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_ETHERH_CARDS
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|my_ethers
(braket
id|i
)braket
)paren
(brace
id|unregister_netdev
c_func
(paren
id|my_ethers
(braket
id|i
)braket
)paren
suffix:semicolon
id|release_region
(paren
id|my_ethers
(braket
id|i
)braket
op_member_access_from_pointer
id|base_addr
comma
l_int|16
)paren
suffix:semicolon
id|kfree
(paren
id|my_ethers
(braket
id|i
)braket
)paren
suffix:semicolon
id|my_ethers
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|ec
(braket
id|i
)braket
)paren
(brace
id|ec
(braket
id|i
)braket
op_member_access_from_pointer
id|ops
op_assign
l_int|NULL
suffix:semicolon
id|ecard_release
c_func
(paren
id|ec
(braket
id|i
)braket
)paren
suffix:semicolon
id|ec
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
)brace
macro_line|#endif /* MODULE */
eof
