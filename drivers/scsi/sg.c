...
id|ENXIO
suffix:semicolon
multiline_comment|/*&n;     * If we are in the middle of error recovery, then don&squot;t allow any&n;     * access to this device.  Also, error recovery *may* have taken the&n;     * device offline, in which case all further access is prohibited.&n;     */
r_if
c_cond
(paren
op_logical_neg
...
suffix:colon
multiline_comment|/*&n;&t;  Allow SCSI_IOCTL_SEND_COMMAND without checking suser() since the&n;&t;  user already has read/write access to the generic device and so&n;&t;  can execute arbitrary SCSI commands.&n;&t;*/
r_return
id|scsi_ioctl_send_command
c_func
(paren
