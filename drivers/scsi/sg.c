...
id|sg_version_str
op_assign
l_string|&quot;Version: 2.1.34 (990603)&quot;
suffix:semicolon
...
multiline_comment|/*&n; *  D. P. Gilbert (dgilbert@interlog.com, dougg@triode.net.au), notes:&n; *      - scsi logging is available via SCSI_LOG_TIMEOUT macros. First&n; *        the kernel/module needs to be built with CONFIG_SCSI_LOGGING&n; *        (otherwise the macros compile to empty statements), then do&n; *        something like: &squot;echo &quot;scsi log all&quot; &gt; /proc/scsi/scsi&squot; to log&n; *        everything or &squot;echo &quot;scsi log {token} #N&quot; &gt; /proc/scsi/scsi&squot;&n; *        where {token} is one of [error,timeout,scan,mlqueue,mlcomplete,&n; *        llqueue,llcomplete,hlqueue,hlcomplete,ioctl] and #N is 0...7&n; *        (with 0 meaning off). For example: &squot;scsi log timeout 7 &gt; &n; *        /proc/scsi/scsi&squot; to get all logging messages from this driver.&n; *        Should use hlcomplete but it is too &quot;noisy&quot; (sd uses it).&n; *&n; *      - This driver obtains memory (heap) for the low-level driver to&n; *        transfer/dma to and from. It is obtained from up to 3 sources:&n; *              - obtain heap via get_free_pages()&n; *              - obtain heap from the shared scsi dma pool&n; *              - obtain heap from kernel directly (kmalloc) [last choice]&n; *        Each open() attempts to obtain a &quot;reserve&quot; buffer of&n; *        SG_DEF_RESERVED_SIZE bytes (or 0 bytes if opened O_RDONLY). The&n; *        amount actually obtained [which could be 0 bytes] can be found from&n; *        the SG_GET_RESERVED_SIZE ioctl(). This reserved buffer size can&n; *        be changed by calling the SG_SET_RESERVED_SIZE ioctl(). Since this&n; *        is an ambit claim, it should be followed by a SG_GET_RESERVED_SIZE&n; *        ioctl() to find out how much was actually obtained.&n; *        A subsequent write() to this file descriptor will use the&n; *        reserved buffer unless:&n; *              - it is already in use (eg during command queuing)&n; *              - or the write() needs a buffer size larger than the&n; *                reserved size&n; *        In these cases the write() will attempt to get the required memory&n; *        for the duration of this request but, if memory is low, it may&n; *        fail with ENOMEM.&n; *&n; *      - The &squot;alt_address&squot; field in the scatter_list structure and the&n; *        related &squot;mem_src&squot; indicate the source of the heap allocation.&n; *&n; */
...
id|Sg_fd
suffix:semicolon
multiline_comment|/* 1208 bytes long on i386 */
