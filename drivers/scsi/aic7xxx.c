...
mdefine_line|#define AIC7XXX_C_VERSION  &quot;5.2.0&quot;
DECL|macro|NUMBER
...
suffix:semicolon
multiline_comment|/*&n;   * We are starting to do real work on the card....it&squot;s possible we could&n;   * generate some spurious interrupts at this point, especially in the&n;   * event of a PCI error or some such.  If there are other devices already&n;   * registered on the same interrupt as us, this could cause the machine&n;   * to lock up.  So, we disable the interrupt this card is on until we&n;   * finish our card setup.  We only need to do this for modules, if we are&n;   * compiled into the kernel then interrupts are already off during this&n;   * part of the code.&n;   */
macro_line|#ifdef MODULE
id|disable_irq
c_func
(paren
