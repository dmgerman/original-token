...
multiline_comment|/*&n; * Modify this as you see fit for your system.  By setting tag_enable&n; * to 0, the driver will use it&squot;s own algorithm for determining the&n; * number of commands to use (see above).  When -1, the driver will&n; * not enable tagged queueing for that particular device.  When positive&n; * (&gt; 0) the values in the array are used for the queue_depth.&n; *&n; * In this example, the first line will enable tagged queueing for all&n; * the devices on the first probed aic7xxx adapter and tells the driver&n; * to use it&squot;s own algorithm for determining commands/LUN.&n; *&n; * The second line enables tagged queueing with 4 commands/LUN for IDs&n; * (1, 2-11, 13-15), disables tagged queueing for ID 12, and tells the&n; * driver to use its own algorithm for ID 1.&n; *&n; * The third line is the same as the first line.&n; *&n; * The fourth line disables tagged queueing for devices 0 and 3.  It&n; * enables tagged queueing for the other IDs, with 16 commands/LUN&n; * for IDs 1 and 4, 128 commands/LUN for ID 8, and 4 commands/LUN for&n; * IDs 2, 5-7, and 9-15.&n; */
...
multiline_comment|/*&n; * We keep a pool of kernel memory for hardware scatter/gather&n; * arrays.  One page (PAGE_SIZE) of kernel memory is allocated&n; * at a time, and SG arrays are taken from this pool.  An SG&n; * array is 2048 bytes (256 * 8) long.  A page of memory is either&n; * 4096 (i386) or 8192 (alpha) bytes from which SG arrays can be&n; * taken without fragmenting memory.&n; *&n; * The number of free SG arrays left in the SG pool is kept in&n; * aic7xxx_sg_arrays_free.  The pool of SG arrays is kept in&n; * aic7xxx_next_sg_array; after each each allocation of an SG array,&n; * this variable is updated to point to the next available SG array&n; * in the pool.  When aic7xxx_sg_arrays_free is zero, a new page is&n; * allocated for the pool.  See aic7xxx_allocate_sglist() for&n; * further details.&n; *&n; * The SG memory pool is global to the driver, and is not per&n; * instance of the driver.&n; */
DECL|variable|aic7xxx_next_sg_array
r_static
r_struct
id|hw_scatterlist
op_star
id|aic7xxx_next_sg_array
op_assign
l_int|NULL
suffix:semicolon
DECL|variable|aic7xxx_sg_arrays_free
r_static
r_int
id|aic7xxx_sg_arrays_free
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/*+F*************************************************************************&n; * Function:&n; *   aic7xxx_allocate_sglist&n; *&n; * Description:&n; *   Allocate a hardware SG array.&n; *-F*************************************************************************/
r_static
r_struct
id|hw_scatterlist
op_star
DECL|function|aic7xxx_allocate_sglist
id|aic7xxx_allocate_sglist
c_func
(paren
r_void
)paren
...
c_func
(paren
l_string|&quot;aic7xxx: SG list doesn&squot;t fit in a page.&bslash;n&quot;
)paren
suffix:semicolon
)brace
macro_line|#ifdef DMA_KMALLOC
id|aic7xxx_next_sg_array
op_assign
(paren
r_struct
...
r_else
(brace
multiline_comment|/*&n;       * Point to the next available SG array in the pool.&n;       */
id|aic7xxx_next_sg_array
op_assign
op_amp
(paren
