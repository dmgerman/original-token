...
mdefine_line|#define AIC7XXX_FAKE_NEGOTIATION_CMDS
...
mdefine_line|#define AHC_SYNCRATE_ULTRA2 0
DECL|macro|AHC_SYNCRATE_ULTRA
mdefine_line|#define AHC_SYNCRATE_ULTRA  2
DECL|macro|AHC_SYNCRATE_FAST
mdefine_line|#define AHC_SYNCRATE_FAST   5
...
l_int|0
suffix:semicolon
multiline_comment|/*&n; * This setting enables a hack to fix the IRQ settings on buggy 7895&n; * MB controller setups:&n; *    -1 == Disable this hack&n; *     0 == Use the Channel A IRQ for both channels&n; *     1 == Use the Channel B IRQ for both channels&n; */
...
(brace
multiline_comment|/*&n;       * When responding to a target that requests sync, the requested rate&n;       * may fall between two rates that we can output, but still be a rate&n;       * that we can receive.  Because of this, we want to respond with the&n;       * same rate that it sent to us even if the persiod we use to send&n;       * data to it is lower.  Only lower the response period if we must.&n;       */
r_if
c_cond
(paren
id|syncrate
...
id|width
suffix:semicolon
multiline_comment|/*&n;   * Having just set the width, the SDTR should come next, and we need a valid&n;   * offset for the SDTR.  So, we make sure we put a valid one in here now as&n;   * the goal_offset.&n;   */
r_if
c_cond
(paren
id|p-&gt;transinfo
...
(paren
id|INFO_LEAD
l_string|&quot;Reducing Queue depth for untagged command.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|INFO_LEAD
l_string|&quot;Moving SCB to Delayed Queue.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|INFO_LEAD
l_string|&quot;Sending command %d/0x%x to QINFIFO&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|INFO_LEAD
l_string|&quot;Sending commands to QINFIFO&bslash;n&quot;
comma
id|p-&gt;host_no
comma
op_minus
...
id|p
comma
l_string|&quot;While sending commands to QINFIFO&quot;
)paren
suffix:semicolon
)brace
macro_line|#endif
...
(brace
multiline_comment|/*&n;                   * This is already a negotiation command, so we must have&n;                   * already done either WDTR or SDTR (or maybe both).  So&n;                   * we simply check sdtr_pending and needsdtr to see if we&n;                   * should throw out SDTR on this command.&n;                   *&n;                   * Note: Don&squot;t check the needsdtr_copy here, instead just&n;                   * check to see if WDTR wiped out our SDTR and set needsdtr.&n;                   * Even if WDTR did wipe out SDTR and set needsdtr, if&n;                   * parse_msg() then turned around and started our SDTR&n;                   * in back to back fasion, then conclusion of that should&n;                   * have negated any needsdtr setting.  That&squot;s why we only&n;                   * check needsdtr and sdtr_pending.&n;                   */
id|scb-&gt;flags
op_and_assign
op_complement
id|SCB_MSGOUT_BITS
...
multiline_comment|/*&n;                   * This is the important part though.  We are getting sense&n;                   * info back from this device.  It&squot;s going into a fake&n;                   * command.  We need to put that into the real command&n;                   * instead so that the mid level SCSI code can act upon it.&n;                   * So, when we set up these fake commands, the next pointer&n;                   * is used to point to the real command.  Use that to change&n;                   * the address of our sense_buffer[] to the real command.&n;                   * However, don&squot;t do this if the real command is also a&n;                   * TEST_UNIT_READY as it will most likely pull down its own&n;                   * SENSE information anyway.&n;                   */
r_if
c_cond
(paren
id|cmd-&gt;next-&gt;cmnd
...
id|SCB_MSGOUT_BITS
suffix:semicolon
macro_line|#endif /* AIC7XXX_FAKE_NEGOTIATION_CMDS */
id|scb-&gt;flags
op_or_assign
id|SCB_SENSE
suffix:semicolon
...
id|period
suffix:semicolon
multiline_comment|/*&n;           * We need to set an accurate goal_offset instead of&n;           * the ridiculously high one we default to.  We should&n;           * now know if we are wide.  Plus, the WDTR code will &n;           * set our goal_offset for us as well.&n;           */
r_if
c_cond
(paren
id|p-&gt;transinfo
...
(paren
id|INFO_LEAD
l_string|&quot;Finished receipt of SDTR, parsing %d/%d&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|INFO_LEAD
l_string|&quot;After find_syncrate() %d/%d&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|INFO_LEAD
l_string|&quot;After validate_offset() %d/%d&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|INFO_LEAD
l_string|&quot;Final values of Period/Offset as set: %d/%d&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(brace
multiline_comment|/*&n;           * Uhh ohh, things fell through to async....update the goal&n;           * items and the needsdtr_copy to reflect this...&n;           */
id|aic7xxx_set_syncrate
c_func
(paren
id|p
...
r_else
(brace
multiline_comment|/*&n;           * Send a reply SDTR back.  Even if we sent the first one, it&n;           * is valid to send another one out immediately to re-negotiate&n;           * things, and a few devices don&squot;t like getting rejects after&n;           * we already sent them one SDTR.  Just send an SDTR for async&n;           * this time if need be (or for the correct params if we didn&squot;t&n;           * start all of this).  If this is a Reject Reply type message,&n;           * then we&squot;ve put the async settings into the goal area for&n;           * future reference (when we get the AWAITING_MSG interrupt).&n;           * If this is a case where we are responding to the target&squot;s&n;           * initiated SDTR, then leave our own goal and user values in&n;           * place (unless the device hasn&squot;t been scanned yet, in which&n;           * case, put the user values into the goal values so we don&squot;t&n;           * send out an Async message).&n;           */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;HSCB %d bad, SCB_PREV invalid(%d).&bslash;n&quot;
comma
id|i
comma
id|temp
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) Please verify driver detected settings are &quot;
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) If not, then please properly set the device &quot;
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) in the Adaptec SCSI BIOS by hitting CTRL-A &quot;
...
suffix:semicolon
multiline_comment|/*&n;       * Now set the termination based on what we found.  BRDDAT6&n;       * controls wide termination enable.&n;       * Flash Enable = BRDDAT7&n;       * SE High Term Enable = BRDDAT6&n;       * SE Low Term Enable = BRDDAT5 (7890)&n;       * LVD High Term Enable = BRDDAT4 (7890)&n;       */
r_if
c_cond
(paren
op_logical_neg
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) LVD Low byte termination Enabled&bslash;n&quot;
comma
id|p-&gt;host_no
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) LVD High byte termination Enabled&bslash;n&quot;
comma
id|p-&gt;host_no
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Set the prev pointer. */
id|aic_outb
c_func
(paren
id|p
...
c_func
(paren
l_string|&quot;(scsi%d) Force setting STPWLEV bit&bslash;n&quot;
comma
id|p-&gt;host_no
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;(scsi%d) Force clearing STPWLEV bit&bslash;n&quot;
comma
id|p-&gt;host_no
)paren
suffix:semicolon
...
(brace
multiline_comment|/*&n;       * A little padding so we can align thing the way we want&n;       */
id|p-&gt;scb_data-&gt;hscbs
op_assign
id|kmalloc
c_func
...
multiline_comment|/*&n;     * Save the actual kmalloc buffer pointer off, then align our&n;     * buffer to a 32 byte boundary&n;     */
...
id|p
suffix:semicolon
multiline_comment|/*&n;   * Clear out any possible pending interrupts, again.&n;   */
id|aic7xxx_clear_intstat
c_func
(paren
id|p
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) The no_reset option is known to break some &quot;
l_string|&quot;systems,&bslash;n&quot;
comma
id|p-&gt;host_no
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;(scsi%d) and is not supported by the driver author&bslash;n&quot;
comma
id|p-&gt;host_no
)paren
suffix:semicolon
...
l_int|1000
suffix:semicolon
multiline_comment|/* 1 second (1000 * 1 msec) */
r_while
c_loop
(paren
op_decrement
...
macro_line|#else  /* LINUX_VERSION_CODE &gt; KERNEL_VERSION(2,1,92) */
id|temp_p-&gt;pci_bus
op_assign
id|pci_bus
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;               * We used to set DPARCKEN in this register, but after talking&n;               * to a tech from Adaptec, I found out they don&squot;t use that&n;               * particular bit in their own register settings, and when you&n;               * combine that with the fact that I determined that we were&n;               * seeing Data-Path Parity Errors on things we shouldn&squot;t see&n;               * them on, I think there is a bug in the silicon and the way&n;               * to work around it is to disable this particular check.  Also&n;               * This bug only showed up on certain commands, so it seems to&n;               * be pattern related or some such.  The commands we would&n;               * typically send as a linux TEST_UNIT_READY or INQUIRY command&n;               * could cause it to be triggered, while regular commands that&n;               * actually made reasonable use of the SG array capabilities&n;               * seemed not to cause the problem.&n;               */
multiline_comment|/*&n;              aic_outb(temp_p, aic_inb(temp_p, DSCOMMAND0) |&n;                               CACHETHEN | DPARCKEN | MPARCKEN |&n;                               USCBSIZE32 | CIOPARCKEN,&n;                               DSCOMMAND0);&n;               */
id|aic_outb
c_func
(paren
id|temp_p
...
r_default
suffix:colon
multiline_comment|/*&n;               * We attempt to read a SEEPROM on *everything*.  If we fail,&n;               * then we fail, but this covers things like 2910c cards that&n;               * now have SEEPROMs with their 7856 chipset that we would&n;               * otherwise ignore.  They still don&squot;t have a BIOS, but they&n;               * have a SEEPROM that the SCSISelect utility on the Adaptec&n;               * diskettes can configure.&n;               */
id|aic7xxx_load_seeprom
c_func
(paren
id|temp_p
...
id|AHC_AIC7880
suffix:colon
multiline_comment|/*&n;               * Only set the DSCOMMAND0 register if this is a Rev B.&n;               * chipset.  For those, we also enable Ultra mode by&n;               * force due to brain-damage on the part of some BIOSes&n;               * We overload the devconfig variable here since we can.&n;               */
macro_line|#if LINUX_VERSION_CODE &gt; KERNEL_VERSION(2,1,92)
id|pci_read_config_dword
c_func
(paren
...
multiline_comment|/*&n;     * Before sending this thing out, we also amke the cmd-&gt;next pointer&n;     * point to the real command so we can stuff any possible SENSE data&n;     * intp the real command instead of this fake command.  This has to be&n;     * done each time the command is built, not just the first time, hence&n;     * it&squot;s outside of the above if()...&n;     */
id|p-&gt;dev_wdtr_cmnd
(braket
id|tindex
)braket
...
(paren
id|INFO_LEAD
l_string|&quot;Building WDTR command.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_CMD
...
(paren
id|INFO_LEAD
l_string|&quot;Building SDTR command.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_CMD
...
(paren
id|INFO_LEAD
l_string|&quot;dev_flags=0x%x, WDTR:%c/%c/%c, SDTR:%c/%c/%c,&quot;
l_string|&quot; q_depth=%d:%d:%d&bslash;n&quot;
comma
id|p-&gt;host_no
comma
l_int|0
...
multiline_comment|/*&n;   * Search for this command and see if we can&squot;t track it down, it&squot;s the&n;   * one causing the timeout.  Print out this command first, then all other&n;   * active commands afterwords.&n;   */
id|need_tag
op_assign
op_minus
l_int|1
...
c_func
(paren
l_string|&quot;Timed out command is scb #%d:&bslash;n&quot;
comma
id|scb-&gt;hscb-&gt;tag
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Tag%d: flags=0x%x, control=0x%x, TCL=0x%x, %s&bslash;n&quot;
comma
id|scb-&gt;hscb-&gt;tag
comma
id|scb-&gt;flags
...
c_func
(paren
l_string|&quot;Current SCB: (SCBPTR/TAG/CONTROL) %d/%d/0x%x&bslash;n&quot;
comma
id|aic_inb
c_func
(paren
...
c_func
(paren
l_string|&quot;WAITING_SCBS: (SCBPTR/TAG/CONTROL) %d-&gt;&quot;
comma
id|hscbp
op_assign
id|aic_inb
...
c_func
(paren
l_string|&quot;DISCONNECTED_SCBS: (SCBPTR/TAG/CONTROL) %d-&gt;&quot;
comma
id|hscbp
op_assign
id|aic_inb
...
c_func
(paren
l_string|&quot;FREE_SCBS: (SCBPTR/TAG/CONTROL) %d-&gt;&quot;
comma
id|hscbp
op_assign
id|aic_inb
...
(brace
multiline_comment|/*&n;     * We haven&squot;t found the offending SCB yet, and it should be around&n;     * somewhere, so go look for it in the cards SCBs.&n;     */
id|printk
c_func
(paren
l_string|&quot;SCBPTR CONTROL TAG PREV NEXT&bslash;n&quot;
)paren
suffix:semicolon
r_for
c_loop
...
c_func
(paren
l_string|&quot;   %3d      %02x  %02x   %02x   %02x&bslash;n&quot;
comma
id|i
comma
id|aic_inb
...
id|AHC_CHIPID_MASK
suffix:semicolon
multiline_comment|/*&n;   * Let&squot;s run through the PCI space first....&n;   */
id|printk
c_func
(paren
l_string|&quot;%s at &quot;
...
macro_line|#endif /* CONFIG_PCI */
multiline_comment|/*&n;   * Now the registers on the card....&n;   */
id|printk
c_func
(paren
...
c_func
(paren
l_string|&quot;SEEPROM dump.&bslash;n&quot;
)paren
suffix:semicolon
r_for
c_loop
