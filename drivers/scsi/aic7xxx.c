...
macro_line|#    error &quot;PPC and Sparc platforms are only support under 2.1.x and above&quot;
macro_line|#  endif
...
(brace
multiline_comment|/*&n;           * We&squot;ve successfully completed the wide negotiation, so let&squot;s start&n;           * up the sync negotiation now.&n;           */
id|scb-&gt;flags
op_and_assign
op_complement
id|SCB_MSGOUT_WDTR_16BIT
...
(paren
id|WARN_LEAD
l_string|&quot;Error reading PCI config register during PCI ERROR&quot;
...
id|dev_id
suffix:semicolon
multiline_comment|/*&n;   * Just a few sanity checks.  Make sure p != NULL, that we have an&n;   * interrupt pending, and that we aren&squot;t already in our int handler.&n;   * Also, if PCI, then we are going to check for a PCI bus error status&n;   * should we get too many spurious interrupts.&n;   */
r_if
c_cond
(paren
id|p
...
(paren
id|KERN_WARNING
l_string|&quot;aic7xxx: ISR routine called with NULL dev_id&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
...
multiline_comment|/*&n;   * XXXXXXXX  FIXXXXXMEEEEEE.  How do we unmap the I/O range we have mapped&n;   * if we are doing MMAPed I/O ??????????  Our biggest concern is the issue&n;   * of possibly calling unmap on an area that *might* be used on another&n;   * controller as well (aka, the 4096 byte MMAPed area is back to back&n;   * with another controller, and the PAGE_SIZE is greater then 4096, allowing&n;   * us to remap in a shared page).&n;   */
id|scsi_unregister
c_func
(paren
id|p-&gt;host
...
suffix:semicolon
multiline_comment|/* stop the selection since we just&n;                                         * grabbed the scb out from under the&n;                                         * card */
)brace
r_else
(brace
id|scb_next_ptr
