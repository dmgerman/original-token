...
mdefine_line|#define PCI_DEVICE_ID_ADAPTEC_1480A 0x6075
macro_line|#endif
r_static
r_inline
r_int
...
suffix:semicolon
multiline_comment|/*&n;         * Did we drop to async?  If so, are we sending a reply?  If we are,&n;         * then we have to make sure that the reply value reflects the proper&n;         * settings so we need to set the goal values according to what&n;         * we need to send.&n;         */
r_if
c_cond
(paren
(paren
