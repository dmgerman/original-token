...
DECL|macro|VERSION
mdefine_line|#define VERSION          &quot;$Revision: 5.31 $&quot;
...
suffix:semicolon
multiline_comment|/* If this driver works for later FD PCI&n;                                   boards, we will have to modify banner&n;                                   for additional PCI cards, but for now if&n;                                   it&squot;s PCI it&squot;s a TMC-3260 - JTM */
id|printk
c_func
(paren
...
DECL|function|fdomain_pci_nobios_detect
r_static
r_int
id|fdomain_pci_nobios_detect
c_func
(paren
r_int
op_star
...
l_int|0
suffix:semicolon
multiline_comment|/* The proper way of doing this is to use ask the PCI bus for the device&n;      IRQ and interrupt level.  But we can&squot;t do that if PCI BIOS32 support&n;      isn&squot;t compiled into the kernel, or if a PCI BIOS32 isn&squot;t present.&n;&n;      Instead, we scan down a bunch of addresses (Future Domain tech&n;      support says we will probably find the address before we get to&n;      0xf800).  This works fine on some systems -- other systems may have&n;      to scan more addresses.  If you have to modify this section for your&n;      installation, please send mail to faith@cs.unc.edu. */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* PCI detection function: int fdomain_36c70_detect(int* irq, int* iobase)&n;   This function gets the Interrupt Level and I/O base address from the PCI&n;   configuration registers.  The I/O base address is masked with 0xfff8&n;   since on my card the address read from the PCI config registers is off&n;   by one from the actual I/O base address necessary for accessing the&n;   status and control registers on the card (PCI config register gives&n;   0xf801, actual address is 0xf800).  This is likely a bug in the FD&n;   config code that writes to the PCI registers, however using a mask&n;   should be safe since I think the scan done by the card to determine the&n;   I/O base is done in increments of 8 (i.e., 0xf800, 0xf808, ...), at&n;   least the old scan code we used to use to get the I/O base did...  Also,&n;   the device ID from the PCI config registers is 0x0 and should be 0x60e9&n;   as it is in the status registers (offset 5 from I/O base).  If this is&n;   changed in future hardware/BIOS changes it will need to be fixed in this&n;   detection function.  Comments, bug reports, etc... on this function&n;   should be sent to mckinley@msupa.pa.msu.edu - James T. McKinley.  */
macro_line|#ifdef PCI_CONFIG
DECL|function|fdomain_36c70_detect
r_static
r_int
id|fdomain_36c70_detect
c_func
(paren
r_int
op_star
...
id|pci_dev_fn
suffix:semicolon
multiline_comment|/* PCI bus &amp; device function */
r_int
r_char
id|pci_irq
suffix:semicolon
...
id|pci_device
suffix:semicolon
multiline_comment|/* PCI vendor &amp; device IDs */
multiline_comment|/* If the PCI BIOS doesn&squot;t exist, use the old-style detection routines.&n;      Otherwise, get the I/O base address and interrupt from the PCI config&n;      registers. */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;&bslash;nINFO: cat /proc/pci to see list of PCI devices from bios32&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
...
suffix:semicolon
macro_line|#endif 
multiline_comment|/* We will have to change this if more than 1 PCI bus is present and the&n;      FD scsi host is not on the first bus (i.e., a PCI to PCI bridge,&n;      which is not supported by bios32 right now anyway).  This should&n;      probably be done by a call to pcibios_find_device but I can&squot;t get it&n;      to work...  Also the device ID reported from the PCI config registers&n;      does not match the device ID quoted in the tech manual or available&n;      from offset 5 from the I/O base address.  It should be 0x60E9, but it&n;      is 0x0 if read from the PCI config registers.  I guess the FD folks&n;      neglected to write it to the PCI registers...  This loop is necessary&n;      to get the device function (at least until someone can get&n;      pcibios_find_device to work, I cannot but 53c7,8xx.c uses it...). */
id|pci_bus
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/* Break out once we have the correct device.  If other FD&n;&t;       PCI devices are added to this driver we will need to add&n;&t;       an or of the other PCI_DEVICE_ID_FD_XXXXX&squot;s here. */
r_break
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* If we can&squot;t find an FD scsi card we give up. */
r_return
l_int|0
suffix:semicolon
)brace
...
id|printk
(paren
l_string|&quot;PCI ERROR: Future Domain 36C70 not initializing&quot;
l_string|&quot; due to error reading configuration space&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* Now we have the I/O base address and interrupt from the PCI&n;&t; configuration registers.  Unfortunately it seems that the I/O base&n;&t; address is off by one on my card so I mask it with 0xfff8.  This&n;&t; must be some kind of goof in the FD code that does the autoconfig&n;&t; and writes to the PCI registers (or maybe I just don&squot;t understand&n;&t; something).  If they fix it in later versions of the card or BIOS&n;&t; we may have to adjust the address based on the signature or&n;&t; something...  */
op_star
id|irq
op_assign
id|pci_irq
...
c_func
(paren
l_string|&quot;Send mail to mckinley@msupa.pa.msu.edu.&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_return
...
multiline_comment|/* Skip first failure for 18C30 chips. */
op_increment
id|flag
suffix:semicolon
r_else
...
id|data_count
suffix:semicolon
multiline_comment|/* The fdomain_16x0_intr is only called via&n;                                   the interrupt handler.  The goal of the&n;                                   sti() here is to allow other&n;                                   interruptions while this routine is&n;                                   running. */
id|sti
c_func
(paren
)paren
