...
mdefine_line|#define   PPA_VERSION   &quot;0.20&quot;
...
l_int|1
suffix:semicolon
multiline_comment|/* give up the CPU ?        */
...
multiline_comment|/*      Wait for the high bit to be set.&n;&n;        In principle, this could be tied to an interrupt, but the adapter&n;        doesn&squot;t appear to be designed to support interrupts.  We spin on&n;        the 0x80 ready bit.  If ppa_call_sched is 1, we call the scheduler &n;        to allow other processes to run while we are waiting, just like&n;        the lp driver does in polling mode.  The performance hit is&n;        significant, so this behaviour is configurable.&n;&n;*/
(brace
r_int
id|k
suffix:semicolon
...
multiline_comment|/* This is not really a queued interface at all, but apparently there may&n;   be some bugs in the mid-level support for the non-queued interface.&n;   This function is re-entrant, but only to one level.&n;*/
(brace
r_int
id|s
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* can reenter this function */
)brace
id|ppa_busy
op_assign
l_int|0
...
multiline_comment|/* Command line parameters (for built-in driver):&n;&n;   Syntax:  ppa=base[,speed_high[,speed_low[,call_sched[,nybble]]]]&n;&n;   For example:  ppa=0x378   or   ppa=0x378,0,3&n;&n;*/
