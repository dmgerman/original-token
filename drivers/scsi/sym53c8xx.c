...
macro_line|#  define memcpy_to_pci(a, b, c)&t;memcpy_toio((u_long) (a), (b), (c))
macro_line|#elif defined(__alpha__)
DECL|macro|pcivtobus
...
suffix:semicolon
multiline_comment|/* SWIDE handling */
DECL|member|swide_ma_32
id|ncrcmd
id|swide_ma_32
(braket
l_int|4
...
l_int|16
comma
multiline_comment|/*&n;&t;**&t;Report to the C code the next position in &n;&t;**&t;the start queue the SCRIPTS will schedule.&n;&t;*/
id|SCR_LOAD_ABS
(paren
id|scratcha
comma
...
multiline_comment|/*-------------------------&lt; DISP_MSG_IN &gt;----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;Anticipate MSG_IN phase then STATUS phase.&n;&t;**&n;&t;**&t;May spare 2 SCRIPTS instructions when we have &n;&t;**&t;completed the OUTPUT of the data and the device &n;&t;**&t;goes directly to STATUS phase.&n;&t;*/
id|SCR_JUMP
op_xor
id|IFTRUE
(paren
...
l_int|0
comma
multiline_comment|/*&n;&t;**&t;Since the device is required to send any &n;&t;**&t;IGNORE WIDE RESIDUE message prior to any&n;&t;**&t;other information, we just snoop the SCSI &n;&t;**&t;BUS to check for such a message.&n;&t;*/
id|SCR_JUMP
op_xor
id|IFFALSE
(paren
...
comma
multiline_comment|/*&n;&t;**&t;We have been ODD at the end of the transfer, &n;&t;**&t;but the device hasn&squot;t be so.&n;&t;**&t;Signal a DATA OVERRUN condition to the C code.&n;&t;*/
id|SCR_INT
comma
...
comma
multiline_comment|/*&n;&t;**&t;It is an IDENTIFY message,&n;&t;**&t;Load the LUN control block address.&n;&t;**&t;If LUN 0, avoid a PCI BUS ownership by loading &n;&t;**&t;directly &squot;b_lun0&squot; from the TCB.&n;&t;*/
id|SCR_JUMPR
op_xor
id|IFTRUE
(paren
...
comma
multiline_comment|/*&n;&t;**&t;Count this byte.&n;&t;**&t;This will allow to return a positive &n;&t;**&t;residual to user.&n;&t;*/
id|SCR_LOAD_REL
(paren
id|scratcha
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_MA_32 &gt;-----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;Handling of the SWIDE for 32 bit chips.&n;&t;**&n;&t;**&t;We jump here from the C code with SCRATCHA &n;&t;**&t;containing the address to write the SWIDE.&n;&t;**&t;- Save 32 bit address in &lt;scratch&gt;.&n;&t;*/
id|SCR_STORE_ABS
(paren
id|scratcha
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_MA_64 &gt;-----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;Handling of the SWIDE for 64 bit chips when the &n;&t;**&t;hardware handling of phase mismatch is disabled.&n;&t;**&n;&t;**&t;We jump here from the C code with SCRATCHA &n;&t;**&t;containing the address to write the SWIDE and &n;&t;**&t;SBR containing bit 32..39 of this address.&n;&t;**&t;- Save 32 bit address in &lt;scratch&gt;.&n;&t;**&t;- Move address bit 32..39 to SFBR.&n;&t;*/
id|SCR_STORE_ABS
(paren
id|scratcha
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_SCR_64 &gt;-----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;Handling of the SWIDE for 64 bit chips when &n;&t;**&t;hardware phase mismatch is enabled.&n;&t;**&t;We are entered with a SCR_CALL from PMO_SAVE &n;&t;**&t;and PM1_SAVE sub-scripts.&n;&t;**&n;&t;**&t;Snoop the SCSI BUS in case of the device &n;&t;**&t;willing to ignore this residue.&n;&t;**&t;If it does, we must only increment the RBC, &n;&t;**&t;since this register does reflect all bytes &n;&t;**&t;received from the SCSI BUS including the SWIDE.&n;&t;*/
id|SCR_JUMP
op_xor
id|IFFALSE
(paren
...
l_int|0
comma
multiline_comment|/*&n;&t;**&t;Save UA and RBC, since the PM0/1_SAVE &n;&t;**&t;sub-scripts haven&squot;t moved them to the &n;&t;**&t;context yet and the below MOV may just &n;&t;**&t;change their value.&n;&t;*/
id|SCR_STORE_ABS
(paren
id|ua
comma
...
comma
multiline_comment|/*&n;&t;**&t;Throw away the IGNORE WIDE RESIDUE message.&n;&t;**&t;since we just did take care of it.&n;&t;*/
id|SCR_MOVE_ABS
(paren
l_int|2
)paren
...
l_int|0
comma
multiline_comment|/*&n;&t;**&t;Restore UA and RBC registers and return.&n;&t;*/
id|SCR_LOAD_ABS
(paren
id|ua
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_SCR_64_1 &gt;---------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;We must grab the SWIDE and move it to &n;&t;**&t;memory.&n;&t;**&n;&t;**&t;- Save UA (32 bit address) in &lt;scratch&gt;.&n;&t;**&t;- Move address bit 32..39 to SFBR.&n;&t;**&t;- Increment UA (updated address).&n;&t;*/
id|SCR_STORE_ABS
(paren
id|ua
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_COM_64 &gt;-----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;- Save DRS.&n;&t;**&t;- Load DRS with address bit 32..39 of the&n;&t;**&t;  location to write the SWIDE.&n;&t;**&t;  SFBR has been loaded with these bits.&n;&t;**&t;  (Look above).&n;&t;*/
id|SCR_STORE_ABS
(paren
id|drs
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_COMMON &gt;-----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;- Save current DSA&n;&t;**&t;- Load DSA with bit 0..31 of the memory &n;&t;**&t;  location to write the SWIDE.&n;&t;*/
id|SCR_STORE_ABS
(paren
id|dsa
comma
...
comma
multiline_comment|/*&n;&t;**&t;Move the SWIDE to memory.&n;&t;**&t;Clear the WSR bit.&n;&t;*/
id|SCR_STORE_REL
(paren
id|swide
comma
...
l_int|0
comma
multiline_comment|/*&n;&t;**&t;Restore the original DSA.&n;&t;*/
id|SCR_LOAD_ABS
(paren
id|dsa
comma
...
multiline_comment|/*--------------------------&lt; SWIDE_FIN_32 &gt;-----------------------*/
comma
(brace
multiline_comment|/*&n;&t;**&t;For 32 bit chip, the following SCRIPTS &n;&t;**&t;instruction is patched with a JUMP to dispatcher.&n;&t;**&t;(Look into the C code).&n;&t;*/
id|SCR_LOAD_ABS
(paren
id|drs
comma
...
comma
multiline_comment|/*&n;&t;**&t;64 bit chip only.&n;&t;**&t;If PM handling from SCRIPTS, we are just &n;&t;**&t;a helper for the C code, so jump to &n;&t;**&t;dispatcher now.&n;&t;*/
id|SCR_FROM_REG
(paren
id|ccntl0
)paren
...
comma
multiline_comment|/*&n;&t;**&t;64 bit chip with hardware PM handling enabled.&n;&t;**&n;&t;**&t;Since we are paranoid:), we donnot want &n;&t;**&t;a SWIDE followed by a CHMOV(1) to lead to &n;&t;**&t;a CHMOV(0) in our PM context.&n;&t;**&t;We check against such a condition.&n;&t;**&t;Also does the C code.&n;&t;*/
id|SCR_FROM_REG
(paren
id|rbc
)paren
...
l_int|0
comma
multiline_comment|/*&n;&t;**&t;If we are there, RBC(0..23) is zero, &n;&t;**&t;and we just have to load the current &n;&t;**&t;DATA SCRIPTS address (register TEMP) &n;&t;**&t;with the IA and go to dispatch.&n;&t;**&t;No PM context is needed.&n;&t;*/
id|SCR_STORE_ABS
(paren
id|ia
comma
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t;**&t;Phase mismatch handled by SCRIPTS (53C895A or 53C896) ?&n;  &t;*/
r_if
c_cond
(paren
id|np-&gt;features
...
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Check the PCI clock frequency.&n;&t;**&t;Must be done after prepare_setting since it &n;&t;**&t;destroys STEST1 that is used to probe for &n;&t;**&t;the clock doubler.&n;&t;*/
id|i
op_assign
(paren
r_int
...
(paren
id|KERN_ERR
l_string|&quot;%s: PCI clock seems too high (%u KHz).&bslash;n&quot;
comma
id|ncr_name
c_func
(paren
...
suffix:semicolon
multiline_comment|/*&n;&t;**&t;If not 64 bit chip, patch some places in SCRIPTS.&n;&t;*/
r_if
c_cond
(paren
op_logical_neg
...
id|DPR
suffix:semicolon
multiline_comment|/*&n;&t;**&t;If 64 bit (53C895A or 53C896) enable 40 bit address table &n;&t;**&t;indirect addressing for MOVE.&n;&t;*/
r_if
c_cond
(paren
id|np-&gt;features
...
multiline_comment|/*&n;&t;**&t;If phase mismatch handled by scripts (53C895A or 53C896),&n;&t;**&t;set PM jump addresses.&n;&t;*/
r_if
c_cond
(paren
id|np-&gt;features
...
(brace
multiline_comment|/*&n;&t;&t;**&t;Hmmm... The device may want to also ignore &n;&t;&t;**&t;this residue but it must send immediately the&n;&t;&t;**&t;appropriate message. We snoop the SCSI BUS &n;&t;&t;**&t;and will just throw away this message from &n;&t;&t;**&t;SCRIPTS if the SWIDE is to be ignored.&n;&t;&t;*/
r_if
c_cond
(paren
(paren
...
multiline_comment|/*&n;&t;&t;**&t;We must grab the SWIDE.&n;&t;&t;**&t;We will use some complex SCRIPTS for that.&n;&t;&t;*/
r_else
(brace
id|OUTL
(paren
...
multiline_comment|/*&n;&t;&t;&t;**&t;Adjust our data pointer context.&n;&t;&t;&t;*/
op_increment
id|pm-&gt;sg.addr
suffix:semicolon
op_decrement
id|pm-&gt;sg.size
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;**&t;Hmmm... Could it be possible that a SWIDE that &n;&t;&t;&t;**&t;is followed by a 1 byte CHMOV would lead to &n;&t;&t;&t;**&t;a CHMOV(0). Anyway, we handle it by just &n;&t;&t;&t;**&t;skipping context that would attempt a CHMOV(0).&n;&t;&t;&t;*/
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n;&t;**&t;Deduce the index of the sg entry.&n;&t;**&t;Keep track of the index of the first valid entry.&n;&t;**&t;If result is dp_sg = MAX_SCATTER, then we are at the &n;&t;**&t;end of the data.&n;&t;*/
id|tmp
op_assign
id|scr_to_cpu
c_func
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Should have been checked by the caller.&n;&t;*/
r_if
c_cond
(paren
id|cp-&gt;phys.header.lastp
...
multiline_comment|/*==========================================================&n;**&n;**&t;ncr chip handler for MESSAGE REJECT received for &n;**&t;a WIDE or SYNCHRONOUS negotiation.&n;**&n;**==========================================================&n;**&n;**&t;Read comments above.&n;**&n;**----------------------------------------------------------&n;*/
...
macro_line|#elif&t;LINUX_VERSION_CODE &lt;= LinuxVersionCode(2,3,12)
r_static
r_int
id|__init
...
macro_line|#else&t;/* LINUX_VERSION_CODE &gt; LinuxVersionCode(2,3,12) */
r_static
r_int
id|__init
DECL|function|pci_get_base_address
...
c_func
(paren
l_string|&quot;sym53c8xx: NvRAM type=%x trailer=%x %x %x %x %x %x byte_count=%d/%d checksum=%x/%x&bslash;n&quot;
comma
id|nvram-&gt;type
comma
id|nvram-&gt;trailer
