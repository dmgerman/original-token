...
suffix:semicolon
macro_line|#if 1
multiline_comment|/* Tell tray is open if the drive is not ready.  Seems there is&n;&t; * no way to check whenever the tray is really open, but this way&n;&t; * we get auto-close-on-open work. And it seems to have no ill&n;         * effects with caddy drives... */
r_return
id|CDS_TRAY_OPEN
suffix:semicolon
macro_line|#else
...
(brace
multiline_comment|/* these are compatible with the ide-cd driver */
r_case
id|CDROMREADRAW
suffix:colon
r_case
...
id|CDROMREADMODE2
suffix:colon
macro_line|#if CONFIG_BLK_DEV_SR_VENDOR 
(brace
r_int
r_char
op_star
...
suffix:semicolon
multiline_comment|/* 2336 */
r_else
multiline_comment|/* some SCSI drives do not allow this one */
id|blocksize
op_assign
id|CD_FRAMESIZE_RAW
suffix:semicolon
multiline_comment|/* 2352 */
r_if
c_cond
(paren
id|copy_from_user
