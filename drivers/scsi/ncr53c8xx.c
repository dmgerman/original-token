...
DECL|macro|NCR_DATE
mdefine_line|#define NCR_DATE &quot;pl24 96/12/14&quot;
...
suffix:semicolon
DECL|variable|driver_safe_setup
r_static
r_struct
id|ncr_driver_setup
...
mdefine_line|#define bootverbose (driver_setup.verbose)
...
id|rv_scntl3
suffix:semicolon
multiline_comment|/*&n;&t;**&t;controller chip dependent maximal offset.&n;&t;*/
DECL|member|maxoffs
id|u_char
id|maxoffs
suffix:semicolon
multiline_comment|/*&n;&t;**&t;controller scsi clock frequency and available divisors&n;&t;*/
...
mdefine_line|#define PRINT_LUN(np, target, lun) &bslash;&n;printf(&quot;%s-&lt;%d,%d&gt;: &quot;, ncr_name(np), (int) (target), (int) (lun))
...
id|printf
(paren
l_string|&quot;%s: Ultra%s SCSI support enabled&bslash;n&quot;
comma
id|ncr_name
c_func
(paren
...
suffix:semicolon
multiline_comment|/*&n; * Return immediately from recursive call&n; */
r_if
c_cond
(paren
id|np-&gt;resetting
...
id|np-&gt;resetting
suffix:semicolon
multiline_comment|/*&n; * Perform chip reset, SCSI reset, wait 2 seconds&n; */
id|OUTB
(paren
id|nc_istat
comma
...
l_int|1
suffix:semicolon
multiline_comment|/*&n; * Reinitialise the NCR and wake-up pending commands&n; */
id|ncr_init
c_func
(paren
id|np
...
l_int|1
suffix:semicolon
multiline_comment|/*&n; * Complete awaiting commands with DID_RESET&n; */
...
suffix:semicolon
multiline_comment|/*&n; * If the involved command was not in a driver queue, complete it &n; * with DID_RESET, in order to keep it alive.&n; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*===============================================================&n;**&n;**&t;NCR chips allow burst lengths of 2, 4, 8, 16, 32, 64, 128 &n;**&t;transfers. 32,64,128 are only supported by 875 and 895 chips.&n;**&t;We use log base 2 (burst length) as internal code, with &n;**&t;value 0 meaning &quot;burst disabled&quot;.&n;**&n;**===============================================================&n;*/
...
r_else
multiline_comment|/**&t;NCR53C825A or NCR53C875 or NCR53C895&t;&t;**/
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* burst op-code fetch, read multiple */
multiline_comment|/* read line, burst 128 (ctest5&amp;4) */
id|np-&gt;rv_dcntl
op_assign
id|PFEN
op_or
...
l_int|0x20
suffix:semicolon
multiline_comment|/* large dma fifo (0x20) */
)brace
)brace
...
macro_line|#if 0
multiline_comment|/*&n;&t; *&t;Do not enable read-multiple for 810A revision 0x11&n;&t; */
r_if
c_cond
(paren
id|np-&gt;device_id
...
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Set differential mode.&n;&t;*/
r_switch
c_cond
(paren
...
id|printf
(paren
l_string|&quot;%s: initial value of dmode/dcntl/ctest3/4/5 = (hex) %02x/%02x/%02x/%02x/%02x&bslash;n&quot;
comma
id|ncr_name
c_func
(paren
...
id|printf
(paren
l_string|&quot;%s: final value of dmode/dcntl/ctest3/4/5 = (hex) %02x/%02x/%02x/%02x/%02x&bslash;n&quot;
comma
id|ncr_name
c_func
(paren
...
id|printf
(paren
l_string|&quot;%s: setting up differential mode&bslash;n&quot;
comma
id|ncr_name
c_func
(paren
...
multiline_comment|/*&n;&t;**    Enable GPIO0 pin for writing.&n;&t;**    Patch the script for LED support.&n;&t;*/
r_if
c_cond
(paren
...
multiline_comment|/*==========================================================&n;**&n;**&t;Get clock factor and sync divisor.&n;**&n;**==========================================================&n;*/
DECL|macro|SCSI_NCR_USE_ALL_DIVISORS
mdefine_line|#define SCSI_NCR_USE_ALL_DIVISORS
multiline_comment|/*&n;**&t;NCR chip clock divisor table.&n;**&t;Multiplied by 2x2000000 in order to avoid useless operations in &n;**&t;the code that gets clock factor and sync divisor from sync factor.&n;*/
DECL|macro|_2M
mdefine_line|#define _2M 2000000
DECL|variable|ncr_div2_2M
r_static
id|u_long
id|ncr_div2_2M
(braket
...
suffix:semicolon
multiline_comment|/*&n;**&t;Get clock factor and sync divisor for a given sync factor period.&n;**&t;Returns the clock factor, scntl3 and resulting period.&n;*/
...
id|np-&gt;clock_khz
suffix:semicolon
multiline_comment|/* Clock in kHz&t;&t;*/
r_int
id|idiv
op_assign
id|np-&gt;clock_divn
suffix:semicolon
multiline_comment|/* # divisors supported&t;*/
id|u_long
id|fak
comma
id|per
...
id|per_clk
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Compute the synchronous period in nano-seconds&n;&t;*/
r_if
c_cond
(paren
id|fac
...
id|fac
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Find the greatest divisor that allows an input speed &n;&t;**&t;faster than the period.&n;&t;*/
id|per_clk
op_assign
id|per
op_star
...
(brace
macro_line|#ifndef SCSI_NCR_USE_ALL_DIVISORS
r_if
c_cond
(paren
id|idiv
...
suffix:semicolon
macro_line|#ifdef SCSI_NCR_USE_ALL_DIVISORS
multiline_comment|/*&n;&t;**&t;Try the next divisor and choose the one that give &n;&t;**&t;the fastest output speed.&n;&t;*/
r_if
c_cond
(paren
id|idiv
...
c_func
(paren
l_string|&quot;fac=%d idiv=%d per=%d fak=%x &quot;
comma
id|fac
comma
id|idiv
...
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Deduce the value of controller sync period from scntl3.&n;&t;*/
id|idiv
op_assign
(paren
(paren
...
id|printf
(paren
l_string|&quot;%s%d.%d MB/s (%d ns, offset %d)&bslash;n&quot;
comma
id|msg
comma
id|mb10
...
l_int|0xffffff
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Take into account dma fifo and various buffers and latches,&n;&t;**&t;only if the interrupted phase in an OUTPUT phase.&n;&t;*/
r_if
c_cond
(paren
(paren
...
c_func
(paren
l_string|&quot;INPUT phase mismatch with DMA fifo not empty, P%x%x RL=%d&bslash;n&quot;
comma
id|cmd
op_amp
l_int|7
