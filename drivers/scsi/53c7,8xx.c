...
multiline_comment|/*&n; * Function static struct NCR53c7x0_cmd *allocate_cmd (Scsi_Cmnd *cmd)&n; * &n; * Purpose : If we have not already allocated enough NCR53c7x0_cmd&n; *&t;structures to satisfy any allowable number of simultaneous &n; *&t;commands for this host; do so (using either scsi_malloc()&n; *&t;or kmalloc() depending on configuration), and add them to the &n; *&t;free list.&n; *&n; *&t;Return the first free NCR53c7x0_cmd structure (which are &n; * &t;reused in a LIFO maner to minimize cache thrashing).&n; *&n; * Inputs : cmd - SCSI command&n; *&n; * Returns : NCR53c7x0_cmd structure allocated on behalf of cmd;&n; *&t;NULL on failure.&n; */
r_static
r_struct
id|NCR53c7x0_cmd
op_star
...
id|printk
(paren
l_string|&quot;scsi%d : scsi_malloc(%d) failed&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|size
...
id|cmd-&gt;lun
suffix:semicolon
multiline_comment|/* Keep the last one out of the free list so we can use it now */
r_if
c_cond
(paren
id|i
...
multiline_comment|/* If for some reason, our allocation attempt failed here, we&squot;ll&n;&t;try again later */
r_if
c_cond
(paren
id|i
