...
suffix:semicolon
multiline_comment|/*&n; * Function:    scsi_get_request_handler()&n; *&n; * Purpose:     Selects queue handler function for a device.&n; *&n; * Arguments:   SDpnt   - device for which we need a handler function.&n; *&n; * Returns:     Nothing&n; *&n; * Lock status: No locking assumed or required.&n; *&n; * Notes:       Most devices will end up using scsi_request_fn for the&n; *              handler function (at least as things are done now).&n; *              The &quot;block&quot; feature basically ensures that only one of&n; *              the blocked hosts is active at one time, mainly to work around&n; *              buggy DMA chipsets where the memory gets starved.&n; *              For this case, we have a special handler function, which&n; *              does some checks and ultimately calls scsi_request_fn.&n; *&n; *              As a future enhancement, it might be worthwhile to add support&n; *              for stacked handlers - there might get to be too many permutations&n; *              otherwise.  Then again, we might just have one handler that does&n; *              all of the special cases (a little bit slower), and those devices&n; *              that don&squot;t need the special case code would directly call &n; *              scsi_request_fn.&n; *&n; *              As it stands, I can think of a number of special cases that&n; *              we might need to handle.  This would not only include the blocked&n; *              case, but single_lun (for changers), and any special handling&n; *              we might need for a spun-down disk to spin it back up again.&n; */
DECL|function|scsi_get_request_handler
id|request_fn_proc
op_star
