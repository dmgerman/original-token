...
mdefine_line|#define BUSLOGIC_VERSION &quot;1.15&quot;
...
multiline_comment|/* Since the host adapters have room to buffer 32 commands internally, there&n;   is some virtue in setting BUSLOGIC_MAILBOXES to 32.  The maximum value&n;   appears to be 255, since the Count parameter to the Initialize Extended&n;   Mailbox command is limited to one byte. */
DECL|macro|BUSLOGIC_MAILBOXES
mdefine_line|#define BUSLOGIC_MAILBOXES 32
DECL|macro|BUSLOGIC_CMDLUN
mdefine_line|#define BUSLOGIC_CMDLUN 4&t;/* Arbitrary, but seems to work well. */
...
multiline_comment|/*&n;  This is a major rewrite of the interrupt handler to support the newer&n;  and faster PCI cards.  While the previous interrupt handler was supposed&n;  to handle multiple incoming becoming available mailboxes during the same&n;  interrupt, my testing showed that in practice only a single mailbox was&n;  ever made available.  With the 946C and 956C, multiple incoming mailboxes&n;  being ready for processing during a single interrupt occurs much more&n;  frequently, and so care must be taken to avoid race conditions managing&n;  the Host Adapter Interrupt Register, which can lead to lost interrupts.&n;&n;  Leonard N. Zubkoff, 23-Mar-95&n;*/
...
id|shpnt-&gt;io_port
suffix:semicolon
multiline_comment|/*&n;      This interrupt handler is now specified to use the SA_INTERRUPT&n;      protocol, so interrupts are inhibited on entry until explicitly&n;      allowed again.  Read the Host Adapter Interrupt Register, and&n;      complain if there is no pending interrupt being signaled.&n;    */
id|interrupt_flags
op_assign
id|inb
c_func
...
multiline_comment|/*&n;      With interrupts still inhibited, scan through the incoming mailboxes&n;      in strict round robin fashion saving the status information and&n;      then freeing the mailbox.  A second pass over the completed commands&n;      will be made separately to complete their processing.&n;    */
id|mbi
op_assign
id|HOSTDATA
c_func
...
multiline_comment|/*&n;      With interrupts no longer inhibited, iterate over the completed&n;      commands freeing resources and calling the completion routines.&n;      Since we exit upon completion of this loop, there is no need to&n;      inhibit interrupts before exit, as this will be handled by the&n;      fast interrupt assembly code we return to.&n;    */
id|sti
c_func
(paren
)paren
...
id|scpnt-&gt;sense_buffer
suffix:semicolon
multiline_comment|/* ccbcontrol, commlinkid, and linkptr are 0 due to above memset. */
macro_line|#if (BUSLOGIC_DEBUG &amp; BD_COMMAND)
(brace
r_int
id|i
