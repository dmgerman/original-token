...
multiline_comment|/* $Id: tmscsim.c,v 2.4 1998/10/24 08:50:47 garloff Exp $&t;&t;*/
multiline_comment|/*&t;Enhancements and bugfixes by&t;&t;&t;&t;&t;*&n; *&t;Kurt Garloff &lt;K.Garloff@ping.de&gt;&t;&t;&t;&t;*&n; ***********************************************************************/
...
DECL|macro|DC390_LOCK_INIT
macro_line|#   define DC390_LOCK_INIT { spinlock_t __unlocked = SPIN_LOCK_UNLOCKED; pACB-&gt;lock = __unlocked; };
macro_line|#  else
DECL|macro|DC390_LOCK_INIT
macro_line|#   define DC390_LOCK_INIT
macro_line|#  endif
...
singleline_comment|//#  define DC390_LOCK_INIT spin_lock_init (&amp;(pACB-&gt;lock))
macro_line|# else
...
macro_line|#   define DC390_UNLOCK_ACB_NI /* spin_unlock (&amp;(pACB-&gt;lock)) */
DECL|macro|DC390_LOCK_INIT
macro_line|#   define DC390_LOCK_INIT /* DC390_LOCK_INIT */
macro_line|#  endif /* 2 */
macro_line|# endif /* 3 */
...
l_int|0
suffix:semicolon
DECL|variable|dc390_CurrSyncOffset
r_static
id|UCHAR
...
suffix:semicolon
macro_line|# if defined(MODULE) &amp;&amp; LINUX_VERSION_CODE &gt;= LinuxVersionCode(2,1,30)
id|MODULE_PARM
c_func
(paren
id|tmscsim
...
id|tmscsim
comma
l_string|&quot;Host SCSI ID, Speed (0=10MHz), Device Flags, Adapter Flags, Max Tags (log2(tags)-1)&quot;
)paren
suffix:semicolon
macro_line|# endif
macro_line|#endif /* CONFIG_SCSI_DC390T_NOGENSUPP */
macro_line|#if defined(MODULE) &amp;&amp; LINUX_VERSION_CODE &gt;= LinuxVersionCode(2,1,30)
id|MODULE_AUTHOR
c_func
(paren
...
(paren
id|KERN_ERR
l_string|&quot;DC390: DCB not found (DCB=%08x, DCBmap[%2x]=%2x)&bslash;n&quot;
comma
(paren
r_int
)paren
...
(paren
id|KERN_DEBUG
l_string|&quot;DCB %08x (%02x,%02x) found.&bslash;n&quot;
comma
"&bslash;"
(paren
r_int
...
DECL|function|dc390_RewaitSRB0
r_static
id|__inline__
r_void
id|dc390_RewaitSRB0
...
c_func
(paren
l_string|&quot;DC390: RewaitSRB (%p, %p) pid = %li&bslash;n&quot;
comma
id|pDCB
comma
id|pSRB
...
DECL|function|dc390_DoWaitingSRB
r_static
r_void
...
DECL|function|dc390_SRBwaiting
r_static
id|__inline__
r_void
id|dc390_SRBwaiting
...
(paren
id|KERN_DEBUG
l_string|&quot;DC390: Queue Cmd=%02x,ID=%d,LUN=%d (pid=%li)&bslash;n&quot;
comma
"&bslash;"
id|cmd-&gt;cmnd
(braket
...
(paren
id|KERN_ERR
l_string|&quot;DC390: kmalloc for DCB failed, ID=%2x&bslash;n&quot;
comma
id|cmd-&gt;target
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Get first command */
id|pACB-&gt;CmdInQ
op_increment
suffix:semicolon
)brace
...
suffix:semicolon
multiline_comment|/* Queue command at the end */
id|pACB-&gt;CmdOutOfSRB
op_increment
suffix:semicolon
id|DC390_UNLOCK_ACB
...
(paren
id|KERN_DEBUG
l_string|&quot; ... command (%02x) queued successfully.&bslash;n&quot;
comma
id|pcmd-&gt;cmnd
(braket
l_int|0
...
id|printk
(paren
l_string|&quot;DC390: SRB: Xferred %08lx, Remain %08lx, State %08lx, Phase %02x&bslash;n&quot;
comma
id|pSRB-&gt;TotalXferredLen
comma
id|pSRB-&gt;SGToBeXferLen
...
id|printk
(paren
l_string|&quot;DC390: Status of last IRQ (DMA/SC/Int/IRQ): %08lx&bslash;n&quot;
comma
id|dc390_laststatus
)paren
suffix:semicolon
...
id|printk
(paren
l_string|&quot;DC390: Please report driver trouble to K.Garloff@ping.de&bslash;n&quot;
)paren
suffix:semicolon
)brace
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* abort() is too buggy at the moment. If it&squot;s called we are in trouble anyway.&n;     * so let&squot;s dump some info into the syslog at least. (KG, 98/08/20) */
r_if
c_cond
(paren
id|pDCB
...
id|printk
(paren
l_string|&quot;DC390: Abort command (pid %li, DCB %p, SRB %p)&bslash;n&quot;
comma
id|cmd-&gt;pid
comma
id|pDCB
...
(paren
id|KERN_INFO
l_string|&quot;DC390: alloc mem for DCB (ID %i, LUN %i): 0x%08x&bslash;n&quot;
comma
"&bslash;"
id|cmd-&gt;target
comma
...
multiline_comment|/***********************************************************************&n; * Function : static void dc390_updateDCB&n; *&n; * Purpose :  Set the configuration dependent DCB parameters&n; *&n; ***********************************************************************/
...
suffix:semicolon
multiline_comment|/***********************************************************************&n; * Function : static void dc390_updateDCBs&n; *&n; * Purpose :  Set the configuration dependent DCB params for all DCBs&n; *&n; ***********************************************************************/
...
id|psh-&gt;hostdata
suffix:semicolon
multiline_comment|/* pACB-&gt;IOPortBase = (USHORT) io_port; */
id|DC390_write8_
(paren
id|CtrlReg1
comma
...
macro_line|#ifndef CONFIG_SCSI_DC390T_NOGENSUPP
DECL|function|dc390_EEpromDefaults
r_static
r_void
id|__init
id|dc390_EEpromDefaults
(paren
id|UCHAR
id|index
)paren
...
suffix:semicolon
multiline_comment|/* Tagged Comds */
...
DECL|function|dc390_checkparams
r_static
r_void
id|__init
id|dc390_checkparams
(paren
r_void
)paren
(brace
...
(paren
id|KERN_INFO
l_string|&quot;DC390: setup %08x %08x %08x %08x %08x&bslash;n&quot;
comma
id|tmscsim
(braket
l_int|0
...
r_else
(brace
multiline_comment|/* if (tmscsim[0] &lt; 0 || tmscsim[0] &gt; 7) tmscsim[0] = 7; */
r_if
c_cond
(paren
id|tmscsim
...
(paren
id|KERN_INFO
l_string|&quot;DC390_init: Trying default EEPROM settings:&bslash;n&quot;
)paren
suffix:semicolon
id|dc390_checkparams
(paren
...
l_string|&quot; DevMode=0x%02x, AdaptMode=0x%02x, TaggedCmnds=%i (%i)&bslash;n&quot;
comma
id|tmscsim
(braket
l_int|0
...
id|DC390_UNLOCK_IO
suffix:semicolon
multiline_comment|/* Remove when going to new eh */
)brace
r_else
id|printk
(paren
...
id|ok
suffix:semicolon
multiline_comment|/* Speed: NegoPeriod */
r_if
c_cond
(paren
op_star
...
c_func
(paren
l_string|&quot;AdapterID %i, SelTimeout %i ms&bslash;n&quot;
comma
id|shpnt-&gt;this_id
comma
(paren
...
c_func
(paren
l_string|&quot;TagMaxNum %i, Status %i, ACBFlag %i, GlitchEater %i ns&bslash;n&quot;
comma
id|pACB-&gt;TagMaxNum
comma
id|pACB-&gt;status
...
c_func
(paren
l_string|&quot;Statistics: Cmnds %li, Cmnds not sent directly %li, Out of SRB conds %li&bslash;n&quot;
comma
id|pACB-&gt;Cmds
comma
id|pACB-&gt;CmdInQ
...
c_func
(paren
l_string|&quot;            Lost arbitrations %li&bslash;n&quot;
comma
id|pACB-&gt;SelLost
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Idx ID LUN Prty Sync DsCn SndS TagQ STOP NegoPeriod SyncSpeed SyncOffs&bslash;n&quot;
)paren
suffix:semicolon
id|pDCB
op_assign
...
suffix:semicolon
singleline_comment|//YESNO(pDCB-&gt;SyncMode &amp; EN_ATN_STOP);
id|YESNO
c_func
(paren
id|pDCB-&gt;DevMode
...
(paren
id|KERN_INFO
l_string|&quot;DC390: Free DCB (ID %i, LUN %i): 0x%08x&bslash;n&quot;
comma
"&bslash;"
id|pDCB-&gt;UnitSCSIID
comma
