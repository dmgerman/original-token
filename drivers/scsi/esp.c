...
l_int|0x08000000
suffix:semicolon
multiline_comment|/* Reset interface to FAS */
id|dregs-&gt;cond_reg
op_assign
id|DMA_RST_SCSI
suffix:semicolon
multiline_comment|/* Reset DVMA itself */
id|tmp
op_assign
(paren
id|DMA_PARITY_OFF
...
multiline_comment|/* If we don&squot;t know how to handle the dvma,&n;&t;&t;&t; * do not use this device.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
r_else
(brace
multiline_comment|/* On HME, two reg sets exist, first is DVMA,&n;&t;&t;&t;&t; * second is ESP registers.&n;&t;&t;&t;&t; */
id|esp-&gt;eregs
op_assign
id|eregs
op_assign
...
multiline_comment|/* This is getting messy but it has to be done&n;&t;&t;&t; * correctly or else you get weird behavior all&n;&t;&t;&t; * over the place.  We are trying to basically&n;&t;&t;&t; * figure out three pieces of information.&n;&t;&t;&t; *&n;&t;&t;&t; * a) Clock Conversion Factor&n;&t;&t;&t; *&n;&t;&t;&t; *    This is a representation of the input&n;&t;&t;&t; *    crystal clock frequency going into the&n;&t;&t;&t; *    ESP on this machine.  Any operation whose&n;&t;&t;&t; *    timing is longer than 400ns depends on this&n;&t;&t;&t; *    value being correct.  For example, you&squot;ll&n;&t;&t;&t; *    get blips for arbitration/selection during&n;&t;&t;&t; *    high load or with multiple targets if this&n;&t;&t;&t; *    is not set correctly.&n;&t;&t;&t; *&n;&t;&t;&t; * b) Selection Time-Out&n;&t;&t;&t; *&n;&t;&t;&t; *    The ESP isn&squot;t very bright and will arbitrate&n;&t;&t;&t; *    for the bus and try to select a target&n;&t;&t;&t; *    forever if you let it.  This value tells&n;&t;&t;&t; *    the ESP when it has taken too long to&n;&t;&t;&t; *    negotiate and that it should interrupt&n;&t;&t;&t; *    the CPU so we can see what happened.&n;&t;&t;&t; *    The value is computed as follows (from&n;&t;&t;&t; *    NCR/Symbios chip docs).&n;&t;&t;&t; *&n;&t;&t;&t; *          (Time Out Period) *  (Input Clock)&n;&t;&t;&t; *    STO = ----------------------------------&n;&t;&t;&t; *          (8192) * (Clock Conversion Factor)&n;&t;&t;&t; *&n;&t;&t;&t; *    You usually want the time out period to be&n;&t;&t;&t; *    around 250ms, I think we&squot;ll set it a little&n;&t;&t;&t; *    bit higher to account for fully loaded SCSI&n;&t;&t;&t; *    bus&squot;s and slow devices that don&squot;t respond so&n;&t;&t;&t; *    quickly to selection attempts. (yeah, I know&n;&t;&t;&t; *    this is out of spec. but there is a lot of&n;&t;&t;&t; *    buggy pieces of firmware out there so bite me)&n;&t;&t;&t; *&n;&t;&t;&t; * c) Imperical constants for synchronous offset&n;&t;&t;&t; *    and transfer period register values&n;&t;&t;&t; *&n;&t;&t;&t; *    This entails the smallest and largest sync&n;&t;&t;&t; *    period we could ever handle on this ESP.&n;&t;&t;&t; */
id|fmhz
op_assign
id|prom_getintdefault
c_func
...
(brace
multiline_comment|/* If we can&squot;t find anything reasonable,&n;&t;&t;&t;&t; * just assume 20MHZ.  This is the clock&n;&t;&t;&t;&t; * frequency of the older sun4c&squot;s where I&squot;ve&n;&t;&t;&t;&t; * been unable to find the clock-frequency&n;&t;&t;&t;&t; * PROM property.  All other machines provide&n;&t;&t;&t;&t; * useful values it seems.&n;&t;&t;&t;&t; */
id|ccf
op_assign
id|ESP_CCF_F4
suffix:semicolon
...
c_func
(paren
l_string|&quot;SCSI ID %d  Clock %d MHz CCF=%d Time-Out %d &quot;
comma
id|esp-&gt;scsi_id
comma
(paren
...
suffix:semicolon
multiline_comment|/* Find the burst sizes this dma/sbus/esp supports. */
id|bsizes
op_assign
id|prom_getintdefault
c_func
...
id|info
comma
l_string|&quot;&bslash;tPROM node&bslash;t&bslash;t%08lx&bslash;n&quot;
comma
(paren
r_int
r_int
...
id|i
suffix:semicolon
multiline_comment|/* Hold off if we&squot;ve been reselected. */
r_if
c_cond
(paren
id|esp-&gt;disconnected_SC
...
(brace
multiline_comment|/* But we are nice and allow tapes to disconnect. */
r_if
c_cond
(paren
id|SDptr-&gt;type
...
op_logical_or
macro_line|#if 1 /* Until I find out why HME barfs with disconnects enabled... */
id|toshiba_cdrom_hwbug_wkaround
op_logical_or
id|SDptr-&gt;borken
op_logical_or
...
c_func
(paren
l_string|&quot;esp%d: sglist at %p with %d buffers&bslash;n&quot;
comma
id|esp-&gt;esp_id
comma
id|SCpnt-&gt;buffer
...
id|SCpnt-&gt;SCp.buffer-&gt;dvma_address
suffix:semicolon
)brace
id|SCpnt-&gt;SCp.Status
op_assign
...
multiline_comment|/* Did they drop these fabs on the floor or what?!?!! */
DECL|function|hme_fifo_hwbug_workaround
r_static
r_inline
r_void
id|hme_fifo_hwbug_workaround
c_func
(paren
r_struct
id|Sparc_ESP
...
id|esp-&gt;sreg
suffix:semicolon
multiline_comment|/* Cannot safely frob the fifo for these following cases. */
r_if
c_cond
(paren
id|sreg_datainp
...
(brace
multiline_comment|/* SMCC can bite me. */
id|tmp
op_assign
id|dregs-&gt;cond_reg
suffix:semicolon
...
multiline_comment|/* Misc. esp helper routines. */
DECL|function|esp_setcount
r_static
r_inline
r_void
id|esp_setcount
...
DECL|function|sreg_to_dataphase
r_static
r_inline
r_int
...
multiline_comment|/* Complete a command, this is the only way to safely finish&n; * a command with disconnected commands or a reset interrupt&n; * pending because scsi_done() can enable interrupts full blast.&n; */
DECL|function|safe_esp_done
r_static
r_inline
r_void
id|safe_esp_done
...
id|dregs-&gt;cond_reg
suffix:semicolon
multiline_comment|/* Touchy chip, this stupid HME scsi adapter... */
id|esp_setcount
c_func
(paren
id|eregs
...
id|esp-&gt;current_transfer_size
suffix:semicolon
multiline_comment|/* Uhhh, might not want both of these conditionals to run&n;&t; * at once on HME due to the fifo problems it has.  Consider&n;&t; * changing it to:&n;&t; *&n;&t; * &t;if(!(esp-&gt;sreg &amp; ESP_STAT_TCNT)) {&n;&t; * &t;&t;bytes_sent -= ecount;&n;&t; * &t;} else if(SCptr-&gt;SCp.phase == in_dataout) {&n;&t; * &t;&t;bytes_sent -= fifocnt;&n;&t; *&t;}&n;&t; *&n;&t; * But only for the HME case, leave the current code alone&n;&t; * for all other ESP revisions as we know the existing code&n;&t; * works just fine for them.&n;&t; */
id|ESPDATA
c_func
(paren
(paren
...
(brace
multiline_comment|/* I&squot;ve seen this happen due to lost state in this&n;&t;&t; * driver.  No idea why it happened, but allowing&n;&t;&t; * this value to be negative caused things to&n;&t;&t; * lock up.  This allows greater chance of recovery.&n;&t;&t; */
id|ESPLOG
c_func
(paren
(paren
...
suffix:semicolon
multiline_comment|/* Must flush both FIFO and the DVMA on HME. */
r_if
c_cond
(paren
id|esp-&gt;erev
...
(brace
multiline_comment|/* XXX this still doesn&squot;t fix the problem... */
id|esp_cmd
c_func
(paren
id|esp
...
suffix:semicolon
multiline_comment|/* No need for explicit restore pointers operation. */
id|esp-&gt;snip
op_assign
l_int|0
suffix:semicolon
...
(paren
(paren
l_string|&quot;esp_do_phase_determine: &quot;
)paren
)paren
suffix:semicolon
r_if
...
(paren
(paren
l_string|&quot;to data phase&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_return
...
id|ESP_STATP
suffix:colon
multiline_comment|/* Whee, status phase, finish up the command. */
id|ESPPHASE
c_func
(paren
(paren
l_string|&quot;to status phase&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|esp_cmd
...
r_else
(brace
multiline_comment|/* Using DVMA for status/message bytes is&n;&t;&t;&t;&t; * unreliable on HME, nice job QLogic.&n;&t;&t;&t;&t; * Happy Meal indeed....&n;&t;&t;&t;&t; */
id|esp_cmd
c_func
(paren
id|esp
...
(paren
(paren
l_string|&quot;to msgout phase&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|esp_advance_phase
...
(paren
(paren
l_string|&quot;to msgin phase&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|esp_advance_phase
...
id|ESP_CMDP
suffix:colon
multiline_comment|/* Ugh, we&squot;re running a non-standard command the&n;&t;&t;&t; * ESP doesn&squot;t understand, one byte at a time.&n;&t;&t;&t; */
id|ESPPHASE
c_func
(paren
(paren
l_string|&quot;to cmd phase&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|esp_advance_phase
...
id|SCptr-&gt;device
suffix:semicolon
multiline_comment|/* This means real problems if we see this&n;&t;&t; * here.  Unless we were actually trying&n;&t;&t; * to force the device to abort/reset.&n;&t;&t; */
id|ESPLOG
c_func
(paren
(paren
...
(paren
(paren
l_string|&quot;esp%d: to unknown phase&bslash;n&quot;
comma
id|esp-&gt;esp_id
)paren
)paren
...
id|SAVE_POINTERS
suffix:colon
multiline_comment|/* We handle this all automatically. */
r_return
l_int|0
suffix:semicolon
r_case
...
(paren
id|KERN_INFO
l_string|&quot;esp%d: target %d [period %dns offset %d %d.%02dMHz %s SCSI%s]&bslash;n&quot;
comma
id|esp-&gt;esp_id
comma
id|esp-&gt;current_SC-&gt;target
...
c_func
(paren
l_string|&quot;esp%d: AIEEE wide transfer for %d size not supported.&bslash;n&quot;
comma
id|esp-&gt;esp_id
comma
id|size
...
c_func
(paren
l_string|&quot;esp%d: 16 byte WIDE transfers enabled for target %d.&bslash;n&quot;
comma
id|esp-&gt;esp_id
comma
id|SCptr-&gt;target
...
(paren
(paren
l_string|&quot;finish selection&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_return
...
(paren
(paren
l_string|&quot;esp%d: DMA error %08lx&bslash;n&quot;
comma
id|esp-&gt;esp_id
comma
id|dregs-&gt;cond_reg
...
multiline_comment|/* We&squot;re trying to fight stack problems, and inline as much as&n;&t; * possible without making this driver a mess. hate hate hate&n;&t; * This is tier-one in our dual level SCSI state machine.&n;&t; */
id|again
suffix:colon
r_switch
c_cond
...
id|do_reset_complete
suffix:colon
multiline_comment|/* Tricky, we don&squot;t want to cause any more commands to&n;&t;&t; * go out until we clear all the live cmds by hand.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|SCptr-&gt;SCp.have_data_in
comma
id|SCptr-&gt;request_bufflen
comma
id|esp-&gt;edev-&gt;my_bus
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;esp%d: weird weird weird, current_SC not NULL after &quot;
...
multiline_comment|/* Now it is safe to execute more things. */
r_if
c_cond
(paren
id|esp-&gt;issue_SC
...
id|again
suffix:semicolon
multiline_comment|/* It is ok to take irq&squot;s on one esp while the other&n;&t; * is amidst the processing of a reset.&n;&t; */
id|running
op_increment
suffix:semicolon
r_if
...
(paren
(paren
l_string|&quot;esp_intr: yieee, recursive interrupt!&bslash;n&quot;
)paren
)paren
suffix:semicolon
)brace
