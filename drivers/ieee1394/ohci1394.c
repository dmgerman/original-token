...
multiline_comment|/*&n; * Things known to be working:&n; * . Async Request Transmit&n; * . Async Response Receive&n; * . Async Request Receive&n; * . Async Response Transmit&n; * . Iso Receive&n; * &n; * Things not implemented:&n; * . Iso Transmit&n; * . DMA to user&squot;s space in iso receive mode&n; * . DMA error recovery&n; *&n; * Things to be fixed:&n; * . Config ROM&n; *&n; * Known bugs:&n; * . Self-id are sometimes not received properly &n; *   if card is initialized with no other nodes &n; *   on the bus&n; * . SONY CXD3222 chip is not working properly&n; * . Apple PowerBook detected but not working yet&n; */
multiline_comment|/* &n; * Acknowledgments:&n; *&n; * Emilie Chung&t;&lt;emilie.chung@axis.com&gt;&n; *  . Tip on Async Request Filter&n; * Pascal Drolet &lt;pascal.drolet@informission.ca&gt;&n; *  . Various tips for optimization and functionnalities&n; * Robert Ficklin &lt;rficklin@westengineering.com&gt;&n; *  . Loop in irq_handler&n; * James Goodwin &lt;jamesg@Filanet.com&gt;&n; *  . Various tips on initialization, self-id reception, etc.&n; * Albrecht Dress &lt;ad@mpifr-bonn.mpg.de&gt;&n; *  . Apple PowerBook detection&n; * Daniel Kobras &lt;daniel.kobras@student.uni-tuebingen.de&gt;&n; *  . Reset the board properly before leaving&n; */
...
DECL|function|free_dma_fbuf_ctx
r_static
r_int
id|free_dma_fbuf_ctx
c_func
(paren
r_struct
id|dma_fbuf_ctx
...
id|ohci-&gt;id
comma
l_string|&quot;Freeing dma_fbuf_ctx %d&quot;
comma
(paren
op_star
id|d
...
id|dma_fbuf_ctx
op_star
DECL|function|alloc_dma_fbuf_ctx
id|alloc_dma_fbuf_ctx
c_func
(paren
r_struct
id|ti_ohci
...
id|ohci-&gt;id
comma
l_string|&quot;failed to allocate dma_fbuf_ctx&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
...
id|ohci-&gt;id
comma
l_string|&quot;failed to allocate dma fbuffer&quot;
)paren
suffix:semicolon
id|free_dma_fbuf_ctx
c_func
...
id|ohci-&gt;id
comma
l_string|&quot;failed to allocate dma fbuf prg&quot;
)paren
suffix:semicolon
id|free_dma_fbuf_ctx
c_func
...
id|ohci-&gt;id
comma
l_string|&quot;Iso DMA to User&squot;s Space: %d buffers &quot;
...
DECL|function|initialize_dma_fbuf_prg
r_static
r_void
id|initialize_dma_fbuf_prg
c_func
(paren
r_struct
id|dma_cmd
...
id|PAGE_SIZE
suffix:semicolon
multiline_comment|/* the first descriptor will sync and read only 4 bytes */
id|prg
(braket
l_int|0
)braket
...
DECL|function|initialize_dma_fbuf_ctx
r_static
r_void
id|initialize_dma_fbuf_ctx
c_func
(paren
r_struct
id|dma_fbuf_ctx
...
multiline_comment|/* Set bufferFill, no header */
id|reg_write
c_func
(paren
id|ohci
...
DECL|function|fbuf_ctx_listening
r_int
id|fbuf_ctx_listening
c_func
(paren
r_struct
id|ti_ohci
...
multiline_comment|/* find a free iso context */
r_for
c_loop
(paren
id|i
...
id|ohci-&gt;id
comma
l_string|&quot;Couldn&squot;t allocate fbuf context&quot;
)paren
suffix:semicolon
r_return
op_minus
...
id|ohci-&gt;id
comma
l_string|&quot;fbuf context %d buf size is different from mmap size&quot;
comma
id|d-&gt;ctx
)paren
suffix:semicolon
...
id|ohci-&gt;id
comma
l_string|&quot;fbuf context %d is not allocated&quot;
comma
id|d-&gt;ctx
)paren
suffix:semicolon
...
id|ohci-&gt;id
comma
l_string|&quot;current fbuf context not set&quot;
)paren
suffix:semicolon
r_return
op_minus
...
suffix:semicolon
DECL|function|wakeup_dma_fbuf_ctx
r_int
id|wakeup_dma_fbuf_ctx
c_func
(paren
r_struct
id|ti_ohci
...
id|retval
suffix:semicolon
multiline_comment|/* &n;&t; *Delay aftger soft reset to make sure everything has settled&n;&t; * down (sanity)&n;&t; */
id|mdelay
c_func
(paren
l_int|100
...
id|ohci-&gt;id
comma
l_string|&quot;%d iso contexts available&quot;
comma
id|ohci-&gt;nb_iso_ctx
)paren
suffix:semicolon
...
id|ohci-&gt;id
comma
l_string|&quot;Cannot allocate fbuf_context&quot;
)paren
suffix:semicolon
r_return
op_minus
...
multiline_comment|/* Insert a packet in the AT DMA fifo and generate the DMA prg */
...
id|ohci-&gt;id
comma
l_string|&quot;AT DMA ctx=%d Running out of prgs... waiting&quot;
comma
id|d-&gt;ctx
)paren
suffix:semicolon
...
id|d-&gt;ctrlClear
comma
l_string|&quot;AT DMA runaway loop... bailing out&quot;
)paren
suffix:semicolon
r_return
l_int|0
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * enable cycleTimer cycleMaster cycleSource&n;&t;&t;&t;&t; */
id|DBGMSG
c_func
(paren
id|ohci-&gt;id
...
id|ohci-&gt;id
comma
l_string|&quot;!!! try listen on channel %d !!!&quot;
comma
id|arg
)paren
suffix:semicolon
...
multiline_comment|/*&n;&t;&t;&t; * Tip by James Goodwin &lt;jamesg@Filanet.com&gt;&n;&t;&t;&t; * We need to handle write requests that are received &n;&t;&t;&t; * to our middle address space (posted writes).&n;&t;&t;&t; * In this case, the hardware generates an &n;&t;&t;&t; * ack_complete... but, if we pass the packet up to &n;&t;&t;&t; * the subsystem, it will try and send a response &n;&t;&t;&t; * (which it shouldn&squot;t), because it assumes we &n;&t;&t;&t; * returned ack_pending.&n;&t;&t;&t; */
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * Tip by James Goodwin &lt;jamesg@Filanet.com&gt;&n;&t;&t;&t;&t; * Handle case of posted writes. If we receive&n;&t;&t;&t;&t; * an ack_complete, we should not send a&n;&t;&t;&t;&t; * response. Fake out upper layers by turning &n;&t;&t;&t;&t; * the packet into a broadcast packet... we &n;&t;&t;&t;&t; * should really modify the core stack to &n;&t;&t;&t;&t; * accept an ack received argument and figure &n;&t;&t;&t;&t; * out whether to reply.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
(paren
...
id|ohci-&gt;id
comma
l_string|&quot;Packet sent to node %d ack=0x%X spd=%d ctx=%d&quot;
comma
(paren
id|packet-&gt;header
(braket
...
id|p
comma
l_string|&quot;prg[%d][%d]: %p %08x %08x %08x %08x&bslash;n&quot;
comma
id|i
comma
id|j
...
suffix:semicolon
macro_line|#ifdef _VIDEO_1394_H
multiline_comment|/* Free the frame buffer context */
r_if
c_cond
(paren
id|ohci-&gt;fbuf_context
...
c_func
(paren
l_string|&quot;ohci1394: unable to get major %d&bslash;n&quot;
comma
id|OHCI1394_MAJOR
)paren
suffix:semicolon
