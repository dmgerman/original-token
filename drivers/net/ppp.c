...
mdefine_line|#define bsd_compress&t;(*ppp-&gt;sc_xcomp-&gt;compress)
...
macro_line|#ifndef PPP_MAX_DEV
DECL|macro|PPP_MAX_DEV
mdefine_line|#define PPP_MAX_DEV 256
macro_line|#endif
...
DECL|macro|ppp2tty
mdefine_line|#define ppp2tty(ppp)&t;((struct tty_struct *) ppp-&gt;tty)
DECL|macro|ppp2dev
mdefine_line|#define ppp2dev(ppp)&t;((struct device *) ppp-&gt;dev)
DECL|struct|ppp_hdr
r_struct
id|ppp_hdr
(brace
...
mdefine_line|#define PPP_HARD_HDR_LEN&t;(sizeof (struct ppp_hdr))
DECL|struct|ppp_ctrl
r_typedef
r_struct
id|ppp_ctrl
(brace
...
id|next
suffix:semicolon
multiline_comment|/* Next structure in the list&t;*/
DECL|member|name
r_char
id|name
(braket
...
suffix:semicolon
multiline_comment|/* Name of the device&t;&t;*/
DECL|member|ppp
r_struct
id|ppp
id|ppp
suffix:semicolon
multiline_comment|/* PPP control table&t;&t;*/
DECL|member|dev
r_struct
id|device
id|dev
suffix:semicolon
multiline_comment|/* Device information table&t;*/
DECL|typedef|ppp_ctrl_t
)brace
id|ppp_ctrl_t
suffix:semicolon
...
suffix:semicolon
DECL|macro|ctl2ppp
mdefine_line|#define ctl2ppp(ctl) (struct ppp *)    &amp;ctl-&gt;ppp
DECL|macro|ctl2dev
...
(paren
id|KERN_INFO
l_string|&quot;PPP Dynamic channel allocation code copyright 1995 &quot;
...
(paren
id|KERN_ERR
l_string|&quot;ppp_tty_open: couldn&squot;t allocate ppp channel&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
(paren
id|KERN_ERR
l_string|&quot;ppp_tty_open: no space for compression buffers!&bslash;n&quot;
)paren
suffix:semicolon
id|ppp_release
(paren
...
(paren
id|KERN_ERR
l_string|&quot;ppp_tty_open: no space for user receive buffer&bslash;n&quot;
)paren
suffix:semicolon
id|ppp_release
(paren
...
multiline_comment|/*&n; * Set the flags for 8 data bits and no parity.&n; *&n; * Actually, it sets the flags for d7 being 0/1 and parity being even/odd&n; * so that the normal processing would have all flags set at the end of the&n; * session. A missing flag bit would denote an error condition.&n; */
macro_line|#ifdef CHECK_CHARACTERS
r_if
c_cond
(paren
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n; * Branch on the character. Process the escape character. The sequence ESC ESC&n; * is defined to be ESC.&n; */
r_switch
c_cond
(paren
id|chr
...
id|PPP_ESCAPE
suffix:colon
multiline_comment|/* PPP_ESCAPE: invert bit in next character */
id|ppp-&gt;escape
op_assign
id|PPP_TRANS
suffix:semicolon
...
r_break
suffix:semicolon
multiline_comment|/*&n; * Adjust the character and if the frame is to be discarded then simply&n; * ignore the character until the ending FLAG is received.&n; */
id|chr
op_xor_assign
id|ppp-&gt;escape
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;ppp: successfully queued %d bytes, flags = %x&bslash;n&quot;
comma
id|len
op_plus
l_int|2
...
(paren
id|KERN_WARNING
l_string|&quot;ppp: dropping packet on the floor.&bslash;n&quot;
)paren
suffix:semicolon
id|slhc_toss
(paren
...
r_break
suffix:semicolon
multiline_comment|/*&n; * The protocol sequence is complete at this end&n; */
r_case
id|CCP_RESETACK
suffix:colon
r_if
...
(paren
id|KERN_WARNING
l_string|&quot;ppp: frame with bad fcs, excess = %x&bslash;n&quot;
comma
id|ppp-&gt;rbuf-&gt;fcs
op_xor
id|PPP_GOODFCS
...
multiline_comment|/*&n; * Ignore the leading ADDRESS and CONTROL fields in the frame.&n; */
id|addr
op_assign
id|PPP_ALLSTATIONS
suffix:semicolon
...
multiline_comment|/*&n; * Rewrite the header with the full information. This may encroach upon&n; * the &squot;filler&squot; area in the buffer header. This is the purpose for the&n; * filler.&n; */
op_star
(paren
op_decrement
id|data
...
multiline_comment|/*&n; * Decompress the frame&n; */
id|new_count
op_assign
id|bsd_decompress
(paren
...
(paren
id|KERN_ERR
l_string|&quot;ppp_proto_comp: &quot;
l_string|&quot;decompress err %d&bslash;n&quot;
comma
id|new_count
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Reset the time of the last read operation.&n; */
r_if
c_cond
(paren
id|ppp-&gt;flags
...
multiline_comment|/*&n; * Before we attempt to write the frame to the user, ensure that the&n; * page tables are proper.&n; */
id|indx
op_assign
id|verify_area
(paren
...
(paren
id|KERN_DEBUG
l_string|&quot;ppp_stuff_char: %x %d&bslash;n&quot;
comma
(paren
r_int
r_int
...
(paren
id|KERN_DEBUG
l_string|&quot;ppp_dev_xmit_lower: fcs is %hx&bslash;n&quot;
comma
id|write_fcs
)paren
suffix:semicolon
...
(paren
id|KERN_DEBUG
l_string|&quot;ppp_dev_xmit: writing %d chars&bslash;n&quot;
comma
id|buf-&gt;count
)paren
suffix:semicolon
...
multiline_comment|/*&n; * The frame could not be compressed.&n; */
id|kfree
(paren
id|new_data
)paren
...
multiline_comment|/*&n; * The frame may not be compressed. Update the statistics before the&n; * count field is destroyed. The frame will be transmitted.&n; */
op_increment
id|ppp-&gt;stats.ppp_opackets
suffix:semicolon
id|ppp-&gt;stats.ppp_ooctects
...
multiline_comment|/*&n; * lock this PPP unit so we will be the only writer;&n; * sleep if necessary&n; */
r_while
c_loop
(paren
id|lock_buffer
...
(paren
id|KERN_DEBUG
l_string|&quot;ppp_tty_write: sleeping&bslash;n&quot;
)paren
suffix:semicolon
id|interruptible_sleep_on
(paren
...
multiline_comment|/*&n; * Ensure that the caller&squot;s buffer is valid.&n; */
id|status
op_assign
id|verify_area
(paren
...
(brace
multiline_comment|/*&n;&t;&t; * Found a handler for the protocol - try to allocate&n;&t;&t; * a compressor or decompressor.&n;&t;&t; */
id|error
op_assign
l_int|0
suffix:semicolon
...
c_func
(paren
l_string|&quot;ppp%ld: comp_alloc failed&bslash;n&quot;
comma
id|ppp2dev
(paren
id|ppp
...
c_func
(paren
l_string|&quot;ppp%ld: decomp_alloc failed&bslash;n&quot;
comma
id|ppp2dev
(paren
id|ppp
...
(paren
id|KERN_DEBUG
l_string|&quot;ppp%ld: no compressor for [%x %x %x], %x&bslash;n&quot;
comma
id|ppp2dev
(paren
id|ppp
...
(paren
id|KERN_DEBUG
l_string|&quot;ppp_tty_ioctl: get flags: addr %lx flags &quot;
l_string|&quot;%x&bslash;n&quot;
comma
id|param3
comma
...
(paren
id|KERN_INFO
l_string|&quot;ppp_tty_ioctl: get unit: %ld&quot;
comma
id|ppp2dev
(paren
id|ppp
...
(paren
id|KERN_INFO
l_string|&quot;ppp_tty_ioctl: get debug level %d&bslash;n&quot;
comma
id|temp_i
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;ppp_tty_ioctl: read demand dial info&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_break
...
(paren
id|KERN_INFO
l_string|&quot;ppp_tty_ioctl: get xasyncmap: addr %lx&bslash;n&quot;
comma
id|param3
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;ppp_tty_ioctl: invalid ioctl: %x, addr %lx&bslash;n&quot;
comma
id|param2
comma
id|param3
...
id|result
suffix:semicolon
multiline_comment|/*&n; * Must have write access to the buffer.&n; */
id|result
op_assign
(paren
r_char
...
suffix:semicolon
multiline_comment|/*&n; * Obtain the length from the IP header.&n; */
id|len
op_assign
(paren
(paren
...
multiline_comment|/*&n; * Ensure that the PPP device is still up&n; */
r_if
c_cond
(paren
op_logical_neg
...
(paren
id|KERN_WARNING
l_string|&quot;ppp_dev_xmit: packet sent on interface %s,&quot;
l_string|&quot; which is down for IP&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Print the frame being sent&n; */
r_if
c_cond
(paren
id|ppp-&gt;flags
...
id|KERN_WARNING
l_string|&quot;ppp_dev_xmit: packet sent on interface %s,&quot;
l_string|&quot; which is down&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Validate the tty linkage&n; */
r_if
c_cond
(paren
id|ppp-&gt;flags
...
multiline_comment|/*&n; * There are no available items. Allocate a device from the system pool&n; */
id|ctl
op_assign
(paren
id|ppp_ctrl_t
...
suffix:semicolon
multiline_comment|/* link in the new channel */
id|ctl-&gt;next
op_assign
id|ppp_list
suffix:semicolon
