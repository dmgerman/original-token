...
DECL|macro|OUT_INTR
mdefine_line|#define OUT_INTR&t;1
DECL|macro|IN_INTR
mdefine_line|#define IN_INTR&t;&t;0
...
suffix:semicolon
DECL|macro|dprintk
mdefine_line|#define dprintk(format, a...) printk(format, ##a)
macro_line|#else
DECL|macro|hex_dump
...
(paren
id|KERN_INFO
l_string|&quot;cyx_isr: %s, wandev.critical set to 0x%02X&bslash;n&quot;
comma
id|card-&gt;devname
comma
id|card-&gt;wandev.critical
...
multiline_comment|/* For all interrupts set the critical flag to CRITICAL_RX_INTR.&n;         * If the if_send routine is called with this flag set it will set&n;         * the enable transmit flag to 1. (for a delayed interrupt) */
id|card-&gt;wandev.critical
op_assign
id|CRITICAL_IN_ISR
suffix:semicolon
...
macro_line|#endif
multiline_comment|/* CYCLOM X Firmware-Specific Functions&n; *&n; * Almost all X.25 commands can unexpetedly fail due to so called &squot;X.25&n; * asynchronous events&squot; such as restart, interrupt, incoming call request,&n; * call clear request, etc.  They can&squot;t be ignored and have to be dealt with&n; * immediately.  To tackle with this problem we execute each interface command&n; * in a loop until good return code is received or maximum number of retries&n; * is reached.  Each interface command returns non-zero return code, an&n; * asynchronous event/error handler x25_error() is called.&n; */
...
(paren
id|KERN_INFO
l_string|&quot;name: addr:           tbusy:&bslash;n&quot;
)paren
suffix:semicolon
id|printk
(paren
...
(paren
id|KERN_INFO
l_string|&quot;%-5.5s %-15.15s   %ld&bslash;n&quot;
comma
id|chan-&gt;name
comma
id|chan-&gt;addr
