...
multiline_comment|/* the first 7 takes it out of tri-state (but still high) */
multiline_comment|/* the second resets it */
multiline_comment|/* note that after this point, any read of 6 will trigger an interrupt */
r_if
c_cond
(paren
id|dev-&gt;irq
...
(brace
multiline_comment|/* we&squot;re set up for interrupts */
r_if
c_cond
(paren
l_int|0xf8
...
(brace
multiline_comment|/* trigger an interrupt */
(paren
r_void
)paren
id|inb_p
...
r_else
(brace
multiline_comment|/* we&squot;re strictly polling mode */
id|idle
c_func
(paren
id|dev
...
id|err
suffix:semicolon
multiline_comment|/* occasionally the card comes up with reset latched, so we need&n;&t; * to &quot;reset the reset&quot; first of all -- check the irq first also&n;&t; */
id|save_flags
c_func
(paren
id|flags
...
multiline_comment|/* wait for strays */
id|straymask
op_assign
id|irqhitmask
suffix:semicolon
multiline_comment|/* pick up any strays */
multiline_comment|/* if someone already owns this address, don&squot;t probe */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* at this point, either we have an irq and the base addr, or&n;&t; * there isn&squot;t any irq and we don&squot;t know the base address, but&n;&t; * in either event the card is no longer latched in reset and&n;&t; * the irq request line is tri-stated.&n;&t; */
id|cli
c_func
(paren
)paren
...
c_func
(paren
l_string|&quot;ltpc card raised more than one interrupt!&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
multiline_comment|/* now, figure out which dma channel we&squot;re using */
multiline_comment|/* set up both dma 1 and 3 for read call */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* release the other dma channel */
r_if
c_cond
(paren
(paren
...
c_func
(paren
l_string|&quot;LocalTalk card found at %03x, IR%d, DMA%d.&bslash;n&quot;
comma
id|base
comma
id|irq
...
c_func
(paren
l_string|&quot;LocalTalk card found at %03x, DMA%d.  Using polled mode.&bslash;n&quot;
comma
id|base
comma
id|dma
...
c_func
(paren
l_string|&quot;finishing up transfer&bslash;n&quot;
)paren
suffix:semicolon
)brace
id|disable_dma
...
l_int|100
suffix:semicolon
multiline_comment|/* poll it once per second just in case */
)brace
r_else
(brace
id|ltpc_timer.expires
...
c_func
(paren
l_string|&quot;EIO from register_netdev&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_return
