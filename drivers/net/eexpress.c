...
l_int|0x0000
comma
multiline_comment|/* offsett,address (lo,hi) of SCB */
l_int|0x0000
comma
l_int|0x0000
comma
...
multiline_comment|/*&n; * Sanity check the suspected EtherExpress card&n; * Read hardware address, reset card, size memory and&n; * initialize buffer memory pointers. These should&n; * probably be held in dev-&gt;priv, incase someone has 2&n; * differently configured cards in their box (Arghhh!)&n; */
...
multiline_comment|/*&n; * Writes down the list of transmit buffers into card&n; * memory. Initial seperate, repeated transmits link&n; * them into a circular list, such that the CU can&n; * be constantly active, and unlink them as we reap&n; * transmitted packet buffers, so the CU doesn&squot;t loop&n; * and endlessly transmit packets. (Try hacking the driver&n; * to send continuous broadcast messages, say ARP requests&n; * on a subnet with Windows boxes running on Novell and&n; * LAN Workplace with EMM386. Amusing to watch them all die&n; * horribly leaving the Linux boxes up!)&n; */
...
id|set_loopback
suffix:semicolon
multiline_comment|/* yet more paranoia - since we&squot;re resetting the ASIC&n;&t;&t;&t;* that controls this function, how can it possibly work?&n;                        */
id|started
op_assign
l_int|0
suffix:semicolon
