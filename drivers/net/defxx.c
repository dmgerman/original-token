...
id|__initdata
op_assign
l_string|&quot;defxx.c:v1.05a 2000/06/11  Lawrence V. Stefani (stefani@lkg.dec.com) and others&bslash;n&quot;
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* have we already entered dfx_probe? */
"&f;"
...
id|bp_root
suffix:semicolon
DECL|variable|version_disp
r_static
r_int
id|version_disp
suffix:semicolon
DECL|variable|have_pci_driver
r_static
r_int
id|have_pci_driver
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;defxx: I/O range allocated to adapter (0x%X-0x%X) is already being used!&bslash;n&quot;
comma
id|port
comma
(paren
...
multiline_comment|/* Allocate a new device structure for this adapter */
id|dev
op_assign
id|dfx_alloc_device
(paren
...
(paren
id|KERN_ERR
l_string|&quot;defxx: alloc device failed&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|err_out_region
...
id|pdev
suffix:semicolon
multiline_comment|/*&n;&t; * FIXME FIXME FIXME&n;&t; * Suck!  The original driver didn&squot;t clean up after&n;&t; * itself at this stage, so we won&squot;t either.  Someone&n;&t; * needs to go back and see what (if anything) we need&n;&t; * to free here...  -jgarzik&n;&t; */
r_if
c_cond
(paren
id|dfx_driver_init
...
l_int|0
suffix:semicolon
multiline_comment|/* clear port address field in device structure on failure */
r_goto
id|err_out_region
suffix:semicolon
)brace
...
c_func
(paren
l_string|&quot;In dfx_probe...&bslash;n&quot;
)paren
suffix:semicolon
id|already_probed
op_assign
...
op_increment
suffix:semicolon
multiline_comment|/* only increment global board count on success */
r_else
id|dev-&gt;base_addr
op_assign
l_int|0
...
c_func
(paren
l_string|&quot;I/O range allocated to adapter (0x%X-0x%X) is already being used!&bslash;n&quot;
comma
id|port
comma
(paren
...
multiline_comment|/* Scan for FDDI PCI controllers */
id|i
op_assign
...
multiline_comment|/*&n;&t; * If we&squot;re at this point we&squot;re going through dfx_probe() for the first&n;&t; * time.  Return success (0) if we&squot;ve initialized 1 or more boards.&n;&t; * Otherwise, return failure (-ENODEV).&n;&t; */
r_return
(paren
id|num_boards
OG
...
c_func
(paren
l_string|&quot;In dfx_alloc_device...&bslash;n&quot;
)paren
suffix:semicolon
multiline_comment|/* Find next free fddi entry */
id|tmp_dev
op_assign
id|dev_alloc
c_func
...
(paren
id|KERN_ERR
l_string|&quot;Could not find free FDDI device structure for this adapter!&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
...
c_func
(paren
l_string|&quot;Device entry free, device name = %s&bslash;n&quot;
comma
id|tmp_dev-&gt;name
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;defxx: Could not allocate memory for private board structure!&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
...
l_int|0
suffix:semicolon
multiline_comment|/* set in dfx_bus_init() */
id|tmp_dev-&gt;if_port
op_assign
l_int|0
suffix:semicolon
...
id|dfx_close
suffix:semicolon
id|tmp_dev-&gt;hard_start_xmit
op_assign
op_amp
id|dfx_xmt_queue_pkt
suffix:semicolon
id|tmp_dev-&gt;hard_header
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/* set in fddi_setup() */
id|tmp_dev-&gt;rebuild_header
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/* set in fddi_setup() */
id|tmp_dev-&gt;set_multicast_list
op_assign
op_amp
id|dfx_ctl_set_multicast_list
suffix:semicolon
id|tmp_dev-&gt;set_mac_address
op_assign
op_amp
id|dfx_ctl_set_mac_address
suffix:semicolon
...
id|tmp_dev-&gt;header_cache_update
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/* not supported */
id|tmp_dev-&gt;change_mtu
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/* set in fddi_setup() */
...
"&f;"
multiline_comment|/*&n; * ============&n; * = dfx_open =&n; * ============&n; *   &n; * Overview:&n; *   Opens the adapter&n; *  &n; * Returns:&n; *   Condition code&n; *       &n; * Arguments:&n; *   dev - pointer to device information&n; *&n; * Functional Description:&n; *   This function brings the adapter to an operational state.&n; *&n; * Return Codes:&n; *   0&t;&t; - Adapter was successfully opened&n; *   -EAGAIN - Could not register IRQ or adapter initialization failed&n; *&n; * Assumptions:&n; *   This routine should only be called for a device that was&n; *   initialized successfully during the dfx_probe process.&n; *&n; * Side Effects:&n; *   Adapter should be in LINK_AVAILABLE or LINK_UNAVAILABLE state&n; *   if the open is successful.&n; */
...
id|priv-&gt;next
suffix:semicolon
multiline_comment|/* FIXME: need to unregister FDDI device here?&n;&t;&t; * The original driver didn&squot;t do it, but I think so..&n;&t;&t; *&t;-jgarzik&n;&t;&t; */
r_if
c_cond
(paren
id|priv-&gt;bus_type
