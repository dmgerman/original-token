multiline_comment|/*&n;** *************************************************************************&n;**&n;**&n;**     R C M T L . H             $Revision: 3 $&n;**&n;**&n;**  RedCreek Message Transport Layer header file.&n;**&n;**  ---------------------------------------------------------------------&n;**  ---     Copyright (c) 1997-1998, RedCreek Communications Inc.     ---&n;**  ---                   All rights reserved.                        ---&n;**  ---------------------------------------------------------------------&n;**&n;**  File Description:&n;**&n;**  Header file for host message transport layer API and data types.&n;**&n;**  This program is free software; you can redistribute it and/or modify&n;**  it under the terms of the GNU General Public License as published by&n;**  the Free Software Foundation; either version 2 of the License, or&n;**  (at your option) any later version.&n;&n;**  This program is distributed in the hope that it will be useful,&n;**  but WITHOUT ANY WARRANTY; without even the implied warranty of&n;**  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the&n;**  GNU General Public License for more details.&n;&n;**  You should have received a copy of the GNU General Public License&n;**  along with this program; if not, write to the Free Software&n;**  Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.&n;**&n;** *************************************************************************&n;*/
macro_line|#ifndef RCMTL_H
DECL|macro|RCMTL_H
mdefine_line|#define RCMTL_H
...
DECL|macro|RC_RTN_NOT_INIT
mdefine_line|#define RC_RTN_NOT_INIT             1
...
DECL|macro|RC_RTN_NO_STATUS
mdefine_line|#define RC_RTN_NO_STATUS            9
...
multiline_comment|/*&n; ** type PFNTXCALLBACK &n; **&n; ** Pointer to user&squot;s transmit callback function.  This user function is&n; ** called from RCProcMsgQ() when packet have been transmitted from buffers&n; ** given in the RCSendPacket() function.  BufferContext is a pointer to&n; ** an array of 32 bit context values.  These are the values the user assigned&n; ** and passed in the TCB to the RCSendPacket() function.  PcktCount&n; ** indicates the number of buffer context values in the BufferContext[] array.&n; ** The User&squot;s TransmitCallbackFunction should recover (put back in free queue)&n; ** the packet buffers associated with the buffer context values.&n; */
...
DECL|macro|RC_DSC_SUCCESS
mdefine_line|#define    RC_DSC_SUCCESS                         0x0000
DECL|macro|RC_DSC_DEVICE_FAILURE
mdefine_line|#define    RC_DSC_DEVICE_FAILURE                  0x0001
DECL|macro|RC_DSC_DESTINATION_NOT_FOUND
mdefine_line|#define    RC_DSC_DESTINATION_NOT_FOUND           0x0002
DECL|macro|RC_DSC_TRANSMIT_ERROR
mdefine_line|#define    RC_DSC_TRANSMIT_ERROR                  0x0003
DECL|macro|RC_DSC_TRANSMIT_ABORTED
mdefine_line|#define    RC_DSC_TRANSMIT_ABORTED                0x0004
DECL|macro|RC_DSC_RECEIVE_ERROR
mdefine_line|#define    RC_DSC_RECEIVE_ERROR                   0x0005
DECL|macro|RC_DSC_RECEIVE_ABORTED
mdefine_line|#define    RC_DSC_RECEIVE_ABORTED                 0x0006
DECL|macro|RC_DSC_DMA_ERROR
mdefine_line|#define    RC_DSC_DMA_ERROR                       0x0007
DECL|macro|RC_DSC_BAD_PACKET_DETECTED
mdefine_line|#define    RC_DSC_BAD_PACKET_DETECTED             0x0008
DECL|macro|RC_DSC_OUT_OF_MEMORY
mdefine_line|#define    RC_DSC_OUT_OF_MEMORY                   0x0009
DECL|macro|RC_DSC_BUCKET_OVERRUN
mdefine_line|#define    RC_DSC_BUCKET_OVERRUN                  0x000A
DECL|macro|RC_DSC_IOP_INTERNAL_ERROR
mdefine_line|#define    RC_DSC_IOP_INTERNAL_ERROR              0x000B
DECL|macro|RC_DSC_CANCELED
mdefine_line|#define    RC_DSC_CANCELED                        0x000C
DECL|macro|RC_DSC_INVALID_TRANSACTION_CONTEXT
mdefine_line|#define    RC_DSC_INVALID_TRANSACTION_CONTEXT     0x000D
DECL|macro|RC_DSC_DESTINATION_ADDRESS_DETECTED
mdefine_line|#define    RC_DSC_DESTINATION_ADDRESS_DETECTED    0x000E
DECL|macro|RC_DSC_DESTINATION_ADDRESS_OMITTED
mdefine_line|#define    RC_DSC_DESTINATION_ADDRESS_OMITTED     0x000F
DECL|macro|RC_DSC_PARTIAL_PACKET_RETURNED
mdefine_line|#define    RC_DSC_PARTIAL_PACKET_RETURNED         0x0010
...
DECL|macro|LAN_LINK_STATUS_DOWN
mdefine_line|#define LAN_LINK_STATUS_DOWN     0
DECL|macro|LAN_LINK_STATUS_UP
mdefine_line|#define LAN_LINK_STATUS_UP       1
...
multiline_comment|/*&n; ** RCResetLANCard()&n; **&n; ** Reset LAN card operation.  Causes a software reset of the ethernet&n; ** controller and restarts the command and receive units. Depending on &n; ** the ResourceFlags given, the buffers are either returned to the&n; ** host with reply status of RC_REPLY_STATUS_ABORT_NO_DATA_TRANSFER and&n; ** detailed status of RC_DSC_CANCELED (new receive buffers must be&n; ** posted after issuing this) OR the buffers are kept and reused by&n; ** the ethernet controller. If CallbackFunction is not NULL, the function&n; ** will be called when the reset is complete.  If the CallbackFunction is&n; ** NULL,a 1 will be put into the ReturnAddr after waiting for the reset &n; ** to complete (please disable adapter interrupts during this method).&n; ** Any outstanding transmit or receive buffers that are complete will be&n; ** returned via the normal reply messages before the requested resource&n; ** buffers are returned.&n; ** A call to RCPostRecvBuffers() is needed to return the ethernet to full&n; ** operation if the receive buffers were returned during LANReset.&n; ** Note: The IOP status is not affected by a LAN reset.&n; */
id|RC_RETURN
id|RCResetLANCard
c_func
(paren
...
suffix:semicolon
multiline_comment|/*&n; ** RCResetAdapter();&n; **     Initializes ADAPTERState to ADAPTER_STATE_RESET.&n; **     Stops access to outbound message Q.&n; **     Discards any outstanding transmit or posted receive buffers.&n; **     Clears outbound message Q. &n; */
id|RC_RETURN
id|RCResetAdapter
c_func
(paren
