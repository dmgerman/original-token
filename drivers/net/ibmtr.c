...
l_string|&quot;  modified 10/3/94 DW Morris, modified at VIA, ECP, France&bslash;n&quot;
l_string|&quot;  (3/9/95 F Farid and P Andre, 9/7/95 PA and 2/20/95 ML/PA/YD)&bslash;n&quot;
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* this is the major adapter probe loop.  For each call to tok_probe,&n;&t;   we try each remaining entry in TokBaseAddrs[] as a possible&n;&t;   adapter.  Once an entry is rejected or assigned, we zero it to&n;&t;   avoid duplicate use or worthless trial for the tok probe call*/
r_for
c_loop
(paren
id|iAddr
...
multiline_comment|/* Make sure PIO address not already assigned&n;&t;&t;   elsewhere before we muck with IO address */
r_if
c_cond
(paren
id|check_region
...
multiline_comment|/* Query the adapter PIO base port which will return&n;&t;&t;   indication of where MMIO was placed (per tech ref&n;&t;&t;   this assignment is done by BIOS - what is rational for&n;&t;&t;   where it is?).  We also have a coded interrupt address. */
id|segment
op_assign
id|inb
c_func
...
suffix:semicolon
multiline_comment|/* out of range values so we&squot;ll assume non-existant IO device */
r_if
c_cond
(paren
id|segment
...
multiline_comment|/* Compute the linear base address of the MMIO area&n;&t;&t;   as LINUX doesn&squot;t care about segments          */
id|t_mmio
op_assign
(paren
(paren
...
suffix:semicolon
multiline_comment|/* Now we will compare expected &squot;channelid&squot; strings with&n;&t;&t;   what we is there to learn of ISA/MCA or not TR card */
multiline_comment|/* !!!WARNING:!!!! It seems pretty silly to blunder ahead&n;&t;&t;   w/o verification that the mmio address we have found&n;&t;&t;   is valid storage -- perhaps this is tolerable for current&n;&t;&t;   hardware state??? */
id|cd_chanid
op_assign
(paren
id|CHANNEL_ID
...
multiline_comment|/* If we have an ISA board check for the ISA P&amp;P version,&n;&t;&t;   as it has different IRQ settings */
r_if
c_cond
(paren
id|cardpresent
...
multiline_comment|/* !!!! we could tighten validation by checking the HW Address&n;&t;   against the 1-s complement..  Move the get HW logic to here */
)brace
multiline_comment|/* The search loop has either completed with a presumed TR adapter&n;&t;   or none found.  Check situation ... march on if possible */
r_if
c_cond
(paren
id|PIOaddr
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* we assign the address for ISA devices; set RRR even to D000 for &n;&t;   shared RAM address */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;Initial interrupt : shared RAM located at %08X.&bslash;n&quot;
comma
id|encoded_addr
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Arrg. Transmitter busy for more than 50 msec. &quot;
l_string|&quot;Donald resets adapter, but resetting&bslash;n&quot;
l_string|&quot;the IBM tokenring adapter takes a long time.&quot;
l_string|&quot; It might not even help when the&bslash;n&quot;
l_string|&quot;ring is very busy, so we just wait a little longer &quot;
l_string|&quot;and hope for the best.&bslash;n&quot;
)paren
suffix:semicolon
id|dev-&gt;trans_start
op_add_assign
