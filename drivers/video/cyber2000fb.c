multiline_comment|/*&n; * linux/drivers/video/cyber2000fb.c&n; *&n; * Integraphics Cyber2000 frame buffer device&n; *&n; * Based on cyberfb.c&n; */
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/tty.h&gt;
macro_line|#include &lt;linux/malloc.h&gt;
macro_line|#include &lt;linux/delay.h&gt;
macro_line|#include &lt;linux/fb.h&gt;
macro_line|#include &lt;linux/pci.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;asm/irq.h&gt;
macro_line|#include &lt;asm/pgtable.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
macro_line|#include &lt;video/fbcon.h&gt;
macro_line|#include &lt;video/fbcon-cfb8.h&gt;
macro_line|#include &lt;video/fbcon-cfb16.h&gt;
macro_line|#include &lt;video/fbcon-cfb24.h&gt;
multiline_comment|/*&n; *    Some defaults&n; */
DECL|macro|DEFAULT_XRES
mdefine_line|#define DEFAULT_XRES&t;640
DECL|macro|DEFAULT_YRES
mdefine_line|#define DEFAULT_YRES&t;480
DECL|macro|DEFAULT_BPP
mdefine_line|#define DEFAULT_BPP&t;8
DECL|variable|CyberRegs
r_static
r_volatile
r_int
r_char
op_star
id|CyberRegs
suffix:semicolon
macro_line|#include &quot;cyber2000fb.h&quot;
DECL|variable|global_disp
r_static
r_struct
id|display
id|global_disp
suffix:semicolon
DECL|variable|fb_info
r_static
r_struct
id|fb_info
id|fb_info
suffix:semicolon
DECL|variable|current_par
r_static
r_struct
id|cyber2000fb_par
id|current_par
suffix:semicolon
DECL|variable|dispsw
r_static
r_struct
id|display_switch
op_star
id|dispsw
suffix:semicolon
DECL|variable|init_var
r_static
r_struct
id|fb_var_screeninfo
id|__initdata
id|init_var
op_assign
(brace
)brace
suffix:semicolon
macro_line|#ifdef DEBUG
DECL|function|debug_printf
r_static
r_void
id|debug_printf
c_func
(paren
r_char
op_star
id|fmt
comma
dot
dot
dot
)paren
(brace
r_char
id|buffer
(braket
l_int|128
)braket
suffix:semicolon
id|va_list
id|ap
suffix:semicolon
id|va_start
c_func
(paren
id|ap
comma
id|fmt
)paren
suffix:semicolon
id|vsprintf
c_func
(paren
id|buffer
comma
id|fmt
comma
id|ap
)paren
suffix:semicolon
id|va_end
c_func
(paren
id|ap
)paren
suffix:semicolon
id|printascii
c_func
(paren
id|buffer
)paren
suffix:semicolon
)brace
macro_line|#else
DECL|macro|debug_printf
mdefine_line|#define debug_printf(x...) do { } while (0)
macro_line|#endif
multiline_comment|/*&n; *    Predefined Video Modes&n; */
DECL|variable|cyber2000_res
r_static
r_const
r_struct
id|res
id|cyber2000_res
(braket
)braket
op_assign
(brace
(brace
l_int|640
comma
l_int|480
comma
(brace
l_int|0x5f
comma
l_int|0x4f
comma
l_int|0x50
comma
l_int|0x80
comma
l_int|0x52
comma
l_int|0x9d
comma
l_int|0x0b
comma
l_int|0x3e
comma
l_int|0x00
comma
l_int|0x40
comma
l_int|0xe9
comma
l_int|0x8b
comma
l_int|0xdf
comma
l_int|0x50
comma
l_int|0x00
comma
l_int|0xe6
comma
l_int|0x04
comma
l_int|0xc3
)brace
comma
l_int|0x00
comma
(brace
l_int|0xd2
comma
l_int|0xce
comma
l_int|0xdb
comma
l_int|0x54
)brace
)brace
comma
(brace
l_int|800
comma
l_int|600
comma
(brace
l_int|0x7f
comma
l_int|0x63
comma
l_int|0x64
comma
l_int|0x00
comma
l_int|0x66
comma
l_int|0x10
comma
l_int|0x6f
comma
l_int|0xf0
comma
l_int|0x00
comma
l_int|0x60
comma
l_int|0x5b
comma
l_int|0x8f
comma
l_int|0x57
comma
l_int|0x64
comma
l_int|0x00
comma
l_int|0x59
comma
l_int|0x6e
comma
l_int|0xe3
)brace
comma
l_int|0x00
comma
(brace
l_int|0x52
comma
l_int|0x85
comma
l_int|0xdb
comma
l_int|0x54
)brace
)brace
comma
(brace
l_int|1024
comma
l_int|768
comma
(brace
l_int|0x9f
comma
l_int|0x7f
comma
l_int|0x80
comma
l_int|0x80
comma
l_int|0x8b
comma
l_int|0x94
comma
l_int|0x1e
comma
l_int|0xfd
comma
l_int|0x00
comma
l_int|0x60
comma
l_int|0x03
comma
l_int|0x86
comma
l_int|0xff
comma
l_int|0x80
comma
l_int|0x0f
comma
l_int|0x00
comma
l_int|0x1e
comma
l_int|0xe3
)brace
comma
l_int|0x00
comma
(brace
l_int|0xd0
comma
l_int|0x52
comma
l_int|0xdb
comma
l_int|0x54
)brace
)brace
comma
macro_line|#if 0
(brace
l_int|1152
comma
l_int|886
comma
(brace
)brace
comma
(brace
)brace
)brace
comma
macro_line|#endif
(brace
l_int|1280
comma
l_int|1024
comma
(brace
l_int|0xce
comma
l_int|0x9f
comma
l_int|0xa0
comma
l_int|0x8f
comma
l_int|0xa2
comma
l_int|0x1f
comma
l_int|0x28
comma
l_int|0x52
comma
l_int|0x00
comma
l_int|0x40
comma
l_int|0x08
comma
l_int|0x8f
comma
l_int|0xff
comma
l_int|0xa0
comma
l_int|0x00
comma
l_int|0x03
comma
l_int|0x27
comma
l_int|0xe3
)brace
comma
l_int|0x1d
comma
(brace
l_int|0xb4
comma
l_int|0x4b
comma
l_int|0xdb
comma
l_int|0x54
)brace
)brace
comma
(brace
l_int|1600
comma
l_int|1200
comma
(brace
l_int|0xff
comma
l_int|0xc7
comma
l_int|0xc9
comma
l_int|0x9f
comma
l_int|0xcf
comma
l_int|0xa0
comma
l_int|0xfe
comma
l_int|0x10
comma
l_int|0x00
comma
l_int|0x40
comma
l_int|0xcf
comma
l_int|0x89
comma
l_int|0xaf
comma
l_int|0xc8
comma
l_int|0x00
comma
l_int|0xbc
comma
l_int|0xf1
comma
l_int|0xe3
)brace
comma
l_int|0x1f
comma
(brace
l_int|0xbd
comma
l_int|0x10
comma
l_int|0xdb
comma
l_int|0x54
)brace
)brace
)brace
suffix:semicolon
DECL|macro|NUM_TOTAL_MODES
mdefine_line|#define NUM_TOTAL_MODES    arraysize(cyber2000_res)
DECL|variable|igs_regs
r_static
r_const
r_char
id|igs_regs
(braket
)braket
op_assign
(brace
l_int|0x10
comma
l_int|0x10
comma
l_int|0x12
comma
l_int|0x00
comma
l_int|0x13
comma
l_int|0x00
comma
l_int|0x30
comma
l_int|0x21
comma
l_int|0x31
comma
l_int|0x00
comma
l_int|0x32
comma
l_int|0x00
comma
l_int|0x33
comma
l_int|0x01
comma
l_int|0x50
comma
l_int|0x00
comma
l_int|0x51
comma
l_int|0x00
comma
l_int|0x52
comma
l_int|0x00
comma
l_int|0x53
comma
l_int|0x00
comma
l_int|0x54
comma
l_int|0x00
comma
l_int|0x55
comma
l_int|0x00
comma
l_int|0x56
comma
l_int|0x00
comma
l_int|0x57
comma
l_int|0x01
comma
l_int|0x58
comma
l_int|0x00
comma
l_int|0x59
comma
l_int|0x00
comma
l_int|0x5a
comma
l_int|0x00
comma
l_int|0x70
comma
l_int|0x0b
comma
l_int|0x71
comma
l_int|0x10
comma
l_int|0x72
comma
l_int|0x45
comma
l_int|0x73
comma
l_int|0x30
comma
l_int|0x74
comma
l_int|0x1b
comma
l_int|0x75
comma
l_int|0x1e
comma
l_int|0x76
comma
l_int|0x00
comma
l_int|0x7a
comma
l_int|0xc8
)brace
suffix:semicolon
DECL|variable|crtc_idx
r_static
r_const
r_char
id|crtc_idx
(braket
)braket
op_assign
(brace
l_int|0x00
comma
l_int|0x01
comma
l_int|0x02
comma
l_int|0x03
comma
l_int|0x04
comma
l_int|0x05
comma
l_int|0x06
comma
l_int|0x07
comma
l_int|0x08
comma
l_int|0x09
comma
l_int|0x10
comma
l_int|0x11
comma
l_int|0x12
comma
l_int|0x13
comma
l_int|0x14
comma
l_int|0x15
comma
l_int|0x16
comma
l_int|0x17
)brace
suffix:semicolon
DECL|function|cyber2000_init_hw
r_static
r_void
id|cyber2000_init_hw
c_func
(paren
r_const
r_struct
id|res
op_star
id|res
)paren
(brace
r_int
id|i
suffix:semicolon
id|debug_printf
c_func
(paren
l_string|&quot;init vga hw for %dx%d&bslash;n&quot;
comma
id|res-&gt;xres
comma
id|res-&gt;yres
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0xef
comma
l_int|0x3c2
)paren
suffix:semicolon
id|cyber2000_crtcw
c_func
(paren
l_int|0x11
comma
l_int|0x0b
)paren
suffix:semicolon
id|cyber2000_attrw
c_func
(paren
l_int|0x11
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_seqw
c_func
(paren
l_int|0x00
comma
l_int|0x01
)paren
suffix:semicolon
id|cyber2000_seqw
c_func
(paren
l_int|0x01
comma
l_int|0x01
)paren
suffix:semicolon
id|cyber2000_seqw
c_func
(paren
l_int|0x02
comma
l_int|0x0f
)paren
suffix:semicolon
id|cyber2000_seqw
c_func
(paren
l_int|0x03
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_seqw
c_func
(paren
l_int|0x04
comma
l_int|0x0e
)paren
suffix:semicolon
id|cyber2000_seqw
c_func
(paren
l_int|0x00
comma
l_int|0x03
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
r_sizeof
(paren
id|crtc_idx
)paren
suffix:semicolon
id|i
op_increment
)paren
id|cyber2000_crtcw
c_func
(paren
id|crtc_idx
(braket
id|i
)braket
comma
id|res-&gt;crtc_regs
(braket
id|i
)braket
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0x0a
suffix:semicolon
id|i
OL
l_int|0x10
suffix:semicolon
id|i
op_increment
)paren
id|cyber2000_crtcw
c_func
(paren
id|i
comma
l_int|0
)paren
suffix:semicolon
id|cyber2000_crtcw
c_func
(paren
l_int|0x18
comma
l_int|0xff
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x00
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x01
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x02
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x03
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x04
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x05
comma
l_int|0x60
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x06
comma
l_int|0x05
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x07
comma
l_int|0x0f
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x08
comma
l_int|0xff
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
)paren
id|cyber2000_attrw
c_func
(paren
id|i
comma
id|i
)paren
suffix:semicolon
id|cyber2000_attrw
c_func
(paren
l_int|0x10
comma
l_int|0x01
)paren
suffix:semicolon
id|cyber2000_attrw
c_func
(paren
l_int|0x11
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_attrw
c_func
(paren
l_int|0x12
comma
l_int|0x0f
)paren
suffix:semicolon
id|cyber2000_attrw
c_func
(paren
l_int|0x13
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_attrw
c_func
(paren
l_int|0x14
comma
l_int|0x00
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
r_sizeof
(paren
id|igs_regs
)paren
suffix:semicolon
id|i
op_add_assign
l_int|2
)paren
id|cyber2000_grphw
c_func
(paren
id|igs_regs
(braket
id|i
)braket
comma
id|igs_regs
(braket
id|i
op_plus
l_int|1
)braket
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x11
comma
id|res-&gt;crtc_ofl
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|4
suffix:semicolon
id|i
op_add_assign
l_int|1
)paren
id|cyber2000_grphw
c_func
(paren
l_int|0xb0
op_plus
id|i
comma
id|res-&gt;clk_regs
(braket
id|i
)braket
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x90
comma
l_int|0x01
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0xb9
comma
l_int|0x80
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0xb9
comma
l_int|0x00
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x56
comma
l_int|0x3ce
)paren
suffix:semicolon
id|i
op_assign
id|cyber2000_inb
c_func
(paren
l_int|0x3cf
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|i
op_or
l_int|4
comma
l_int|0x3cf
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x04
comma
l_int|0x3c6
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|i
comma
l_int|0x3cf
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x20
comma
l_int|0x3c0
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0xff
comma
l_int|0x3c6
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|256
suffix:semicolon
id|i
op_increment
)paren
(brace
id|cyber2000_outb
c_func
(paren
id|i
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0x3c9
)paren
suffix:semicolon
)brace
)brace
DECL|variable|cyber2000fb_ops
r_static
r_struct
id|fb_ops
id|cyber2000fb_ops
suffix:semicolon
multiline_comment|/* -------------------- Hardware specific routines ------------------------- */
multiline_comment|/*&n; * Hardware Cyber2000 Acceleration&n; */
DECL|function|cyber2000_accel_wait
r_static
r_void
id|cyber2000_accel_wait
c_func
(paren
r_void
)paren
(brace
r_int
id|count
op_assign
l_int|10000
suffix:semicolon
r_while
c_loop
(paren
id|cyber2000_inb
c_func
(paren
l_int|0xbf011
)paren
op_amp
l_int|0x80
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|count
op_decrement
)paren
(brace
id|debug_printf
c_func
(paren
l_string|&quot;accel_wait timed out&bslash;n&quot;
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0xbf011
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|udelay
c_func
(paren
l_int|10
)paren
suffix:semicolon
)brace
)brace
r_static
r_void
DECL|function|cyber2000_accel_setup
id|cyber2000_accel_setup
c_func
(paren
r_struct
id|display
op_star
id|p
)paren
(brace
id|dispsw
op_member_access_from_pointer
id|setup
c_func
(paren
id|p
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|cyber2000_accel_bmove
id|cyber2000_accel_bmove
c_func
(paren
r_struct
id|display
op_star
id|p
comma
r_int
id|sy
comma
r_int
id|sx
comma
r_int
id|dy
comma
r_int
id|dx
comma
r_int
id|height
comma
r_int
id|width
)paren
(brace
r_int
r_int
id|src
comma
id|dst
comma
id|chwidth
op_assign
id|p-&gt;var.xres_virtual
op_star
id|fontheight
c_func
(paren
id|p
)paren
suffix:semicolon
r_int
id|v
op_assign
l_int|0x8000
suffix:semicolon
r_if
c_cond
(paren
id|sx
OL
id|dx
)paren
(brace
id|sx
op_add_assign
id|width
op_minus
l_int|1
suffix:semicolon
id|dx
op_add_assign
id|width
op_minus
l_int|1
suffix:semicolon
id|v
op_or_assign
l_int|4
suffix:semicolon
)brace
r_if
c_cond
(paren
id|sy
OL
id|dy
)paren
(brace
id|sy
op_add_assign
id|height
op_minus
l_int|1
suffix:semicolon
id|dy
op_add_assign
id|height
op_minus
l_int|1
suffix:semicolon
id|v
op_or_assign
l_int|2
suffix:semicolon
)brace
id|sx
op_mul_assign
id|fontwidth
c_func
(paren
id|p
)paren
suffix:semicolon
id|dx
op_mul_assign
id|fontwidth
c_func
(paren
id|p
)paren
suffix:semicolon
id|src
op_assign
id|sx
op_plus
id|sy
op_star
id|chwidth
suffix:semicolon
id|dst
op_assign
id|dx
op_plus
id|dy
op_star
id|chwidth
suffix:semicolon
id|width
op_assign
id|width
op_star
id|fontwidth
c_func
(paren
id|p
)paren
op_minus
l_int|1
suffix:semicolon
id|height
op_assign
id|height
op_star
id|fontheight
c_func
(paren
id|p
)paren
op_minus
l_int|1
suffix:semicolon
id|cyber2000_accel_wait
c_func
(paren
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x00
comma
l_int|0xbf011
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x03
comma
l_int|0xbf048
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
id|width
comma
l_int|0xbf060
)paren
suffix:semicolon
r_if
c_cond
(paren
id|p-&gt;var.bits_per_pixel
op_ne
l_int|24
)paren
(brace
id|cyber2000_outl
c_func
(paren
id|dst
comma
l_int|0xbf178
)paren
suffix:semicolon
id|cyber2000_outl
c_func
(paren
id|src
comma
l_int|0xbf170
)paren
suffix:semicolon
)brace
r_else
(brace
id|cyber2000_outl
c_func
(paren
id|dst
op_star
l_int|3
comma
l_int|0xbf178
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|dst
comma
l_int|0xbf078
)paren
suffix:semicolon
id|cyber2000_outl
c_func
(paren
id|src
op_star
l_int|3
comma
l_int|0xbf170
)paren
suffix:semicolon
)brace
id|cyber2000_outw
c_func
(paren
id|height
comma
l_int|0xbf062
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
id|v
comma
l_int|0xbf07c
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
l_int|0x2800
comma
l_int|0xbf07e
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|cyber2000_accel_clear
id|cyber2000_accel_clear
c_func
(paren
r_struct
id|vc_data
op_star
id|conp
comma
r_struct
id|display
op_star
id|p
comma
r_int
id|sy
comma
r_int
id|sx
comma
r_int
id|height
comma
r_int
id|width
)paren
(brace
r_int
r_int
id|dst
suffix:semicolon
id|u32
id|bgx
op_assign
id|attr_bgcol_ec
c_func
(paren
id|p
comma
id|conp
)paren
suffix:semicolon
id|dst
op_assign
id|sx
op_star
id|fontwidth
c_func
(paren
id|p
)paren
op_plus
id|sy
op_star
id|p-&gt;var.xres_virtual
op_star
id|fontheight
c_func
(paren
id|p
)paren
suffix:semicolon
id|width
op_assign
id|width
op_star
id|fontwidth
c_func
(paren
id|p
)paren
op_minus
l_int|1
suffix:semicolon
id|height
op_assign
id|height
op_star
id|fontheight
c_func
(paren
id|p
)paren
op_minus
l_int|1
suffix:semicolon
id|cyber2000_accel_wait
c_func
(paren
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x00
comma
l_int|0xbf011
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x03
comma
l_int|0xbf048
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
id|width
comma
l_int|0xbf060
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
id|height
comma
l_int|0xbf062
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|p-&gt;var.bits_per_pixel
)paren
(brace
r_case
l_int|15
suffix:colon
r_case
l_int|16
suffix:colon
id|bgx
op_assign
(paren
(paren
id|u16
op_star
)paren
id|p-&gt;dispsw_data
)paren
(braket
id|bgx
)braket
suffix:semicolon
r_case
l_int|8
suffix:colon
id|cyber2000_outl
c_func
(paren
id|dst
comma
l_int|0xbf178
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|24
suffix:colon
id|cyber2000_outl
c_func
(paren
id|dst
op_star
l_int|3
comma
l_int|0xbf178
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|dst
comma
l_int|0xbf078
)paren
suffix:semicolon
id|bgx
op_assign
(paren
(paren
id|u32
op_star
)paren
id|p-&gt;dispsw_data
)paren
(braket
id|bgx
)braket
suffix:semicolon
r_break
suffix:semicolon
)brace
id|cyber2000_outl
c_func
(paren
id|bgx
comma
l_int|0xbf058
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
l_int|0x8000
comma
l_int|0xbf07c
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
l_int|0x0800
comma
l_int|0xbf07e
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|cyber2000_accel_putc
id|cyber2000_accel_putc
c_func
(paren
r_struct
id|vc_data
op_star
id|conp
comma
r_struct
id|display
op_star
id|p
comma
r_int
id|c
comma
r_int
id|yy
comma
r_int
id|xx
)paren
(brace
id|cyber2000_accel_wait
c_func
(paren
)paren
suffix:semicolon
id|dispsw
op_member_access_from_pointer
id|putc
c_func
(paren
id|conp
comma
id|p
comma
id|c
comma
id|yy
comma
id|xx
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|cyber2000_accel_putcs
id|cyber2000_accel_putcs
c_func
(paren
r_struct
id|vc_data
op_star
id|conp
comma
r_struct
id|display
op_star
id|p
comma
r_const
r_int
r_int
op_star
id|s
comma
r_int
id|count
comma
r_int
id|yy
comma
r_int
id|xx
)paren
(brace
id|cyber2000_accel_wait
c_func
(paren
)paren
suffix:semicolon
id|dispsw
op_member_access_from_pointer
id|putcs
c_func
(paren
id|conp
comma
id|p
comma
id|s
comma
id|count
comma
id|yy
comma
id|xx
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|cyber2000_accel_revc
id|cyber2000_accel_revc
c_func
(paren
r_struct
id|display
op_star
id|p
comma
r_int
id|xx
comma
r_int
id|yy
)paren
(brace
id|cyber2000_accel_wait
c_func
(paren
)paren
suffix:semicolon
id|dispsw
op_member_access_from_pointer
id|revc
c_func
(paren
id|p
comma
id|xx
comma
id|yy
)paren
suffix:semicolon
)brace
r_static
r_void
DECL|function|cyber2000_accel_clear_margins
id|cyber2000_accel_clear_margins
c_func
(paren
r_struct
id|vc_data
op_star
id|conp
comma
r_struct
id|display
op_star
id|p
comma
r_int
id|bottom_only
)paren
(brace
id|dispsw
op_member_access_from_pointer
id|clear_margins
c_func
(paren
id|conp
comma
id|p
comma
id|bottom_only
)paren
suffix:semicolon
)brace
DECL|variable|fbcon_cyber_accel
r_static
r_struct
id|display_switch
id|fbcon_cyber_accel
op_assign
(brace
id|cyber2000_accel_setup
comma
id|cyber2000_accel_bmove
comma
id|cyber2000_accel_clear
comma
id|cyber2000_accel_putc
comma
id|cyber2000_accel_putcs
comma
id|cyber2000_accel_revc
comma
l_int|NULL
comma
l_int|NULL
comma
id|cyber2000_accel_clear_margins
comma
id|FONTWIDTH
c_func
(paren
l_int|8
)paren
op_or
id|FONTWIDTH
c_func
(paren
l_int|16
)paren
)brace
suffix:semicolon
multiline_comment|/*&n; * Palette&n; */
r_static
r_int
DECL|function|cyber2000_getcolreg
id|cyber2000_getcolreg
c_func
(paren
id|u_int
id|regno
comma
id|u_int
op_star
id|red
comma
id|u_int
op_star
id|green
comma
id|u_int
op_star
id|blue
comma
id|u_int
op_star
id|transp
comma
r_struct
id|fb_info
op_star
id|fb_info
)paren
(brace
r_int
id|t
suffix:semicolon
r_if
c_cond
(paren
id|regno
op_ge
l_int|256
)paren
r_return
l_int|1
suffix:semicolon
id|t
op_assign
id|current_par.palette
(braket
id|regno
)braket
dot
id|red
suffix:semicolon
op_star
id|red
op_assign
id|t
op_lshift
l_int|10
op_or
id|t
op_lshift
l_int|4
op_or
id|t
op_rshift
l_int|2
suffix:semicolon
id|t
op_assign
id|current_par.palette
(braket
id|regno
)braket
dot
id|green
suffix:semicolon
op_star
id|green
op_assign
id|t
op_lshift
l_int|10
op_or
id|t
op_lshift
l_int|4
op_or
id|t
op_rshift
l_int|2
suffix:semicolon
id|t
op_assign
id|current_par.palette
(braket
id|regno
)braket
dot
id|blue
suffix:semicolon
op_star
id|blue
op_assign
id|t
op_lshift
l_int|10
op_or
id|t
op_lshift
l_int|4
op_or
id|t
op_rshift
l_int|2
suffix:semicolon
op_star
id|transp
op_assign
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    Set a single color register. Return != 0 for invalid regno.&n; */
r_static
r_int
DECL|function|cyber2000_setcolreg
id|cyber2000_setcolreg
c_func
(paren
id|u_int
id|regno
comma
id|u_int
id|red
comma
id|u_int
id|green
comma
id|u_int
id|blue
comma
id|u_int
id|transp
comma
r_struct
id|fb_info
op_star
id|fb_info
)paren
(brace
r_if
c_cond
(paren
id|regno
OG
l_int|255
)paren
r_return
l_int|1
suffix:semicolon
id|red
op_rshift_assign
l_int|10
suffix:semicolon
id|green
op_rshift_assign
l_int|10
suffix:semicolon
id|blue
op_rshift_assign
l_int|10
suffix:semicolon
id|current_par.palette
(braket
id|regno
)braket
dot
id|red
op_assign
id|red
suffix:semicolon
id|current_par.palette
(braket
id|regno
)braket
dot
id|green
op_assign
id|green
suffix:semicolon
id|current_par.palette
(braket
id|regno
)braket
dot
id|blue
op_assign
id|blue
suffix:semicolon
r_switch
c_cond
(paren
id|fb_display
(braket
id|current_par.currcon
)braket
dot
id|var.bits_per_pixel
)paren
(brace
macro_line|#ifdef FBCON_HAS_CFB8
r_case
l_int|8
suffix:colon
id|cyber2000_outb
c_func
(paren
id|regno
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|red
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|green
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|blue
comma
l_int|0x3c9
)paren
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
macro_line|#ifdef FBCON_HAS_CFB16
r_case
l_int|15
suffix:colon
r_if
c_cond
(paren
id|regno
OL
l_int|32
)paren
(brace
id|cyber2000_outb
c_func
(paren
id|regno
op_lshift
l_int|3
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|red
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|green
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|blue
comma
l_int|0x3c9
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|regno
OL
l_int|16
)paren
id|current_par.c_table.cfb16
(braket
id|regno
)braket
op_assign
id|regno
op_or
id|regno
op_lshift
l_int|5
op_or
id|regno
op_lshift
l_int|10
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|16
suffix:colon
r_if
c_cond
(paren
id|regno
OL
l_int|64
)paren
(brace
multiline_comment|/* write green */
id|cyber2000_outb
c_func
(paren
id|regno
op_lshift
l_int|2
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|current_par.palette
(braket
id|regno
op_rshift
l_int|1
)braket
dot
id|red
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|green
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|current_par.palette
(braket
id|regno
op_rshift
l_int|1
)braket
dot
id|blue
comma
l_int|0x3c9
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|regno
OL
l_int|32
)paren
(brace
multiline_comment|/* write red,blue */
id|cyber2000_outb
c_func
(paren
id|regno
op_lshift
l_int|3
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|red
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|current_par.palette
(braket
id|regno
op_lshift
l_int|1
)braket
dot
id|green
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|blue
comma
l_int|0x3c9
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|regno
OL
l_int|16
)paren
id|current_par.c_table.cfb16
(braket
id|regno
)braket
op_assign
id|regno
op_or
id|regno
op_lshift
l_int|5
op_or
id|regno
op_lshift
l_int|11
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
macro_line|#ifdef FBCON_HAS_CFB24
r_case
l_int|24
suffix:colon
id|cyber2000_outb
c_func
(paren
id|regno
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|red
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|green
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|blue
comma
l_int|0x3c9
)paren
suffix:semicolon
r_if
c_cond
(paren
id|regno
OL
l_int|16
)paren
id|current_par.c_table.cfb24
(braket
id|regno
)braket
op_assign
id|regno
op_or
id|regno
op_lshift
l_int|8
op_or
id|regno
op_lshift
l_int|16
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
r_default
suffix:colon
r_return
l_int|1
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cyber2000fb_calculate_timing
r_static
r_void
id|cyber2000fb_calculate_timing
c_func
(paren
r_int
r_char
op_star
id|v
comma
r_struct
id|fb_var_screeninfo
op_star
id|var
)paren
(brace
r_int
id|Htotal
comma
id|Hdispend
comma
id|Hblankstart
comma
id|Hblankend
comma
id|Hsyncstart
comma
id|Hsyncend
suffix:semicolon
r_int
id|Vtotal
comma
id|Vdispend
comma
id|Vblankstart
comma
id|Vblankend
comma
id|Vsyncstart
comma
id|Vsyncend
suffix:semicolon
DECL|macro|BIT
mdefine_line|#define BIT(v,b1,m,b2) (((v &gt;&gt; b1) &amp; m) &lt;&lt; b2)
id|Hdispend
op_assign
id|var-&gt;xres
suffix:semicolon
id|Hsyncstart
op_assign
id|var-&gt;xres
op_plus
id|var-&gt;right_margin
suffix:semicolon
id|Hsyncend
op_assign
id|var-&gt;xres
op_plus
id|var-&gt;right_margin
op_plus
id|var-&gt;hsync_len
suffix:semicolon
id|Htotal
op_assign
id|var-&gt;xres
op_plus
id|var-&gt;right_margin
op_plus
id|var-&gt;hsync_len
op_plus
id|var-&gt;left_margin
suffix:semicolon
id|Hblankstart
op_assign
id|var-&gt;xres
suffix:semicolon
id|Hblankend
op_assign
id|Htotal
op_minus
l_int|4
op_star
l_int|8
suffix:semicolon
id|Vdispend
op_assign
id|var-&gt;yres
suffix:semicolon
id|Vsyncstart
op_assign
id|var-&gt;yres
op_plus
id|var-&gt;lower_margin
suffix:semicolon
id|Vsyncend
op_assign
id|var-&gt;yres
op_plus
id|var-&gt;lower_margin
op_plus
id|var-&gt;vsync_len
suffix:semicolon
id|Vtotal
op_assign
id|var-&gt;yres
op_plus
id|var-&gt;lower_margin
op_plus
id|var-&gt;vsync_len
op_plus
id|var-&gt;upper_margin
suffix:semicolon
id|Vblankstart
op_assign
id|var-&gt;yres
op_plus
l_int|7
suffix:semicolon
id|Vblankend
op_assign
id|Vtotal
op_minus
l_int|11
suffix:semicolon
id|Hdispend
op_rshift_assign
l_int|3
suffix:semicolon
id|Hsyncstart
op_rshift_assign
l_int|3
suffix:semicolon
id|Hsyncend
op_rshift_assign
l_int|3
suffix:semicolon
id|Htotal
op_rshift_assign
l_int|3
suffix:semicolon
id|Hblankstart
op_rshift_assign
l_int|3
suffix:semicolon
id|Hblankend
op_rshift_assign
l_int|3
suffix:semicolon
id|Htotal
op_sub_assign
l_int|5
suffix:semicolon
id|Hdispend
op_sub_assign
l_int|1
suffix:semicolon
id|Vtotal
op_sub_assign
l_int|2
suffix:semicolon
id|Vdispend
op_sub_assign
l_int|1
suffix:semicolon
id|Vblankstart
op_sub_assign
l_int|1
suffix:semicolon
id|Vblankend
op_sub_assign
l_int|1
suffix:semicolon
id|v
(braket
l_int|0
)braket
op_assign
id|Htotal
suffix:semicolon
id|v
(braket
l_int|1
)braket
op_assign
id|Hdispend
suffix:semicolon
id|v
(braket
l_int|2
)braket
op_assign
id|Hblankstart
suffix:semicolon
id|v
(braket
l_int|3
)braket
op_assign
id|BIT
c_func
(paren
id|Hblankend
comma
l_int|0
comma
l_int|0x1f
comma
l_int|0
)paren
op_or
id|BIT
c_func
(paren
l_int|1
comma
l_int|0
comma
l_int|0x01
comma
l_int|7
)paren
suffix:semicolon
id|v
(braket
l_int|4
)braket
op_assign
id|Hsyncstart
suffix:semicolon
id|v
(braket
l_int|5
)braket
op_assign
id|BIT
c_func
(paren
id|Hsyncend
comma
l_int|0
comma
l_int|0x1f
comma
l_int|0
)paren
op_or
id|BIT
c_func
(paren
id|Hblankend
comma
l_int|5
comma
l_int|0x01
comma
l_int|7
)paren
suffix:semicolon
id|v
(braket
l_int|6
)braket
op_assign
id|Vtotal
suffix:semicolon
id|v
(braket
l_int|7
)braket
op_assign
id|BIT
c_func
(paren
id|Vtotal
comma
l_int|8
comma
l_int|0x01
comma
l_int|0
)paren
op_or
id|BIT
c_func
(paren
id|Vdispend
comma
l_int|8
comma
l_int|0x01
comma
l_int|1
)paren
op_or
id|BIT
c_func
(paren
id|Vsyncstart
comma
l_int|8
comma
l_int|0x01
comma
l_int|2
)paren
op_or
id|BIT
c_func
(paren
id|Vblankstart
comma
l_int|8
comma
l_int|0x01
comma
l_int|3
)paren
op_or
id|BIT
c_func
(paren
l_int|1
comma
l_int|0
comma
l_int|0x01
comma
l_int|4
)paren
op_or
id|BIT
c_func
(paren
id|Vtotal
comma
l_int|9
comma
l_int|0x01
comma
l_int|5
)paren
op_or
id|BIT
c_func
(paren
id|Vdispend
comma
l_int|9
comma
l_int|0x01
comma
l_int|6
)paren
op_or
id|BIT
c_func
(paren
id|Vsyncstart
comma
l_int|9
comma
l_int|0x01
comma
l_int|7
)paren
suffix:semicolon
id|v
(braket
l_int|8
)braket
op_assign
l_int|0
suffix:semicolon
id|v
(braket
l_int|9
)braket
op_assign
id|BIT
c_func
(paren
l_int|0
comma
l_int|0
comma
l_int|0x1f
comma
l_int|0
)paren
op_or
id|BIT
c_func
(paren
id|Vblankstart
comma
l_int|9
comma
l_int|0x01
comma
l_int|5
)paren
op_or
id|BIT
c_func
(paren
l_int|1
comma
l_int|0
comma
l_int|0x01
comma
l_int|6
)paren
suffix:semicolon
id|v
(braket
l_int|10
)braket
op_assign
id|Vsyncstart
suffix:semicolon
id|v
(braket
l_int|11
)braket
op_assign
id|BIT
c_func
(paren
id|Vsyncend
comma
l_int|0
comma
l_int|0x0f
comma
l_int|0
)paren
op_or
id|BIT
c_func
(paren
l_int|1
comma
l_int|0
comma
l_int|0x01
comma
l_int|7
)paren
suffix:semicolon
id|v
(braket
l_int|12
)braket
op_assign
id|Vdispend
suffix:semicolon
id|v
(braket
l_int|14
)braket
op_assign
l_int|0
suffix:semicolon
id|v
(braket
l_int|15
)braket
op_assign
id|Vblankstart
suffix:semicolon
id|v
(braket
l_int|16
)braket
op_assign
id|Vblankend
suffix:semicolon
id|v
(braket
l_int|17
)braket
op_assign
l_int|0xe3
suffix:semicolon
multiline_comment|/* overflow - graphics reg 0x11 */
id|v
(braket
l_int|18
)braket
op_assign
id|BIT
c_func
(paren
id|Vtotal
comma
l_int|10
comma
l_int|0x01
comma
l_int|0
)paren
op_or
multiline_comment|/* guess */
id|BIT
c_func
(paren
id|Vdispend
comma
l_int|10
comma
l_int|0x01
comma
l_int|1
)paren
op_or
id|BIT
c_func
(paren
id|Vsyncstart
comma
l_int|10
comma
l_int|0x01
comma
l_int|2
)paren
op_or
multiline_comment|/* guess */
id|BIT
c_func
(paren
id|Vblankstart
comma
l_int|10
comma
l_int|0x01
comma
l_int|3
)paren
op_or
multiline_comment|/* guess */
id|BIT
c_func
(paren
id|Hblankend
comma
l_int|6
comma
l_int|0x01
comma
l_int|4
)paren
suffix:semicolon
multiline_comment|/* guess */
)brace
DECL|function|cyber2000fb_set_timing
r_static
r_void
id|cyber2000fb_set_timing
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
)paren
(brace
r_int
r_int
id|width
op_assign
id|var-&gt;xres_virtual
suffix:semicolon
r_int
r_int
id|scr_pitch
comma
id|fetchrow
comma
id|i
suffix:semicolon
r_char
id|b
comma
id|graph_r77
comma
id|crtc
(braket
l_int|32
)braket
suffix:semicolon
r_switch
c_cond
(paren
id|var-&gt;bits_per_pixel
)paren
(brace
r_case
l_int|8
suffix:colon
multiline_comment|/* PSEUDOCOLOUR, 256 */
id|b
op_assign
l_int|0
suffix:semicolon
id|graph_r77
op_assign
l_int|1
suffix:semicolon
id|scr_pitch
op_assign
id|width
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|15
suffix:colon
multiline_comment|/* DIRECTCOLOUR, 32k */
id|b
op_assign
l_int|1
suffix:semicolon
id|graph_r77
op_assign
l_int|6
suffix:semicolon
id|scr_pitch
op_assign
id|width
op_star
l_int|2
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|16
suffix:colon
multiline_comment|/* DIRECTCOLOUR, 64k */
id|b
op_assign
l_int|1
suffix:semicolon
id|graph_r77
op_assign
l_int|2
suffix:semicolon
id|scr_pitch
op_assign
id|width
op_star
l_int|2
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|24
suffix:colon
multiline_comment|/* TRUECOLOUR, 16m */
id|b
op_assign
l_int|2
suffix:semicolon
id|graph_r77
op_assign
l_int|4
suffix:semicolon
id|width
op_mul_assign
l_int|3
suffix:semicolon
id|scr_pitch
op_assign
id|width
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
r_return
suffix:semicolon
)brace
id|width
op_sub_assign
l_int|1
suffix:semicolon
id|scr_pitch
op_rshift_assign
l_int|3
suffix:semicolon
id|fetchrow
op_assign
id|scr_pitch
op_plus
l_int|1
suffix:semicolon
id|cyber2000fb_calculate_timing
c_func
(paren
id|crtc
comma
id|var
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|NUM_TOTAL_MODES
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|var-&gt;xres
op_eq
id|cyber2000_res
(braket
id|i
)braket
dot
id|xres
op_logical_and
id|var-&gt;yres
op_eq
id|cyber2000_res
(braket
id|i
)braket
dot
id|yres
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|i
OL
id|NUM_TOTAL_MODES
)paren
id|cyber2000_init_hw
c_func
(paren
id|cyber2000_res
op_plus
id|i
)paren
suffix:semicolon
id|crtc
(braket
l_int|13
)braket
op_assign
id|scr_pitch
suffix:semicolon
multiline_comment|/*&n;&t; * reprogram the CRTC with the values we calculated&n;&t; * above.  This should be cleaned up once we&squot;re&n;&t; * confident that we&squot;re generating the correct&n;&t; * values.  Disable this if you&squot;re having problems,&n;&t; * and report the values obtained from the kernel&n;&t; * messages.&n;&t; */
macro_line|#if 1
id|cyber2000_crtcw
c_func
(paren
l_int|0x11
comma
l_int|0x0b
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
r_sizeof
(paren
id|crtc_idx
)paren
suffix:semicolon
id|i
op_increment
)paren
id|cyber2000_crtcw
c_func
(paren
id|crtc_idx
(braket
id|i
)braket
comma
id|crtc
(braket
id|i
)braket
)paren
suffix:semicolon
macro_line|#else
id|cyber2000_crtcw
c_func
(paren
l_int|0x13
comma
id|crtc
(braket
l_int|13
)braket
)paren
suffix:semicolon
macro_line|#endif
id|cyber2000_grphw
c_func
(paren
l_int|0x14
comma
id|fetchrow
)paren
suffix:semicolon
multiline_comment|/* FIXME: is this the right way round? */
id|cyber2000_grphw
c_func
(paren
l_int|0x15
comma
(paren
(paren
id|fetchrow
op_rshift
l_int|4
)paren
op_amp
l_int|0xf0
)paren
op_or
(paren
(paren
id|scr_pitch
op_rshift
l_int|8
)paren
op_amp
l_int|0x0f
)paren
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x77
comma
id|graph_r77
)paren
suffix:semicolon
id|cyber2000_grphw
c_func
(paren
l_int|0x33
comma
l_int|0x1c
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
id|width
comma
l_int|0xbf018
)paren
suffix:semicolon
id|cyber2000_outw
c_func
(paren
id|width
comma
l_int|0xbf218
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|b
comma
l_int|0xbf01c
)paren
suffix:semicolon
macro_line|#if 0
(brace
r_int
id|j
suffix:semicolon
id|i
op_assign
l_int|0
suffix:semicolon
id|printk
c_func
(paren
id|KERN_DEBUG
)paren
suffix:semicolon
r_for
c_loop
(paren
id|j
op_assign
l_int|0
suffix:semicolon
id|j
OL
l_int|19
suffix:semicolon
id|j
op_increment
)paren
id|printk
c_func
(paren
l_string|&quot;%2d &quot;
comma
id|j
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
id|KERN_DEBUG
)paren
suffix:semicolon
r_for
c_loop
(paren
id|j
op_assign
l_int|0
suffix:semicolon
id|j
OL
l_int|19
suffix:semicolon
id|j
op_increment
)paren
id|printk
c_func
(paren
l_string|&quot;%02X &quot;
comma
id|crtc
(braket
id|j
)braket
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
id|KERN_DEBUG
)paren
suffix:semicolon
r_for
c_loop
(paren
id|j
op_assign
l_int|0
suffix:semicolon
id|j
OL
l_int|18
suffix:semicolon
id|j
op_increment
)paren
id|printk
c_func
(paren
l_string|&quot;%02X &quot;
comma
id|cyber2000_res
(braket
id|i
)braket
dot
id|crtc_regs
(braket
id|j
)braket
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;%02X&bslash;n&quot;
comma
id|cyber2000_res
(braket
id|i
)braket
dot
id|crtc_ofl
)paren
suffix:semicolon
)brace
macro_line|#endif
)brace
r_static
r_inline
r_void
DECL|function|cyber2000fb_update_start
id|cyber2000fb_update_start
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
)paren
(brace
macro_line|#if 0
r_int
r_int
id|base
suffix:semicolon
id|base
op_assign
id|var-&gt;yoffset
op_star
id|var-&gt;xres_virtual
op_plus
id|var-&gt;xoffset
suffix:semicolon
id|cyber2000_crtcw
c_func
(paren
l_int|0x0c
comma
id|base
)paren
suffix:semicolon
id|cyber2000_crtcw
c_func
(paren
l_int|0x0d
comma
id|base
op_rshift
l_int|8
)paren
suffix:semicolon
multiline_comment|/* FIXME: need the upper bits of the start offset */
multiline_comment|/*&t;cyber2000_crtcw(0x??, base &gt;&gt; 16);*/
macro_line|#endif
)brace
multiline_comment|/*&n; *  Open/Release the frame buffer device&n; */
DECL|function|cyber2000fb_open
r_static
r_int
id|cyber2000fb_open
c_func
(paren
r_struct
id|fb_info
op_star
id|info
comma
r_int
id|user
)paren
(brace
id|MOD_INC_USE_COUNT
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cyber2000fb_release
r_static
r_int
id|cyber2000fb_release
c_func
(paren
r_struct
id|fb_info
op_star
id|info
comma
r_int
id|user
)paren
(brace
id|MOD_DEC_USE_COUNT
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    Get the Colormap&n; */
r_static
r_int
DECL|function|cyber2000fb_get_cmap
id|cyber2000fb_get_cmap
c_func
(paren
r_struct
id|fb_cmap
op_star
id|cmap
comma
r_int
id|kspc
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
r_int
id|err
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|con
op_eq
id|current_par.currcon
)paren
multiline_comment|/* current console? */
id|err
op_assign
id|fb_get_cmap
c_func
(paren
id|cmap
comma
id|kspc
comma
id|cyber2000_getcolreg
comma
id|info
)paren
suffix:semicolon
r_else
r_if
c_cond
(paren
id|fb_display
(braket
id|con
)braket
dot
id|cmap.len
)paren
multiline_comment|/* non default colormap? */
id|fb_copy_cmap
c_func
(paren
op_amp
id|fb_display
(braket
id|con
)braket
dot
id|cmap
comma
id|cmap
comma
id|kspc
ques
c_cond
l_int|0
suffix:colon
l_int|2
)paren
suffix:semicolon
r_else
id|fb_copy_cmap
c_func
(paren
id|fb_default_cmap
c_func
(paren
l_int|1
op_lshift
id|fb_display
(braket
id|con
)braket
dot
id|var.bits_per_pixel
)paren
comma
id|cmap
comma
id|kspc
ques
c_cond
l_int|0
suffix:colon
l_int|2
)paren
suffix:semicolon
r_return
id|err
suffix:semicolon
)brace
multiline_comment|/*&n; *    Set the Colormap&n; */
r_static
r_int
DECL|function|cyber2000fb_set_cmap
id|cyber2000fb_set_cmap
c_func
(paren
r_struct
id|fb_cmap
op_star
id|cmap
comma
r_int
id|kspc
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
r_struct
id|display
op_star
id|disp
op_assign
op_amp
id|fb_display
(braket
id|con
)braket
suffix:semicolon
r_int
id|err
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|disp-&gt;cmap.len
)paren
(brace
multiline_comment|/* no colormap allocated? */
r_int
id|size
suffix:semicolon
r_if
c_cond
(paren
id|disp-&gt;var.bits_per_pixel
op_eq
l_int|16
)paren
id|size
op_assign
l_int|32
suffix:semicolon
r_else
id|size
op_assign
l_int|256
suffix:semicolon
id|err
op_assign
id|fb_alloc_cmap
c_func
(paren
op_amp
id|disp-&gt;cmap
comma
id|size
comma
l_int|0
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|err
)paren
(brace
r_if
c_cond
(paren
id|con
op_eq
id|current_par.currcon
)paren
multiline_comment|/* current console? */
id|err
op_assign
id|fb_set_cmap
c_func
(paren
id|cmap
comma
id|kspc
comma
id|cyber2000_setcolreg
comma
id|info
)paren
suffix:semicolon
r_else
id|fb_copy_cmap
c_func
(paren
id|cmap
comma
op_amp
id|disp-&gt;cmap
comma
id|kspc
ques
c_cond
l_int|0
suffix:colon
l_int|1
)paren
suffix:semicolon
)brace
r_return
id|err
suffix:semicolon
)brace
r_static
r_int
DECL|function|cyber2000fb_decode_var
id|cyber2000fb_decode_var
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
comma
r_int
id|con
comma
r_int
op_star
id|visual
)paren
(brace
r_switch
c_cond
(paren
id|var-&gt;bits_per_pixel
)paren
(brace
macro_line|#ifdef FBCON_HAS_CFB8
r_case
l_int|8
suffix:colon
op_star
id|visual
op_assign
id|FB_VISUAL_PSEUDOCOLOR
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
macro_line|#ifdef FBCON_HAS_CFB16
r_case
l_int|15
suffix:colon
r_case
l_int|16
suffix:colon
op_star
id|visual
op_assign
id|FB_VISUAL_DIRECTCOLOR
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
macro_line|#ifdef FBCON_HAS_CFB24
r_case
l_int|24
suffix:colon
op_star
id|visual
op_assign
id|FB_VISUAL_TRUECOLOR
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    Get the Fixed Part of the Display&n; */
r_static
r_int
DECL|function|cyber2000fb_get_fix
id|cyber2000fb_get_fix
c_func
(paren
r_struct
id|fb_fix_screeninfo
op_star
id|fix
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|fb_info
)paren
(brace
r_struct
id|display
op_star
id|display
suffix:semicolon
id|memset
c_func
(paren
id|fix
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|fb_fix_screeninfo
)paren
)paren
suffix:semicolon
id|strcpy
c_func
(paren
id|fix-&gt;id
comma
l_string|&quot;Cyber2000&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|con
op_ge
l_int|0
)paren
id|display
op_assign
id|fb_display
op_plus
id|con
suffix:semicolon
r_else
id|display
op_assign
op_amp
id|global_disp
suffix:semicolon
id|fix-&gt;smem_start
op_assign
id|current_par.screen_base_p
suffix:semicolon
id|fix-&gt;smem_len
op_assign
id|current_par.screen_size
suffix:semicolon
id|fix-&gt;mmio_start
op_assign
id|current_par.regs_base_p
suffix:semicolon
id|fix-&gt;mmio_len
op_assign
l_int|0x000c0000
suffix:semicolon
id|fix-&gt;type
op_assign
id|display-&gt;type
suffix:semicolon
id|fix-&gt;type_aux
op_assign
id|display-&gt;type_aux
suffix:semicolon
id|fix-&gt;xpanstep
op_assign
l_int|0
suffix:semicolon
id|fix-&gt;ypanstep
op_assign
id|display-&gt;ypanstep
suffix:semicolon
id|fix-&gt;ywrapstep
op_assign
id|display-&gt;ywrapstep
suffix:semicolon
id|fix-&gt;visual
op_assign
id|display-&gt;visual
suffix:semicolon
id|fix-&gt;line_length
op_assign
id|display-&gt;line_length
suffix:semicolon
id|fix-&gt;accel
op_assign
l_int|22
suffix:semicolon
multiline_comment|/*FB_ACCEL_IGS_CYBER2000*/
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    Get the User Defined Part of the Display&n; */
r_static
r_int
DECL|function|cyber2000fb_get_var
id|cyber2000fb_get_var
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|fb_info
)paren
(brace
r_if
c_cond
(paren
id|con
op_eq
op_minus
l_int|1
)paren
op_star
id|var
op_assign
id|global_disp.var
suffix:semicolon
r_else
op_star
id|var
op_assign
id|fb_display
(braket
id|con
)braket
dot
id|var
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    Set the User Defined Part of the Display&n; */
r_static
r_int
DECL|function|cyber2000fb_set_var
id|cyber2000fb_set_var
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
r_struct
id|display
op_star
id|display
suffix:semicolon
r_int
id|err
comma
id|chgvar
op_assign
l_int|0
comma
id|visual
suffix:semicolon
r_if
c_cond
(paren
id|con
op_ge
l_int|0
)paren
id|display
op_assign
id|fb_display
op_plus
id|con
suffix:semicolon
r_else
id|display
op_assign
op_amp
id|global_disp
suffix:semicolon
id|err
op_assign
id|cyber2000fb_decode_var
c_func
(paren
id|var
comma
id|con
comma
op_amp
id|visual
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
r_switch
c_cond
(paren
id|var-&gt;activate
op_amp
id|FB_ACTIVATE_MASK
)paren
(brace
r_case
id|FB_ACTIVATE_TEST
suffix:colon
r_return
l_int|0
suffix:semicolon
r_case
id|FB_ACTIVATE_NXTOPEN
suffix:colon
r_case
id|FB_ACTIVATE_NOW
suffix:colon
r_break
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|con
op_ge
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|display-&gt;var.xres
op_ne
id|var-&gt;xres
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|display-&gt;var.yres
op_ne
id|var-&gt;yres
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|display-&gt;var.xres_virtual
op_ne
id|var-&gt;xres_virtual
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|display-&gt;var.yres_virtual
op_ne
id|var-&gt;yres_virtual
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|display-&gt;var.accel_flags
op_ne
id|var-&gt;accel_flags
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|memcmp
c_func
(paren
op_amp
id|display-&gt;var.red
comma
op_amp
id|var-&gt;red
comma
r_sizeof
(paren
id|var-&gt;red
)paren
)paren
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|memcmp
c_func
(paren
op_amp
id|display-&gt;var.green
comma
op_amp
id|var-&gt;green
comma
r_sizeof
(paren
id|var-&gt;green
)paren
)paren
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|memcmp
c_func
(paren
op_amp
id|display-&gt;var.blue
comma
op_amp
id|var-&gt;blue
comma
r_sizeof
(paren
id|var-&gt;green
)paren
)paren
)paren
id|chgvar
op_assign
l_int|1
suffix:semicolon
)brace
id|display-&gt;var
op_assign
op_star
id|var
suffix:semicolon
id|display-&gt;var.activate
op_and_assign
op_complement
id|FB_ACTIVATE_ALL
suffix:semicolon
r_if
c_cond
(paren
id|var-&gt;activate
op_amp
id|FB_ACTIVATE_ALL
)paren
id|global_disp.var
op_assign
id|display-&gt;var
suffix:semicolon
id|display-&gt;screen_base
op_assign
(paren
r_char
op_star
)paren
id|current_par.screen_base
suffix:semicolon
id|display-&gt;visual
op_assign
id|visual
suffix:semicolon
id|display-&gt;type
op_assign
id|FB_TYPE_PACKED_PIXELS
suffix:semicolon
id|display-&gt;type_aux
op_assign
l_int|0
suffix:semicolon
id|display-&gt;ypanstep
op_assign
l_int|0
suffix:semicolon
id|display-&gt;ywrapstep
op_assign
l_int|0
suffix:semicolon
id|display-&gt;can_soft_blank
op_assign
l_int|1
suffix:semicolon
id|display-&gt;inverse
op_assign
l_int|0
suffix:semicolon
r_switch
c_cond
(paren
id|display-&gt;var.bits_per_pixel
)paren
(brace
macro_line|#ifdef FBCON_HAS_CFB8
r_case
l_int|8
suffix:colon
id|dispsw
op_assign
op_amp
id|fbcon_cfb8
suffix:semicolon
id|display-&gt;dispsw_data
op_assign
l_int|NULL
suffix:semicolon
id|display-&gt;next_line
op_assign
id|var-&gt;xres_virtual
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
macro_line|#ifdef FBCON_HAS_CFB16
r_case
l_int|15
suffix:colon
r_case
l_int|16
suffix:colon
id|dispsw
op_assign
op_amp
id|fbcon_cfb16
suffix:semicolon
id|display-&gt;dispsw_data
op_assign
id|current_par.c_table.cfb16
suffix:semicolon
id|display-&gt;next_line
op_assign
id|var-&gt;xres_virtual
op_star
l_int|2
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
macro_line|#ifdef FBCON_HAS_CFB24
r_case
l_int|24
suffix:colon
id|dispsw
op_assign
op_amp
id|fbcon_cfb24
suffix:semicolon
id|display-&gt;dispsw_data
op_assign
id|current_par.c_table.cfb24
suffix:semicolon
id|display-&gt;next_line
op_assign
id|var-&gt;xres_virtual
op_star
l_int|3
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
r_default
suffix:colon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;cyber2000: no support for %dbpp&bslash;n&quot;
comma
id|display-&gt;var.bits_per_pixel
)paren
suffix:semicolon
id|dispsw
op_assign
op_amp
id|fbcon_dummy
suffix:semicolon
r_break
suffix:semicolon
)brace
id|display-&gt;line_length
op_assign
id|display-&gt;next_line
suffix:semicolon
r_if
c_cond
(paren
id|display-&gt;var.accel_flags
op_amp
id|FB_ACCELF_TEXT
op_logical_and
id|dispsw
op_ne
op_amp
id|fbcon_dummy
)paren
id|display-&gt;dispsw
op_assign
op_amp
id|fbcon_cyber_accel
suffix:semicolon
r_else
id|display-&gt;dispsw
op_assign
id|dispsw
suffix:semicolon
r_if
c_cond
(paren
id|chgvar
op_logical_and
id|info
op_logical_and
id|info-&gt;changevar
)paren
id|info
op_member_access_from_pointer
id|changevar
c_func
(paren
id|con
)paren
suffix:semicolon
r_if
c_cond
(paren
id|con
op_eq
id|current_par.currcon
)paren
(brace
r_struct
id|fb_cmap
op_star
id|cmap
suffix:semicolon
id|cyber2000fb_update_start
c_func
(paren
id|var
)paren
suffix:semicolon
id|cyber2000fb_set_timing
c_func
(paren
id|var
)paren
suffix:semicolon
r_if
c_cond
(paren
id|display-&gt;cmap.len
)paren
id|cmap
op_assign
op_amp
id|display-&gt;cmap
suffix:semicolon
r_else
id|cmap
op_assign
id|fb_default_cmap
c_func
(paren
id|current_par.palette_size
)paren
suffix:semicolon
id|fb_set_cmap
c_func
(paren
id|cmap
comma
l_int|1
comma
id|cyber2000_setcolreg
comma
id|info
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    Pan or Wrap the Display&n; */
DECL|function|cyber2000fb_pan_display
r_static
r_int
id|cyber2000fb_pan_display
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
id|u_int
id|y_bottom
suffix:semicolon
id|y_bottom
op_assign
id|var-&gt;yoffset
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|var-&gt;vmode
op_amp
id|FB_VMODE_YWRAP
)paren
)paren
id|y_bottom
op_add_assign
id|var-&gt;yres
suffix:semicolon
r_if
c_cond
(paren
id|var-&gt;xoffset
OG
(paren
id|var-&gt;xres_virtual
op_minus
id|var-&gt;xres
)paren
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
id|y_bottom
OG
id|fb_display
(braket
id|con
)braket
dot
id|var.yres_virtual
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
multiline_comment|/*disabled until we can update the start address properly */
r_return
op_minus
id|EINVAL
suffix:semicolon
id|cyber2000fb_update_start
c_func
(paren
id|var
)paren
suffix:semicolon
id|fb_display
(braket
id|con
)braket
dot
id|var.xoffset
op_assign
id|var-&gt;xoffset
suffix:semicolon
id|fb_display
(braket
id|con
)braket
dot
id|var.yoffset
op_assign
id|var-&gt;yoffset
suffix:semicolon
r_if
c_cond
(paren
id|var-&gt;vmode
op_amp
id|FB_VMODE_YWRAP
)paren
id|fb_display
(braket
id|con
)braket
dot
id|var.vmode
op_or_assign
id|FB_VMODE_YWRAP
suffix:semicolon
r_else
id|fb_display
(braket
id|con
)braket
dot
id|var.vmode
op_and_assign
op_complement
id|FB_VMODE_YWRAP
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cyber2000fb_ioctl
r_static
r_int
id|cyber2000fb_ioctl
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|file
comma
id|u_int
id|cmd
comma
id|u_long
id|arg
comma
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
multiline_comment|/*&n; *    Update the `var&squot; structure (called by fbcon.c)&n; *&n; *    This call looks only at yoffset and the FB_VMODE_YWRAP flag in `var&squot;.&n; *    Since it&squot;s called by a kernel driver, no range checking is done.&n; */
r_static
r_int
DECL|function|cyber2000fb_updatevar
id|cyber2000fb_updatevar
c_func
(paren
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
r_if
c_cond
(paren
id|con
op_eq
id|current_par.currcon
)paren
id|cyber2000fb_update_start
c_func
(paren
op_amp
id|fb_display
(braket
id|con
)braket
dot
id|var
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_static
r_int
DECL|function|cyber2000fb_switch
id|cyber2000fb_switch
c_func
(paren
r_int
id|con
comma
r_struct
id|fb_info
op_star
id|info
)paren
(brace
r_struct
id|fb_cmap
op_star
id|cmap
suffix:semicolon
r_if
c_cond
(paren
id|current_par.currcon
op_ge
l_int|0
)paren
(brace
id|cmap
op_assign
op_amp
id|fb_display
(braket
id|current_par.currcon
)braket
dot
id|cmap
suffix:semicolon
r_if
c_cond
(paren
id|cmap-&gt;len
)paren
id|fb_get_cmap
c_func
(paren
id|cmap
comma
l_int|1
comma
id|cyber2000_getcolreg
comma
id|info
)paren
suffix:semicolon
)brace
id|current_par.currcon
op_assign
id|con
suffix:semicolon
id|fb_display
(braket
id|con
)braket
dot
id|var.activate
op_assign
id|FB_ACTIVATE_NOW
suffix:semicolon
id|cyber2000fb_set_var
c_func
(paren
op_amp
id|fb_display
(braket
id|con
)braket
dot
id|var
comma
id|con
comma
id|info
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *    (Un)Blank the display.&n; */
DECL|function|cyber2000fb_blank
r_static
r_void
id|cyber2000fb_blank
c_func
(paren
r_int
id|blank
comma
r_struct
id|fb_info
op_star
id|fb_info
)paren
(brace
r_int
id|i
suffix:semicolon
r_if
c_cond
(paren
id|blank
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|256
suffix:semicolon
id|i
op_increment
)paren
(brace
id|cyber2000_outb
c_func
(paren
id|i
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0
comma
l_int|0x3c9
)paren
suffix:semicolon
)brace
)brace
r_else
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|256
suffix:semicolon
id|i
op_increment
)paren
(brace
id|cyber2000_outb
c_func
(paren
id|i
comma
l_int|0x3c8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|current_par.palette
(braket
id|i
)braket
dot
id|red
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|current_par.palette
(braket
id|i
)braket
dot
id|green
comma
l_int|0x3c9
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
id|current_par.palette
(braket
id|i
)braket
dot
id|blue
comma
l_int|0x3c9
)paren
suffix:semicolon
)brace
)brace
)brace
DECL|function|cyber2000fb_setup
r_int
id|__init
id|cyber2000fb_setup
c_func
(paren
r_char
op_star
id|options
)paren
(brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|cyber2000fb_ops
r_static
r_struct
id|fb_ops
id|cyber2000fb_ops
op_assign
(brace
id|cyber2000fb_open
comma
id|cyber2000fb_release
comma
id|cyber2000fb_get_fix
comma
id|cyber2000fb_get_var
comma
id|cyber2000fb_set_var
comma
id|cyber2000fb_get_cmap
comma
id|cyber2000fb_set_cmap
comma
id|cyber2000fb_pan_display
comma
id|cyber2000fb_ioctl
)brace
suffix:semicolon
r_static
r_void
id|__init
DECL|function|cyber2000fb_init_fbinfo
id|cyber2000fb_init_fbinfo
c_func
(paren
r_void
)paren
(brace
r_static
r_int
id|first
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|first
)paren
r_return
suffix:semicolon
id|first
op_assign
l_int|0
suffix:semicolon
id|strcpy
c_func
(paren
id|fb_info.modename
comma
l_string|&quot;Cyber2000&quot;
)paren
suffix:semicolon
id|strcpy
c_func
(paren
id|fb_info.fontname
comma
l_string|&quot;Acorn8x8&quot;
)paren
suffix:semicolon
id|fb_info.node
op_assign
op_minus
l_int|1
suffix:semicolon
id|fb_info.fbops
op_assign
op_amp
id|cyber2000fb_ops
suffix:semicolon
id|fb_info.disp
op_assign
op_amp
id|global_disp
suffix:semicolon
id|fb_info.changevar
op_assign
l_int|NULL
suffix:semicolon
id|fb_info.switch_con
op_assign
id|cyber2000fb_switch
suffix:semicolon
id|fb_info.updatevar
op_assign
id|cyber2000fb_updatevar
suffix:semicolon
id|fb_info.blank
op_assign
id|cyber2000fb_blank
suffix:semicolon
id|fb_info.flags
op_assign
id|FBINFO_FLAG_DEFAULT
suffix:semicolon
multiline_comment|/*&n;&t; * setup initial parameters&n;&t; */
id|memset
c_func
(paren
op_amp
id|init_var
comma
l_int|0
comma
r_sizeof
(paren
id|init_var
)paren
)paren
suffix:semicolon
id|init_var.xres_virtual
op_assign
id|init_var.xres
op_assign
id|DEFAULT_XRES
suffix:semicolon
id|init_var.yres_virtual
op_assign
id|init_var.yres
op_assign
id|DEFAULT_YRES
suffix:semicolon
id|init_var.bits_per_pixel
op_assign
id|DEFAULT_BPP
suffix:semicolon
multiline_comment|/*&n;&t; * These parameters give&n;&t; * 640x480, hsync 31.5kHz, vsync 60Hz&n;&t; */
id|init_var.left_margin
op_assign
l_int|56
suffix:semicolon
id|init_var.right_margin
op_assign
l_int|16
suffix:semicolon
id|init_var.upper_margin
op_assign
l_int|34
suffix:semicolon
id|init_var.lower_margin
op_assign
l_int|9
suffix:semicolon
id|init_var.hsync_len
op_assign
l_int|88
suffix:semicolon
id|init_var.vsync_len
op_assign
l_int|2
suffix:semicolon
id|init_var.pixclock
op_assign
l_int|39722
suffix:semicolon
id|init_var.red.msb_right
op_assign
l_int|0
suffix:semicolon
id|init_var.green.msb_right
op_assign
l_int|0
suffix:semicolon
id|init_var.blue.msb_right
op_assign
l_int|0
suffix:semicolon
r_switch
c_cond
(paren
id|init_var.bits_per_pixel
)paren
(brace
r_default
suffix:colon
id|init_var.bits_per_pixel
op_assign
l_int|8
suffix:semicolon
r_case
l_int|8
suffix:colon
multiline_comment|/* PSEUDOCOLOUR */
id|init_var.bits_per_pixel
op_assign
l_int|8
suffix:semicolon
id|init_var.red.offset
op_assign
l_int|0
suffix:semicolon
id|init_var.red.length
op_assign
l_int|8
suffix:semicolon
id|init_var.green.offset
op_assign
l_int|0
suffix:semicolon
id|init_var.green.length
op_assign
l_int|8
suffix:semicolon
id|init_var.blue.offset
op_assign
l_int|0
suffix:semicolon
id|init_var.blue.length
op_assign
l_int|8
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|15
suffix:colon
multiline_comment|/* RGB555 */
id|init_var.bits_per_pixel
op_assign
l_int|15
suffix:semicolon
id|init_var.red.offset
op_assign
l_int|10
suffix:semicolon
id|init_var.red.length
op_assign
l_int|5
suffix:semicolon
id|init_var.green.offset
op_assign
l_int|5
suffix:semicolon
id|init_var.green.length
op_assign
l_int|5
suffix:semicolon
id|init_var.blue.offset
op_assign
l_int|0
suffix:semicolon
id|init_var.blue.length
op_assign
l_int|5
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|16
suffix:colon
multiline_comment|/* RGB565 */
id|init_var.bits_per_pixel
op_assign
l_int|16
suffix:semicolon
id|init_var.red.offset
op_assign
l_int|11
suffix:semicolon
id|init_var.red.length
op_assign
l_int|5
suffix:semicolon
id|init_var.green.offset
op_assign
l_int|5
suffix:semicolon
id|init_var.green.length
op_assign
l_int|6
suffix:semicolon
id|init_var.blue.offset
op_assign
l_int|0
suffix:semicolon
id|init_var.blue.length
op_assign
l_int|5
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|24
suffix:colon
multiline_comment|/* RGB888 */
id|init_var.bits_per_pixel
op_assign
l_int|24
suffix:semicolon
id|init_var.red.offset
op_assign
l_int|16
suffix:semicolon
id|init_var.red.length
op_assign
l_int|8
suffix:semicolon
id|init_var.green.offset
op_assign
l_int|8
suffix:semicolon
id|init_var.green.length
op_assign
l_int|8
suffix:semicolon
id|init_var.blue.offset
op_assign
l_int|0
suffix:semicolon
id|init_var.blue.length
op_assign
l_int|8
suffix:semicolon
r_break
suffix:semicolon
)brace
id|init_var.nonstd
op_assign
l_int|0
suffix:semicolon
id|init_var.activate
op_assign
id|FB_ACTIVATE_NOW
suffix:semicolon
id|init_var.height
op_assign
op_minus
l_int|1
suffix:semicolon
id|init_var.width
op_assign
op_minus
l_int|1
suffix:semicolon
id|init_var.accel_flags
op_assign
id|FB_ACCELF_TEXT
suffix:semicolon
id|init_var.sync
op_assign
id|FB_SYNC_COMP_HIGH_ACT
op_or
id|FB_SYNC_VERT_HIGH_ACT
suffix:semicolon
id|init_var.vmode
op_assign
id|FB_VMODE_NONINTERLACED
suffix:semicolon
)brace
multiline_comment|/*&n; *    Initialization&n; */
DECL|function|cyber2000fb_init
r_int
id|__init
id|cyber2000fb_init
c_func
(paren
r_void
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
suffix:semicolon
id|u_int
id|h_sync
comma
id|v_sync
suffix:semicolon
id|u_long
id|base_p
comma
id|base_v
suffix:semicolon
id|dev
op_assign
id|pci_find_device
c_func
(paren
id|PCI_VENDOR_ID_INTERG
comma
l_int|0x2000
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dev
)paren
r_return
op_minus
id|ENXIO
suffix:semicolon
multiline_comment|/* this should be done by PCI generic code */
id|base_p
op_assign
l_int|0x80000000
op_plus
id|dev-&gt;resource
(braket
l_int|0
)braket
dot
id|start
suffix:semicolon
multiline_comment|/*&n;&t; * This should be ioremap&squot;d, thus:&n;&t; *&n;&t; * base_v = ioremap(dev-&gt;resource[0].start, dev-&gt;resource[0].end - dev-&gt;resource[0].start + 1);&n;&t; */
id|base_v
op_assign
(paren
id|u_long
)paren
id|bus_to_virt
c_func
(paren
id|dev-&gt;resource
(braket
l_int|0
)braket
dot
id|start
)paren
suffix:semicolon
multiline_comment|/*FIXME*/
id|CyberRegs
op_assign
id|base_v
op_plus
l_int|0x00800000
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x18
comma
l_int|0x46e8
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x01
comma
l_int|0x102
)paren
suffix:semicolon
id|cyber2000_outb
c_func
(paren
l_int|0x08
comma
l_int|0x46e8
)paren
suffix:semicolon
id|cyber2000fb_init_fbinfo
c_func
(paren
)paren
suffix:semicolon
id|current_par.currcon
op_assign
op_minus
l_int|1
suffix:semicolon
id|current_par.screen_base_p
op_assign
id|base_p
suffix:semicolon
id|current_par.screen_base
op_assign
id|base_v
suffix:semicolon
id|current_par.screen_size
op_assign
l_int|0x00200000
suffix:semicolon
id|current_par.regs_base_p
op_assign
id|base_p
op_plus
l_int|0x00800000
suffix:semicolon
id|cyber2000fb_set_var
c_func
(paren
op_amp
id|init_var
comma
op_minus
l_int|1
comma
op_amp
id|fb_info
)paren
suffix:semicolon
id|h_sync
op_assign
l_int|1953125000
op_div
id|init_var.pixclock
suffix:semicolon
id|h_sync
op_assign
id|h_sync
op_star
l_int|512
op_div
(paren
id|init_var.xres
op_plus
id|init_var.left_margin
op_plus
id|init_var.right_margin
op_plus
id|init_var.hsync_len
)paren
suffix:semicolon
id|v_sync
op_assign
id|h_sync
op_div
(paren
id|init_var.yres
op_plus
id|init_var.upper_margin
op_plus
id|init_var.lower_margin
op_plus
id|init_var.vsync_len
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;Cyber2000: %ldkB VRAM, using %dx%d, %d.%03dkHz, %dHz&bslash;n&quot;
comma
id|current_par.screen_size
op_rshift
l_int|10
comma
id|init_var.xres
comma
id|init_var.yres
comma
id|h_sync
op_div
l_int|1000
comma
id|h_sync
op_mod
l_int|1000
comma
id|v_sync
)paren
suffix:semicolon
r_if
c_cond
(paren
id|register_framebuffer
c_func
(paren
op_amp
id|fb_info
)paren
OL
l_int|0
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|MOD_INC_USE_COUNT
suffix:semicolon
multiline_comment|/* TODO: This driver cannot be unloaded yet */
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#ifdef MODULE
DECL|function|init_module
r_int
id|__init
id|init_module
c_func
(paren
r_void
)paren
(brace
r_return
id|cyber2000fb_init
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|cleanup_module
r_void
id|cleanup_module
c_func
(paren
r_void
)paren
(brace
multiline_comment|/* Not reached because the usecount will never be&n;&t;   decremented to zero */
id|unregister_framebuffer
c_func
(paren
op_amp
id|fb_info
)paren
suffix:semicolon
multiline_comment|/* TODO: clean up ... */
)brace
macro_line|#endif&t;&t;&t;&t;/* MODULE */
eof
