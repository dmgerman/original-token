...
id|PAGE_SHIFT
suffix:semicolon
multiline_comment|/* &n; * This used to be just ugly, now it&squot;s downright broken - we can&squot;t do&n; * this when we&squot;re holding the mm semaphore (because the page fault&n; * will also try to get the semaphore - quite correctly). Besides, this&n; * never worked correctly anyway (we may not have read permission to&n; * the area in the first place).&n; */
macro_line|#if 0
r_do
(brace
r_char
...
id|mpnt-&gt;vm_next
suffix:semicolon
multiline_comment|/* we have mpnt-&gt;vm_next = next and addr &lt; mpnt-&gt;vm_end */
id|free
op_assign
l_int|NULL
suffix:semicolon
...
multiline_comment|/* Merge the list of memory segments if possible.&n; * Redundant vm_area_structs are freed.&n; * This assumes that the list is ordered by address.&n; * We don&squot;t need to traverse the entire list, only those segments&n; * which intersect or are adjacent to a given interval.&n; */
...
id|mpnt-&gt;vm_next
suffix:semicolon
multiline_comment|/* To share, we must have the same inode, operations.. */
r_if
c_cond
(paren
(paren
...
r_continue
suffix:semicolon
multiline_comment|/* and if we have an inode, the offsets must be contiguous.. */
r_if
c_cond
(paren
(paren
