...
id|PAGE_SHIFT
suffix:semicolon
multiline_comment|/*&n; * This used to be just slightly broken, now it&squot;s just completely&n; * buggered. We can&squot;t take a page fault here, because we already&n; * hold the mm semaphore (as is proper). We should do this by hand&n; * by calling the appropriate fault-in routine.&n; *&n; * That would also fix this routine wrt writes and PROT_NONE&n; * areas, both of which can&squot;t be handled by the page fault&n; * approach anyway.&n; */
macro_line|#if 0
r_int
r_int
id|start
