...
suffix:semicolon
multiline_comment|/*&n; * This routine is used by the kernel swap daemon to determine&n; * whether we have &quot;enough&quot; free pages. It is fairly arbitrary,&n; * having a low-water and high-water mark.&n; *&n; * This returns:&n; *  0 - urgent need for memory&n; *  1 - need some memory, but do it slowly in the background&n; *  2 - no need to even think about it.&n; */
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Low priority (user) allocations must not&n;&t;&t;&t; * succeed if we didn&squot;t have enough memory&n;&t;&t;&t; * and we couldn&squot;t get more..&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/*&n;&t; * If we failed to find anything, we&squot;ll return NULL, but we&squot;ll&n;&t; * wake up kswapd _now_ ad even wait for it synchronously if&n;&t; * we can.. This way we&squot;ll at least make some forward progress&n;&t; * over time.&n;&t; */
id|wake_up
c_func
(paren
op_amp
