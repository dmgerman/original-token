...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n; * This routine is used by the kernel swap deamon to determine&n; * whether we have &quot;enough&quot; free pages. It is fairly arbitrary,&n; * but this had better return false if any reasonable &quot;get_free_page()&quot;&n; * allocation could currently fail..&n; *&n; * Right now we just require that the highest memory order should&n; * have at least two entries. Whether this makes sense or not&n; * under real load is to be tested, but it also gives us some&n; * guarantee about memory fragmentation (essentially, it means&n; * that there should be at least two large areas available).&n; */
