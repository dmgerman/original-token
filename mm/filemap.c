...
mdefine_line|#define ITERATIONS 100
...
multiline_comment|/* At this stage we have passed through the list&n;&t;&t; * once, and there may still be locked pages. */
r_if
c_cond
(paren
id|head-&gt;next
...
suffix:semicolon
multiline_comment|/* We need to block */
id|lock_page
c_func
(paren
id|page
...
op_decrement
suffix:semicolon
multiline_comment|/*&n;&t;&t; * I&squot;m ambivalent on this one.. Should we try to&n;&t;&t; * maintain LRU on the LRU list, and put pages that&n;&t;&t; * are old at the end of the queue, even if that&n;&t;&t; * means that we&squot;ll re-scan then again soon and&n;&t;&t; * often waste CPU time? Or should be just let any&n;&t;&t; * pages we do not want to touch now for one reason&n;&t;&t; * or another percolate to be &quot;young&quot;?&n;&t;&t; *&n;&t;&t;dispose = &amp;old;&n;&t;&t; *&n;&t;&t; */
...
multiline_comment|/*&n; * Application no longer needs these pages.  If the pages are dirty,&n; * it&squot;s OK to just throw them away.  The app will be more careful about&n; * data it wants to keep.  Be sure to free swap resources too.  The&n; * zap_page_range call sets things up for shrink_mmap to actually free&n; * these pages later if no one else has touched them in the meantime,&n; * although we could add these pages to a global reuse list for&n; * shrink_mmap to pick up before reclaiming other pages.&n; *&n; * NB: This interface discards data rather than pushes it out to swap,&n; * as some implementations do.  This has performance implications for&n; * applications like large transactional databases which want to discard&n; * pages in anonymous maps after committing to backing store the data&n; * that was kept in them.  There is no reason to write this data out to&n; * the swap area if the application is discarding it.&n; *&n; * An interface that causes the system to free clean pages and flush&n; * dirty pages is already available as msync(MS_INVALIDATE).&n; */
