...
DECL|macro|READAHEAD_PAGES
mdefine_line|#define READAHEAD_PAGES 3
DECL|macro|MAX_IO_PAGES
mdefine_line|#define MAX_IO_PAGES 4
...
suffix:semicolon
multiline_comment|/* We only set &quot;newpage&quot; when we encounter a&n;                   completely uncached page.  This way, we do no&n;                   readahead if we are still just reading data out of&n;                   the cache (even if the cached page is not yet&n;                   uptodate --- it may be currently being read as a&n;                   result of previous readahead).  -- sct */
id|newpage
op_assign
l_int|1
suffix:semicolon
...
id|count
suffix:semicolon
multiline_comment|/* We have two readahead cases.  First, do data&n;                   pre-read if the current read request is for more&n;                   than one page, so we can merge the adjacent&n;                   requests. */
r_if
c_cond
(paren
id|newpage
...
r_else
multiline_comment|/* Second, do readahead at the end of the read, if we&n;                   are still waiting on the current IO to complete, if&n;                   readahead is flagged for the file, and if we have&n;                   finished with the current block. */
r_if
c_cond
(paren
id|newpage
