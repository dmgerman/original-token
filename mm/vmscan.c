...
r_continue
suffix:semicolon
multiline_comment|/* Skip tasks which haven&squot;t slept long enough yet when idle-swapping. */
r_if
c_cond
(paren
id|idle_time
...
id|page_active
suffix:semicolon
multiline_comment|/* Can&squot;t start IO? Move it to the back of the list */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/*&n;&t; * Calculate the minimum time (in seconds) a process must&n;&t; * have slept before we consider it for idle swapping.&n;&t; * This must be the number of seconds it takes to go through&n;&t; * all of the cache. Doing this idle swapping makes the VM&n;&t; * smoother once we start hitting swap.&n;&t; */
id|idle_time
op_assign
id|atomic_read
c_func
