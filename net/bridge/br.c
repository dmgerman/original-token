...
multiline_comment|/* MAG: Maximum port registered - used to speed up flooding and to make&n; * have a large ports array more efficient&n; */
DECL|variable|max_port_used
r_static
r_int
id|max_port_used
op_assign
...
suffix:semicolon
multiline_comment|/* &n; * the following data is for the bridge network device&n; */
DECL|struct|brg_if
r_struct
id|brg_if
(brace
...
suffix:semicolon
DECL|variable|brg_if
r_static
r_struct
id|brg_if
id|brg_if
suffix:semicolon
multiline_comment|/* &n; * Here to save linkage? problems&n; */
...
(paren
id|KERN_INFO
l_string|&quot;NET4: Ethernet Bridge 006 for NET4.0&bslash;n&quot;
)paren
suffix:semicolon
multiline_comment|/* Set up brg device information */
id|bridge_info.instance
op_assign
l_int|0
suffix:semicolon
id|brg_init
...
multiline_comment|/* initial state = Disable */
id|user_port_state
(braket
id|port_no
)braket
...
id|skb
suffix:semicolon
multiline_comment|/*&n;&t; *&t;Keep silent when disabled or when STP disabled&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|NOTIFY_DONE
suffix:semicolon
multiline_comment|/* Don&squot;t attach the brg device to a port! */
r_switch
c_cond
(paren
id|event
...
multiline_comment|/* Add local MAC address */
id|br_add_local_mac
c_func
(paren
id|dev-&gt;dev_addr
...
suffix:semicolon
multiline_comment|/* Save MAC address for latter change address events */
id|memcpy
c_func
(paren
id|port_info
...
(paren
id|KERN_DEBUG
l_string|&quot;br_device_event: NETDEV_REGISTER...&bslash;n&quot;
)paren
suffix:semicolon
multiline_comment|/* printk(KERN_ERR &quot;br_device_event: NETDEV_REGISTER...&bslash;n&quot;); */
multiline_comment|/* printk(KERN_ERR &quot;br_device_event: dev-&gt;type: 0x%X&bslash;n&quot;, dev-&gt;type); */
multiline_comment|/* Only handle ethernet ports */
r_if
c_cond
(paren
...
multiline_comment|/* printk(KERN_ERR &quot;br_device_event: Looking for port...&bslash;n&quot;); */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/* printk(KERN_ERR &quot;br_device_event: Found port %d&bslash;n&quot;, i); */
id|port_info
(braket
id|i
)braket
...
multiline_comment|/* handle local MAC address minuplations */
id|br_add_local_mac
c_func
(paren
id|dev-&gt;dev_addr
...
(paren
id|KERN_DEBUG
l_string|&quot;br_device_event: NETDEV_UNREGISTER...&bslash;n&quot;
)paren
suffix:semicolon
id|i
op_assign
...
(paren
id|KERN_DEBUG
l_string|&quot;br_device_event: NETDEV_CHANGEADDR...&bslash;n&quot;
)paren
suffix:semicolon
id|i
op_assign
...
r_break
suffix:semicolon
multiline_comment|/* Don&squot;t worry about a change of hardware broadcast address! */
r_if
c_cond
(paren
id|dev-&gt;start
...
(paren
id|KERN_CRIT
l_string|&quot;br_device_event: NETDEV_CHANGEADDR on busy device %s - FIX DRIVER!&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
multiline_comment|/*  return NOTIFY_BAD;  It SHOULD be this, but I want to be friendly... */
r_return
id|NOTIFY_DONE
suffix:semicolon
)brace
...
multiline_comment|/* Routine to loop over device list and register &n; * interfaces to bridge.  Called from last part of net_dev_init just before&n; * bootp/rarp interface setup&n; */
DECL|function|br_spacedevice_register
r_void
...
multiline_comment|/* This is for SPEED in the kernel in net_bh.c */
DECL|function|br_call_bridge
r_int
id|br_call_bridge
c_func
(paren
...
id|dev
suffix:semicolon
macro_line|#if 0  /* Checked first in handle_bridge to save expense of function call */ 
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Sanity - make sure we are not leaping off into fairy space! */
r_if
c_cond
(paren
id|port
...
(paren
id|KERN_CRIT
l_string|&quot;br_call_bridge: device %s has invalid port ID %d!&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|dev-&gt;bridge_port_id
...
multiline_comment|/* Hand off to brg_rx BEFORE we screw up the skb */
r_if
c_cond
(paren
id|brg_rx
...
multiline_comment|/* Don&squot;t go scanning empty port entries */
r_break
suffix:semicolon
r_if
c_cond
...
multiline_comment|/*&t;&t;&t;printk(KERN_DEBUG &quot;Flood to port %d&bslash;n&quot;,i);*/
id|nskb-&gt;nh.raw
op_assign
id|nskb-&gt;data
op_plus
...
multiline_comment|/* ATM Lan Emulation (LANE) */
r_return
l_int|7
suffix:semicolon
multiline_comment|/* 155 Mbs */
r_if
c_cond
(paren
id|strncmp
...
multiline_comment|/* Fill in interface names in port_info structure&n; */
DECL|function|br_get_ifnames
r_static
r_void
id|br_get_ifnames
c_func
...
(brace
multiline_comment|/* memset IS needed.  Kernel strncpy does NOT NULL terminate strings when limit&n;       reached */
id|memset
c_func
(paren
id|port_info
...
suffix:semicolon
multiline_comment|/* Being paranoid */
id|port_info
(braket
id|i
)braket
...
multiline_comment|/* Given an interface index, loop over port array to see if configured.  If&n;   so, return port number, otherwise error */
DECL|function|br_find_port
r_static
r_int
id|br_find_port
c_func
...
id|EUNATCH
suffix:semicolon
multiline_comment|/* Tell me if this is incorrect error code for this case */
)brace
...
suffix:semicolon
multiline_comment|/* Fill in interface names in port_info*/
id|br_get_ifnames
c_func
(paren
)paren
suffix:semicolon
id|br_stats.num_ports
op_assign
id|No_of_ports
suffix:semicolon
id|memcpy
...
(paren
id|KERN_DEBUG
l_string|&quot;br: %s spanning tree protcol&bslash;n&quot;
comma
(paren
id|br_stats.flags
op_amp
...
r_else
(brace
multiline_comment|/* STP was enabled, now disable it */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* --------------------------------------------------------------------------------&n; *&n; *&n; *  Bridge network device here for future modularization - device structures&n; *  must be &squot;static&squot; inside bridge instance&n; *  Modelled after sch_teql.c&n; * &n; */
...
(paren
id|KERN_INFO
l_string|&quot;%s: network interface for Ethernet Bridge 006/NET4.0&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
multiline_comment|/*&n;   *&t;Initialize the device structure.&n;   */
id|dev-&gt;priv
op_assign
id|kmalloc
c_func
...
suffix:semicolon
multiline_comment|/* Set up MAC address based on BogoMIPs figure for first CPU and time&n;   */
id|bogomips
op_assign
(paren
id|loops_per_sec
...
suffix:semicolon
multiline_comment|/* Ummmm....  YES! */
id|dev-&gt;dev_addr
(braket
l_int|0
)braket
op_assign
l_char|&squot;&bslash;xFE&squot;
suffix:semicolon
id|dev-&gt;dev_addr
(braket
l_int|1
...
(paren
id|KERN_INFO
l_string|&quot;%s: generated MAC address %2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|dev-&gt;dev_addr
...
(paren
id|KERN_INFO
l_string|&quot;%s: attached to bridge instance %lu&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|dev-&gt;base_addr
...
suffix:semicolon
multiline_comment|/*&n;   *&t;The brg specific entries in the device structure.&n;   */
id|dev-&gt;open
op_assign
id|brg_open
suffix:semicolon
...
id|brg_set_multicast_list
suffix:semicolon
multiline_comment|/*&n;   *&t;Setup the generic properties&n;   */
id|ether_setup
c_func
(paren
id|dev
...
DECL|function|brg_set_multicast_list
r_static
r_void
id|brg_set_multicast_list
c_func
...
multiline_comment|/*&n; *&t;We transmit by throwing the packet at the bridge.&n; */
DECL|function|brg_start_xmit
r_static
r_int
id|brg_start_xmit
c_func
...
id|port
suffix:semicolon
multiline_comment|/* Deal with the bridge being disabled */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* Either this */
multiline_comment|/* lp-&gt;stats.tx_errors++; */
multiline_comment|/* this condition is NOT an error */
multiline_comment|/* or this  (implied by RFC 2233) */
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: brg_start_xmit - src %02x:%02x:%02x:%02x:%02x:%02x&quot;
l_string|&quot; dest %02x:%02x:%02x:%02x:%02x:%02x&bslash;n&quot;
comma
id|dev-&gt;name
comma
...
suffix:semicolon
multiline_comment|/* Forward the packet ! */
r_if
c_cond
(paren
id|br_forward
...
multiline_comment|/* Throw packet initially */
id|dev_kfree_skb
c_func
(paren
id|skb
...
multiline_comment|/*&n; *&t;The typical workload of the driver:&n; *&t;Handle the ether interface interrupts.&n; *&n; *&t;(In this case handle the packets posted from the bridge)&n; */
DECL|function|brg_rx
r_static
r_int
id|brg_rx
c_func
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: brg_rx()&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
multiline_comment|/* Get out of here if the bridge interface is not up&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Check that the port that this thing came off is in the forwarding state &n;&t; * We sould only listen to the same address scope we will transmit to.&n;&t; */
r_if
c_cond
(paren
id|port_info
...
multiline_comment|/* Is this for us? - broadcast/mulitcast/promiscuous packets need cloning,&n;         * with uni-cast we eat the packet&n;&t; */
id|clone
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/* Clone things here - we want to be transparent before we check packet data &n;&t; * integrity&n;&t; */
r_if
c_cond
(paren
id|clone
...
multiline_comment|/* Check packet length &n;&t; */
r_if
c_cond
(paren
id|len
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: brg_rx - src %02x:%02x:%02x:%02x:%02x:%02x&quot;
l_string|&quot; dest %02x:%02x:%02x:%02x:%02x:%02x&bslash;n&quot;
comma
id|dev-&gt;name
comma
...
macro_line|#if 0&t;&t;&t;&t;/* Its here if we ever need it... */
...
suffix:semicolon
multiline_comment|/*&n;   *&t;Free up the private structure.&n;   */
id|kfree
c_func
(paren
id|brg_if.dev.priv
...
l_int|NULL
suffix:semicolon
multiline_comment|/* gets re-allocated by brg_probe */
)brace
