...
(paren
(paren
l_string|&quot;tcp.c: icmp_err got error&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|sk-&gt;err
...
id|errno
suffix:semicolon
multiline_comment|/* if we&squot;ve already connected we will keep trying&n;      until we time out, or the user gives up. */
r_if
c_cond
(paren
id|icmp_err_convert
...
suffix:semicolon
multiline_comment|/* should this return an error? */
id|release_sock
(paren
id|sk
)paren
...
suffix:semicolon
multiline_comment|/* this area has caused the most trouble.  The current strategy&n;      is to simply do nothing if the other end has room to send atleast&n;      3 full packets, because the ack from those will automatically&n;      update the window.  If the other end doesn&squot;t think we have much&n;      space left, but we have room for atleast 1 more complete packet&n;      than it thinks we do, we will send an ack immediatedly.  Otherwise&n;      we will wait up to .5 seconds in case the user reads some more. */
id|sk-&gt;ack_backlog
op_increment
suffix:semicolon
r_if
...
multiline_comment|/* or we could have changed state. */
id|cli
c_func
(paren
)paren
...
multiline_comment|/* sends a fin without closing the connection.  Not called&n;   at interrupt time. */
r_void
...
l_int|NULL
suffix:semicolon
multiline_comment|/* Written; Tim MacKenzie (tym@dibbler.cs.monash.edu.au) 4 Dec &squot;92.&n; * Most of this is guesswork, so maybe it will work...&n; */
...
multiline_comment|/* make sure we can accept more.  This will prevent a flurry of&n;     syns from eating up all our memory. */
r_if
c_cond
(paren
id|sk-&gt;ack_backlog
...
l_int|NULL
suffix:semicolon
multiline_comment|/* get rid on any half completed packets. */
r_if
c_cond
(paren
id|sk-&gt;send_tmp
...
(brace
multiline_comment|/* we may need to move packets from the send queue to the&n;&t; write queue. if the window has been shrunk on us. */
multiline_comment|/* the rfc says you are not allowed to shrink your window like&n;&t; this, but if the other end does, you must be able to deal&n;&t; with it. */
r_struct
id|sk_buff
op_star
id|skb
...
(brace
multiline_comment|/* remove it from the send queue. */
r_if
c_cond
(paren
id|skb2
...
multiline_comment|/* there is no longer any reason to do this.  Just let tcp_data&n;     deal with it. */
id|sk-&gt;ack_backlog
op_increment
suffix:semicolon
macro_line|#if 0
multiline_comment|/* send an ack */
id|buff
op_assign
id|sk-&gt;prot
op_member_access_from_pointer
...
multiline_comment|/* a reset with a fin just means that the&n;&t;     data was not all read. */
r_if
c_cond
(paren
op_logical_neg
