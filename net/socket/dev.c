...
macro_line|#endif
macro_line|#ifdef CONFIG_IPX
multiline_comment|/*&n; *&t;These describe how each lowest level protocol is processed.&n; */
...
comma
multiline_comment|/* IPX over DIX - eg PDIPX */
l_int|0
comma
id|ipx_rcv
comma
...
suffix:semicolon
multiline_comment|/*&n; *&t;The list of known protocols. Note that&n; *&t;SOCK_PACKET sockets dynamically alter this.&n; */
...
macro_line|#endif
macro_line|#endif
multiline_comment|/* A queue of all the packets we have to deal with */
...
multiline_comment|/*&n; *&t;Add a protocol ID to the list of known protocols&n; *&t;(see SOCK_PACKET code)&n; */
...
multiline_comment|/* &t;&n; *&t;Remove a protocol ID from the list. Also used&n; *&t;for SOCK_PACKET. Maybe one day we will do loadable&n; *&t;protocol layers too.&n; */
...
multiline_comment|/*&n; *&t;Find an interface in the list.&n; */
...
(brace
multiline_comment|/* It went out without fuss */
r_return
suffix:semicolon
)brace
multiline_comment|/* The driver was busy.. */
...
multiline_comment|/* We need to know if we are re-entering the bottom level handler&n;   for the network */
DECL|variable|in_bh
r_static
r_volatile
r_char
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Bump the pointer to the next structure.&n;&t;&t; * This assumes that the basic &squot;skb&squot; pointer points to&n;&t;&t; * the MAC header, if any (as indicated by its &quot;length&quot;&n;&t;&t; * field).  Take care now!&n;&t;&t; */
id|skb-&gt;h.raw
op_assign
(paren
r_int
...
id|skb-&gt;dev-&gt;hard_header_len
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Fetch the packet protocol ID.  This is also quite ugly, as&n;&t; &t; * it depends on the protocol driver (the interface itself) to&n;&t;&t; * know what the type is, or where to get it from.  The Ethernet&n;&t;&t; * interfaces fetch the ID from the two bytes in the Ethernet MAC&n;&t;&t; * header (the h_proto field in struct ethhdr), but drivers like&n;&t;&t; * SLIP and PLIP have no alternative but to force the type to be&n;&t;&t; * IP or something like that.  Sigh- FvK&n;&t;&t; */
id|type
op_assign
id|skb-&gt;dev
op_member_access_from_pointer
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We got a packet ID.  Now loop over the &quot;known protocols&quot;&n;&t;&t; * table (which is actually a linked list, but this will&n;&t;&t; * change soon if I get my way- FvK), and forward the packet&n;&t;&t; * to anyone who wants it.&n;&t;&t; */
r_for
c_loop
(paren
id|ptype
...
multiline_comment|/* This used to be in the &squot;else&squot; part, but then&n;&t;&t;&t;&t; * we don&squot;t have this flag set when we get a&n;&t;&t;&t;&t; * protocol that *does* require copying... -FvK&n;&t;&t;&t;&t; */
id|flag
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/*&n;&t;&t; * That&squot;s odd.  We got an unknown packet.  Who&squot;s using&n;&t;&t; * stuff like Novell or Amoeba on this network??&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Try and kick anything this processing produced */
)brace
...
multiline_comment|/* &n; *&t;Perform a SIOCGIFCONF call. &n; */
...
multiline_comment|/* &n; *&t;Print device statistics. &n; */
...
multiline_comment|/*&n; *&t;Called from the PROCfs module (/proc/net/dev).&n; */
...
multiline_comment|/* Failed to come up so go down again */
)brace
)brace
r_break
suffix:semicolon
...
multiline_comment|/*&n; *&t;This function handles all &quot;interface&quot;-type I/O control requests. &n; */
...
c_func
(paren
l_string|&quot;IP_SET_DEV is obsolete. You need newer network tools.&bslash;n&quot;
)paren
suffix:semicolon
id|ret
op_assign
...
multiline_comment|/*&n; *&t;Setup an ethernet type interface&n; */
...
r_return
suffix:semicolon
multiline_comment|/* Walk the device list */
r_while
c_loop
(paren
id|d
...
multiline_comment|/* &n; *&t;Initialize the DEV module. &n; */
...
id|dev2
suffix:semicolon
multiline_comment|/* Add the devices.&n;  &t; * If the call to dev-&gt;init fails, the dev is removed&n;  &t; * from the chain disconnecting the device until the&n;  &t; * next reboot.&n;  &t; */
id|dev2
op_assign
l_int|NULL
suffix:semicolon
