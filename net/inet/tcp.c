...
suffix:semicolon
multiline_comment|/* Enforce RFC793 - we&squot;ve offered it we must live with it */
r_if
c_cond
(paren
id|new_window
...
multiline_comment|/*&n; *  If we had the full V-J mechanism, this might be right.  But&n; *  for the moment we want simple slow start after error.&n; *&n; *  if (sk-&gt;cong_window &gt; 4)&n; *       sk-&gt;cong_window = sk-&gt;cong_window / 2;&n; */
id|sk-&gt;cong_window
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/* This is wrong: It breaks Chameleon amongst other stacks */
r_if
c_cond
(paren
id|count
...
id|DBG_TCP
comma
l_string|&quot;tcp_readable, more than 20 packets without a psh&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;tcp_read: possible read_queue corruption.&bslash;n&quot;
)paren
suffix:semicolon
r_return
id|amount
...
suffix:semicolon
macro_line|#else
multiline_comment|/* This also prevents silly windows by simply ignoring the offered window.. */
id|copy
op_assign
id|min
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Now we may have some data waiting or we could&n;&t;&t; * have changed state.&n;&t;&t; */
id|cli
c_func
(paren
)paren
...
suffix:semicolon
multiline_comment|/* That may have been null if we were beaten, if so we loop again */
)brace
multiline_comment|/*&n;&t; * Copy anything from the current block that needs&n;&t; * to go into the user buffer.&n;&t; */
id|offset
op_assign
id|sk-&gt;copied_seq
op_plus
...
(brace
multiline_comment|/*&n;&t;&t; * If there is urgent data we must either&n;&t;&t; * return or skip over it.&n;&t;&t; */
r_if
c_cond
(paren
id|skb-&gt;h.th-&gt;urg
...
id|used
suffix:semicolon
multiline_comment|/* If we were reading the data is &squot;eaten&squot; */
r_if
c_cond
(paren
op_logical_neg
...
id|used
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Mark this data used if we are really reading it,&n;&t;&t; * and if it doesn&squot;t contain any urgent data. And we&n;&t;&t; * have used all the data.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
l_int|1
suffix:semicolon
multiline_comment|/*&n;&t;&t; * See if this is the end of a message or if the&n;&t;&t; * remaining data is urgent.&n;&t;&t; */
r_if
c_cond
(paren
multiline_comment|/*skb-&gt;h.th-&gt;psh || */
id|skb-&gt;h.th-&gt;urg
)paren
(brace
r_break
...
multiline_comment|/* Move along a packet */
id|skb
op_assign
(paren
r_struct
...
(brace
multiline_comment|/* if we&squot;re retransmitting, don&squot;t start any new&n;&t;&t;   * packets until after everything in retransmit queue&n;&t;&t;   * is acked.  That&squot;s as close as I can come at the&n;&t;&t;   * moment to slow start the way this code is organized&n;&t;&t;   */
r_if
c_cond
(paren
id|sk-&gt;send_head-&gt;link3
...
multiline_comment|/*&n;&t;&t; * need to restart backoff whenever we get a response,&n;&t;&t; * or things get impossible if we lose a window-full of&n;&t;&t; * data with very small MSS&n;&t;&t; */
id|sk-&gt;backoff
op_assign
l_int|0
suffix:semicolon
...
id|sk-&gt;send_head
suffix:semicolon
multiline_comment|/* &n;&t;&t; * In theory we&squot;re supposed to ignore rtt&squot;s when there&squot;s&n;&t;&t; * retransmission in process.  Unfortunately this means&n;&t;&t; * that if there&squot;s a sharp increase in RTT, we may &n;&t;&t; * never get out of retransmission.  For the moment&n;&t;&t; * ignore the test.&n;&t;&t; */
r_if
c_cond
(paren
multiline_comment|/* sk-&gt;retransmits == 0 &amp;&amp; */
op_logical_neg
(paren
id|flag
op_amp
...
id|oskb-&gt;when
suffix:semicolon
multiline_comment|/*&n;&t;&t;   * Berkeley&squot;s code puts these limits on a separate timeout&n;&t;&t;   * field, not on the RTT estimate itself.  However the way this&n;&t;&t;   * code is done, that would complicate things.  If we&squot;re going&n;&t;&t;   * to clamp the values, we have to do so before calculating&n;&t;&t;   * the mdev, or we&squot;ll get unreasonably large mdev&squot;s.  Experience&n;&t;&t;   * shows that with a minium rtt of .1 sec, we get spurious&n;&t;&t;   * retransmits, due to delayed acks on some hosts.  Berkeley uses&n;&t;&t;   * 1 sec, so why not?&n;&t;&t;   */
r_if
c_cond
(paren
id|rtt
...
l_int|12000
suffix:semicolon
multiline_comment|/* 2 min - max rtt allowed by protocol */
r_if
c_cond
(paren
id|sk-&gt;state
...
id|rtt
suffix:semicolon
multiline_comment|/* overcautious initial estimate */
)brace
r_else
(brace
id|abserr
...
l_int|NULL
suffix:semicolon
multiline_comment|/* so we know we might be able&n;&t;&t;&t;&t;&t;to ack stuff. */
r_break
suffix:semicolon
)brace
macro_line|#else
...
multiline_comment|/* This functions checks to see if the tcp header is actually acceptible. */
r_static
r_int
...
suffix:semicolon
multiline_comment|/*&n;   *&t;Send a reset if we get something not ours and we are&n;   *&t;unsynchronized. Note: We don&squot;t do anything to our end. We&n;   *&t;are just killing the bogus remote connection then we will&n;   *&t;connect again and it will work (with luck).&n;   */
r_if
c_cond
(paren
id|sk-&gt;state
...
multiline_comment|/*&n;   * If it&squot;s too far ahead, send an ack to let the&n;   * other end know what we expect.&n;   */
r_if
c_cond
(paren
id|after
...
macro_line|#if 0&t;/* FIXME: it&squot;s ok for protocol to be NULL */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* FIXME: it&squot;s ok for opt to be NULL */
id|DPRINTF
c_func
(paren
(paren
...
multiline_comment|/*&n; * This routine probes a zero window.  It makes a copy of the first&n; * packet in the write queue, but with just one byte of data.&n; */
r_void
...
c_func
(paren
l_string|&quot;tcp_send_probe0: NULL device bug!&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
...
(brace
multiline_comment|/*    printk(&quot;alloc failed raw %x th %x hlen %d data %d len %d&bslash;n&quot;,&n;&t;   raw, skb-&gt;h.th, hlen, data, len); */
id|reset_timer
(paren
id|sk
comma
...
id|hlen
suffix:semicolon
multiline_comment|/* it&squot;s now h.th -- pointer to the tcp header */
id|t1
op_assign
id|skb2-&gt;h.th
suffix:semicolon
multiline_comment|/* source, dest, seq, from existing packet */
id|t1-&gt;ack_seq
op_assign
id|ntohl
c_func
...
l_int|0
suffix:semicolon
multiline_comment|/* doff, fin, from existing packet.  Fin is safe because Linux always&n; * sends fin in a separate packet&n; * syn, rst, had better be zero in original */
id|t1-&gt;ack
op_assign
l_int|1
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* urgent pointer might be beyond this fragment */
id|t1-&gt;res2
op_assign
l_int|0
suffix:semicolon
