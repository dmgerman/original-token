...
op_increment
suffix:semicolon
multiline_comment|/*&n;   * in the case of retransmissions, there&squot;s good reason to limit&n;   * rto to 120 sec, as that&squot;s the maximum legal RTT on the Internet.&n;   * For probes it could reasonably be longer.  However making it&n;   * much longer could cause unacceptable delays in some situation,&n;   * so we might as well use the same value&n;   */
id|sk-&gt;rto
op_assign
id|min
c_func
