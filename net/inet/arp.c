...
multiline_comment|/* If free was 0, magic is now 0, next is 0 and &n;&t;&t;&t;   the write queue will notice and kill */
id|sti
c_func
(paren
)paren
...
multiline_comment|/*&n; *&t;We are now a bit smarter. We know the old buffer must be big enough&n; *&t;so why allocate a new one for the reply ?&n; */
...
l_int|NULL
suffix:semicolon
multiline_comment|/* =NULL otherwise the compiler gives warnings */
...
suffix:semicolon
multiline_comment|/* Grow the packet back to its original form */
multiline_comment|/* Can&squot;t check for exceeding the size - some people pad. */
id|len
op_assign
r_sizeof
(paren
...
c_func
(paren
l_string|&quot;Received runt ARP request!&bslash;n&quot;
)paren
suffix:semicolon
id|kfree_skb
c_func
...
r_else
multiline_comment|/* Proxy arp, so pull from the table */
id|memcpy
c_func
(paren
id|ptr2
...
id|DBG_ARP
comma
l_string|&quot;ARP: lookup proxy(%s)&bslash;n&quot;
comma
id|in_ntoa
c_func
(paren
...
id|arp-&gt;ar_pln
suffix:semicolon
multiline_comment|/*memcpy(ptr, dev-&gt;broadcast, arp-&gt;ar_hln);*/
id|memset
c_func
(paren
id|ptr
...
multiline_comment|/* Count proxy arps so we know if to use it */
r_return
l_int|0
suffix:semicolon
)brace
