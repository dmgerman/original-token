...
multiline_comment|/*&n; * Used by &squot;rt_add()&squot; when we can&squot;t get the netmask from the device..&n; *&n; * If the lower byte or two are zero, we guess the mask based on the&n; * number of zero 8-bit net numbers, otherwise we use the &quot;default&quot;&n; * masks judging by the destination address.&n; *&n; * We should really use masks everywhere, but the current system&n; * interface for adding routes doesn&squot;t even contain a netmask field.&n; * Similarly, ICMP redirect messages contain only the address to&n; * redirect.. Anyway, this function should give reasonable values&n; * for almost anything.&n; */
...
multiline_comment|/*&n; * rewrote this too.. Maybe somebody can understand it now. Linus&n; */
...
multiline_comment|/*&n;   * This depends on &squot;rt_mask&squot; and the ordering set up in &squot;rt_add()&squot; - Linus&n;   */
r_for
c_loop
(paren
id|rt
