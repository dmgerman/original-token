...
comma
id|xprt-&gt;snd_buf.io_vec-&gt;iov_base
comma
id|xprt-&gt;snd_buf.io_vec-&gt;iov_len
)paren
suffix:semicolon
...
id|msg.msg_iov
op_assign
id|xprt-&gt;snd_buf.io_vec
suffix:semicolon
id|msg.msg_iovlen
op_assign
id|xprt-&gt;snd_buf.io_nr
suffix:semicolon
id|msg.msg_name
op_assign
(paren
...
id|inet-&gt;data_ready
op_assign
id|xprt-&gt;inet-&gt;data_ready
suffix:semicolon
id|inet-&gt;state_change
op_assign
id|xprt-&gt;inet-&gt;state_change
suffix:semicolon
id|inet-&gt;write_space
op_assign
...
c_func
(paren
l_string|&quot;RPC: %4d closing old socket&bslash;n&quot;
comma
id|task-&gt;tk_pid
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Reset to new socket and default congestion */
id|xprt-&gt;sock
op_assign
id|sock
suffix:semicolon
...
multiline_comment|/*&n; * Reconnect status&n; */
r_static
r_void
...
c_func
(paren
l_string|&quot;RPC: %4d xprt_reconn_status %d&bslash;n&quot;
comma
id|task-&gt;tk_pid
comma
id|task-&gt;tk_status
...
r_static
r_void
DECL|function|xprt_reconn_timeout
id|xprt_reconn_timeout
c_func
(paren
r_struct
...
l_int|8
suffix:semicolon
multiline_comment|/* don&squot;t account for UDP header */
r_if
c_cond
(paren
id|repsize
...
multiline_comment|/* Look up the request corresponding to the given XID */
r_if
c_cond
(paren
op_logical_neg
...
id|repsize
suffix:semicolon
multiline_comment|/* Okay, we have it. Copy datagram... */
id|memcpy
c_func
(paren
id|iov
...
multiline_comment|/* Get the record length and mask out the more_fragments bit */
id|reclen
op_assign
id|ntohl
c_func
...
c_func
(paren
l_string|&quot;RPC:      reclen %08x&bslash;n&quot;
comma
id|reclen
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;RPC:      got xid %08x reclen %d morefrags %d&bslash;n&quot;
comma
id|xprt-&gt;tcp_xid
comma
id|xprt-&gt;tcp_reclen
...
multiline_comment|/* Request must be re-encoded before retransmit */
id|req-&gt;rq_damaged
op_assign
l_int|1
suffix:semicolon
...
c_func
(paren
l_string|&quot;RPC:      tcp_input_record done (off %d total %d copied %d)&bslash;n&quot;
comma
id|offset
comma
id|xprt-&gt;tcp_total
...
(paren
id|KERN_WARNING
l_string|&quot;RPC: unexpected error %d from tcp_input_record&bslash;n&quot;
comma
id|result
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t; *&t;If we are not waiting for the RPC bh run then&n;&t; *&t;we are now&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;RPC:     xprt queued already %p&bslash;n&quot;
comma
id|xprt
)paren
suffix:semicolon
...
multiline_comment|/*&n; * (Partly) transmit the RPC packet&n; * Note that task-&gt;tk_status is either 0 or negative on return.&n; * Only when the reply is received will the status be set to a&n; * positive value.&n; */
r_static
r_inline
r_int
DECL|function|xprt_transmit_some
id|xprt_transmit_some
c_func
(paren
r_struct
id|rpc_xprt
...
multiline_comment|/* If we&squot;re not already in the process of transmitting our call,&n;&t; * set up everything as needed. */
r_if
c_cond
(paren
id|xprt-&gt;snd_task
...
multiline_comment|/* Reset timeout parameters */
id|timeo
op_assign
op_amp
id|req-&gt;rq_timeout
...
c_func
(paren
l_string|&quot;RPC: %4d xprt_transmit reset timeo&bslash;n&quot;
comma
id|task-&gt;tk_pid
)paren
suffix:semicolon
...
(brace
multiline_comment|/* Remove from pending */
...
multiline_comment|/*&n; * This callback is invoked when the sending task is forced to sleep&n; * because the TCP write buffers are full&n; */
r_static
r_void
DECL|function|xprt_transmit_status
id|xprt_transmit_status
c_func
(paren
r_struct
...
id|xprt
op_assign
id|task-&gt;tk_client-&gt;cl_xprt
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;RPC: %4d transmit_status %d&bslash;n&quot;
comma
id|task-&gt;tk_pid
comma
id|task-&gt;tk_status
...
multiline_comment|/*&n; * Wait for the reply to our call.&n; * When the callback is invoked, the congestion window should have&n; * been updated already.&n; */
r_void
...
multiline_comment|/*&n;&t; * Wait until rq_gotit goes non-null, or timeout elapsed.&n;&t; */
id|task-&gt;tk_timeout
op_assign
...
c_func
(paren
l_string|&quot;RPC: %4d xprt_receive returns %d&bslash;n&quot;
comma
id|task-&gt;tk_pid
comma
id|task-&gt;tk_status
...
r_static
r_void
DECL|function|xprt_receive_status
id|xprt_receive_status
c_func
(paren
r_struct
...
c_func
(paren
l_string|&quot;RPC: (task is on %s)&bslash;n&quot;
comma
id|rpc_qname
c_func
(paren
...
multiline_comment|/*&n; * Create and initialize an RPC client given an open file.&n; * This is obsolete now.&n; */
macro_line|#if 0
r_struct
id|rpc_xprt
op_star
...
c_func
(paren
l_string|&quot;RPC: file == NULL in xprt_create!&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
...
(paren
id|KERN_WARNING
l_string|&quot;RPC: only INET sockets supported&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|NULL
...
c_func
(paren
l_string|&quot;RPC: TCP connect failed (%d).&bslash;n&quot;
comma
op_minus
id|err
)paren
