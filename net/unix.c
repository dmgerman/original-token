...
multiline_comment|/*&n; * to do a socketpair, we just connect the two datas, easy! since we&n; * always wait on the socket inode, they&squot;re no contention for a wait area,&n; * and deadlock prevention in the case of a process writing to itself is,&n; * ignored, in true unix fashion!&n; */
r_static
r_int
