...
multiline_comment|/*&n;   NOTE. The most unpleasent question is serialization of&n;   accesses to resolved addresses. The problem is that addresses&n;   are modified by bh, but they are referenced from normal&n;   kernel thread. Before today no locking was made.&n;   My reasoning was that corrupted address token will be copied&n;   to packet with cosmologically small probability&n;   (it is even difficult to estimate such small number)&n;   and it is very silly to waste cycles in fast path to lock them.&n;&n;   But now I changed my mind, but not because previous statement&n;   is wrong. Actually, neigh-&gt;ha MAY BE not opaque byte array,&n;   but reference to some private data. In this case even neglibible&n;   corruption probability becomes bug.&n;&n;   - hh cache is protected by rwlock. It assumes that&n;     hh cache update procedure is short and fast, and that&n;     read_lock is cheaper than start_bh_atomic().&n;   - ha tokens, saved in neighbour entries, are protected&n;     by bh_atomic().&n;   - no protection is made in /proc reading. It is OK, because&n;     /proc is broken by design in any case, and&n;     corrupted output is normal behaviour there.&n;&n;     --ANK (981025)&n; */
