multiline_comment|/*&n; * INET&t;&t;An implementation of the TCP/IP protocol suite for the LINUX&n; *&t;&t;operating system.  INET is implemented using the  BSD Socket&n; *&t;&t;interface as the means of communication with the user level.&n; *&n; *&t;&t;Routing netlink socket interface: protocol independent part.&n; *&n; * Authors:&t;Alexey Kuznetsov, &lt;kuznet@ms2.inr.ac.ru&gt;&n; *&n; *&t;&t;This program is free software; you can redistribute it and/or&n; *&t;&t;modify it under the terms of the GNU General Public License&n; *&t;&t;as published by the Free Software Foundation; either version&n; *&t;&t;2 of the License, or (at your option) any later version.&n; *&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/socket.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/major.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/timer.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/sockios.h&gt;
macro_line|#include &lt;linux/net.h&gt;
macro_line|#include &lt;linux/fcntl.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/interrupt.h&gt;
macro_line|#include &lt;linux/skbuff.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/string.h&gt;
macro_line|#include &lt;linux/inet.h&gt;
macro_line|#include &lt;linux/netdevice.h&gt;
macro_line|#include &lt;net/pkt_sched.h&gt;
macro_line|#include &lt;net/ip.h&gt;
macro_line|#include &lt;net/protocol.h&gt;
macro_line|#include &lt;net/arp.h&gt;
macro_line|#include &lt;net/route.h&gt;
macro_line|#include &lt;net/tcp.h&gt;
macro_line|#include &lt;net/udp.h&gt;
macro_line|#include &lt;net/sock.h&gt;
DECL|variable|rtnl_rlockct
id|atomic_t
id|rtnl_rlockct
suffix:semicolon
DECL|variable|rtnl_wait
r_struct
id|wait_queue
op_star
id|rtnl_wait
suffix:semicolon
DECL|function|rtnl_lock
r_void
id|rtnl_lock
c_func
(paren
)paren
(brace
id|rtnl_shlock
c_func
(paren
)paren
suffix:semicolon
id|rtnl_exlock
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|rtnl_unlock
r_void
id|rtnl_unlock
c_func
(paren
)paren
(brace
id|rtnl_exunlock
c_func
(paren
)paren
suffix:semicolon
id|rtnl_shunlock
c_func
(paren
)paren
suffix:semicolon
)brace
macro_line|#ifdef CONFIG_RTNETLINK
DECL|variable|rtnl
r_struct
id|sock
op_star
id|rtnl
suffix:semicolon
DECL|variable|rtnl_wlockct
r_int
r_int
id|rtnl_wlockct
suffix:semicolon
DECL|variable|rtnetlink_links
r_struct
id|rtnetlink_link
op_star
id|rtnetlink_links
(braket
id|NPROTO
)braket
suffix:semicolon
DECL|macro|_S
mdefine_line|#define _S&t;1&t;/* superuser privileges required */
DECL|macro|_X
mdefine_line|#define _X&t;2&t;/* exclusive access to tables required */
DECL|macro|_G
mdefine_line|#define _G&t;4&t;/* GET request */
DECL|variable|rtm_properties
r_static
r_int
r_char
id|rtm_properties
(braket
id|RTM_MAX
op_minus
id|RTM_BASE
op_plus
l_int|1
)braket
op_assign
(brace
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_NEWLINK */
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_DELLINK */
id|_G
comma
multiline_comment|/* RTM_GETLINK */
l_int|0
comma
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_NEWADDR */
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_DELADDR */
id|_G
comma
multiline_comment|/* RTM_GETADDR */
l_int|0
comma
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_NEWROUTE */
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_DELROUTE */
id|_G
comma
multiline_comment|/* RTM_GETROUTE */
l_int|0
comma
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_NEWNEIGH */
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_DELNEIGH */
id|_G
comma
multiline_comment|/* RTM_GETNEIGH */
l_int|0
comma
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_NEWRULE */
id|_S
op_or
id|_X
comma
multiline_comment|/* RTM_DELRULE */
id|_G
comma
multiline_comment|/* RTM_GETRULE */
l_int|0
)brace
suffix:semicolon
DECL|function|rtnetlink_get_rta
r_static
r_int
id|rtnetlink_get_rta
c_func
(paren
r_struct
id|kern_rta
op_star
id|rta
comma
r_struct
id|rtattr
op_star
id|attr
comma
r_int
id|attrlen
)paren
(brace
r_void
op_star
op_star
id|rta_data
op_assign
(paren
r_void
op_star
op_star
)paren
id|rta
suffix:semicolon
r_while
c_loop
(paren
id|RTA_OK
c_func
(paren
id|attr
comma
id|attrlen
)paren
)paren
(brace
r_int
id|type
op_assign
id|attr-&gt;rta_type
suffix:semicolon
r_if
c_cond
(paren
id|type
op_ne
id|RTA_UNSPEC
)paren
(brace
r_if
c_cond
(paren
id|type
OG
id|RTA_MAX
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|rta_data
(braket
id|type
op_minus
l_int|1
)braket
op_assign
id|RTA_DATA
c_func
(paren
id|attr
)paren
suffix:semicolon
)brace
id|attr
op_assign
id|RTA_NEXT
c_func
(paren
id|attr
comma
id|attrlen
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|rtnetlink_get_ifa
r_static
r_int
id|rtnetlink_get_ifa
c_func
(paren
r_struct
id|kern_ifa
op_star
id|ifa
comma
r_struct
id|rtattr
op_star
id|attr
comma
r_int
id|attrlen
)paren
(brace
r_void
op_star
op_star
id|ifa_data
op_assign
(paren
r_void
op_star
op_star
)paren
id|ifa
suffix:semicolon
r_while
c_loop
(paren
id|RTA_OK
c_func
(paren
id|attr
comma
id|attrlen
)paren
)paren
(brace
r_int
id|type
op_assign
id|attr-&gt;rta_type
suffix:semicolon
r_if
c_cond
(paren
id|type
op_ne
id|IFA_UNSPEC
)paren
(brace
r_if
c_cond
(paren
id|type
OG
id|IFA_MAX
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|ifa_data
(braket
id|type
op_minus
l_int|1
)braket
op_assign
id|RTA_DATA
c_func
(paren
id|attr
)paren
suffix:semicolon
)brace
id|attr
op_assign
id|RTA_NEXT
c_func
(paren
id|attr
comma
id|attrlen
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|rtnetlink_get_ga
r_static
r_int
id|rtnetlink_get_ga
c_func
(paren
r_struct
id|rtattr
op_star
op_star
id|rta
comma
r_int
id|sz
comma
r_struct
id|rtattr
op_star
id|attr
comma
r_int
id|attrlen
)paren
(brace
r_while
c_loop
(paren
id|RTA_OK
c_func
(paren
id|attr
comma
id|attrlen
)paren
)paren
(brace
r_int
id|type
op_assign
id|attr-&gt;rta_type
suffix:semicolon
r_if
c_cond
(paren
id|type
OG
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|type
OG
id|sz
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|rta
(braket
id|type
op_minus
l_int|1
)braket
op_assign
id|attr
suffix:semicolon
)brace
id|attr
op_assign
id|RTA_NEXT
c_func
(paren
id|attr
comma
id|attrlen
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|__rta_fill
r_void
id|__rta_fill
c_func
(paren
r_struct
id|sk_buff
op_star
id|skb
comma
r_int
id|attrtype
comma
r_int
id|attrlen
comma
r_const
r_void
op_star
id|data
)paren
(brace
r_struct
id|rtattr
op_star
id|rta
suffix:semicolon
r_int
id|size
op_assign
id|RTA_LENGTH
c_func
(paren
id|attrlen
)paren
suffix:semicolon
id|rta
op_assign
(paren
r_struct
id|rtattr
op_star
)paren
id|skb_put
c_func
(paren
id|skb
comma
id|RTA_ALIGN
c_func
(paren
id|size
)paren
)paren
suffix:semicolon
id|rta-&gt;rta_type
op_assign
id|attrtype
suffix:semicolon
id|rta-&gt;rta_len
op_assign
id|size
suffix:semicolon
id|memcpy
c_func
(paren
id|RTA_DATA
c_func
(paren
id|rta
)paren
comma
id|data
comma
id|attrlen
)paren
suffix:semicolon
)brace
DECL|function|rtnetlink_fill_ifinfo
r_static
r_int
id|rtnetlink_fill_ifinfo
c_func
(paren
r_struct
id|sk_buff
op_star
id|skb
comma
r_struct
id|device
op_star
id|dev
comma
r_int
id|type
comma
id|pid_t
id|pid
comma
id|u32
id|seq
)paren
(brace
r_struct
id|ifinfomsg
op_star
id|r
suffix:semicolon
r_struct
id|nlmsghdr
op_star
id|nlh
suffix:semicolon
r_int
r_char
op_star
id|b
op_assign
id|skb-&gt;tail
suffix:semicolon
id|nlh
op_assign
id|NLMSG_PUT
c_func
(paren
id|skb
comma
id|pid
comma
id|seq
comma
id|type
comma
r_sizeof
(paren
op_star
id|r
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pid
)paren
id|nlh-&gt;nlmsg_flags
op_or_assign
id|NLM_F_MULTI
suffix:semicolon
id|r
op_assign
id|NLMSG_DATA
c_func
(paren
id|nlh
)paren
suffix:semicolon
id|r-&gt;ifi_addrlen
op_assign
id|dev-&gt;addr_len
suffix:semicolon
id|r-&gt;ifi_address.sa_family
op_assign
id|dev-&gt;type
suffix:semicolon
id|memcpy
c_func
(paren
op_amp
id|r-&gt;ifi_address.sa_data
comma
id|dev-&gt;dev_addr
comma
id|dev-&gt;addr_len
)paren
suffix:semicolon
id|r-&gt;ifi_broadcast.sa_family
op_assign
id|dev-&gt;type
suffix:semicolon
id|memcpy
c_func
(paren
op_amp
id|r-&gt;ifi_broadcast.sa_data
comma
id|dev-&gt;broadcast
comma
id|dev-&gt;addr_len
)paren
suffix:semicolon
id|r-&gt;ifi_flags
op_assign
id|dev-&gt;flags
suffix:semicolon
id|r-&gt;ifi_mtu
op_assign
id|dev-&gt;mtu
suffix:semicolon
id|r-&gt;ifi_index
op_assign
id|dev-&gt;ifindex
suffix:semicolon
id|r-&gt;ifi_link
op_assign
id|dev-&gt;iflink
suffix:semicolon
id|strncpy
c_func
(paren
id|r-&gt;ifi_name
comma
id|dev-&gt;name
comma
id|IFNAMSIZ
op_minus
l_int|1
)paren
suffix:semicolon
id|r-&gt;ifi_qdiscname
(braket
l_int|0
)braket
op_assign
l_int|0
suffix:semicolon
id|r-&gt;ifi_qdisc
op_assign
id|dev-&gt;qdisc_sleeping-&gt;handle
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;qdisc_sleeping-&gt;ops
)paren
id|strcpy
c_func
(paren
id|r-&gt;ifi_qdiscname
comma
id|dev-&gt;qdisc_sleeping-&gt;ops-&gt;id
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;get_stats
)paren
(brace
r_struct
id|net_device_stats
op_star
id|stats
op_assign
id|dev
op_member_access_from_pointer
id|get_stats
c_func
(paren
id|dev
)paren
suffix:semicolon
r_if
c_cond
(paren
id|stats
)paren
id|RTA_PUT
c_func
(paren
id|skb
comma
id|IFLA_STATS
comma
r_sizeof
(paren
op_star
id|stats
)paren
comma
id|stats
)paren
suffix:semicolon
)brace
id|nlh-&gt;nlmsg_len
op_assign
id|skb-&gt;tail
op_minus
id|b
suffix:semicolon
r_return
id|skb-&gt;len
suffix:semicolon
id|nlmsg_failure
suffix:colon
id|rtattr_failure
suffix:colon
id|skb_put
c_func
(paren
id|skb
comma
id|b
op_minus
id|skb-&gt;tail
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
DECL|function|rtnetlink_dump_ifinfo
r_int
id|rtnetlink_dump_ifinfo
c_func
(paren
r_struct
id|sk_buff
op_star
id|skb
comma
r_struct
id|netlink_callback
op_star
id|cb
)paren
(brace
r_int
id|idx
suffix:semicolon
r_int
id|s_idx
op_assign
id|cb-&gt;args
(braket
l_int|0
)braket
suffix:semicolon
r_struct
id|device
op_star
id|dev
suffix:semicolon
r_for
c_loop
(paren
id|dev
op_assign
id|dev_base
comma
id|idx
op_assign
l_int|0
suffix:semicolon
id|dev
suffix:semicolon
id|dev
op_assign
id|dev-&gt;next
comma
id|idx
op_increment
)paren
(brace
r_if
c_cond
(paren
id|idx
OL
id|s_idx
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|rtnetlink_fill_ifinfo
c_func
(paren
id|skb
comma
id|dev
comma
id|RTM_NEWLINK
comma
id|NETLINK_CB
c_func
(paren
id|cb-&gt;skb
)paren
dot
id|pid
comma
id|cb-&gt;nlh-&gt;nlmsg_seq
)paren
op_le
l_int|0
)paren
r_break
suffix:semicolon
)brace
id|cb-&gt;args
(braket
l_int|0
)braket
op_assign
id|idx
suffix:semicolon
r_return
id|skb-&gt;len
suffix:semicolon
)brace
DECL|function|rtnetlink_dump_all
r_int
id|rtnetlink_dump_all
c_func
(paren
r_struct
id|sk_buff
op_star
id|skb
comma
r_struct
id|netlink_callback
op_star
id|cb
)paren
(brace
r_int
id|idx
suffix:semicolon
r_int
id|s_idx
op_assign
id|cb-&gt;family
suffix:semicolon
r_if
c_cond
(paren
id|s_idx
op_eq
l_int|0
)paren
id|s_idx
op_assign
l_int|1
suffix:semicolon
r_for
c_loop
(paren
id|idx
op_assign
l_int|1
suffix:semicolon
id|idx
OL
id|NPROTO
suffix:semicolon
id|idx
op_increment
)paren
(brace
r_int
id|type
op_assign
id|cb-&gt;nlh-&gt;nlmsg_type
op_minus
id|RTM_BASE
suffix:semicolon
r_if
c_cond
(paren
id|idx
OL
id|s_idx
op_logical_or
id|idx
op_eq
id|AF_PACKET
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|rtnetlink_links
(braket
id|idx
)braket
op_eq
l_int|NULL
op_logical_or
id|rtnetlink_links
(braket
id|idx
)braket
(braket
id|type
)braket
dot
id|dumpit
op_eq
l_int|NULL
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|idx
OG
id|s_idx
)paren
id|memset
c_func
(paren
op_amp
id|cb-&gt;args
(braket
l_int|0
)braket
comma
l_int|0
comma
r_sizeof
(paren
id|cb-&gt;args
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|rtnetlink_links
(braket
id|idx
)braket
(braket
id|type
)braket
dot
id|dumpit
c_func
(paren
id|skb
comma
id|cb
)paren
op_eq
l_int|0
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|skb_tailroom
c_func
(paren
id|skb
)paren
OL
l_int|256
)paren
r_break
suffix:semicolon
)brace
id|cb-&gt;family
op_assign
id|idx
suffix:semicolon
r_return
id|skb-&gt;len
suffix:semicolon
)brace
DECL|function|rtmsg_ifinfo
r_void
id|rtmsg_ifinfo
c_func
(paren
r_int
id|type
comma
r_struct
id|device
op_star
id|dev
)paren
(brace
r_struct
id|sk_buff
op_star
id|skb
suffix:semicolon
r_int
id|size
op_assign
id|NLMSG_SPACE
c_func
(paren
r_sizeof
(paren
r_struct
id|ifinfomsg
)paren
op_plus
id|RTA_LENGTH
c_func
(paren
r_sizeof
(paren
r_struct
id|net_device_stats
)paren
)paren
)paren
suffix:semicolon
id|skb
op_assign
id|alloc_skb
c_func
(paren
id|size
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|skb
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|rtnetlink_fill_ifinfo
c_func
(paren
id|skb
comma
id|dev
comma
id|type
comma
l_int|0
comma
l_int|0
)paren
OL
l_int|0
)paren
(brace
id|kfree_skb
c_func
(paren
id|skb
comma
l_int|0
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
id|NETLINK_CB
c_func
(paren
id|skb
)paren
dot
id|dst_groups
op_assign
id|RTMGRP_LINK
suffix:semicolon
id|netlink_broadcast
c_func
(paren
id|rtnl
comma
id|skb
comma
l_int|0
comma
id|RTMGRP_LINK
comma
id|GFP_KERNEL
)paren
suffix:semicolon
)brace
DECL|function|rtnetlink_done
r_static
r_int
id|rtnetlink_done
c_func
(paren
r_struct
id|netlink_callback
op_star
id|cb
)paren
(brace
r_if
c_cond
(paren
id|NETLINK_CREDS
c_func
(paren
id|cb-&gt;skb
)paren
op_member_access_from_pointer
id|uid
op_eq
l_int|0
op_logical_and
id|cb-&gt;nlh-&gt;nlmsg_flags
op_amp
id|NLM_F_ATOMIC
)paren
id|rtnl_shunlock
c_func
(paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Process one rtnetlink message. */
r_extern
id|__inline__
r_int
DECL|function|rtnetlink_rcv_msg
id|rtnetlink_rcv_msg
c_func
(paren
r_struct
id|sk_buff
op_star
id|skb
comma
r_struct
id|nlmsghdr
op_star
id|nlh
comma
r_int
op_star
id|errp
)paren
(brace
r_union
(brace
r_struct
id|kern_rta
id|rta
suffix:semicolon
r_struct
id|kern_ifa
id|ifa
suffix:semicolon
r_struct
id|rtattr
op_star
id|ga
(braket
id|RTA_MAX
op_minus
l_int|1
)braket
suffix:semicolon
)brace
id|u
suffix:semicolon
r_struct
id|rtmsg
op_star
id|rtm
suffix:semicolon
r_struct
id|ifaddrmsg
op_star
id|ifm
suffix:semicolon
r_struct
id|ndmsg
op_star
id|ndm
suffix:semicolon
r_int
id|exclusive
op_assign
l_int|0
suffix:semicolon
r_int
id|family
suffix:semicolon
r_int
id|type
suffix:semicolon
r_int
id|err
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|nlh-&gt;nlmsg_flags
op_amp
id|NLM_F_REQUEST
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|type
op_assign
id|nlh-&gt;nlmsg_type
suffix:semicolon
r_if
c_cond
(paren
id|type
OL
id|RTM_BASE
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|type
OG
id|RTM_MAX
)paren
r_goto
id|err_inval
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OL
id|NLMSG_LENGTH
c_func
(paren
r_sizeof
(paren
r_struct
id|rtgenmsg
)paren
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|family
op_assign
(paren
(paren
r_struct
id|rtgenmsg
op_star
)paren
id|NLMSG_DATA
c_func
(paren
id|nlh
)paren
)paren
op_member_access_from_pointer
id|rtgen_family
suffix:semicolon
r_if
c_cond
(paren
id|family
OG
id|NPROTO
op_logical_or
id|rtnetlink_links
(braket
id|family
)braket
op_eq
l_int|NULL
)paren
(brace
op_star
id|errp
op_assign
op_minus
id|EAFNOSUPPORT
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|rtm_properties
(braket
id|type
op_minus
id|RTM_BASE
)braket
op_amp
id|_S
)paren
(brace
r_if
c_cond
(paren
id|NETLINK_CREDS
c_func
(paren
id|skb
)paren
op_member_access_from_pointer
id|uid
)paren
(brace
op_star
id|errp
op_assign
op_minus
id|EPERM
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|rtm_properties
(braket
id|type
op_minus
id|RTM_BASE
)braket
op_amp
id|_G
op_logical_and
id|nlh-&gt;nlmsg_flags
op_amp
id|NLM_F_DUMP
)paren
(brace
r_if
c_cond
(paren
id|rtnetlink_links
(braket
id|family
)braket
(braket
id|type
op_minus
id|RTM_BASE
)braket
dot
id|dumpit
op_eq
l_int|NULL
)paren
r_goto
id|err_inval
suffix:semicolon
multiline_comment|/* Super-user locks all the tables to get atomic snapshot */
r_if
c_cond
(paren
id|NETLINK_CREDS
c_func
(paren
id|skb
)paren
op_member_access_from_pointer
id|uid
op_eq
l_int|0
op_logical_and
id|nlh-&gt;nlmsg_flags
op_amp
id|NLM_F_ATOMIC
)paren
id|atomic_inc
c_func
(paren
op_amp
id|rtnl_rlockct
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
op_star
id|errp
op_assign
id|netlink_dump_start
c_func
(paren
id|rtnl
comma
id|skb
comma
id|nlh
comma
id|rtnetlink_links
(braket
id|family
)braket
(braket
id|type
op_minus
id|RTM_BASE
)braket
dot
id|dumpit
comma
id|rtnetlink_done
)paren
)paren
op_ne
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|NETLINK_CREDS
c_func
(paren
id|skb
)paren
op_member_access_from_pointer
id|uid
op_eq
l_int|0
op_logical_and
id|nlh-&gt;nlmsg_flags
op_amp
id|NLM_F_ATOMIC
)paren
id|atomic_dec
c_func
(paren
op_amp
id|rtnl_rlockct
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
id|skb_pull
c_func
(paren
id|skb
comma
id|NLMSG_ALIGN
c_func
(paren
id|nlh-&gt;nlmsg_len
)paren
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|rtm_properties
(braket
id|type
op_minus
id|RTM_BASE
)braket
op_amp
id|_X
)paren
(brace
r_if
c_cond
(paren
id|rtnl_exlock_nowait
c_func
(paren
)paren
)paren
(brace
op_star
id|errp
op_assign
l_int|0
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
id|exclusive
op_assign
l_int|1
suffix:semicolon
)brace
id|memset
c_func
(paren
op_amp
id|u
comma
l_int|0
comma
r_sizeof
(paren
id|u
)paren
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|nlh-&gt;nlmsg_type
)paren
(brace
r_case
id|RTM_NEWROUTE
suffix:colon
r_case
id|RTM_DELROUTE
suffix:colon
r_case
id|RTM_GETROUTE
suffix:colon
r_case
id|RTM_NEWRULE
suffix:colon
r_case
id|RTM_DELRULE
suffix:colon
r_case
id|RTM_GETRULE
suffix:colon
id|rtm
op_assign
id|NLMSG_DATA
c_func
(paren
id|nlh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OL
r_sizeof
(paren
op_star
id|rtm
)paren
)paren
r_goto
id|err_inval
suffix:semicolon
r_if
c_cond
(paren
id|rtm-&gt;rtm_optlen
op_logical_and
id|rtnetlink_get_rta
c_func
(paren
op_amp
id|u.rta
comma
id|RTM_RTA
c_func
(paren
id|rtm
)paren
comma
id|rtm-&gt;rtm_optlen
)paren
OL
l_int|0
)paren
r_goto
id|err_inval
suffix:semicolon
r_break
suffix:semicolon
r_case
id|RTM_NEWADDR
suffix:colon
r_case
id|RTM_DELADDR
suffix:colon
r_case
id|RTM_GETADDR
suffix:colon
id|ifm
op_assign
id|NLMSG_DATA
c_func
(paren
id|nlh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OL
r_sizeof
(paren
op_star
id|ifm
)paren
)paren
r_goto
id|err_inval
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OG
id|NLMSG_LENGTH
c_func
(paren
r_sizeof
(paren
op_star
id|ifm
)paren
)paren
op_logical_and
id|rtnetlink_get_ifa
c_func
(paren
op_amp
id|u.ifa
comma
id|IFA_RTA
c_func
(paren
id|ifm
)paren
comma
id|nlh-&gt;nlmsg_len
op_minus
id|NLMSG_LENGTH
c_func
(paren
r_sizeof
(paren
op_star
id|ifm
)paren
)paren
)paren
OL
l_int|0
)paren
r_goto
id|err_inval
suffix:semicolon
r_break
suffix:semicolon
r_case
id|RTM_NEWNEIGH
suffix:colon
r_case
id|RTM_DELNEIGH
suffix:colon
r_case
id|RTM_GETNEIGH
suffix:colon
id|ndm
op_assign
id|NLMSG_DATA
c_func
(paren
id|nlh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OL
r_sizeof
(paren
op_star
id|ndm
)paren
)paren
r_goto
id|err_inval
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OG
id|NLMSG_LENGTH
c_func
(paren
r_sizeof
(paren
op_star
id|ndm
)paren
)paren
op_logical_and
id|rtnetlink_get_ga
c_func
(paren
id|u.ga
comma
id|NDA_MAX
comma
id|NDA_RTA
c_func
(paren
id|ndm
)paren
comma
id|nlh-&gt;nlmsg_len
op_minus
id|NLMSG_LENGTH
c_func
(paren
r_sizeof
(paren
op_star
id|ndm
)paren
)paren
)paren
OL
l_int|0
)paren
r_goto
id|err_inval
suffix:semicolon
r_break
suffix:semicolon
r_case
id|RTM_NEWLINK
suffix:colon
r_case
id|RTM_DELLINK
suffix:colon
r_case
id|RTM_GETLINK
suffix:colon
multiline_comment|/* Not urgent and even not necessary */
r_default
suffix:colon
r_goto
id|err_inval
suffix:semicolon
)brace
r_if
c_cond
(paren
id|rtnetlink_links
(braket
id|family
)braket
(braket
id|type
op_minus
id|RTM_BASE
)braket
dot
id|doit
op_eq
l_int|NULL
)paren
r_goto
id|err_inval
suffix:semicolon
id|err
op_assign
id|rtnetlink_links
(braket
id|family
)braket
(braket
id|type
op_minus
id|RTM_BASE
)braket
dot
id|doit
c_func
(paren
id|skb
comma
id|nlh
comma
(paren
r_void
op_star
)paren
op_amp
id|u
)paren
suffix:semicolon
r_if
c_cond
(paren
id|exclusive
)paren
id|rtnl_exunlock
c_func
(paren
)paren
suffix:semicolon
op_star
id|errp
op_assign
id|err
suffix:semicolon
r_return
id|err
suffix:semicolon
id|err_inval
suffix:colon
r_if
c_cond
(paren
id|exclusive
)paren
id|rtnl_exunlock
c_func
(paren
)paren
suffix:semicolon
op_star
id|errp
op_assign
op_minus
id|EINVAL
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
multiline_comment|/* &n; * Process one packet of messages.&n; * Malformed skbs with wrong lengths of messages are discarded silently.&n; */
DECL|function|rtnetlink_rcv_skb
r_extern
id|__inline__
r_int
id|rtnetlink_rcv_skb
c_func
(paren
r_struct
id|sk_buff
op_star
id|skb
)paren
(brace
r_int
id|err
suffix:semicolon
r_struct
id|nlmsghdr
op_star
id|nlh
suffix:semicolon
r_while
c_loop
(paren
id|skb-&gt;len
op_ge
id|NLMSG_SPACE
c_func
(paren
l_int|0
)paren
)paren
(brace
r_int
id|rlen
suffix:semicolon
id|nlh
op_assign
(paren
r_struct
id|nlmsghdr
op_star
)paren
id|skb-&gt;data
suffix:semicolon
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_len
OL
r_sizeof
(paren
op_star
id|nlh
)paren
op_logical_or
id|skb-&gt;len
OL
id|nlh-&gt;nlmsg_len
)paren
r_return
l_int|0
suffix:semicolon
id|rlen
op_assign
id|NLMSG_ALIGN
c_func
(paren
id|nlh-&gt;nlmsg_len
)paren
suffix:semicolon
r_if
c_cond
(paren
id|rlen
OG
id|skb-&gt;len
)paren
id|rlen
op_assign
id|skb-&gt;len
suffix:semicolon
r_if
c_cond
(paren
id|rtnetlink_rcv_msg
c_func
(paren
id|skb
comma
id|nlh
comma
op_amp
id|err
)paren
)paren
(brace
multiline_comment|/* Not error, but we must interrupt processing here:&n;&t;&t;&t; *   Note, that in this case we do not pull message&n;&t;&t;&t; *   from skb, it will be processed later.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|err
op_eq
l_int|0
)paren
r_return
op_minus
l_int|1
suffix:semicolon
id|netlink_ack
c_func
(paren
id|skb
comma
id|nlh
comma
id|err
)paren
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|nlh-&gt;nlmsg_flags
op_amp
id|NLM_F_ACK
)paren
id|netlink_ack
c_func
(paren
id|skb
comma
id|nlh
comma
l_int|0
)paren
suffix:semicolon
id|skb_pull
c_func
(paren
id|skb
comma
id|rlen
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *  rtnetlink input queue processing routine:&n; *&t;- try to acquire shared lock. If it is failed, defer processing.&n; *&t;- feed skbs to rtnetlink_rcv_skb, until it refuse a message,&n; *&t;  that will occur, when a dump started and/or acquisition of&n; *&t;  exclusive lock failed.&n; */
DECL|function|rtnetlink_rcv
r_static
r_void
id|rtnetlink_rcv
c_func
(paren
r_struct
id|sock
op_star
id|sk
comma
r_int
id|len
)paren
(brace
r_struct
id|sk_buff
op_star
id|skb
suffix:semicolon
r_if
c_cond
(paren
id|rtnl_shlock_nowait
c_func
(paren
)paren
)paren
r_return
suffix:semicolon
r_while
c_loop
(paren
(paren
id|skb
op_assign
id|skb_dequeue
c_func
(paren
op_amp
id|sk-&gt;receive_queue
)paren
)paren
op_ne
l_int|NULL
)paren
(brace
r_if
c_cond
(paren
id|rtnetlink_rcv_skb
c_func
(paren
id|skb
)paren
)paren
(brace
r_if
c_cond
(paren
id|skb-&gt;len
)paren
id|skb_queue_head
c_func
(paren
op_amp
id|sk-&gt;receive_queue
comma
id|skb
)paren
suffix:semicolon
r_else
id|kfree_skb
c_func
(paren
id|skb
comma
id|FREE_READ
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
id|kfree_skb
c_func
(paren
id|skb
comma
id|FREE_READ
)paren
suffix:semicolon
)brace
id|rtnl_shunlock
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|variable|link_rtnetlink_table
r_static
r_struct
id|rtnetlink_link
id|link_rtnetlink_table
(braket
id|RTM_MAX
op_minus
id|RTM_BASE
op_plus
l_int|1
)braket
op_assign
(brace
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
id|rtnetlink_dump_ifinfo
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
id|rtnetlink_dump_all
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
id|rtnetlink_dump_all
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
id|neigh_dump_info
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
(brace
l_int|NULL
comma
l_int|NULL
comma
)brace
comma
)brace
suffix:semicolon
DECL|function|rtnetlink_event
r_static
r_int
id|rtnetlink_event
c_func
(paren
r_struct
id|notifier_block
op_star
id|this
comma
r_int
r_int
id|event
comma
r_void
op_star
id|ptr
)paren
(brace
r_struct
id|device
op_star
id|dev
op_assign
id|ptr
suffix:semicolon
r_switch
c_cond
(paren
id|event
)paren
(brace
r_case
id|NETDEV_UNREGISTER
suffix:colon
id|rtmsg_ifinfo
c_func
(paren
id|RTM_DELLINK
comma
id|dev
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|rtmsg_ifinfo
c_func
(paren
id|RTM_NEWLINK
comma
id|dev
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
r_return
id|NOTIFY_DONE
suffix:semicolon
)brace
DECL|variable|rtnetlink_dev_notifier
r_struct
id|notifier_block
id|rtnetlink_dev_notifier
op_assign
(brace
id|rtnetlink_event
comma
l_int|NULL
comma
l_int|0
)brace
suffix:semicolon
DECL|function|__initfunc
id|__initfunc
c_func
(paren
r_void
id|rtnetlink_init
c_func
(paren
r_void
)paren
)paren
(brace
macro_line|#ifdef RTNL_DEBUG
id|printk
c_func
(paren
l_string|&quot;Initializing RT netlink socket&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
id|rtnl
op_assign
id|netlink_kernel_create
c_func
(paren
id|NETLINK_ROUTE
comma
id|rtnetlink_rcv
)paren
suffix:semicolon
r_if
c_cond
(paren
id|rtnl
op_eq
l_int|NULL
)paren
id|panic
c_func
(paren
l_string|&quot;rtnetlink_init: cannot initialize rtnetlink&bslash;n&quot;
)paren
suffix:semicolon
id|register_netdevice_notifier
c_func
(paren
op_amp
id|rtnetlink_dev_notifier
)paren
suffix:semicolon
id|rtnetlink_links
(braket
id|AF_UNSPEC
)braket
op_assign
id|link_rtnetlink_table
suffix:semicolon
id|rtnetlink_links
(braket
id|AF_PACKET
)braket
op_assign
id|link_rtnetlink_table
suffix:semicolon
)brace
macro_line|#endif
eof
