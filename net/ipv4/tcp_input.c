...
multiline_comment|/* Fast parse options. This hopes to only see timestamps.&n; * If it is wrong it falls back on tcp_parse_option().&n; * This should probably get extended for timestamps as well.&n; * Assembly code anyone? -- erics&n; */
...
suffix:semicolon
multiline_comment|/* FIXME: This assumes that when we are retransmitting&n;&t; * we should only ever respond with one packet.&n;&t; * This means congestion windows should not grow&n;&t; * during recovery. In 2.0.X we allow the congestion&n;&t; * window to grow. It is not clear to me which&n;&t; * decision is correct. The RFCs should be double&n;&t; * checked as should the behavior of other stacks.&n;&t; * Also note that if we do want to allow the&n;&t; * congestion window to grow during retransmits&n;&t; * we have to fix the call to congestion window&n;&t; * updates so that it works during retransmission.&n;&t; */
r_if
c_cond
(paren
id|tp-&gt;retransmits
...
l_int|NULL
suffix:semicolon
multiline_comment|/* This is tricky. We are retransmiting a &n;&t;&t; * segment of a window when congestion occured.&n;&t;&t; */
id|tcp_do_retransmit
c_func
(paren
id|sk
...
id|__arg
suffix:semicolon
multiline_comment|/* Zap the timer. */
id|del_timer
c_func
(paren
op_amp
...
(brace
multiline_comment|/* Never toss anything when we&squot;ve seen the FIN.&n;&t;&t; * It&squot;s just too complex to recover from it.&n;&t;&t; */
r_if
c_cond
(paren
id|skb-&gt;h.th-&gt;fin
...
id|sk
comma
l_string|&quot;prune_queue: removing %x-%x (c=%x)&bslash;n&quot;
comma
id|skb-&gt;seq
comma
id|skb-&gt;end_seq
