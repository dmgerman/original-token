...
multiline_comment|/* When incoming ACK allowed to free some skb from write_queue,&n; * we remember this in flag tp-&gt;sorry and wake up socket on the exit&n; * from tcp input handler. Probably, handler has already eat this space&n; * sending ACK and cloned frames from tcp_write_xmit().&n; */
...
multiline_comment|/* Avoid to send immediate ACK from input path, if it&n;&t;      * does not advance window far enough. tcp_recvmsg() will do this.&n;&t;      */
op_logical_and
(paren
op_logical_neg
