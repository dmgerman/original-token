...
mdefine_line|#define PAWS_24DAYS&t;(60 * 60 * 24 * 24)
...
multiline_comment|/* When recycling, always follow rfc1337,&n;&t;&t;&t; * but mark bucket as ready to recycling immediately.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|sysctl_tcp_tw_recycle
...
(brace
multiline_comment|/* May kill it now. */
id|tw-&gt;rto
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/* In this case we must reset the TIMEWAIT timer.&n;&n;&t;&t;   If it is ACKless SYN it may be both old duplicate&n;&t;&t;   and new good SYN with random sequence number &lt;rcv_nxt.&n;&t;&t;   Do not reschedule in the last case.&n;&t;&t; */
r_if
c_cond
(paren
id|paws_reject
...
suffix:semicolon
multiline_comment|/* Step 4: Un-charge protocol socket in-use count. */
id|sk-&gt;prot-&gt;inuse
op_decrement
suffix:semicolon
)brace
multiline_comment|/* &n; * Move a socket to time-wait.&n; */
...
id|tw-&gt;snd_nxt
op_assign
id|sk-&gt;tp_pinfo.af_tcp.snd_nxt
suffix:semicolon
id|tw-&gt;ts_recent
op_assign
id|sk-&gt;tp_pinfo.af_tcp.ts_recent
suffix:semicolon
id|tw-&gt;ts_recent_stamp
op_assign
id|sk-&gt;tp_pinfo.af_tcp.ts_recent_stamp
suffix:semicolon
macro_line|#ifdef CONFIG_TCP_TW_RECYCLE
id|tw-&gt;rto
op_assign
id|sk-&gt;tp_pinfo.af_tcp.rto
suffix:semicolon
id|tw-&gt;ttd
op_assign
id|jiffies
...
l_int|4
suffix:semicolon
multiline_comment|/* Timestamp header prediction */
multiline_comment|/* Non-standard header f.e. SACKs -&gt; slow path */
r_if
c_cond
(paren
id|tcp_header_len
...
suffix:semicolon
multiline_comment|/* Is it possible to simplify this? */
id|tcp_measure_rcv_mss
c_func
(paren
id|sk
...
suffix:semicolon
multiline_comment|/* DO NOT notify forward progress here.&n;&t;&t;&t; * It saves dozen of CPU instructions in fast path. --ANK&n;&t;&t;&t; * And where is it signaled then ? -AK&n;&t;&t;&t; */
id|__skb_queue_tail
c_func
(paren
op_amp
...
multiline_comment|/* Packet is in sequence, flags are trivial;&n;&t;&t; * only ACK is strange or we are tough on memory.&n;&t;&t; * Jump to step 5.&n;&t;&t; */
r_goto
id|step5
suffix:semicolon
)brace
...
id|ttp
suffix:semicolon
multiline_comment|/* If socket has already been created, process&n;&t;   packet in its context.&n;&n;&t;   We fall here only due to race, when packets were enqueued&n;&t;   to backlog of listening socket.&n;&t; */
r_if
c_cond
(paren
id|req-&gt;sk
...
(brace
multiline_comment|/* Old duplicate segment. We remember last&n;&t;&t;   ts_recent from this host in timewait bucket.&n;&n;&t;&t;   Actually, we could implement per host cache&n;&t;&t;   to truncate timewait state after RTO. Paranoidal arguments&n;&t;&t;   of rfc1337 are not enough to close this nice possibility.&n;&t;&t;*/
r_if
c_cond
(paren
id|net_ratelimit
...
(paren
id|KERN_DEBUG
l_string|&quot;TCP: tw recycle, PAWS worked. Good.&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
(brace
multiline_comment|/* Save one ACK. Data will be ready after&n;&t;&t;&t; * several ticks, if write_pending is set.&n;&t;&t;&t; *&n;&t;&t;&t; * How to make this correctly?&n;&t;&t;&t; */
id|tp-&gt;delayed_acks
op_increment
suffix:semicolon
r_if
...
id|ack_seq
suffix:semicolon
multiline_comment|/* tcp_ack considers this ACK as duplicate&n;&t;&t;&t;&t; * and does not calculate rtt. It is wrong.&n;&t;&t;&t;&t; * Fix it at least with timestamps.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|tp-&gt;saw_tstamp
