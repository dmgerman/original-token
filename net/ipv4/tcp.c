...
multiline_comment|/*&n; *&t;Socket write_space callback.&n; *&t;This (or rather the sock_wake_async) should agree with poll.&n; *&n; *&t;WARNING. This callback is called from any context (process,&n; *&t;bh or irq). Do not make anything more smart from it.&n; */
...
(brace
multiline_comment|/* Why??!! Does it really not overshedule? --ANK */
...
multiline_comment|/* Now socket state including sk-&gt;err is changed only under lock,&n;   hence we should check only pending signals.&n; */
...
multiline_comment|/*&n; *&t;This routine closes sockets which have been at least partially&n; *&t;opened, but not yet accepted. Currently it is only called by&n; *&t;tcp_close.&n; */
...
suffix:semicolon
multiline_comment|/* Announce socket dead, detach it from wait queue and inode. */
id|write_lock_irq
c_func
(paren
op_amp
