...
multiline_comment|/*&n; *&t;Walk down the receive queue counting readable data.&n; *&n; *&t;Must be called with the socket lock held.&n; */
...
id|counted
op_assign
id|sk-&gt;tp_pinfo.af_tcp.copied_seq
suffix:semicolon
...
l_int|2048
suffix:semicolon
macro_line|#else /* 2.0 way */
multiline_comment|/* More than half of the socket queue free? */
id|space
op_assign
id|atomic_read
c_func
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* Always wake the user up when an error occurred */
r_if
c_cond
(paren
id|sock_wspace
...
multiline_comment|/*&n; *&t;This routine copies from a user buffer into a socket,&n; *&t;and starts the transmit system.&n; *&n; *&t;Note: must be called with the socket locked.&n; */
DECL|function|tcp_do_sendmsg
r_int
id|tcp_do_sendmsg
c_func
(paren
...
l_int|0
suffix:semicolon
multiline_comment|/* Verify that the socket is locked */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;tcp_do_sendmsg: socket not locked!&bslash;n&quot;
)paren
suffix:semicolon
...
id|skb-&gt;len
suffix:semicolon
multiline_comment|/* If the remote does SWS avoidance we should&n;&t;&t;&t;&t; * queue the best we can if not we should in &n;&t;&t;&t;&t; * fact send multiple packets...&n;&t;&t;&t;&t; * A method for detecting this would be most&n;&t;&t;&t;&t; * welcome.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|skb_tailroom
...
multiline_comment|/* We also need to worry about the window.  If&n;&t;&t;&t; * window &lt; 1/2 the maximum window we&squot;ve seen&n;&t;&t;&t; * from this host, don&squot;t use it.  This is&n;&t;&t;&t; * sender side silly window prevention, as&n;&t;&t;&t; * specified in RFC1122.  (Note that this is&n;&t;&t;&t; * different than earlier versions of SWS&n;&t;&t;&t; * prevention, e.g. RFC813.).  What we&n;&t;&t;&t; * actually do is use the whole MSS.  Since&n;&t;&t;&t; * the results in the right edge of the packet&n;&t;&t;&t; * being outside the window, it will be queued&n;&t;&t;&t; * for later rather than sent.&n;&t;&t;&t; */
id|copy
op_assign
id|tp-&gt;snd_wnd
op_minus
...
suffix:semicolon
multiline_comment|/* What is happening here is that we want to&n;&t;&t;&t;&t; * tack on later members of the users iovec&n;&t;&t;&t;&t; * if possible into a single frame.  When we&n;&t;&t;&t;&t; * leave this loop our caller checks to see if&n;&t;&t;&t;&t; * we can send queued frames onto the wire.&n;&t;&t;&t;&t; * See tcp_v[46]_sendmsg() for this.&n;&t;&t;&t;&t; */
id|queue_it
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/*&n; *&t;Send an ack if one is backlogged at this point. Ought to merge&n; *&t;this with tcp_send_ack().&n; *      This is called for delayed acks also.&n; */
...
suffix:semicolon
multiline_comment|/* N.B. already set above ... */
id|msg-&gt;msg_flags
op_or_assign
id|MSG_OOB
suffix:semicolon
...
id|MSG_TRUNC
suffix:semicolon
multiline_comment|/* N.B. Is this right?? If len == 0 we didn&squot;t read any data */
r_return
id|err
ques
c_cond
...
id|sk
comma
l_string|&quot;sk-&gt;rspace = %lu&bslash;n&quot;
comma
id|sock_rspace
c_func
(paren
...
suffix:semicolon
multiline_comment|/* We send an ACK if we can now advertise a non-zero window&n;&t; * which has been raised &quot;significantly&quot;.&n;  &t; */
r_if
c_cond
(paren
id|copied
...
suffix:semicolon
multiline_comment|/* We won&squot;t be raising the window any further than&n;&t;&t; * the window-clamp allows.  Our window selection&n;&t;&t; * also keeps things a nice multiple of MSS.  These&n;&t;&t; * checks are necessary to prevent spurious ACKs&n;&t;&t; * which don&squot;t advertize a larger window.&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/*&n;&t; *&t;BUG BUG BUG&n;&t; *&t;This violates 1003.1g compliance. We must wait for &n;&t; *&t;data to exist even if we read none!&n;&t; */
r_while
c_loop
(paren
id|len
...
multiline_comment|/*&n; * Check whether to renew the timer.&n; */
DECL|function|tcp_check_fin_timer
r_static
r_inline
r_void
id|tcp_check_fin_timer
c_func
(paren
r_struct
id|sock
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * Check whether the socket is locked ... supposedly&n;&t; * it&squot;s impossible to tcp_close() a locked socket.&n;&t; */
r_if
c_cond
(paren
id|sk-&gt;sock_readers
...
c_func
(paren
l_string|&quot;tcp_close: socket already locked!&bslash;n&quot;
)paren
suffix:semicolon
...
multiline_comment|/* It is questionable, what the role of this is now.&n;&t; * In any event either it should be removed, or&n;&t; * increment of SLT_KEEPALIVE be done, this is causing&n;&t; * big problems.  For now I comment it out.  -DaveM&n;&t; */
multiline_comment|/* sk-&gt;keepopen = 1; */
id|sk-&gt;shutdown
op_assign
id|SHUTDOWN_MASK
suffix:semicolon
...
op_decrement
suffix:semicolon
multiline_comment|/* FIXME: need to check here if newsk has already&n;&t; * an soft_err or err set.&n;&t; * We have two options here then: reply (this behaviour matches&n;&t; * Solaris) or return the error to the application (old Linux)&n;&t; */
id|error
op_assign
l_int|0
suffix:semicolon
