...
id|last_primary
suffix:semicolon
multiline_comment|/* Allow 0.0.0.0, but it must be the only address to avoid&n;           multiple matches. */
r_if
c_cond
(paren
id|in_dev-&gt;ifa_list
...
macro_line|#ifdef CONFIG_RTNETLINK
multiline_comment|/* rtm_{add|del} functions are not reenterable, so that&n;   this structure can be made static&n; */
r_int
...
multiline_comment|/*&n; *&t;Addition and deletion of fake interface addresses&n; *&t;for sending of BOOTP packets. In this case, we must&n; *&t;set the local address to zero which is not permitted&n; *&t;otherwise.&n; */
