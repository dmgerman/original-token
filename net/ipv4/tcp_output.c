...
suffix:semicolon
multiline_comment|/*&n;&t;&t; *&t;See if we really need to send the whole packet. &n;&t;&t; */
r_if
c_cond
(paren
id|before
...
(brace
multiline_comment|/*&n;&t;&t;&t; *&t;This is acked data. We can discard it.&n;&t;&t;&t; *&t;This implies the packet was sent out&n;&t;&t;&t; *&t;of the write queue by a zero window probe.&n;&t;&t;&t; */
id|sk-&gt;retransmits
op_assign
l_int|0
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* See if we need to shrink the leading packet on&n;                         * the retransmit queue. Strictly speaking, we&n;                         * should never need to do this, but some buggy TCP&n;                         * implementations get confused if you send them&n;                         * a packet that contains both old and new data. (Feh!)&n;                         * Soooo, we have this uglyness here.&n;                         */
r_if
c_cond
(paren
id|after
...
multiline_comment|/*&n; * Remove the portion of a packet that has already been sent.&n; * Needed to deal with buggy TCP implementations that can&squot;t deal&n; * with seeing a packet that contains some data that has already&n; * been received.&n; */
DECL|function|tcp_shrink_skb
r_void
id|tcp_shrink_skb
c_func
(paren
...
id|diff
suffix:semicolon
multiline_comment|/*&n;&t; *&t;Recover the buffer pointers&n;&t; */
id|iph
op_assign
(paren
r_struct
...
suffix:semicolon
multiline_comment|/* how much data are we droping from the tcp frame */
id|diff
op_assign
id|ack
op_minus
id|skb-&gt;seq
suffix:semicolon
multiline_comment|/* how much data are we keeping in the tcp frame */
id|len
op_assign
(paren
id|skb-&gt;end_seq
...
id|ack
suffix:semicolon
multiline_comment|/* pointers to new start of remaining data, and old start */
r_new
op_assign
(paren
r_int
...
id|diff
suffix:semicolon
multiline_comment|/* Update our starting seq number */
id|skb-&gt;seq
op_assign
id|ack
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Get the partial checksum for the IP options */
r_if
c_cond
(paren
id|th-&gt;doff
...
l_int|0
suffix:semicolon
multiline_comment|/* Copy the good data down and get it&squot;s checksum */
id|skb-&gt;csum
op_assign
id|csum_partial_copy
c_func
...
suffix:semicolon
multiline_comment|/* shorten the skb */
id|skb_trim
c_func
(paren
id|skb
...
suffix:semicolon
multiline_comment|/* Checksum the shrunk buffer */
id|tcp_send_check
c_func
(paren
id|th
