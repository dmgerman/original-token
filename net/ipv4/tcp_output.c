...
multiline_comment|/* Do a simple retransmit without using the backoff mechanisms in&n; * tcp_timer. This is used to speed up path mtu recovery. Note that&n; * these simple retransmits aren&squot;t counted in the usual tcp retransmit&n; * backoff counters. &n; * The socket is already locked here.&n; */
...
id|sk-&gt;mss
suffix:semicolon
multiline_comment|/* Account for outgoing SACKS and IP options, if any. */
r_if
c_cond
(paren
id|tp-&gt;sack_ok
...
multiline_comment|/* This gets called after a retransmit timeout, and the initially&n; * retransmitted data is acknowledged.  It tries to continue&n; * resending the rest of the retransmit queue, until either&n; * we&squot;ve sent it all or the congestion window limit is reached.&n; */
...
multiline_comment|/* Count retransmissions locally. */
id|ct
op_increment
suffix:semicolon
