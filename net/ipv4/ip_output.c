...
multiline_comment|/* Now we have no better mechanism to notify about error. */
r_switch
c_cond
(paren
...
multiline_comment|/* We have a route, so grab a reference. */
id|skb-&gt;dst
op_assign
id|dst_clone
c_func
...
multiline_comment|/* Do we need to fragment.  Again this is inefficient.  We&n;&t; * need to somehow lock the original buffer and use bits of it.&n;&t; */
r_if
c_cond
(paren
id|tot_len
...
suffix:semicolon
multiline_comment|/* icmp_send is not reenterable, so that bh_atomic... --ANK */
id|start_bh_atomic
c_func
(paren
)paren
...
r_return
suffix:semicolon
multiline_comment|/* And let IP do all the hard work. */
id|ip_build_xmit
c_func
(paren
id|sk
