...
id|arp_backlog
suffix:semicolon
multiline_comment|/* If we have arpd configured, assume that we will be running arpd and keep&n;   the internal cache small */
macro_line|#ifdef CONFIG_ARPD
...
multiline_comment|/*&n; *&t;Force the expiry of an entry in the internal cache so the memory&n; *&t;can be used for a new request or for loading a query from arpd.&n; *&t;I&squot;m not really sure what the best algorithm should be, so I just&n; *&t;search for the oldest.  NOTE:  make sure the cache is locked before&n; *      jumping into this function!  If someone wants to do something&n; *&t;other than searching the whole cache, by all means do so!&n; */
macro_line|#ifdef CONFIG_ARPD
...
c_func
(paren
l_string|&quot;Looking for something to force expire.&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#endif
r_for
...
c_func
(paren
l_string|&quot;Force expiring %08x&bslash;n&quot;
comma
id|entry-&gt;ip
)paren
suffix:semicolon
...
multiline_comment|/* &n; * Allocate memory for a new entry.  If we are at the maximum limit&n; * of the internal ARP cache, arp_force_expire() an entry.  NOTE:  &n; * arp_force_expire() needs the cache to be locked, so therefore&n; * arp_add_entry() should only be called with the cache locked too!&n; */
DECL|function|arp_add_entry
r_static
r_struct
id|arp_table
op_star
...
multiline_comment|/*&n; * Lookup ARP entry by (addr, dev) pair in the arpd.&n; */
...
suffix:semicolon
multiline_comment|/* don&squot;t worry about rv != 0 too much, it&squot;s probably&n;&t;   because arpd isn&squot;t running or an entry couldn&squot;t&n;&t;   be found */
r_if
c_cond
(paren
id|rv
...
c_func
(paren
l_string|&quot;Inserting arpd entry %08x&bslash;n in local cache.&quot;
comma
id|entry-&gt;ip
)paren
suffix:semicolon
